...
void FHktVMInterpreter::Op_SpawnEntity(FHktVMRuntime& Runtime, int32 TagIndex)
{
    if (WorldState)
    {
        ...
        if (Runtime.Context)
        {
            ...
            Runtime.Context->WriteEntity(NewEntity, PropertyId::CollisionRadius, 30);
            Runtime.Context->WriteEntity(NewEntity, PropertyId::CollisionHalfHeight, 45);  // 캡슐 반높이 90cm (전체 높이 180cm, 표준 휴머노이드)
