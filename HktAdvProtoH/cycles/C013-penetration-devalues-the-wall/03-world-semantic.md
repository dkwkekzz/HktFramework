...
## BALANCE — 이 Cycle 이 소유하는 수치
    ...
    근거 문서는 관통의 초기값을 지정하지 않는다. 세 가지 제약 아래에서 정했다.
        ...
        (2) **관통을 지니는 쪽은 플레이어여야 한다** — Cycle Goal 의 주어가 플레이어다.
            관찰자의 몸은 rabbit-swordsman 이다 (RULE-OBSERVER-JOIN-001)
        (2') 관통을 지니지 않은 쪽도 세계 안에 있어야 한다 —
            그래야 "관통이 없으면 어떤가" 를 견줄 수 있다 (wanderer)
    ...
    ── 종류별 관통 ───────────────────────────────────────────────────
        rabbit-swordsman   ArmorPenetration 0 · ResistancePenetration 60
            **관찰자(플레이어)의 몸이 이 종류다** (RULE-OBSERVER-JOIN-001).
            이 세계에서 이 종류가 마주하는 벽은 wanderer 의 Resistance 90 이다 —
            AuraAttack 40 을 지니고도 그 벽 때문에 오라를 쓸 수 없었다.
            관통은 그 벽을 깎아 통과하는 것이며, 이것이
            MP-PIERCE-THE-HARD-DEFENSE 가 말하는 처지 그대로다.
        ...
        왜 물리 쪽이 아니라 오라 쪽인가
            아래 세 층(C007 체감 · C010 공식 · C011 막기)의 기준값이 전부 **물리 타격**이다.
            관찰자에게 물리 관통을 주면 그 기준값이 모두 흔들리고, 각 층이 위층 없이도
            서 있다는 증거가 C013 만 설명할 수 있는 숫자로 바뀐다
            (DC-COMBAT-ONE-LAYER-AT-A-TIME — "각 층은 상위 층 없이 완전히 동작한다").
            오라 쪽에 두면 그 증거가 그대로 남고, 동시에 이 세계에서 실제로 두꺼운
            방어(Resistance 90)를 겨냥하게 된다.

    ── 실제로 갈리는 값 ──────────────────────────────────────────────

        같은 상대 · 같은 스킬 · 같은 공격 능력(AuraAttack 40, raw 26).
        다른 것은 치는 자의 관통뿐이다 — 대상은 wanderer (Resistance 90).

            관통 0 으로 친다     걷힘 없음      Resistance 90 → 90      26 × 100/190 = 14
            관통 60 으로 친다    62.5% 남는다   Resistance 90 → 56.25   26 × 100/156.25 = 17

            **14 대 17.** 공격력도 스킬도 상대도 같다. 관통만 다르다.
            그리고 걷힌 방어가 90 → 56.25 로 관찰에 그대로 실린다.

        같은 관통이 상대에 따라 다르게 값한다 (관찰자가 aura-strike 로 친다, raw 26).

            Resistance    0  → 0        걷힘 0        26   관통 없을 때와 **같다**
            ...
            Resistance 0 인 상대에게 관통은 문자 그대로 아무 일도 하지 않는다.
            두꺼울수록 **걷어내는 방어량**이 커진다 — 0 / 7.5 / 33.75 / 112.5.
            이 층이 약속하는 단조는 걷어내는 방어량이며 피해 증가폭이 아니다.
            피해 증가폭(0/+1/+3/+2)은 감쇄식이 완만해지는 구간에서 다시 작아진다 —
            그것은 C010 의 체감식이 이미 지닌 성질이지 관통의 성질이 아니다.
            Resistance 0 · 300 은 디버그 명령으로 만든다 (01 SCOPE NOTE).

        물리 쪽은 한 값도 움직이지 않는다 — 관찰자의 ArmorPenetration 이 0 이다.
            attack        raw 26 → Armor 30 그대로 → 20        C010 · C012 와 같다
            heavy-attack  raw 72 → Armor 30 그대로 → 55        C010 · C012 와 같다
            → rabbit-swordsman (Armor 50)                17    C012 와 같다

    ── C012 대비 달라지는 값의 전부 ──────────────────────────────────
        관찰자(rabbit-swordsman)가 내는 **오라** 피해뿐이다.
            → wanderer            14 → 17
            → rabbit-swordsman    22 → 23
        그 외 모든 조합은 관통이 0 이므로 C012 의 숫자가 그대로다
        (모든 물리 타격 · wanderer 의 모든 타격).
