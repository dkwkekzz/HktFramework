...
## WORLD STATE
    ...
    기본 스킬(본래 20)을 받을 때:
        ...
        > R1 정정 (World Implementation 중 실측으로 발견) — 아래 대수는 처음 쓸 때 틀렸다.
        > 상수는 그대로이고 틀린 것은 이 설명뿐이며, 실제 값은 world/tests/guard.spec.ts
        > "막기만 해서는 버틸 수 없다" 가 실행으로 보인다.
        >     틀렸던 문장  시작 기력 30 으로 3대를 막고 4대째에 무너진다
        >     실제         시작 기력 30 으로 2대를 막고 3대째에 무너진다
        >                  (30 → 19.8 → 9.6, 세 번째는 10.2 가 없어 무너짐)

        시작 기력 30 으로는 기본 스킬 2대를 막고 3대째에 무너진다.
        가득 찬 기력 100 으로는 9대를 막고 10대째에 무너진다 (100 - 9×10.2 = 8.2 < 10.2).
        막지 않으면 같은 3대에 Hp 45 가 나가지만 막으면 Hp 4.5 + 기력 20.4 다 —
        막는 것이 분명히 이득이되, 기력이 마르면 그 이득이 끊기고 그때부터는 그냥 얻어맞는다.
        기력을 되찾는 길은 여전히 때리는 것뿐이고(C007) 막는 동안은 때릴 수 없으므로,
        막기만 하는 선택은 반드시 끝난다.

        이 회전이 너무 빠른지(2대는 짧다)는 Human Play 확인의 판단 대상이다 —
        고치는 자리는 GUARD_CP_PER_DAMAGE 하나이며 다른 의미는 건드리지 않는다.
