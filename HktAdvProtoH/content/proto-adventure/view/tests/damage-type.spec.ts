...
describe('strikeEvents.breakdown — 무엇으로 쳤고 무엇을 읽었는가', () => {
  ...
  it('관찰을 켜면 방식과 고른 두 능력이 이름과 함께 나온다', () => {
    ...
    expect(strike('npc-1', true)?.detail).toBe(
      ...
      // C015 — 관찰을 켜면 안 터진 타격에도 그 사실과 가능성이 앞에 붙는다.
      // 가능성이 0 보다 크므로 "터질 리 없다" 가 아니라 이번에 운이 없었던 것이다
      '치명타 없음 (25%) · 오라 · 6+20=26 (오라 공격 40) ×53%(오라 방어 90 · 관통 0 → 90) = 14',
    );
  });
...
describe('interactions.skillAura — 고를 수 있는 표면이 열린다', () => {
  ...
  it('기본 스킬과 나란한 키에 걸린다 — 둘은 나란한 선택이다', () => {
    ...
    // C025 CHANGED — R 에서 H 로 옮겼다. `KeyR` 은 엔진의 시점 조작이라 눌려도 삼켜졌고,
