...
describe('hud.self — 자기 자원·능력치·배율은 늘 눈앞에 있다', () => {
  ...
  it('전투 능력치와 템포 능력치가 줄로 실린다', () => {
    ...
    expect(lines[1]).toContain('물리 공격 40');
    expect(lines[1]).toContain('물리 방어 50');
    expect(lines[1]).toContain('받는 피해 67%'); // 100/150 — 체감을 백분율로 읽는다
    expect(lines[2]).toContain('오라 공격 40');
    // C013 — 내 관통이 능력치 두 줄 바로 뒤에 온다. 0 인 쪽도 쓴다
    expect(lines[3]).toBe('관통 물리 0 · 오라 0');
    expect(lines[4]).toContain('내 약점');
    // C015 — 내가 얼마나 터뜨리는 몸인가. 방어 읽기가 끝난 뒤에 온다.
    // 이 fixture 의 몸은 터뜨리지 못한다 — 0% 라고만 쓰면 옆의 배율과 헷갈리므로
    // "터뜨리지 못함" 이라고 쓴다
    expect(lines[5]).toBe('치명타 터뜨리지 못함');
    expect(lines[6]).toContain('이동 속도 6');
    expect(lines[6]).toContain('달리기 ×1.8');
    expect(lines[7]).toBe('공격 속도 ×1');
  });
