...
| ENGINE | OPEN | **없음.** 조립의 눌림 배분이 V-021 로 섰다 — 같은 코드에 바인딩을 둘 이상 둘 수 있고, 바인딩이 **사양**하면 그 눌림이 세계로 흐른다 (`KeyBinding.invoke -> boolean | void` · `dispatchKey`). 표면 손짓도 표면 이름 없는 자리 하나로 갔다 (`view/surface-dispatch.ts` · `content/active-view.ts`). 기록에 남은 기반 부채 둘은 규칙이 미룬 것이다 (rule of two · 요구가 설 때). 새 일은 다른 레인이 기반을 부를 때 선다 | 없음 |
| PROCESS | OPEN | **Q67 이 열렸다** — 병렬 레인이 공유하는 두 자리가 어긋난다 (Q 번호공간이 레인 소유가 아니다 · `frontier/README.md` 트랙 표가 트랙 파일과 어긋난다). 이번 합류에서 둘 다 실제로 났고 손으로 고쳤다. 그 밖에는 없음 — 보고 형식이 guides/works.md 의 "보고" 절에 섰다 | 없음 · **Q68 이 열렸다** — 조립(`app/` · `content/active*.ts`)이 어느 레인의 것인지 레인 표에 없다 (V-021 REPORT ①: 이번에 실제로 그 자리를 고쳐야 했고 규칙이 없어 기반 트랙으로 보고 진행했다) |
