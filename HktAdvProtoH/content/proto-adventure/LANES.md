...
| WORLD·GROWTH | HUMAN | `C-GROWTH-001-what-you-did-makes-you` — Stage 1~8 닫힘. 1521 tests · 실제 브라우저 왕복 실측. **실측이 판정 하나를 내려 잡았다** — 치명이 터지면 덜 때리고 넘어뜨려 덜 쌓이므로 한 마리의 벌이가 18~21 로 흔들린다 (DC-COMBAT-PLAYER-CAUSALITY SATISFIED → PARTIAL · Master Gap ③ 에 선택지 셋). **STATUS 는 IN PROGRESS** — Gate 14. 남은 후보 둘 | Human Play (`npm run dev` → 왼쪽 위로 걸어가 `F`) |
