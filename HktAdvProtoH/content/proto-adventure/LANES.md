...
| WORLD·GROWTH | HUMAN | `C-GROWTH-001-what-you-did-makes-you` — Stage 1~8 닫힘. 1520 tests · 실제 브라우저에서 키→요청→세계→관찰 왕복 실측(쌓임 0→19). **STATUS 는 IN PROGRESS** — 사람이 손으로 넘어뜨려 본 뒤에만 COMPLETE 다 (Gate 14). 남은 후보 둘(스킬이 새 수를 배운다 · 몸이 자기 방식을 기억한다) | Human Play (`npm run dev` → 왼쪽 위로 걸어가 `F`) |
