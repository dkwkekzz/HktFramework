...
| ENGINE | OPEN | 하나 — 지면 구역 장치 `SceneGroundZone` ([design/Design-Terrain-Visualization.md](../../design/Design-Terrain-Visualization.md)) — C-TERRAIN-001 의 04 확정 뒤. **자리 잡기도 섰다** — 곁말이 가장자리에서 반대쪽으로 접히고, 이어진 같은 이름의 구획이 나란히 서며(`section.group` · `groupMin`), 표면이 청한 너비를 받는다(`surface.width`). BACKLOG 의 `tip-flip-at-the-edge` · `workspace-two-columns` · `escape-leaves-the-field` · `skill-focus-order` 는 기반 쪽이 전부 풀렸다 (VIEW 가 잡는다) | 없음 |
