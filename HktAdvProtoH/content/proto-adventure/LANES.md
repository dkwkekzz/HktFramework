...
| VIEW | OPEN | 다음은 `skill-slot-icon` (아주 작음) — 의존이 빈 것은 그것과 `allocation-armed-shows` · `allocation-list-crowds-the-top` · `growth-*` 둘이다. **백로그의 의존을 실제와 맞췄다** (V-018 세션이 코드로 쟀다): `drag-and-drop` · `responsive-workspace` · `contrast-and-zoom` · `reduced-motion` · `skill-bar-responsive` 는 **기반 동반**이고(끌기 손잡이 0건 · 팩이 화면 폭을 모른다 · 색과 배치가 `index.html` 에 있다), `skill-activation-progress` 는 **Frontier 재료**다(어느 기술의 구간인지를 말하는 값이 계약에 없다). 닫힌 넷: V-015 · V-016 · V-017 · V-018 | 없음 |
...
| ENGINE | OPEN | **없음 — 배차판이 부른 셋이 전부 닫혔다.** 지면 구역 장치(`SceneGroundZone`)는 C-TERRAIN-001 이 합류시켰다 (`npm run terrain:shot` 재현 · 판정 5/5). 자판이 표면 안을 다니는 길과 자리 잡기는 이 레인이 세웠다 — 겪는 사람이 옮긴 초점도 이제 되돌아온다(`onFocusCell` · `onFocusRow`). 기록에 남은 기반 부채 둘([design/Design-System-Content-Separation.md](../../design/Design-System-Content-Separation.md) 남은 부채)은 **규칙이 미룬 것**이다 — `SceneSelf`·`SceneCommandSurface` 승격은 rule of two, 팩별 지형 표현은 요구가 설 때. 새 일은 다른 레인이 기반을 부를 때 선다 | 없음 |
