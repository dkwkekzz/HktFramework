...
| VIEW | OPEN | 다음은 `exchange-second-step-shows` (아주 작음 — 코드는 섰고 **검증만 남았다**) 또는 `allocation-list-crowds-the-top`. 의존이 빈 것은 그 둘과 `growth-*` 둘이다. 나머지는 **기반·조립 동반**(`drag-and-drop` · 폭으로 갈리는 셋 · 색 · `armed-escape-backs-out`) 이거나 **Frontier 재료**(`skill-activation-progress`)다. 닫힌 여섯: V-015 ~ V-020 | 없음 |
