...
| 레인 | 상태 | 지금 | 기다리는 것 |
|---|---|---|---|
| FEEDBACK | OPEN | 미처리 **3건** — `C-COMBAT-003`(Overlay 둘 · Constraint 일곱 · 후보 둘 · Gap 셋) · `C-COMBAT-004`(MC-MARK MISSING→IMPLEMENTED · Constraint 일곱 · 후보 둘 · Gap 셋) · `C-TERRAIN-003`(MW-SHAPED-LANDFORM · MW-WORLD-PRESSURE ABSENT→PARTIAL 제안 · Gap 셋). `npm run feedback:gate` | 없음 — 다만 **병합 뒤 최신 main 위에서** 돈다 (`overlay.md` 는 공유 파일이다) |
