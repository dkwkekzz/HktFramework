...
| WORLD·COMBAT | RUNNING | `C-COMBAT-003` — `FR-THE-WORLD-DECIDES-WHAT-IS-POSSIBLE`(세계가 무엇이 가능한지를 정한다). 선택은 Human 이 Agent 추천으로 위임했다. `C-COMBAT-001` 은 Feedback 반영이 끝났으나 여전히 Human Play 대기 (`npm run dev` → `U` → `2`) | 없음 |
