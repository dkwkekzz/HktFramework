...
| VIEW | OPEN | BACKLOG 다음 항목(`drag-and-drop`)부터 — 의존이 빈 것이 넷이다 (`drag-and-drop` · `responsive-workspace` · `contrast-and-zoom` · `skill-*`). 기반 동반이 필요했던 셋은 닫혔다 — `escape-leaves-the-field`(V-015) · `tip-flip-at-the-edge`(V-016) · `workspace-two-columns`(V-017). `responsive-workspace` 는 이제 의존이 비었고 **접히는 자리(창 폭 1013~1100)를 그 항목이 정한다** (V-017 REPORT ①) | 없음 |
