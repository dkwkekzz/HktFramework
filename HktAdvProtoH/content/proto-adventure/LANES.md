...
| VIEW | OPEN | 다음은 `allocation-armed-shows` — 의존이 빈 것은 그것과 `allocation-list-crowds-the-top` · `growth-cause-in-the-breakdown` · `growth-step-is-a-moment` 넷이다. 나머지는 **기반 동반**이거나(끌기 손잡이 0건 · 팩이 화면 폭을 모른다 · 색과 배치가 `index.html` 에 있다) **Frontier 재료**다(`skill-activation-progress` — 어느 기술의 구간인지를 말하는 값이 계약에 없다). 닫힌 다섯: V-015 · V-016 · V-017 · V-018 · V-019 | 없음 |
