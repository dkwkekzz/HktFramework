...
### self-panel-outgrew-its-corner — 내 패널이 자기 모서리보다 커졌다
    출처   ENGINE 레인 (자리판 · hud-layout) 이 재어 넘긴 것
    목표   self 패널과 위쪽 띠가 화면 한 모서리에 담기는 분량으로 돌아온다.
           지금 재어 보면 self 패널이 **서른 줄**(1280×800 에서 708px)이고 위 띠가
           **열두 칸**이다. 자리판이 겹침은 막았으므로 이제 이것은 겹침이 아니라
           **잘림**으로 나타난다 — 1280×800 에서 self 패널의 뒷줄이 잘린다
    크기   중간
    의존   없음 — 이미 실려 온 것을 무엇으로 접고 무엇을 감출지의 문제다
           (접는 절 · 관찰 토글로 여는 상세 · 덜 급한 줄을 표면으로 옮기기)
    상태   PROPOSED
    주     자리와 겹침은 이제 기반이 지닌다 (`engine/view-kernel/hud/hud-layout.ts` ·
           `npm run hud:shot` 이 폭 셋에서 겹침 0 을 잰다). 팩이 정하는 것은
           **무엇을 얼마나 낼 것인가**뿐이다
