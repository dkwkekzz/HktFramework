...
### armed-escape-backs-out — 걸어 둔 것을 `Esc` 로 놓는다
    출처   V-020 REPORT ①
    목표   `U`(또는 B·N·M·,)로 걸어 둔 뒤 `Esc` 를 누르면 걸린 줄이 사라지고,
           같은 `Esc` 가 **지목 해제도 그대로 한다**
    크기   작음
    의존   **기반·조립 동반** — 팩이 `Escape` 에 바인딩을 두면 지목 해제가 죽는다.
           조립이 `KEY_BINDINGS.find` 로 첫 바인딩을 찾으면 거기서 멈추고 세계의
           interaction(`clear-target`)까지 내려가지 않으며, 바인딩이 **사양할 방법이
           없다** (`KeyBinding.invoke` 가 아무것도 돌려주지 않는다 — 그 형은 기반의 것).
           V-018 REPORT ② 가 올린 같은 자리다
    상태   PROPOSED
    주     Frontier 재료가 아니다 — 걸린 것은 화면의 조작 상태이고 세계는 그것을 모른다
