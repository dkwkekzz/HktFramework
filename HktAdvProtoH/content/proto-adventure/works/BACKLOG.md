...
### view-search-by-name — 이름으로 찾는다
    출처   UX 문서 §6 · V-008 REPORT ①
    목표   도구 띠에서 이름을 쳐 넣으면 그 이름이 든 것만 남고, 지우면 다시 전부 선다
    크기   작음 (화면 쪽) — **ENGINE 동반 없이는 시작할 수 없다**
    의존   **기반** — 겹침 표면에 **글자를 받는 자리**가 없다 (칸과 줄뿐이다).
           팩이 자판으로 글자를 모을 길도 없다 — 조립은 등록된 키 코드만 넘긴다.
           명령 표면은 이미 글자를 받으므로(`hud/command-console.ts` 의 `<input>`)
           그 능력을 범용 표면으로 옮기는 것이 그 자리다
    상태   BLOCKED (ENGINE)
    주     계약이 주는 **검색 태그**는 여기 없다 — 계약에 없으므로 표시 이름만이다.
           거르기와 보기 정렬은 V-008 로 닫혔다
