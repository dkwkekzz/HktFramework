...
### discard-confirm — 되돌릴 수 없는 것을 실수로 잃지 않는다
    출처   UX 문서 §7 · §11 VUX-IE-05
    목표   덜어내기를 실행하면 무엇이 얼마나 사라지는지 먼저 뜨고, 취소하면 세계로
           아무 요청도 나가지 않는다
    크기   작음
    의존   없음
    상태   PROPOSED
    주     수량을 고르는 부분은 여기 없다 — 세계가 부분 수량 덜어내기를 모른다.
           지금 덜어내기는 그 종류를 통째로 지운다
