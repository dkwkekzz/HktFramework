...
### equipment-slot-key-hint — 자리도 자기 번호를 지닌다
    출처   V-013 REPORT ①
    목표   장비 자리 칸과 그 상세가 **하나의 번호**로 불린다 — 푸는 지름길(`M` → 번호)이
           세는 그 번호와 화면에 적힌 번호가 같다
    크기   아주 작음
    의존   없음
    상태   PROPOSED
    주     지금 화면은 자리 여섯 **전부**의 차례를 쓰고(`고른 것 — 자리 3`), 지름길은
           **걸린 자리만** 센다. E1 이 비고 E2 가 차면 갈라진다. 둘 중 무엇을 화면에
           둘지 정하는 일이 이 항목의 본체다
