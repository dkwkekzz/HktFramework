...
## 검사 6종
    ...
    [ ] Human Play 확인 — 대기 중. STATUS 는 그전에 COMPLETE 로 바꾸지 않는다
...
거기서 나온 요청을 다시 세계에 넣는다.
    ...
    클라이언트 빌드    `npx vite build` — 성공 (1.62s)

    **Human Play 는 아직이다.** 위 실측은 조립 루트가 하는 일을 코드로 재현한 것이지
    사람이 브라우저에서 X 를 누른 것이 아니다. 그 확인 뒤에 STATUS 를 바꾼다.
...
## STATUS
    ...
    기계 검증은 전부 끝났다 (911 tests · tsc 0 · boundary 0 · catalog 정합 · 빌드 성공).
    사람이 브라우저에서 다음을 확인하면 COMPLETE 로 바꾼다.

        1. 광맥 앞에서 네 번 캔다 → 가방이 `3/3 — 가득 찼다` 가 된다
        2. 다섯 번째를 시도한다 → 채집이 '가방에 넣을 자리가 없다' 로 막히고
           광맥의 잔량은 그대로다
        3. X 를 누른다 → 돌 자리 하나가 사라지고 `2/3` 이 된다
        4. 다시 캔다 → 받아진다
        5. 곡괭이 줄에 '다시 캘 수 없다' 가 붙어 있고, 그 자리는 X 로 사라지지 않는다
