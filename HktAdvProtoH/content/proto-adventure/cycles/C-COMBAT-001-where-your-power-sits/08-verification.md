...
### HUMAN PLAY 보조 — 사람이 볼 셋을 실제 크기에서 미리 잰다

    앞의 PLAYABLE 이 900×600 에서 **왕복이 도는가**를 닫았다면, 여기서는 사람이
    실제로 쓰는 창(1440×900)에서 **눈에 어떻게 서는가**를 잰다. 이것으로 Gate 14 항이
    닫히지 않는다 — 손의 느낌은 사람만 답할 수 있다. 여기 적는 것은 그 사람이
    무엇을 보게 될지를 미리 재어 둔 것이다.

    조건   `npx vite` · `/opt/pw-browsers/chromium-1194` · 1440×900 · 페이지 오류 0
           세계 시계가 실제로 흘렀다 (`1s → 4s → 7s → 10s → 13s`, 마지막 잰 값 85s)
           **교전 상태에서 쟀다** — 사람 몸이 `200 / 200` → `135 / 200` 으로 맞은 뒤다

    왕복   `U` → `2` 가 교전 중에 그대로 돌았다

               배분 줄     `배분 균형 (몸 2 · 능력 2 · 인지 2)`
                        →  `배분 강화 (몸 4 · 능력 1 · 인지 1)`
               물리        `공격 40 · 방어 50 (받는 피해 67%)` → `56 · 70 (59%)`
               오라        `공격 40 · 방어 20 (받는 피해 83%)` → `28 · 32 (76%)`
               기력        `30 / 100` → `15 / 100`
               기술 값     기본 `26` → `34` · 오라 `26` → `20`
               기술 가부   고급 `✓ G` → **`✗ 기력이 모자란다`**
               목록 뒤집힘  강화가 `지금 여기` 로, 균형이 `기력 15 · U → 1` 로

           **배분이 무엇을 할 수 있는가까지 건드린 자리가 여기서 처음 눈에 잡혔다** —
           고급 스킬이 기력 부족으로 닫힌 것은 값의 변화가 아니라 가부의 변화다.
           다만 이것은 배분이 목록을 여닫은 것이 아니라 **같은 주머니를 다툰 결과**다
           (DC-COMBAT-SHARED-BUDGET). MC-AURA-ALLOCATION 의 남은 결손은 그대로다.

#### ① 두 걸음 — **첫 걸음이 화면에 남지 않는다** (측정)

    `U` 를 누른 앞과 뒤의 HUD 글자를 통째로 떠서 견주었다. **한 자도 다르지 않다.**
    누른 뒤에도 화면은 "지금 숫자를 기다리는 중" 이라고 말하지 않는다.

    코드로도 같다 — `armed` 는 `view/bindings.ts` 안에만 살고, 그것을 읽는
    `armedAction()` 을 부르는 자리가 팩 어디에도 없다 (검색 0건).
    **이미 적혀 있던 것이다**: C026 이 소지품에 표면을 세우며 같은 사유를 남겼다
    (`bindings.ts` — "무엇을 고르는 중인지가 화면에 남지 않으므로 사람이 기억해야 했다").
    소지품은 `I` 표면으로 그 기억을 화면에 옮겼고, **배분에는 그 자리가 없다.**

    이것은 세계의 결손이 아니라 화면의 몫이다 — 세계로 나가는 요청은 여전히 하나다.
    `works/BACKLOG.md` 의 `allocation-armed-shows` 로 이관했다.

#### ② 목록 넷 — 세로가 아니라 **위 패널을 옆으로 채운다** (측정)

    "세로 목록 넷" 이라 적었던 것은 실제 배치와 다르다. 넷은 위 패널 안에서
    옆으로 흐르며, 폭이 모자라 두 줄을 채운다.

        y  23   균형   320px      강화   381px
        y  54   발현   381px      사냥꾼 398px
        (넷의 폭 합 1480px · 패널 폭 1400px)

    그 결과 **위 패널이 세 줄로 섰고**(y 23 · 54 · 85), 나머지 여덟(고른 대상 ·
    걸린 것 · 자리 · 소지품 둘 · 행동 · 세계 시간 · 함께)이 배분 뒤로 밀려 났다.
    글자 크기는 17px 로 본문(11px)보다 크다 — 눈에 안 걸리는 것이 아니라
    **위 패널의 대부분을 배분이 가져갔다**는 쪽이 실제다.

    이것도 화면의 몫이다 — 세계가 넷을 보내는 것은 04 가 정한 그대로다.
    `works/BACKLOG.md` 의 `allocation-list-crowds-the-top` 으로 이관했다.

#### ③ `[몸]` 표시 — 뜬다. 다만 **상대 쪽은 재현하지 못했다**

    떴다   `[몸]Player 1` — 이름 바로 앞, 사이 공백 없이. 12.5px.
           내 몸이 강화로 옮겨 가자 그 자리에서 바로 붙었다 (`4-강화적용` 화면)

    못 쟀다 **상대의 `[몸]` 은 이 런에서 일어나지 않았다.** Wanderer 1 이 끝까지
           `120 / 120` 이었다 — 다가가며 `F` 를 마흔 번 쳤으나 사거리에 닿지 않았고,
           절반 아래로 내려가야 몸에 모는 자율 존재의 판단(06 · 08 REGRESSION)이
           일어나지 않았다. 사람이 맞으면 그 순간에 확인된다

    남은 물음 하나는 재는 것이 아니라 **보는 것**이다 — 이름표에 표시가 최대 셋까지
    앞에 붙는다 (태도 · 선딜 · 배분 → `[적대] 준비! [몸]Wanderer 1 ?`).
    셋이 한꺼번에 붙은 이름이 교전 중에 읽히는지는 그 장면을 본 사람이 답한다.
...
## STATUS
    ...
    그 셋을 실제 크기(1440×900)에서 미리 재어 두었다 (위 HUMAN PLAY 보조).
    재고 나니 물음의 모양이 달라졌다 — 사람이 답할 것은 이제 셋이 아니라 둘이다.

        ① 두 걸음    **잴 것이 남지 않았다.** 첫 걸음이 화면에 한 자도 남지 않는다는
                    것이 측정으로 확정됐다. 손의 문제가 아니라 화면의 결손이며
                    BACKLOG 로 이관했다
        ② 목록 넷    **세로가 아니었다** — 위 패널을 옆으로 채워 패널을 세 줄로 만든다.
                    이것도 이관했다. 사람이 정할 것은 "그대로 두어도 되는가" 하나다
        ③ `[몸]`     내 몸에서는 떴다 (`[몸]Player 1`). **상대 쪽은 재현하지 못했다** —
                    Wanderer 가 절반 아래로 내려가지 않았다. 이것만이 아직 손에 남는다.
                    함께 볼 것: 표시가 셋까지 앞에 붙은 이름(`[적대] 준비! [몸]Wanderer 1 ?`)이
                    교전 중에 읽히는가

    재현: `./scripts/run.sh` (또는 `npm run dev`) 후 브라우저에서 `U` → `2`.
    상대의 `[몸]` 을 보려면 Wanderer 를 절반 아래(`60 / 120`)까지 때려야 한다.
