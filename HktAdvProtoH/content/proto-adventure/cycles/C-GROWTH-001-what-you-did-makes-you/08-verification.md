...
[PART] Playable             (실제 브라우저에서 왕복 실측 · 쌓임 0 → 19 까지 눈으로 확인.
                             그 판은 다섯 대에 넘어뜨린 판이라 문턱에 1 이 모자랐다 —
                             **세계가 실제로 그렇다**. 아래 PLAYABLE 4 · WORLD SCENARIO ①')
...
페이지 오류 0 · 왕복 81ms · 보낸 요청 375.
    ...
    4. **문턱을 넘는 순간은 잡히지 않았다 — 그리고 그 까닭이 이 Cycle 의 발견이다.**

       19 에서 멈춘 뒤 280 번을 더 눌러도 움직이지 않았다. 처음에는 각본이 상대를
       놓친 것이라 읽었으나, 세계에서 흔들림을 켜고 다시 재어 보니 답이 나왔다:
       **그 판은 다섯 대에 넘어뜨린 판이었다** (5 + 14 = 19). 상대는 이미 쓰러져
       있었고 그 뒤의 휘두름은 아무것도 닿지 않은 것이다.

       그러므로 이것은 각본의 한계가 아니라 **세계가 실제로 그런 것**이다 —
       치명이 잘 터진 판은 덜 쌓는다 (위 WORLD SCENARIO ①'). 브라우저가 우연히
       그 판을 뽑았고, 그 덕에 셈으로는 보이지 않던 폭이 드러났다.

       Cycle Goal 은 이것으로 흔들리지 않는다 — Goal 이 적은 길은 "쓰러뜨리고
       **광맥을 캐면**" 이고 그 길은 열 판 모두 넘는다. 흔들리는 것은 03 의
       BALANCE ② 가 셈한 "정확히 20" 하나이며, 그것을 위에 고쳐 적었다.
