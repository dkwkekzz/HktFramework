...
## SPEC CONSUMED
    ...
    inventory[].actions[*]               view/inventory-presentation.ts
                                         `inventoryDetailLines()` — **self 패널의 줄**로 선다.
                                         `available` 이면 손가락 자리를, 아니면 사유의
                                         짧은 표기를 옮긴다.
                                         **지닌 모든 항목에 붙는다** — 세계가 그렇게 보낸다
                                         (자리를 옮긴 경위는 아래 REVISION)
...
## FIXTURE TESTS
    ...
    view/tests/inventory-room.spec.ts    14 tests — **전부 통과** (World 미기동)
        ...
        VIEW CLOSURE 5·6·7  덜어내기가 손가락 자리와 함께 뜬다 · `no-way-back` 이
                        문구로 뜬다 · **지닌 모든 항목**에 붙는다 ·
                        **띠에는 항목마다 한 칸만 남는다** (곁줄이 띠를 밀어내지 않는다)
    ...
    회귀                 `npm test` → **55 files · 943 tests 전부 통과**
...
## REVISION — 띠를 가르다 (첫 판을 실제로 띄워 보고 고쳤다)

    첫 판은 항목마다 곁줄 둘(`쓰기` · `덜어내기`)을 **가로 띠에 함께** 세웠다.
    빌드해서 띄워 보니 종류 둘에 이미 여덟 칸이 되어 **오른쪽이 화면 밖으로 잘렸다**
    (`행동: 대…` 에서 끊겼다). 원인은 둘이었다.

        ① 띠가 감싸지 않는다      `.hud-panel` 이 데스크톱에서 `flex` 무개행 · 무 max-width.
                                모바일 규칙에는 이미 `flex-wrap: wrap` 이 있었다 —
                                넘친 칸이 조용히 사라지고 있었던 것이다
        ② 띠가 항목 수의 세 배로 자란다   사유는 **문장**이고 띠는 **가로로만** 자란다.
                                종류가 열이면 서른 칸이 된다

    고친 것 넷. **세계가 보낸 것 중 사라진 것은 하나도 없다 — 자리만 옮겼다.**

        띠 (`scene.hud`)         한눈에 읽는 것만: 자리 · 항목마다 **수량 한 칸**.
                                곁줄을 만들지 않는다. 수량이 띠에 남는 이유는 자주
                                바뀌고 한눈에 읽히기 때문이며, 캔 것이 반짝이는 자리도
                                여기다 (celebrateGain 은 counter 위젯에만 붙는다)

        self 패널 (`SceneSelf.lines`)   읽어야 아는 것: `1. 돌 ×9 · 쓰기 ✗ 이 대상엔 안 됨
                                · 덜어내기 ✓ B → 1`. **세로로 자라므로** 종류가 늘어도
                                밀려나지 않는다. 조립(`resolve.ts`)이 둘을 잇는다 —
                                소지품 표현이 전투 표현을 알 필요가 없다

        문구 두 벌 (`code-text.ts`)     `shortCodeText()` 를 더했다.
                                긴 문장은 **지금 하려는 행동 하나**에 (채집 프롬프트:
                                `자리가 없다 — 무엇을 덜어내야 한다`), 짧은 표기는
                                **목록**에 (`자리 없음`). 목록에서 항목 수만큼 반복되는
                                문장은 목록이 아니라 문단이 되고, 문단은 아무도 안 읽는다.
                                표에 없으면 긴 문장이 그대로 나온다 — 짧은 말을 **짓지 않는다**

        `.hud-panel` (index.html)  `flex-wrap: wrap` · `max-width: calc(100% - 230px)`.
                                오른쪽 위 조작 안내 앞에서 멈춘다. 이제 넘치면 잘리는
                                대신 다음 줄로 간다

    `engine/` 은 한 줄도 건드리지 않았다. `SceneSelf.lines` 가 이미 팩이 채우는 자리였다.

    Fixture 정정 — `mining-available` · `deposit-depleted` · `inventory-full` 에 몸의 값
    (`self.hp` 계열)이 없었다. 실제 관찰에서는 **소지품이 있으면 반드시 몸도 있다**
    (`projectObserverView` 가 몸 없는 관찰자에게는 아무것도 보내지 않는다). fixture 가
    불완전했던 것이므로 채워 넣었다 — 코드에 조건을 더하지 않았다.

    실측 (빌드 산출물 · 실제 세계 · 헤드리스 브라우저 · 가방이 가득 찬 몸)
        띠      `고른 대상: 돌 광맥 · 지금: 캘 수 있다 · 살펴보기: … · 채집: 자리가 없다 —
                무엇을 덜어내야 한다 · 자리: 4 / 4 (가득) · 🪨 1. 돌: 9 · ⛏ 2. 곡괭이: 1 ·
                행동: 대기 · 세계 시간 · 함께` — 세 줄로 감싸지고 **잘리는 곳이 없다**
        패널    `소지품` / `1. 돌 ×9 · 쓰기 ✗ 이 대상엔 안 됨 · 덜어내기 ✓ B → 1`
                / `2. 곡괭이 ×1 · 쓰기 ✗ 자리 없음 · 덜어내기 ✗ 되돌릴 수 없음`
        페이지 오류 0

    남은 것 — 띠에서 가장 긴 것은 이제 **고른 대상 블록**(C017)이다. 네 칸이 전부 문장이고
    그중 둘은 오른쪽 위 조작 안내가 이미 흐리게 답하고 있다. C022 가 만든 것이 아니므로
    건드리지 않았다. 위층의 의미가 아니라 표면의 일이므로 MASTER FEEDBACK 에도 올리지
    않는다 — 다음에 이 자리를 다룰 Cycle 이 가져갈 몫이다.
...
## NOTES
    ...
    4. 첫 판을 띄워 보고 고쳤다 — 위 REVISION 이 그 경위다.
       **테스트가 전부 통과해도 화면은 잘려 있었다.** Fixture 검증은 "무엇이 담기는가" 를
       보지 "그것이 읽히는가" 를 보지 않는다. 눈으로 본 것이 이 Cycle 에서 가장 값진
       검증이었다.

    5. 남은 거친 자리 하나 — 열림에 눈에 보이는 표시가 없다
