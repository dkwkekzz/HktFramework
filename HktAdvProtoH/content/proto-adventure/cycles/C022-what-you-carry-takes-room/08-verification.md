...
## PLAYABLE — 실제 세계 프로세스 + 실제 이어짐
    ...
    브라우저 클라이언트 (빌드된 dist 를 세계 프로세스가 서빙 · 헤드리스 Chromium)
        HUD 실측  `고른 대상: 없음 | 자리: 1 / 4 | ⛏ 1. 곡괭이: 1 |
                   쓰기: 먼저 대상을 고르자 | 덜어내기: 이걸 놓으면 되돌릴 수 없다 |
                   행동: 대기 | 세계 시간: 103s | 👥 함께: 1명`
        입력 경로  `B` → `1` 을 눌렀다 → 세계가 거절 → 곡괭이 1 그대로 · 페이지 오류 0
        화면       스크린샷으로 확인 (HUD 상단에 `자리: 1 / 4`)
...
## REGRESSION
    ...
    전체        `npm test` → **55 files · 942 tests 전부 통과** (boundary:check 포함)
