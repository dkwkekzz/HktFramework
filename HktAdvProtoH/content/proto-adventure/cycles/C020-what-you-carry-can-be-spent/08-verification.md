...
## PLAYABLE — 실측
    ...
    첫 실측에서 아홉 번 던져 아홉 번 다 끊겼다. 원인을 쟀다.
        ...
        **C019(행동 구간) 병합 뒤 다시 쟀다** — 사거리 안 프레임 194 → 214(23.8%),
        조건 충족은 89(9.9%) 그대로. 60초 시도/성립 16회/9회 → **12회/9회**
        (상대 생명 120 → 90). 세계 프로세스 실측도 그대로다 — 타격 기록 `["stone", 3]`.
...
## REGRESSION — 실측
    ...
    막기 · 살펴봄 · 지목 · 명령 · 시점 · 충돌
        전부 통과. 전체 **914 passed (53 files)** — C019 병합 뒤의 값이다 · `boundary:check` 경계 위반 0.
...
## C019 병합 — 두 Cycle 이 만난 자리

    이 Cycle 을 닫기 직전 C019(행동 구간)가 main 에 들어와 병합했다. 01-cycle.md
    SCOPE NOTE ① 이 그어 둔 선이 그대로 맞았다.

    겹친 실물 파일
        world/tests/critical.spec.ts · penetration.spec.ts   `SWING_BEGIN` 이
            `semantic/collision` 에서 `semantic/combat` 의 `DEFAULT_SWING_BEGIN` 으로
            옮겨졌다 (C019). 이 Cycle 의 `forceOfSkill` 과 같은 import 줄에서 만났다
        protocol/gameview.ts   두 Cycle 이 같은 자리에 새 타입을 더했다 —
            `CancelEventView`(C019) 와 `InventoryItemView`(C020). 둘 다 남긴다
        view/code-text.ts      두 Cycle 이 문구 표에 항목을 더했다. 둘 다 남긴다
        view/tests/fixtures/   서로가 더한 자리가 상대 픽스처에 없었다 —
            `cancels`(C019) 와 `inventory`(C020) 를 모든 픽스처에 채웠다

    **규칙 코드는 한 곳도 충돌하지 않았다.** 아이템 사용은 기술이 아니므로
    C019 의 구간 판정(`skillPhase`)이 `null` 을 돌려주고, 맞으면 지금까지대로 끊긴다
    (`ruleHit` 의 "기술이 아닌 행동" 갈래). C019 가 기술에 준 "이미 나간 칼은 멈추지
    않는다" 는 보호를 던지기는 받지 않는다 — 그것이 이 Cycle 이 잰 창의 전제였고,
    병합 뒤 재측정에서도 같은 결과가 나왔다.

## STATUS

    COMPLETE

    검사 6종 전부 통과. 914개 자동 검증(C019 병합 뒤)과 세계 프로세스 실측이 근거다.
    Human Play 확인 완료 (2026-08-21).
