...
## SEMANTIC DELTA
    ...
    ADDED
        ...
        World.EquipSlotCatalog              적용 자리의 정의 목록 — id · 받는 성격 · 차례
        EquipSlotDefinition.Accepts         이 자리가 받는 성격들
        ItemDefinition.EquipTargets         이 물건이 걸릴 수 있는 성격들. 비면 걸 수 없다
        ItemDefinition.Contributions        걸린 동안 몸의 값에 보태는 것. 없을 수 있다
        ...
        ItemActionView.EquipSlotId          어느 자리에 대한 답인가
        ActionRequest.EquipSlotId           어느 자리에 걸/풀 것인가
...
## WORLD STATE
    ...
    World.EquipSlotCatalog                     World Authority · 고정
        EquipSlotDefinition
            Id           자리의 식별자 — 의미 코드
            Accepts      이 자리가 받는 성격들 (EquipTag)
            Order        관찰에 실리는 차례 — 같은 세계면 같은 순서

    ItemDefinition                             World Authority · 고정 (C020 REUSED)
        EquipTargets     이 물건이 걸릴 수 있는 성격들. **빈 목록이면 걸 수 없다**
...
## WORLD RULE
    ...
    RULE-EQUIP-SLOT-ACCEPTS-001
        Implements     INTENT-FITNESS-COMES-FROM-THE-DEFINITION-001
        Input          EquipSlotDefinition, ItemDefinition
        Preconditions  없음 — 언제나 답할 수 있다
        Transition     없음 (읽기 판정)
        Result         Accepts ∩ EquipTargets ≠ ∅
        Note           **종류 이름이 이 식에 없다.** 정의 둘이 마주 보아 답한다
    ...
    RULE-ITEM-EQUIP-001
        ...
        Input          Actor, ItemKind, SlotId
        Preconditions  1. 그 종류의 정의가 있다                       (unknown-item)
                       2. Items[kind] > 0                            (not-enough)
                       3. 그 자리가 세계에 있다                        (unknown-slot)
                       4. 정의.EquipTargets 가 비어 있지 않다          (not-equippable)
                       5. RULE-EQUIP-SLOT-ACCEPTS-001 이 참이다        (slot-mismatch)
                       6. Equipment[slot] 이 비어 있다                 (slot-occupied)
        Transition     RULE-INVENTORY-REMOVE-001(kind, 1)
                       Equipment[slot] = kind
        Result         Success | Failure(reason)
        Note           둘은 **하나의 성공 단위**다. 앞의 여섯이 전부 참일 때만 둘 다
...
## BALANCE
    ...
    EquipSlots                     2      hand(accepts: held) · body(accepts: worn)
    pickaxe.EquipTargets           held
    pickaxe.Contributions          physicalAttack +12
    stone.EquipTargets             (빈 목록)  → 걸 수 없다
    World.InventoryCapacity        4      **C022 의 값 — 바꾸지 않는다**

    이 값들이 만드는 플레이

        시작        곡괭이 1 → 가방 1 / 4 · 자리 둘 다 비었다.
                    **캘 수 없다** — 가지고만 있기 때문이다
        건다        hand 에 곡괭이 → 가방 0 / 4 · 물리 공격 40 → 52.
                    기본 기술의 raw 26 → 32. 이제 캘 수 있다
        body 에     같은 곡괭이를 body 에 걸려 하면 slot-mismatch —
                    **정의가 답한 거절이다.** 규칙은 곡괭이를 모른다
        돌을        stone 은 not-equippable — 어느 자리에도 들어가지 않는다
    ...
    **다섯 값 어느 것을 바꿔도 규칙 코드는 한 줄도 열리지 않는다.**
    자리를 하나 더 두는 것도, 곡괭이가 보태는 값을 바꾸는 것도, 새 장비를 세우는 것도
    정의가 늘어나는 일이다. 이것이 Stage 8 의 검증 항목이다.
...
## JUDGEMENT — Human 확인이 필요한 판단
    ...
    ① 자리를 둘로 둔다 — 하나는 지금 비어 있을 수밖에 없다
       IE §10 은 자리가 담을 칸보다 **훨씬 좁을 것**을 요구한다 (30 : 6). 이 세계의
       가방은 4 칸이라 그 비를 지키면 자리가 하나뿐이고, 자리가 하나면 "적합성을
       정의가 답한다" 가 관찰될 수 없다 — 어느 자리에 물어도 답이 같기 때문이다.
       그래서 성격이 다른 자리 둘을 둔다. body 는 지금 맞는 물건이 세계에 없어
       비어 있으며, **그 비어 있음이 slot-mismatch 를 관찰 가능하게 만든다.**
       대안은 자리 하나 + 적합성 검증 포기이며, 그것은 이 Cycle 의 INCLUDED 하나를
       비우는 일이라 택하지 않았다.
...
## SEMANTIC CLOSURE
    ...
    "몸에 유한한 적용 자리가 있다"          → World.EquipSlotCatalog · Actor.Equipment
    "자리의 수와 이름은 값이다"             → EquipSlotDefinition (규칙에 이름이 없다)
    "적합성을 정의가 답한다"                → RULE-EQUIP-SLOT-ACCEPTS-001
    ...
    "기여를 정의가 답한다"                  → ItemDefinition.Contributions
    "모든 판정이 유효 값을 읽는다"           → offense/defense/penetration/critical 읽는
                                            자리 전부 CHANGED
    "건다 — 하나의 성공 단위"               → RULE-ITEM-EQUIP-001 Precondition 6 + Transition
    "빈 자리를 요구한다"                    → RULE-ITEM-EQUIP-001 Precondition 6 (slot-occupied)
