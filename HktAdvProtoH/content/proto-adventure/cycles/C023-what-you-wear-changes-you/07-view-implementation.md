...
## INPUT → ACTION REQUEST
    ...
    N → 숫자      소지품 N 번 칸을 건다      { interactionId: 'equip-item', itemKind }
    M → 숫자      걸린 N 번 자리를 푼다      { interactionId: 'unequip-item', equipSlotId }
    B → 숫자      (C022) 덜어내기
    숫자          (C020) 쓰기

    **아래줄 세 칸이 나란하다** — B · N · M. 셋이 같은 형태의 두 걸음이므로
    손가락 자리도 나란한 것이 맞다. 첫 판은 V·U 였고 **실제 브라우저에서 걸렸다** —
    V 가 이미 속성 관찰이었다 (REVISION ①).
...
## VIEW NOTE
    ...
    ③ 번호를 걸린 자리에만 붙인 이유
       ...
       하나 걸렸으면 `M → 1` 하나뿐이고, 자리 수가 여섯에서 셋으로 줄어도 화면은
       고치지 않는다.
    ...
    ④ 손가락 자리가 세 걸음이 된 것 — 조작 계층의 사정이지 게임의 판정이 아니다
       ...
       같은 형태로 N·M 을 더했다. 셋 중 하나만 열리게 한 것은 잘못 눌러 엉뚱한 요청이
...
## REVISION — 실제 브라우저가 되돌린 것

    ① 손가락 자리 V·U → **N·M**

       첫 판은 걸기를 `V`, 풀기를 `U` 에 두었다. World 시험도 Fixture 시험도 전부
       통과했다 — **둘 다 키를 모르기 때문이다.**

       실제 브라우저에서 `V` 를 누르자 곡괭이가 걸리지 않고 **속성 관찰 패널이 열렸다.**
       `V` 는 이미 `app/main.ts` 의 INSPECT_KEY 이고 기반의 터치 패드에도 '속성' 으로
       실려 있다. 화면 안내는 `걸기 ✓ V → 1` 이라고 말하고 있었으므로, **화면이 거짓을
       말하는 상태**였다.

       B(덜어내기) 옆으로 옮겼다 — B · N · M 이 나란하고 셋 다 같은 형태의 두 걸음이다.
       Z·X 는 카메라 회전이, C·V 는 두 관찰 토글이 이미 쓴다.

       **이것이 Stage 8 이 Fixture 로 대신될 수 없는 이유다.** 계약도 결정 Layer 도
       옳았고 어긋난 것은 사람의 손가락과 화면의 말 사이였다.
