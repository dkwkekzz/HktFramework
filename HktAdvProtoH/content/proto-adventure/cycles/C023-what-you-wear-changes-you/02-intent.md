...
## GOAL / POSSIBILITY
    ...
    GOAL-THE-BODY-HAS-PLACES-TO-APPLY       몸에 무엇을 걸어 둘 자리가 있다
        └── POSSIBILITY-THE-PLACES-ARE-FINITE
                                            적용의 자리는 유한하다. 그 수는 세계가 지닌
                                            값이며, 그것이 언젠가 담을 칸보다 좁아질 때
                                            "무엇을 걸어 둘까" 가 선택이 된다
        └── POSSIBILITY-THE-PLACES-ARE-A-VALUE-NOT-A-RULE
                                            자리의 수와 이름은 세계가 지닌 값이다.
                                            판정은 자리의 이름을 조건으로 삼지 않는다
        └── POSSIBILITY-THE-PLACES-ARE-ALIKE
                                            자리들은 서로 같다. 저마다의 성격도 전용
                                            용도도 지니지 않으므로 **어디에 걸리는지는
                                            고를 것이 아니다**
        └── POSSIBILITY-FITNESS-IS-ANSWERED-BY-THE-DEFINITION
                                            무엇이 걸릴 수 있는가는 그 종류의 정의가
                                            답한다. 자리는 아무것도 묻지 않는다 —
                                            전용 자리는 물건이 스스로 선언할 때만 생기는
                                            **예외**이며 기본은 제한 없음이다
...
## INTENT SET
    ...
    INTENT-BODY-HAS-APPLY-PLACES-001 (ADDED)
        ...
        **자리들은 서로 같다.** 저마다 받는 성격도, 전용 용도도, 서로 다른 무엇도
        지니지 않는다. 그러므로 무엇을 걸 때 어느 자리에 걸지는 고를 것이 아니며,
        세계가 빈 자리를 고른다.

        자리의 수와 이름은 **세계가 지닌 값**이다. 판정은 "빈 자리가 있는가" 를 물을 뿐
        그 자리가 무엇이라 불리는지를 조건으로 삼지 않는다. 그러므로 자리를 하나
        늘리거나 이름을 바꾸는 일에 규칙의 문장이 한 줄도 열리지 않는다.
    ...
    INTENT-FITNESS-COMES-FROM-THE-DEFINITION-001 (ADDED)
        ...
        무엇이 걸릴 수 있는가는 **정의가 답한다.** 물건의 정의가 자신이 걸릴 수 있는
        것임을 밝히고, 밝히지 않은 물건은 걸리지 않는다.

        **자리는 아무것도 묻지 않는다.** 어느 자리에 걸리는가에 제한이 붙는 것은
        물건이 스스로 전용 자리를 선언했을 때뿐이며, 그것은 예외다. 기본은 제한 없음
        — 걸릴 수 있는 물건은 어느 자리에나 걸린다.

        규칙은 종류 이름을 묻지 않는다. 그러므로 새 장비가 생기는 일도, 어떤 물건이
        전용 자리를 요구하게 되는 일도 **정의가 늘어나는 일**이며, 거는 규칙도 그것을
        싣는 관찰도 그것을 검증하는 시험도 열리지 않는다.

        걸릴 수 없는 물건이 있는 것도 정상이다 — 돌은 걸 수 있는 물건이 아니다.
        그것은 결손이 아니라 그 물건의 내용이며, **자리 탓이 아니다.**
    ...
    INTENT-APPLY-NEEDS-AN-EMPTY-PLACE-001 (ADDED · 이 Cycle 한정)
        ...
        이 Cycle 에서 거는 일은 **빈 자리**를 요구한다. 걸 수 있는 빈 자리가 하나도
        없으면 사유와 함께 거절되고 아무것도 바뀌지 않는다.

        **이것은 세계의 최종 형태가 아니다.** 이미 찬 자리의 것을 빼면서 거는 것은
        본래 실패가 아니라 교체라는 정상 성공 경로다 (IE §29.1). 그 경로를 여는 것은
        다음 Cycle 이며 (`FR-ONE-SLOT-ONE-ITEM`), 그때 이 Intent 는 CHANGED 로 대체된다.
...
## DESIGN TRACE
    ...
    INTENT-BODY-HAS-APPLY-PLACES-001
        ...
        Source Possibility  POSSIBILITY-THE-PLACES-ARE-FINITE ·
                            POSSIBILITY-THE-PLACES-ARE-A-VALUE-NOT-A-RULE ·
                            POSSIBILITY-THE-PLACES-ARE-ALIKE
