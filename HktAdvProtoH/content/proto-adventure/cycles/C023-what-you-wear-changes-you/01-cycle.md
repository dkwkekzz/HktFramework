...
## MASTER TRACE
    ...
    Constraint Note
        ...
        DC-ITEM-CAPACITY-IS-FINITE
            C022 가 세운 칸을 그대로 쓴다. 적용 자리도 유한하다 — 그 수는 값이며
            규칙에 박지 않는다. 푸는 것이 담을 칸을 요구하므로 두 유한함이 여기서
            처음 만난다.
            **IE §49 P3 의 "자리가 칸보다 훨씬 좁다" 는 이 Cycle 에서 성립하지 않는다**
            (자리 6 · 칸 4). 걸 수 있는 종류가 하나뿐이라 지금은 겪히지 않으며,
            어느 값을 움직일지는 Human 판단이다 (03 JUDGEMENT ①)
...
### ⑥ 새 장비 종류를 하나도 세우지 않는다

    **장비 아이템을 여럿 만드는 일이 아니다.** 이 후보가 여는 것은 종류가 아니라
    적용이다. 그리고 세계에 이미 있는 둘로 필요한 관찰이 전부 선다.

        용도를 주는 것       곡괭이 — 걸면 캘 수 있고 풀면 캘 수 없다
        값을 바꾸는 것       곡괭이 — 같은 물건이 몸의 값도 바꾼다 (Stage 3 이 값을 정한다)
        걸 수 없는 것        돌 — 걸 수 있는 물건이 아니므로 사유와 함께 거절된다

    **정의소에 항목이 늘지 않는다.** 늘어나는 것은 곡괭이 정의 안의 몇 줄뿐이며,
    그것이 "새 장비가 생기는 일은 정의가 늘어나는 일이지 규칙이 늘어나는 일이 아니다" 를
    이 Cycle 안에서 이미 보여 준다.
...
## INCLUDED
    ...
    적용 자리                몸이 이름 있는 유한한 자리들을 지닌다. 자리는 물건을 직접
                            담는다 — 걸린 것은 소지품에 없다. **자리의 수는 Stage 3 이
                            소유한다.** 자리들은 서로 같고 저마다의 성격을 지니지 않는다

    적합성                   무엇이 걸릴 수 있는가를 **정의가 답한다.** 걸 수 없는 물건은
                            사유와 함께 거절된다. 전용 자리는 물건이 스스로 선언할 때만
                            생기는 예외이며, 규칙이 종류 이름을 묻지 않는다
...
## EXCLUDED
    ...
    교체                     이미 찬 자리의 것을 빼면서 다른 것을 거는 것 — 빼기와 걸기가
                            한 번에 일어나는 단위. **후보 3 (FR-ONE-SLOT-ONE-ITEM)** 이
                            닫는다. 이 Cycle 에서는 빈 자리가 하나도 없으면 사유와 함께
                            거절된다
