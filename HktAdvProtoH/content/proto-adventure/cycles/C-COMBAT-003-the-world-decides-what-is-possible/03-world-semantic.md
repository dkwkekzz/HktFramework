...
## BALANCE — 수치와 그 근거
    ...
    ① 새 기술 `hatsu-burst` (발현 일격)
        ...
        기본 피해    10   — heavy-attack(32)보다 훨씬 낮다. **이 기술의 크기는 몰아 둔
                    ...
                    기본값에 가려진다
        계수        1.0  — heavy-attack 과 같다. 몰아 둔 힘이 그대로 실린다
    ...
    ② 이 기술이 실제로 내는 값 (관찰자 rabbit-swordsman → 자율 존재 wanderer)

        기준 몸      AuraAttack 40 · 오라 관통 60 · Hp 200 · Cp 100 (시작 30)
        상대 몸      Resistance 90 → 관통 60 이 걷혀 유효 방어 30

        배분이 hatsu 면 AuraAttack = 40 + (4−2)×12 = **64**

            사정 없음        10 + 64×1.0 = 74   → 74 × 100/130 = **57**
            하나 참          10 + 64×1.4 = 100  → 100 × 100/130 = **77**
            둘 다 참         10 + 64×1.8 = 125  → 125 × 100/130 = **96**

        같은 배분의 heavy-attack 은 물리 32 + 32 = 64 → 64 × 100/130 = **49** 이다.
        관문을 지난 기술이 큰 기술보다 한 대에 8 더 들어가고, 사정을 하나 만들면 28 더
        들어간다. **관문의 값이 눈에 보이고, 사정을 만들러 갈 값도 눈에 보인다.**
    ...
    ③ 조건의 몫 — 0.4 씩
        ...
        0.4 는 한 대에 20 (57 → 77) 이다. 상대 생명의 10% 를 넘으므로 눈에 보이고,
        두 대 차이를 만들지는 않으므로 조건 없이 싸우는 길이 죽지 않는다.
