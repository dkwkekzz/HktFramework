...
세계는 여전히 한 줄도 바뀌지 않았다 (`world/` 변경 0). 화면의 자리만 옮겼다.

    npm test                68 파일 · **1169 통과 / 0 실패**
    npx tsc --noEmit        오류 0
    npm run build           성공
...
### 슬롯 띠에서 다시 읽은 다섯 상태
    ...
    1 입력 전        [F] 지금 됨          [G] 지금 됨        [H] 지금 됨
    2 **칸을 눌러**   [F] 지금 됨          [G] **나갔다**     [H] 지금 됨
    3 막기를 건 뒤    [F] 불가 · 막는 중    [G] 불가 · 막는 중  [H] 불가 · 막는 중
    4 막는 중에 F     [F] **거절 · 막는 중** [G] 불가 · 막는 중  [H] 불가 · 막는 중
    5 막기를 푼 뒤    [F] 지금 됨          [G] 지금 됨        [H] 지금 됨

**②가 이번 실측의 새로운 것이다** — 키가 아니라 **포인터로 칸을 눌러** 요청했고,
그 대답이 그 칸으로 돌아왔다. 키와 포인터가 `requestInteraction` 하나로 수렴한다
(`VUX-SK-V-02` 가 이제 코드의 모양으로도 참이다).

### 기획서 §9 (접근성) — 이 Cycle 이 범위 밖에 두었던 것

`01-cycle.md` 의 EXCLUDED 에 §9 를 적지 않은 것은 이 Cycle 의 누락이다. 여기 기록한다.

기반의 슬롯 띠가 그중 셋을 세웠다.

    접근성 이름     칸마다 `이름, F 키, 지금 됨` — 이름·부르는 자리·상태가 함께 들어간다.
                   **부를 수 없는 칸도 이름을 가진다** (`이름뿐, 부를 수 없음`)
    색만이 아니다   되는 것 · 안 되는 것 · 기다리는 것이 테두리 **모양**(실선·파선·점선)과
                   진하기로도 갈린다
    초점 링         `:focus-visible` 로 그린다 — 칸이 button 이므로 자판으로 닿는다

아직 없는 것 (기반 트랙에 남는다)

    포커스 순서     `대상 → Skill Bar → 상세 → 오버레이` 라는 정해진 차례가 없다
    200% 확대       잘리지 않는지 확인하지 않았다
    애니메이션 감소  `prefers-reduced-motion` 을 읽는 자리가 없다
    World-space Tooltip · Target Cycle 로 여는 길
