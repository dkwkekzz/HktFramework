...
// 항목마다 [도메인] 을 표기한다 — 어느 도메인의 Actor 필드를 세우는 값인지가 곧 소유다.
//
// [base] Actor.Body — 몸 캡슐의 반경·높이·질량 (C006).
...
// TempoStats (C007) — 세계의 속도를 정하는 능력치 ([movement] 둘 · [combat] actionSpeed)
...
// [combat] 전투 능력치 (C010 → C012 CHANGED) — 한 방의 크기를 정하는 네 값.
...
export interface CharacterDefinition {
  body: BodySpec;
  facing: Readonly<WorldPosition>; // [base] 스폰 시 몸이 향하는 방향 (단위 벡터)
  tempo: TempoSpec;
  resources: ResourceSpec;
  combat: CombatSpec; // C010
  attackRange: number; // [combat] RULE-ATTACK-001 Precondition 2 의 거리 한계
  perceptionRange: number; // [autonomy] RULE-NPC-DECIDE-001 의 인지 거리 — control = autonomous 일 때만 의미
}
