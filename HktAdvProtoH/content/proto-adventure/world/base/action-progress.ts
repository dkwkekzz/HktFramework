...
//
// 완료 효과 Rule 은 **그 행동을 소유한 도메인**이 표 항목으로 내놓는다
// (WorldDomain.actionCompletions) — 이 파일은 어느 도메인도 알지 못한다.
//   mine   → RULE-MINE-COMPLETE-001   (mining 이 낸다)
//   스킬   → 없음 (C006 CHANGED — 타격은 완료가 아니라 휘두름 구간의 접촉이 정한다.
//                  RULE-SWING-STRIKE-001, domains/combat/swing-strike.ts)
//   hit    → 없음 (그냥 끝나고 대기로 돌아간다)
//
// 진행 대상은 Tick 이 시작될 때의 행동으로 고정한다. 이 Tick 안에서 새로 시작된 행동
// (예: 타격받아 들어간 피격)까지 같은 dt 로 밀면, 시작하자마자 끝나 버린다.

import { idleAction, type ActionKind, type CurrentAction } from './action';
import type { ActorState } from './actor';
import type { WorldState } from './world-state';

/** 행동 하나가 Duration 을 채웠을 때 그 행동을 소유한 도메인이 하는 일 */
export type ActionCompletion = (state: WorldState, actor: ActorState) => void;

export type ActionCompletions = Readonly<Partial<Record<ActionKind, ActionCompletion>>>;

export function ruleActionProgress(
  state: WorldState,
  dt: number,
  ...
  for (const { actor, action } of advancing) {
    ...
    completions[action.kind]?.(state, actor);
