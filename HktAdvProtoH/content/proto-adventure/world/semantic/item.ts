...
export const ITEM_CATALOG: Readonly<Record<ItemKind, ItemDefinition>> = {
  ...
  stone: {
    ...
    use: {
      ...
      //
      // 처음에는 0.8 이었다. Stage 8 실측이 그 값을 되돌렸다 — 사냥터를 지키는 존재
      // 옆에서 방해 없이 서 있을 수 있는 창이 **0.90초**로 재어졌고(그 존재의 타격
      // 주기), 0.8 은 여유가 0.1초라 사람이 맞힐 수 없었다. 던지려 할 때마다 맞아
      // 끊기면 "가진 것을 써서 세계를 바꾼다" 가 세계에서 성립하지 않는다.
      // 0.5 는 그 창 안에 0.4초의 여유를 남긴다 — 값을 치르되 불가능하지는 않다.
      duration: 0.5,
    },
  },
