...
// Input          World.StrikeEvents, World.UnharmedContacts, World.CancelEvents, World.Time
...
export function ruleStrikeEventExpire(state: WorldState): number {
  const before =
    state.strikeEvents.length + state.unharmedContacts.length + state.cancelEvents.length;
