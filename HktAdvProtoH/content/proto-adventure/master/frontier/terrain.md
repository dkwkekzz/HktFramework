...
### FR-LOSING-IS-A-PROCESS — 잃는 것은 한순간이 아니다
    ...
    의존                 없음 — `FR-THE-GROUND-HAS-A-LAW`(C-TERRAIN-001)가 이미 닫혔고 그
                         위에 바로 선다. `FR-THE-LAND-KEEPS-WHAT-IT-TAKES` 와 겹치지 않는다
...
### FR-THE-LAND-IS-ALIVE-AND-ANSWERS — 땅이 살아 있고 응답한다
    ...
    Why one Cycle        자리가 이미 State 이고 법칙의 이름을 지니며, 존재 사이의 관계를
                         ...
                         자리 사이의 관계** 하나다
    의존                 `FR-THE-LAND-KEEPS-WHAT-IT-TAKES` 가 먼저인 편이 낫다 — 땅이 내부
                         상태를 쌓지 않으면 이어져도 **읽을 것이 얇다** (지금 자리가 지닌
                         것은 법칙의 이름과 역할뿐이다). 막는 의존은 아니다
    Status               PROPOSED
