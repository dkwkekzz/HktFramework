...
이 문서는 그 칸이 지금 닫혀 있는가만 답한다.
    ...
    기준 시점   C022(자리가 유한해진다) 실측 완료 — Human Play 확인 대기.
                ...
                **그것을 담을 곳이 유한해졌다 (C022)** — 물건이 정의소를 갖고, 써서
                상대의 상태를 바꾸며, 쓴 만큼 줄고, 담을 자리가 차면 더 받지 못한다
...
**넷 중 하나가 섰다 (C020)** — 그 하나의 판정은 Cycle 실측이고, 나머지 셋은 코드 대조다.
...
| MC-EQUIP-ITEM | MISSING | 코드 대조 — 몸에 적용된 것이라는 개념이 없다. 지닌 것과 몸의 능력치는 서로를 모른다. **IE 주입으로 결손의 형태가 확정되었고**, **C020 으로 공통 앞칸(정의소 · 변경 단일 통로 · 소모 · 원자성 · 용도 사슬의 절반)이, C022 로 자리가 섰다** — IE §15 · §16.1 의 비대칭("가방이 가득할 때 해제는 막히고 교체는 된다")을 표현할 자리가 이제 세계에 있다 (`Inventory.UsedSlots` · 사유 `no-room`). 남은 것은 적용 자체다 | 자리 · 적합성 · 적용/해제 · 유효 능력치 · 장착이 여는 행동 전부. 이것이 없어 `IM-*` 의 grants 가 몸에 닿지 못한다. IE 가 더한 것: 자리가 담을 칸보다 좁다는 것 · 교체가 하나의 성공 단위라는 것 · 담을 곳이 가득할 때 해제는 막히고 교체는 되는 비대칭 · 적용 상태가 누적이 아니라 재계산이라는 것 |
...
| MP-ADAPT-BY-RESOURCE | MC-RESTORE-BIOLOGICAL-STATE · MC-CUT-ABNORMAL-STRUCTURE + **MC-EQUIP-ITEM · MC-CRAFT-FROM-MATERIALS** + MK-LOCAL-WORLDSTATE + 자원 | **첫 칸이 세계에서 굴러간다** — C020 으로 MC-USE-ITEM 이 서서 "물건이 대신해 주고, 물건을 잃으면 도로 못 하게 된다" 가 실제로 성립한다. 설계상 획득 경로는 Q22 로 섰다 (경계결정 → IM-BOUNDARY-EDGED). 남은 둘(적용 · 제작)이 회복·절단 앞을 막고 있고, 쓸 물건의 종류가 하나(던지는 돌)뿐이라 갈래가 좁다 |
