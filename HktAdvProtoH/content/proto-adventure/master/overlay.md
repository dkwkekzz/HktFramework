...
| Possibility | 요구 중 없는 것 | 비고 |
|---|---|---|
| MP-LEARN-TO-HANDLE-THE-LAYER | MC-PREDICT (+ MC-OBSERVE 는 PARTIAL) | **C014·C016 이 첫 칸을 세웠다** — 살펴봄이 서고 거기에 이르는 길이 둘이 되었다. 남은 것은 둘째 칸(예측) 하나이며 그것이 MC-OBSERVE 의 마지막 결손과 같은 자리다. 탐험의 기본 갈래이자 다른 둘의 앞이다 (먼저 겪은 사람이 없으면 살 정보도 가져올 자원도 없다) |
