...
이 문서는 그 칸이 지금 닫혀 있는가만 답한다.
    ...
    근거 문서   전투 R1 · DT · 탐험 BW · 성장 GR · 지목 TG · 아이템 IS · 인벤토리 IE ·
                스킬 형태 SF — 근거는 영역을 넘지 않는다 (HISTORY Q15)
...
SF(스킬 형태) 주입은 이 표에 **줄을 하나도 더하지 않았다.** 그 문서는 효과가 대상에
닿는 방식(전달 형태)만 공급하고 그 형태를 요구하는 Possibility 는 공급하지 않기 때문에,
...
나머지 열세 칸이 왜 비어 있는지, 무엇이 그것을 요구하게 될지는 open-questions Q35 와 HISTORY Q44 다.
