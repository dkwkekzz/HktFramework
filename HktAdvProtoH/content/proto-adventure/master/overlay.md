...
이 문서는 그 칸이 지금 닫혀 있는가만 답한다.
    ...
    기준 시점   C020(아이템의 바닥 — 쓴다 · 없어진다) 완료. 전투 사다리는 Critical
                층까지, 탐험은 FRINGE 의 첫 칸까지 서 있고, 그 위에 고른 대상(C017) ·
                둘 사이의 태도(C018) · 행동 안의 시점(C019)이 얹혔다.
                **세계에 처음으로 가진 것이 사라지는 경로가 생겼다 (C020)** —
                물건이 정의소를 갖고, 써서 상대의 상태를 바꾸며, 쓴 만큼 준다
...
나머지 열세 칸이 왜 비어 있는지, 무엇이 그것을 요구하게 될지는 open-questions Q35 · Q37 다.
