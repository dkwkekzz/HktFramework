...
이 문서는 그 칸이 지금 닫혀 있는가만 답한다.
    ...
    기준 시점   C023(걸어 둔 것만이 몸을 바꾼다) 닫힘.
                ...
                **지닌 것과 몸에 적용된 것이 갈렸다 (C023)** — 물건이 정의소를 갖고,
                써서 상대의 상태를 바꾸며, 쓴 만큼 줄고, 담을 자리가 차면 더 받지 못하고,
                **걸어 둔 것만이 몸의 값과 할 수 있는 일을 바꾼다**
...
C024 로 그 칸의 **모양이 값이 되었다** — 남은 다섯 칸이 서면 그 축을 새로 만들지 않고
...
|---|---|---|---|
| MC-COMBAT-STRIKE | IMPLEMENTED | C007 · C010 이 피해 산정을 하나의 공식으로 교체. C024 가 휘두름의 **모양**(각·길이·굵기)을 전역 상수에서 기술 정의로 내려 이 노드에 걸린 마지막 UNRESOLVED 를 닫았다 | — |
...
| MC-ATTACK-POWER | PARTIAL | C010 — 공격력 40→80 변경이 피해 20→35 로 실측. **C023 으로 세계 안의 첫 경로가 생겼다** — 곡괭이를 걸면 물리 공격 40 → 52 가 플레이로 관찰되고, 풀면 정확히 40 으로 돌아온다 (C023 08 PLAYABLE ④·⑧) | **값을 키우는 축이 없다.** 물건으로 값이 *달라지는* 것은 섰으나 그것을 *키우는* 것 — 될 Class 도, 배울 상대도, 자라는 경로도 — 은 여전히 없다. 물건 쪽도 지금은 곡괭이 하나뿐이다 |
...
| MC-EVADE | MISSING | — | 회피 행동이 없다. 다만 공격이 이미 공간 판정이라 얹힐 바닥은 서 있다 (R1 §13 이연). C024 로 그 공간이 **기술마다 달라졌다** — 피할 대상이 하나가 아니므로 회피가 설 때 다룰 것이 늘었다 |
...
**넷 중 하나가 섰고 하나가 절반 섰다 (C020 · C023)** — 그 둘의 판정은 Cycle 실측이고,
나머지 둘은 코드 대조다.
...
| MC-EQUIP-ITEM | **PARTIAL** | **C023** — 세계 프로세스 실측으로 world_shape 의 앞 절반이 닫혔다: 같은 곡괭이를 가지고만 있으면 캐지지 않고 걸어야 캐지며(사유 `no-mining-tool`), 걸면 물리 공격 40 → 52 · 풀면 **정확히 40** 으로 돌아오고 캘 수 없게 된다. 걸릴 수 없는 물건은 `not-equippable` 로 거절되고, 담을 곳이 모자라면 풀기가 `no-room` 으로 거절되며 자리도 수량도 건드리지 않는다. 유효 값은 저장하지 않는 파생이라 백 번 걸고 풀어도 표류하지 않고, 둘을 걸면 두 번 더해지고 하나만 풀면 정확히 한 번어치가 남는다 | **교체가 없다.** world_shape 의 남은 두 문장이다 — 이미 찬 자리에 넣으면 넣기와 빼내기가 한 번에 일어나는 것, 그리고 가방이 가득할 때 해제는 막히고 교체는 되는 비대칭 (IE §16 · §16.1). 지금은 빈 자리가 없으면 `no-empty-slot` 으로 거절된다. 그 밖: 걸 수 있는 물건이 곡괭이 하나뿐이고, 전용 자리를 선언한 물건이 없어 자리별 적합성이 플레이에서 겪히지 않는다 |
...
| MP-ADAPT-BY-RESOURCE | MC-RESTORE-BIOLOGICAL-STATE · MC-CUT-ABNORMAL-STRUCTURE + MC-EQUIP-ITEM(PARTIAL) + **MC-CRAFT-FROM-MATERIALS** + MK-LOCAL-WORLDSTATE + 자원 | **이 갈래의 문장이 세계에서 통째로 참이 되었다** — "물건이 대신해 주고, **물건을 잃으면 도로 못 하게 된다**"(BW §17). 앞 절반은 C020(쓰면 없어진다)이, 뒤 절반은 **C023**(풀면 캘 수 없게 된다)이 세웠다. 남은 것은 제작이며 그것이 회복·절단 앞을 막고 있다. 쓸 물건도 걸 물건도 여전히 한 종류씩뿐이라 갈래가 좁다 |
