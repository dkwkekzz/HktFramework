...
| 3 | **행동 구간** | 행동의 시간이 준비·발동·회수로 나뉘고, 준비 구간이 관찰되고 판정에 쓰인다 | 구간의 **의미** (구간 자체는 있다) | 중간 |
...
### 3. FR-ACTION-PHASE — 행동 구간
    ...
    이것이 무엇인가    행동의 시간이 **준비 · 발동 · 회수** 세 구간으로 나뉘고, 그 비율이
                       행동마다 다르며, 준비 구간이 관찰되고 판정에 쓰인다.
                       구간 자체는 이미 있다 — 없는 것은 그 구간의 **의미**다.
    세계에 생기는 것    ① 행동마다 다른 구간 비율 — 큰 것일수록 준비가 길다
                          (지금은 모든 휘두름이 같은 비율을 쓴다)
                       ② 준비 구간이 무엇을 준비하는지가 관찰에 실린다 — 진행도 숫자가
                          아니라 "지금 무엇이 오고 있는가" 로
                       ③ 준비 구간에 조건이 닿으면 그 행동이 **취소된다** — 발동하지 않는다.
                          지금은 시작한 행동이 무조건 끝까지 간다
                       ④ 취소된 행동의 대가 — 치른 것은 돌아오지 않는다
                       ⑤ 자율 존재도 같은 규칙을 진다 — 예외를 갖지 않는다
    이 기능이 아닌 것   확률·회피·경직·기절이 아니다. 되받아치기가 아니다 (취소한 뒤의
                       이득은 오지 않는다). 다음다음 수를 읽는 것도 아니다 — 지금 나오는
                       하나다. 막기를 행동으로 편입하는 것도 아니다 (막기는 여전히 상태다)
    이미 있는 것        **행동은 이미 세계의 개념이다** — 모든 존재는 언제나 정확히 하나의
                       행동 안에 있고, 행동은 종류·소요 시간·대상·진행도·대체 가능 여부를
                       지니며 진행도가 관찰에 실린다. 휘두름의 구간도 이미 있다
                       (앞의 얼마는 준비, 가운데가 유효, 뒤는 회수). 맞으면 하던 행동이
                       끊기는 규칙도 이미 있다. 더할 것은 비율을 종류마다 다르게 두고,
                       준비 구간을 의미로 노출하고, 그 구간을 취소 판정에 쓰는 일이다
    플레이어가 하는 것  상대가 큰 것을 내기 전에 그것을 읽고, 막거나 물러나거나 끊는다
    무엇을 보면 되는가  상대 위에 무엇이 오는지가 먼저 뜨고, 큰 행동일수록 그 시간이 길다.
                       그때 넣은 개입이 그 행동을 무산시키고, 같은 개입을 늦게(이미 칼이
                       나간 뒤에) 넣으면 무산되지 않는다. 아무것도 안 하면 그대로 맞는다
    Source Goal        MG-EXPLORE-BEIRA · MG-OVERCOME-SUPERIOR-OPPONENT
    Source Possibility MP-LEARN-TO-HANDLE-THE-LAYER · MP-INTERRUPT
    Missing / Partial  MC-PREDICT (MISSING) — MC-OBSERVE 에 남은 마지막 결손(행동·습성)이
                       이 자리다. 이것이 닫히면 MC-OBSERVE 도 IMPLEMENTED 가 된다.
                       MC-INTERRUPT (PARTIAL) — 끊김 규칙은 있고 노리는 수단이 없다
    원본 근거          BW §21 (FRINGE — 관찰 · 예측 · 지형) · BW §32 (관찰 → 이해 → 대응) ·
                       BW §28 (범용 전투 그래프의 여덟 갈래 중 하나) ·
    ...
    Constraint Eval    SATISFIED — 읽히는 것도 취소도 세계가 이미 정한 시점 관계이므로 새
                       난수가 없다 (PLAYER-CAUSALITY). 피해 공식을 건드리지 않는다
                       (ONE-FORMULA). 능동 방어 · Aura 를 손대지 않는다 (ONE-LAYER).
                       진행이 수치가 아니라 이해의 확장으로 나타나고
                       (PROGRESSION-IS-REACH), 읽지 않고 싸우는 길이 남는다 (UNFIXED-PATH)
    왜 둘이 한 Cycle 인가  읽는 것과 끊는 것은 **같은 개념의 두 면**이다. 준비 구간을
                       노출만 하고 판정에 쓰지 않으면 그 Cycle 은 표시만 바꾼 것이 되어
                       플레이가 달라지지 않고, 반대로 취소만 넣고 노출하지 않으면
                       "아무 때나 때리면 끊긴다" 가 되어 지금과 같다. 둘 중 하나만으로는
                       닫히지 않으므로 한 Cycle 이다
    7 조건             1 MISSING+PARTIAL · 2 익히는 갈래와 전투 갈래를 함께 전진시킨다 ·
                       3 Client 실측 가능 · 4 한 Cycle (위 문단) · 5 새 World 규칙 ·
                       6 Active 와 양립 · 7 되받아치기 · 완벽한 막기 · DANGER 층 요구가
                       모두 이 위에 얹힌다
    Status             PROPOSED
...
3. 행동 구간        익히는 갈래의 마지막 결손이고, 닫으면 MC-OBSERVE 가 완성된다.
                    되받아치기 · 완벽한 막기 · DANGER 층 요구가 모두 이 위에 얹히므로
                    얹힘이 가장 넓다. 전투 갈래도 함께 하나 연다 (MP-INTERRUPT)
...
| 장비 (걸치고 유지한다) | 같음 — 후보 4 다음 칸. 관통·치명을 얻는 경로도 여기에 얹힌다 |
| 감정 도구로 아는 경로 | 같음 — 후보 4 다음 칸. C016 이 부분 공개를 이미 세웠다 |
| 능동 방어 (완벽한 막기·되받아치기·Break) | 아래층은 C013·C015 로 섰다. 이제 막는 것은 **그 층의 설계 문서**다 — 두 전투 문서는 이름만 예고한다. 후보 3 이 서면 시점 판정의 바닥이 생긴다 |
