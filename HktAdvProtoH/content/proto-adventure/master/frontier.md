...
### 7. FR-THINGS-LIE-IN-THE-WORLD — 물건이 몸 밖에 놓인다
    ...
    이미 있는 것        코드 대조 — 위치를 가진 비-Actor 존재의 **선례가 하나 있다**:
                       ...
                       관찰에 실린다. 줍기·버리기의 원자성 형태도 C020·C021 이 세운다.
...
### 8. FR-THE-SHAPE-IS-DATA — 휘두름의 모양이 값이 된다
    ...
    이 기능이 아닌 것   **새 전달 형태가 아니다** — 투사체 · 장판 · 광선은 여기 없다
                       (HISTORY Q40 — 추가 기획을 기다린다).
                       여럿을 동시에 치는 것 자체가 목적이 아니다 — 모양의 결과로
                       그렇게 될 뿐이다.
                       타게팅 방식의 갈래가 아니다 (HISTORY Q38 — 같은 문서를 기다린다).
...
레인 A — 아이템 (IS · IE)          레인 B — 전투 (SF)
  ...
  1 자리가 유한해진다  ← 지금 돎      8 휘두름의 모양이 값이 된다
        ↓                              (의존 없음 · 단독으로 닫힌다)
  2 걸어 둔 것만이 몸을 바꾼다
        ↓
  3 한 자리에는 하나
...
5 (자리 정리)  7 조건 2·5 가 약하다 — 새 게임 의미보다 편의 쪽이다. 급하지 않다
...
| **스킬 전달 형태** (MS-SKILL-FORM 의 빈 열세 칸) | 둘이 막는다. ① 그 형태를 요구하는 Possibility 가 없다 (Q35 의 7 조건 2 — OPTIONS 작업이 먼저다) ② SF 가 답하지 않은 넷과 **타게팅 방식의 갈래**(범위 타격 · 단일 대상 타격 · 유도 · 연쇄)를 **이후 추가 기획**이 가져오기로 했다 (HISTORY Q40 · Q38). 특히 투사체·장판·설치·소환은 세계에 **몸이 아닌 존재**가 먼저 서야 한다 |
...
| 감정 도구 | 후보 2 · 6 · 7 뒤다. 지금은 감정할 대상(개체 상태)도 없다 (IE §41 — 위 칸) |
