...
|---|---|---|---|---|
| 1 | **준비를 끊는다** | 상대의 행동이 나가기 전 **준비 구간**을 노려 무산시킨다 — 그 구간이 행동마다 다르고, 관찰되고, 취소 판정에 쓰인다 | 준비 구간의 **의미** (구간 자체는 있다) | 중간 |
| 2 | **아이템 사용** | 가진 물건을 써서 세계나 몸을 바꾼다 | 쓴다는 개념 전부 | 중간 |

## 후보

### 1. FR-INTERRUPT-THE-WINDUP — 준비를 끊는다

    이것이 무엇인가    상대의 행동이 **나가기 전**의 준비 구간을 노려 무산시킨다.
                       그러기 위해 행동의 시간이 **준비 · 발동 · 회수** 로 나뉘고, 그 비율이
                       행동마다 다르며, 준비 구간이 의미로 관찰되고 **판정에 쓰인다**.
                       구간 자체는 이미 있다. 없는 것은 그 구간의 **의미**다 —
                       척추에서 이 후보의 자리는 MP-INTERRUPT 다 (완성된 것을 처리하는
                       모든 갈래와 달리 유일하게 날아오지 못하게 하는 갈래).
    ...
    Missing / Partial  MC-INTERRUPT (PARTIAL) — 끊김 규칙은 있고 **노리는 수단**이 없다.
                       그 수단이 곧 준비 구간의 의미다. MP-INTERRUPT 가 요구하는 것은
...
2. 준비를 끊는다     전투 후보 중 가장 값싸다 — 구간도 끊김 규칙도 이미 있어, 더할 것은
