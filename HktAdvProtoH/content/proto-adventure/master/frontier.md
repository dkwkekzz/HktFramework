...
|---|---|---|---|---|---|
| 1 | A | **한 자리에는 하나** | 이미 찬 자리에 다른 것을 걸면 빼기와 걸기가 한 번에 일어난다. 가방이 가득해도 바꿔 끼우는 것은 된다 | 교체라는 단위 · 가득 찬 상태의 비대칭 | 작음 |
| 2 | A | **걸기 전에 안다** | 걸면 무엇이 어떻게 달라지는지를 걸기 전에 세계가 계산해 보여 준다 | 적용하지 않은 채 계산한 결과 | 아주 작음 |
| 3 | A | **자리를 손으로 정리한다** | 담긴 것을 옮기고 맞바꾸고 겹친 것을 나누고 정렬한다 | 자리 사이의 이동 · 나누기 · 정렬 | 작음 |
| 4 | A | **재료가 다른 것이 된다** | 가진 재료를 정해진 조합으로 다른 것으로 바꾼다 | 제작법이라는 데이터와 그것을 평가하는 자리 | 중간 |
| 5 | A | **물건이 몸 밖에 놓인다** | 아이템이 누구의 것도 아닌 채로 세계의 한 자리에 놓인다 | 위치를 가진 아이템 · 줍기 · 버리기 · 소유 · 소멸 | 큼 |
| 6 | B | **휘두름의 모양이 값이 된다** | 기술이 닿는 모양이 코드가 아니라 기술의 정의에 담긴다 | 모양 자체 — 반경 · 각도 · 길이가 코드 상수다 | 작음 |

## 후보

### 1. FR-ONE-SLOT-ONE-ITEM — 한 자리에는 하나
...
### 2. FR-SEE-BEFORE-YOU-WEAR — 걸기 전에 안다
    ...
    Why one Cycle      아주 작다. **후보 1 에 얹어 한 Cycle 로 돌아도 된다** — 따로 세운
                       것은 그것이 없어도 C023 이 완결되었기 때문이지, 별도 Cycle 이어야
...
### 3. FR-ARRANGE-WHAT-YOU-CARRY — 자리를 손으로 정리한다
    ...
    Missing / Partial  **Capability 노드를 목표로 삼지 않는다** — 배치를 바꾸는 것은
                       할 수 있는 일을 늘리지 않는다. 후보 1 · 2 와 같은 사유의 예외다
    ...
    의존               **없다 — C022 가 이미 섰다.** 장착과는 무관하므로 후보 1~2 와
                       순서를 다툴 이유가 없다.
...
### 4. FR-MATERIALS-BECOME-SOMETHING-ELSE — 재료가 다른 것이 된다
...
### 5. FR-THINGS-LIE-IN-THE-WORLD — 물건이 몸 밖에 놓인다
...
### 6. FR-THE-SHAPE-IS-DATA — 휘두름의 모양이 값이 된다
    ...
    이 기능이 아닌 것   **새 전달 형태가 아니다** — 투사체 · 장판 · 광선은 여기 없다
                       (HISTORY Q44 — 추가 기획을 기다린다).
                       여럿을 동시에 치는 것 자체가 목적이 아니다 — 모양의 결과로
                       그렇게 될 뿐이다.
                       타게팅 방식의 갈래가 아니다 (HISTORY Q42 — 같은 문서를 기다린다).
    ...
    원본 근거          SF §6 · §20 · §28.1 · §29 · §32 수용 기준 1 · 12 ·
    ...
    Constraint Eval    SATISFIED — 모양을 값으로 내리는 것이 IS-COMBINATION-NOT-NAME 이
                       ...
                       것이므로 COMBINE-BEFORE-NEW-FORM 과도 맞는다(§29-2 의 정석 사례).
...
```text
레인 A — 아이템 (IS · IE)          레인 B — 전투 (SF)

  (바닥 C020 · 자리 C022 · 적용 C023 — 닫혔다)   6 휘두름의 모양이 값이 된다
        ↓                                        (의존 없음 · 단독으로 닫힌다)
  1 한 자리에는 하나
        ↓
  2 걸기 전에 안다
  ...
  3 자리를 손으로 정리한다  ← 아무 때나. 레인 안에서도 순서가 자유롭다
```
...
```text
1 · 2         **C023 이 남긴 절반과 덤이다.** 1(교체)은 MC-EQUIP-ITEM 을 PARTIAL 에서
              끝까지 밀고, 2(미리보기)는 C023 이 세운 유효 값 재계산을 적용하지 않은
              채로 한 번 더 부르는 일이다. 둘 다 작고, 둘을 한 Cycle 로 묶어도 된다 —
              IE §48 의 Cycle 2 가 원래 셋을 한 묶음으로 그렸다

4 (제작)      C023 뒤. 만든 것이 쓸모를 가지려면 걸 수 있어야 한다 (IS §6) — 이제 걸 수 있다

5 (세계의     여섯 중 여는 것이 가장 넓다 — 전리품 · 드롭 · 거래 · C017 의 미도달
   아이템)    규칙이 전부 이 위에 얹힌다. 대신 가장 크다. IE §35 의 앞칸도 C023 이 섰다

3 (자리 정리)  7 조건 2·5 가 약하다 — 새 게임 의미보다 편의 쪽이다. 급하지 않고,
...
| **스킬 전달 형태** (MS-SKILL-FORM 의 빈 열세 칸) | 둘이 막는다. ① 그 형태를 요구하는 Possibility 가 없다 (Q35 의 7 조건 2 — OPTIONS 작업이 먼저다) ② SF 가 답하지 않은 넷과 **타게팅 방식의 갈래**(범위 타격 · 단일 대상 타격 · 유도 · 연쇄)를 **이후 추가 기획**이 가져오기로 했다 (HISTORY Q44 · Q42). 특히 투사체·장판·설치·소환은 세계에 **몸이 아닌 존재**가 먼저 서야 한다 |
...
| 감정 도구 | 후보 4 · 5 뒤다 (장착은 C023 으로 섰다). 지금은 감정할 대상(개체 상태)도 없다 (IE §41 — 위 칸) || 능동 방어 · Aura/Nen · 베이라 사다리의 잠정 조각 전부 | 그 전체의 설계 문서가 없다 (`part_of.grounded: false` — 척추 시각화의 점선). 능동 방어가 요구하는 **행동 안의 시점 판정**은 C019 로 바닥이 섰다 — 남은 것은 문서뿐이다 |
...
| 지형 · 문명권 준비 갈래 · 희귀 기관 갈래 | 세계 기반(지역 · 문명권 · 거래 주체)이 없다 (overlay.md World 표 ABSENT). 희귀 기관 쪽은 그 위에 **물건이 몸 밖에 놓인다**(후보 5)까지 필요하다 — IS 주입으로 공통 앞칸이 드러났다 |
