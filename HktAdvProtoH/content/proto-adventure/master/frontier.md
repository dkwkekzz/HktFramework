...
```
    ...
    **레인 B 는 아직 소진되지 않았다.** 여덟 Stage 의 실측은 끝났고 Gate 15항 중 열넷이
    ...
    확인되면 후보 6 을 지우고 결과를 HISTORY 로 옮긴다 (C025 08-verification.md).

    **레인 A 에서 후보 없이 한 바퀴가 돌고 닫혔다.** `C026-open-what-you-carry`
    (가진 것을 여는 자리) — 기획서가 스스로 `[DIRECT-CYCLE]` 로 표시한 관찰 표면 작업이며
    Human 이 후보 등록을 건너뛰고 착수를 지시했다. **이 파일에 후보로 서지 않았던 것이
    맞다** — 세계가 할 수 있는 일을 하나도 늘리지 않으므로 Master Layer 가 고를 것이
    없었고, 고를 것이 없는 자리에 후보를 세우면 Frontier 가 표면 작업으로 채워진다.
    결과는 HISTORY.md 로 옮겼다. 그 Cycle 이 후보 1·2 를 **더 싸게** 만들었다 (위 두 블록).

    **레인 A 의 후보 자리는 비어 있다.** 직전에 돌던 `FR-ONE-SLOT-ONE-ITEM`(한 자리에는 하나)이
    **C025 로 닫혔고**(Gate 15항 전부 충족), 그것으로 **MC-EQUIP-ITEM 이 IMPLEMENTED**
    가 되었다 — 아이템 넷 중 둘이 섰다. 결과는 HISTORY.md 로 옮겼다.

    레인 A 의 다음은 남은 다섯(후보 1~5) 중 하나를 Human 이 고르는 자리다. Agent 추천은
    **후보 5(자리가 걸 것보다 좁아진다)** — 다섯 중 가장 싸게 게임 의미를 늘린다.
    C025 가 교체라는 수단을 세웠는데 그것을 **써야 할 이유**가 아직 세계에 없기 때문이다
    (자리 여섯 · 걸 것 둘). 값 하나를 움직이는 일이고, 그것이 서야 "물건으로 감당한다" 가
    실제 선택이 된다. 후보 1(미리보기)을 얹어 한 Cycle 로 돌아도 된다 — 고를 거리가
    생긴 뒤에 미리 보는 것이 순서로도 맞다.

    〔이 칸의 형태가 바뀌었다〕 지금까지 `SELECTED` 는 **하나만** 담았고, 그래서 레인 둘이
    동시에 돌 때 한쪽이 이 칸 밖에서 출발해야 했다 (C025 가 실제로 그랬다). 레인별로
    적는 형태로 바꾼 것은 그 결손을 드러내기 위한 최소 변경이며, **이대로 둘지는 Human
    판단**이다 (open-questions Q46).

    같은 뿌리의 다른 반쪽인 **번호**는 위 "병렬 배치" 절이 이미 규칙을 얻었다 —
    `Cycle 번호를 먼저 예약한다`. 레인 A 가 세웠고, 레인 B 는 그것이 서기 전에
    출발해 **번호를 두 번 옮겼다** (C023 → C025 → C025).
