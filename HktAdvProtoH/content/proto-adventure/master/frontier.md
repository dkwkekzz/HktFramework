...
익힌다 (MP-LEARN-…)                 네 칸 중 셋이 섰다 — 살펴봄(C014·C016) · 지목(C017) ·
                                    관계(C018). 남은 것은 **예측 하나** — 근거가 없어 보류, AI 기획서 대기
...
```
    ...
    후보가 둘로 줄어든 이유 — 지목과 관계가 C017·C018 로 닫혔고, 익히는 갈래를 완주시킬
    "예측" 은 근거가 서지 않아 보류되었다 (아래 "지금 열 수 없는 것"). 남은 둘은 각각
    새 갈래를 여는 첫 칸이며, 어느 쪽도 다른 쪽을 요구하지 않는다.
...
| 기능 / 층 | 무엇이 막고 있는가 |
|---|---|
| 제작 (재료 → 결과물) | 후보 2(쓴다는 개념)가 먼저다. 그 뒤에는 바로 후보가 된다 |
| 장비 (걸치고 유지한다) | 같음 — 후보 2 다음 칸 |
| 감정 도구로 아는 경로 | 같음 — 후보 2 다음 칸. C016 이 부분 공개를 이미 세웠다 |
| 관통·치명을 얻는 경로 | 같음 — 후보 2 다음 칸. C013·C015 가 각각 남긴 결손이며 셋이 묻는 것은 하나다: **얻은 것이 나를 바꾸는가**. C016 이 형태의 선례를 만들었다 (능력치가 Capability 의 문을 연다) |
| 능동 방어 (완벽한 막기·되받아치기·Break) | 아래층은 C013·C015 로 섰다. 이제 막는 것은 **그 층의 설계 문서**다 — 두 전투 문서는 이름만 예고한다. 후보 1(행동 구간)이 서면 시점 판정의 바닥이 생긴다 |
...
**기다리는 기획 문서 — AI (자율 존재의 행동)**: C018 이 **지키는 자리**(GuardedGround)와
**물러나면 풀린다**를 세워 목적의 첫 칸이 코드에 생겼다. 그러나 그 자리 안에서 자율 존재가
하는 일은 여전히 하나다 — 가장 가까운 사냥감을 무조건 쫓아 언제나 같은 것을 휘두른다
(`world/simulation/npc-decide.ts`). 목적은 있고 **목적이 행동을 고르는 경로가 없다.**
근거가 될 문서도 없다 — BW 가 이 자리에 대해 말하는 것은 §21 · §26 · §27 셋뿐이고 셋 다
그 존재가 **무엇을 하는가**를 정하지 않는다. 그래서 `MG-HOLD-HUNTING-GROUND` 은 지금도
**갈래(Possibility)가 없는 유일한 Goal** 이며, 그 주석이 "세울 때가 되면 그것은 곧
Creature 의 행동 설계이며 그 층의 문서가 먼저다" 라고 적고 있다.
