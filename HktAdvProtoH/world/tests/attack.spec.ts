...
describe('RULE-ATTACK-001', () => {
  ...
  it('사거리 밖 대상 → Failure(out-of-range) + 사유 투영', () => {
    ...
    expect(attackOn(world.observe(), 'npc-1')?.reason).toBe('out-of-range');
  ...
  it('공격 중에는 이동·공격 요청이 거부된다 (attack 은 Replaceable 이 아니다)', () => {
    ...
    expect(attackOn(world.observe(), 'npc-1')?.reason).toBe('action-busy');
  });
  ...
  it('이동 중에는 공격을 시작할 수 있다 (move 는 Replaceable)', () => {
    ...
    expect(player(world.observe())?.state).toBe('attack');
  });
});
