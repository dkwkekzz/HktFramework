...
describe('RULE-MINE-001', () => {
  ...
  it('곡괭이 없음 → Failure(no-mining-tool), 상태 불변 + 사유 코드 투영', () => {
    const world = createWorld({ ...solo, actorPosition: { x: 8, z: -5 }, actorItems: {} });
  ...
  it('거리 밖 → Failure(out-of-range)', () => {
    const world = createWorld({ ...solo, actorPosition: { x: 0, z: 0 } }); // deposit 까지 10
  ...
  it('자원 고갈 → Failure(deposit-depleted), depleted 상태 관찰', () => {
    const world = createWorld({ ...solo, actorPosition: { x: 8, z: -5 }, depositAmount: 0 });
...
describe('RULE-MINE-COMPLETE-001', () => {
  it('채굴 행동이 소요 시간을 채우면 Stone 1 획득, Deposit 1 감소, 대기 복귀', () => {
    const world = createWorld({ ...solo, actorPosition: { x: 8, z: -5 }, depositAmount: 5 });
  ...
  it('마지막 1개를 캐면 available → depleted 로 전이', () => {
    const world = createWorld({ ...solo, actorPosition: { x: 8, z: -5 }, depositAmount: 1 });
  ...
  it('C001 REGRESSION — 이동해서 광맥에 도달한 뒤 캐면 Stone 을 얻는다', () => {
    const world = createWorld({ ...solo, actorPosition: { x: 0, z: 0 }, depositAmount: 5 });
    ...
    expect(stoneCount(world.projectPlayerView())).toBe(1);
  });
});
