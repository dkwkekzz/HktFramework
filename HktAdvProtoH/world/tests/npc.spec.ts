...
describe('RULE-NPC-DECIDE-001 — 인지 대상이 없을 때', () => {
  it('스스로 순회 지점으로 이동한다 (플레이어 입력 없이 행동이 시작된다)', () => {
    const world = createWorld(wanderingOnly);
    expect(npc(world.projectPlayerView())?.state).toBe('idle');

    world.tick(0.1);
    expect(npc(world.projectPlayerView())?.state).toBe('move');

    for (let i = 0; i < 60; i++) world.tick(1 / 30); // 2초 — NPC 속도 2.5 → 5 이동
    const p = npc(world.projectPlayerView())?.position;
  ...
  it('순회 지점에 도달하면 다음 지점으로 방향을 바꾼다', () => {
    ...
    for (let i = 0; i < 150; i++) {
      ...
      const z = npc(world.projectPlayerView())?.position.z ?? 4;
      if (z < reached) reached = z;
    }

    expect(reached).toBeLessThanOrEqual(-5.9); // 첫 순회 지점에 도달했다
    const after = npc(world.projectPlayerView());
...
describe('RULE-NPC-DECIDE-001 — 인지 대상이 있을 때', () => {
  it('인지 범위 안의 캐릭터에게 접근하고, 사거리에 이르면 공격한다', () => {
    ...
    world.tick(0.1);
    expect(npc(world.projectPlayerView())?.state).toBe('move'); // 접근

    let sawAttack = false;
    for (let i = 0; i < 150; i++) {
      world.tick(1 / 30);
      if (npc(world.projectPlayerView())?.state === 'attack') {
        sawAttack = true;
        break;
      }
    }
    expect(sawAttack).toBe(true);
    expect(npc(world.projectPlayerView())?.targetEntityId).toBe('player');
  });
  ...
  it('대체 불가 행동(공격) 중에는 새로운 결정을 하지 않는다', () => {
    ...
    world.tick(1 / 30);
    expect(npc(world.projectPlayerView())?.state).toBe('attack');

    for (let i = 0; i < 10; i++) world.tick(1 / 60); // 총 0.17초 — ATTACK_DURATION 0.6 미만
    const during = npc(world.projectPlayerView());
