...
// Transition     0. 도착한 참여/이탈 처리 (RULE-OBSERVER-JOIN-001 · RULE-OBSERVER-LEAVE-001)
...
export function ruleWorldTick(
  ...
  const observerResults = observerEvents.map((event) =>
    event.kind === 'join'
      ? ruleObserverJoin(state, event.observerId, bodyDefaults)
      : ruleObserverLeave(state, event.observerId),
  );
