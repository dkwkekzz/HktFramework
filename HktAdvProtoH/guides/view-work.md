...
백로그 항목 형식 — 키는 슬러그다. V 번호는 **착수할 때** 발급한다 (미리 받아 두면
버려진 항목이 번호를 소모한다).

```text
### <슬러그 — 한 줄 이름>
    출처   <UX 문서 §n | Human 지시 | Cxxx 08 | V-NNN REPORT>
    목표   화면에서 확인 가능한 한 문장
    크기   아주 작음 | 작음 | 중간
    의존   없음 | <먼저 있어야 하는 항목·관찰>
    상태   PROPOSED | IN PROGRESS (V-NNN)
```
...
2. 할일을 정한다 — 지정이 없으면 `works/BACKLOG.md` 의 다음 항목. 백로그 밖의 새
   목표면 항목을 먼저 세운다 (위 형식).
3. ID 를 딴다 — `V-NNN` (`works/` 에서 최대 +1. VIEW 레인은 동시에 한 세션이므로
   충돌하지 않는다). 백로그 항목의 상태를 `IN PROGRESS (V-NNN)` 로 바꾼다.
...
7. `works/V-NNN-<name>.md` 를 남기고 **백로그에서 그 항목을 지운다.** 작업 중 발견한
   **세계 관찰의 결손**은 REPORT 절에 적는다 — 스스로 메우지 않는다. 작업이 낳은
   후속 화면 할일은 백로그에 새 항목으로 남긴다.

## Output

`view/` 코드 + `works/V-NNN-<name>.md` + `works/BACKLOG.md` 갱신
(주입만 한 바퀴면 BACKLOG.md 만 — 코드 없이 끝나는 것도 정상이다)
...
# V-001 — reason-tooltip
    ...
    바뀐 표면    <어느 화면·어느 요소>
    이전 → 지금  <같은 자리가 전에 무엇이었고 지금 무엇인가 — 화면의 글자 그대로>
...
- 기존 `works/V-*` 기록을 수정하지 않는다 — History 다. BACKLOG.md 는 살아 있는
  문서다 — 완료·경위를 쌓지 않고, 닫힌 항목은 지운다.
