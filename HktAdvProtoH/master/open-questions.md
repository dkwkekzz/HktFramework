...
## Q14. 심연(Abyss) 예시의 지위 — 삽화인가 세계 설정의 씨앗인가 — OPEN

    무엇          GR 은 심연·가론·심연의 버서커 계보(§24.2 · §30 · §36)로 구조를
                  설명한다. 이번 주입에서는 삽화로 판정해 그래프에 옮기지 않았다 —
                  현재 세계에 심연이라는 세계 사실(MW-*)이 존재하지 않고,
                  문서도 이것을 확정 설정으로 선언하지 않기 때문이다.

    영향          비주입 자체는 안전하다. 다만 Human 이 심연을 실제 세계관의 씨앗으로
                  쓸 생각이었다면 그 결정이 기록 없이 사라진다.

    선택지        (a) 삽화로 확정 — 실제 세계관은 root.md 부터 새로 정한다 (Q2)
                  (b) 씨앗으로 채택 — root.md World Premise 에 심연 계열 성질을 쓰고
                      WHY 단계가 MW-ABYSS-* 를 세운다 → 첫 CL/IT 노드의 무대가 된다
