// World Link 단독 테스트 — 소켓 없이 이어짐 상태를 검증한다 (C003).
// Implements INTENT-OBSERVER-LINK-001
...
const snapshot = (time: number): GameViewSnapshot => ({
  ...
  scene: 'mining-field',
  entities: [
    { id: 'player', role: 'player-character', state: 'idle', position: { x: time, z: 0 } },
  ],
  interactions: [],
...
describe('INTENT-OBSERVER-LINK-001 — 이어짐 상태', () => {
  it('처음에는 잇는 중이고, 열리면 이어짐이 된다', () => {
    const w = fakeWorld();
    const link = createWorldLink(w.factory, w.schedule);
  ...
  it('끊긴 동안에는 요청을 보낼 수 없다', () => {
    ...
    expect(JSON.parse(w.sent[0]!)).toEqual({
      type: 'action',
    ...
    expect(w.sent).toHaveLength(1); // 늘지 않았다
  });
  ...
  it('망가진 메시지는 무시된다 — 화면이 깨지지 않는다', () => {
    ...
    createWorldLink(w.factory, w.schedule);
...
describe('조용히 죽은 이어짐', () => {
  it('관찰 결과가 한동안 오지 않으면 끊긴 것으로 본다 (close 를 못 받아도)', () => {
    const w = fakeWorld();
    let clock = 1000;
    const link = createWorldLink(w.factory, w.schedule, () => clock);
    w.open();
    w.push(5);
