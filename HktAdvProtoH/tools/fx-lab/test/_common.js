...
// ROOT = 프로젝트 루트. 랩(tools/fx-lab/)과 기반 런타임(engine/view-kernel/fx/splat/)이
// 서로 다른 자리에 있으므로 둘을 함께 담는 자리를 서빙한다.
const ROOT = path.join(__dirname, '..', '..', '..');
const LAB = '/tools/fx-lab';                           // 랩 페이지·에셋의 자리
const SPLAT = '/engine/view-kernel/fx/splat';          // 스플랫 런타임의 자리 (기반)
...
function serve(port, routes) {
	const server = http.createServer((req, res) => {
		...
		const p = path.join(ROOT, url === '/' ? `${LAB}/index.html` : url);
...
// 엔진 직접 구동용 최소 페이지 (app 없이 모듈만). HktSplatLife 는 built-in 스켈레톤만 쓰므로
// three(FBX 전용)는 불필요 — 플랫 js/ 경로로 7개 모듈만 로드한다.
const HARNESS_ROUTE = (req, res) => {
	...
	res.end('<!doctype html><meta charset="utf-8"><canvas id="gpu" width="640" height="640"></canvas><canvas id="c2d" width="640" height="640"></canvas>'
		...
		+ `<script src="${SPLAT}/wgsl.js"><\/script><script src="${SPLAT}/engine.js"><\/script><script src="${SPLAT}/fx.js"><\/script>`);
};
...
module.exports = { serve, launch, collectErrors, savePng, ROOT, LAB, SPLAT, HARNESS_ROUTE, DRIVE_AND_SHOOT };
