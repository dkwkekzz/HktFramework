...
검증 완료된 Cycle의 Semantic을 `state/WORLD-BASELINE.md`에 병합하고 Cycle을 닫는다.

## 입력 (Gate — 모두 충족해야 진행 가능)

- `state/cycles/cycle-XXX/05-verification-report.md` — 4개 Closure 모두 통과
- `state/cycles/cycle-XXX/06-evolution-review.md` — PASS

## 출력

- `state/WORLD-BASELINE.md` — 버전 증가 (vN → vN+1), Supported State / Rules / Goals / Possibilities / Observable 갱신, History 행 추가
- `state/CURRENT-CYCLE.md` — "활성 Cycle 없음" 상태로 초기화 (다음 Contract 대기)
...
Baseline 병합 + CURRENT-CYCLE 초기화 후 STOP. 다음 Cycle의 Scope Definition을 이어서 실행하지 않는다.
