...
- **분리 완료** — HktSplatGenesis 의 무대 절반을 자립 실행판으로 떼어냈고, 문서(CLAUDE·STATE·
  ...
  동작 의존은 없다.
- **퀄리티 트랙 진행 중** — 목표 레퍼런스는 스타일라이즈드 오픈월드(원신급 vista). 진단 결론:
  품질 병목은 렌더러가 아니라 굽는 데이터(스플랫 크기·정렬·색 디테일·조명 bake).
  - 1묶음(E14·E15) 완료: 표면 정렬 surfel + 밀도 상향 + 경사 재질·알베도 디테일.
    기본 진입(env-app)이 temperate mood(하늘 돔·구름·fog)를 배선하도록 함께 정리(E9 활용).
  - 다음: ② 식생 스플랫 품질(E13 — 나무가 현재 최대 병목: 단색 블롭 14개) ③ 조명 bake
    심화(태양·하늘색 2색 분리, cast shadow, AO) ④ 원경 fog 프리블렌드(지평선 경계 완화).
- **검증**: `test/env-shot.js` OK (타일 25 · 스플랫 184k · 콘솔 오류 0).
