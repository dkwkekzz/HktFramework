...
> **범위 경계(인덱스)**: HktInfra = 이벤트 전송·클라 전파(netcode)만 — 결정론 시뮬 *내부 구현*은 범위 밖(이음새 뒤 HktGameplay). 전문·강제 숙지는 **[SPINE.md](SPINE.md) §0 범위 경계**(step 시작 전 필독).
...
- **headless·원격 검증(전송·전파 서버, 협상 불가)**: HktInfra 가 *짓는* 박스(전송·전파)는 UE·GUI 없이 headless 로 원격(이 환경/CI)서 빌드·통신·검증된다(더미=Node 자명 충족). 시뮬 *내부 구현*은 범위 밖 — 자세히는 [SPINE.md](SPINE.md) §0.
...
**산출물 형태 = 헤드리스 프로토타입으로 *netcode 프로토콜·토폴로지*를 증명**. HktInfra 는 전송·전파 계약을 결정론적 헤드리스 하니스로 증명한다(원격 검증 루프 보존, [TOOLS.md](TOOLS.md) §4). 프로토타입의 더미 시뮬은 *전파 자극용 영구 stub*(교체 대기 임시물 아님) — 결정론 시뮬 코어는 범위 밖([SPINE.md](SPINE.md) §0).
