...
# ⛔ 범위 경계 (SCOPE — 반복 오해 금지·미래 세션 필독)

> **HktInfra 는 *netcode/transport* 다 — 이벤트를 전송하고 클라이언트에 전파하는 것만 구현·검증한다.** 토폴로지·라우팅·pub/sub·AOI·핸드오프·복제 전파·영속·코디네이션이 전부. **결정론 시뮬레이션의 *내부 구현*은 HktInfra 범위가 아니다.**
>
> 시뮬은 동결된 `ISimCore` 이음새 *뒤*에 사는 **블랙박스**이고 **HktGameplay(C++ HktCore)의 소관**이다. HktInfra 는 시뮬을 *이벤트 소스/싱크*로만 다룬다 — intent 를 받아 이음새로 넘기고, 그 결과(이벤트)를 받아 클라에 전파한다. 프로토타입의 더미 시뮬(위치맵+AOI)은 *전파를 자극하기 충분한 최소 stub* 일 뿐, HktInfra 가 짓는 결정론 코어가 아니다.
>
> **따라서 "C++ 시뮬 코어를 headless 로 빌드/분리/순수화(UObject 0)"는 HktInfra 의 과제가 아니다** — 그건 HktGameplay 의 일이다. HktInfra 의 headless·원격 검증 요구는 *전송·전파 서버*(로그인·게이트웨이·존-as-이벤트-릴레이·서비스·버스·코디네이션·데이터)에만 걸린다. "결정론"은 HktInfra 맥락에선 *같은 이벤트 스트림이 같은 클라 뷰로 전파/수렴*(재현·desync 0)을 뜻하지, 시뮬 내부 계산을 HktInfra 가 구현한다는 뜻이 *아니다*.
>
> **남은 HktInfra 작업에 "C++ 시뮬 코어"를 넣지 마라.** 존 박스에서 HktInfra 가 할 일은 *이벤트를 받아 클라들에게 전파하는 배선*뿐이다.
...
- **진행은 단계적(너비→고도화)**: 1차는 모든 박스의 *기본 통신*부터, 심화는 2차. **단계 판정**은 `infra-review`(너비 지도 `reviews/progress/` 기준), **다음 조각**은 [STATE.md](STATE.md) §2. 기준·절차 세부는 스킬에 있다(이 문서는 인덱스만).
...
- **headless·원격 검증(전송·전파 서버, 협상 불가)**: HktInfra 가 *짓는* 박스(로그인·게이트웨이·존-as-이벤트-릴레이·서비스·버스·코디네이션·데이터)의 동작 검증은 Unreal Engine·GUI 없이 headless 로, 원격 환경에서 가능해야 한다(더미 단계 Node 라 자명 충족). **시뮬 *내부 구현*은 범위 밖**(⛔ 범위 경계) — `ISimCore` 이음새 뒤 블랙박스(HktGameplay C++ HktCore 소관)이며, 그 headless 빌드/UObject 0 순수성은 HktInfra 의 통과 조건이 *아니다*. HktInfra 가 시뮬에 대해 지킬 것은 단 하나: *이음새를 통해 이벤트만 주고받고 클라에 전파한다*(시뮬 코드를 끌어안지 않는다). "UE 없이, 원격에서, 한 줄로 띄워 빌드·통신·검증되는가"는 *전송·전파 박스*의 통과 조건 ([TOOLS.md](TOOLS.md) §1·§4).
- **코어 netcode 불변(위반 금지·HktInfra 가 *지키되 시뮬 코어는 안 짓는다*)**: ① 서버 권위(클라는 읽기전용 뷰만) ② ISP 3-Layer(Intent→Sim→Presentation 역방향 의존 금지) ③ 시뮬 상태 직접쓰기 금지(intent 경유). 인프라는 이 불변들을 *확장*하되 *깨지 않는다*. (시뮬 코어의 결정론·UObject 0 순수성은 이음새 뒤 HktGameplay 의 불변 — HktInfra 는 이음새를 *존중*할 뿐 그 코어를 구현·headless 화하지 않는다.)
...
**산출물 형태 = 헤드리스 프로토타입으로 *netcode 프로토콜·토폴로지*를 증명**. HktInfra 는 전송·전파 계약을 결정론적 헤드리스 하니스로 증명한다(원격 검증 루프 보존, [TOOLS.md](TOOLS.md) §4). 프로토타입의 "VM"·더미 시뮬은 *netcode 불변을 자극하기에 충분한 최소 stub* 이며 — **그 자리에 들어갈 무거운 결정론 시뮬 코어(C++ HktCore)는 `ISimCore` 이음새 뒤 HktGameplay 의 일이지 HktInfra 의 "C++ 교체 단계"가 아니다**(⛔ 범위 경계). HktInfra 에서 더미는 *교체 대기 임시물*이 아니라 *영구 전파 stub* 으로 본다.
