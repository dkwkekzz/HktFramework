...
- **닫힌 step**: [step-0023](step-0023.md) — 저널 홉 신뢰 전달 (`InventoryService.sentBuffer`/`_resend` + `PersistStore` 갭 감지/NAK/dedup — 가방 write-behind 저널 홉의 0008 판). STATE §2 ⒜(exactly-once 내구성)의 sub-piece "저널 홉 ack/resend 만": journalReliable OFF 면 0022 비트 동일(reg 0) · ON 이면 저널 홉 손실 아래 **PersistStore 가 seq 갭을 감지→NAK**(`[0..maxRecvSeq]` 미수신 색출, `m.from` 에 발신)→**inventory 가 sentBuffer 에서 재전송**→PersistStore 가 `recvSeqs` 로 **멱등 수신**(중복 push 0). 핵심=**갭 감지는 수신측의 일**(seq 빈칸은 받는 쪽이 안다 → 양쪽 반응형·onTick 0·신성한 tick 보존)·**at-least-once 전송 + 멱등 수신 = effectively-once 영속**(seq=0017 단조 순번이 멱등 키)·**손실 아래에서도 저널 완전→복구 무손실**. 더한 것은 inventory sentBuffer/_resend + persist 갭감지/NAK/dedup 뿐·host.js/`engine/` 무수정.
- **한 줄 상태**: write-behind 저널 홉이 전송-신뢰화 — 손실 아래에서도 영속 완전·복구 무손실. 핵심: ① **갭 감지=수신측 일**(NAK·양쪽 반응형·신성한 tick) ② **at-least-once+멱등=effectively-once**(재전송+recvSeqs dedup·0017 단조 seq 가 토대) ③ **write-behind 신뢰성의 *전송-손실 절반* 해소**(in-flight·tail 손실은 남김). 검증: 회귀 0(`reg` 25/25 → 0022 비트 동일·persist+rel+rst 로 무손실 휴면 직접 증명)·가설(`reliable`, 신뢰 ON 저널 완전·복구 무손실(invDigest==무손실 기준) / OFF **16~18개 갭**·복구 손실·NAK 166~187·재전송 223~239)·E2E(`e2e`, chat-restart 14 프로세스·신뢰 휴면 비트 동일)·sacred/degrade/hide/repro 유지·spine 23-step 사슬. **단 (a) tail 손실 미감지(NAK-only 본질·heartbeat 필요)** **(b) in-flight 손실 절반(crash 가 sentBuffer 비움)·클라 resend 미착수** **(c) NAK 폭주(매-수신 재-NAK·재전송 13배 증폭)·압축×신뢰 미결합·채팅 홉 미적용** **(d) 버스/월드 영속 0·Node·C++ 시뮬 미교체·lockstep 배리어**.
- **다음**: step-0024 (§2).

---

## 2. NEXT — step-0024 가설 (후보, 권위는 이 절)

**step-0023 이 저널 홉을 전송-신뢰화(갭 감지 NAK + 재전송 + 멱등 dedup)해 write-behind 신뢰성의 *전송-손실 절반*을 닫았다. 남은 격차는 명확하다. 가장 자연한 다음 한 조각은 ⒜ *write-behind 신뢰성의 나머지 절반* — in-flight 손실(활성 중 가방 죽음=sentBuffer 미-ack 소멸) 또는 tail 손실(heartbeat/ack-timeout 으로 마지막 항목 감지). 또는 ⒝ *버스 영속·failover*(서비스 복구 패턴의 버스 판), 또는 ⒞ *월드 영속(존 intent 로그)*(계층 6 월드 판 — 단 클라 seen 의 tick-결합 재구성 = 고위험·풀 tick-replay 필요), 또는 ⒟ *활성 중 서비스 다운타임 + 재발행*(quiescent 한계 — 클라 resend 로 belief 재수렴).**

남은 격차: ⓐ *write-behind 윈도의 나머지 절반* — in-flight 손실(crash 가 sentBuffer 비움)·tail 손실(NAK-only 미감지·0023 §9) — exactly-once 의 잔여 메커니즘(클라 resend·write-ahead·heartbeat) ⓑ 버스·월드는 *영속 0*·서비스 복구는 *활성 중 다운타임 미검증*(0020~0022) ⓒ 스냅샷 *비유계*·압축 *단일 트리거*(0018/0022)·NAK 폭주·압축×신뢰 미결합(0023) ⓓ PersistStore 가 *단일점*. **권위는 이 절이되, 다음 step 시작 시 한 조각으로 좁힐 것**. ⒜ 의 tail-손실 sub-piece(heartbeat/스냅샷 fence)는 0023 위에 *작은 한 조각*(저난도). in-flight sub-piece(클라 resend)는 중난도. ⒝ 는 검증된 영속·복구 패턴의 버스 판이나 *버스 라우팅이 정적 spec* 이라 동적 구독 선결 필요. ⒞ 는 *고위험*(tick-결합 코어·클라 seen 재구성). 큰 조각(비동기 결정론·C++ 시뮬 교체)도 여전히 강한 후보.

**검증할 것(후보 — 영속 신뢰성 나머지·버스 영속 기준)**: ① **회귀 0** — 새 항 OFF 면 0023 비트 동일. ② **신성한 tick** — 새 항(heartbeat/resend·영속)이 존 tick 밖·존 비-침습. ③ **E2E 동치** — 멀티프로세스=인프로세스 비트 동일·은닉. ④ **가설** — ⒜-tail 이면 "heartbeat 로 tail 손실 감지→재전송→저널 완전(0023 의 tail 사각 해소)", ⒜-inflight 면 "활성 중 kill→클라 resend 로 in-flight 손실분 belief→원장 수렴·dupe 0", ⒝ 면 "버스 라우팅 kill→재구성 후 결합 0 보존".

**병행 백로그(블로킹 아님)**: ⬜ 저널 홉 tail 손실 해소·heartbeat/ack-timeout(0023 §9·저난도) · ⬜ in-flight 손실 + 클라 resend·exactly-once 나머지(0023 §9) · ⬜ 버스 영속·failover(0016 §9-2·0017 §9·동적 구독 선결) · ⬜ 활성 중 서비스 다운타임 + 재발행(0020~0022 §9) · ⬜ 읽기 모델 증분 follow·자체 스냅샷(0020 §9) · ⬜ 증분 스냅샷·압축 트리거 다양화·NAK 배칭·압축×신뢰 결합(0018/0022/0023 §9) · ⬜ 월드 영속(존 intent 로그·크래시 복구·late-join — 계층 6 월드 판·tick-결합 고위험) · ⬜ PersistStore failover·디스크 fsync·다중 복제(0017 §9 — 영속 단일점) · ⬜ 채팅 저널 홉 신뢰 전달(0023 의 채팅 판) · ⬜ 버스 분산·failover·동적 구독·이력/replay(0016 §9-2·§9-3) · ⬜ 런타임 consume→publish 사이클 정적 탐지(0019 §9) · ⬜ 거래소·우편(서비스 분리 패턴 반복) · ⬜ 존 넘는 거래·2-party 동의·에스크로(0014 §9-2) · ⬜ 비동기 실행 아래 결정론(논리 클럭, 0012 §9-3·0013 §9-1) · ⬜ 분산 시스템 심화 묶음(다중-브로커 버스 분산·분산 epoch·거짓 사망 플래핑·부분 복귀·다중 standby·디스커버리·소켓 층 비대칭 분단 — 0012~0013 §9) · ⬜ 월드 심화 묶음(다중 클라 복제·예측[C++ 승격]·존 N개·동적 경계·존↔존 ghost 증분화 — 0006~0008 §8) · ⬜ 서버간 인증·재접속·티켓 만료(0001 §8.3·§8.5). (✅ 해소분 0014~0023 = §3 ✅ 묶음·§7 INDEX 참조.)

**빌드 인프라 — `engine/` 추출 완료(0004), 0005~0023 무수정 재사용**: 공통 하니스(시드 PRNG·FNV·전송 모델 `Net`·엣지/코디 스텁·동결 `ISimCore` 2구현 = `engine/index.js`, 시각 관찰 키트 = `engine/panel-kit.js`)를 `engine/` 한 곳에. 코어는 **dual-mode**(Node `require` + 브라우저 `<script>` 전역). 0023 은 `engine/`·host.js·cluster.js 무수정(신뢰 opt 는 spec 직렬화로 자동 흐름·NAK 는 기존 라우팅 재사용)·존/orch/추종자/버스/감사/랭킹/채팅 영속·압축 안무 무수정 — net-core 는 0022 그대로 + **InventoryService `sentBuffer`/`_resend`**(미-ack 보존·NAK 응답 재전송)·**PersistStore 갭감지/NAK/dedup**(`recvSeqs`·`[0..maxRecvSeq]` 미수신 색출·멱등 수신)만 더함. 손실은 `routeFilters.persist`+redundancy 1+loss 로 전송층 주입. step-0024 의 net-core 출발 템플릿은 `step-0023/net-core.js`·`step-0023/cluster.js`·`step-0023/host.js`.
...
| 🟡 | **서비스 영속·failover (가방 ✅+압축 ✅+저널홉 신뢰 ✅·채팅 ✅+압축 ✅·버스 ⬜)·존 넘는 거래** | 서비스/데이터 | 0017 가방 원장 = event sourcing 효과 저널(복구 투명)·0018 스냅샷 압축. 0021 채팅 = *커맨드 로그* event sourcing·0022 라우팅 스냅샷 압축. 0023 *저널 홉 신뢰 전달*(갭 감지 NAK+재전송+멱등 dedup → 전송 손실 아래 저널 완전·복구 무손실). 단 write-behind 윈도의 *나머지 절반*(in-flight 손실·tail 손실 — 0023 §9)·버스 라우팅은 *영속 0*·채팅 홉 신뢰 미적용. 가방 일방 give(2PC 없음 — 0014 §9-2). |
...
| 3 | 게임 서비스 | 가방 · 채팅 · 길드 · 거래소 · 우편 · 랭킹 | 🟡 0014 가방(InventoryService) 첫 박스 — 단일 소유·쌍 거래·dupe 0 +0015 채팅(ChatService) 둘째 박스 — 채널 팬아웃·지역 격리 +0016 서비스 경로가 버스 발행/구독으로 + AuditService(관찰 소비자) +0017 가방 failover·영속(kill→저널 replay) +0018 저널 스냅샷 압축 +0019 **RankingService**(발신하는 소비자 — svc.item.out 소비→rank 투영→svc.rank.out 발행·CQRS 읽기 모델) +0020 **읽기 모델 영속·late-join**(랭킹 crash→쓰기 저널 reconstruct·자기 영속 0·투영==원장) +0021 **채팅 영속·failover**(커맨드 로그 replay·라우팅+deliveries event sourcing 복구·*서비스 복구 3부작 완성*) +0022 **채팅 커맨드 로그 스냅샷 압축**(라우팅 스냅샷+tail replay·무손실·효과/커맨드소싱 둘 다 압축 완성) +0023 **저널 홉 신뢰 전달**(가방 저널 홉 갭 감지 NAK+재전송·전송 손실 아래 저널 완전·복구 무손실). 전부 별 프로세스·tick 무관 순수 반응형(onTick 0)·신성한 tick·E2E 비트 동일·은닉. 활성 중 다운타임+재발행·write-behind 나머지 절반(in-flight·tail)·버스 영속·거래소/우편/길드 후속 |
