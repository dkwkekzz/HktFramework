...
- **닫힌 step**: [step-0105](step-0105.md) — **active 공지 epoch 펜싱**(announceEpoch·메아리 정리): 0070 presmon 은 svc.presence.active 공지를 무조건 재타깃 → 낡은/메아리 공지가 죽은 박스로 역-재타깃(0068 §9). 공지에 epoch(=promotedAt·단조)를 실어 presmon 이 본 최고 epoch 이하를 메아리로 거부·더 높은 것만 재타깃 → 디스커버리 split-brain 방지(0013/0090 의 읽기-디스커버리 판). epoch 없으면 무조건 재타깃=0070 비트 동일. 닿는 박스: svc-presence(announceEpoch)·svc-presence-monitor(activeEpoch·staleAnnounces)·topology(presAnnounceStraggler 훅)·topo-build(announceEpoch).
- **한 줄 상태**: reg ALL OK(src=baseline=0104 비트 동일·월드해시 `0x7a122947`(seed42)… 보존)·E2E 14프로세스 비트동일·pannep: 공지 2건(t70 presence2·e30·t72 메아리 presence·e10)·ON presence2 유지/retargets 1/stale 1 vs OFF presence 역-재타깃/retargets 2/stale 0·minted ON==OFF·spine 105 OK.
...
**step-0105 가 *active 공지 epoch 펜싱*(메아리 정리·0068 §9)을 닫음. 기능 후보: *파티 cluster kill→replay*(0085 §9)·*거래소/우편/길드*(새 게임 서비스 박스)·*논리 클럭 term*(0105 §9·🔴)·*비동기 결정론*(🔴·0012 §9-3). 🔧 buildTopology 24KB 단일 함수 — 더 자라면 구독 테이블 분리. 🔎 0101~0110 묶음 리뷰는 0110.**
...
| 3 | 게임 서비스 | 가방 · 채팅 · 길드 · 거래소 · 우편 · 랭킹 | 🟡 0014 가방·0015 채팅(단일 소유·쌍 거래·팬아웃·격리)→0016 버스+audit→0017~0022 가방/채팅 영속+압축·ranking·읽기모델→0023~0032 write-behind 신뢰성·persist failover/N-replica/quorum/윈도→대체 소비자 호(0061~0063). **0071~0103 귓속말/파티 라우팅 호(wrouter)**: 라우팅·failover·파티 1:N·멤버십 SSOT·전달 신뢰(0076~0082)·파티 집계/영속/압축(0083~0086)·ack 집계·epoch 펜싱(0087~0091)·파티 종결/발행(0092~0097)·수신함 유계/드레인/읽음확인/소비발행(0099~0103·상세 §7). 전부 별 프로세스·신성한 tick·권위 0. 거래소/우편/길드 후속 |
| 4 | 버스 | 이벤트 버스 | 🟡 0004 전송 substrate→0012 토픽 pub/sub→0016 ServiceBus(발행자 무수정 소비자)→0019 발신 소비자→0033 동적 구독→0034 failover(진실원천=소비자)→0036/0037 결과/요청 무손실(producer replay)→0039~0042 replay 유계·ack 자기조정→0044 min-워터마크→0045 lease→0046/0047 producer ns→0048 lifecycle→0050~0052 적응형 leaseSpan/grace/cadence→0054 관측. 분산·per-producer ack·라우팅 영속 후속 |
| 5 | 코디네이션 | 세션/프레즌스 · 오케스트레이터 | 🟡 0001 레지스트리 +0009 Orchestrator(lease·failover) +0010~0013 broker(lockstep→TCP→버스 허브·분단/펜싱·kill·split-brain 0). 0054~0063 lease 관측→프레즌스 SSOT→self-healing. 프레즌스 박스 호(0064~0070): 박스 분리(⟂orch)→버스화→shadow→failover 승격→사망 자율 감지→질의·질의 failover →0105 공지 epoch 펜싱(메아리 정리). broker 물리 분산·진짜 비동기 후속 |
...
| [0080](step-0080.md) | 수신측 dedup(deliverDedup — Mailbox seq 기억·중복 재적재 차단·재-ack·exactly-once) | 통과 · ON dup 1 vs OFF 0 |
...
| [0083](step-0083.md) | 파티 1:N 영수증 집계(partyReceipt — partyId 별 {members,routed,bounced} 원장·N-of-M) | 통과 · ON {3,2,1}/done vs OFF 0 |
| [0084](step-0084.md) | 증분 가입/탈퇴+변경 발행(partyChange — Join/Leave 델타·svc.party.changed) | 통과 · ON 발행/audit 2 vs OFF 0 |
...
| [0087](step-0087.md) | 전달 수명주기 관측(deliveredPublish — whisperAck→svc.whisper.delivered·audit) | 통과 · ON pub/audit 1 vs OFF 0 |
| [0088](step-0088.md) | 파티 ack 집계(partyAckTally — whisperAck→delivered·acked=delivered==routed) | 통과 · ON acked vs OFF 0 |
| [0089](step-0089.md) | producer epoch 워터마크(epochKeyed — restart 시 epoch++·Mailbox (prod,epoch) 키) | 통과 · ON rec 12/dup 0 vs OFF 6/6 |
| [0090](step-0090.md) | epoch 워터마크 유계화(epochBound — 높은 epoch 도착 시 낮은 epoch 가지치기) | 통과 · 3재시작·ON epochKeys 1 vs OFF 4·rec 8/dup 0 |
...
| [0096](step-0096.md) | 멤버별 Mailbox 토폴로지(mailbox2 — 둘째 수신함·파티원마다 ack·0088 §9 해소) | 통과 · ON deliv 2/acked true vs OFF 1/false·spine 96 |
...
| [0105](step-0105.md) | active 공지 epoch 펜싱(announceEpoch — presmon 이 낡은 svc.presence.active 메아리 거부·역-재타깃 0·0013/0090 디스커버리 판) | 통과 · ON presence2/retargets 1/stale 1 vs OFF presence 역-재타깃/retargets 2·spine 105 |
