...
- **닫힌 step**: [step-0253](step-0253.md) — **캐시 bulk get(cacheMget)**. {keys[]} 한 요청에 여러 키 read-through 일괄 조회 → cacheMReply{values[]} 한 회신(라운드트립 N→1·배치 페치). 새 메시지 타입·미수신→0252 비트 동일. 4차 고도화 캐시 #2.
- **한 줄 상태**: reg ALL OK·cachemget: 5/5 mget[k1,k2,k3,k4]=[v1,v2,s3,undefined]·hits 2/misses 2·`run.js all` ALL OK·spine ALL OK.
