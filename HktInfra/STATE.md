...
- **닫힌 step**: [step-0035](step-0035.md) — **정리 step: `cluster.js` 박스-부품 4분할(45KB>30KB 트리거·기능 0·reg 0)**: broker 드라이버 cluster.js(45,293B)가 30KB 비대화 트리거 초과 → `net-core.js` 패턴으로 진입점 1 + 부품 4 분할. cluster-wire(frameOf·Framer)·cluster-core(Cluster 클래스)·cluster-reconstruct(reconstruct)·cluster-run(computePlacement·runMulti). 코드는 *바이트 동일*(sed verbatim 이동), export 집합 불변(`{runMulti,Cluster,computePlacement,frameOf,Framer}`), topology.js `require('./cluster.js').runMulti` 무변경. 0030 net-core 분할의 cluster 판.
- **한 줄 상태**: 최대 박스-부품 cluster-core 19.7KB(<30KB)·step 디렉토리 272KB(<300KB). reg(인프로세스) net-core 무관 휴면·**분할 투명성 = 멀티프로세스 E2E 비트 동일**(`runMulti`==`run()`·전 시드 true)·busfail(0034) 통과. 18+1모드 ALL OK·spine **35-step 사슬**·close-step 게이트 통과.
- **다음**: §2 참조 — 기능 트랙 복귀: 버스 failover 무손실화(요청경로 in-flight 재발신→gap desync 0) · 비동기 결정론(🔴 최우선) · 활성 중 다운타임 일반 재발행. 다음 정리 후보: topology.js 29.7KB(30KB 근접).

---

## 2. NEXT — step-0035 후 가설 (후보, 권위는 이 절)

**step-0035 가 *정리 step*(cluster.js 45KB>30KB 트리거 → 박스-부품 4분할·기능 0·reg 0)을 끝냈다 — 복사 전진 페이로드 유계화. 기능 트랙 복귀. 0034 가 버스 *failover*(구독 재협상으로 라우팅 복구)를 깔았으나 crash gap 의 in-flight 메시지는 at-most-once 손실(요청 드롭 desync=6·손상 아닌 손실). 다음 후보: ⒝ *버스 failover 무손실화*(요청 경로 in-flight 재발신 = 0023 홉 NAK·0025 give-resend 의 버스 판 → gap desync 0, 또는 라우팅 테이블 영속·이력 replay·0016 §9-2), ⒞ *활성 중 서비스 다운타임 일반 재발행*(0025/0026 은 quiescent — 온라인 kill+재발행 핸드셰이크), ⒟ *비동기 결정론(lockstep 배리어 해제)*(논리/벡터 클럭·🔴 최우선·broker=cluster-core.js/cluster-run.js 대공사·0012 §9-3), 🔧 *다음 정리 후보*(topology.js 29.7KB — 30KB 근접·기능 step 이 키우면 재분할).**
...
| 🟡 | **버스 단일점·분산·영속(동적 구독 ✅·failover 재협상 ✅)** | 버스 | 0016 ServiceBus = *단일 박스·영속 0*. **0033 동적 구독/해지 ✅**(런타임 `unsub`/`sub`·양방향 라우팅) + **0034 failover ✅**(crash→소비자 재구독→routing 재구성·진실 원천=소비자·버스 영속 불필요). 남은 것: crash gap 의 in-flight *요청* 드롭 무손실화(desync=6 — 요청경로 재발신·0034 §9)·라우팅 영속/이력 replay 0·구독자 장애 감지 0·다중 브로커 분산(0012 §9-1). 월드·제어 평면·존간 핸드오프는 직접 유지(의도). |
...
| 4 | 버스 | 이벤트 버스 | 🟡 0004 전송 substrate(`engine/Net`) +0008 핸드오프/델타 라우트 +0010 IPC +0011 실 TCP 소켓 +0012 토픽 pub/sub 버스(*전송*·소켓 열화 내성) +0016 **서비스 의미**(ServiceBus — 발행/구독 계약·직접 결합 0·발행자 무수정 소비자 추가) +0019 **발신 소비자**(ranking consume→publish 루프·발행자 무수정·루프 없음) +0033 **동적 구독/해지**(runtime `unsub`/`sub` — 라우팅 테이블 양방향 변경·토글은 그 소비자만·공동 구독자/발행자 비트 동일) +0034 **failover**(`bus.crash()`→소비자 재구독으로 routing 재구성·진실 원천=소비자·버스 영속 불필요·gap=at-most-once 손실·원장 valid). 버스 단일점 제거(분산)·요청경로 무손실화·라우팅 영속·이력/replay·런타임 사이클 탐지·서버간 인증 후속 |
...
| [0034](step-0034.md) | 버스 *failover*(`bus.crash()`→구독 재협상으로 라우팅 복구 — 진실 원천=소비자·버스 영속 불필요·0016 §9-2) | 통과 · busRestart 0=0033 비트 동일(reg 25/25)·crash@12→재협상@14 → A0=60/crashOnly=6(subN 0·영구 단절)/recover=30(subN 3 복원·팬아웃 재개)·ranking 60→6→27·원장 valid(보존·정합)·요청 드롭 desync=6(손상 아닌 손실)·spine 34-step 사슬 |
| [0035](step-0035.md) | 정리 step: `cluster.js` 박스-부품 4분할(45KB>30KB 트리거·기능 0·바이트 동일·export 불변 — 0030 net-core 분할의 cluster 판) | 통과 · 진입점 1+부품 4(wire·core·reconstruct·run)·최대 cluster-core 19.7KB(<30KB)·디렉토리 272KB·분할 투명성=멀티프로세스 E2E 비트 동일(전 시드)·18+1모드 ALL OK·spine 35-step 사슬 |
