...
- **닫힌 step**: [step-0256](step-0256.md) — **캐시 per-key TTL(cacheSetEx·SETEX)**. 키별 만료 수명(keyTtl) 저장·cacheExpire 스윕이 per-key ttl 우선(없으면 글로벌·차등 만료). keyTtl 비면→0255 비트 동일. 4차 고도화 캐시 #5.
- **한 줄 상태**: reg ALL OK·cachesetex: 5/5 k1(ttl2) 만료·k2(글로벌10) 생존·evicted 1·`run.js all` ALL OK·spine ALL OK.
