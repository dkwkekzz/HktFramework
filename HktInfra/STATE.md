...
- **닫힌 step**: [step-0034](step-0034.md) — **버스 failover: 구독 재협상으로 라우팅 복구**: 0033 동적 sub 의 failover 용례. `ServiceBus.crash()`(라우팅 RAM 소실 = 서비스 경로 단절) + 복구는 *소비자 재구독*(정적 subs 재발신). 핵심 통찰: 버스가 드는 라우팅은 *파생 상태*(진실 원천 = 소비자) → 버스 내부 영속/이력 replay 없이 재협상만으로 routing 재구성. 닿는 파일 svc-bus.js·topology.js 둘뿐(발행자 같은 'bus' 주소라 무수정·cluster.js 무수정).
- **한 줄 상태**: 검증 reg 25/25(0033 비트 동일)·신규 busfail: bus.crash@12→재협상@14 → A0=60/crashOnly=6(subN 0·영구 단절)/recover=30(subN 3 복원·팬아웃 재개)·ranking 60→6→27·gap 손실=at-most-once·원장 valid(보존·정합·dupe 0). 단 svc.item *요청* 드롭 itemDesync=6(손상 아닌 손실·요청경로 무손실은 후속). 18+1모드 ALL OK·spine **34-step 사슬**·close-step 게이트 통과.
- **다음**: §2 참조 — 버스 영속·failover 본체(재협상 ✅ → 라우팅 영속+요청경로 in-flight 재발신으로 gap desync 0) · 비동기 결정론(🔴 최우선) · 활성 중 다운타임 일반 재발행 · 정리(cluster.js 45KB 재분할).

---

## 2. NEXT — step-0034 후 가설 (후보, 권위는 이 절)

**step-0034 가 버스 *failover*(구독 재협상으로 라우팅 복구)를 깔았다(crash→소비자 재구독→routing 재구성·버스 내부 영속 불필요·진실 원천=소비자). 단 crash gap 의 in-flight 메시지는 at-most-once 손실(요청 드롭 desync=6·손상 아닌 손실). 다음 후보: ⒝ *버스 failover 무손실화*(요청 경로 in-flight 재발신 = 0023 홉 NAK·0025 give-resend 의 버스 판 → gap desync 0, 또는 라우팅 테이블 영속·이력 replay·0016 §9-2), ⒞ *활성 중 서비스 다운타임 일반 재발행*(0025/0026 은 quiescent — 온라인 kill+재발행 핸드셰이크), ⒟ *비동기 결정론(lockstep 배리어 해제)*(논리/벡터 클럭·🔴 최우선·broker=cluster.js 대공사·0012 §9-3), 🔧 *정리 step*(cluster.js 45KB>30KB 박스 트리거 재분할·기능 0·reg 0).**
