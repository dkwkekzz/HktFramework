...
- **닫힌 step**: [step-0031](step-0031.md) — **정합성 윈도 *해소*(quorum-fill)**: 0029 가 윈도를 워터마크 위로 *감지*만 했다면, 이 step 은 그 윈도를 durable 로 *전환*한다. InventoryService.onTick 의 주기 sweep 이 `durableSeq` 위 W 미달 seq(0<ack<W)를 *아직 ack 안 한* 스토어에 재-fan-out(resend:true·q:true — 0023/0029 부품 재사용·새 메시지 0) → 저장·ack → 정족수 충족 → 워터마크가 끝까지 전진. 멱등(보유 스토어 skip·wfPeriod≥3 → 이중 발신 0).
- **한 줄 상태**: 검증 reg 25/25(0030 비트 동일)·신규 wfill 모드: OFF=durableSeq T-1/윈도 24(0029 감지)·ON=durableSeq total-1/윈도 0·fill 46~48·crash{primary,p4} 에 ON 전 seq 생존/OFF 윈도소실·dupe 0·desync 0. 18+1모드 ALL OK·spine **31-step 사슬**·close-step 게이트 통과.
- **다음**: §2 참조 — 버스 영속·failover · 활성 중 다운타임 일반 재발행 · 비동기 결정론(🔴 최우선) · fill 재시도/적응형 주기.

---

## 2. NEXT — step-0031 후 가설 (후보, 권위는 이 절)

**step-0031 이 정합성 윈도를 *해소*(quorum-fill)해 0029 §9 ⒜(윈도 감지→전환)를 닫았다. 다음 후보: ⒝ *버스 영속·failover*(서비스 복구 패턴의 버스 판·동적 구독 선결·0016 §9-2), 또는 ⒞ *활성 중 서비스 다운타임의 일반 재발행*(0025/0026 는 quiescent 복구 — 온라인 중 kill + 재발행 핸드셰이크), 또는 ⒟ *비동기 결정론(lockstep 배리어 해제)*(논리 클럭·Lamport·벡터 클럭·🔴 최우선·0012 §9-3), 또는 ⒠ *fill 재시도/적응형 주기*(fill 자체 손실 retry·sweep 범위 bound·0031 §9).**

남은 격차: ⓐ 버스·월드 *영속 0*·서비스 복구 *활성 중 다운타임 일반형 미검증* ⓑ 스냅샷 *비유계*·압축 *단일 트리거*·재전송 amplification·heartbeat×압축 미결합·채팅 in-flight/홉 신뢰 미적용 ⓒ 디스크 fsync 0·복제 anti-entropy 0·fill 재발신 손실 retry 미모델(0031 §9) ⓓ 비동기 결정론(lockstep 배리어) ⓔ give-resend opSeq 견고화·clientResync 멀티프로세스 패리티(0025 §9ⓕⓖ).

**검증할 것(공통)**: ① **회귀 0** — 새 항 OFF 면 0031 비트 동일. ② **신성한 tick** — 새 항이 존 tick 밖·존 비-침습. ③ **E2E 동치** — 멀티프로세스=인프로세스 비트 동일·은닉. ④ **가설** — 선택한 조각의 고장 시나리오 주입·복구 수렴 증명.
...
| 🟡 | **캐시 + write-behind 영속 (가방·채팅 저널+압축 ✅·저널홉 신뢰+tail+in-flight give/mint ✅·persist failover+N-replica+quorum read/write ✅·윈도 해소 ✅·월드/fsync ⬜)** | 데이터 | PersistStore(0017·계층 6 첫 박스) = 가방 효과 저널(write-behind)+스냅샷 압축(0018). 저널홉 신뢰 수신·tail·in-flight 복구 0023~0026. failover 0027(이중쓰기)·N-replica quorum-read 0028·quorum-write ack 0029(durableSeq 워터마크·정합성 윈도 가시)·**윈도 해소 0031(quorum-fill — W 미달 seq 재발신→durable 전환)**·전문 §7 INDEX. 단 *디스크 fsync 0·복제 anti-entropy 0·fill 손실 retry 미모델·스냅샷 비유계·월드 영속 0* — 0031/0029/0018 §9. |
