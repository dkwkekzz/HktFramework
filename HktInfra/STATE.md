...
- **닫힌 step**: [step-0022](step-0022.md) — 채팅 커맨드 로그 스냅샷 압축 (`ChatService._snapshot()` + `replay(journal, snapshot)` — 라우팅 스냅샷+tail replay). 0018 가방 효과 저널 압축의 *커맨드-소싱 판*: chatSnapshot OFF(0) 면 0021 비트 동일(reg 0) · ON 이면 커맨드 N항목마다 **라우팅 파생 상태**(channels/byAvatar/deliveries/계측)를 통째로 떠 chatpersist 로 발신 → PersistStore(0018 제네릭 핸들러·무변경)가 upToSeq 이하 커맨드 폐기 → 커맨드 로그 (스냅샷 1개 + 짧은 tail)로 유계. replay 는 스냅샷 베이스 복원 + tail 커맨드만 `_process`(replaying 가드 재발신 0). 핵심=**효과소싱 vs 커맨드소싱의 스냅샷 내용 차이**(가방=원장 값 / 채팅=리듀스 결과 라우팅 전체)·**무손실**(스냅샷+tail==전체 replay==무재시작 chatDigest 동일)·**압축-인지 완전성**(`leaves` 누적 추가·`writes==joins+says+whispers+leaves`). 더한 것은 chat._snapshot+replay 베이스+leaves+cluster 전달뿐·host.js/`engine/` 무수정.
- **한 줄 상태**: 채팅 커맨드 로그도 유계화 — 라우팅 스냅샷+tail replay. 핵심: ① **커맨드소싱 스냅샷은 *리듀스 결과 전체*** (say 팬아웃이 라우팅 의존 파생이라 channels+byAvatar+deliveries 통째) ② **PersistStore 압축은 제네릭** (snap 모양 무관·`upToSeq` 만 — 가방·채팅 무변경 공유) ③ **완전성은 누적 카운터** (압축이 journal.length 줄여도 writes 불변·0018 함정의 채팅 판) ④ **효과소싱·커맨드소싱 둘 다 압축 완성**. 검증: 회귀 0(`reg` 25/25 → 0021 비트 동일·chatp+chatrst 구성으로 휴면 직접 증명)·가설(`chat-compact`, 무손실 chatDigest 동일·로그 **78→3**(96% 절감)·15 스냅샷·desync 0·누설 0)·E2E(`e2e`, +chatpersist 라우팅 스냅샷·**chat-restart 14** 프로세스)·recover-chat/sacred/hide/repro 유지·spine 22-step 사슬. **단 (a) quiescent restart 만** **(b) 스냅샷 비유계·단일 트리거(0018 잔존 채팅 판)** **(c) write-behind 윈도(0017 잔존)·버스/월드 영속 0** **(d) Node·C++ 시뮬 미교체·lockstep 배리어**.
- **다음**: step-0023 (§2).

---

## 2. NEXT — step-0023 가설 (후보, 권위는 이 절)

**step-0022 가 채팅 커맨드 로그를 라우팅 스냅샷으로 유계화해 효과소싱(가방)·커맨드소싱(채팅) *둘 다* 영속+압축을 완성했다. 영속의 *신뢰성* 반쪽(write-behind 윈도)은 0017 부터 여전히 열려 있다. 가장 자연한 다음 한 조각은 ⒜ *활성 중 가방 failover + write-behind 손실 재수렴*(가방이 op 활성 중 죽어도 ack/resend·write 손실 감지로 완전성 수렴 — exactly-once 내구성·가장 큰 구멍). 또는 ⒝ *버스 영속·failover*(서비스 복구 패턴의 버스 판), 또는 ⒞ *월드 영속(존 intent 로그)*(계층 6 월드 판 — 크래시 복구·late-join), 또는 ⒟ *활성 중 서비스 다운타임 + 재발행*(0020/0021/0022 §9 quiescent 한계 — 다운타임 이벤트 후 클라 belief 재수렴).**

남은 격차: ⓐ *write-behind 윈도*(활성 중 죽음=in-flight 손실 — ack/resend·손실 감지 없음·exactly-once 미달·0017 부터 잔존, 가장 큰 신뢰성 구멍) ⓑ 버스·월드는 *영속 0*·서비스 복구는 *활성 중 다운타임 미검증*(0020~0022) ⓒ 스냅샷 *비유계*(전체 직렬화·증분 0)·압축 *단일 트리거*(0018/0022) ⓓ PersistStore 가 *단일점*. **권위는 이 절이되, 다음 step 시작 시 한 조각으로 좁힐 것**. ⒜ 는 exactly-once 내구성(write-ahead + opId 멱등 + 재전송)이라 *다중 메커니즘·고난도* — 한 조각으로 좁히려면 sub-piece(저널 홉 ack/resend 만·또는 클라 resend 만)로 쪼갤 것. ⒝/⒞ 는 0017~0022 의 검증된 영속·복구 패턴을 새 박스(버스·월드)에 반복(중난도·E2E 안전). 큰 조각(비동기 결정론·C++ 시뮬 교체)도 여전히 강한 후보.

**검증할 것(후보 — 영속 신뢰성·버스/월드 영속 기준)**: ① **회귀 0** — 새 항 OFF 면 0022 비트 동일. ② **신성한 tick** — 새 항(ack/resend·영속)이 존 tick 밖·존 비-침습(월드 비트 동일). ③ **E2E 동치** — 멀티프로세스=인프로세스 비트 동일·은닉. ④ **가설** — ⒜ 면 *활성 중 kill→복구 후 완전성 수렴*(in-flight 손실분 재전송/감지로 belief→원장 수렴·dupe 0), ⒝ 면 "버스 라우팅 kill→재구성 후 결합 0 보존", ⒞ 면 "존 intent 로그 replay==무재시작 월드 비트 동일·late-join".

> 후속 한 조각: **활성 중 가방 failover + write 손실 재수렴**(ack/resend·손실 감지·exactly-once — 0017 §9·고난도/분할 필요) · **버스 영속·failover**(서비스 복구 패턴의 버스 판) · **월드 영속(존 intent 로그)**(계층 6 월드 판·크래시 복구·late-join) · **활성 중 서비스 다운타임 + 재발행**(0020~0022 §9 — quiescent 한계 해소) · **PersistStore failover·다중 복제·디스크 fsync**(영속 단일점 제거) · **증분 스냅샷·압축 트리거 다양화**(0018/0022 §9) · **존 넘는 거래**(2PC — 0014 §9-2) · **비동기 실행 아래 결정론**(lockstep 해제·논리 클럭 — 0013 §9-1·큰 조각) · **C++ 시뮬 코어 headless 교체**(§3 🔴).

**병행 백로그(블로킹 아님)**: ⬜ 활성 중 가방 failover + write 손실 재수렴·ack/resend·exactly-once(0017 §9) · ⬜ 버스 영속·failover(0016 §9-2·0017 §9) · ⬜ 활성 중 서비스 다운타임 + 재발행(0020~0022 §9 — quiescent 한계) · ⬜ 읽기 모델 증분 follow·자체 스냅샷(0020 §9) · ⬜ 증분 스냅샷·압축 트리거 다양화·스냅샷 손실 ack(0018/0022 §9) · ⬜ 월드 영속(존 intent 로그·크래시 복구·late-join — 계층 6 월드 판) · ⬜ PersistStore failover·디스크 fsync·다중 복제(0017 §9 — 영속 단일점) · ⬜ 채팅 신뢰 전달·채널 시퀀스·ack/resend(0015 §9-2) · ⬜ 버스 분산·failover·동적 구독·이력/replay(0016 §9-2·§9-3) · ⬜ 런타임 consume→publish 사이클 정적 탐지(0019 §9) · ⬜ 거래소·우편(서비스 분리 패턴 반복) · ⬜ 존 넘는 거래·2-party 동의·에스크로(0014 §9-2) · ⬜ 비동기 실행 아래 결정론(논리 클럭, 0012 §9-3·0013 §9-1) · ⬜ 물리적 다중-브로커 버스 분산·분산 epoch(0012 §9-1·0013 §9-3) · ⬜ 거짓 사망 플래핑·부분 복귀·stale 호스트 재합류(0013 §9-4) · ⬜ 다중 standby·자가 디스커버리·점진 상태 동기(0013 §9-2) · ⬜ 소켓 층 지연·재정렬·비대칭 분단(0012 §9-4) · ⬜ 다중 클라 결정론 복제·예측(C++ 승격에서 부활) · ⬜ 존 N개·동적 경계(0006 §8.2) · ⬜ 존↔존 ghost 증분화·복원(0007 §8.3·0008 §8.6) · ⬜ 서버간 인증(0001 §8.3) · ⬜ 재접속·티켓 만료(0001 §8.5). (✅ 해소분 0014~0022 = §3 ✅ 묶음·§7 INDEX 참조.)

**빌드 인프라 — `engine/` 추출 완료(0004), 0005~0022 무수정 재사용**: 공통 하니스(시드 PRNG·FNV·전송 모델 `Net`·엣지/코디 스텁·동결 `ISimCore` 2구현 = `engine/index.js`, 시각 관찰 키트 = `engine/panel-kit.js`)를 `engine/` 한 곳에. 코어는 **dual-mode**(Node `require` + 브라우저 `<script>` 전역). 0022 는 `engine/`·host.js 무수정·존/orch/추종자/가방/버스/감사/영속·압축/랭킹/채팅 failover 안무 무수정 — net-core 는 0021 그대로 + **ChatService `_snapshot()`**(라우팅 파생 상태 스냅샷)·**`replay(journal, snapshot)`** 스냅샷 베이스 복원·**`leaves` 누적 회계**(압축-인지 완전성)만 더함. `cluster.js` 는 chatRestart 블록에 + 스냅샷 수집·replay 전달, `host.js` 는 **'replay' cmd·snapshotActor 제네릭 재사용**(chat.replay 가 snapshot 인자 받음·chatpersist=kind:'persist' 라 snapshot/snapshots/compacted 자동 반환). step-0023 의 net-core 출발 템플릿은 `step-0022/net-core.js`·`step-0022/cluster.js`·`step-0022/host.js`.
...
| 🟡 | **서비스 영속·failover (가방 ✅+압축 ✅·채팅 ✅+압축 ✅·버스 ⬜)·존 넘는 거래** | 서비스/데이터 | 0017 가방 원장 = event sourcing 효과 저널(복구 투명)·0018 스냅샷 압축. 0021 채팅 = *커맨드 로그* event sourcing(라우팅+deliveries 리듀서 재실행 복구)·0022 *라우팅 스냅샷 압축*(효과/커맨드소싱 둘 다 압축 완성). 단 *write-behind 윈도*(활성 중 죽음=in-flight 손실·ack/resend 없음 — 0017 잔존, 가장 큰 신뢰성 구멍)·버스 라우팅은 *영속 0*. 가방 일방 give(2PC 없음 — 0014 §9-2). |
