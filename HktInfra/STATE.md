...
- **닫힌 step**: [step-0018](step-0018.md) — 가방 저널 스냅샷 압축(event sourcing 의 "intent 로그 + 주기 스냅샷"). 0017 영속 저널의 *무한 성장*(한계 ⓑ)을 닫는다: snapshot OFF(=0) 면 0017 비트 동일(reg 0) · ON(`snapshot:N`)이면 가방이 저널 N항목마다 *원장 스냅샷*(`{upToSeq,ledger,mintTotal,minted,transfers}`)을 persist 로 write-behind(반응형·**onTick 0 유지**·라이브 원장 비-침습=invDigest 불변), PersistStore 가 `upToSeq` 이하 저널을 *폐기*(압축)→저널이 **(스냅샷 1개 + 짧은 tail)로 유계**. 복구 `replay(journal, snapshot)`=스냅샷 원장 베이스 + tail(seq>upToSeq)만 적용. 가설의 핵=**무손실 압축**: 세 복구 경로(스냅샷+tail·전체 저널·무재시작)가 *같은 invDigest*. 압축은 전부 persist-측/제어 평면(net.log 비-기여)→inproc/멀티 비트 동일. 복구 안무(crash→spawn→replay·라우팅 전환)·월드·버스(0016)·채팅(0015) *무변경*(가방 `_snapshot`·PersistStore 압축·`replay` snapshot 인자만 가산). `engine/` 무수정.
- **한 줄 상태**: 데이터 계층 PersistStore 가 *유계 저장*으로 한 걸음 — 무한 성장 저널을 (스냅샷+tail)로 압축하되 **스냅샷+tail replay == 전체-저널 replay 비트 동일**(무손실)을 증명했다. 핵심: ① **압축은 라이브 원장에 비-침습** — 스냅샷은 원장 *사본*을 persist 로 보낼 뿐, invDigest 가 0017 값(42=0x7a122947…2026=0x4bc176e1)과 *동일* ② **완전성의 진실 = 누적 writes**(압축 불변) — 압축이 `journal.length` 를 줄이므로 `journalComplete` 를 length→writes 로(압축은 *보관*을 줄일 뿐 *영속된 변이 수*는 불변) ③ **세 복구 경로 동일 invDigest = 무손실의 수치 증명** ④ **극한(tail 0)에서도 복구 투명** — `replay([],snapshot)` 순수 스냅샷만으로 원장 재현. 검증: 회귀 0(`reg` 25/25, persist+restart 포함 → 0017 비트 동일)·가설(`compact`, 저널 60→0·57→3 92~100% 절감·9~10 스냅샷·세 경로 무손실·desync 0)·E2E(`e2e`, 멀티=인프로세스 7중 비트 동일·persist-bus 11·restart-bus 12 프로세스)·신성한 tick(`sacred`, world on=off·writes 57~60·존도달 0·persist onTick 없음)·0017 가설 유지(`recover`, 복구 투명·소실 0 vs 51~56)·열화(`degrade`, 누설/phantom 0·보존/정합·소유≤1)·분리(`isolate`, persist 자기 pid·안 죽음·60항목 압축·tail 0 스냅샷 복구)·은닉(`hide` 0)·재현(`repro`, 멀티 2회=인프로세스 동일)·spine 18-step 사슬. **단 (a) write-behind 윈도(활성 중 죽음=in-flight 손실·ack/resend 없음 — 0017 잔존)** **(b) 스냅샷 자체 비유계(전체 원장 직렬화·증분 0)·압축 단일 트리거(항목 수만)** **(c) PersistStore=단일 박스·디스크 fsync/다중 복제 0** **(d) 가방만 영속(채팅·버스·월드 intent 로그는 영속 0)** **(e) Node·C++ 시뮬 미교체·lockstep 배리어**.
- **다음**: step-0019 (§2).

---

## 2. NEXT — step-0019 가설 (후보, 권위는 이 절)

**step-0018 이 가방 저널을 (스냅샷+tail)로 유계화했으나(무손실 압축), *write-behind 윈도*(활성 중 죽음=in-flight 손실 — ack/resend 없음)는 0017 부터 여전히 열려 있고, 채팅 구독·버스 라우팅·월드는 영속 0이다. 가장 자연한 다음 한 조각은 ⒜ *활성 중 failover + write-behind 손실 재수렴*(가방이 op 활성 중 죽어도 ack/resend·write 손실 감지로 완전성 수렴·클라 belief 가 원장 진실로 수렴 — 0017·0018 이 남긴 영속의 *신뢰성* 반쪽). 또는 ⒝ *채팅·버스 영속*(0017~0018 영속/압축 패턴의 서비스 판 반복 — 구독/라우팅을 죽여도 보존), 또는 ⒟ *월드 영속(존 intent 로그)*(계층 6 월드 판 — 크래시 복구·late-join, 존 세계가 세션보다 오래 산다), 또는 ⒠ *발신하는 둘째 소비자*(랭킹/거래소 — 0016 §9-4), 또는 ⒡ *채팅 신뢰 전달*(0015 §9-2).**

step-0018 이 PersistStore 를 *유계 저장*(스냅샷+tail·무손실 압축)으로 한 걸음 더 밀었다. 남은 격차: ⓐ *write-behind 윈도*(활성 중 죽음=in-flight 손실 — ack/resend·손실 감지 없음·0017~0018 잔존) ⓑ 채팅·버스·월드는 *영속 0* ⓒ 스냅샷 자체 *비유계*(전체 원장 직렬화·증분 0)·압축 *단일 트리거*(항목 수만) ⓓ PersistStore 가 *단일점*(디스크 fsync/다중 복제/failover 0). **권위는 이 절이되, 다음 step 시작 시 한 조각으로 좁힐 것** — 큰 조각(비동기 결정론·C++ 시뮬 교체)도 여전히 강한 후보다.

**검증할 것(후보 — 활성 중 failover·write 신뢰성 기준)**: ① **회귀 0** — 새 항 OFF 면 0018 비트 동일. ② **신성한 tick** — 새 항(ack/resend·영속)이 존 tick 밖·존 비-침습(월드 비트 동일). ③ **E2E 동치** — 멀티프로세스=인프로세스 비트 동일·은닉. ④ **가설** — ⒜ 면 *가방 활성 중 kill→복구 후 완전성 수렴*(in-flight 손실분이 재전송/감지로 메워져 클라 belief→원장 진실 수렴·dupe 0·소유≤1), *채팅·버스 영속* 면 "구독/라우팅 kill→재구성 후 누설 0·격리 보존", *월드 영속* 면 "존 kill→intent 로그 replay 로 월드 재현·desync 0".

> 후속 한 조각: **활성 중 가방 failover + write 손실 재수렴**(ack/resend·손실 감지 — 0017 §9) · **채팅·버스 영속**(서비스 영속 판 반복) · **증분 스냅샷·압축 트리거 다양화**(0018 §9 — 전체 원장 직렬화·항목-수 단일 트리거) · **월드 영속(존 intent 로그)**(계층 6 월드 판·크래시 복구·late-join) · **PersistStore failover·다중 복제·디스크 fsync**(영속 단일점 제거) · **발신하는 둘째 소비자(랭킹/거래소)**(0016 §9-4) · **채팅 신뢰 전달**(0015 §9-2) · **존 넘는 거래**(2PC — 0014 §9-2) · **비동기 실행 아래 결정론**(lockstep 해제·논리 클럭 — 0013 §9-1·큰 조각) · **C++ 시뮬 코어 headless 교체**(§3 🔴).

**병행 백로그(블로킹 아님)**: ⬜ 활성 중 가방 failover + write 손실 재수렴·ack/resend(0017 §9) · ⬜ 채팅·버스 영속·failover(0015 §9-3·0016 §9-2·0017 §9) · ⬜ 증분 스냅샷·압축 트리거 다양화·스냅샷 손실 ack(0018 §9 — 스냅샷 비유계) · ⬜ 월드 영속(존 intent 로그·크래시 복구·late-join — 계층 6 월드 판) · ⬜ PersistStore failover·디스크 fsync·다중 복제(0017 §9 — 영속 단일점) · ⬜ 발신하는 둘째 소비자—랭킹/거래소(0016 §9-4) · ⬜ 채팅 신뢰 전달·채널 시퀀스·ack/resend(0015 §9-2) · ⬜ 버스 분산·failover·동적 구독·이력/replay(0016 §9-2·§9-3) · ⬜ 존간 이벤트(핸드오프·ghost)의 버스 승격 판단(0016 §9-1) · ⬜ 길드·우편(서비스 분리 패턴 반복) · ⬜ 존 넘는 거래·2-party 동의·에스크로(0014 §9-2) · ⬜ 비동기 실행 아래 결정론(논리 클럭, 0012 §9-3·0013 §9-1) · ⬜ 물리적 다중-브로커 버스 분산·분산 epoch(0012 §9-1·0013 §9-3) · ⬜ 거짓 사망 플래핑·부분 복귀·stale 호스트 재합류(0013 §9-4) · ⬜ 다중 standby·자가 디스커버리·점진 상태 동기(0013 §9-2) · ⬜ 소켓 층 지연·재정렬·비대칭 분단(0012 §9-4) · ⬜ 다중 클라 결정론 복제·예측(C++ 승격에서 부활) · ⬜ 존 N개·동적 경계(0006 §8.2) · ⬜ 존↔존 ghost 증분화·복원(0007 §8.3·0008 §8.6) · ⬜ 서버간 인증(0001 §8.3) · ⬜ 재접속·티켓 만료(0001 §8.5) · ✅ 가방 저널 스냅샷 압축(0018 — intent 로그+스냅샷·무손실·저널 유계화) · ✅ 가방 서비스 영속·failover(0017 — event sourcing 저널·복구 투명·계층 6 첫 진입) · ✅ 이벤트 버스 서비스 층(0016 — 발행/구독 의미·직접 결합 0·발행자 무수정 소비자 추가) · ✅ 채팅 서비스 분리(0015) · ✅ 가방 서비스 분리(0014) · ✅ 시각화·검증 testbed 단일화(0010 후: `run.js`+`report.html`+`live.js` — [TESTBED.md](TESTBED.md)) · ✅ 시나리오 `inject`(0016 write-seam — run.js 자동 소비).

**빌드 인프라 — `engine/` 추출 완료(0004), 0005~0018 무수정 재사용**: 공통 하니스(시드 PRNG·FNV·전송 모델 `Net`·엣지/코디 스텁·동결 `ISimCore` 2구현 = `engine/index.js`, 시각 관찰 키트 = `engine/panel-kit.js`)를 `engine/` 한 곳에. 코어는 **dual-mode**(Node `require` + 브라우저 `<script>` 전역). 0018 은 `engine/` 무수정·존/orch/추종자/버스/감사/채팅/PersistStore failover 안무 무수정 — net-core 는 0017 그대로 + 가방 `_snapshot`(저널 N항목마다 원장 스냅샷·반응형·onTick 0) + PersistStore *압축*(snapshot 수신 시 upToSeq 이하 폐기) + `replay(journal, snapshot)`(스냅샷 베이스+tail)만 더함. `cluster.js`/`host.js` 는 0017 그대로 + snapshotOne 이 `{journal, snapshot}` 동반·replay RPC 에 snapshot 인자만 가산. step-0019 의 net-core 출발 템플릿은 `step-0018/net-core.js`·`step-0018/cluster.js`·`step-0018/host.js`.
