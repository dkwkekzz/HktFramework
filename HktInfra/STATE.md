...
- **닫힌 step**: [step-0036](step-0036.md) — **버스 failover *결과 경로* 무손실: producer replay 로 gap desync 0**. 0034 는 재구독으로 routing 만 복구 → crash gap 에 떨군 `svc.item.out` *결과*(원장 적용·클라 미수신)는 영구 손실(클라 belief < 원장 = itemDesync·0034 §9). 한 조각: 가방(producer)이 발신 결과를 `outBuffer` 보관·버스 복구 시 `resendOut()` 재발행(0023 홉 신뢰·0025 give-resend 의 *버스 판*) → 뒤처진 클라 따라잡음. 클라 belief 는 `Set` → 재배달 멱등(consumer dedup 불요). 닿는 파일 svc-inventory.js·topology.js 둘.
- **한 줄 상태**: busfail recover(재구독만·desync 6 대조군)→resend(busResend ON·**desync 6→0**·outResends 18·원장 보존/정합). 발견: desync 범인은 *요청* 드롭(0034 §9 가정)이 아니라 *결과* 드롭(6 전부 belief<원장=behind) → 결과 재발행만으로 0. reg 25/25(busResend OFF=0035 비트 동일)·18+busfail ALL OK·spine **36-step 사슬**·close-step 통과.
- **다음**: §2 참조 — ⒝-남은(요청 경로 무손실·base 대비 mint 손실 0·유계 outBuffer·give ahead 결합) · 비동기 결정론(🔴 최우선) · 활성 중 다운타임 일반 재발행. 정리 후보: topology.js 30KB 근접.

---

## 2. NEXT — step-0036 후 가설 (후보, 권위는 이 절)

**step-0036 이 버스 failover *결과 경로* 무손실(producer replay → gap desync 0)을 깔았다. 결과 드롭만 desync 를 냈고 결과 재발행으로 0. 남은 후보: ⒝-요청경로 *버스 failover 요청 무손실화*(gap 에 떨군 *요청* 재발행 → base 대비 mint 손실 0·gateway/client producer replay·give-resend 결합) + 유계 outBuffer(0032 wfWindow 패턴), ⒞ *활성 중 서비스 다운타임 일반 재발행*(0025/0026 은 quiescent — 온라인 kill+재발행 핸드셰이크), ⒟ *비동기 결정론(lockstep 배리어 해제)*(논리/벡터 클럭·🔴 최우선·broker=cluster-core.js/cluster-run.js 대공사·0012 §9-3), 🔧 *다음 정리 후보*(topology.js 30KB 근접·기능 step 이 키우면 재분할).**
...
**병행 백로그(블로킹 아님)**: ⬜ 디스크 fsync·복제 anti-entropy·적응형 sweep(0031/0029 §9) · ⬜ 버스 요청경로 무손실화·라우팅 영속·분산·이력/replay(0034/0016 §9) · ⬜ 활성 중 서비스 다운타임 + 재발행 핸드셰이크(0020~0022 §9) · ⬜ give-resend opSeq 매칭·clientResync 멀티프로세스 패리티(0025 §9ⓕⓖ) · ⬜ 재전송 amplification 배칭·heartbeat×압축·채팅 in-flight/홉 신뢰(0023/0024 §9) · ⬜ 읽기 모델 증분 follow·증분 스냅샷·압축 트리거 다양화(0018/0020/0022 §9) · ⬜ 월드 영속(존 intent 로그·tick-결합 고위험) · ⬜ 런타임 consume→publish 사이클 정적 탐지(0019 §9) · ⬜ 거래소·우편·길드·존 넘는 거래·2-party 에스크로(0014 §9-2) · ⬜ 비동기 결정론(논리 클럭·0012 §9-3) · ⬜ 서버간 인증·재접속·티켓 만료. (✅ 해소분 0014~0034 = §3 ✅ 묶음·§7 INDEX 참조.)
...
| 🟡 | **버스 단일점·분산·영속(동적 구독 ✅·failover 재협상 ✅·결과경로 무손실 ✅)** | 버스 | 0016 ServiceBus = *단일 박스·영속 0*. **0033 동적 구독/해지 ✅** + **0034 failover ✅**(crash→재구독→routing 재구성·진실 원천=소비자·버스 영속 불필요) + **0036 결과경로 무손실 ✅**(producer replay — 가방이 발신 결과 보관·복구 시 재발행 → gap desync 6→0·belief Set 멱등). 남은 것: crash gap 의 *요청* 드롭(base 대비 mint 손실·요청경로 재발신·0036 §9)·유계 outBuffer·라우팅 영속/이력 replay 0·구독자 장애 감지 0·다중 브로커 분산(0012 §9-1). 월드·제어 평면·존간 핸드오프는 직접 유지(의도). |
...
> **✅ 해소된 격차 (0001~0034)** — 전문은 §7 INDEX(1줄/step)·각 `step-NNNN.md`. 묶음: 골격/복제/Sim 동결/현실 전송(0001~0004)·AOI·분할·핸드오프·증분·복원·failover(0005~0009)·프로세스 경계·TCP·버스 분산·진짜 kill failover(0010~0013)·게임 서비스 분리(가방·채팅 0014~0015)·이벤트 버스 의미(0016)·가방/채팅 영속+압축(0017~0022)·발신 소비자·읽기 모델 영속(0019~0020)·write-behind 신뢰성 완결(0023~0026)·persist failover·N-replica·quorum write ack·윈도 해소+유계 sweep(0027~0032)·버스 동적 구독+failover(0033~0034).
...
| 4 | 버스 | 이벤트 버스 | 🟡 0004 전송 substrate(`engine/Net`) +0008 핸드오프/델타 라우트 +0010 IPC +0011 실 TCP 소켓 +0012 토픽 pub/sub 버스(*전송*·소켓 열화 내성) +0016 **서비스 의미**(ServiceBus — 발행/구독 계약·직접 결합 0·발행자 무수정 소비자 추가) +0019 **발신 소비자**(ranking consume→publish 루프·발행자 무수정·루프 없음) +0033 **동적 구독/해지**(runtime `unsub`/`sub` — 라우팅 테이블 양방향 변경·토글은 그 소비자만·공동 구독자/발행자 비트 동일) +0034 **failover**(`bus.crash()`→소비자 재구독으로 routing 재구성·진실 원천=소비자·버스 영속 불필요·gap=at-most-once 손실) +0036 **결과경로 무손실**(producer replay — 가방이 발신 결과 보관·복구 시 재발행 → gap desync 6→0·belief Set 멱등·dedup 불요). 버스 단일점 제거(분산)·요청경로 무손실·유계 outBuffer·라우팅 영속·이력/replay·서버간 인증 후속 |
...
| [0021](step-0021.md) | 채팅 서비스 영속·failover (ChatService crash→커맨드 로그 replay — 라우팅+deliveries 의 event sourcing 복구) | 통과 · chatpersist OFF=0020 비트 동일(25/25)·채팅 kill→replay 복구 투명(chatDigest 동일·배달 211~229)·OFF+kill 소실·누설 0·chatDesync 0·E2E 비트 동일(chat-restart 14)·spine 21-step 사슬 |
...
| [0028](step-0028.md) | PersistStore N-replica + quorum: 복제 N fan-out·생존 저널 union 복구로 영속 단일점 일반 해소 (quorumMergeJournals·0027 §9) | 통과 · persistReplicas 0=0027 비트 동일(25/25)·완전복제/부분쓰기(생존3 union==base) 무손실·생존1 부분=손실(정족수 경계)·desync 0·spine 28-step 사슬 |
| [0029](step-0029.md) | PersistStore quorum *쓰기* ack: W 정족수 ack 후 durable 선언·정합성 윈도 가시화 (durableSeq 워터마크·0028 §9) | 통과 · quorumW 0=0028 비트 동일(25/25)·무손실=전 seq durable·tail 정족수 미달=durableSeq T-1·윈도 23~24·워터마크가 복구 프런티어 정확 예측·desync 0·spine 29-step 사슬 |
...
| [0031](step-0031.md) | 정합성 윈도 *해소*(quorum-fill — 주기 sweep 이 W 미달 윈도 seq 를 비-홀더 스토어에 재-fan-out resend → durable 전환·0029 §9) | 통과 · windowFill 0=0030 비트 동일(reg 25/25)·ON durableSeq total-1/윈도 0·fill 46~48·crash{primary,p4} ON 전 seq 생존/OFF 윈도소실·dupe 0·desync 0·spine 31-step 사슬 |
| [0032](step-0032.md) | 윈도 해소의 *유계 sweep + fill 손실 retry*(`wfWindow` 미끄러지는 K 창·주기 재-scan=내장 retry·0031 §9) | 통과 · wfWindow 0=0031 비트 동일(reg 25/25)·유계 K=8 durSeq=total-1/윈도0·fills 92~96·dupe 0·crash{primary,p4} 전 seq 생존·desync 0·spine 32-step 사슬 |
| [0033](step-0033.md) | 버스 *동적 구독/해지*(runtime `unsub`/`sub` — 라우팅 런타임 양방향 변경·failover 선결·0016 §9-2) | 통과 · busReSub 0=0032 비트 동일(reg 25/25)·unsub@15→re-sub@18 audit 30→42(gap 손실 12)·공동구독 비트 동일·보존/정합/desync 0·spine 33-step 사슬 |
| [0034](step-0034.md) | 버스 *failover*(`bus.crash()`→구독 재협상으로 라우팅 복구 — 진실 원천=소비자·버스 영속 불필요·0016 §9-2) | 통과 · busRestart 0=0033 비트 동일(reg 25/25)·crash@12→재협상@14·crashOnly subN 0(영구 단절)/recover subN 3 복원·원장 valid·gap 결과 드롭 desync 6(0036 해소)·spine 34-step 사슬 |
| [0035](step-0035.md) | 정리 step: `cluster.js` 박스-부품 4분할(45KB>30KB 트리거·기능 0·바이트 동일·export 불변 — 0030 net-core 분할의 cluster 판) | 통과 · 진입점 1+부품 4(wire·core·reconstruct·run)·최대 cluster-core 19.7KB(<30KB)·디렉토리 272KB·분할 투명성=멀티프로세스 E2E 비트 동일·18+1모드 ALL OK·spine 35-step 사슬 |
| [0036](step-0036.md) | 버스 failover *결과 경로* 무손실(producer replay — 가방이 svc.item.out 결과 보관·복구 시 재발행 = 0023 홉/0025 give-resend 의 버스 판·0034 §9) | 통과 · busResend 0=0035 비트 동일(reg 25/25)·recover(재구독만) desync 6→resend(ON) **desync 0**·outResends 18·원장 보존/정합·발견: 범인은 요청 아닌 *결과* 드롭(belief<원장)·belief Set 멱등→dedup 불요·spine 36-step 사슬 |
