...
- **닫힌 step**: [step-0247](step-0247.md) — **배치 SSOT 실배선(#51) 7: executed placeAuto**. 부하 기반 자동 배치(`placeAuto`)가 `placeExecute` ON 이면 최소부하 host 선택 + paper 갱신에 더해 실 존 런타임도 가동(`_start`·0217 advisory 자동 배치의 집행 판). OFF→0246 비트 동일.
- **한 줄 상태**: reg ALL OK·placeautoexec: 5/5 z1@A→z2/z3/z4 자동→running A2/B1/C1·starts 4·drift 0·`run.js all` 17모드+classics ALL OK·spine ALL OK.
- **다음**: 🎯 **#51 실배선 진행**(0241~0247·placeZone/Migrate/Rebalance/Drain/Stop/Auto executed). **다음 = 0248 존 런타임 crash/recover**(host 장애 시 running 의 그 host 존 재배치·failover 의 배치 판) 또는 게이트 #49(wiring 분할·topo-build 31.5/topo-run 35.9KB). #51 거의 닫힘 → 다음 묶음(0241~0250) 방향은 `infra-review` 평결.
...
**#51 실배선 arc:** 0241 존 런타임 SSOT(start) ✅ → 0242 executed migrate ✅ → 0243 executed rebalance ✅ → 0244 executed drain ✅ → 0245 reconcile capstone(drift 0) ✅. **1차 arc 완료.** 다음 심화: 0246 executed placeStop(런타임 종료)·0247~ executed placeAuto·런타임 crash/recover. 그 뒤 게이트 #49(wiring 분할·topo-build 31.5/topo-run 35.9/svc-exchange-core 30.7KB).
