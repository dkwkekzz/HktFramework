...
- **닫힌 step**: [step-0221](step-0221.md) — **인스턴스 플레이어 이탈**: `instanceLeave{player}`→배정 player route 해제(occupancy 감소·권위 release·0216 acquire 짝), 미배정 멱등 no-op. 미주입→0220 비트 동일. **3차 고도화 인스턴스 #1·균형 라운드(0221~0230) 시작**. 닿는 박스: instance.
- **한 줄 상태**: reg ALL OK·instanceleave: 5/5 d1 occ 2→1·p1 route null·left 1·miss 1·spine ALL OK.
- **다음**: 🎯 **3차 균형 라운드 진행 중**(0221~0230·5박스 각 2조각 심화·backlog §2 인스턴스/오케/캐시/월드영속/로그인). 다음: 0222 인스턴스 수요 자동 despawn(instanceReap·active>target·occupancy 0 회수·0215 짝).
...
1. **인스턴스(던전) 서버** (계층2) — ✅ 이탈(0221·occupancy 감소·release)+⬜ 수요 자동 despawn(0222·active>target 회수).
...
| 2 | 월드 | 존 · 인스턴스 (분할·AOI·조정·핸드오프) | 🟡 존 VM+결정론 복제+AOI+분할·핸드오프(소유자=1)+failover+별 프로세스(0001~0013) · **인스턴스 🟡 spawn+despawn(0201~0202)+수요 자동 spawn(0215)+라우팅(0216)+이탈(0221·occupancy release)**. 존 N개 후속 |
