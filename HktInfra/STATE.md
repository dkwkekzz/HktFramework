...
- **닫힌 step**: [step-0029](step-0029.md) — PersistStore quorum *쓰기* ack: `_journal` 이 `q:true` 부착→각 스토어가 저장 후 `journal_ack{seq}` 회신→InventoryService 가 seq 별 ack 집계 → `durableSeq`(≥W ack 인 연속 커밋 워터마크). 윈도 = (보낸 최대 seq − durableSeq) = fire-and-forget(0028)이 못 본 정합성 윈도를 가시화. quorumW 0 면 휴면 → 0028 비트 동일(reg 0). 0028 §9(write quorum 없음) 해소.
- **한 줄 상태**: 핵심 ① **"보냈다"≠"durable 하다" — W개 ack 후에만 durable 선언(낙관 fire-and-forget 금지)** ② **durableSeq 워터마크 = ≥W 사본 보유 프런티어 = (N+1−W) 죽음 견딤·정합성 윈도를 가시·유계화**. 검증: 회귀 0(`reg` 25/25 → 0028 비트 동일·q/ack 휴면)·가설(`wquorum`: ⒜무손실=전 seq durable win0 / ⒝per-link 손실 3홀더=W=여전 durable win0 / ⒞tail 정족수 미달=durableSeq T-1 정지·윈도 23~24·crash{primary,p4} 에 durable 전부 복구·윈도 전부 소실 = 워터마크가 복구 프런티어 정확 예측)·라이브 desync 0·spine 29-step 사슬. **단 (a) 워터마크는 윈도 *감지*지 *전환* 아님(W 미달 seq 재발행 미구현) (b) ack=RAM 도달이지 디스크 fsync 아님 (c) 버스·월드 영속 0 (d) 활성 중 다운타임 일반 재발행 (e) C++ 시뮬·lockstep 배리어**.
- **다음**: §2 참조 — quorum write 후 새 후보 선택.

---

## 2. NEXT — step-0029 가설 (후보, 권위는 이 절)

**step-0029 가 quorum *쓰기* ack(W 정족수 후 durable 선언·durableSeq 워터마크)로 0028 의 fire-and-forget 정합성 윈도를 가시·유계화했다. 다음 후보: ⒜ *정합성 윈도 해소*(W 미달 seq 를 0023 재전송으로 재발행해 정족수 채우기 — 현재 워터마크는 윈도를 *감지*만·0029 §9), 또는 ⒝ *버스 영속·failover*(서비스 복구 패턴의 버스 판·동적 구독 선결·0016 §9-2), 또는 ⒞ *활성 중 서비스 다운타임의 일반 재발행*(0025/0026 는 quiescent 복구 — 온라인 중 kill + 재발행 핸드셰이크), 또는 ⒟ *비동기 결정론(lockstep 배리어 해제)*(논리 클럭·Lamport·벡터 클럭·🔴 최우선·0012 §9-3).**
...
**빌드 인프라 — `engine/` 추출(0004), 이후 무수정 재사용**: 공통 하니스(시드 PRNG·FNV·전송 모델 `Net`·엣지/코디 스텁·동결 `ISimCore` 2구현 = `engine/index.js`, 관찰 키트 = `engine/panel-kit.js`)가 한 곳. 코어는 **dual-mode**(Node `require` + 브라우저 `<script>` 전역). 매 step 은 `node engine/new-step.js`(직전 step 복사 전진·자기참조/reg 경로 치환) 후 net-core/verify 델타만 Edit. step-0029 출발 템플릿 = `step-0028/`.
