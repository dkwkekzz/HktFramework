...
- **닫힌 step**: [step-0037](step-0037.md) — **버스 failover *요청 경로* 무손실: gateway producer replay 로 base 대비 mint 손실 0**. 0036 은 *결과* 경로(svc.item.out)를 가방 replay 로 무손실화했으나 거울인 *요청* 경로(svc.item)는 손실 — gap 에 떨군 pickup/give 요청은 가방 미도달 → mint 자체 안 됨(원장 < base·0036 §9). 한 조각: 요청 producer 인 *게이트웨이*가 발행 요청을 `inBuffer` 보관·`reqId` 태깅·버스 복구 시 `resendIn()` 재발행 → gap 요청이 가방 도달·mint. pickup 은 *멱등 아님*(매번 새 itemId) → 가방이 `seenReqs` reqId dedup → 이중 mint 0. 닿는 파일 gateway.js·svc-inventory.js·topology.js 셋.
- **한 줄 상태**: busreq recover(재구독만·mint −18 대조군)→resendReq(busResendReq ON·**minted==base·손실 18 전량 복구**·inResends 36·원장 보존/정합). 핵심: `minted==base` 정확함이 손실 0 *그리고* 이중 mint 0(dedup)을 한 수로 증명(gap 전 도달분도 재발행되므로 dedup 없으면 base 초과). reg 25/25(busResendReq OFF=0036 비트 동일)·18+busreq ALL OK·spine **37-step 사슬**·close-step 통과.
- **다음**: §2 참조 — 유계 in/outBuffer(0032 wfWindow 패턴) · give 요청×result-ahead 결합 · 비동기 결정론(🔴 최우선). 정리 후보: topology.js 30KB 근접.

---

## 2. NEXT — step-0037 후 가설 (후보, 권위는 이 절)

**step-0037 이 버스 failover *요청 경로* 무손실(gateway producer replay + reqId dedup → base 대비 mint 손실 0)을 깔았다. 0036(결과 경로)의 거울 — pickup 요청 드롭(mint 누락)을 게이트웨이 재발행으로 정확 복구, pickup 멱등 불가라 reqId dedup 으로 이중 mint 0. 남은 후보: ⒝-남은 *유계 inBuffer/outBuffer*(0036/0037 무계 → 0032 wfWindow 슬라이딩 K 창으로 유계화·gap 만 덮기) + *give 요청 × result-ahead 결합*(give 재발행은 transfers≠base·클라 재-give 와 얽힘 → 0025 give-resend 통합 복구), ⒞ *활성 중 서비스 다운타임 일반 재발행*(0025/0026 은 quiescent — 온라인 kill+재발행 핸드셰이크), ⒟ *비동기 결정론(lockstep 배리어 해제)*(논리/벡터 클럭·🔴 최우선·broker=cluster-core.js/cluster-run.js 대공사·0012 §9-3), 🔧 *다음 정리 후보*(topology.js 30KB 근접·기능 step 이 키우면 재분할).**
...
**병행 백로그(블로킹 아님)**: ⬜ 유계 in/outBuffer·give 요청×result-ahead 결합(0037 §9) · ⬜ 디스크 fsync·복제 anti-entropy·적응형 sweep(0031/0029 §9) · ⬜ 버스 라우팅 영속·분산·이력/replay(0034/0016 §9) · ⬜ 활성 중 서비스 다운타임 + 재발행 핸드셰이크(0020~0022 §9) · ⬜ give-resend opSeq 매칭·clientResync 멀티프로세스 패리티(0025 §9ⓕⓖ) · ⬜ 재전송 amplification 배칭·heartbeat×압축·채팅 in-flight/홉 신뢰(0023/0024 §9) · ⬜ 읽기 모델 증분 follow·증분 스냅샷(0018/0020 §9) · ⬜ 월드 영속(존 intent 로그·tick-결합 고위험) · ⬜ 거래소·우편·길드·존 넘는 거래·2-party 에스크로(0014 §9-2) · ⬜ 비동기 결정론(논리 클럭·0012 §9-3) · ⬜ 서버간 인증·재접속·티켓 만료. (✅ 해소분 0014~0037 = §3 ✅ 묶음·§7 INDEX 참조.)
...
> **✅ 해소된 격차 (0001~0037)** — 전문은 §7 INDEX(1줄/step)·각 `step-NNNN.md`. 묶음: 골격/복제/Sim 동결/현실 전송(0001~0004)·AOI·분할·핸드오프·증분·복원·failover(0005~0009)·프로세스 경계·TCP·버스 분산·진짜 kill failover(0010~0013)·게임 서비스 분리(0014~0015)·이벤트 버스 의미(0016)·가방/채팅 영속+압축(0017~0022)·발신 소비자·읽기 모델 영속(0019~0020)·write-behind 신뢰성 완결(0023~0026)·persist failover·N-replica·quorum write ack·윈도 해소(0027~0032)·버스 동적 구독+failover+결과/요청경로 무손실(0033~0037).
...
| [0028](step-0028.md) | PersistStore N-replica + quorum: 복제 N fan-out·생존 저널 union 복구로 영속 단일점 일반 해소 (quorumMergeJournals·0027 §9) | 통과 · persistReplicas 0=0027 비트 동일(25/25)·생존3 union==base 무손실·생존1 부분=손실(정족수 경계)·spine 28-step 사슬 |
...
| [0030](step-0030.md) | 정리 step: 박스 1개=파일 1개 분할 + verify 누적회귀 18모드 engine 승격(verify-kit) + 닫기 게이트(close-step) + 2-커밋 관행 — 기능 추가 0 | 통과 · reg 25/25(0029 비트 동일·verbatim)·18모드 ALL OK·디렉토리 267.5→203.3KB·spine 30-step 사슬 |
| [0031](step-0031.md) | 정합성 윈도 *해소*(quorum-fill — 주기 sweep 이 W 미달 윈도 seq 를 비-홀더 스토어에 재-fan-out resend → durable 전환·0029 §9) | 통과 · windowFill 0=0030 비트 동일(reg 25/25)·ON durableSeq total-1/윈도 0·fill 46~48·crash 전 seq 생존·dupe 0·spine 31-step 사슬 |
| [0032](step-0032.md) | 윈도 해소의 *유계 sweep + fill 손실 retry*(`wfWindow` 미끄러지는 K 창·주기 재-scan=내장 retry·0031 §9) | 통과 · wfWindow 0=0031 비트 동일(reg 25/25)·유계 K=8 durSeq=total-1/윈도0·fills 92~96·dupe 0·spine 32-step 사슬 |
...
| [0034](step-0034.md) | 버스 *failover*(`bus.crash()`→구독 재협상으로 라우팅 복구 — 진실 원천=소비자·버스 영속 불필요·0016 §9-2) | 통과 · busRestart 0=0033 비트 동일(reg 25/25)·crash@12→재협상@14·crashOnly subN 0/recover subN 3 복원·gap 결과 드롭 desync 6(0036 해소)·spine 34-step 사슬 |
| [0035](step-0035.md) | 정리 step: `cluster.js` 박스-부품 4분할(45KB>30KB 트리거·기능 0·바이트 동일·export 불변 — 0030 net-core 분할의 cluster 판) | 통과 · 진입점 1+부품 4·최대 cluster-core 19.7KB(<30KB)·E2E 비트 동일·18+1모드 ALL OK·spine 35-step 사슬 |
| [0036](step-0036.md) | 버스 failover *결과 경로* 무손실(producer replay — 가방이 svc.item.out 결과 보관·복구 시 재발행 = 0023 홉/0025 give-resend 의 버스 판·0034 §9) | 통과 · busResend 0=0035 비트 동일(reg 25/25)·recover desync 6→resend(ON) **desync 0**·outResends 18·발견: 범인은 *결과* 드롭(belief<원장)·belief Set 멱등→dedup 불요·spine 36-step 사슬 |
| [0037](step-0037.md) | 버스 failover *요청 경로* 무손실(gateway producer replay + reqId dedup — 게이트웨이가 svc.item 요청 보관·복구 시 재발행·pickup 멱등 불가라 가방 seenReqs dedup·0036 §9 거울) | 통과 · busResendReq 0=0036 비트 동일(reg 25/25)·recover(재구독만) mint −18→resendReq(ON) **minted==base**(손실 18 전량·이중 mint 0)·inResends 36·원장 보존/정합·give 요청은 transfers≠base(0025 결합 후속)·spine 37-step 사슬 |
