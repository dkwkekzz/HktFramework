...
- **닫힌 step**: [step-0377](step-0377.md) — **#62 runMulti 통합 7: syncPlan 비파괴 자가 치유** — orch.hostSpawnPlan 대비 현 snapshot 차분→누락 존만 zoneadd(기존 상태 보존·idempotent). 토폴로지 drift(존 소실)를 orch 목표로 되돌림. 직전: 0376 상주 failover.
- **한 줄 상태**: reg ALL OK·coordsync 5/5(z3 drift→복원·acted1·coherent)·박스 >30KB 0개·spine ALL OK.
