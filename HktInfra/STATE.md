...
- **닫힌 step**: [step-0399](step-0399.md) — **#66+#67 통합 정합 술어**: `unifiedCoherent()` = syncedCoherent(#65/#66) && authoritiesAgree(#67). 양방향 동기·tick placement-aware·orch 이중 권위 합류가 모두 한 몸. orch.running stale 주입 시 N(실측 검출·by-construction 아님). **#66 닫힘·#67 진행(0394~)**. 직전: 0398 report authoritiesAgree.
- **한 줄 상태**: reg ALL OK·coordunified 5/5(unifiedCoherent Y·stale 주입 N)·cluster-coord.js 23.9KB·박스 >30KB 0개·spine ALL OK.
...
> 🎯 **#65 양방향 동기 sub-arc(0381~0390 ✅ 닫힘)** — 코디네이터가 *placement 권위(where)*를 직접 소유(orch=entity 권위/what 분리)하고 lifecycle 마다 갱신: placement SSOT(0381)·coordDesync(placement 기준·0382)·migrate placement 갱신(0383·#65 핵심 fix)·coordCoherent 가 coordDesync 채택(0384)·failover placement 갱신+lost 추적(0385)·lost 기대 부재 제외(0386)·placement-aware report(0387)·syncPlan placement 기준(0388)·placementCoherent bijection(0389)·grand capstone syncedCoherent(0390·migrate/failover 포함 정합). **다음 후보(권위는 infra-review)**: ⒜ cluster-run.js 옛 lockstep runMulti 와 코디네이터 코드 합류(#62 잔여)·⒝ 코디네이터 placement↔orch 내부 zoneHost API 합류·⒞ 업스트림 intent 실 클라(#61)·진짜 비동기(#4).
