...
- **닫힌 step**: [step-0019](step-0019.md) — 발신하는 둘째 소비자 `RankingService`(버스 consume→publish 루프·이벤트 기반 읽기 모델/CQRS). 0016 의 audit 이 *관찰 전용*(발신 0)이라 "이벤트를 소비해 *다시 발행*하는" 서비스를 못 보였던 것(0016 §9-4)을 닫는다: ranking OFF 면 0018 비트 동일(reg 0) · ON(bus+가방 전제)이면 RankingService 가 `svc.item.out`(가방 결과)을 *구독해 소비*→아바타별 보유 수(rank) 투영(pickup→+1·give→from-1/to+1·실패 op 무변경)→변경분을 `svc.rank.out` 으로 *발행*(consume→publish·**onTick 0**), gateway 가 svc.rank.out 구독→대상 클라에 rank_update 중계(클라 rankBelief 가 원장 진실로 수렴). 가설의 핵=**발행자(inventory) 무수정으로 발신 소비자가 얹힌다**(구독 행 추가뿐·senderDigest 불변=decouple)·**rank 프로젝션==원장 byOwner 정확 일치**(읽기 모델 ≡ 쓰기 모델)·**루프 없음**(svc.rank.out 을 item 서비스가 안 먹음·발행 유계). 더한 박스는 RankingService 하나·월드/영속(0017)/압축(0018)/채팅(0015) 무변경·`engine/` 무수정.
- **한 줄 상태**: 이벤트 버스가 *발신 소비자*를 받는다 — 소비하고 다시 발행하는 읽기 모델(랭킹)이 발행자 무수정으로 얹히고, 그 투영이 쓰기 모델과 정확히 일치(CQRS). 핵심: ① **읽기 모델은 권위를 *복제*하지 않고 *재계산*** — rank 는 원장에 안 쓰고 item_result 스트림에서 다시 셈 → "권위 단일 소유"가 구조적 성립·byOwner 와 자동 일치 ② **발행자 무수정 = 발신 소비자에도 성립** — 0016 audit(관찰 소비자) decouple 을 *발신 소비자*로 확장(inventory senderDigest on/off 비트 동일) ③ **발행 > 소비는 정상**(give 가 from/to 둘 다 갱신 → 발행 69 vs 소비 60)·루프 아님(bounded ×2 로 구별) ④ **루프 방지는 토폴로지 규약**(svc.rank.out 미소비 by item). 검증: 회귀 0(`reg` 25/25 → 0018 비트 동일)·가설(`rank`, 소비 57~60·발행 61~69·프로젝션==원장·decouple·rankDesync 0·발행 유계·audit 전수 관찰)·E2E(`e2e`, 멀티=인프로세스 8중 비트 동일 +rank·persist-bus 12·restart-bus 13 프로세스)·신성한 tick(`sacred`, world 비-침습·존도달 0)·은닉(`hide`, rank_update=count 만·누설 0)·재현(`repro`, rankDigest 포함 멀티 2회=인프로세스 동일)·0017~0018 가설 유지(`recover`/`compact`)·spine 19-step 사슬. **단 (a) rank belief 는 최신값만(이력·top-N 리더보드 없음)** **(b) 읽기 모델 영속 0(ranking 크래시 시 이벤트 재생 필요·late-join 미착수)** **(c) 루프 방지=구독 설계 규약(런타임 사이클 탐지 없음)** **(d) write-behind 윈도(0017 잔존)·채팅/버스/월드 영속 0** **(e) Node·C++ 시뮬 미교체·lockstep 배리어**.
- **다음**: step-0020 (§2).

---

## 2. NEXT — step-0020 가설 (후보, 권위는 이 절)

**step-0019 가 발신 소비자(랭킹·읽기 모델)로 버스의 consume→publish 를 닫았으나, 영속의 *신뢰성* 반쪽(write-behind 윈도)은 0017 부터 열려 있고, 채팅·버스·월드·읽기 모델은 영속 0이다. 가장 자연한 다음 한 조각은 ⒜ *활성 중 가방 failover + write-behind 손실 재수렴*(가방이 op 활성 중 죽어도 ack/resend·write 손실 감지로 완전성 수렴·클라 belief 가 원장 진실로 수렴 — 0017~0018 이 남긴 영속의 *신뢰성* 반쪽·exactly-once 내구성). 또는 ⒝ *채팅·버스 영속*(0017~0018 영속/압축 패턴의 서비스 판 반복), 또는 ⒟ *월드 영속(존 intent 로그)*(계층 6 월드 판 — 크래시 복구·late-join), 또는 ⒢ *읽기 모델 영속·late-join*(ranking 크래시→이벤트 재생 재구성 — 0019 §9), 또는 ⒡ *채팅 신뢰 전달*(0015 §9-2).**

step-0019 가 게임 서비스 층에 발신 소비자(랭킹·CQRS 읽기 모델)를 더했다. 남은 격차: ⓐ *write-behind 윈도*(활성 중 죽음=in-flight 손실 — ack/resend·손실 감지 없음·exactly-once 내구성 미달·0017~0018 잔존, 가장 큰 신뢰성 구멍) ⓑ 채팅·버스·월드·읽기 모델은 *영속 0* ⓒ 스냅샷 자체 *비유계*·압축 *단일 트리거* ⓓ PersistStore 가 *단일점*. **권위는 이 절이되, 다음 step 시작 시 한 조각으로 좁힐 것**. ⒜ 는 exactly-once 내구성(write-ahead + opId 멱등 + 재전송)이라 *다중 메커니즘·고난도* — 한 조각으로 좁히려면 sub-piece(예: 저널 홉 ack/resend 만, 또는 클라 resend 만)로 쪼갤 것. 큰 조각(비동기 결정론·C++ 시뮬 교체)도 여전히 강한 후보.

**검증할 것(후보 — 영속 신뢰성·서비스 영속 기준)**: ① **회귀 0** — 새 항 OFF 면 0019 비트 동일. ② **신성한 tick** — 새 항(ack/resend·영속)이 존 tick 밖·존 비-침습(월드 비트 동일). ③ **E2E 동치** — 멀티프로세스=인프로세스 비트 동일·은닉. ④ **가설** — ⒜ 면 *활성 중 kill→복구 후 완전성 수렴*(in-flight 손실분 재전송/감지로 메워져 belief→원장 수렴·dupe 0·소유≤1), *채팅·버스 영속* 면 "구독/라우팅 kill→재구성 후 누설 0·격리 보존", *읽기 모델 영속* 면 "ranking kill→이벤트 재생 재구성 후 프로젝션==원장·rankDesync 0".

> 후속 한 조각: **활성 중 가방 failover + write 손실 재수렴**(ack/resend·손실 감지·exactly-once — 0017 §9·고난도/분할 필요) · **채팅·버스 영속**(서비스 영속 판 반복) · **읽기 모델 영속·late-join**(0019 §9 — ranking 이벤트 재생 재구성) · **증분 스냅샷·압축 트리거 다양화**(0018 §9) · **월드 영속(존 intent 로그)**(계층 6 월드 판·크래시 복구·late-join) · **PersistStore failover·다중 복제·디스크 fsync**(영속 단일점 제거) · **채팅 신뢰 전달**(0015 §9-2) · **존 넘는 거래**(2PC — 0014 §9-2) · **비동기 실행 아래 결정론**(lockstep 해제·논리 클럭 — 0013 §9-1·큰 조각) · **C++ 시뮬 코어 headless 교체**(§3 🔴).

**병행 백로그(블로킹 아님)**: ⬜ 활성 중 가방 failover + write 손실 재수렴·ack/resend·exactly-once(0017 §9) · ⬜ 채팅·버스 영속·failover(0015 §9-3·0016 §9-2·0017 §9) · ⬜ 읽기 모델 영속·late-join—ranking 이벤트 재생(0019 §9) · ⬜ 증분 스냅샷·압축 트리거 다양화·스냅샷 손실 ack(0018 §9) · ⬜ 월드 영속(존 intent 로그·크래시 복구·late-join — 계층 6 월드 판) · ⬜ PersistStore failover·디스크 fsync·다중 복제(0017 §9 — 영속 단일점) · ⬜ 채팅 신뢰 전달·채널 시퀀스·ack/resend(0015 §9-2) · ⬜ 버스 분산·failover·동적 구독·이력/replay(0016 §9-2·§9-3) · ⬜ 런타임 consume→publish 사이클 정적 탐지(0019 §9 — 루프 방지 그래프 검증) · ⬜ 거래소·우편(서비스 분리 패턴 반복) · ⬜ 존 넘는 거래·2-party 동의·에스크로(0014 §9-2) · ⬜ 비동기 실행 아래 결정론(논리 클럭, 0012 §9-3·0013 §9-1) · ⬜ 물리적 다중-브로커 버스 분산·분산 epoch(0012 §9-1·0013 §9-3) · ⬜ 거짓 사망 플래핑·부분 복귀·stale 호스트 재합류(0013 §9-4) · ⬜ 다중 standby·자가 디스커버리·점진 상태 동기(0013 §9-2) · ⬜ 소켓 층 지연·재정렬·비대칭 분단(0012 §9-4) · ⬜ 다중 클라 결정론 복제·예측(C++ 승격에서 부활) · ⬜ 존 N개·동적 경계(0006 §8.2) · ⬜ 존↔존 ghost 증분화·복원(0007 §8.3·0008 §8.6) · ⬜ 서버간 인증(0001 §8.3) · ⬜ 재접속·티켓 만료(0001 §8.5) · ✅ 발신하는 둘째 소비자—랭킹(0019 — consume→publish·CQRS 읽기 모델·발행자 무수정·루프 없음) · ✅ 가방 저널 스냅샷 압축(0018 — intent 로그+스냅샷·무손실·저널 유계화) · ✅ 가방 서비스 영속·failover(0017 — event sourcing 저널·복구 투명·계층 6 첫 진입) · ✅ 이벤트 버스 서비스 층(0016) · ✅ 채팅 서비스 분리(0015) · ✅ 가방 서비스 분리(0014) · ✅ 시각화·검증 testbed 단일화(0010 후 — [TESTBED.md](TESTBED.md)) · ✅ 시나리오 `inject`(0016 write-seam).

**빌드 인프라 — `engine/` 추출 완료(0004), 0005~0019 무수정 재사용**: 공통 하니스(시드 PRNG·FNV·전송 모델 `Net`·엣지/코디 스텁·동결 `ISimCore` 2구현 = `engine/index.js`, 시각 관찰 키트 = `engine/panel-kit.js`)를 `engine/` 한 곳에. 코어는 **dual-mode**(Node `require` + 브라우저 `<script>` 전역). 0019 는 `engine/` 무수정·존/orch/추종자/가방/채팅/버스/감사/영속·압축/failover 안무 무수정 — net-core 는 0018 그대로 + **RankingService**(svc.item.out 소비→rank 투영→svc.rank.out 발행·onTick 0) + 버스 구독 행(ranking·svc.rank.out) + gateway `_relayRank` + 클라 `rankBelief` 만 더함. `cluster.js`/`host.js` 는 0018 그대로 + ranking snapshot/reconstruct 항만 가산. step-0020 의 net-core 출발 템플릿은 `step-0019/net-core.js`·`step-0019/cluster.js`·`step-0019/host.js`.
...
| 🟡 | **서비스 영속·failover (가방 ✅+압축 ✅·채팅/버스 ⬜)·존 넘는 거래** | 서비스/데이터 | 0017 가방 원장 = event sourcing 저널로 영속·failover(복구 투명). 0018 = 저널 스냅샷 압축(유계화·무손실). 단 *write-behind 윈도*(활성 중 죽음=in-flight 손실·ack/resend 없음 — 0017~0018 잔존)·채팅 구독·버스 라우팅은 *영속 0*. 가방 일방 give(2PC 없음 — 0014 §9-2)·채팅 best-effort(0015 §9-2). |
| 🟡 | **길드·거래소·우편(서비스 반복)·랭킹 ✅** | 서비스 | 0019 RankingService = *발신하는 소비자*(consume→publish·CQRS 읽기 모델·발행자 무수정·루프 없음·프로젝션==원장 — 0016 §9-4 해소). 단 *읽기 모델 영속 0*(ranking 크래시→이벤트 재생 재구성·late-join 미착수·0019 §9)·런타임 사이클 탐지 0. 거래소·우편·길드는 반복 미착수. |
| ⬜ | **세션/프레즌스 + 오케스트레이터** | 코디네이션 | "누가 어디에" SSOT·존 배치·부하 분산·인스턴스 spawn. |
| 🟡 | **캐시 + write-behind 영속 (가방 저널+압축 ✅·복제/월드 ⬜)** | 데이터 | 0017 PersistStore = 가방 효과 저널(write-behind)·계층 6 첫 박스. 0018 = *스냅샷 압축*(intent 로그+주기 스냅샷·무손실·저널 (스냅샷+tail) 유계화 — 종전 "무한 성장" 해소). 단 *스냅샷 비유계(전체 원장 직렬화·증분 0)·디스크 fsync/다중 복제/persist failover 0(단일점)·월드 영속 0(존 intent 로그 미착수)* — 0018 §9. |
| ⬜ | **크래시 복구·재접속·late-join** | 전체 | 영속에서 뷰/권위 재구성 — 소실 권위의 고리 닫기. |

> **✅ 해소된 격차 (0001~0016)** — 골격 토폴로지(0001)·존 결정론 복제(0002)·Sim 인터페이스 동결(0003)·현실 전송 위 복제(0004)·AOI 브로드캐스트(0005)·공간 분할·권위 핸드오프(0006)·증분 AOI(0007)·전송 열화 아래 반응적 복원(0008)·추종자 승격 failover(0009)·프로세스 경계 E2E(0010)·실 TCP 소켓 와이어(0011)·버스 분산·열화 내성(0012)·진짜 프로세스 kill 아래 failover(0013, epoch 펜싱·재-provisioning)·가방 서비스 분리(0014, 게임 서비스 계층 첫 박스·단일 소유·쌍 거래·dupe 0)·채팅 서비스 분리(0015, 게임 서비스 둘째 박스·채널 팬아웃·비-구독자 누설 0·지역 격리)·이벤트 버스 서비스 층(0016, 발행/구독 의미·직접 결합 0·발행자 무수정 소비자 추가)·가방 서비스 영속·failover(0017, event sourcing 저널·복구 투명·계층 6 데이터 첫 진입)·가방 저널 스냅샷 압축(0018, intent 로그+주기 스냅샷·무손실·저널 유계화)·**발신하는 둘째 소비자(0019, RankingService — consume→publish·CQRS 읽기 모델·발행자 무수정·루프 없음 — 0016 §9-4 해소)**. 상세 = 각 `step-NNNN.md` · §7 INDEX · [STATE-INDEX-ARCHIVE.md](STATE-INDEX-ARCHIVE.md).
...
| 3 | 게임 서비스 | 가방 · 채팅 · 길드 · 거래소 · 우편 · 랭킹 | 🟡 0014 가방(InventoryService) 첫 박스 — 단일 소유·쌍 거래·dupe 0 +0015 채팅(ChatService) 둘째 박스 — 채널 팬아웃·지역 격리 +0016 서비스 경로가 버스 발행/구독으로 + AuditService(관찰 소비자) +0017 가방 failover·영속(kill→저널 replay) +0018 저널 스냅샷 압축 +0019 **RankingService**(발신하는 소비자 — svc.item.out 소비→rank 투영→svc.rank.out 발행·CQRS 읽기 모델·발행자 무수정·루프 없음). 전부 별 프로세스·tick 무관 순수 반응형(onTick 0)·신성한 tick·E2E 비트 동일·은닉. 읽기 모델 영속·write-behind 신뢰성·채팅/버스 영속·거래소/우편/길드 후속 |
...
| 6 | 데이터 | 캐시 · DB · write-behind | 🟡 0017 PersistStore 첫 박스 — 가방 효과 저널(event sourcing·write-behind)·진짜 kill→replay 로 원장 재현(복구 투명) +0018 **저널 스냅샷 압축**(intent 로그+주기 스냅샷·무손실·저널 (스냅샷+tail) 유계화). 증분 스냅샷·디스크 fsync·다중 복제·persist failover·월드 영속(존 intent 로그)·채팅/버스 영속·write-behind 윈도 후속 |
