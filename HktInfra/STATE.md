...
- **닫힌 step**: [step-0020](step-0020.md) — 읽기 모델 영속·late-join (`RankingService.crash()`+`reconstruct()` — 자기 영속 0 인 CQRS 읽기 모델의 복구). 0019 가 남긴 한계(랭킹 rank 투영이 RAM·영속 0 → 크래시 시 소실·late-join 미착수, 0019 §9)를 닫는다: rankRestart OFF 면 0019 비트 동일(reg 0) · ON(persist 전제)이면 ranking 을 *진짜 kill*(RAM 투영 소실) 후 **쓰기 모델의 영속 저널**(PersistStore)을 `reconstruct` — mint→owner +1·xfer→from-1/to+1(=item_result 투영과 정확히 같음·저널은 수락 효과만이라 ev.ok 게이트와 1:1)·스냅샷(0018) 베이스+tail — 해 투영을 재계산. 가설의 핵=**읽기 모델은 자기 영속이 필요 없다**(복구 투영==원장 byOwner·자기 영속 0)·**대조군**(persist OFF+kill)은 투영 소실(영속이 복원의 원인)·**같은 저널을 쓰기/읽기 모델이 다른 투영으로 접는다**(inventory.replay 의 읽기 모델 판)·**0018 압축이 reconstruct 도 자동 가속**(tail 0~5항목). 더한 로직은 ranking crash/reconstruct + rankRestart 훅뿐·월드/가방 영속(0017)/압축(0018)/버스/발행구독(0019) 무변경·`engine/` 무수정.
- **한 줄 상태**: 읽기 모델(랭킹)이 *자기 영속 0* 인데도 크래시에서 살아 돌아온다 — 권위(쓰기 모델)의 event 저널을 다시 접으면 투영이 재계산되기 때문(CQRS 읽기 모델 = 버릴 수 있는 캐시, 쓰기 로그가 살아 있는 한). 핵심: ① **읽기 모델 복구 = "내 상태 저장/로드"가 아니라 "쓰기 로그 재생"** — ranking 은 한 바이트도 영속 안 하는데 PersistStore 저널만으로 투영==원장 완전 복원 ② **영속은 *읽기 모델 자신*에 없어도 되지만 *시스템 어딘가*엔 필요**(persist OFF 대조군 소실) ③ **같은 저널, 다른 투영**(inventory.replay→원장 / ranking.reconstruct→보유 수 — 하나의 효과 로그가 여러 읽기 모델·복구를 발행자 무수정으로 먹임) ④ **복구원은 휘발 버스 스트림 아닌 내구 저널**(다운타임 이벤트도 구조적 복원). 검증: 회귀 0(`reg` 25/25 → 0019 비트 동일·rank 포함)·가설(`recover-rank`, 복구 투영==원장 6아바타·persist OFF+kill 소실 rank 0·rankDesync 0·발행 유계)·E2E(`e2e`, 멀티=인프로세스 비트 동일 9중 +rank·persist-bus 12·restart-bus 13·**rank-restart 13** 프로세스)·신성한 tick(`sacred`)·은닉(`hide`)·재현(`repro`)·0017~0019 가설 유지(`recover`/`compact`/`rank`)·spine 20-step 사슬. **단 (a) quiescent restart 만(활성 중 다운타임+재발행 미검증·복구원 내구성은 입증)** **(b) late-join=완전 재계산만(증분 follow·읽기 모델 자체 스냅샷 없음)** **(c) rank belief 최신값만·루프 정적 탐지 0(0019 잔존)** **(d) write-behind 윈도(0017 잔존)·채팅/버스/월드 영속 0** **(e) Node·C++ 시뮬 미교체·lockstep 배리어**.
- **다음**: step-0021 (§2).

---

## 2. NEXT — step-0021 가설 (후보, 권위는 이 절)

**step-0020 이 읽기 모델 복구(랭킹 crash→쓰기 저널 reconstruct·자기 영속 0)로 0019 §9 를 닫았으나, *quiescent* 재시작만 봤고 — *활성 중* 다운타임·재발행과 영속의 *신뢰성* 반쪽(write-behind 윈도)은 여전히 열려 있다. 가장 자연한 다음 한 조각은 ⒜ *활성 중 가방 failover + write-behind 손실 재수렴*(가방이 op 활성 중 죽어도 ack/resend·write 손실 감지로 완전성 수렴 — 0017~0018 영속의 *신뢰성* 반쪽·exactly-once 내구성·가장 큰 구멍). 또는 ⒝ *채팅·버스 영속*(0017~0020 영속/복구 패턴의 서비스 판 반복), 또는 ⒞ *활성 중 읽기 모델 다운타임 + 재발행*(다운타임에 svc.item.out 흘림→reconstruct 후 클라 belief 재발행 재수렴 — 0020 §9 quiescent 한계), 또는 ⒟ *월드 영속(존 intent 로그)*(계층 6 월드 판 — 크래시 복구·late-join), 또는 ⒡ *채팅 신뢰 전달*(0015 §9-2).**

step-0020 이 데이터 계층 복구를 *읽기 모델*로 확장했다. 남은 격차: ⓐ *write-behind 윈도*(활성 중 죽음=in-flight 손실 — ack/resend·손실 감지 없음·exactly-once 내구성 미달·0017~0018 잔존, 가장 큰 신뢰성 구멍) ⓑ 채팅·버스·월드는 *영속 0*·읽기 모델은 *활성 중 다운타임 미검증*(0020) ⓒ 스냅샷 *비유계*·압축 *단일 트리거*(0018) ⓓ PersistStore 가 *단일점*. **권위는 이 절이되, 다음 step 시작 시 한 조각으로 좁힐 것**. ⒜ 는 exactly-once 내구성(write-ahead + opId 멱등 + 재전송)이라 *다중 메커니즘·고난도* — 한 조각으로 좁히려면 sub-piece(저널 홉 ack/resend 만·또는 클라 resend 만)로 쪼갤 것. 큰 조각(비동기 결정론·C++ 시뮬 교체)도 여전히 강한 후보.

**검증할 것(후보 — 영속 신뢰성·서비스 영속 기준)**: ① **회귀 0** — 새 항 OFF 면 0020 비트 동일. ② **신성한 tick** — 새 항(ack/resend·영속)이 존 tick 밖·존 비-침습(월드 비트 동일). ③ **E2E 동치** — 멀티프로세스=인프로세스 비트 동일·은닉. ④ **가설** — ⒜ 면 *활성 중 kill→복구 후 완전성 수렴*(in-flight 손실분 재전송/감지로 메워져 belief→원장 수렴·dupe 0·소유≤1), ⒝ 면 "구독/라우팅 kill→재구성 후 누설 0·격리 보존", ⒞ 면 "활성 중 ranking kill→다운타임 이벤트 reconstruct + 재발행 후 프로젝션==원장·rankDesync 0".

> 후속 한 조각: **활성 중 가방 failover + write 손실 재수렴**(ack/resend·손실 감지·exactly-once — 0017 §9·고난도/분할 필요) · **채팅·버스 영속**(서비스 영속 판 반복) · **활성 중 읽기 모델 다운타임 + 재발행**(0020 §9 — quiescent 한계 해소) · **증분 스냅샷·압축 트리거 다양화**(0018 §9) · **월드 영속(존 intent 로그)**(계층 6 월드 판·크래시 복구·late-join) · **PersistStore failover·다중 복제·디스크 fsync**(영속 단일점 제거) · **채팅 신뢰 전달**(0015 §9-2) · **존 넘는 거래**(2PC — 0014 §9-2) · **비동기 실행 아래 결정론**(lockstep 해제·논리 클럭 — 0013 §9-1·큰 조각) · **C++ 시뮬 코어 headless 교체**(§3 🔴).

**병행 백로그(블로킹 아님)**: ⬜ 활성 중 가방 failover + write 손실 재수렴·ack/resend·exactly-once(0017 §9) · ⬜ 채팅·버스 영속·failover(0015 §9-3·0016 §9-2·0017 §9) · ⬜ 활성 중 읽기 모델 다운타임 + 재발행(0020 §9 — quiescent 한계) · ⬜ 읽기 모델 증분 follow·자체 스냅샷(0020 §9) · ⬜ 증분 스냅샷·압축 트리거 다양화·스냅샷 손실 ack(0018 §9) · ⬜ 월드 영속(존 intent 로그·크래시 복구·late-join — 계층 6 월드 판) · ⬜ PersistStore failover·디스크 fsync·다중 복제(0017 §9 — 영속 단일점) · ⬜ 채팅 신뢰 전달·채널 시퀀스·ack/resend(0015 §9-2) · ⬜ 버스 분산·failover·동적 구독·이력/replay(0016 §9-2·§9-3) · ⬜ 런타임 consume→publish 사이클 정적 탐지(0019 §9 — 루프 방지 그래프 검증) · ⬜ 거래소·우편(서비스 분리 패턴 반복) · ⬜ 존 넘는 거래·2-party 동의·에스크로(0014 §9-2) · ⬜ 비동기 실행 아래 결정론(논리 클럭, 0012 §9-3·0013 §9-1) · ⬜ 물리적 다중-브로커 버스 분산·분산 epoch(0012 §9-1·0013 §9-3) · ⬜ 거짓 사망 플래핑·부분 복귀·stale 호스트 재합류(0013 §9-4) · ⬜ 다중 standby·자가 디스커버리·점진 상태 동기(0013 §9-2) · ⬜ 소켓 층 지연·재정렬·비대칭 분단(0012 §9-4) · ⬜ 다중 클라 결정론 복제·예측(C++ 승격에서 부활) · ⬜ 존 N개·동적 경계(0006 §8.2) · ⬜ 존↔존 ghost 증분화·복원(0007 §8.3·0008 §8.6) · ⬜ 서버간 인증(0001 §8.3) · ⬜ 재접속·티켓 만료(0001 §8.5) · ✅ 읽기 모델 영속·late-join(0020 — 랭킹 crash→쓰기 저널 reconstruct·자기 영속 0·투영==원장·quiescent) · ✅ 발신하는 둘째 소비자—랭킹(0019 — consume→publish·CQRS 읽기 모델·발행자 무수정·루프 없음) · ✅ 가방 저널 스냅샷 압축(0018 — intent 로그+스냅샷·무손실·저널 유계화) · ✅ 가방 서비스 영속·failover(0017 — event sourcing 저널·복구 투명·계층 6 첫 진입) · ✅ 이벤트 버스 서비스 층(0016) · ✅ 채팅 서비스 분리(0015) · ✅ 가방 서비스 분리(0014) · ✅ 시각화·검증 testbed 단일화(0010 후 — [TESTBED.md](TESTBED.md)) · ✅ 시나리오 `inject`(0016 write-seam).

**빌드 인프라 — `engine/` 추출 완료(0004), 0005~0020 무수정 재사용**: 공통 하니스(시드 PRNG·FNV·전송 모델 `Net`·엣지/코디 스텁·동결 `ISimCore` 2구현 = `engine/index.js`, 시각 관찰 키트 = `engine/panel-kit.js`)를 `engine/` 한 곳에. 코어는 **dual-mode**(Node `require` + 브라우저 `<script>` 전역). 0020 은 `engine/` 무수정·존/orch/추종자/가방/채팅/버스/감사/영속·압축/failover/발행구독 안무 무수정 — net-core 는 0019 그대로 + **RankingService.crash()/reconstruct()**(쓰기 저널을 보유 수 투영으로 fold·발신 0) + run() **rankRestart 훅**(crash+reconstruct·invRestart 와 같은 위치)만 더함. `cluster.js` 는 0019 + rankRestart 블록(랭킹 kill→ranking_r spawn·init·reconstruct), `host.js` 는 + `cmd:'reconstruct'` 만 가산. step-0021 의 net-core 출발 템플릿은 `step-0020/net-core.js`·`step-0020/cluster.js`·`step-0020/host.js`.
...
| 6 | 데이터 | 캐시 · DB · write-behind | 🟡 0017 PersistStore 첫 박스 — 가방 효과 저널(event sourcing·write-behind)·진짜 kill→replay 로 원장 재현(복구 투명) +0018 **저널 스냅샷 압축**(intent 로그+주기 스냅샷·무손실·저널 (스냅샷+tail) 유계화) +0020 같은 저널이 *읽기 모델 복구원*으로도 쓰임(랭킹 reconstruct — 쓰기·읽기 모델이 하나의 효과 로그를 다른 투영으로 fold). 증분 스냅샷·디스크 fsync·다중 복제·persist failover·월드 영속(존 intent 로그)·채팅/버스 영속·write-behind 윈도 후속 |
