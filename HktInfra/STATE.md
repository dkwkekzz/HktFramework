...
- **닫힌 step**: [step-0040](step-0040.md) — **요청 replay 버퍼 *자기-크기조정*(ack 가지치기·busAck)**. 0039 고정 K(busWindow)는 *최대 예상 gap(다운타임×발신율)* 사전 추정 필요(작으면 손실·크면 낭비·0039 §9). 가방이 처리한 reqId 를 `svc.item.ack` 로 통보 → 게이트웨이가 ack 워터마크(`inAcked`) 이하 `inBuffer` 가지치기 → 버퍼엔 *미-ack(in-flight)* 만 남는다(자기-크기조정). 정상 구간 ack 흘러 drain·gap 구간 ack 끊겨 자동 성장 → 복구 replay 가 그만큼 덮어 *K 추정 없이* 무손실. 닿는 파일 svc-inventory.js(ack 발행)·gateway.js(가지치기·peak)·topo-build.js(배선·구독)·verify.js.
- **한 줄 상태**: reg 25/25(0039 비트 동일·busAck=0 휴면)·busack ALL OK — ack minted==base·desync 0(K 추정 0) / 미-튜닝 fixedK8(<gap) minted−10·desync 4(대조) / ack final 0(idle drain)·peak 가동-길이 무관(unbnd 60→120→180 vs ack peak 24 고정). dupe 0·존 tick 비-침습·E2E 비트 동일·spine **40-step 사슬**·close-step 통과.
- **다음**: §2 참조 — 결과 경로 ack-가지치기 · seenReqs 유계화 · give 요청×result-ahead 결합 · 비동기 결정론(🔴 최우선). 정리 후보: svc-inventory.js 27KB(30KB 근접).

---

## 2. NEXT — step-0040 후 가설 (후보, 권위는 이 절)

**step-0040 가 ⒝′-*요청 버퍼 자기-크기조정*(ack 가지치기·K 추정 없이 무손실·가동-길이 무관·idle drain)을 닫았다. 남은 후보: ⒝″ *결과 경로 ack-가지치기*(0036 outBuffer 는 아직 고정 busWindow — 게이트웨이→가방 결과 ack 의 거울·0040 §9), ⒝‴ *seenReqs 유계화*(가방 dedup 집합 무계 — ack 워터마크 기반 유계화·0040 §9), ⒝-*give 요청×result-ahead 결합*(give 재발행 transfers≠base·0025 give-resend 통합·0037 §9), ⒞ *활성 중 서비스 다운타임 일반 재발행*(0025/0026 quiescent → 온라인 kill+재발행 핸드셰이크), ⒟ *비동기 결정론(lockstep 배리어 해제)*(논리/벡터 클럭·🔴 최우선·broker 대공사·0012 §9-3), 🔧 정리 후보(svc-inventory.js 27KB→30KB 근접).**
...
> **✅ 해소된 격차 (0001~0037)** — 전문은 §7 INDEX(1줄/step)·각 `step-NNNN.md`. 묶음: 골격/복제/Sim 동결/현실 전송(0001~0004)·AOI·분할·핸드오프·증분·복원·failover(0005~0009)·프로세스 경계·TCP·버스 분산·진짜 kill failover(0010~0013)·게임 서비스 분리(0014~0015)·이벤트 버스 의미(0016)·가방/채팅 영속+압축(0017~0022)·발신 소비자·읽기 모델 영속(0019~0020)·write-behind 신뢰성 완결(0023~0026)·persist failover·N-replica·quorum write ack·윈도 해소(0027~0032)·버스 동적 구독+failover+결과/요청경로 무손실+replay 버퍼 유계화+요청버퍼 ack 자기-크기조정(0033~0037·0039~0040).
