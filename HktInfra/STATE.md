...
- **닫힌 step**: [step-0038](step-0038.md) — **정리 step: topology.js 박스-부품 분할(build / run·entry)·기능 0·바이트 동일**. 0037 에서 topology.js 가 31KB>30KB 박스 임계를 넘김 → 다음 기능 step 전 재분할(0030 net-core·0035 cluster 의 topology 판). *토폴로지 구성*(routeFilters·buildTopology·makeActor)을 신규 `topo-build.js`(15.9KB)로 verbatim 이동, `topology.js`(17.1KB)는 *run 드라이버+진입점*(quorumMergeJournals·run·runMulti)으로 남아 build 부품 require·동일 6 export 재노출. net-core 부품 목록에 topo-build 추가. 닿는 파일 topology.js·net-core.js + 신규 topo-build.js.
- **한 줄 상태**: 함수 본문 무편집 이동 → 동작·export 불변. reg 25/25(0037 비트 동일)·busreq carried(분할 후 요청 replay 그대로·minted==base·split 투명성)·박스 전부 <30KB(topology 31→17.1·topo-build 15.9)·18+busreq ALL OK·멀티프로세스 E2E 비트 동일(cluster 무수정)·spine **38-step 사슬**·close-step 통과. 부수: 죽은 import(itemDesync·invDigest) 정리.
- **다음**: §2 참조 — 유계 in/outBuffer(0032 wfWindow 패턴) · give 요청×result-ahead 결합 · 비동기 결정론(🔴 최우선). 정리 후보: svc-inventory.js 26.5KB(30KB 근접).

---

## 2. NEXT — step-0038 후 가설 (후보, 권위는 이 절)

**step-0038 은 정리 step(topology.js 박스-부품 분할·기능 0·reg 0)이었다 — 새 가설 없음. 0037 이 깐 버스 failover 요청 경로 무손실(gateway producer replay + reqId dedup) 위에서 남은 기능 후보: ⒝-남은 *유계 inBuffer/outBuffer*(0036/0037 무계 → 0032 wfWindow 슬라이딩 K 창으로 유계화·gap 만 덮기) + *give 요청 × result-ahead 결합*(give 재발행은 transfers≠base·클라 재-give 와 얽힘 → 0025 give-resend 통합 복구), ⒞ *활성 중 서비스 다운타임 일반 재발행*(0025/0026 은 quiescent — 온라인 kill+재발행 핸드셰이크), ⒟ *비동기 결정론(lockstep 배리어 해제)*(논리/벡터 클럭·🔴 최우선·broker=cluster-core.js/cluster-run.js 대공사·0012 §9-3), 🔧 *다음 정리 후보*(svc-inventory.js 26.5KB→30KB 근접·기능 step 이 키우면 원장코어/영속/replay 재분할).**
