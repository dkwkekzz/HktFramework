...
- **닫힌 step**: [step-0435](step-0435.md) — **#4 진짜 비동기 5: 인과 의존 배달**: `async-core.js` `causalDeliver`(deps=happens-before 선행 충족 시에만 방출·FIFO-free)·`causalViolations`. 적대적 도착 6종(역순/셔플) 전부 인과 위반 0·미배달 0. run() 미호출 → reg 구조적 0. **#4 substrate sub-arc(0431~0440) 진행**.
- **한 줄 상태**: reg ALL OK(async-core run() 미호출)·lccausal 5/5(적대적 도착 위반0·stuck0)·async-core.js 10.5KB·박스 >30KB 0개·spine ALL OK.
