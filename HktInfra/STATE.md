...
- **닫힌 step**: [step-0028](step-0028.md) — PersistStore N-replica + quorum: `_journal` 이 primary+복제 N개(`persist2`..)에 fan-out, `invRestart` 복구 = 생존 저널 union(`quorumMergeJournals`). primary 포함 N 죽어도·각 복제 부분 저널만 가져도 union 완전 → 무손실. persistReplicas 0 면 휴면 → 0027 비트 동일(reg 0). 0027 §9(2복제→backup죽으면 단일점) 해소.
- **한 줄 상태**: 핵심 ① **2복제로는 부족 — N복제로 일반화** ② **복구 = 한 사본 고르기 아닌 생존 사본 union(quorum read) — 부분쓰기도 메움, 단 정족수 경계 있음**. 검증: 회귀 0(`reg` 25/25 → 0027 비트 동일·N-replica 휴면)·가설(`quorum`: ⒜완전복제 3죽음 생존1 무손실 / ⒝부분쓰기 생존3 union==base 무손실(복제 ~2/3 보유) / ⒞생존1 부분→손실=정족수 경계)·desync 0·보존정합·spine 28-step 사슬. **단 (a) quorum *write* ack(W 정족수) 미구현 — fan-out 은 fire-and-forget (b) 버스·월드 영속 0 (c) 활성 중 다운타임 일반 재발행 미검증 (d) C++ 시뮬·lockstep 배리어**.
- **다음**: §2 참조 — N-replica 후 새 후보 선택.

---

## 2. NEXT — step-0028 가설 (후보, 권위는 이 절)

**step-0028 이 N-replica + quorum-merge 복구로 영속 단일점을 일반 해소했다(quorum *read*). 다음 후보: ⒜ *Quorum 쓰기 ack*(W개 복제 ack 후 durable 선언 — 현재 fan-out 은 fire-and-forget·primary 도달 후 일부 복제 미도달 crash 시 정합성 윈도·0028 §9), 또는 ⒝ *버스 영속·failover*(서비스 복구 패턴의 버스 판·동적 구독 선결·0016 §9-2), 또는 ⒞ *활성 중 서비스 다운타임의 일반 재발행*(0025/0026 는 quiescent 복구 — 온라인 중 kill + 재발행 핸드셰이크), 또는 ⒟ *비동기 결정론(lockstep 배리어 해제)*(논리 클럭·Lamport·벡터 클럭·🔴 최우선·0012 §9-3).**

남은 격차: ⓐ 버스·월드 *영속 0*·서비스 복구 *활성 중 다운타임 일반형 미검증* ⓑ 스냅샷 *비유계*·압축 *단일 트리거*·재전송 amplification·heartbeat×압축 미결합·채팅 in-flight/홉 신뢰 미적용 ⓒ quorum *write* ack 미구현(fan-out=fire-and-forget·primary 도달 후 일부 복제 미도달 crash 시 정합성 윈도·디스크 fsync 0·복제 anti-entropy 0) ⓓ 비동기 결정론(lockstep 배리어) ⓔ give-resend opSeq 견고화·clientResync 멀티프로세스 패리티(0025 §9ⓕⓖ).

**검증할 것(공통)**: ① **회귀 0** — 새 항 OFF 면 0027 비트 동일. ② **신성한 tick** — 새 항이 존 tick 밖·존 비-침습. ③ **E2E 동치** — 멀티프로세스=인프로세스 비트 동일·은닉. ④ **가설** — 선택한 조각의 고장 시나리오 주입·복구 수렴 증명.

**병행 백로그(블로킹 아님)**: ⬜ **give-resend 견고화: opSeq 키 매칭**(0025 §9ⓕ) · ⬜ **clientResync/resendGives 멀티프로세스 패리티**(0025 §9ⓖ) · ⬜ 활성 중 다운타임의 *일반* 재발행 핸드셰이크 · ⬜ 손실 재전송·재전송 amplification 배칭(0023/0024 §9) · ⬜ heartbeat×압축 결합·채팅 in-flight/홉 신뢰(0024/0025 §9) · ⬜ 버스 영속·failover(0016 §9-2·동적 구독 선결) · ⬜ 활성 중 서비스 다운타임 + 재발행(0020~0022 §9) · ⬜ 읽기 모델 증분 follow·자체 스냅샷(0020 §9) · ⬜ 증분 스냅샷·압축 트리거 다양화(0018/0022 §9) · ⬜ 월드 영속(존 intent 로그·tick-결합 고위험) · ⬜ Quorum *write* ack(W 정족수 후 durable)·디스크 fsync·복제 anti-entropy(0028 §9) · ⬜ 버스 분산·failover·동적 구독·이력/replay(0016 §9-2·§9-3) · ⬜ 런타임 consume→publish 사이클 정적 탐지(0019 §9) · ⬜ 거래소·우편(서비스 분리 패턴 반복) · ⬜ 존 넘는 거래·2-party 동의·에스크로(0014 §9-2) · ⬜ 비동기 실행 아래 결정론(논리 클럭, 0012 §9-3) · ⬜ 분산 시스템 심화 묶음 · ⬜ 월드 심화 묶음 · ⬜ 서버간 인증·재접속·티켓 만료. (✅ 해소분 0014~0027 = §3 ✅ 묶음·§7 INDEX 참조.)
...
| 🟡 | **서비스 영속·failover (가방 ✅+압축 ✅+저널홉 신뢰 ✅+tail 감지 ✅+in-flight give ✅+mint id-reconciliation ✅·채팅 ✅+압축 ✅·버스 ⬜)·존 넘는 거래** | 서비스/데이터 | 0017 가방 원장 = event sourcing 효과 저널(복구 투명)·0018 스냅샷 압축. 0021 채팅 = *커맨드 로그* event sourcing·0022 라우팅 스냅샷 압축. 0023 *저널 홉 신뢰 전달*(중간 갭 NAK)·0024 *tail 손실 감지*(heartbeat)·0025 *in-flight give 손실 복구*(클라 give-resend → 복구 원장 belief 재수렴·itemId 보존·멱등)·0026 *in-flight mint 손실 복구*(id-reconciliation — 클라 belief 선언→서버 re-mint→newId 채택→itemDesync 0·write-behind 신뢰성 완결). 단 버스 라우팅은 *영속 0*·채팅 in-flight/홉 신뢰 미적용. 가방 일방 give(2PC 없음 — 0014 §9-2). |
...
| 🟡 | **캐시 + write-behind 영속 (가방·채팅 저널+압축 ✅·저널홉 신뢰 ✅+tail 감지 ✅+in-flight give+mint ✅·persist failover ✅·N-replica+quorum-read ✅·quorum-write/월드 ⬜)** | 데이터 | 0017 PersistStore = 가방 효과 저널(write-behind)·계층 6 첫 박스. 0018 가방·0022 채팅 스냅샷 압축. 0023 *저널 홉 신뢰 수신*(seq 갭 NAK·dedup). 0024 *tail 손실 감지*(heartbeat→갭 스캔 상한 확장). 0025 *in-flight give 손실 복구*(클라 give-resend → 복구 원장==무손실 truth·라이브 ack≠내구성). 0026 *in-flight mint 손실 복구*(id-reconciliation — 클라 belief 선언→서버 re-mint→클라 newId 채택·write-behind 신뢰성 완결). 0027 *PersistStore failover*(이중쓰기 backup·2복제). 0028 *N-replica + quorum-read*(복제 N개 fan-out·생존 저널 union 복구 — primary 포함 N 죽음·부분쓰기에도 무손실·정족수 경계). 단 *quorum write ack 미구현(fan-out=fire-and-forget)·정합성 윈도·디스크 fsync·복제 anti-entropy·스냅샷 비유계·압축 단일 트리거·월드 영속 0* — 0028/0018/0022/0024~0027 §9. |
| ⬜ | **크래시 복구·재접속·late-join** | 전체 | 영속에서 뷰/권위 재구성 — 소실 권위의 고리 닫기. |

> **✅ 해소된 격차 (0001~0027)** — 전문은 §7 INDEX(1줄/step)·각 `step-NNNN.md`. 묶음: 골격/복제/Sim 동결/현실 전송(0001~0004)·AOI·공간 분할·핸드오프·증분·반응적 복원·failover(0005~0009)·프로세스 경계·TCP·버스 분산·진짜 kill failover(0010~0013)·게임 서비스 분리(가방 0014·채팅 0015)·이벤트 버스 의미(0016)·가방 영속+압축(0017~0018)·발신 소비자·읽기 모델 영속(0019~0020)·채팅 영속+압축(0021~0022)·write-behind 신뢰성 완결: 중간 갭 NAK(0023)+tail heartbeat(0024)+in-flight give 복구(0025)+in-flight mint id-reconciliation(0026)·PersistStore 이중쓰기 backup(0027)·**N-replica + quorum-read 복구(0028 — 영속 단일점 일반 해소)**.
...
| [0026](step-0026.md) | in-flight mint 손실 복구: id-reconciliation (클라 belief 선언→서버 re-mint→newId 채택 — write-behind 신뢰성 완결·0025 §9 해소) | 통과 · mintRecon OFF=0025 비트 동일(25/25·reconcile 휴면)·in-flight mint 손실 아래 recon ON=itemDesync **0**·per-avatar count==truth(전 시드)·reconcile 6회·dupe 0·ledgerConsistent 예 / OFF=itemDesync **6**·0 복원. 버그 2건: replay xfer maxMintId 미추적(mintTotal=0 충돌)·item_reconcile owned 하한 미보정(re-mint ID 충돌). E2E 비트 동일(휴면)·spine 26-step 사슬 |
