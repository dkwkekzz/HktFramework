...
- **닫힌 step**: [step-0025](step-0025.md) — in-flight give 손실 복구 (`Client.givePending`/`giveLog`/`resendGives`/`_confirmGive` + run 루프 `clientResync` 트리거 — 0024 §9 in-flight 해소[give 한정]). clientResend OFF 면 0024 비트 동일(reg 0) · ON 이면 가방 *활성 중* 죽음으로 미-ack sentBuffer(in-flight xfer 저널) 소실 → 복구 원장이 클라 belief 보다 *뒤처짐* 인데, 클라가 *확인한 give* 를 **같은 itemId 로 재발행**→복구 원장(아이템이 sender 소유)이 `owner==from` 으로 재적용→**원장이 belief 따라잡음**. 가방/persist/host.js/`engine/` **무수정** — 순수 클라 측 한 조각(give 멱등은 0014 `owner==from` 가 이미 제공). **give 한정: pickup mint 손실은 itemId 서버 할당이라 재발행=새 id→belief 복구 불가(§9 id-reconciliation)**.
- **한 줄 상태**: 핵심 ① **라이브 ack ≠ 내구성** ② **belief = 의도된 진실**(클라가 다시 말하면 원장이 따라잡음·give 는 id 보존) ③ **멱등은 0014 `owner==from` 가 공짜**. 검증: 회귀 0(`reg` 25/25 → 0024 비트 동일·clientResend 휴면)·가설(`inflight`, OFF=itemDesync **3~6**·전송 0반영 / ON=재발행 3~9→itemDesync **0**·복구 원장==무손실 truth·dupe 0·부분손실 0.5 도 멱등)·0023~0024 유지·E2E 휴면 비트 동일·spine 25-step 사슬. **단 (a) in-flight *mint* 손실(id-reconciliation)=write-behind 마지막 잔여 (b) 재발행 배달 신뢰 모델·트리거 run 루프 제어 평면·채팅 in-flight 미적용 (c) persist 단일점·버스/월드 영속 0·재전송 amplification·C++ 시뮬·lockstep 배리어**.
- **다음**: step-0026 (§2).

---

## 2. NEXT — step-0026 가설 (후보, 권위는 이 절)

**step-0025 가 in-flight *give* 손실을 클라 give-resend 로 닫았다 — write-behind 신뢰성은 전송손실(중간 0023·tail 0024) + in-flight give(0025)까지 닫혔다. 남은 잔여는 ⒜ *in-flight mint 손실*(itemId 서버 할당이라 재발행=새 id → 클라 belief 재구성[id-reconciliation] 필요 — 0025 §9 의 사용자-연기분). 또는 ⒝ *버스 영속·failover*(서비스 복구 패턴의 버스 판·동적 구독 선결), 또는 ⒞ *월드 영속(존 intent 로그)*(계층 6 월드 판 — 클라 seen 의 tick-결합 재구성=고위험), 또는 ⒟ *활성 중 서비스 다운타임의 일반 재발행*(0025 는 quiescent 복구 핸드셰이크·트리거가 run 루프 제어 평면).**

남은 격차: ⓐ **in-flight mint 손실**(id-reconciliation — 클라가 복구 시 새 id 채택·belief 재구성·0025 §9) — write-behind 의 *마지막* 잔여 ⓑ 버스·월드는 *영속 0*·서비스 복구는 *활성 중 다운타임 일반형 미검증*(0020~0022·0025 는 quiescent) ⓒ 스냅샷 *비유계*·압축 *단일 트리거*(0018/0022)·재전송 amplification(NAK 폭주·0023/0024)·heartbeat×압축 미결합(0024)·채팅 in-flight/홉 신뢰 미적용 ⓓ PersistStore 가 *단일점*. **권위는 이 절이되, 다음 step 시작 시 한 조각으로 좁힐 것**. ⒜ mint id-reconciliation 은 중~고난도(클라 belief 재구성·서버 새 id 채택 프로토콜). ⒝ 는 검증된 영속·복구 패턴의 버스 판이나 *버스 라우팅이 정적 spec* 이라 동적 구독 선결. ⒞ 는 *고위험*(tick-결합 코어·클라 seen 재구성). 큰 조각(비동기 결정론·C++ 시뮬 교체)도 여전히 강한 후보.

**검증할 것(후보 — 영속 신뢰성 마지막·버스 영속 기준)**: ① **회귀 0** — 새 항 OFF 면 0025 비트 동일. ② **신성한 tick** — 새 항(클라 reconciliation·영속)이 존 tick 밖·존 비-침습. ③ **E2E 동치** — 멀티프로세스=인프로세스 비트 동일·은닉. ④ **가설** — ⒜-mint 면 "활성 중 kill→클라가 새 id 채택·belief 재구성→원장 수렴·dupe 0", ⒝ 면 "버스 라우팅 kill→재구성 후 결합 0 보존".

**병행 백로그(블로킹 아님)**: ⬜ **in-flight mint 손실 + id-reconciliation**(0025 §9·write-behind 마지막 잔여·클라 새 id 채택·belief 재구성) · ⬜ **give-resend 견고화: opSeq 키 매칭**(0025 §9ⓕ·리뷰 — `_confirmGive` 가 클라 홉 in-order 에 암묵 의존·재정렬 시 틀린 toAvatar 기록·발신 순번 키로 견고화) · ⬜ **clientResync/resendGives 멀티프로세스 패리티**(0025 §9ⓖ·리뷰 — 트리거가 인프로세스 run() 에만·host.js/runMulti 미구현) · ⬜ 활성 중 다운타임의 *일반* 재발행 핸드셰이크(0025 는 quiescent·run 루프 트리거) · ⬜ 손실 재전송·재전송 amplification(NAK 폭주) 배칭(0023/0024 §9) · ⬜ heartbeat×압축 결합·채팅 in-flight/홉 신뢰(0024/0025 §9) · ⬜ 버스 영속·failover(0016 §9-2·0017 §9·동적 구독 선결) · ⬜ 활성 중 서비스 다운타임 + 재발행(0020~0022 §9) · ⬜ 읽기 모델 증분 follow·자체 스냅샷(0020 §9) · ⬜ 증분 스냅샷·압축 트리거 다양화(0018/0022 §9) · ⬜ 월드 영속(존 intent 로그·크래시 복구·late-join — 계층 6 월드 판·tick-결합 고위험) · ⬜ PersistStore failover·디스크 fsync·다중 복제(0017 §9 — 영속 단일점) · ⬜ 버스 분산·failover·동적 구독·이력/replay(0016 §9-2·§9-3) · ⬜ 런타임 consume→publish 사이클 정적 탐지(0019 §9) · ⬜ 거래소·우편(서비스 분리 패턴 반복) · ⬜ 존 넘는 거래·2-party 동의·에스크로(0014 §9-2) · ⬜ 비동기 실행 아래 결정론(논리 클럭, 0012 §9-3·0013 §9-1) · ⬜ 분산 시스템 심화 묶음(다중-브로커 버스 분산·분산 epoch·거짓 사망 플래핑·부분 복귀·다중 standby·디스커버리·소켓 층 비대칭 분단 — 0012~0013 §9) · ⬜ 월드 심화 묶음(다중 클라 복제·예측[C++ 승격]·존 N개·동적 경계·존↔존 ghost 증분화 — 0006~0008 §8) · ⬜ 서버간 인증·재접속·티켓 만료(0001 §8.3·§8.5). (✅ 해소분 0014~0025 = §3 ✅ 묶음·§7 INDEX 참조.)

**빌드 인프라 — `engine/` 추출 완료(0004), 0005~0025 무수정 재사용**: 공통 하니스(시드 PRNG·FNV·전송 모델 `Net`·엣지/코디 스텁·동결 `ISimCore` 2구현 = `engine/index.js`, 시각 관찰 키트 = `engine/panel-kit.js`)를 `engine/` 한 곳에. 코어는 **dual-mode**(Node `require` + 브라우저 `<script>` 전역). 0025 는 `engine/`·host.js·cluster.js·**가방·persist 무수정**(give 멱등은 이미 0014 `owner==from`·클라 재발행은 정규 item_give 와이어)·존/orch/추종자/버스/감사/랭킹·채팅·0023~0024 신뢰 전달 무수정 — net-core 는 0024 그대로 + **Client `givePending`/`giveLog`/`resendGives`/`_confirmGive`**(확인된 give 보관·복구 시 재발행·OFF 면 휴면 = 0024 비트 동일) + **run 루프 `clientResync` 트리거**만 더함. in-flight give 손실은 `XGIVELOSS`(xfer 저널만 드롭·mint durable→id 보존·재발행 제외)로 전송층 주입. step-0026 의 net-core 출발 템플릿은 `step-0025/net-core.js`·`step-0025/cluster.js`·`step-0025/host.js`.
...
| 🟡 | **서비스 영속·failover (가방 ✅+압축 ✅+저널홉 신뢰 ✅+tail 감지 ✅+in-flight give ✅·채팅 ✅+압축 ✅·버스 ⬜)·존 넘는 거래** | 서비스/데이터 | 0017 가방 원장 = event sourcing 효과 저널(복구 투명)·0018 스냅샷 압축. 0021 채팅 = *커맨드 로그* event sourcing·0022 라우팅 스냅샷 압축. 0023 *저널 홉 신뢰 전달*(중간 갭 NAK)·0024 *tail 손실 감지*(heartbeat)·0025 *in-flight give 손실 복구*(클라 give-resend → 복구 원장을 belief 로 재수렴·itemId 보존·멱등). write-behind 신뢰성 거의 완성. 단 *in-flight mint 손실*(itemId 재할당·id-reconciliation — 0025 §9)·버스 라우팅은 *영속 0*·채팅 in-flight/홉 신뢰 미적용. 가방 일방 give(2PC 없음 — 0014 §9-2). |
...
| 🟡 | **캐시 + write-behind 영속 (가방·채팅 저널+압축 ✅·저널홉 신뢰 ✅+tail 감지 ✅+in-flight give ✅·복제/월드 ⬜)** | 데이터 | 0017 PersistStore = 가방 효과 저널(write-behind)·계층 6 첫 박스. 0018 가방·0022 채팅 스냅샷 압축. 0023 *저널 홉 신뢰 수신*(seq 갭 NAK·dedup). 0024 *tail 손실 감지*(heartbeat→갭 스캔 상한 확장). 0025 *in-flight give 손실 복구*(클라 give-resend → 복구 원장==무손실 truth·라이브 ack≠내구성 격차 닫힘). write-behind 신뢰성 거의 완성. 단 *in-flight mint 손실(id-reconciliation·0025 §9)·스냅샷 비유계·압축 단일 트리거·재전송 amplification·heartbeat×압축 미결합·디스크 fsync/다중 복제/persist failover 0(단일점)·월드 영속 0* — 0018/0022/0024/0025 §9. |
| ⬜ | **크래시 복구·재접속·late-join** | 전체 | 영속에서 뷰/권위 재구성 — 소실 권위의 고리 닫기. |

> **✅ 해소된 격차 (0001~0025)** — 전문은 §7 INDEX(1줄/step)·각 `step-NNNN.md`. 묶음: 골격/복제/Sim 동결/현실 전송(0001~0004)·AOI·공간 분할·핸드오프·증분·반응적 복원·failover(0005~0009)·프로세스 경계·TCP·버스 분산·진짜 kill failover(0010~0013)·게임 서비스 분리(가방 0014·채팅 0015)·이벤트 버스 의미(0016)·가방 영속+압축(0017~0018)·발신 소비자·읽기 모델 영속(0019~0020)·채팅 영속+압축(0021~0022)·**write-behind 신뢰성: 중간 갭 NAK(0023)+tail heartbeat(0024)+in-flight give 복구(0025)**.
...
| 6 | 데이터 | 캐시 · DB · write-behind | 🟡 0017 PersistStore 첫 박스 — 가방 효과 저널(event sourcing·write-behind)·진짜 kill→replay 로 원장 재현(복구 투명) +0018 **저널 스냅샷 압축**(intent 로그+주기 스냅샷·무손실·저널 (스냅샷+tail) 유계화) +0020 같은 저널이 *읽기 모델 복구원*으로도(랭킹 reconstruct) +0021 **chatpersist**(PersistStore 재사용 독립 인스턴스 — 채팅 커맨드 로그) +0022 **채팅 커맨드 로그 스냅샷 압축**(라우팅 파생 상태 스냅샷·제네릭 PersistStore 핸들러 가방·채팅 공용) +0023 **저널 홉 신뢰 수신**(PersistStore seq 갭 감지 NAK·recvSeqs 멱등 dedup) +0024 **tail 손실 감지**(expectedMaxSeq — heartbeat 의 maxSentSeq 로 갭 스캔 상한 확장) +0025 **in-flight give 손실 복구**(가방/persist 무수정 — 클라 give-resend 가 복구 원장을 belief 로 재수렴·복구 원장==무손실 truth·라이브 ack≠내구성 격차 닫힘). in-flight mint 손실(id-reconciliation)·증분 스냅샷·NAK 배칭·디스크 fsync·다중 복제·persist failover·월드 영속(존 intent 로그)·버스 영속 후속 |
...
| [0025](step-0025.md) | in-flight give 손실 복구 (클라 give-resend 로 복구 원장을 belief 로 재수렴 — 라이브 ack≠내구성·0024 §9 in-flight 해소[give 한정]) | 통과 · clientResend OFF=0024 비트 동일(25/25·give-resend 휴면 증명)·in-flight give 손실 아래 resend ON=itemDesync **0**·복구 원장==무손실 truth(invDigest 동일)·재발행 3~9·전송 전부 재적용·dupe 0(own≤1)·conserved/consistent / OFF=원장 belief 뒤처짐 **itemDesync 3~6**·전송 0반영·가방/persist 무수정(give 멱등=0014 owner==from)·E2E 비트 동일(휴면)·spine 25-step 사슬. in-flight *mint* 손실(id-reconciliation) §9 |
