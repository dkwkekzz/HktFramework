...
- **닫힌 step**: [step-0021](step-0021.md) — 채팅 서비스 영속·failover (`ChatService.crash()`+`replay()` — 커맨드 로그 event sourcing 으로 라우팅+deliveries 복구). 0017 가방·0020 랭킹에 이은 *서비스 복구 3부작의 마지막*: chatpersist OFF 면 0020 비트 동일(reg 0) · ON 이면 채팅 *진짜 kill*(RAM 라우팅 소실) 후 **커맨드 로그**(chatpersist=PersistStore 독립 인스턴스)를 `replay`(_process 리듀서를 seq 순 재실행·`replaying` 가드로 재발신 0)해 라우팅+deliveries 재유도. 핵심=**커맨드 소싱**(가방은 *효과* mint/xfer / 채팅 say 팬아웃은 *그 시점 라우팅* 의존이라 *커맨드*를 적고 리듀서 재실행)·**복구 투명**(chatDigest 무재시작 동일)·**복구 후 say 누설 0·격리**·**단일 리듀서 공유**(onMsg·replay 둘 다 `_process`). 더한 것은 ChatService.crash/replay+커맨드 로그+chatRestart 훅뿐·0017~0020/버스 무변경·`engine/` 무수정.
- **한 줄 상태**: 채팅도 죽었다 살아난다 — 커맨드 로그 replay 로 라우팅+deliveries 재유도. 핵심: ① **효과 소싱 vs 커맨드 소싱은 상태의 성질이 가른다**(가방 원장=값→효과 재적용 / 채팅 deliveries=라우팅 의존 파생→커맨드 재실행) ② **단일 리듀서 공유가 복구 정확성의 토대**(live·replay 가 같은 `_process`·replaying 으로 재발신만 끔) ③ **라우팅 복구는 *작동적으로* 검증**(복구 후 say 누설 0·격리) ④ **서비스 복구 3부작 완성**(가방·랭킹·채팅). 검증: 회귀 0(`reg` 25/25 → 0020 비트 동일)·가설(`recover-chat`, 복구 투명 배달 211~229·OFF+kill 소실 0·누설 0·chatDesync 0)·E2E(`e2e`, +chatpersist·**chat-restart 14** 프로세스·커맨드 78 replay)·신성한 tick/은닉/재현·0017~0020 가설 유지·spine 21-step 사슬. **단 (a) quiescent restart 만(활성 중 다운타임+재발행 미검증)** **(b) 커맨드 로그 비유계(채팅 판 0018 압축 미착수)** **(c) write-behind 윈도(0017 잔존)·버스/월드 영속 0** **(d) Node·C++ 시뮬 미교체·lockstep 배리어**.
- **다음**: step-0022 (§2).

---

## 2. NEXT — step-0022 가설 (후보, 권위는 이 절)

**step-0021 이 채팅 복구(커맨드 로그 replay)로 서비스 복구 3부작(가방·랭킹·채팅)을 완성했으나, 전부 *quiescent* 재시작이고 — 영속의 *신뢰성* 반쪽(write-behind 윈도)은 0017 부터 열려 있다. 가장 자연한 다음 한 조각은 ⒜ *활성 중 가방 failover + write-behind 손실 재수렴*(가방이 op 활성 중 죽어도 ack/resend·write 손실 감지로 완전성 수렴 — exactly-once 내구성·가장 큰 구멍). 또는 ⒝ *채팅 커맨드 로그 스냅샷 압축*(0018 의 채팅 판 — 라우팅 스냅샷+tail·커맨드 로그 유계화), 또는 ⒞ *버스 영속·failover*(서비스 복구 패턴의 버스 판), 또는 ⒟ *월드 영속(존 intent 로그)*(계층 6 월드 판 — 크래시 복구·late-join), 또는 ⒠ *활성 중 서비스 다운타임 + 재발행*(0020/0021 §9 quiescent 한계 — 다운타임 이벤트 후 클라 belief 재수렴).**

step-0021 이 서비스 복구를 채팅까지 확장했다. 남은 격차: ⓐ *write-behind 윈도*(활성 중 죽음=in-flight 손실 — ack/resend·손실 감지 없음·exactly-once 내구성 미달·0017 부터 잔존, 가장 큰 신뢰성 구멍) ⓑ 버스·월드는 *영속 0*·서비스 복구는 *활성 중 다운타임 미검증*(0020/0021) ⓒ 채팅 커맨드 로그 *비유계*(압축 없음·0021)·가방 스냅샷 *비유계*·압축 *단일 트리거*(0018) ⓓ PersistStore 가 *단일점*. **권위는 이 절이되, 다음 step 시작 시 한 조각으로 좁힐 것**. ⒜ 는 exactly-once 내구성(write-ahead + opId 멱등 + 재전송)이라 *다중 메커니즘·고난도* — 한 조각으로 좁히려면 sub-piece(저널 홉 ack/resend 만·또는 클라 resend 만)로 쪼갤 것. ⒝ 는 0018 의 검증된 압축 패턴을 채팅 커맨드 로그에 반복(저난도·E2E 안전). 큰 조각(비동기 결정론·C++ 시뮬 교체)도 여전히 강한 후보.

**검증할 것(후보 — 영속 신뢰성·압축·버스 영속 기준)**: ① **회귀 0** — 새 항 OFF 면 0021 비트 동일. ② **신성한 tick** — 새 항(ack/resend·압축·영속)이 존 tick 밖·존 비-침습(월드 비트 동일). ③ **E2E 동치** — 멀티프로세스=인프로세스 비트 동일·은닉. ④ **가설** — ⒜ 면 *활성 중 kill→복구 후 완전성 수렴*(in-flight 손실분 재전송/감지로 메워져 belief→원장 수렴·dupe 0), ⒝ 면 "라우팅 스냅샷+tail replay == 전체 커맨드 replay == 무재시작 비트 동일·커맨드 로그 절감", ⒞ 면 "버스 라우팅 kill→재구성 후 결합 0 보존".

> 후속 한 조각: **활성 중 가방 failover + write 손실 재수렴**(ack/resend·손실 감지·exactly-once — 0017 §9·고난도/분할 필요) · **채팅 커맨드 로그 스냅샷 압축**(0018 의 채팅 판·0021 §9 — 저난도) · **버스 영속·failover**(서비스 복구 패턴의 버스 판) · **활성 중 서비스 다운타임 + 재발행**(0020/0021 §9 — quiescent 한계 해소) · **월드 영속(존 intent 로그)**(계층 6 월드 판·크래시 복구·late-join) · **PersistStore failover·다중 복제·디스크 fsync**(영속 단일점 제거) · **채팅 신뢰 전달**(0015 §9-2) · **존 넘는 거래**(2PC — 0014 §9-2) · **비동기 실행 아래 결정론**(lockstep 해제·논리 클럭 — 0013 §9-1·큰 조각) · **C++ 시뮬 코어 headless 교체**(§3 🔴).

**병행 백로그(블로킹 아님)**: ⬜ 활성 중 가방 failover + write 손실 재수렴·ack/resend·exactly-once(0017 §9) · ⬜ 채팅 커맨드 로그 스냅샷 압축(0018 의 채팅 판·0021 §9) · ⬜ 버스 영속·failover(0016 §9-2·0017 §9) · ⬜ 활성 중 서비스 다운타임 + 재발행(0020/0021 §9 — quiescent 한계) · ⬜ 읽기 모델 증분 follow·자체 스냅샷(0020 §9) · ⬜ 증분 스냅샷·압축 트리거 다양화·스냅샷 손실 ack(0018 §9) · ⬜ 월드 영속(존 intent 로그·크래시 복구·late-join — 계층 6 월드 판) · ⬜ PersistStore failover·디스크 fsync·다중 복제(0017 §9 — 영속 단일점) · ⬜ 채팅 신뢰 전달·채널 시퀀스·ack/resend(0015 §9-2) · ⬜ 버스 분산·failover·동적 구독·이력/replay(0016 §9-2·§9-3) · ⬜ 런타임 consume→publish 사이클 정적 탐지(0019 §9 — 루프 방지 그래프 검증) · ⬜ 거래소·우편(서비스 분리 패턴 반복) · ⬜ 존 넘는 거래·2-party 동의·에스크로(0014 §9-2) · ⬜ 비동기 실행 아래 결정론(논리 클럭, 0012 §9-3·0013 §9-1) · ⬜ 물리적 다중-브로커 버스 분산·분산 epoch(0012 §9-1·0013 §9-3) · ⬜ 거짓 사망 플래핑·부분 복귀·stale 호스트 재합류(0013 §9-4) · ⬜ 다중 standby·자가 디스커버리·점진 상태 동기(0013 §9-2) · ⬜ 소켓 층 지연·재정렬·비대칭 분단(0012 §9-4) · ⬜ 다중 클라 결정론 복제·예측(C++ 승격에서 부활) · ⬜ 존 N개·동적 경계(0006 §8.2) · ⬜ 존↔존 ghost 증분화·복원(0007 §8.3·0008 §8.6) · ⬜ 서버간 인증(0001 §8.3) · ⬜ 재접속·티켓 만료(0001 §8.5) · ✅ 채팅 서비스 영속·failover(0021 — 커맨드 로그 replay·라우팅+deliveries event sourcing 복구·복구 투명·누설 0·서비스 복구 3부작 완성) · ✅ 읽기 모델 영속·late-join(0020 — 랭킹 crash→쓰기 저널 reconstruct·자기 영속 0·투영==원장·quiescent) · ✅ 발신하는 둘째 소비자—랭킹(0019 — consume→publish·CQRS 읽기 모델·발행자 무수정·루프 없음) · ✅ 가방 저널 스냅샷 압축(0018 — intent 로그+스냅샷·무손실·저널 유계화) · ✅ 가방 서비스 영속·failover(0017 — event sourcing 저널·복구 투명·계층 6 첫 진입) · ✅ 이벤트 버스 서비스 층(0016) · ✅ 채팅 서비스 분리(0015) · ✅ 가방 서비스 분리(0014) · ✅ 시각화·검증 testbed 단일화(0010 후 — [TESTBED.md](TESTBED.md)) · ✅ 시나리오 `inject`(0016 write-seam).

**빌드 인프라 — `engine/` 추출 완료(0004), 0005~0021 무수정 재사용**: 공통 하니스(시드 PRNG·FNV·전송 모델 `Net`·엣지/코디 스텁·동결 `ISimCore` 2구현 = `engine/index.js`, 시각 관찰 키트 = `engine/panel-kit.js`)를 `engine/` 한 곳에. 코어는 **dual-mode**(Node `require` + 브라우저 `<script>` 전역). 0021 은 `engine/` 무수정·존/orch/추종자/가방/버스/감사/영속·압축/랭킹 복구 안무 무수정 — net-core 는 0020 그대로 + **ChatService 리팩터**(onMsg→정규화+`_process` 단일 리듀서)·**crash()/replay()**(커맨드 로그 리듀서 재실행·replaying 가드로 재발신 0)·**`_journal`**(커맨드 로그)·chatpersist 박스(PersistStore 재사용)·run() **chatRestart 훅**만 더함. `cluster.js` 는 + chatRestart 블록(채팅 kill→chat_r spawn·init·replay), `host.js` 는 **'replay' cmd 재사용**(chat.replay 가 snapshot 무시). step-0022 의 net-core 출발 템플릿은 `step-0021/net-core.js`·`step-0021/cluster.js`·`step-0021/host.js`.
...
| 🟡 | **서비스 영속·failover (가방 ✅+압축 ✅·채팅 ✅·버스 ⬜)·존 넘는 거래** | 서비스/데이터 | 0017 가방 원장 = event sourcing 효과 저널(복구 투명)·0018 스냅샷 압축. 0021 채팅 = *커맨드 로그* event sourcing(라우팅+deliveries 리듀서 재실행 복구·복구 투명·누설 0 — 서비스 복구 3부작 완성). 단 *write-behind 윈도*(활성 중 죽음=in-flight 손실·ack/resend 없음 — 0017 잔존)·*채팅 커맨드 로그 비유계*(압축 없음·0021 §9)·버스 라우팅은 *영속 0*. 가방 일방 give(2PC 없음 — 0014 §9-2). |
