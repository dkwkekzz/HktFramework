...
- **닫힌 step**: [step-0041](step-0041.md) — ***결과* replay 버퍼 *자기-크기조정*(busOutAck)** — 0040 요청 버퍼 ack 가지치기의 거울(0040 §9). 게이트웨이가 중계한 `outSeq` 를 `svc.item.out.ack` 로 통보 → 가방이 `outAcked` 워터마크 이하 `outBuffer` 가지치기(미-ack 만). 정상 drain·gap 자동 성장 → 복구 `resendOut` 이 덮어 *K 추정 없이* 무손실. 결과 손실은 minted 아닌 **desync**(mint 됐으나 belief 뒤처짐). 닿는 파일 svc-inventory.js·gateway.js·topo-build.js·verify.js.
- **한 줄 상태**: reg 25/25(0040 비트 동일·busOutAck=0 휴면)·busoutack ALL OK — ack desync 0(unbnd 와 동일) / fixedK8(<gap) desync 4(대조) / ack outBuf final 0(idle drain)·peak 가동-길이 무관(unbnd 60→120→180 vs ack peak 24)·rankDesync 0·dupe 0·E2E 비트 동일·spine **41-step**·close-step 통과.
- **다음**: §2 참조 — seenReqs 유계화 · 다중 소비자 min-워터마크 · give×result-ahead · 비동기 결정론(🔴 최우선). 정리 후보: svc-inventory.js 30KB 근접.

---

## 2. NEXT — step-0041 후 가설 (후보, 권위는 이 절)

**step-0041 이 ⒝″-*결과 버퍼 자기-크기조정*(busOutAck — 게이트웨이 outSeq ack→가방 outBuffer 가지치기·K 추정 없이 desync 0·가동-길이 무관·idle drain)을 닫았다. 요청(0040)·결과(0041) 양 경로 모두 ack 자기-크기조정 완료. 남은 후보: ⒝‴ *seenReqs 유계화*(가방 dedup 집합 무계 — ack 워터마크 기반 유계화·0040/0041 §9), *다중 소비자 min-워터마크*(결과 ack 가 게이트웨이에만 키잉 — ranking 등 둘째 소비자가 뒤처지면 min-워터마크 필요·0041 §9), ⒝-*give 요청×result-ahead 결합*(give 재발행 transfers≠base·0025 give-resend 통합·0037 §9), ⒞ *활성 중 서비스 다운타임 일반 재발행*(0025/0026 quiescent → 온라인 kill+재발행 핸드셰이크), ⒟ *비동기 결정론(lockstep 배리어 해제)*(논리/벡터 클럭·🔴 최우선·broker 대공사·0012 §9-3), 🔧 정리 후보(svc-inventory.js 30KB 근접).**
...
| 🟡 | **버스 단일점·분산·영속(동적 구독·failover·결과/요청경로 무손실·replay 유계화 ✅)** | 버스 | 0016 ServiceBus = *단일 박스·영속 0*. 0033 동적 구독/해지 + 0034 failover(crash→재구독 routing 재구성·진실 원천=소비자) + 0036 결과경로/0037 요청경로 무손실(producer replay·belief Set 멱등·pickup dedup) + 0039 replay 유계화(busWindow 고정 K) + 0040/0041 요청·결과 버퍼 ack 자기-크기조정(소비자 ack 워터마크 가지치기·K 추정 없이 무손실·idle drain — 전문 §7). 남은 것: seenReqs 유계화·다중 소비자 min-워터마크(0040/0041 §9)·give×result-ahead·라우팅 영속/replay 0·다중 브로커 분산. |
...
> **✅ 해소된 격차 (0001~0037)** — 전문은 §7 INDEX(1줄/step)·각 `step-NNNN.md`. 묶음: 골격/복제/Sim 동결/현실 전송(0001~0004)·AOI·분할·핸드오프·증분·복원·failover(0005~0009)·프로세스 경계·TCP·버스 분산·진짜 kill failover(0010~0013)·게임 서비스 분리(0014~0015)·이벤트 버스 의미(0016)·가방/채팅 영속+압축(0017~0022)·발신 소비자·읽기 모델 영속(0019~0020)·write-behind 신뢰성 완결(0023~0026)·persist failover·N-replica·quorum write ack·윈도 해소(0027~0032)·버스 동적 구독+failover+결과/요청경로 무손실+replay 버퍼 유계화+요청/결과버퍼 ack 자기-크기조정(0033~0037·0039~0041).
...
| 4 | 버스 | 이벤트 버스 | 🟡 0004 전송 substrate→0012 토픽 pub/sub 버스(소켓 열화 내성)→0016 **서비스 의미**(ServiceBus·직접 결합 0·발행자 무수정 소비자 추가)→0019 발신 소비자(ranking)→0033 동적 구독/해지→0034 **failover**(crash→재구독 routing 재구성·진실 원천=소비자)→0036 결과경로 무손실+0037 요청경로 무손실(producer replay·mint 손실 18→0·dedup)→0039 replay 버퍼 유계화(busWindow 고정 K)→0040 요청 버퍼 자기-크기조정(busAck — reqId ack→워터마크 가지치기)→0041 **결과 버퍼 자기-크기조정**(busOutAck — 게이트웨이 outSeq ack→가방 outBuffer 가지치기·요청 경로의 거울·미-ack 만·K 추정 없이 desync 0·idle drain). 버스 단일점 제거(분산)·seenReqs 유계화·다중 소비자 min-워터마크·give×result-ahead·라우팅 영속·서버간 인증 후속 |
