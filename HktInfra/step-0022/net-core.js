// HktInfra step-0022 — 채팅 커맨드 로그 스냅샷 압축 (ChatService 커맨드 로그 + 주기 *라우팅 스냅샷* — 0018 가방 압축의 커맨드-소싱 판)
// step-0021(채팅 영속·failover — 커맨드 로그 replay) 위에 *한 조각*만 더한다 — 0021 이 남긴 §3 격차(채팅 커맨드 로그 *비유계*: 압축 없음)를
//   메운다. 0018 이 가방 *효과* 저널을 "intent 로그 + 주기 스냅샷"으로 유계화했듯, 이 step 은 채팅 *커맨드* 로그를 그 패턴으로 유계화한다.
//   핵심 차이(효과소싱 vs 커맨드소싱의 압축): 가방 스냅샷은 *원장 값*(itemId→owner)이지만, 채팅 스냅샷은 *파생 라우팅 상태*
//   (channels:channel→Set<avatar> · byAvatar 역인덱스 · deliveries · 계측)다 — 커맨드 로그의 *리듀스 결과*를 통째로 떠서 헤드 커맨드를 폐기.
//   이 step 의 한 조각 = 그 채팅 압축:
//     · chatSnapshot 플래그 OFF(0) 면 0021 *그대로* — 0021 과 *비트 동일*(reg 0). 스냅샷·압축 휴면(전체 커맨드 로그).
//     · chatSnapshot=N: 커맨드 N항목마다 ChatService 가 *라우팅 스냅샷*을 chatpersist 로 발신(write-behind·반응형·onTick 0). PersistStore 가
//        upToSeq 이하 커맨드를 폐기 → 커맨드 로그가 (스냅샷 1개 + 짧은 tail)로 유계. replay 는 스냅샷 베이스 복원 + tail 커맨드만 _process 재실행.
//     · 무손실: 라우팅 스냅샷+tail replay == 전체-커맨드 replay == 무재시작 *chatDigest 비트 동일*(압축은 *보관*만 줄이고 복구 결과는 불변).
//
// 핵심 설계 — chat._snapshot/replay 는 inventory._snapshot/replay(0018) 의 *커맨드-소싱 판*: 스냅샷이 *효과 베이스*가 아니라 *라우팅 베이스*다.
//   replay 가 스냅샷으로 channels/byAvatar/deliveries/계측을 직접 복원하고 tail 커맨드만 replaying 가드로 재실행(재발신 0). Set/Map 은 삽입 순서
//   보존 직렬화(say 의 구독자 순회 순서 = deliveries 순서 결정론 → 복원도 동순서). PersistStore 의 snapshot 핸들러는 *제네릭*(가방·채팅 공용·무변경).
//   월드 경로·제어 평면·가방 영속/압축(0017~0018)·버스·발행/구독(0019)·읽기 모델 복구(0020)·채팅 failover(0021)는 *그대로*(복사 전진). 더한 것은 chat._snapshot + replay 스냅샷 베이스 + leaves 누적 회계뿐.
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: 스냅샷·압축·replay 는 *존 tick 밖*(제어 평면·chat onTick 0). 존 net.log·상태가 chatSnapshot on/off 에 비트 동일·존 도달 0.
//   - 결정론 코어: 복구 라우팅+deliveries = (스냅샷 베이스 + tail 커맨드)의 함수 → 전체-커맨드 replay·무재시작과 정확히 일치. = §4 "복제=재현"(상태 전송 아님).
//   - 권위 단일 소유: 채팅 라우팅 테이블의 쓰기 권위는 ChatService 하나 — 압축은 *보관*을 줄일 뿐 권위 비-침습(라이브 chatDigest 불변). PersistStore 는 데이터 계층.
//   - 은닉·단일 연결: 스냅샷/replay 는 커맨드 로그를 *제어 평면*으로 읽을 뿐(클라 비가시). 복구 후 say 도 누설 0/격리 보존.
//   - headless·원격 검증: node verify.js 한 줄이 채팅을 *진짜 kill→스냅샷+tail replay* 로 띄워 무손실 압축·복구 투명·누설 0 을 4기둥 검증.
