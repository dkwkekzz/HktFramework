'use strict';
// step-0286 — #56 브리지 존 데이터 평면 6: zoneEntitiesDiscarded 계측(stop 폐기·계획적·hostdown 비자발과 구분).
...
// step-0332 — 브리지·데이터평면·host컨테이너·egress 필드 init 믹스인(생성자 0272~0331 대입 블록 분리·orchestrator.js >30KB 유계화·기능 0·reg 0).
const { OrchBridgeInit } = (typeof module !== 'undefined' && module.exports && typeof require !== 'undefined')
  ? require('./orch-bridge-init.js') : globalThis.__HktNetParts.orch_bridge_init;
...
class Orchestrator {
  constructor(opts = {}) {
    ...
    // 실 zone.js 브리지·데이터 평면·host 프로세스 컨테이너·egress 필드(0272~0331) 초기화는 step-0332 정리에서 orch-zonebridge.js `_initBridgeFields(opts)` 로 분리(orchestrator.js >30KB 트리거 유계화). 같은 순서의 동일 대입을 생성자 같은 지점에서 호출 = 동작 비트 불변(reg 0·orch-placement 0251·orch-control 0267·orch-hostproc 0305·orch-views 0323 투명 분할 계보).
    this._initBridgeFields(opts);
...
Object.assign(Orchestrator.prototype, OrchBridgeInit);   // step-0332 — 생성자가 호출하는 _initBridgeFields 제공(필드 init 분리·투명·reg 0).
