...
function buildSubs(c) {
  ...
  if (c.audit && c.guildChangePublish && c.guildService) subs.push(['svc.guild.changed', 'audit']);   // 길드 변경 발행(0183) — audit 가 svc.guild.changed 구독(가입/탈퇴 관측). guildChangePublish OFF 면 미추가(0182 토폴로지 비트 동일).
  if (c.guildFeed && c.guildService && c.guildChangePublish) subs.push(['svc.guild.changed', 'guildfeed']);   // 길드 멤버 수 배지(0186) — GuildFeed 가 svc.guild.changed 구독(memberCount 투영). guildFeed OFF·발행 부재면 미추가(0185 토폴로지 비트 동일).
  if (c.audit && c.guildBankPublish && c.guildBank && c.guildService) subs.push(['svc.guild.bank.changed', 'audit']);   // 길드 금고 변경 발행(0193) — audit 가 svc.guild.bank.changed 구독(예치/인출 관측). guildBankPublish OFF 면 미추가(0192 토폴로지 비트 동일).
  if (c.guildBankFeed && c.guildFeed && c.guildBank && c.guildBankPublish && c.guildService) subs.push(['svc.guild.bank.changed', 'guildfeed']);   // 길드 금고 아이템 수 배지(0196) — GuildFeed 가 svc.guild.bank.changed 구독(bankCount 투영). guildBankFeed OFF·발행 부재면 미추가(0195 토폴로지 비트 동일).
  ...
  if (c.audit && c.mail && c.mailFailPublish) subs.push(['svc.mail.saga_failed', 'audit']);   // 우편 saga 영구 실패 발행(0179) — audit 가 svc.mail.saga_failed 구독(saga liveness 종결 관측·거래소 0138 의 우편 판). mailFailPublish OFF 면 미추가(0178 토폴로지 비트 동일).
