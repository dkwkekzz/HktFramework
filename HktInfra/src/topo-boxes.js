...
function addServiceBoxes(ctx, add) {
  ...
  if (guildService) add({ addr: 'guild', kind: 'guild', opts: { bus: busAddr, changePublish: guildChangePublish, persist: guildPersist, snapInterval: guildPersist ? guildSnapshot : 0, bank: guildBank, bankPublish: guildBankPublish, inv: (guildBankInv && inventory) ? inventoryAddr : null, invMode: guildBankInv } });   // inv/invMode(0511·guildBankInv): 금고↔가방 escrow 실연동(예치=멤버→escrow·인출=escrow→멤버 give). OFF 면 추상 vault(0510 비트 동일).   // changePublish(0183): svc.guild.changed 발행. persist(0184): 변경 저널 영속·crash replay. snapInterval(0185): 저널 스냅샷 압축(0 면 0184 비트 동일).
