...
function buildTopology(opts) {
  ...
  if (mail) add({ addr: 'mail', kind: 'mail', opts: { bus: busAddr, sentPublish: mailSentPublish, readPublish: mailReadPublish, persist: mailPersist, snapInterval: mailPersist ? mailSnapshot : 0, ttl: mailTtl, expirePublish: mailExpirePublish, item: mailItem, inv: (mailInv && inventory) ? inventoryAddr : null, invMode: mailInv, saga: mailSaga } });   // sentPublish(0144): svc.mail.sent. readPublish(0147): svc.mail.read. expirePublish(0149): svc.mail.expired. persist(0145): replay. snapInterval(0146): 압축. ttl(0148): 만료. inv/invMode(0161·mailInv): 아이템 우편↔가방 escrow custody. OFF 면 직전 비트 동일.
