// HktInfra step-0014 — broker(버스 허브). 멀티프로세스 lockstep 배리어를 *토픽 pub/sub 버스* 위에서 구동한다(Node 전용).
//   0013 대비 *가방 서비스(inventory) 재구성만* 더한다(버스·배리어·kill 생애주기·재-provisioning 은 0013 그대로):
...
function reconstruct(net_, topo, snaps, placement, clusterInfo, opts) {
  ...
  const clientProxy = (s) => {
    ...
    return {
      addr: s.addr, avatar: s.avatar, views: s.views, seen, items,
  ...
  return {
    net: net_, zones: zoneObjs, followers, reprovZones, allZones, zoneAddrs: topo.zoneAddrs, clients: clis, inventory,
