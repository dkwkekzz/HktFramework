// HktInfra step-0017 — broker(버스 허브). 멀티프로세스 lockstep 배리어를 *토픽 pub/sub 버스* 위에서 구동한다(Node 전용).
//   0016 대비 *영속 스토어(persist) 재구성 + 가방 failover(invRestart) 만* 더한다(이하 전부 0016 그대로):
...
async function runMulti(opts, deps) {
  ...
  const persistPlaced = placement.has('persist');  // 영속 스토어 존재(replay 소스 — 안 죽는다)
  ...
  for (let T = 0; T < ticks; T++) {
    ...
    if (invRestart && net_.tick === invRestart.at && !invRestartDone) {
      ...
      let journal = [];
      if (persistPlaced) { const snap = await cluster.snapshotOne(placeOf('persist'), 'persist'); journal = (snap && snap.journal) || []; }
      ...
      await cluster.rpc(newHost, { cmd: 'replay', addr: 'inventory', journal });   // 저널 replay → 죽기 전 원장 재현
...
function reconstruct(net_, topo, snaps, placement, clusterInfo, opts) {
  ...
  return {
    net: net_, zones: zoneObjs, followers, reprovZones, allZones, zoneAddrs: topo.zoneAddrs, clients: clis, inventory, chat, bus, audit, persist,
