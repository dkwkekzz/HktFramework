// HktInfra step-0017 — 가방 서비스 failover·영속 (계층 6 데이터 첫 진입 · 인벤토리 원장을 영속 저널에서 재구성 · "세계가 세션보다 오래 산다")
// step-0016(이벤트 버스 서비스 층 — 발행/구독 의미·발행자 무수정 소비자) 위에 *한 조각*만 더한다 — SPINE 계층 6(데이터)의 *첫 박스*:
//   0014~0016 의 게임 서비스(가방·채팅·버스·감사)는 전부 *영속 0*이었다 — 프로세스가 죽으면 원장/구독/라우팅이 RAM 과 함께
//   소멸했다(세계가 세션과 함께 죽는다). 이 step 은 가방 원장에 *영속 진실*을 깐다 — PersistStore(append-only 저널, onTick 없음)를
//   데이터 계층 박스로 세우고:
...
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: 저널 쓰기·replay 는 *존 tick 밖*(persist onTick 0). 존 net.log·상태가 persist on/off·restart 유무에 비트 동일·존 도달 persist 0.
...
class PersistStore {
  constructor() {
    this.journal = [];            // append-only 효과 로그(SSOT — replay 입력). seq 순서 보존.
    this.writes = 0;              // append 수신 수(가방 수락 변이 = mint+xfer 와 대조)
  }
  onMsg(m) {
    const p = m.payload;
    if (p.type !== 'journal') return;
    this.journal.push(p.entry);
    this.writes++;
  }
...
function buildTopology(opts) {
  ...
  if (inventory) add({ addr: 'inventory', kind: 'inventory', opts: { gateway: 'gateway', bus: busAddr, persist: persistAddr } });
...
function run(opts) {
  ...
  for (let i = 0; i < ticks; i++) {
    ...
    if (opts.invRestart && inventory && i + 1 === opts.invRestart.at) {
      inventory.crash();
      if (persist) inventory.replay(persist.journal);
    }
...
function persistDigest(r) {
  ...
  return fnv1a(j + '#w' + (r.persist.writes || 0));
}
// 저널 완전성 — 영속 저널 항목 수 == 가방이 수락한 변이 수(mint+transfer). quiescent restart 면 저널이 원장 효과 전부를 담는다.
function journalComplete(r) {
  if (!r.persist || !r.inventory) return true;
  return (r.persist.journal || []).length === (r.inventory.minted + r.inventory.transfers);
}
