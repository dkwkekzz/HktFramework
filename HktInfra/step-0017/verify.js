// HktInfra step-0017 — 헤드리스 검증 (가방 서비스 failover·영속 — 인벤토리 원장을 영속 저널에서 재구성)
// 사용: node step-0017/verify.js <mode> [seed]
//   mode: reg | e2e | sacred | recover | degrade | inject | isolate | hide | repro | all
//     reg        — 회귀 0: 인프로세스 모드(persist OFF) → step-0016 과 *비트 동일*(net.log + 상태 + inv/chat/bus/audit).
//                  zones 1·2 · recovery off/on · failover off/on · svc(가방+채팅)+버스+감사. persist 박스·저널/replay seam 도입이 *비-침습*.
...
const NET16 = require('../step-0016/net-core.js');   // reg 대조용(직전 step)
...
function svcMsgsToZones(r) {
  return r.net.log.filter(m => /^zone/.test(m.to) && m.payload && /^(chat|item|pub|sub|ev|journal)/.test(m.payload.type || '')).length;
}
...
const PERSIST = (seed) => ({ ...BUSA(seed), persist: true });        // + 영속(저널, restart 없음)
const RESTART = (seed) => ({ ...PERSIST(seed), invRestart: { at: RESTART_AT } });   // + 가방 진짜 kill→replay
...
// ── reg: 인프로세스 0017(persist OFF) → 0016 비트 동일(persist 박스·저널/replay seam 도입 비-침습) ──
function reg(seeds) {
  console.log('== reg: 인프로세스 모드(persist OFF) → step-0016 과 비트 동일(net.log + 상태 + inv/chat/bus/audit). persist 박스·저널/replay seam 도입 = 비-침습 ==');
  console.log('seed   | 구성                | 0016 logHash | 0017(inproc) | log동일 | 상태동일 | inv/chat/bus 동일 | 판정');
  for (const seed of seeds) {
    ...
    for (const [name, c] of cfgs) {
      ...
      const r16 = NET16.run(p);
      const r17 = run({ ...p });
      const okL = logDigest(r16) === logDigest(r17), okS = worldDigest(r16) === worldDigest(r17);
      const okX = invDigest(r16) === invDigest(r17) && chatDigest(r16) === chatDigest(r17) && busDigest(r16) === busDigest(r17);
      check(okL, `seed ${seed} ${name.trim()}: net.log 다름`);
      check(okS, `seed ${seed} ${name.trim()}: 상태 다름`);
      check(okX, `seed ${seed} ${name.trim()}: inv/chat/bus 다름`);
      console.log(`${pad(seed, 6)} | ${name} | ${hex(logDigest(r16))}   | ${hex(logDigest(r17))}   | ${(okL ? '예' : '아니오').padEnd(6)} | ${(okS ? '예' : '아니오').padEnd(8)} | ${(okX ? '예' : '아니오').padEnd(16)} | ${okL && okS && okX ? 'OK' : 'FAIL'}`);
    }
  }
  console.log('  → persist OFF 면 토폴로지에 persist 박스가 *없고*, 가방의 저널/crash/replay 분기는 휴면 — 0016 과 비트 동일.');
}
...
async function isolate(seeds) {
  ...
  const ok =
    ...
    check(b.persist && b.persist.journal.length > 0, `persist 저널 비어있음`) &&
    check(invDigest(b) === invDigest(run(RESTART(seed))), `멀티 복구 원장 != 인프로세스 복구 원장`) &&
    check(C.allSerializable, `경계 넘는 메시지에 비직렬화 데이터 존재`);
  console.log(`  broker pid ${C.parentPid}(TCP :${C.port}) · 호스트 ${C.hostIds.length}개 · persist pid ${persistPid}(killed=${C.killed.includes('persist')}) · 저널 ${b.persist.journal.length}항목`);
  console.log(`  가방 failover: ${ir ? ir.oldHost : '?'}(pid ${oldInvPid}·killed) → ${ir ? ir.newHost : '?'}(pid ${newInvPid}) @tick ${ir ? ir.at : '?'} · replay ${ir ? ir.entries : '?'}항목`);
...
async function summary(seeds) {
  console.log('== summary: 가방 서비스 failover·영속 — 원장을 영속 저널에서 재구성·복구 투명·E2E 비트 동일 ==');
  for (const seed of seeds) {
    ...
    const ok = logDigest(a) === logDigest(b) && worldDigest(a) === worldDigest(b) && invDigest(a) === invDigest(b) && persistDigest(a) === persistDigest(b)
      && invDigest(a) === invDigest(run(PERSIST(seed))) && itemDesync(b) === 0 && itemConserved(b) && ledgerConsistent(b) && svcMsgsToZones(b) === 0;
    if (!ok) FAILED = true;
    const ir = C.invRestarted;
    console.log(`  seed ${pad(seed, 4)}: 프로세스 ${C.pids.length}개 · 가방 failover ${ir ? ir.oldHost + '→' + ir.newHost + '@' + ir.at : '?'}(replay ${ir ? ir.entries : '?'}) · 저널 ${b.persist.journal.length} · 복구투명 ${invDigest(a) === invDigest(run(PERSIST(seed)))} · log동일 ${logDigest(a) === logDigest(b)} | ${hex(invDigest(b))}`);
  }
  console.log('가방 원장이 영속 진실(저널)에서 재현된다 — 가방 죽어도 데이터 계층은 산다 · persist OFF 면 0016 비트 동일');
}

// ── CLI ──
const MODES = { reg, e2e, sacred, recover, degrade, inject, isolate, hide, repro };
(async () => {
  ...
  } else { console.log('mode: reg | e2e | sacred | recover | degrade | inject | isolate | hide | repro | all'); process.exit(2); }
