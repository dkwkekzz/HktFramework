// HktInfra step-0017 — 호스트(자식 프로세스) 진입점. child_process.spawn 으로 띄워지는 *독립 OS 프로세스*.
...
function snapshotActor(addr, a) {
  ...
  if (spec.kind === 'persist') {
    return { kind: 'persist', addr, journal: a.journal, writes: a.writes };   // 영속 저널 — 가방 죽음과 독립(데이터 계층)
  }
...
function handle(msg) {
  ...
  if (cmd === 'init') {
    ...
    const a = actors.get(msg.addr);
    if (a && a.replay) a.replay(msg.journal || []);
