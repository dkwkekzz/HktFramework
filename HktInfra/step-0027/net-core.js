// HktInfra step-0027 — PersistStore failover: 이중쓰기 보조 persist(persist2)로 primary 단일점 제거
// step-0026(in-flight mint id-reconciliation) 위에 *한 조각*만 더한다 — STATE §2 ⒝: "PersistStore failover·다중 복제(영속 단일점 제거)".
//   0023~0026 의 write-behind 신뢰성 체인은 가방·채팅 서비스의 복구를 완결했다. 남은 단일점: PersistStore 자체가 죽으면?
//   · PersistStore crash → journal 전량 소실 → invRestart 시 replay 불가 → 원장 소실(대조군: persistBackup OFF).
//   · persistBackup ON: InventoryService 가 primary('persist')와 backup('persist2')에 *동시* journal 기록(이중쓰기).
//     primary crash 후에도 persist2 가 전체 저널 보유 → invRestart 가 persist2 에서 replay → invDigest == 무손실 기준.
//   · persistBackup OFF 면 0026 *그대로* — persist2 스폰 0·_journal 추가 발신 0·비트 동일(reg 0).
//
// 핵심 설계 — *이중쓰기 = 단일점 제거의 가장 단순한 패턴*:
//   PersistStore 는 *데이터 계층*(세션보다 오래) 이지만 현재 단일 인스턴스.
//   이 step 에서 backup persist 에 fire-and-forget 이중쓰기로 full journal 복제를 확보한다.
//   primary 가 죽어도 backup 에서 완전 복구 가능 → PersistStore 단일점 해소.
//   가방/채팅/클라/게이트웨이/버스/랭킹/0023~0026 전 코드 *그대로*(복사 전진). 더한 것: InventoryService._journal 이중쓰기 + buildTopology persist2 스폰 + run 루프 persistRestart 핸들러뿐.
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: journal 이중쓰기는 *비동기 서비스 메시지*(존 tick 밖). 존 net.log·상태가 persistBackup on/off 에 비트 동일·존 도달 0.
//   - 결정론 코어: 복구 원장 = persist2 저널 replay → 무손실 truth 와 비트 동일(invDigest). = §4 "복제=재현".
//   - 권위 단일 소유: primary/backup 둘 다 *수신만*(읽기전용 data store). 원장 권위는 InventoryService 단일. 쌍 발신은 복제·쓰기 권위 이동이 아님.
//   - 은닉·단일 연결: persist2 는 서버간 내부 주소. 클라는 gateway 만 알고, persist2 존재를 모름(은닉 유지).
//   - headless·원격 검증: node verify.js persist-failover 한 줄이 primary crash 주입 → backup ON=invDigest 동일 / OFF=소실 을 4기둥 검증.
'use strict';
...
class InventoryService {
  constructor(opts = {}) {
    ...
    this.persistBackup = opts.persistBackup || null;  // 보조 영속 스토어 주소(이 step·persistBackup) — _journal 이중쓰기 대상. null = 0026 비트 동일(단일 persist).
  ...
  _journal(entry) {
    ...
    // 이중쓰기(이 step) — persistBackup ON 이면 backup persist 에도 동일 항목 발신(fire-and-forget). primary crash 후 backup 에서 완전 복구 가능.
    if (this.persistBackup) this.net.send(this.addr, this.persistBackup, { type: 'journal', entry: full });
...
function buildTopology(opts) {
  ...
  const persistBackupAddr = (persistBackup && persistAddr) ? 'persist2' : null;   // 보조 영속(이 step) = primary persist 전제. OFF → 0026 비트 동일(이중쓰기 0).
  ...
  if (inventory) add({ addr: 'inventory', kind: 'inventory', opts: { gateway: 'gateway', bus: busAddr, persist: persistAddr, persistBackup: persistBackupAddr, snapshot: persistAddr ? snapshot : 0, reliable: persistAddr ? journalReliable : false, journalHb: persistAddr ? journalHeartbeat : false } });
...
function run(opts) {
  ...
  for (let i = 0; i < ticks; i++) {
    ...
    if (opts.invRestart && inventory && i + 1 === opts.invRestart.at) {
      inventory.crash();
      // persistBackup ON 이고 primary 가 crash 된 경우 persist2 에서 복구 — 단일점 제거 가설 검증 핵심.
  ...
  return { net, login, registry, gateway, orch, inventory, chat, bus, audit, ranking, persist, persist2, chatpersist, zones: zoneObjs, followers, allZones, zoneAddrs: topo.zoneAddrs, clients: clis, trace, seenTrace, deltaTrace, replicaTrace, totals, H: topo.H, grid: topo.grid, radius: topo.radius, deathTick: opts.deathTick != null ? opts.deathTick : null, killZone: opts.killZone || 'zone1', mode: 'inproc' };
}
