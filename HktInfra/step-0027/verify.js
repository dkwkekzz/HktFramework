// HktInfra step-0027 — 헤드리스 검증 (PersistStore failover — 이중쓰기 보조 persist 로 primary 단일점 제거·복구 무손실)
// 사용: node step-0027/verify.js <mode> [seed]
//   mode: reg | persist-failover | inflight | tail | reliable | chat-compact | recover-chat | rank | recover-rank | e2e | sacred | recover | compact | degrade | inject | isolate | hide | repro | all
//     reg          — 회귀 0: 인프로세스 모드(persistBackup OFF) → step-0026 와 *비트 동일*(net.log + 상태 + inv/chat/bus/rank).
//                  이중쓰기(persist2 발신)·persistRestart 핸들러는 persistBackup OFF 면 *휴면*(persist2 스폰 0·추가 발신 0)임을 직접 증명.
//     persist-failover — *이 step 의 가설*: ① persistBackup OFF: primary persist crash → journal 전량 소실 → invRestart replay 불가 → 원장 소실(invDigest != 무손실 기준)
//                  ② persistBackup ON: InventoryService 가 primary+backup 이중쓰기 → primary crash 후에도 persist2 전체 저널 보유 → invRestart persist2 에서 복구 → invDigest == 무손실 기준
//                  ③ persist2 writes == primary writes(완전 복제) ④ 이중쓰기 자체가 이미 동작하는 경로의 부산물(신성한 tick 밖·비-침습)
...
const NET16 = require('../step-0026/net-core.js');   // reg 대조용(step-0026)
...
// ── reg: 인프로세스 모드(persistBackup OFF) → step-0026 와 비트 동일(이중쓰기 도입 비-침습) ──
//   persistBackup 미제공 = OFF → persist2 스폰 0·이중쓰기 발신 0·persistRestart 핸들러 0 → 0026 와 비트 동일.
function reg(seeds) {
  console.log('== reg: 인프로세스 모드(persistBackup OFF) → step-0026 와 비트 동일(net.log + 상태 + inv/chat/bus/rank). 이중쓰기 = 비-침습 ==');
  console.log('seed   | 구성                | 0026 logHash | 0027(inproc) | log동일 | 상태동일 | inv/chat/bus/rank 동일 | 판정');
  ...
  console.log('  → persistBackup 미제공이면 persist2 스폰 0·이중쓰기 발신 0·persistRestart 핸들러 0 — 0026 와 비트 동일.');
}
...
// ── persist-failover: 이 step 의 가설 — primary persist crash 아래 이중쓰기 backup 이 복구원이 된다 ──
function persistFailover(seeds) {
  console.log('== persist-failover: *가설* — ① persistBackup OFF: primary crash → journal 소실 → 원장 소실(invDigest != base) ② persistBackup ON: persist2 이중쓰기 전체 저널 보유 → primary crash 후에도 persist2 에서 복구 → invDigest == base ③ persist2 writes == primary writes(완전 복제) ==');
  console.log('seed   | 무손실 base | OFF 복구   | ON 복구    | OFF손실 | ON==base | persist2writes | 복제완전 | desync(ON) | 보존/정합(ON) | 판정');
  for (const seed of seeds) {
    const base = run(RESTART_NOSNAP(seed));             // 무손실 기준(primary crash 없음 — 전체 저널 replay)
    const off = run(PF_OFF(seed));                      // 이중쓰기 OFF — primary crash → journal 소실 → 복구 원장 손실
    const on = run(PF_ON(seed));                        // 이중쓰기 ON — primary crash → persist2 에서 복구
    const baseDigest = invDigest(base);
    const offLossy = invDigest(off) !== baseDigest;     // ① primary crash + 이중쓰기 OFF → 원장 손실 확인
    const onLossless = invDigest(on) === baseDigest;    // ② 이중쓰기 ON → persist2 에서 완전 복구
    // ③ persist2 writes == primary writes(before crash): 이중쓰기 완전성
    const primaryWrites = base.persist ? (base.persist.writes || 0) : 0;
    const p2Writes = on.persist2 ? (on.persist2.writes || 0) : 0;
    const replicated = on.persist2 != null && p2Writes === primaryWrites;
    const desync0 = itemDesync(on) === 0;
    const cons = itemConserved(on) && ledgerConsistent(on);
    const ok =
      check(offLossy, `seed ${seed}: 이중쓰기 OFF + primary crash 인데 원장 소실 안 됨(대조 실패: OFF invDigest == base)`) &&
      check(onLossless, `seed ${seed}: 이중쓰기 ON 복구 원장 != 무손실 기준(persist2 복구 실패)`) &&
      check(replicated, `seed ${seed}: persist2 writes(${p2Writes}) != primary writes(${primaryWrites}) — 이중쓰기 불완전`) &&
      check(desync0, `seed ${seed}: persist2 복구 후 itemDesync ${itemDesync(on)}`) &&
      check(cons, `seed ${seed}: persist2 복구 후 원장 보존/정합 깨짐`);
    console.log(`${pad(seed, 6)} | ${hex(baseDigest)} | ${hex(invDigest(off))} | ${hex(invDigest(on))} | ${(offLossy ? '예' : '아니오').padEnd(7)} | ${(onLossless ? '예' : '아니오').padEnd(8)} | ${pad(p2Writes + '/' + primaryWrites, 14)} | ${(replicated ? '예' : '아니오').padEnd(8)} | ${pad(itemDesync(on), 10)} | ${(cons ? '예' : '아니오').padEnd(13)} | ${ok ? 'OK' : 'FAIL'}`);
  }
  console.log('  → PersistStore 단일점 제거: InventoryService 가 primary+backup 에 동시 저널 기록 → primary 가 죽어도 persist2 가 전체 저널 보유 → invRestart 가 persist2 에서 완전 복구.');
  console.log('    이중쓰기는 _journal() 내부 한 줄 — 기존 write-behind 경로의 부산물(신성한 tick 밖·비-침습). persistBackup OFF 면 reg 0(0026 비트 동일).');
}
...
async function summary(seeds) {
  console.log('== summary: PersistStore failover(이중쓰기 backup) — write-behind 신뢰성 체인(0023~0026) 무손실 + persist 단일점 제거(persist2 완전 복구) + 멀티프로세스 E2E 비트 동일(휴면 경로) ==');
  for (const seed of seeds) {
  ...
  console.log('write-behind 신뢰성 체인(0023 중간·0024 tail·0025 give·0026 mint) 완결 + PersistStore 단일점 제거(0027 이중쓰기 backup) — primary crash 후 persist2 완전 복구. 남은 §9 = N-replica/Quorum 쓰기·버스 영속·월드 영속 후속.');
}

// ── CLI ──
const MODES = { reg, 'persist-failover': persistFailover, rank, e2e, sacred, recover, 'recover-rank': recoverRank, 'recover-chat': recoverChat, compact, 'chat-compact': chatCompact, reliable, tail, inflight, degrade, inject, isolate, hide, repro };
(async () => {
  ...
  if (MODES[mode]) await MODES[mode](seedArg);
    ...
    persistFailover(seedArg); console.log('');
  ...
  } else { console.log('mode: reg | persist-failover | rank | e2e | sacred | recover | recover-rank | recover-chat | compact | chat-compact | reliable | tail | inflight | degrade | inject | isolate | hide | repro | all'); process.exit(2); }
