// HktInfra step-0032 — 시각 관찰 셸 (프로세스 토폴로지 + 가방 영속 — 데이터 계층 persist 박스) + 헤드리스 ASCII(Node)
...
(function () {
  ...
  function printAscii(seed) {
    ...
    console.log(`\nstep-0032 관찰(가방 영속 — 데이터 계층 persist 박스) — seed ${seed} · 사망 tick ${r.deathTick} · lease 타임아웃 ${r.leaseTimeout}`);
