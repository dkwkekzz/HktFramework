// HktInfra step-0024 — 저널 홉 tail 손실 감지 (가방 onTick heartbeat 로 maxSentSeq 통보 → persist 가 NAK-only 사각인 tail 갭을 감지 — 0023 §9 해소)
// step-0023(저널 홉 신뢰 전달) 위에 *한 조각*만 더한다 — STATE §2 ⒜(write-behind 신뢰성의 나머지)의 *sanctioned sub-piece*: "tail 손실 감지(heartbeat)".
//   0023 의 NAK 은 *받은 최고 seq(maxRecvSeq)까지의 갭*만 본다 — [0..maxRecvSeq] 의 빈칸은 잡지만, *최고 수신 위*(tail)에서 사라진 항목은
//   maxRecvSeq 가 그만큼 안 올라가 *구조적으로 미감지*다(0023 §9 사각). 활동 정지 후 마지막 저널들이 손실되면 영영 갭 → 복구 손실. 이 step 은 그 tail 을 닫는다:
//     · journalHeartbeat 플래그 OFF 면 0023 *그대로* — heartbeat 휴면·비트 동일(reg 0). onTick no-op·journal_hb/expectedMaxSeq 미사용.
//     · ON: 가방이 주기적(hbPeriod)으로 *내가 보낸 최대 seq*(maxSentSeq = journalSeq-1)를 persist 에 heartbeat(journal_hb)로 통보 →
//        persist 가 갭 스캔 상한을 max(maxRecvSeq, maxSentSeq) 로 올려 *tail 갭도 NAK* → 가방이 sentBuffer 에서 재전송(0023 기계 그대로) → 저널 완전.
//     · 무손실: tail 손실 아래에서도 journalHeartbeat 면 persist 저널 *완전*(writes==무손실 기준) → 복구 원장 == 무손실 기준(invDigest 동일).
//        대조군(OFF=NAK-only): 같은 tail 손실이 *영구 갭*(tailNAK 0·구조적 미감지) → 복구 원장 *손실*.
//
// 핵심 설계 — *끝은 발신자만 안다*: 수신자는 *받은 것의 빈칸*만 보지만 tail(끝)은 뒤에 올 게 없어 빈칸으로 안 보인다 → 발신자가 high-water mark(maxSentSeq)를
//   통보해야 감지가 가능. **tail 은 *배달* 문제가 아니라 *감지* 문제** — 알면 0023 재전송이 그대로 메운다(그래서 더한 건 *통보* 한 조각). heartbeat 는 *가방 자체 제어 평면 onTick*(존 시뮬 tick 아님 = 신성한 tick 보존).
//   *tail 손실은 전송층*(JTAIL — tick≥TAIL_FROM 의 *최초 전송*만 드롭·재전송/NAK/heartbeat 제외 → 갭을 *전송*이 아니라 *감지* 문제로 격리). 압축(0018)과 직교(snapshot OFF·§9).
//   월드 경로·제어 평면·채팅 영속/압축(0021~0022)·버스·랭킹(0019~0020)·0023 신뢰 전달은 *그대로*(복사 전진). 더한 것은 inventory.onTick/journalHb + persist.expectedMaxSeq/_scanAndNak/journal_hb 뿐·host.js/`engine/` 무수정.
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: heartbeat·tail NAK·재전송은 *존 tick 밖*(가방 자체 제어 평면 onTick·persist 반응형). 존 net.log·상태가 journalHeartbeat on/off 에 비트 동일·존 도달 0. 가방 onTick = *서비스* 주기(존 시뮬 tick 아님).
//   - 결정론 코어: 복구 원장 = persist 저널(seq 순서·dedup)의 함수 → tail 손실 아래에서도 무손실 기준과 정확히 일치. heartbeat 주기는 결정론 상수(seed 무관). = §4 "복제=재현".
//   - 권위 단일 소유: 원장의 쓰기 권위는 InventoryService 하나 — heartbeat 는 *영속 완전성*만 보장·라이브 원장 비-침습(invDigest 불변).
//   - 은닉·단일 연결: heartbeat/NAK/재전송은 inventory↔persist *서버간 제어 평면*(클라 비가시). 신뢰 안무 누설 0.
//   - headless·원격 검증: node verify.js tail 한 줄이 저널 홉에 *tail 손실을 주입*해 hb ON=저널 완전·복구 무손실 / OFF=tail 갭·복구 손실을 4기둥 검증.
//   (가방·랭킹·버스·감사·영속/압축·존/orch/추종자·배리어·kill 생애주기·0023 신뢰 전달은 *그대로 잇는다* — 복사 전진. 더한 로직은 inventory.onTick + persist heartbeat 처리 뿐.)
'use strict';
