// HktInfra step-0024 — 헤드리스 검증 (저널 홉 tail 손실 감지 — heartbeat 로 maxSentSeq 통보 → NAK-only(0023) 사각인 tail 갭 감지·0023 §9 해소)
// 사용: node step-0024/verify.js <mode> [seed]
//   mode: reg | tail | reliable | chat-compact | recover-chat | rank | recover-rank | e2e | sacred | recover | compact | degrade | inject | isolate | hide | repro | all
//     reg          — 회귀 0: 인프로세스 모드(journalHeartbeat OFF) → step-0023 과 *비트 동일*(net.log + 상태 + inv/chat/bus/rank).
//                  마지막 cfg 는 journalReliable ON 인데 *손실 없음*·heartbeat OFF — 새 heartbeat 코드(onTick·journal_hb·expectedMaxSeq)가 *휴면*(메시지 0)임을 직접 증명.
...
const NET16 = require('../step-0023/net-core.js');   // reg 대조용(직전 step)
...
async function summary(seeds) {
  ...
  console.log('저널 홉이 전송-신뢰화(0023 중간 갭 NAK + 0024 tail heartbeat) — 손실 아래 영속 완전·복구 무손실. write-behind 신뢰성의 전송-손실 닫힘(중간+tail). 남은 §9 = in-flight 손실(활성 중 죽음).');
}

// ── CLI ──
const MODES = { reg, rank, e2e, sacred, recover, 'recover-rank': recoverRank, 'recover-chat': recoverChat, compact, 'chat-compact': chatCompact, reliable, tail, degrade, inject, isolate, hide, repro };
(async () => {
  ...
  } else { console.log('mode: reg | rank | e2e | sacred | recover | recover-rank | recover-chat | compact | chat-compact | reliable | tail | degrade | inject | isolate | hide | repro | all'); process.exit(2); }
