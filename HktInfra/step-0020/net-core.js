// HktInfra step-0020 — 읽기 모델 영속·late-join (RankingService crash→쓰기 저널 reconstruct — 자기 영속 0 인 CQRS 읽기 모델의 복구)
// step-0019(발신하는 둘째 소비자·RankingService) 위에 *한 조각*만 더한다 — 0019 가 남긴 한계(0019 §9: ranking 의 rank 투영은 RAM·영속 0
//   → 크래시 시 소실·late-join 미착수)를 닫는다. 핵심 통찰: **읽기 모델은 권위를 *복제*하지 않고 *재계산*한다**(0019 §9 발견) — 그러므로
//   읽기 모델은 *자기 영속이 필요 없다*. 크래시하면 *쓰기 모델의 영속 저널*(PersistStore — 0017 event sourcing·0018 스냅샷 압축)을 replay 해
//   투영을 재계산(reconstruct)하면 된다. 이 step 의 한 조각 = 그 late-join 복구:
//     · rankRestart 플래그 OFF 면 0019 *그대로* — 0019 와 *비트 동일*(reg 0). crash/reconstruct 휴면.
//     · rankRestart ON(persist 전제): ranking 을 *진짜 kill*(RAM 투영 소실) 후 PersistStore 저널을 reconstruct — mint→owner +1·
//        xfer→from -1/to +1 (= item_result 투영과 정확히 같음·저널은 가방이 *수락한* 효과만이라 ev.ok 게이트와 1:1). 스냅샷 베이스 + tail.
//        복구된 ranks 투영이 원장 byOwner 와 *정확히 일치*(복구 정합) → 읽기 모델은 자기 영속 없이 쓰기 저널로 완전 복원.
//     · 대조군: persist OFF + kill → reconstruct 불가 = 투영 소실(읽기 모델도 *어딘가의* 내구 로그가 없으면 못 산다 = 영속이 복원의 *원인*).
//
// 핵심 설계 — reconstruct 는 inventory.replay(0017) 의 *읽기 모델 판*: 같은 PersistStore 저널을 *다른 투영*으로 접는다(원장 vs 보유 수).
//   휘발 svc.item.out 스트림이 아니라 *내구 저널*이 복구원이므로 다운타임에 놓친 이벤트까지 복원된다(CQRS 읽기 모델 late-join 의 본질).
//   crash+reconstruct 는 invRestart 와 *같은 위치*(deliver 직전·제어 평면·net.log 비-기여) → 멀티프로세스 kill→reconstruct 와 비트 동일.
//   월드 경로·제어 평면·가방 영속(0017)·압축(0018)·버스·채팅(0015)·발행/구독(0019)은 *그대로*(복사 전진). 더한 것은 ranking 의 crash/reconstruct + rankRestart 훅뿐.
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: reconstruct 는 *존 tick 밖*(제어 평면·ranking onTick 0). 존 net.log·상태가 rankRestart on/off 에 비트 동일·존 도달 0.
//   - 결정론 코어: 복구 투영 = 저널(seq 순서)의 함수 → byOwner 와 정확히 일치. 저널 replay = §4 "복제=재현"(상태 전송 아님).
//   - 권위 단일 소유: rank 는 *권위 아닌 파생 투영*(원장 권위는 가방·CQRS) — 복구가 권위를 *복제*하지 않고 *재계산*(권위 충돌 여지 0).
//   - 은닉·단일 연결: reconstruct 는 쓰기 모델 저널을 *제어 평면*으로 읽을 뿐(클라 비가시). 복구 choreography 누설 0.
//   - headless·원격 검증: node verify.js 한 줄이 ranking 을 *진짜 kill→새 호스트 reconstruct* 로 띄워 복구 정합을 4기둥 검증.
//   (가방·채팅·버스·감사·영속/압축·존/orch/추종자·배리어·kill 생애주기는 *그대로 잇는다* — 복사 전진. 더한 박스 로직은 RankingService.reconstruct 뿐.)
'use strict';
