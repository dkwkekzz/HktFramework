// HktInfra step-0020 — 헤드리스 검증 (읽기 모델 영속·late-join — RankingService crash→쓰기 저널 reconstruct·자기 영속 0 인 CQRS 복구)
// 사용: node step-0020/verify.js <mode> [seed]
//   mode: reg | rank | e2e | sacred | recover | recover-rank | compact | degrade | inject | isolate | hide | repro | all
//     reg          — 회귀 0: 인프로세스 모드(rankRestart OFF) → step-0019 와 *비트 동일*(net.log + 상태 + inv/chat/bus/audit/rank).
//                  persist+invRestart 포함. ranking crash/reconstruct 항 도입이 *비-침습*(rankRestart 미제공=no-op).
//     recover-rank — *이 step 의 가설*: 랭킹(읽기 모델) 진짜 kill→*쓰기 모델 저널* reconstruct 후 ① 복구 투영 == 원장 byOwner(복구 정합)
//                  ② persist OFF+kill 대조군은 투영 *소실*(ranks.size 급감) ③ 복구 후 rankDesync 0 ④ 자기 영속 0(쓰기 저널이 복구원).
//     rank       — (0019 잔존 가설) consume→publish 루프·rank 프로젝션 == 원장 byOwner·발행자 무수정·rankDesync 0·발행 유계.
//     compact    — (0018 잔존 가설) 스냅샷+tail replay == 전체-저널 replay == 무재시작 *비트 동일*(무손실 압축)·저널 절감.
//     e2e        — E2E 동치: 멀티프로세스(persist ON, restart 포함) = 인프로세스 *비트 동일*(log+world+inv+chat+bus+audit+persist+rank)
//                  + 누설 0·phantom 0·chatDesync 0·원장 보존/정합. persist-bus·restart-bus(가방 kill→replay)·rank-restart(랭킹 kill→reconstruct).
...
const NET16 = require('../step-0019/net-core.js');   // reg 대조용(직전 step)
...
const RANK_RESTART = (seed) => ({ ...PERSIST(seed), rankRestart: { at: RESTART_AT } });   // 이 step — 랭킹(읽기 모델) 진짜 kill→쓰기 저널 reconstruct(자기 영속 0)
...
// ── reg: 인프로세스 0019(ranking OFF) → 0018 비트 동일(RankingService 박스·구독 도입 비-침습) ──
function reg(seeds) {
  console.log('== reg: 인프로세스 모드(rankRestart OFF) → step-0019 와 비트 동일(net.log + 상태 + inv/chat/bus/rank). crash/reconstruct 도입 = 비-침습 ==');
  console.log('seed   | 구성                | 0019 logHash | 0020(inproc) | log동일 | 상태동일 | inv/chat/bus/rank 동일 | 판정');
  for (const seed of seeds) {
    const cfgs = [
      ...
      ['persist+rank+rst  ', { zones: 2, recovery: true, failover: false, inventory: true, itemOps: 8, chat: true, chatOps: 10, regions: 2, bus: true, audit: true, ranking: true, persist: true, snapshot: SNAP_N, invRestart: { at: 40 } }],
    ];
    for (const [name, c] of cfgs) {
      const p = { seed, ticks: 48, clients: 4, moves: 30, radius: 4, grid: 16, incremental: true, ...c };
      const r19 = NET16.run(p);
      const r20 = run({ ...p });
      const okL = logDigest(r19) === logDigest(r20), okS = worldDigest(r19) === worldDigest(r20);
      const okX = invDigest(r19) === invDigest(r20) && chatDigest(r19) === chatDigest(r20) && busDigest(r19) === busDigest(r20) && rankDigest(r19) === rankDigest(r20);
      ...
      console.log(`${pad(seed, 6)} | ${name} | ${hex(logDigest(r19))}   | ${hex(logDigest(r20))}   | ${(okL ? '예' : '아니오').padEnd(6)} | ${(okS ? '예' : '아니오').padEnd(8)} | ${(okX ? '예' : '아니오').padEnd(21)} | ${okL && okS && okX ? 'OK' : 'FAIL'}`);
    }
  }
  console.log('  → rankRestart 미제공이면 crash/reconstruct 훅이 *호출 0*(no-op)·ranking 정상 consume→publish — 0019 와 비트 동일.');
}
...
async function e2e(seeds) {
  console.log('== e2e: 멀티프로세스(persist ON·restart) = 인프로세스 *비트 동일*(log+world+inv+chat+bus+audit+persist+rank)·누설 0·원장/투영 보존 ==');
  console.log('seed   | 시나리오      | 프로세스 | log | world | inv | chat | bus | audit | persist | 누설 | 보존/정합 | 판정');
  for (const seed of seeds) {
    for (const [name, cfg] of [['persist-bus', PERSIST(seed)], ['restart-bus', RESTART(seed)], ['rank-restart', RANK_RESTART(seed)]]) {
      const a = run(cfg);
  ...
  console.log('  → 가방 replay·랭킹 reconstruct 둘 다 *제어 평면*(cluster RPC)이라 net.log 비-기여 — 인프로세스 crash()+replay()/reconstruct() 와 비트 동일.');
}
...
async function summary(seeds) {
  console.log('== summary: 읽기 모델 영속·late-join — 랭킹(CQRS 읽기 모델) 진짜 kill→*쓰기 모델 저널* reconstruct 로 투영 재계산·자기 영속 0·E2E 비트 동일 ==');
  for (const seed of seeds) {
    const a = run(RANK_RESTART(seed));
    const b = await runMulti(RANK_RESTART(seed));
    const C = b.cluster;
    const ok = logDigest(a) === logDigest(b) && worldDigest(a) === worldDigest(b) && rankDigest(a) === rankDigest(b)
      && rankProjectionFaithful(b) && rankDesync(b) === 0 && persistDigest(a) === persistDigest(b) && svcMsgsToZones(b) === 0;
    if (!ok) FAILED = true;
    const rr = C.rankRestarted;
    console.log(`  seed ${pad(seed, 4)}: 프로세스 ${C.pids.length}개 · 랭킹 failover ${rr ? rr.oldHost + '→' + rr.newHost + '@' + rr.at : '?'}(저널 ${rr ? rr.entries : '?'}항목 reconstruct) · 투영==원장 ${rankProjectionFaithful(b)}(${b.ranking.ranks.size}아바타) · rankDesync ${rankDesync(b)} · log동일 ${logDigest(a) === logDigest(b)} | ${hex(rankDigest(b))}`);
  }
  console.log('읽기 모델은 *자기 영속 0* — kill 후 쓰기 모델의 영속 저널을 reconstruct 해 투영을 재계산(휘발 스트림 아닌 내구 저널이 복구원) · 멀티프로세스 kill→reconstruct = 인프로세스 crash()+reconstruct() 비트 동일.');
}

// ── CLI ──
const MODES = { reg, rank, e2e, sacred, recover, 'recover-rank': recoverRank, compact, degrade, inject, isolate, hide, repro };
(async () => {
  ...
  } else { console.log('mode: reg | rank | e2e | sacred | recover | recover-rank | compact | degrade | inject | isolate | hide | repro | all'); process.exit(2); }
