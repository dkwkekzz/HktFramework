...
4. **Redis-like 정합/신뢰성 묶음(4차 고도화 arc 0252~0260)** ✅ — 왜: 기본 캐시(읽기·TTL·LRU) 위에 실 Redis 가 갖는 정합·효율·안전 연산이 빠져 있었다(쓰기 정합·배치·침투·동시성·관측). / 어떻게: 각 기능마다 다른 이론 — ⒜ **쓰기 정합**: write-through(set 시 backing source 동시 기록 `step-0252`)·explicit delete(DEL·source 까지 영구 제거 `step-0257`)·namespace 무효화(SCAN+DEL prefix 일괄 `step-0259`) ⒝ **읽기 효율**: bulk get(mget·라운드트립 N→1 `step-0253`)·negative caching(known-absent 침투 방어 `step-0254`) ⒞ **동시성/수명**: put-if-absent(SETNX·최초-기록-승 분산 락 primitive `step-0255`)·per-key TTL(SETEX·차등 만료 `step-0256`) ⒟ **관측**: cacheStats(hit/miss/hitRate INFO `step-0258`) ⒠ **정합 capstone**: coherent()(store↔setAt 1:1·store∩negatives=∅·keyTtl⊆store — 전 메커니즘 혼합에도 구조 불변 `step-0260`). / 했나: 9-step decade·각 새 메시지 타입/토글이라 미수신 시 직전 비트 동일(reg 0). 단 *source 는 여전히 주입 backing map*(실 DB 박스 아님) — write-through 는 "정합의 절반"(아래 5번 ⬜ 와 합류).
5. **write-behind·소스 PersistStore 연결** ⬜ — 왜: 캐시 쓰기를 DB 로 비동기 반영·소스를 실 영속 박스로(write-through 0252 는 *주입 map* 에만 씀). / 어떻게: write-behind 큐·소스를 실 PersistStore 박스로. / 했나: 미착수(현 소스=주입 backing map·2차 잔여).

**지금 어디 / 다음**: set/get + read-through + TTL·무효화·LRU(0211~0226) + **Redis-like 정합/신뢰성 4차 arc(0252~0260·write-through·bulk·negative·SETNX·SETEX·delete·prefix·stats·coherent capstone)**까지 — 캐시 박스 *연산 표면*은 상용 Redis 에 근접. 다음(2차) = 소스를 실 PersistStore 박스로(write-behind·write-through 의 진짜 영속 절반).
