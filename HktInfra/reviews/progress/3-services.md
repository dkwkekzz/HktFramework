...
8. **공유 금고(Guild Bank) — 조직 공유 아이템 원장** 🟡 *자라는 중* — 왜: 길드가 *공유 아이템*(길드 소유 장비·재료)을 멤버끼리 맡기고 꺼내야 — 개인 가방(0014)·거래소 escrow(0117)·우편 custody(0157)의 *조직 공유* 판. / 어떻게(거래소·우편 escrow arc 를 그대로 따른 동형 골격·기능마다 다른 이론):
   ...
   - ⒡ **가방 실연동(escrow 실체화)** ✅ *코어(0511~0520·#46 해소)* — 왜: 0191~0200 금고는 itemId *문자열*만 보유 — 예치해도 멤버 가방서 안 빠짐(가짜 escrow·아이템 우편 0161~0164 가 닫힌 갭의 길드 판). / 어떻게(거래소 0117~0130·우편 0161~0170 의 조직 공유 판·기능마다 다른 이론): 새 `guildBankInv` 플래그로 escrow 를 가방 원장 아바타로 실체화 — 예치=멤버 가방→'escrow' give(0511)·인출='escrow'→멤버 give(0512·거래소 list/buy leg 판)·2-서비스 보존(escrowIds==가방 escrow 소유·Σvault·bankConsistent·0513)·crash→reconstruct escrowIds 재구성(0514)·**saga 신뢰 전달**(회신 닫힌 고리 0515·미해결 추적+손실 감지 0516·재전송+멱등 dedup 0517·회계 정합 bankSagaConsistent 0518·거래소 0121~0129 판)·2-서비스 교차 회계(giveOks==escrowXfers·0519)·grand capstone(네 정합층 물리+보존+saga+교차·0520). / 했나: `step-0511~0520`(`svc-guild.js:76`_custody·`:135`bankSagaConsistent·`svc-guild-txn.js:50,61`deposit/withdraw leg·`guildBankInv`/`guildBankSaga` 플래그·`guildbankcap` 5/5). guildBankInv OFF→give 0=가짜 escrow(0510 비트 동일·reg 0). **잔여**: 완전 liveness(재시도 상한·포기·재admission·영구실패 3분할·give-fail 보상)는 2차 arc(#80·우편 0172~0180 판) — 현 코어는 회신/재전송/dedup 까지.
   - ⒢ **인출 권한 등급·완전 liveness·멀티프로세스** ⬜ — 인출이 멤버십만 가드(예치자/rank 무관·#48)·완전 saga liveness/보상(#80)·host.js 0(#9). / 미착수.
   - ⒣ **박스 유계(정리)** 🔴 — svc-guild.js 31.0KB>30KB(escrow 추가 부작용·#79) — 다음 기능 step 전 saga 헬퍼 분리(0268 svc-mail-saga 패턴).
9. **길드 종료(disband)·발행 게이트 통합** ⬜ — 왜: 솔로-마스터 길드 해소 길 0(#44)·거래소+우편+길드 발행 일원화. / 미착수.

**지금 어디 / 다음**: 분리→증분/master 보호→발행→영속/스냅샷→배지/feed 영속/정합→마스터 이양→single-master capstone(0181~0190)→공유 금고 arc(0191~0200)→**금고↔가방 escrow 실연동 코어(0511~0520·#46 해소·예치/인출 실 이동·2-서비스 보존·crash·saga 신뢰 전달·교차·grand capstone)**까지 — 거래소/우편 escrow 의 *조직 공유* 금고가 이제 가방과 실제로 엮인다(itemId 문자열→실 아이템 이동). **너비 1차 완료 후 승인된 2차 심화**(#46 은 ledger 🔴 load-bearing·2차 대기였던 것을 STATE §2 ②로 집행·과심화 아님). **다음**: ⓪ 박스 분할(#79·svc-guild 31.0KB) 선결 → 완전 liveness(#80·재시도 상한/포기/영구실패/보상·우편 0172~0180 판)·인출 권한(#48)·disband(#44)·멀티프로세스(#9)는 2차 대기.
...
> **이 계층 다음 걸음**: 거래소↔가방 2-서비스 saga 가 *원자성·신뢰 전달·교차 정합·**liveness***까지 닫힘(#31·#35·0121~0140·네 정합층). **우편 박스가 거래소 arc 의 *완전한 동형*으로 섰다**: 전용 박스 ✅(0142~0150) + MailFeed 미읽음 배지 ✅(0151~0156) + 아이템 첨부 ✅(0157~0160) + 가방 연동 3 레그+2-서비스 보존 ✅(0161~0164·#40 해소) + 아이템 give saga ✅(0166~0170) + **saga liveness 유계화·자율 복구·관측·정합 capstone ✅(0172~0180·거래소 0131~0140 의 우편 판·#42 해소)** — 메시지·배지·아이템·가방 연동(3레그+saga+liveness) 다섯 축. **정리 2회로 박스 유계 유지**(0171 영속 부품 분할 + 0175 헤더 압축·#34d 해소·core 25.3KB). **길드 전용 박스 ✅(0181~0190·single-master·이양 쌍 거래) + 공유 금고 ✅(0191~0200·원장 단일 소유·bankConsistent).**
>
> **★ #16 서비스 계층 실행 검증 ✅(0491~0500·검증 인프라 라운드·서버 기능 0)**: 위 세 서버(거래소·우편·길드)의 정합 술어가 오래도록 *생성 step verify.js(git per-commit)*에만 살아 HEAD 재검증 불가였다(#16 "spine 승급" 격차 — 이 계층 곳곳의 "미착수(#16)"). 0491~0500 이 각 술어를 *재작성*(옛 코드 git 소실)해 `engine/verify-kit.js` ORDER 로 편입: 거래소 sagaLiveConsistent/교차 escrowXfers/취소·만료 반환(svcexchangecap·svcexchangexfer·svcexchangecancel `verify-kit.js:1166,1189,1366`)·우편 sagaLiveConsistent/교차/만료 통수(svcmailcap·svcmailxfer·svcmailexpire `:1211,1234,1305`)·길드 rosterConsistent/bankConsistent(svcguildcap·svcbankcap `:1257,1280`)·3 서비스 종합 격리(svcsvccombined `:1327`)·promotedsvc 등록 가드(`:1394`). run() 을 서비스 opts 로 구동·박스 무수정→매 step reg 0. **이제 매 spine 이 거래소·우편·길드 불변을 현재 src 에 재검증**(#16 서비스 계층 잔여 해소). **★ #16 완전 해소(0501~0510·#76)**: 남았던 완전 saga liveness(회신 손실 주입→abandon/permFailed 3분할 *발현*)를 우편 saga 손실 seam 으로 편입 — 정상 흐름 pending 0 자명 참을 *실 손실 체제* 비자명으로 승격(9 capstone + 가드·`verify-kit.js:1408~1586`). *잔여(경미)*: 거래소 측 손실 seam 미배선(#77·우편 판 이식만 남음)·readmit→drain loss-cleared 미검증(#78).
>
> **남은 견고화**: 멀티프로세스 배선(#9·host.js 0)·읽기모델 무압축(#39)·거래소 저널 별 PersistStore(#30b)·채팅 홉 신뢰(#7)·금고↔가방 escrow(#46)·close-step src size(#43)·topo-run 분할(#34c). 수신함 동적 N(#27).
