...
## 귓속말/파티 라우터(wrouter) + 수신함(mbox) ✅ 자라는 중 *(전달 신뢰 → 유계·관측·재시작 안전)*

- **푸는 병목**: "X 에게"·"파티 전원에게" 가 *어디 있는지·살아 있는지* 모른 채 보내진다 — 프레즌스 SSOT(계층5)를 조회해 라우팅하고 *진짜 닿았는지*까지 보장해야(전송은 떨어뜨리니까). 더해 그 보장이 *메모리를 무한히 먹거나 라우터 재시작에 깨지면* 안 되고 결과가 *밖에서 안 보이면* 안 된다.
- **지금 어디**: 프레즌스 질의 첫 실사용 → 전달 exactly-once → **유계·관측·재시작 안전으로 단단히**.
  - `step-0071~0080` — 라우터(질의→전달/반송·failover 재타깃)·1:N 파티·멤버십 분리 → 전달 신뢰 호(영수증→재시도→상한→통지→dedup exactly-once).
  - `step-0081·0089·0090` — dedup 메모리 *두 축 유계화*: seq=연속 워터마크(O(gap)·`svc-mailbox.js:49`)·epoch=재시작 펜싱(0089 가 *재시작 seq 리셋의 조용한 유실 버그*를 수정→0090 옛 epoch 가지쳐 현재만). "재시작 안전을 무계 메모리 없이".
  - `step-0082·0087` — 전달 *양 끝을 버스로 관측*: 포기(svc.whisper.failed)·성공(svc.whisper.delivered{tries})을 audit 에 발행(발행자 무수정).
  - `step-0083·0088` — 파티 1:N 완료 회계: partyId 별 라우팅 결정(routed/bounced·done)+실수신 ack(delivered·acked) → N-of-M 가시.
- **남은 것**: 파티 ack 가 Mailbox 가진 멤버만(#23)·epoch straggler 가정(#24)·멀티프로세스 미배선(#9)·ON-의미 spine 미승격(#16).
...
> **이 계층 다음 걸음**: 채팅에 홉 신뢰(ack/resend)를 채워 "팬아웃도 안 샌다"를 맞추고(#7), 파티 ack 를 멤버별 수신함으로 넓히고(#23) 라우팅/영속 호를 멀티프로세스로 배선한다(#9). 안정된 전달 호를 spine 가설 모드로 승격(#16). 미착수 박스(거래소부터) 씨앗 심기 — 거래소는 *존을 넘는 거래*라 가방 쌍 거래의 확장 무대.
