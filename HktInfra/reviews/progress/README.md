...
상태 기호(STATE §5 정합): 🟡 자라는 중(씨앗+능력 누적·아직 부분) · 🌱 씨앗만(계약만 섰음) · ⬜ 미착수(아직 박스 없음). **★ 0430 현재 ⬜/🌱 박스 0 — 너비 1차 + … + #62 코드 합류(0411~20·옛 runMulti 가 코디네이터 호출) + **#61 업스트림 intent 실 클라(0421~30·UpClient 발신+수신 양방향 실 클라·desync 0)** 완료. #61 — 다운스트림(DownClient)에 이어 업스트림도 실 클라(합성 entityOps 종료·SPINE §4 경로1·경로2 모두 실 클라 in-proc E2E).**

> **기준선**: 닫힌 step **~0430**([../../STATE.md](../../STATE.md) NOW=0430·이 지도 0430 까지 반영). 재현 검증(이 지도가 *주장*이 아니라 *사실*임의 근거): src/ 실 `Math.random` 0 hit(결정론) · UE 모듈 링크 0 hit(headless) · `node run.js spine` = **ALL OK** · 30KB 초과 박스 0(`wc -c src/*.js`·최대 cluster-coord 29.3KB·orch-zonebridge 28.3KB·gateway 27.6KB·client 20.8KB) · orch 가 `zone.js` 직접 import 0 hit(④ 은닉)·cluster-coord `require` 0(순수 주입) · **#61 업스트림 실 클라 ✅**(0421~0430 in-proc 해소·재현: `upe2ecap` 5/5 양방향 E2E·uc0 수렴·b1 제거·보존·발신 회계·`upvsscript` 5/5 실 클라≡합성 entityOps·`reg`/`spine` ALL OK[upClients OFF→비트 동일]). **#16 확장**: 이번 묶음 bespoke 모드(upclient~upe2ecap 10개)도 미승급(현 spine 모드=최신 `upe2ecap` 1개·나머지 git per-commit). **잔여 load-bearing: 진짜 비동기(#4·lockstep 해제)·실 host.js child 경계 업스트림(#70·#57 짝)** · orch zoneHost entity 컨테이너 동기(#68·경미) · 동치 transitive(#69·경미).

## 다음 큰 걸음 (직관) — 🎯 #61 업스트림 intent 실 클라: 경로1 양방향 실 클라 (0421~0430·SKILL §3.6)

> **0421~0430 = #61 해소(in-proc)** — 다운스트림(0342~0350 실 DownClient·경로2)에 이어 *업스트림*(경로1·클라→게이트웨이→존 intent)도 실 클라로. 여태 합성 entityOps 주입(topo-inject)이 클라 대역으로 intent 를 *스크립트*했으나 실 클라가 *생성·발신*하지 않았다(#61). `UpClient`(client.js·kind 'upclient')가 자기 plan 으로 intent 를 발신하고 자기 AOI 뷰를 수신해 권위로 수렴 — 발신+수신 = 실 양방향 클라.

**업스트림 실 클라 sub-arc(0421~0430):** 골격 enter(0421)→move plan(0422)→양방향 수신(0423)→수렴 desync0(0424·seenSig==zoneAuthSig)→≡합성 entityOps 동치(0425)→다중 클라 인터리빙(0426)→leave 생애주기(0427)→손실 하 수렴(0428·egress 손실→gap-resync)→업스트림 회계(0429·발신 손실 0)→grand capstone 양방향 E2E(0430).

**⚠ 단계 평결(SKILL §3.6)**: 너비 1차 완료(⬜/🌱 0)라 2차 허용. 0421~0430 은 직전 0411-0420 verdict 가 STATE §2 로 올린 *load-bearing #61* 을 집행한 **새 능력**(존 서버 기능 9·다중 클라 전파의 업스트림 짝·과심화 아님). 재현 증거(시드 5종 5/5): `upe2ecap`(양방향 E2E·uc0 수렴·b1 제거·보존·발신 회계)·`upvsscript`(실 클라 ≡ 합성 entityOps)·`uplossy`(손실 하 수렴 gaps1·resyncs1)·`reg`/`spine` ALL OK(upClients OFF→비트 동일)·`Math.random`/UObject src 0·>30KB 박스 0(client.js 20.8KB). **#61 🟡대폭 진전(in-proc ✅·잔여=실 OS 프로세스 #70=#57 짝)**. **잔여**: ⒜ **#4 진짜 비동기**(lockstep 배리어 미해제·양방향 실 클라가 섰으니 다음 큰 load-bearing) ⒝ **#70 실 host.js child 경계 업스트림**(다운스트림 #57 의 업스트림 짝) ⒞ **#68 entity 컨테이너 동기**(경미) ⒟ **#69 동치 transitive**(경미). **다음 권고: 진짜 비동기(#4) 또는 실 host.js child 업스트림(#70·#57 짝), 그 다음 경미 정리(#68·#69).** step-loop 이 STATE §2 로 승급.
