...
상태 기호(STATE §5 정합): 🟡 자라는 중(씨앗+능력 누적·아직 부분) · 🌱 씨앗만(계약만 섰음) · ⬜ 미착수(아직 박스 없음). **★ 0390 현재 ⬜/🌱 박스 0 — 너비 1차 + … + 실 cluster 데이터 평면(0361~0370·#57·clusterCoherent desync 0) + broker 측 제어 평면 상주 코디네이터(0371~0380·#62·연속 tick 루프·coordCoherent) + 코디네이터 placement 권위 양방향 동기(0381~0390·#65·migrate/failover 후도 syncedCoherent) 완료. #57/#62/#65 — 실 host.js OS 프로세스 데이터 평면을 상주 broker 제어 평면이 연속 구동하고, 이주/장애 후도 placement 권위로 정합 유지.**

> **기준선**: 닫힌 step **~0390**([../../STATE.md](../../STATE.md) NOW=0390·이 지도 0390 까지 반영). 재현 검증(이 지도가 *주장*이 아니라 *사실*임의 근거): src/ 실 `Math.random` 0 hit(결정론) · UE 모듈 링크 0 hit(headless) · `node run.js spine` = **ALL OK** · 30KB 초과 박스 0(`wc -c src/*.js`·최대 orch-zonebridge 28.96KB·gateway 28.2KB·cluster-coord 19.9KB) · orch 가 `zone.js` 직접 import 0 hit(④ 은닉)·cluster-coord `require` 0(순수 주입) · **코디네이터 placement 권위 양방향 동기 ✅**(0381~0390·#65 해소·재현: `coordsyncedcap` 5/5 syncedCoherent Y·coordDesync 0 vs 옛 clusterDesync 1·migrate/failover 포함·reg 0·spine ALL OK). **#16 확장**: 이번 묶음 bespoke 모드(coordplace~coordsyncedcap 10개)도 미승급(현 spine 모드=최신 `coordsyncedcap` 1개·나머지 git per-commit) → reg 구조적 0 와 겹쳐 cluster-coord spine 안전망 얇음. **잔여 load-bearing: placement↔orch zoneHost 이중 권위 합류(#67)·tick placement-aware(#66)·옛 cluster-run.js runMulti 코드 합류(#62)** · 진짜 비동기(#4) · 업스트림 intent 실 클라(#61).

## 다음 큰 걸음 (직관) — 🎯 #65 양방향 동기: 코디네이터 placement 권위 분리 (0381~0390·SKILL §3.6)

> **0381~0390 = #62 가 남긴 #65 해소** — 코디네이터 lifecycle(migrate/failover)이 placement 권위를 안 들어 실 migrate 후 `clusterDesync`(orch plan 기준 host 조회)가 stale 로 발산했고(0380 capstone 이 migrate/failover 를 *제외*해야 했던 근본 원인), 코디네이터가 *placement 권위(where)*를 직접 소유(orch=entity 권위/what 분리)하는 설계로 풀었다. lifecycle 갱신만 따라가면 *migrate/failover 후도* 정합.

**양방향 동기 sub-arc(0381~0390):** placement SSOT(0381)→coordDesync(placement 기준·0382)→migrate 가 placement 갱신(0383 핵심 fix)→coordCoherent 가 coordDesync 채택(0384)→failover placement 갱신+lostZones(0385)→lost 기대 부재 제외(0386·ghost forward 유지)→placement-aware report(0387)→syncPlan placement 기준(0388)→placementCoherent bijection(0389)→capstone syncedCoherent(0390·migrate/failover 포함 정합).

**⚠ 단계 평결(SKILL §3.6)**: 너비 1차 완료(⬜/🌱 0)라 2차 허용. 0381~0390 은 prior 리뷰 verdict 가 지목한 *load-bearing #65*(실 migrate/failover 후 정합)를 집행한 **새 능력**(오케스트레이터 기능 18·과심화 아님). 재현 증거(시드 5종 5/5): `coordsyncedcap`(syncedCoherent Y·coordDesync 0 vs 옛 clusterDesync 1)·`reg`/`spine` ALL OK·`Math.random`/UObject src 0·cluster-coord require 0(순수 주입)·orch→zone.js import 0·>30KB 박스 0(cluster-coord 19.9KB). **#65 ✅해소**(단 ledger 가 제안한 "orch zoneHost write-back" 이 아니라 where/what 권위 *분리*로 — 잔여는 아래로 분리). **세 잔여**: ⒜ **#67 이중 권위** — placement 가 orch zoneHost(0301~)와 decouple·orch 내부 미동기(write-back 아닌 분리의 대가) ⒝ **#66 tick 미-placement-aware** — `tick()`/deliver 재생이 여전히 orch.hostSpawnPlan 사용(run 중 migrate latent·capstone 은 lifecycle 을 루프 뒤 실행해 미발현) ⒞ **#62 코드 합류** — 코디네이터↔옛 cluster-run.js runMulti 병존. **다음 권고: placement↔orch zoneHost 합류(#67·#62) + tick placement-aware(#66), 그 다음 업스트림 intent 실 클라(#61)·진짜 비동기(#4).** step-loop 이 STATE §2 로 승급.
