...
상태 기호(STATE §5 정합): 🟡 자라는 중(씨앗+능력 누적·아직 부분) · 🌱 씨앗만(계약만 섰음) · ⬜ 미착수(아직 박스 없음). **★ 0470 현재 ⬜/🌱 박스 0 — 너비 1차 + … + **#4 비동기 substrate in-proc(0431~40·`async-core`) + 실 Net·sim seam 브리지 등가(0441~50·`async-net`) + 실 run() net.step 배리어 실제 치환(0451~60·`async-barrier`) + 다중 존 이주 하 유계 resync(0461~70·`async-barrier` wrap-aware interior 가드)** 완료. #4 — 0001 이래 열린 채였던 lockstep 의존을 *실 run() 전송 경로*에서 async substrate 로 치환하고 이주가 있는 다중 존까지 결정론 보존(다중 존 loss+delay world/뷰==lockstep·exactly-once·이주 전 유계 resync·OFF→reg 0). 잔여=완전-ON(경계 포함 async·lockstep 등가 불가)·실 host.js child 업스트림(#70).**

> **기준선**: 닫힌 step **~0470**([../../STATE.md](../../STATE.md) NOW=0470·이 지도 0470 까지 반영). 재현 검증(이 지도가 *주장*이 아니라 *사실*임의 근거): src/ 실 `Math.random` 0 hit(결정론·주석만) · UE 모듈 링크 0 hit(headless) · `node run.js spine` = **ALL OK** · 30KB 초과 박스 0(`wc -c src/*.js`·최대 cluster-coord 29.3KB·async-net 17.4KB·async-barrier 11.3KB) · orch 가 `zone.js` 직접 import 0 hit(④ 은닉) · **#4 substrate ✅ in-proc + 실 Net·sim seam 브리지 등가 ✅ + 실 run() net.step 배리어 실제 치환 ✅ + 다중 존 이주 하 유계 resync ✅**(0431~0470·재현: `mze2ecap` 5/5[다중 존 loss+delay world/뷰==lockstep·exactly-once moveDup0·유계 span3<horizon4·deferredAcrossHandoff0·handoff4~15]·가드 load-bearing[ON 수렴/OFF 발산 5/5]·`reg`/`spine` ALL OK[asyncBarrier OFF→net.step 그대로 비트 동일]·async-barrier `topo-run.js:88`/`:102` gated). **#16 확장**: 이번 묶음 bespoke 모드(mzdiverge~mze2ecap 10개)도 미승급(현 spine 모드=최신 `mze2ecap` 1개·나머지 git per-commit). **잔여 load-bearing: 실 host.js child 경계 업스트림(#70·#57 짝)·완전-ON(경계 포함 async·lockstep 등가 불가·별 정합 기준 필요·#73)** · orch zoneHost entity 컨테이너 동기(#68·경미) · 동치 transitive(#69·경미) · interior/ownerZone peek by-construction(#53 확장).

## 다음 큰 걸음 (직관) — 🎯 #4 완전 async 전환: 다중 존 이주 하 유계 resync (0461~0470·SKILL §3.6)

> **0461~0470 = #4 다중 존 이주 하 유계 resync** — 0451~0460 이 손실/지연을 *단일 존*(무이주)에서만 world==lockstep 로 흡수한 뒤, 이번 묶음은 *이주가 있는 다중 존*까지 결정론을 확장했다. **의외 발견(0461)**: lockstep(무섭동) *자체가* 이주 후 stale 존에 도착한 move 를 폐기(`zone.js` move 핸들러)하는데, 지연/손실이 이주 타이밍을 바꾸면 이 move-drop 집합이 lockstep 과 달라져 발산한다(#72). 해법은 `async-barrier.js` 의 **wrap-aware interior 유계 resync 가드**: barrier 가 소유 존을 peek(`ownerZone`)해 엔티티가 region 양 끝(경계+wrap)에서 `horizon` 이상 떨어진(interior) move 만 흡수 → deferSpan<horizon 이라 deferred move 는 이주 경계 도달 *전* 재배달(이주 전 유계 resync) → 다중 존 world/뷰==lockstep.

**#4 다중 존 유계 resync sub-arc(0461~0470):** 발산 포착(0461·negative control·#72 고정)→ownerZone+interior·loss 게이트(0462)→delay 게이트(0463)→결합+exactly-once(0464)→유계 증명 deferSpan<horizon(0465)→가드 load-bearing 대조(0466·`mzGuard` ON 수렴/OFF 발산)→이주 전 유계 resync 명제 deferredAcrossHandoff0(0467)→exactly-once 완전 회계 moveDup0·lost0·pendingAtEnd0(0468)→다운스트림 뷰 수렴(0469)→grand capstone mze2ecap(0470).

**정직한 한계(0461·0470)**: 완전-ON(경계 포함 async)은 lockstep 등가가 *불가능*하다 — lockstep 자체가 이주 경계 move-drop 을 타이밍 의존적으로 하므로, interior 유계가 이 모델의 정합 최대치. 경계 근처 move 는 즉시 in-order 배달(async 안 됨) = "완전 async" 의 정직한 상한. 완전-ON 은 별 정합 기준(lockstep 아닌)이 필요(#73).

**⚠ 단계 평결(SKILL §3.6)**: 너비 1차 완료(⬜/🌱 0)라 2차 허용. 0461~0470 은 직전 0451-0460 verdict 가 STATE §2 로 올린 *load-bearing #4 완전 async 전환(다중 존 이주 유계 resync)·#72* 를 집행한 **새 능력**(코디네이션 substrate 기능 10·과심화 아님·기존 박스 `async-barrier.js` 심화). 재현 증거(시드 5종 5/5): `mze2ecap`(다중 존 loss+delay world/뷰==lockstep·exactly-once moveDup0·유계 span3<horizon4·deferredAcrossHandoff0·handoff4~15)·가드 load-bearing(ON 수렴/OFF 발산 5/5)·`reg`/`spine` ALL OK(asyncBarrier OFF→net.step 비트 동일)·`Math.random`/UObject src 0(주석만)·async-barrier `topo-run.js:88`/`:102` gated·>30KB 박스 0(async-barrier 11.3KB). **#72 ✅해소·#4 🟡대폭 진전(다중 존 이주 유계 resync ✅·잔여=완전-ON·downstream 재접속)**·부수 **#73 신규(완전-ON 경계 포함 async·lockstep 등가 불가·별 정합 기준 필요·경미)·#53 확장(interior/ownerZone peek by-construction)**. **잔여**: ⒜ **#70 실 host.js child 경계 업스트림**(#57 짝·다음 큰 load-bearing) ⒝ **완전-ON/#73**(별 정합 기준) ⒞ **#68·#69 경미**. **다음 권고: 실 host.js child 업스트림(#70·#57 짝), 그 다음 경미 정리(#68·#69·#73·#16 승급).** step-loop 이 STATE §2 로 승급.
