...
상태 기호(STATE §5 정합): 🟡 자라는 중(씨앗+능력 누적·아직 부분) · 🌱 씨앗만(계약만 섰음) · ⬜ 미착수(아직 박스 없음). **★ 0440 현재 ⬜/🌱 박스 0 — 너비 1차 + … + #61 업스트림 실 클라(0421~30) + **#4 비동기 실행 substrate(0431~40·`async-core.js`·논리 클럭·인과 정렬·in-proc)** 완료. #4 — 0001 이래 열린 채였던 lockstep 의존을 *해제할 기계*(논리 클럭+인과 정렬+holdback+resync)를 in-proc 으로 처음 손에 쥠(잔여=실 net.step 배리어 치환).**

> **기준선**: 닫힌 step **~0440**([../../STATE.md](../../STATE.md) NOW=0440·이 지도 0440 까지 반영). 재현 검증(이 지도가 *주장*이 아니라 *사실*임의 근거): src/ 실 `Math.random` 0 hit(결정론) · UE 모듈 링크 0 hit(headless) · `node run.js spine` = **ALL OK** · 30KB 초과 박스 0(`wc -c src/*.js`·최대 cluster-coord 29.3KB·async-core 16.3KB·client 20.8KB) · orch 가 `zone.js` 직접 import 0 hit(④ 은닉)·cluster-coord `require` 0(순수 주입) · **#4 비동기 substrate ✅ in-proc**(0431~0440 해소·재현: `asynce2ecap` 5/5[M 복제 순열+손실→clock 위반 0·전복제 desync0·인과존중·exactly-once]·`reg`/`spine` ALL OK[async-core run() 미호출→비트 동일]·async-core run() 경로 호출 0 hit). **#16 확장**: 이번 묶음 bespoke 모드(lcstamp~asynce2ecap 10개)도 미승급(현 spine 모드=최신 `asynce2ecap` 1개·나머지 git per-commit). **잔여 load-bearing: 실 net.step 배리어 치환(#4 후속 arc)·실 host.js child 경계 업스트림(#70·#57 짝)** · orch zoneHost entity 컨테이너 동기(#68·경미) · 동치 transitive(#69·경미).

## 다음 큰 걸음 (직관) — 🎯 #4 진짜 비동기 substrate: 논리 클럭·인과 정렬 in-proc (0431~0440·SKILL §3.6)

> **0431~0440 = #4 in-proc substrate** — 0001 이래 열린 채였던 #4(broker lockstep·net.step 동기 배리어가 결정론을 떠받침)를 *해제할 기계*를 처음 세웠다. DownClient/UpClient 가 *in-proc 먼저* 였듯, 실 net.step 치환에 앞서 그 토대(논리 클럭+인과 정렬+holdback+resync)를 신규 박스 `async-core.js`(run() 밖 검증 전용 substrate·매 step reg 구조적 0)에 in-proc 으로 증명했다. 핵심 명제: 메시지가 *물리적으로 다른 순서/손실*로 도착해도 인과(happens-before)를 복원해 결정론 전순서로 적용→수렴(lockstep 없이).

**#4 substrate sub-arc(0431~0440):** Lamport 논리 클럭(0431)→send/recv 인과·clock condition(0432)→결정론 전순서(0433)→holdback 재정렬·low-water-mark(0434)→인과 의존 배달·FIFO-free(0435)→async 수렴 desync0(0436)→배리어-free 진행·비-lockstep(0437)→손실 하 gap-resync(0438)→인과 회계·exactly-once(0439)→grand capstone(0440·M 복제 순열+손실 E2E).

**⚠ 단계 평결(SKILL §3.6)**: 너비 1차 완료(⬜/🌱 0)라 2차 허용. 0431~0440 은 직전 0421-0430 verdict 가 STATE §2 로 올린 *load-bearing #4* 을 집행한 **새 능력**(코디네이션 비동기 substrate 기능 7·과심화 아님 — #4 는 0001 이래 최상위 미해소 load-bearing). 재현 증거(시드 5종 5/5): `asynce2ecap`(M 복제 순열+손실→clock 위반 0·전복제 desync0·인과존중·exactly-once·손실 발생)·직접 재현(`makeHoldback` 6 인터리빙 동일 전순서·`totalOrderSound` strict/causal·`clockConditionViolations` 0)·`reg`/`spine` ALL OK(async-core run() 미호출→비트 동일)·`Math.random`/UObject src 0·async-core run() 경로 호출 0 hit·>30KB 박스 0(async-core 16.3KB). **#4 🟡대폭 진전(in-proc substrate ✅·잔여=실 net.step 배리어 치환)**. **잔여**: ⒜ **#4 실 net.step 치환**(substrate 를 실 전송 배선에·다음 큰 load-bearing) ⒝ **#70 실 host.js child 경계 업스트림**(다운스트림 #57 의 업스트림 짝) ⒞ **#68 entity 컨테이너 동기**(경미) ⒟ **#69 동치 transitive**(경미). **다음 권고: #4 실 net.step 배리어 치환(substrate 배선) 또는 실 host.js child 업스트림(#70·#57 짝), 그 다음 경미 정리(#68·#69).** step-loop 이 STATE §2 로 승급.
