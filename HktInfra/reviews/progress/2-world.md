...
1. **결정론 시뮬 심장(VM)** ✅ — 왜: 세계를 굴릴 코어가 먼저 있어야. / 어떻게: 결정론 상태 머신(시드+입력→상태). / 했나: `step-0001` 존 VM + `step-0003` `ISimCore` 인터페이스 동결(C++ 교체 이음새).
2. **복제(죽어도 이어받기)** ✅(C++서 부활) — 왜: 심장 하나면 죽으면 끝. / 어떻게: 상태 머신 복제 — 추종자가 *입력열만* 미러해 같은 해시 수렴(상태 0바이트). / 했나: `step-0002~0004` 결정론 복제(현실 전송 지연·손실·재정렬에도 desync 597→0). 비트-결정론은 C++ 승격에서 부활(STATE §3 🔴).
...
**무슨 서버인가**: 던전·매치처럼 수요 따라 떴다 사라지는 *일회성* 시뮬 서버. *비유 — 상설 매장(존) vs 팝업 스토어(인스턴스).*

**필요한 기능들**:

1. **수요 탄력 spawn/despawn** ⬜ — 왜: 오픈월드 존과 수명주기를 분리해 탄력 확보. / 어떻게: 오케스트레이터가 일회성 시뮬을 띄우고 내림. / 했나: 미착수(토폴로지에만 존재·`src/` 박스 파일 없음).

**지금 어디 / 다음**: 미착수. 다음 씨앗 = 일회성 시뮬 spawn/despawn 최소 계약.
