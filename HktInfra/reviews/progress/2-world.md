...
8. **다운스트림 AOI 뷰 산출·포착(#9 후속·월드 다운스트림)** 🟡 — 왜: 브리지 런타임 존(#56·0281~)의 `onTick` 이 세션에 `view_delta`(AOI 뷰)를 산출하지만 *no-op 싱크로 드롭*돼 왔다(`topo-actors.js:71`·SPINE §4 경로2 *월드 다운스트림* = 뷰가 *내려가는* 절반이 미배선). / 어떻게: 런타임 존 net 싱크를 *버퍼링*으로(`topo-actors.js:71` `{buf:[],send}`) host 가 산출한 AOI 뷰를 보관·orch 가 읽기 전용 질의로 *내용*을 검증(전역 net 무오염→버퍼 미읽으면 reg/spine 비트 동일). 검증 척도: AOI 정확성(`zoneVisibleIds` == 산출 enter·반경 안만·먼 것 제외·`orch-views.js:69`)·게이트웨이 주소 발신. / 했나: `step-0319` 포착(`zoneViewFrames`·`orch-views.js:73`)→`0320` AOI 정확성(a1·a2 반경 밖→각자 자기만 봄·게이트웨이 발신). 플래그 불요(버퍼 미읽으면 동작 불변). *이어짐: 증분 델타·상호 가시/exit·직렬화·격리·이주 연속성·무굶김·무손실·capstone 은 `0321~0330`(다음 리뷰 묶음). 한계: 산출·포착·검증까지 — 게이트웨이→실 클라 *전파*(재전송/ack·실 클라 하니스)는 후속.*

**지금 어디 / 다음**: 존 하나가 *분할·관심영역·권위 이주·죽어도 부활*까지, 그 위에 **다운스트림 AOI 뷰 산출·포착(0319~0320·#9 후속·예전 드롭하던 뷰를 버퍼링 싱크로 잡고 AOI 정확성 검증)**. 다음 = 동적 경계·N 존 · 다운스트림 뷰 게이트웨이→실 클라 전파(0321~0330 이어 진행 중).
