...
6. **부하 분산·적응형 recoverTimeout·재배치 핸드오프** ⬜ — 왜: 배치를 부하로 결정·존 이주. / 어떻게: 부하 신호 기반 배치 + 재배치 시 권위 핸드오프. / 했나: 미착수(2차).

**지금 어디 / 다음**: zone failover + self-healing 루프 + **존 배치 SSOT/질의 기본 통신(0203~0204)**까지. 다음(2차) = 부하 기반 배치·재배치 핸드오프 + *진짜 비동기*(가장 큰 빚) + 치유 HealService 분리.
