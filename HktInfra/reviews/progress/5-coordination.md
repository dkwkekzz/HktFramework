...
- **지금 어디**: 등록 씨앗 → orch 곁붙이 → **독립 박스(PresenceService)** → 그 박스가 *죽어도 살아남고 물어보면 답하는* failover-safe SSOT.
  ...
  - `step-0071~0074` — **질의 인터페이스의 첫 실사용**: 귓속말/파티 라우터(계층3·`wrouter`)가 이 SSOT 를 *조회처*로 쓴다 — 라우팅이 프레즌스 질의로 구동되고, primary 죽으면 라우터도 공지로 재타깃·윈도 손실 질의 재발신(읽기 경로 failover 가 *소비자 쪽*까지 연속). 0070 §9 의 재타깃 윈도 손실 ✅메움(#17b).
  - `step-0105~0106` — **디스커버리 메아리 펜싱**: 승격 공지(svc.presence.active)에 epoch(=promotedAt·단조)를 실어(`svc-presence.js:70`), 질의자(presmon `:33`·라우터 `:12`)가 *본 최고 epoch 이하* 낡은 공지를 거부 → 지연/flapping 공지가 *죽은 박스로 역-재타깃*하는 것 차단(읽기-디스커버리 split-brain 0·0013 펜싱·0090 epochBound 의 디스커버리 판). 채널 두 소비자 모두 발행자 무수정으로 같은 펜싱.
- **남은 것**: *진짜 플레이어 프레즌스*(SSOT 가 아직 *서비스 소비자* 건강만 앎 — "플레이어 X 가 어느 게이트웨이/존"은 미착수). self-hb 메아리(#17a)·펜싱 epoch 단일 클럭 가정(#29·진짜 분산은 논리 클럭 term)·펜싱 로직 중복(#28)·다중 standby(경미).
