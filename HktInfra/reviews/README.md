...
> 10 step = 1 묶음·십진 경계 정렬(0001–0010 · 0011–0020 · …). 닫힌 step 만 다룬다(STATE NOW 가 가리키는 미닫힘 step 제외). 닫힌 **0140** 까지 → **0001~0140 열네 묶음 감사 완료**(박스별 진행은 [progress/](progress/) 가 0140 까지 반영).
...
- **#16 | bespoke 가설 모드가 누적 회귀(verify-kit)에 미승격** | ②/⑤/정리 | 0061-0070 | 🔴 **확장**(+0131-0140 saga liveness 10 모드·총 ~70) — verify-kit ORDER 가설 모드는 `reliable·tail·inflight`(~0024)에서 멈춤(`engine/verify-kit.js`). 0033+ 버스/lease/heal/프레즌스/라우팅 + 0081~0110 전달/파티/수신함/거래소 + 0121~0130 saga 회계 + **0131~0140 saga liveness 10 모드** 의 *ON-경로 의미*는 close 시 1회만 단언·spine 은 reg-0 만 보증 → ON-의미 미보증 빚이 호 길이에 비례 누적 | **목적지 STATE §3 OPEN GAPS**(정리성) → 게이트: 안정 호(saga liveness·수신함)를 verify-kit 승격(스모크 1~2 모드·끄면 현 spine=회귀 0)
...
- **#33 | STATE §7 INDEX 30KB 천장 — 매 step 트림 강제** | 정리/도구 | 0111-0120 | 🟡 **유지(0131-0140 10/10 발현)** — 140행째 INDEX·close-step 30KB 예산이 *이번 묶음 매 step* 옛 행 압축 강제(10/10 step 실측 반복·여러 step 은 다회 트림) | **목적지 STATE §3 OPEN GAPS**(도구성) → 게이트: INDEX 묶음당 1행 압축(전문 reviews/) 또는 예산 상향(가독성만·회귀 무관)
- **#34 | wiring 박스(topo-build/topology) 30KB 초과** | 정리 | 0121-0130 | 🟡 **절반 해소** — (a) topo-build ✅ **0133**(구독 테이블→`src/topo-subs.js`·buildSubs·33.1→25.5KB·기능 0·reg 0). **(b) 잔여 topology 30.8KB**(run() 단일 함수) | **목적지 STATE §3 OPEN GAPS**(정리성) → 게이트: topology run()/액터 구성 추출(정리 step·기능 0·끄면 현 wiring=회귀 0)
