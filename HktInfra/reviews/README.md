...
| **[PROGRESS.md](PROGRESS.md)** | 6계층 큰 줄기: 지금 어디까지 왔나? (직관) | 큰 그림 | ❌ 덮어쓰기(고정 크기) |
| **review-NNNN-MMMM.md** | 이 10 step: 척추 지켰나·이슈? | 묶음 | 묶음당 1편 |
| `../step-NNNN.md` | 이 한 조각: 어떻게 구현? | step | step당 1편 |

> 늘어나는 건 *아래 두 고도*(묶음·step)다. 위 둘은 거의 동결·고정 크기 — FOUNDATIONS 는 step 표를 두지 않고, **`PROGRESS.md`** 는 매 리뷰가 *덮어 갱신*하는 단일 큰 줄기 진행 서사(누적 0).

> **큰 줄기 진행 서사 = [PROGRESS.md](PROGRESS.md)** — 매 리뷰가 무조건 갱신한다(조건·트리거 없음). 6계층 큰 그림이 *지금 어디까지 왔나*를 비전문가도 30초에 잡게 직관적으로 그린다. 점적 정합 판정은 묶음 감사(§1), 진행 권위 마커는 [../STATE.md](../STATE.md) §5. 방법은 `.claude/skills/infra-review/SKILL.md` §3.5.
