...
> 10 step = 1 묶음·십진 경계 정렬(0001–0010 · 0011–0020 · …). 닫힌 step 만 다룬다(STATE NOW 가 가리키는 미닫힘 step 제외). 닫힌 **0090** 까지 → **0001~0090 아홉 묶음 감사 완료**(박스별 진행은 [progress/](progress/) 가 0090 까지 반영).
...
- **#16 | bespoke 가설 모드가 누적 회귀(verify-kit)에 미승격** | ②/⑤/정리 | 0061-0070 | 🔴 **확장**(+0081-0090·총 ~20 모드) — verify-kit ORDER 가설 모드는 `reliable·tail·inflight`(~0024)에서 멈춤(`engine/verify-kit.js:702`). 0033+ 버스/lease/heal/프레즌스/라우팅 + **0081~0090 전달 유계화/관측/파티 집계/멤버십 영속(10 모드)** 의 *ON-경로 의미*는 close 시 1회만 단언·spine 은 reg-0 만 보증 → ON-의미 미보증 빚이 호 길이에 비례 누적 | **목적지 STATE §3 OPEN GAPS**(정리성) → 게이트: 안정 호를 verify-kit 승격(또는 wrouter/mbox/pservice 스모크 1~2 모드·끄면 현 spine=회귀 0)
...
- **#21 | svc-whisper 헤더 주석 stale** | ②/정리 | 0071-0080 | 🟡 유지(경미·0081-0090 무변) — `src/svc-whisper.js:21` "onTick 없음·시뮬 무관"이 0077 onTick(`:131`·제어평면 timeout) 추가 후 모순(코드 정확·주석만 낡음) | **목적지 STATE §3 OPEN GAPS**(정리성) → 게이트: 주석 수정(정리 step·코드 0·reg 0)
- **#22 | `topo-build.js`·`svc-whisper.js` 30KB 예산 근접** | ⑤/정리 | 0081-0090 | 🟡 신규 — topo-build 30477B(헤드룸 ~240B)·svc-whisper 28725B(28KB)·다음 기능 step 이 30KB 넘길 위험 | **목적지 STATE §3 OPEN GAPS**(정리성) → 게이트: 정리 step(박스 재분할/engine 승격·0030/0035/0043 패턴·기능 0·reg 비트 동일)
- **#23 | 멤버별 Mailbox 토폴로지 부재 → 파티 ack 반쪽** | 서비스 | 0081-0090 | 🟡 신규(경미) — 0088 partyAcked 는 Mailbox 가진 up 멤버만 delivered 기여·현 토폴로지 mbox 1개라 up 멤버 여럿 중 mbox 만 ack(0088 §9) | **목적지 STATE §3** → 게이트: 파티원마다 수신함 박스(끄면 현 1-mbox 거동=회귀 0)
- **#24 | epoch 가지치기 straggler 가정** | ④ | 0081-0090 | 🟡 신규(경미·watch) — 0090 prune(`src/svc-mailbox.js:38`)은 옛 epoch 지연 도착 0 가정·가지친 뒤 straggler 도착 시 신규 오인 재수신 가능(현 토폴로지 restart 가 inflight 비워 무발현) | **목적지 STATE §3** → 게이트: prune grace 윈도(0051 lease grace 패턴·끄면 현 거동=회귀 0)
