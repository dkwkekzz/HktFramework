...
> 10 step = 1 묶음·십진 경계 정렬(0001–0010 · 0011–0020 · …). 닫힌 step 만 다룬다(STATE NOW 가 가리키는 미닫힘 step 제외). 닫힌 **0080** 까지 → **0001~0080 여덟 묶음 감사 완료**(박스별 진행은 [progress/](progress/) 가 0080 까지 반영).
...
- **#9 | give-resend 견고성 + 멀티프로세스 패리티 갭** | ③/⑤ | 0021-0030 | 🟡 유지(범위 확장) — 0025 §9 ⓕⓖ: ⒜ `_confirmGive` in-order 의존(재정렬 시 seed 1234 itemDesync 2) ⒝ `clientResync`/`resendGives` 인프로세스 `run()` 에만. **+0031-0040: 버스 failover replay 인프로세스만**·**+0051-0060: self-healing arc 인프로세스만**·**+0061-0070: 프레즌스 박스 호 전체 인프로세스 전용**·**+0071-0080: wrouter/mbox/pservice 라우팅·전달 신뢰 호 전체 인프로세스 전용**(`src/host.js` 에 박스 0 → 귓속말/파티/전달 신뢰 멀티프로세스 미검증·주입은 `src/topology.js:174~180` run 루프만) | **목적지 STATE §3** → 게이트: opSeq 키 매칭 + 멀티프로세스 failover/heal/라우팅 핸드셰이크(현 시나리오 FIFO·인프로세스라 미발현=회귀 0)
...
- **#16 | bespoke 가설 모드가 누적 회귀(verify-kit)에 미승격** | ②/⑤/정리 | 0061-0070 | 🔴 **확장**(0071-0080) — verify-kit ORDER 가설 모드는 `reliable·tail·inflight`(~0024)에서 멈춤. 0033+ 버스/lease/heal/프레즌스 + **0071~0080 라우팅/전달 신뢰 호(10 모드)** 의 *ON-경로 의미*는 close 시 1회만 단언·spine 은 reg-0 만 보증 → 라우팅/전달 호가 길어질수록 ON-의미 미보증 빚 누적 | **목적지 STATE §3 OPEN GAPS**(정리성) → 게이트: 안정 호를 verify-kit 승격(또는 wrouter/presence 스모크 1 모드·끄면 현 spine=회귀 0)
- **#17 | self-hb 메아리 + 재타깃 윈도 질의 손실** | ④/⑤ | 0061-0070 | 🟡 **(b) 재타깃 윈도 질의 ✅해소 0074**(whisperRetry·`src/svc-whisper.js:61`) — **잔여 (a) self-hb 메아리**: 0068 승격된 active 가 자기 svc.presence.hb 를 유일 구독자로 되받음(`hbRecv` 부풀음·무해·`src/topo-build.js:151`) | **목적지 STATE §3** → 게이트: active 자기-hb 제외(다중 standby/연쇄 failover 기반·끄면 단일 standby 현 거동=회귀 0)
- **#18 | 전달 신뢰 호가 1:1 귓속말 한정** | 서비스 | 0071-0080 | 🟡 신규 — 0076~0080(영수증·재시도·상한·통지·dedup)은 whisper→mbox 단일 수신자만 배선·파티 1:N(0073) 부분 전달은 멤버별 영수증/dedup 집계 미적용 | **목적지 STATE §3** → 게이트: 멤버별 inflight/seen 키잉(0073 위·끄면 현 1:1 거동=회귀 0)
- **#19 | 파티 멤버십 영속·failover·증분 가입/탈퇴 미착수** | 서비스/데이터 | 0071-0080 | 🟡 신규 — 0075 §9: PartyService 멤버십 인메모리(저널/failover 0)·`partyCreate` 덮어쓰기(증분 join/leave·변경 발행 0) | **목적지 STATE §2/§3** → 게이트: 멤버십 저널+재타깃(가방 0017/프레즌스 0066 패턴의 멤버십 판·끄면 회귀 0)
- **#20 | dedup seen 무계 성장 + 전달 실패 발행 부재** | 서비스/④ | 0071-0080 | 🟡 신규 — 0080 §9: Mailbox `seen` 무한 성장(0042/0047 워터마크의 전달 판 미적용)·0079 §9: deliveryFailed point-to-point 만(실패율 svc 토픽 발행·관측 0) | **목적지 STATE §3** → 게이트: dedup 윈도/워터마크 + 실패 발행(끄면 현 거동=회귀 0)
- **#21 | svc-whisper 헤더 주석 stale** | ②/정리 | 0071-0080 | 🟡 신규(경미) — `src/svc-whisper.js:16` "onTick 없음·순수 반응형"이 0077 onTick(`:94`·제어평면 timeout) 추가 후 모순(코드 정확·주석만 낡음) | **목적지 STATE §3 OPEN GAPS**(정리성) → 게이트: 주석 수정(정리 step·코드 0·reg 0)
