...
> 10 step = 1 묶음·십진 경계 정렬(0001–0010 · 0011–0020 · …). 닫힌 step 만 다룬다(STATE NOW 가 가리키는 미닫힘 step 제외). 닫힌 **0120** 까지 → **0001~0120 열두 묶음 감사 완료**(박스별 진행은 [progress/](progress/) 가 0120 까지 반영).
...
- **#16 | bespoke 가설 모드가 누적 회귀(verify-kit)에 미승격** | ②/⑤/정리 | 0061-0070 | 🔴 **확장**(+0111-0120 10 모드·총 ~50) — verify-kit ORDER 가설 모드는 `reliable·tail·inflight`(~0024)에서 멈춤(`engine/verify-kit.js:702`). 0033+ 버스/lease/heal/프레즌스/라우팅 + 0081~0100 전달/파티/수신함 + **0101~0110 읽음확인/checkout/소비·손실 발행/공지 펜싱/거래소 arc(10 모드)** 의 *ON-경로 의미*는 close 시 1회만 단언·spine 은 reg-0 만 보증 → ON-의미 미보증 빚이 호 길이에 비례 누적 | **목적지 STATE §3 OPEN GAPS**(정리성) → 게이트: 안정 호(수신함·거래소)를 verify-kit 승격(스모크 1~2 모드·끄면 현 spine=회귀 0)
...
- **#30 | 거래소↔가방 미결합·자기 박스 영속** | 서비스/데이터 | 0101-0110 | 🟡 **절반 해소** — **(a) 결합 ✅ 0117~0120**: escrow 를 가방 원장 아바타로 실체화·list/buy/cancel/expire 가 실물 give(seller↔escrow↔buyer)·2-서비스 보존 단언(escrowItemIds≡가방 escrow 소유·`src/svc-exchange.js:52,127`). **(b) 잔여 🟡**: op 저널 여전히 거래소 박스 내(별 PersistStore·N-replica 0) | **목적지 STATE §3** → 게이트: 거래소 저널 PersistStore 박스화(끄면 현 단독 영속=회귀 0)
- **#31 | 거래소↔가방 give 낙관적·결과 미수신(2-서비스 원자성 부재)** | 서비스/버스 | 0111-0120 | 🔴 신규(load-bearing) — `_custody` fire-and-forget(give 결과 item_result 미소비)·escrow 미보유 시 가방 give 실패해도 거래소 종결 성립→회계↔권위 갈라짐(현 escrow 항상 보유라 미발현) | **목적지 STATE §2/§3** → 게이트: give 결과 수신+saga 보상(롤백)·#10 give×result-ahead 와 같은 무대(끄면 낙관적 현 거동=회귀 0)
- **#32 | 시세 피드 reconstruct==라이브 가 전-수명주기-발행 전제** | 서비스 | 0111-0120 | 🟡 신규(경미) — 0116 §9: reconstruct(거래소 저널 expire)는 만료를 셈하나 라이브는 발행된 expired 만 소비→만료발행 OFF+ttl ON 이면 불일치(CQRS 완전성=발행 완전성) | **목적지 STATE §3 OPEN GAPS** → 게이트: 발행 ON 전제 명문화 또는 시세 피드 자기 영속(끄면 발행-ON 정합=회귀 0)
- **#33 | STATE §7 INDEX 30KB 천장 — 매 step 트림 강제** | 정리/도구 | 0111-0120 | 🟡 신규(경미) — 120행째 INDEX 가 step 수에 선형→close-step 30KB 예산이 매 step 옛 행 압축 강제(이 묶음 진행 중 반복 발현) | **목적지 STATE §3 OPEN GAPS**(도구성) → 게이트: INDEX 10행→묶음 1행 압축(전문 reviews/) 또는 예산 상향(가독성만·회귀 무관)
