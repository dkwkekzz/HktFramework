...
> 계층 서사는 *큰 줄기 진행을 직관적으로 푸는 살아있는 문서*다 — **묶음이 그 계층을 의미 있게 전진시킬 때마다 작성/갱신**(닫힘을 기다리지 않는다·6계층은 비선형이라 거의 안 닫힌다·진행 권위는 [../STATE.md](../STATE.md) §5). 6계층은 비선형으로 채워지므로 한 묶음이 여러 계층을 건드릴 수 있다 — 서사는 묶음과 직교한 *계층별* 한 편. 방법·트리거는 `.claude/skills/infra-review/SKILL.md` §3.5.
