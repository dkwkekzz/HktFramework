...
> 10 step = 1 묶음·십진 경계 정렬(0001–0010 · 0011–0020 · …). 닫힌 step 만 다룬다(STATE NOW 가 가리키는 미닫힘 step 제외). 닫힌 **0110** 까지 → **0001~0110 열한 묶음 감사 완료**(박스별 진행은 [progress/](progress/) 가 0110 까지 반영).
...
- **#30 | 거래소↔가방 미결합·자기 박스 영속** | 서비스/데이터 | 0101-0110 | 🟡 신규 — 0107 §9: 거래소 escrow 가 *자기 원장*만 굴림(list/buy 가 가방 아이템을 실제로 빼/넣지 않음 → 진짜 존 넘는 거래 아님)·op 저널이 거래소 박스 내(별 PersistStore·N-replica 0·#9 family) | **목적지 STATE §2/§3** → 게이트: 거래소↔가방 2-서비스 원자 거래 + PersistStore 박스화(끄면 현 단독 보존=회귀 0)
