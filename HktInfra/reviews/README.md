...
> 10 step = 1 묶음·십진 경계 정렬(0001–0010 · 0011–0020 · …). 닫힌 step 만 다룬다(STATE NOW 가 가리키는 미닫힘 step 제외). 닫힌 **0130** 까지 → **0001~0130 열세 묶음 감사 완료**(박스별 진행은 [progress/](progress/) 가 0130 까지 반영).
...
- **#16 | bespoke 가설 모드가 누적 회귀(verify-kit)에 미승격** | ②/⑤/정리 | 0061-0070 | 🔴 **확장**(+0121-0130 10 모드·총 ~60) — verify-kit ORDER 가설 모드는 `reliable·tail·inflight`(~0024)에서 멈춤(`engine/verify-kit.js`). 0033+ 버스/lease/heal/프레즌스/라우팅 + 0081~0110 전달/파티/수신함/거래소 + **0121~0130 거래소↔가방 saga 10 모드** 의 *ON-경로 의미*는 close 시 1회만 단언·spine 은 reg-0 만 보증 → ON-의미 미보증 빚이 호 길이에 비례 누적 | **목적지 STATE §3 OPEN GAPS**(정리성) → 게이트: 안정 호(거래소↔가방 saga·수신함)를 verify-kit 승격(스모크 1~2 모드·끄면 현 spine=회귀 0)
...
- **#33 | STATE §7 INDEX 30KB 천장 — 매 step 트림 강제** | 정리/도구 | 0111-0120 | 🟡 **유지(0121-0130 10/10 발현)** — 130행째 INDEX·close-step 30KB 예산이 *이번 묶음 매 step* 옛 행 압축 강제(10/10 step 실측 반복) | **목적지 STATE §3 OPEN GAPS**(도구성) → 게이트: INDEX 묶음당 1행 압축(전문 reviews/) 또는 예산 상향(가독성만·회귀 무관)
- **#34 | wiring 박스(topo-build/topology) 30KB 초과** | 정리 | 0121-0130 | 🟡 신규(경미·#22 family) — 0124 가 svc-exchange 만 분할(core 12.5/txn 7.0/entry 1.1)·saga 옵션 6개(`src/topo-build.js:36-41`)+inventory opts 1줄이 topo-build **32.8KB**·topology **31.5KB** 로 밀어올림 | **목적지 STATE §3 OPEN GAPS**(정리성) → 게이트: topo-build subs/add 블록 분할 또는 topology 액터 구성 추출(정리 step·기능 0·끄면 현 wiring=회귀 0)
- **#35 | saga 자동 재전송 무차별 + 영구 손실 무한 재시도** | 서비스 | 0121-0130 | 🟡 신규(경미·#15 family) — 0129 §9: autoRetry 가 매 exchSweep 에 *모든* pending 을 나이 무관 재전송(`src/svc-exchange-txn.js:53`·대역 낭비)·회신 영구 손실 시 재시도 상한 없음(0059 recoverMaxRetries 의 saga 판 부재) | **목적지 STATE §3** → 게이트: per-give 마지막-시도-시각 + 재시도 상한→포기 발행(끄면 현 무차별=회귀 0)
