// HktInfra step-0021 — broker(버스 허브). 멀티프로세스 lockstep 배리어를 *토픽 pub/sub 버스* 위에서 구동한다(Node 전용).
...
async function runMulti(opts, deps) {
  ...
  for (let T = 0; T < ticks; T++) {
    ...
    if (chatRestart && placement.has('chat') && net_.tick === chatRestart.at && !chatRestartDone) {   // chat 존재 가드(인프로세스 run() 의 `&& chat` 와 정합)
      ...
      let journal = [];
      if (chatPersistPlaced) { const snap = await cluster.snapshotOne(placeOf('chatpersist'), 'chatpersist'); journal = (snap && snap.journal) || []; }
      ...
      await cluster.rpc(newHost, { cmd: 'replay', addr: 'chat', journal });   // 커맨드 로그 replay → 죽기 전 라우팅+deliveries 재현(snapshot 무관)
