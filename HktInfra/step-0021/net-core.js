// HktInfra step-0021 — 채팅 서비스 영속·failover (ChatService crash→커맨드 로그 replay — 라우팅 테이블+deliveries 의 event sourcing 복구)
// step-0020(읽기 모델 영속·랭킹 reconstruct) 위에 *한 조각*만 더한다 — 0017 가방·0020 랭킹에 이은 *서비스 복구 3부작의 마지막*: 채팅도
//   영속·failover 를 갖춘다(채팅 구독·라우팅이 영속 0 이던 §3 격차). 핵심 차이: 가방(0017)은 *효과*(mint/xfer)를 저널했지만, 채팅의 복구
//   대상은 *구독 라우팅 테이블*(channel→Set<avatar>)이고 say 의 팬아웃은 *그 시점 라우팅*에 의존한다 → 채팅은 *커맨드*(join/say/whisper/leave)를
//   저널하고 replay 가 *리듀서를 재실행*해 라우팅+deliveries 를 재유도(순수 event sourcing). 이 step 의 한 조각 = 그 채팅 복구:
//     · chatpersist 플래그 OFF 면 0020 *그대로* — 0020 와 *비트 동일*(reg 0). 커맨드 로그·crash/replay 휴면.
//     · chatpersist ON: 채팅을 *진짜 kill*(RAM 라우팅 소실) 후 커맨드 로그(chatpersist PersistStore)를 replay — _process 를 seq 순서로
//        재실행하되 replaying 플래그로 *재발신 0*(deliveries/팬아웃 계측만 재현). 복구된 채팅 상태가 무재시작과 *비트 동일*(복구 투명).
//        복구 후 새 say 가 *올바로 팬아웃*(비-구독자 누설 0·지역 격리) = 재구성된 라우팅 테이블이 *작동적으로* 정확.
//     · 대조군: chatpersist OFF + kill → replay 불가 = 구독/배달 소실(서비스도 *어딘가의* 내구 로그가 없으면 못 산다 = 영속이 복원의 *원인*).
//
// 핵심 설계 — chat.replay 는 inventory.replay(0017) 의 *커맨드-소싱 판*: 효과 재적용이 아니라 *커맨드 재실행*(라우팅 의존 팬아웃이라 재유도 필요).
//   live(onMsg)·복구(replay) 가 *단일 리듀서 _process* 를 공유(로직 분기 0). replay 는 재발신 0(inventory.replay 처럼 net.log 비-기여) →
//   crash+replay 는 chatRestart 훅(deliver 직전·제어 평면) → 멀티프로세스 kill→replay 와 비트 동일. chatpersist = PersistStore *재사용*(독립 인스턴스).
//   월드 경로·제어 평면·가방 영속/압축(0017~0018)·버스·발행/구독(0019)·읽기 모델 복구(0020)는 *그대로*(복사 전진). 더한 것은 채팅 crash/replay + 커맨드 로그 + chatRestart 훅뿐.
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: 커맨드 로그·replay 는 *존 tick 밖*(제어 평면·chat onTick 0). 존 net.log·상태가 chatpersist on/off 에 비트 동일·존 도달 0.
//   - 결정론 코어: 복구 라우팅+deliveries = 커맨드 로그(seq 순서)의 함수 → 무재시작과 정확히 일치. 커맨드 replay = §4 "복제=재현"(상태 전송 아님).
//   - 권위 단일 소유: 채팅 라우팅 테이블의 쓰기 권위는 ChatService 하나 — 복구는 권위를 *재현*(이중 쓰기 0). PersistStore 는 관찰 전용 데이터 계층.
//   - 은닉·단일 연결: replay 는 커맨드 로그를 *제어 평면*으로 읽을 뿐(클라 비가시). 복구 choreography 누설 0·복구 후 say 도 누설 0/격리 보존.
//   - headless·원격 검증: node verify.js 한 줄이 채팅을 *진짜 kill→새 호스트 replay* 로 띄워 복구 투명·누설 0 을 4기둥 검증.
//   (가방·랭킹·버스·감사·영속/압축·존/orch/추종자·배리어·kill 생애주기는 *그대로 잇는다* — 복사 전진. 더한 박스 로직은 ChatService.crash/replay 뿐.)
'use strict';
...
class ChatService {
  constructor(opts = {}) {
    this.gateway = opts.gateway || 'gateway';
    this.bus = opts.bus || null;  // 이벤트 버스 주소(null = 0015 직접 라우팅 비트 동일 — 버스 ON 이면 gateway 주소 미사용)
    this.persist = opts.persist || null;  // 채팅 영속 스토어 주소(이 step·null = 0020 비트 동일 — write-behind 커맨드 로그 OFF). 0017 가방 PersistStore 의 채팅 판.
    ...
    this.replaying = false;       // 복구 재실행 중 플래그(이 step) — true 면 _deliver 가 deliveries/계측만 재현하고 *재발신 0*(net.log 비-기여)
  ...
  _process(op) {
    if (op.kind === 'join') {
      ...
      if (e) { for (const ch of e.subs) { const s = this.channels.get(ch); if (s) s.delete(op.avatar); } this.byAvatar.delete(op.avatar); this._journal(op); }   // 실제 탈퇴만 저널
    }
  }
...
function buildTopology(opts) {
  ...
  if (chat) add({ addr: 'chat', kind: 'chat', opts: { gateway: 'gateway', bus: busAddr, persist: chatPersistAddr } });
...
function run(opts) {
  ...
  for (let i = 0; i < ticks; i++) {
    ...
    if (opts.chatRestart && chat && i + 1 === opts.chatRestart.at) {
      chat.crash();
      if (chatpersist) chat.replay(chatpersist.journal);   // 커맨드 로그 replay → 라우팅+deliveries 재현. chatpersist OFF → 소실.
    }
...
function chatPersistDigest(r) {
  ...
  return fnv1a(j + '#w' + (r.chatpersist.writes || 0));
}
// 채팅 커맨드 로그 완전성(이 step) — 영속된 커맨드 누적 수(writes) == 보관된 저널 길이(압축 없음·손실 0) AND 효과 커맨드 전수 기록
//   (join/say/whisper 계측 합 == 비-leave 저널 항목 수). 채팅 죽기 전 기준 — replay 의 *입력*이 채팅 효과를 빠짐없이 담았는지.
function chatJournalComplete(r) {
  if (!r.chatpersist || !r.chat) return true;
  const j = r.chatpersist.journal || [];
  const nonLeave = j.filter(e => e.kind !== 'leave').length;
  return (r.chatpersist.writes || 0) === j.length && nonLeave === (r.chat.joins + r.chat.says + r.chat.whispers);
}
