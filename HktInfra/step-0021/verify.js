// HktInfra step-0021 — 헤드리스 검증 (채팅 서비스 영속·failover — ChatService crash→커맨드 로그 replay·라우팅+deliveries 의 event sourcing 복구)
// 사용: node step-0021/verify.js <mode> [seed]
//   mode: reg | recover-chat | rank | recover-rank | e2e | sacred | recover | compact | degrade | inject | isolate | hide | repro | all
//     reg          — 회귀 0: 인프로세스 모드(chatRestart/chatpersist OFF) → step-0020 와 *비트 동일*(net.log + 상태 + inv/chat/bus/rank).
//                  persist+invRestart 포함. 채팅 onMsg→_process 리팩터·crash/replay 도입이 *비-침습*(chatpersist 미제공=no-op).
//     recover-chat — *이 step 의 가설*: 채팅 진짜 kill→*커맨드 로그* replay 후 ① 복구 라우팅+deliveries == 무재시작(복구 투명·chatDigest 비트 동일)
//                  ② chatpersist OFF+kill 대조군은 구독/배달 *소실* ③ 복구 후 say 누설 0·지역 격리 보존(라우팅 작동 정확) ④ 커맨드 로그 완전.
...
const NET16 = require('../step-0020/net-core.js');   // reg 대조용(직전 step)
...
const CHATP = (seed) => ({ ...PERSIST(seed), chatpersist: true });   // 이 step — 채팅 영속(커맨드 로그) ON·restart 없음(복구 투명 기준)
const CHAT_RESTART = (seed) => ({ ...CHATP(seed), chatRestart: { at: RESTART_AT } });   // 이 step — 채팅 진짜 kill→커맨드 로그 replay(라우팅+deliveries 재유도)
...
// ── reg: 인프로세스 0021(chatRestart/chatpersist OFF) → 0020 비트 동일(채팅 리팩터·crash/replay 도입 비-침습) ──
function reg(seeds) {
  console.log('== reg: 인프로세스 모드(chatpersist OFF) → step-0020 와 비트 동일(net.log + 상태 + inv/chat/bus/rank). 채팅 _process 리팩터·crash/replay = 비-침습 ==');
  console.log('seed   | 구성                | 0020 logHash | 0021(inproc) | log동일 | 상태동일 | inv/chat/bus/rank 동일 | 판정');
...
async function summary(seeds) {
  console.log('== summary: 채팅 서비스 영속·failover — 채팅 진짜 kill→*커맨드 로그* replay 로 라우팅+deliveries 를 리듀서 재실행 복구·복구 투명·E2E 비트 동일 ==');
  for (const seed of seeds) {
    ...
    console.log(`  seed ${pad(seed, 4)}: 프로세스 ${C.pids.length}개 · 채팅 failover ${cr ? cr.oldHost + '→' + cr.newHost + '@' + cr.at : '?'}(커맨드 ${cr ? cr.entries : '?'}개 replay) · 복구투명 ${transparent}(배달 ${b.chat.deliveries.length}) · 누설 ${chatLeak(b)} · log동일 ${logDigest(a) === logDigest(b)} | ${hex(chatDigest(b))}`);
  }
  console.log('채팅 복구원은 *커맨드 로그* — replay 가 리듀서를 재실행해 라우팅+deliveries 재유도(순수 event sourcing) · 멀티프로세스 kill→replay = 인프로세스 crash()+replay() 비트 동일 · 복구 후 누설 0·격리 보존.');
}

// ── CLI ──
const MODES = { reg, rank, e2e, sacred, recover, 'recover-rank': recoverRank, 'recover-chat': recoverChat, compact, degrade, inject, isolate, hide, repro };
(async () => {
  ...
  } else { console.log('mode: reg | rank | e2e | sacred | recover | recover-rank | recover-chat | compact | degrade | inject | isolate | hide | repro | all'); process.exit(2); }
