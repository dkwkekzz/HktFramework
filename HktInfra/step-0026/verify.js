// HktInfra step-0026 — 헤드리스 검증 (in-flight mint 손실 복구 — 클라 id-reconciliation 으로 복구 원장을 belief 로 재수렴·0025 §9 mint 손실 해소)
// 사용: node step-0026/verify.js <mode> [seed]
...
//     reg          — 회귀 0: 인프로세스 모드(mintRecon OFF) → step-0025 와 *비트 동일*(net.log + 상태 + inv/chat/bus/rank).
//                  새 클라 reconcile 코드(pickupLog/sendReconcile) + 가방 item_reconcile 핸들러는 mintRecon OFF 면 *휴면*(메시지 0)임을 직접 증명.
//     inflight     — *이 step 의 가설*: in-flight mint 손실 아래 ① recon OFF = 복구 원장에 mint 항목 누락(itemDesync>0·클라 belief 에 없는 id) ② recon ON = id-reconciliation→서버 re-mint→클라 newId 채택→itemDesync 0·원장 == 무손실 truth
...
const NET16 = require('../step-0025/net-core.js');   // reg 대조용(step-0025)
...
// ── reg: 인프로세스 모드(mintRecon OFF) → step-0025 와 비트 동일(mint id-reconciliation 도입 비-침습) ──
//   마지막 cfg 는 clientResend ON + invRestart 이지만 *mintRecon OFF* — 새 reconcile 코드가 OFF 면 *휴면*(메시지 0)임을 직접 증명.
function reg(seeds) {
  console.log('== reg: 인프로세스 모드(mintRecon OFF) → step-0025 와 비트 동일(net.log + 상태 + inv/chat/bus/rank). mint reconcile = 비-침습 ==');
  console.log('seed   | 구성                | 0025 logHash | 0026(inproc) | log동일 | 상태동일 | inv/chat/bus/rank 동일 | 판정');
  ...
  console.log('  → mintRecon 미제공이면 pickupLog 채움 0·sendReconcile 메시지 0·item_reconcile 가방 도달 0 — 0025 와 비트 동일.');
}
...
async function summary(seeds) {
  console.log('== summary: in-flight mint 손실 복구(id-reconciliation) — write-behind 신뢰성 체인(0023 중간·0024 tail·0025 give·0026 mint) 무손실 복구 + 멀티프로세스 E2E 비트 동일(휴면 경로) ==');
  for (const seed of seeds) {
  ...
  console.log('저널 홉 전송 손실(0023 중간·0024 tail)·in-flight give(0025)·in-flight mint(0026 id-reconciliation) 모두 닫힘 — write-behind 신뢰성 사각지대 완결. 남은 §9 = PersistStore 자체 단일점(영속 박스 죽음·이중쓰기/N-replica 후속).');
}
