// HktInfra step-0012 — 버스 분산 + 실 네트워크 열화 내성 (broker 단일점 → 토픽 pub/sub 버스 · 소켓 층 분단·재연결)
// step-0011(실 TCP 소켓 와이어) 위에 *한 조각*만 더한다:
//   0011 은 와이어를 *실 TCP 소켓*(spawn·IPC 0·길이-프리픽스 프레이밍)으로 현실화했다 — 그러나 ⒜ broker 가 여전히
//   *단일 코디네이터*로 각 호스트 소켓을 직접 주소지정(socks.get(host).write)했고 ⒝ 소켓이 localhost·TCP *신뢰 전송*
//   이라 실 네트워크 *분단·재연결* 내성은 미검증이었다(0011 §8.2). 이 step 은 그 *와이어/버스 층*만 친다(액터 무수정):
//     · 인프로세스 모드(run): engine/Net 그대로 — 0011 와 *비트 동일*(reg 0). 버스/열화 도입은 비-침습.
//     · 멀티프로세스 모드(runMulti): broker↔host 소켓을 *토픽 pub/sub 버스*로 재배선(cluster.js) —
//       broker 는 호스트 소켓을 직접 모르고 *토픽*(cmd.<host>/res)으로 발행/구독한다(N×N 주소결합 제거·
//       발행자 무수정으로 소비자 추가=tap). 그 버스 링크에 *열화*(시드 드롭·분단 윈도·재연결)를 주입한다.
//
// 핵심 설계 — 버스·열화만 바뀌고 *lockstep 배리어·액터·프로토콜은 0011 그대로*:
//   broker 는 여전히 engine/Net step() 의미를 비동기 배리어로 재현(발신 전역순서 보존 → net.log 비트 동일).
//   그 위에 두 조각:
//     ① 버스 분산 — 직접 소켓 주소지정 → 토픽 발행/구독. 같은 토픽에 소비자(tap)를 *발행자 무수정*으로 더할 수 있다.
//     ② 열화 내성 — 링크 층에 (a) *시드 프레임 드롭* + seq/ack/resend(reqId 멱등) → 전송 무손실처럼 비트 동일,
//        (b) *결정론 분단 윈도*(권위 호스트 링크를 t0 부터 차단) = 0009 의 추상 '사망'을 *소켓 분단*으로 현실화 →
//        orch lease 가 침묵을 감지·failover 승격 → desync 0, (c) *재연결+펜싱*(분단 호스트 복귀 시 와이어 펜스로
//        출력 차단 — 액터는 살아 있어도 broker 가 수용 0 → split-brain 없음). 분단=죽음 이라 인프로세스 deathTick 과 비트 동일.
//   결정론은 전송 아닌 *순서*의 함수(0011 의 발견) — 드롭·분단·재연결을 *시드/윈도*로 결정론화하면 재현 가능.
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: 액터 로직 무변경(0011 그대로). 버스·열화는 전송 substrate 층 — tick 안에 I/O·인증 0.
//   - 결정론 코어: 액터는 시드 의사난수만(Math.random 0). 버스·드롭·분단이 와도 같은 입력열 → 같은 상태(E2E 동치).
//   - 권위 단일 소유: 분단→failover→재연결+펜싱 전 구간에서 소유자=1·이중쓰기 0(split-brain 펜싱)이 보존.
//   - 은닉·단일 연결: 클라는 게이트웨이만. 버스 토픽·열화는 *서버간* 경계 — 클라엔 비가시(누설 0).
//   - headless·원격 검증: node verify.js 한 줄이 전 프로세스를 버스로 묶어 열화 아래 같은 4기둥 검증(제1 운영 제약).
'use strict';
