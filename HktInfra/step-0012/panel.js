// HktInfra step-0012 — 시각 관찰 셸 (프로세스 토폴로지 + 토픽 버스 트래픽) + 헤드리스 ASCII(Node)
// 관찰하는 것: *전 서버가 별 프로세스가 되어 토픽 pub/sub 버스로 통신*하는 그림을 *눈으로*. 0011 의 소켓 와이어 위에서 —
//   host 태그 경계를 프로세스+버스 경계로 본다: 각 서버 박스(login·gateway·zone·orch·추종자…)를 노드로, 그 사이를 흐르는
//   메시지를 *프로세스 경계를 넘는 버스 토픽 프레임* 으로 본다(공유 메모리 0). 사망/분단/승격 시점의 제어 평면 트래픽(lease·promote·keyframe)을
