// HktInfra step-0013 — 진짜 프로세스 kill 아래 failover (분단 윈도 → 실 child.kill · 타임아웃/소켓 추측 감지 · 재-provisioning · 거짓 사망 epoch 펜싱)
// step-0012(토픽 pub/sub 버스 + 소켓 층 드롭·분단·재연결 펜싱) 위에 *한 조각*만 더한다:
//   0012 의 분단은 *논리 윈도*(broker 가 침묵 시점을 *안다*)였고 분단 호스트는 *살아 있었다*(펜싱 대상). 이 step 은
//   그 분단을 *진짜 kill* 로 내린다(액터 무수정 — *cluster/host 의 와이어 생애주기*만):
//     · 인프로세스 모드(run): engine/Net 그대로 — 0012 와 *비트 동일*(reg 0). kill 생애주기 도입은 비-침습.
//     · 멀티프로세스 모드(runMulti): cluster.js 에 위임 — broker 가
//        ① 실제 `child.kill('SIGKILL')`(프로세스 소멸·소켓 RST)을 결정론 tick 에 주입하고 *소켓 close 신호*로 침묵을
//           *감지*(윈도 아님) → deliver/tick 스킵 = 0009 추상 사망/0012 분단과 비트 동일(죽은 프로세스는 안 돌아옴),
//        ② 소켓이 *살아 있는* 침묵은 침묵 tick 을 세어 leaseTimeout 초과 시 *presumedDead* 로 *추측*(타임아웃 가드),
//        ③ 거짓 사망(일시 침묵 후 복귀)은 stale epoch 호스트의 발신을 전량 드롭(epoch 펜싱) → split-brain 0,
//        ④ kill→승격으로 N=1 이 되면 *새 프로세스를 spawn*·스냅샷 상태 동기·권위 입력 미러로 핫 standby 복원(N≥2).
//
// 핵심 설계 — kill 생애주기만 바뀌고 *lockstep 배리어·액터·프로토콜은 0012 그대로*:
//   broker 는 여전히 engine/Net step() 의미를 비동기 배리어로 재현(발신 전역순서 보존 → net.log 비트 동일).
//   진짜 kill 의 침묵은 *socketDead 멤버십*(소켓 close 가 세움)으로 스킵 — 0012 의 *윈도 비교*(T>at)를 대체하나
//   같은 침묵 패턴(deliver T>at·tick T>=at)을 만든다(kill 을 deliver 후·tick 전에 주입). 결정론은 전송 아닌
//   *순서*의 함수(0011 의 발견) — socketDead 는 kill 호출 직후 세워지고 배리어가 exit 를 기다려 *논리 tick 공간 결정론*.
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: 액터 로직 무변경(0012 그대로). kill 생애주기는 전송 substrate 층 — tick 안에 I/O·인증 0.
//   - 결정론 코어: 액터는 시드 의사난수만(Math.random 0). 진짜 kill·재-provisioning·epoch 펜싱이 와도 같은 입력열 → 같은 상태.
//   - 권위 단일 소유: 진짜 kill→추측 감지→승격→재-provisioning→거짓 사망 epoch 펜싱 전 구간 소유자=1·이중쓰기 0.
//   - 은닉·단일 연결: 클라는 게이트웨이만. kill·epoch·재-provisioning 은 *서버간* 경계 — 클라엔 비가시(누설 0).
//   - headless·원격 검증: node verify.js 한 줄이 전 프로세스를 버스로 묶어 진짜 kill 아래 같은 4기둥 검증(제1 운영 제약).
'use strict';
