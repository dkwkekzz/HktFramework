// HktInfra step-0013 — broker(버스 허브). 멀티프로세스 lockstep 배리어를 *토픽 pub/sub 버스* 위에서 구동한다(Node 전용).
//   0012 대비 *진짜 프로세스 kill 생애주기만* 더한다(버스·배리어·발신 순서·드롭/분단/펜싱은 0012 그대로):
...
function reconstruct(net_, topo, snaps, placement, clusterInfo, opts) {
  ...
  return {
    net: net_, zones: zoneObjs, followers, reprovZones, allZones, zoneAddrs: topo.zoneAddrs, clients: clis,
