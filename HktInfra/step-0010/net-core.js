// HktInfra step-0010 — 프로세스 경계 현실화: 인메모리 큐 → 실 프로세스/IPC (transport seam 현실화)
// step-0009(추종자 승격 failover) 위에 *한 조각*만 더한다:
//   0001~0009 는 전 "서버"(로그인·게이트웨이·존·추종자·orch·레지스트리)를 *단일 Node 프로세스 내 액터*로 두고
//   engine/Net 인메모리 큐로 통신시켜 netcode 프로토콜·토폴로지를 헤드리스로 증명했다(TOOLS §6 'host 태그 경계').
//   이 step 은 그 host 태그 경계를 *실 전송*(child_process IPC = 직렬화 메시지 경계)으로 갈아끼운다:
//     · 인프로세스 모드(run): engine/Net 그대로 — 0009 와 *비트 동일*(reg 0). 전송 seam 교체는 비-침습.
//     · 멀티프로세스 모드(runMulti): 각 서버 박스가 *별도 OS 프로세스*(child_process.fork)에 살고,
//       broker(이 net-core 가 소유한 engine/Net = 전송 substrate)가 *직렬화 IPC 메시지*로만 onMsg/onTick 을
//       원격 디스패치하고 발신을 회수한다. 공유 메모리 0 — 프로세스 간엔 JSON 직렬화 메시지만 오간다.
//
// 핵심 설계 — *결정론 보존하는 lockstep 배리어*:
...
//   배리어가 전역 발신 순서를 *정확히* 보존하므로 broker.net.log 가 인프로세스와 *비트 동일* → E2E 동치 증명이 강력.
//   (logDigest 는 from>to:payload 순서의 함수 — seq/tick 무관 — 이라 순서 보존만으로 충분.)
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: 액터 로직 무변경(0009 그대로). broker 는 전송 substrate 일 뿐 — tick 안에 I/O·인증 0.
//   - 결정론 코어: 액터는 시드 의사난수만(Math.random 0). 프로세스가 갈려도 같은 입력열 → 같은 상태(E2E 동치).
//   - 권위 단일 소유: 0009 불변(소유자=1·이중쓰기 0·failover bounded gap→회복)이 프로세스 경계 너머로도 보존.
//   - 은닉·단일 연결: 클라는 게이트웨이만. 프로세스 분리는 *서버간* 경계 — 클라엔 비가시(누설 0).
//   - headless·원격 검증: node verify.js 한 줄이 전 프로세스를 띄워 같은 4기둥 검증(제1 운영 제약의 본진).
'use strict';
...
// ════════════════════════════════════════════════════════════════════════
//  runMulti — 멀티프로세스 모드(child_process IPC). cluster.js 에 위임(Node 한정 — 브라우저 미로드).
//   같은 buildTopology 로 토폴로지를 짜고, 각 서버 박스를 별 프로세스에 띄워 lockstep 배리어로 구동.
//   반환 r 은 run() 과 같은 digest 함수들이 그대로 먹는 형태(zones/clients/net.log) + r.cluster(pids/placement).
// ════════════════════════════════════════════════════════════════════════
