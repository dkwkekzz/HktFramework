// HktInfra step-0023 — 헤드리스 검증 (저널 홉 신뢰 전달 — 가방 write-behind 저널 홉의 갭 감지 NAK + 재전송·0008 의 저널 홉 판)
// 사용: node step-0023/verify.js <mode> [seed]
//   mode: reg | reliable | chat-compact | recover-chat | rank | recover-rank | e2e | sacred | recover | compact | degrade | inject | isolate | hide | repro | all
//     reg          — 회귀 0: 인프로세스 모드(journalReliable OFF/무손실) → step-0022 와 *비트 동일*(net.log + 상태 + inv/chat/bus/rank).
//                  마지막 cfg 는 journalReliable ON 인데 *손실 없음* — 새 신뢰 코드(sentBuffer·NAK·dedup)가 무손실에선 *휴면*(NAK 0)임을 직접 증명.
//     reliable     — *이 step 의 가설*: 저널 홉 손실 아래 ① 신뢰 ON = persist 저널 완전(writes==무손실 기준)·복구 원장 == 무손실 기준(invDigest 동일)
...
const NET16 = require('../step-0022/net-core.js');   // reg 대조용(직전 step)
...
async function summary(seeds) {
  console.log('== summary: 저널 홉 신뢰 전달 — 손실 아래 NAK+재전송으로 저널 완전화→복구 무손실 + 멀티프로세스 E2E 비트 동일(무손실 경로) ==');
  for (const seed of seeds) {
  ...
  console.log('저널 홉이 전송-신뢰화(persist 갭 감지 NAK → inventory sentBuffer 재전송 → recvSeqs dedup) — 손실 아래 영속 완전·복구 무손실. write-behind 신뢰성의 전송-손실 절반 해소(in-flight 손실 절반·tail 손실은 §9).');
}

// ── CLI ──
const MODES = { reg, rank, e2e, sacred, recover, 'recover-rank': recoverRank, 'recover-chat': recoverChat, compact, 'chat-compact': chatCompact, reliable, degrade, inject, isolate, hide, repro };
(async () => {
  ...
  } else { console.log('mode: reg | rank | e2e | sacred | recover | recover-rank | recover-chat | compact | chat-compact | reliable | degrade | inject | isolate | hide | repro | all'); process.exit(2); }
