// HktInfra step-0023 — 저널 홉 신뢰 전달 (가방 write-behind 저널 홉의 갭 감지 NAK + 재전송 — 0008 ack/NAK 의 저널 홉 판)
// step-0022(채팅 커맨드 로그 압축) 위에 *한 조각*만 더한다 — STATE §2 ⒜(활성 중 failover + write-behind 손실 재수렴)의 *sanctioned sub-piece*:
//   "저널 홉 ack/resend 만". 0017~0022 의 복구는 모두 저널이 *온전히 영속됐다고 가정*했다. 그러나 inventory→persist 저널 홉이 전송 손실되면
//   저널에 *갭*(빠진 효과)이 생겨 복구가 *불완전*해진다(write-behind 신뢰성의 절반 = 가장 큰 구멍). 이 step 은 그 홉을 *전송-신뢰화*한다:
//     · journalReliable 플래그 OFF 면 0022 *그대로* — fire-and-forget·비트 동일(reg 0). sentBuffer/NAK/dedup 휴면.
//     · ON: inventory 가 보낸 저널을 sentBuffer 에 보관(미-ack). PersistStore 가 seq 갭을 감지하면 발신자에 NAK(미수신 seq 요청) →
//        inventory 가 sentBuffer 에서 *재전송*. PersistStore 는 recvSeqs 로 멱등 수신(재전송 중복 push 0) → at-least-once 전송 위 effectively-once 영속.
//     · 무손실: 저널 홉 손실 아래에서도 journalReliable 이면 persist 저널 *완전*(writes==mint+xfer) → 복구 원장 == 무손실 기준(invDigest 동일).
//        대조군(OFF): 같은 손실이 저널 갭 → 복구 원장 *손실*(writes<변이·ledger.size 급감).
//
// 핵심 설계 — 0008(핸드오프 ack/재전송·keyframe/NAK) 패턴의 *저널 홉 판*: 갭 감지는 *수신측(persist)* 이 한다(누가 안 왔는지는 받는 쪽이 안다) →
//   순수 반응형(onMsg·onTick 0 유지·신성한 tick). NAK 는 매 수신 재발행 → NAK 손실에도 결국 수렴. dedup(recvSeqs) 으로 재전송 멱등.
//   *손실은 전송층*(routeFilters.persist + redundancy 1 + loss) — 0008 처럼 손실을 주입해 신뢰 메커니즘을 자극. 압축(0018)과는 직교 — 신뢰 시나리오는 snapshot OFF(§9 상호작용 회피).
//   월드 경로·제어 평면·채팅 영속/압축(0021~0022)·버스·랭킹(0019~0020)은 *그대로*(복사 전진). 더한 것은 inventory.sentBuffer/_resend + persist 갭감지/NAK/dedup 뿐·host.js/`engine/` 무수정.
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: NAK·재전송·dedup 은 *존 tick 밖*(가방·persist onTick 0·제어 평면). 존 net.log·상태가 journalReliable on/off 에 비트 동일·존 도달 0.
//   - 결정론 코어: 복구 원장 = persist 저널(seq 순서·dedup)의 함수 → 손실 아래에서도 무손실 기준과 정확히 일치. = §4 "복제=재현"(상태 전송 아님).
//   - 권위 단일 소유: 원장의 쓰기 권위는 InventoryService 하나 — 신뢰 전달은 *영속의 완전성*을 보장할 뿐 라이브 원장 비-침습(invDigest 불변).
//   - 은닉·단일 연결: NAK/재전송은 inventory↔persist *서버간 제어 평면*(클라 비가시). 신뢰 안무 누설 0.
//   - headless·원격 검증: node verify.js 한 줄이 저널 홉에 *손실을 주입*해 신뢰 ON=저널 완전·복구 무손실 / OFF=저널 갭·복구 손실을 4기둥 검증.
...
class InventoryService {
  constructor(opts = {}) {
    ...
    this.reliable = opts.reliable || false;  // 저널 홉 신뢰 전달(이 step) — ON 이면 보낸 저널을 sentBuffer 에 보관하고 persist NAK 에 재전송(0008 ack/NAK 의 저널 홉 판). OFF = 0022 fire-and-forget 비트 동일.
    ...
    this.sentBuffer = new Map();  // seq -> 보낸 저널 항목(이 step·reliable 일 때만 채움) — persist NAK 시 재전송 소스(미-ack 보존). 압축/bound 는 후속.
    this.resends = 0;             // NAK 에 응답해 재전송한 저널 항목 누적(이 step·계측)
  ...
  _resend(missing) {
    for (const seq of missing) {
      ...
      if (e) { this.net.send(this.addr, this.persist, { type: 'journal', entry: e }); this.resends++; }
    }
  }
  ...
  crash() {
    ...
    this.sentBuffer = new Map(); this.resends = 0;   // 신뢰 전달(이 step) — 새 프로세스는 미-ack 버퍼 0(죽기 전 in-flight 는 소실 = §9 write-behind 윈도 잔존)
    this.minted = 0; this.transfers = 0; this.failedOps = 0;
  }
...
class PersistStore {
  constructor(opts = {}) {
    ...
    this.recvSeqs = new Set();    // 수신한 저널 seq 집합(이 step·reliable) — dedup(재전송 중복 0) + 갭 감지(미수신 seq 색출)
    this.maxRecvSeq = -1;         // 수신한 최대 seq(갭 범위의 상한 — [0..maxRecvSeq] 중 미수신 = 손실 후보)
    this.naks = 0;                // 갭 감지로 보낸 NAK 누적(이 step·계측)
  }
  onMsg(m) {
    const p = m.payload;
    if (p.type === 'journal') {
      ...
      // [0..maxRecvSeq] 중 미수신 seq 색출 → 있으면 발신자(m.from)에 NAK(재전송 요청). 매 수신 재-NAK = NAK 손실에도 결국 수렴(tail 손실은 미감지 — §9).
      const missing = [];
      for (let s = 0; s <= this.maxRecvSeq; s++) if (!this.recvSeqs.has(s)) missing.push(s);
      if (missing.length > 0) { this.net.send(this.addr, m.from, { type: 'journal_nak', missing }); this.naks++; }
      return;
    }
...
function buildTopology(opts) {
  const {
    ...
    persist = false, snapshot = 0, journalReliable = false,
    ranking = false,
  ...
  if (inventory) add({ addr: 'inventory', kind: 'inventory', opts: { gateway: 'gateway', bus: busAddr, persist: persistAddr, snapshot: persistAddr ? snapshot : 0, reliable: persistAddr ? journalReliable : false } });
