// HktInfra step-0015 — broker(버스 허브). 멀티프로세스 lockstep 배리어를 *토픽 pub/sub 버스* 위에서 구동한다(Node 전용).
//   0014 대비 *채팅 서비스(chat) 재구성만* 더한다(가방·버스·배리어·kill 생애주기·재-provisioning 은 0014 그대로):
...
function reconstruct(net_, topo, snaps, placement, clusterInfo, opts) {
  ...
  return {
    net: net_, zones: zoneObjs, followers, reprovZones, allZones, zoneAddrs: topo.zoneAddrs, clients: clis, inventory, chat,
