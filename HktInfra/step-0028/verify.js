// HktInfra step-0028 — 헤드리스 검증 (PersistStore N-replica + quorum — 복제 N개·생존 union 복구로 영속 단일점 일반 해소)
// 사용: node step-0028/verify.js <mode> [seed]
//   mode: reg | quorum | inflight | tail | reliable | chat-compact | recover-chat | rank | recover-rank | e2e | sacred | recover | compact | degrade | inject | isolate | hide | repro | all
//     reg          — 회귀 0: 인프로세스 모드(persistReplicas 0) → step-0027 와 *비트 동일*(net.log + 상태 + inv/chat/bus/rank).
//                  복제 스폰·_journal fan-out·quorum 복구 분기는 persistReplicas 0 이면 *휴면*(스폰 0·추가 발신 0·0027 복구 경로)임을 직접 증명.
//     quorum       — *이 step 의 가설*: R=3 복제 fan-out + 생존 복제 저널 union 복구. ⒜ 완전복제·primary+복제2 죽임(생존1·full)=무손실(0027 은 backup 1 죽음에 단일점)
//                  ⒝ 부분쓰기(각 복제 서로 다른 seq 부분집합 손실)+primary 죽임=생존 복제 전부 부분이나 union 완전 → 무손실(quorum read 가 단일 복제보다 강함)
//                  ⒞ 부분쓰기+생존 복제 1 = 일부 seq 가 어느 생존본에도 없어 손실(merge 도 정족수 경계). ④ 복구 후 itemDesync 0·보존/정합·persistReplicas 0 면 reg 0
...
const NET16 = require('../step-0027/net-core.js');   // reg 대조용(step-0026)
...
const CHAT_SNAP_N = 5;   // 채팅 커맨드 로그 스냅샷 압축 주기(0022) — 커맨드 5항목마다 *라우팅 스냅샷* → upToSeq 이하 폐기
const JLOSS = 0.3;       // 저널 홉 손실율(이 step) — inventory→persist 홉(redundancy 1)에 주입해 신뢰 NAK/재전송 자극(손실 seed 는 시드별 결정론)
let FAILED = false;
...
// N-replica + quorum(이 step) — 복제 N개 fan-out·생존 복제 union 복구. R=3 복제(persist2..persist4).
const QR = 3;   // 복제 수(primary 포함 4 내구 사본)
// 부분 쓰기 손실: 각 복제가 서로 다른 seq 부분집합을 100% 떨군다(persistK 는 seq%R==(K-2)). primary('persist')는 손실 0.
//   → 개별 복제는 *불완전*, 생존 ≥2 의 union 은 *완전*(어떤 seq 든 1개 복제만 떨굼). quorum read 가 단일 복제보다 강함을 강제.
...
// ⒜ 완전 복제 — 손실 0·primary+복제2 죽임(생존 복제 1, full journal) → 무손실(0027 은 backup 1 죽음에 단일점).
const Q_FULL = (seed) => ({ ...RESTART_NOSNAP(seed), persistReplicas: QR, persistRestart: { at: PERSIST_CRASH_AT }, replicaKills: ['persist2', 'persist3'] });
// ⒝ 부분 쓰기 + quorum merge — 각 복제 부분 저널·primary 만 죽임(생존 복제 3, 전부 부분) → union 완전 → 무손실.
const Q_MERGE = (seed) => ({ ...RESTART_NOSNAP(seed), persistReplicas: QR, transport: XREPLICALOSS(seed, QR), persistRestart: { at: PERSIST_CRASH_AT } });
// ⒞ 정족수 미달(대조) — 부분 쓰기 + primary+복제2 죽임(생존 복제 1, 부분) → union=부분 → 손실(merge 도 정족수 필요).
const Q_UNDER = (seed) => ({ ...RESTART_NOSNAP(seed), persistReplicas: QR, transport: XREPLICALOSS(seed, QR), persistRestart: { at: PERSIST_CRASH_AT }, replicaKills: ['persist3', 'persist4'] });
...
// ── quorum: 이 step 의 가설 — N-replica fan-out + 생존 복제 union 복구(2복제 단일점·부분쓰기 정족수) ──
function quorum(seeds) {
  console.log('== quorum: *가설* — ⒜ 완전복제 R=3·primary+복제2 죽임(생존1·full)=무손실(0027 은 backup 1 죽음에 단일점) ⒝ 부분쓰기+primary 죽임(생존복제 전부 부분)=union 완전=무손실 ⒞ 부분쓰기+생존복제1=손실(merge 도 정족수 필요) ==');
  console.log('seed   | base       | ⒜full복구  | ⒝merge복구 | ⒞under복구 | 복제부분 min/writes | ⒜==base | ⒝==base | ⒞손실 | desync | 보존정합 | 판정');
  for (const seed of seeds) {
    const base = run(RESTART_NOSNAP(seed));   // 무손실 기준(primary crash 없음·전체 저널 replay)
    const full = run(Q_FULL(seed));           // 완전복제·3 죽음(생존 복제 1, full)
    const merge = run(Q_MERGE(seed));         // 부분쓰기·primary 만 죽임(생존 복제 3, 전부 부분 → union 완전)
    const under = run(Q_UNDER(seed));         // 부분쓰기·primary+복제2 죽임(생존 복제 1, 부분 → 정족수 미달)
    const bd = invDigest(base);
    const fullOK = invDigest(full) === bd;        // ⒜ 3 죽음에도 생존 1(full) → 무손실 (0027 의 §9 단일점 해소)
    const mergeOK = invDigest(merge) === bd;      // ⒝ 부분 복제 union → 무손실 (quorum read)
    const underLossy = invDigest(under) !== bd;   // ⒞ 생존 복제 1(부분) → 손실 (정족수 경계 대조)
    // ⒝ 각 복제가 *부분* 저널만 가짐(단일 복제론 불완전 — merge 가 메움). 최소 보유 vs 무손실 writes
    const writes = base.persist ? base.persist.writes : 0;
    const repLens = (merge.replicaStores || []).map(s => s.journal.length);
    const minRep = repLens.length ? Math.min(...repLens) : 0;
    const partial = repLens.length === QR && repLens.every(l => l > 0 && l < writes);   // 전부 부분(0<l<writes)
    const desync0 = itemDesync(full) === 0 && itemDesync(merge) === 0;
    const cons = itemConserved(full) && ledgerConsistent(full) && itemConserved(merge) && ledgerConsistent(merge);
    const ok =
      check(fullOK, `seed ${seed}: ⒜ 완전복제 3 죽음 복구 != base(생존 복제 full 복구 실패)`) &&
      check(mergeOK, `seed ${seed}: ⒝ 부분쓰기 union 복구 != base(quorum merge 실패)`) &&
      check(partial, `seed ${seed}: ⒝ 복제가 부분 저널이 아님(min ${minRep}/writes ${writes}·repLens ${repLens}) — XREPLICALOSS 미작동`) &&
      check(underLossy, `seed ${seed}: ⒞ 생존 복제 1(부분)인데 손실 안 됨(정족수 대조 실패)`) &&
      check(desync0, `seed ${seed}: 복구 후 itemDesync != 0(full ${itemDesync(full)}·merge ${itemDesync(merge)})`) &&
      check(cons, `seed ${seed}: 복구 후 원장 보존/정합 깨짐`);
    console.log(`${pad(seed, 6)} | ${hex(bd)} | ${hex(invDigest(full))} | ${hex(invDigest(merge))} | ${hex(invDigest(under))} | ${pad(minRep + '/' + writes, 19)} | ${(fullOK ? '예' : '아니오').padEnd(7)} | ${(mergeOK ? '예' : '아니오').padEnd(7)} | ${(underLossy ? '예' : '아니오').padEnd(5)} | ${pad(itemDesync(merge), 6)} | ${(cons ? '예' : '아니오').padEnd(8)} | ${ok ? 'OK' : 'FAIL'}`);
  }
  console.log('  → N-replica: _journal 이 primary+복제 N 에 fan-out → 복구 = 생존 복제 저널 union(quorum read). 2복제(0027)는 backup 죽으면 단일점 — N 복제는 (primary 포함) 최대 N 죽어도 무손실.');
  console.log('    quorum 핵심: 각 복제가 부분 저널만 가져도(per-link 손실) union 이 메운다. 생존이 정족수 미만이면(⒞) 일부 seq 가 어느 생존본에도 없어 손실 — merge 는 단일 복제보다 강하되 정족수 경계가 있다. persistReplicas 0 면 reg 0(0027 비트 동일).');
}
...
async function summary(seeds) {
  console.log('== summary: PersistStore N-replica + quorum — write-behind 신뢰성 체인(0023~0026)·이중쓰기(0027) 위에 N복제 fan-out + 생존 복제 union 복구(영속 단일점 일반 해소) + 멀티프로세스 E2E 비트 동일(휴면 경로) ==');
  for (const seed of seeds) {
  ...
  console.log('write-behind 신뢰성(0023~0026)·이중쓰기 backup(0027) 위에 N-replica fan-out + quorum-merge 복구(0028) — primary 포함 최대 N 죽어도·부분쓰기에도 생존 복제 union 으로 무손실. 남은 §9 = Quorum *쓰기* ack(W 정족수 확인 후 durable)·디스크 fsync·버스 영속·월드 영속 후속.');
}

// ── CLI ──
const MODES = { reg, quorum, rank, e2e, sacred, recover, 'recover-rank': recoverRank, 'recover-chat': recoverChat, compact, 'chat-compact': chatCompact, reliable, tail, inflight, degrade, inject, isolate, hide, repro };
(async () => {
  ...
  if (MODES[mode]) await MODES[mode](seedArg);
    ...
    reliable(seedArg); console.log('');
    tail(seedArg); console.log('');
    inflight(seedArg); console.log('');
  ...
  } else { console.log('mode: reg | quorum | rank | e2e | sacred | recover | recover-rank | recover-chat | compact | chat-compact | reliable | tail | inflight | degrade | inject | isolate | hide | repro | all'); process.exit(2); }
