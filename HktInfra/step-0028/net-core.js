// HktInfra step-0028 — PersistStore N-replica + quorum: 복제 저장소 N개로 단일점 제거·생존 복제 union 복구
// step-0027(PersistStore failover·이중쓰기 단일 backup) 위에 *한 조각*만 더한다 — STATE §2 ⒜: "N-replica 이중쓰기 동기화(3+ 복제·Quorum 쓰기)".
//   0027 이 backup 하나로 primary 단일점을 없앴지만 §9 한계가 남았다: *2복제(primary+backup) — backup 도 죽으면 다시 단일점*.
//   이 step 은 복제를 N개로 일반화하고, 복구를 *생존 복제들의 저널 union(quorum read)* 으로 바꿔 그 한계를 닫는다.
//   · persistReplicas:N → InventoryService._journal 이 primary('persist') + 복제 N개('persist2'..'persistN+1')에 *동시* fan-out(N+1 내구 사본).
//   · invRestart 복구 = quorumMergeJournals([primary,...replicas]) — 생존 스토어들의 저널을 seq union(dedup)·replay. primary 포함 최대 N개 죽어도 무손실.
//   · *quorum read 가 단일 복제보다 강한 이유*: 전송 손실로 각 복제가 서로 다른 *부분* 저널만 가져도, 어떤 seq 든 ≥1 생존 복제에 있으면 union 이 메운다.
//   · persistReplicas:0 이면 0027 *그대로* — 복제 스폰 0·fan-out 0·복구 경로 0027 분기·비트 동일(reg 0).
//
// 핵심 설계 — *N-replica + quorum-merge = 영속 단일점의 일반 해소*:
//   PersistStore 는 *데이터 계층*(세션보다 오래). 0027 이중쓰기(2복제)는 backup 도 죽으면 단일점 재발.
//   N개 복제로 fan-out 하고, 복구는 한 스토어를 *고르는* 게 아니라 *생존 전부를 union* 한다(quorum read).
//   각 복제가 부분 저널만 가져도(per-link 손실) union 이 완전 저널을 재구성 → 무손실 복구의 정족수 보장.
//   가방/채팅/클라/게이트웨이/버스/랭킹/0023~0027 전 코드 *그대로*(복사 전진). 더한 것: _journal fan-out 루프 + buildTopology 복제 N스폰 + run quorumMergeJournals 복구 + replicaKills 주입뿐.
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: journal fan-out 은 *비동기 서비스 메시지*(존 tick 밖). 존 net.log·상태가 persistReplicas 0/N 에 비트 동일·존 도달 0.
//   - 결정론 코어: 복구 원장 = 생존 복제 union replay → 무손실 truth 와 비트 동일(invDigest). = §4 "복제=재현".
//   - 권위 단일 소유: 복제 전부 *수신만*(읽기전용 data store). 원장 권위는 InventoryService 단일. fan-out 은 복제·쓰기 권위 이동이 아님.
//   - 은닉·단일 연결: 'persist2'..'persistN+1' 은 서버간 내부 주소. 클라는 gateway 만 알고 복제 존재를 모름(은닉 유지).
//   - headless·원격 검증: node verify.js quorum 한 줄이 primary+복제 일부 crash 주입 → 생존 복제 union=invDigest 동일 / 정족수 미달=소실 을 4기둥 검증.
'use strict';
...
class InventoryService {
  constructor(opts = {}) {
    ...
    this.replicas = opts.replicas || [];  // N-replica 영속 스토어 주소 목록(이 step·persistReplicas) — _journal 이 primary + 이 목록 전부에 fan-out. [] = 0027 비트 동일(N-replica 휴면). persistBackup 과 상호배타(토폴로지가 둘 중 하나만 와이어).
  ...
  _journal(entry) {
    ...
    // N-replica fan-out(이 step) — replicas 목록 전부에 동일 항목 발신(각각 독립 저장소·fire-and-forget). primary + N 복제 = N+1 내구 사본.
    //   2복제(0027)는 backup 도 죽으면 다시 단일점 — N-replica 는 복구가 *생존 복제들의 저널 union(quorum read)* 이라 (primary 포함) 최대 N개 죽어도 무손실.
    //   [] 면 발신 0(reg 0 — 0027 비트 동일). 전송 손실로 각 복제가 *부분* 저널만 가져도 union 이 메운다(merge 가 단일 복제보다 강함).
    for (const r of this.replicas) this.net.send(this.addr, r, { type: 'journal', entry: full });
...
function buildTopology(opts) {
  ...
  //   replicas (이 step) — persistReplicas≥1 이면 fan-out 대상 목록. [] 면 0027 비트 동일(N-replica 휴면).
  if (inventory) add({ addr: 'inventory', kind: 'inventory', opts: { gateway: 'gateway', bus: busAddr, persist: persistAddr, persistBackup: persistBackupAddr, replicas: persistReplicaAddrs, snapshot: persistAddr ? snapshot : 0, reliable: persistAddr ? journalReliable : false, journalHb: persistAddr ? journalHeartbeat : false } });
