...
const { Client, DownClient, UpClient } = __p('client');
...
function makeActor(spec, net) {
  ...
  switch (spec.kind) {
    ...
    case 'upclient': a = new UpClient(spec.opts); break;   // step-0421 (#61) — 업스트림 intent 실 클라(자기 plan 으로 zoneEnter/zoneMove 발신·합성 entityOps 대체).
