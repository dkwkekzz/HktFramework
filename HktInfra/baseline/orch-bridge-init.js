...
const OrchBridgeInit = {
  _initBridgeFields(opts) {
    ...
    this.clusterDriver = opts.clusterDriverRecord
      ? { spawns: [], despawns: [], assigns: [], unassigns: [],
          onSpawn(h) { this.spawns.push(h); }, onDespawn(h) { this.despawns.push(h); },
          onAssign(h, z) { this.assigns.push(h + ':' + z); }, onUnassign(h, z) { this.unassigns.push(h + ':' + z); } }   // step-0354 — 존↔host 귀속(실 cluster.init/loadstate·migrate 입력).
      : null;
