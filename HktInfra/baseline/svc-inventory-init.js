'use strict';
// step-0266 정리 분할(#49 인접·선제) — svc-inventory-core.js 가 28.5KB(30KB 근접·성장 박스)라, InventoryService 의 *생성자 필드 초기화*(가방 원장·영속·quorum·버스·saga 상태 ~120 필드)를
//   _init(opts) 메서드로 빼 svc-inventory-init.js 믹스인으로 분리한다. 생성자는 this._init(opts) 한 줄만 — 코어가 Object.assign(prototype) 로 _init 을 되섞고 생성 시 호출.
//   필드 초기화 verbatim 이동·동작 비트 불변·기능 0 → reg 0(0265 비트 동일). 0053 txn/persist/bus 증강의 init 판(생성자 본문 외부화).
// dual-mode: Node require / 브라우저는 <script> 선행 로드(전역 __HktNetParts.svc_inventory_init).
const InventoryInit = {
  _init(opts) {
...
const __part = { InventoryInit };
if (typeof module !== 'undefined' && module.exports) module.exports = __part;
if (typeof globalThis !== 'undefined') (globalThis.__HktNetParts = globalThis.__HktNetParts || {}).svc_inventory_init = __part;
