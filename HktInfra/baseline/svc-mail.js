'use strict';
// step-0165 정리 분할 — MailService 진입점. 원장 코어 + 트랜잭션 핸들러 두 부품을 묶어 동일 export(MailService) 노출(0164 와 바이트·동작 불변·reg 0).
//   net-core.js 가 __p('svc-mail') 로 이 파일을 묶는다. 분할 이유: svc-mail.js 가 30KB 를 넘어(비대화 트리거·박스 1개=파일 1개 유계) — 거래소 svc-exchange core/txn(0124)·가방 svc-inventory(0053)·svc-whisper(0094) 와 같은 패턴.
...
if (__isNode) { require('./svc-mail-txn.js'); }
