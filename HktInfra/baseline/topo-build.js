...
function buildTopology(opts) {
  ...
  if (mail) add({ addr: 'mail', kind: 'mail', opts: { bus: busAddr, sentPublish: mailSentPublish, persist: mailPersist, snapInterval: mailPersist ? mailSnapshot : 0 } });   // sentPublish(0144): svc.mail.sent 발행. persist(0145): op 저널 replay. snapInterval(0146): 저널 스냅샷 압축. OFF 면 직전 비트 동일.
