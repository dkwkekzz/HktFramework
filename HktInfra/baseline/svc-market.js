'use strict';
// step-0116 — 시세 피드 만료 반영(svc.exchange.expired 구독): 0112 MarketFeed 는 체결(sold)·취소(cancelled)만 소비 — 0115 가 발행하기 시작한 *만료*(svc.exchange.expired)는 시세에 반영되지 않았다(0115 §9). 이 step 은 그 셋째 토픽을 구독해 item별 expired 회전을 누적한다 → 거래소 수명주기 3종(체결·취소·만료)이 모두 시세 피드에 흐른다(매물 회전/깊이 추적 완성). reconstruct(0113)도 거래소 op 저널의 'expire' kind 를 expired++ 로 처리(라이브 expired 소비와 정합·단 전제: 만료가 발행돼야 라이브가 본다·CQRS read model 은 전 수명주기 발행을 전제). marketFeed OFF·svc.exchange.expired 미발행이면 expired 0 = 0115 비트 동일.
...
class MarketFeed {
  ...
  _row(item) { if (!this.market.has(item)) this.market.set(item, { last: 0, volume: 0, cancelled: 0, expired: 0 }); return this.market.get(item); }
  onMsg(m) {
    const p = m.payload;
    ...
    // 만료 소비(svc.exchange.expired·0115) — item 의 만료(시간 회전) 누적. ev={id,seller,item,price}.
    if (p.topic === 'svc.exchange.expired') { const r = this._row(p.ev.item); r.expired++; this.consumed++; return; }
  }
  ...
  expiredOf(item) { const r = this.market.get(item); return r ? r.expired : 0; }   // 누적 만료(step-0116)
  ...
  reconstruct(journal) {
    ...
    for (const e of (journal || []).slice().sort((a, b) => a.seq - b.seq)) {
      ...
      else if (e.kind === 'expire') { const l = byId.get(e.id); if (l) { const r = this._row(l.item); r.expired++; this.consumed++; } }   // step-0116: 만료도 시세에 반영(라이브 svc.exchange.expired 소비와 정합·전제: 만료 발행 ON)
    }
  }
}
