// HktInfra step-0019 — broker(버스 허브). 멀티프로세스 lockstep 배리어를 *토픽 pub/sub 버스* 위에서 구동한다(Node 전용).
...
function reconstruct(net_, topo, snaps, placement, clusterInfo, opts) {
  ...
  const persist = persistSnap ? {
    ...
    snapshot: persistSnap.snapshot || null, snapshots: persistSnap.snapshots || 0, compacted: persistSnap.compacted || 0,   // 스냅샷 압축(이 step)
  ...
  return {
    net: net_, zones: zoneObjs, followers, reprovZones, allZones, zoneAddrs: topo.zoneAddrs, clients: clis, inventory, chat, bus, audit, ranking, persist,
