// HktInfra step-0019 — 헤드리스 검증 (발신하는 둘째 소비자 — RankingService 의 버스 consume→publish 루프·이벤트 기반 읽기 모델)
// 사용: node step-0019/verify.js <mode> [seed]
//   mode: reg | rank | e2e | sacred | recover | compact | degrade | inject | isolate | hide | repro | all
//     reg        — 회귀 0: 인프로세스 모드(ranking OFF) → step-0018 과 *비트 동일*(net.log + 상태 + inv/chat/bus/audit).
//                  persist+restart 포함. RankingService 박스·구독 도입이 *비-침습*(발행자 무수정).
//     rank       — *이 step 의 가설*: ① consume→publish 루프 작동(ranking 소비>0·발행>0) ② rank 프로젝션 == 원장 byOwner *정확 일치*
//                  (읽기 모델 ≡ 쓰기 모델) ③ 발행자(inventory) 무수정(senderDigest ranking on/off 불변=decouple) ④ 클라 rankDesync 0
//                  ⑤ 루프 없음(svc.rank.out 을 item 서비스가 안 먹음·발행 유계).
...
const NET16 = require('../step-0018/net-core.js');   // reg 대조용(직전 step)
...
function reg(seeds) {
  console.log('== reg: 인프로세스 모드(ranking OFF) → step-0018 과 비트 동일(net.log + 상태 + inv/chat/bus/audit). RankingService 도입 = 비-침습 ==');
  console.log('seed   | 구성                | 0018 logHash | 0019(inproc) | log동일 | 상태동일 | inv/chat/bus 동일 | 판정');
  for (const seed of seeds) {
    const cfgs = [
      ...
      ['persist+snap+rst  ', { zones: 2, recovery: true, failover: false, inventory: true, itemOps: 8, chat: true, chatOps: 10, regions: 2, bus: true, audit: true, persist: true, snapshot: SNAP_N, invRestart: { at: 40 } }],
    ];
    for (const [name, c] of cfgs) {
      const p = { seed, ticks: 48, clients: 4, moves: 30, radius: 4, grid: 16, incremental: true, ...c };
      const r18 = NET16.run(p);
      const r19 = run({ ...p });
      const okL = logDigest(r18) === logDigest(r19), okS = worldDigest(r18) === worldDigest(r19);
      const okX = invDigest(r18) === invDigest(r19) && chatDigest(r18) === chatDigest(r19) && busDigest(r18) === busDigest(r19);
      check(okL, `seed ${seed} ${name.trim()}: net.log 다름`);
      check(okS, `seed ${seed} ${name.trim()}: 상태 다름`);
      check(okX, `seed ${seed} ${name.trim()}: inv/chat/bus 다름`);
      console.log(`${pad(seed, 6)} | ${name} | ${hex(logDigest(r18))}   | ${hex(logDigest(r19))}   | ${(okL ? '예' : '아니오').padEnd(6)} | ${(okS ? '예' : '아니오').padEnd(8)} | ${(okX ? '예' : '아니오').padEnd(16)} | ${okL && okS && okX ? 'OK' : 'FAIL'}`);
    }
  }
  console.log('  → ranking OFF 면 토폴로지에 ranking 박스·구독이 *없고* svc.rank.out 발행/중계가 휴면 — 0018 과 비트 동일.');
}
...
// ── CLI ──
const MODES = { reg, rank, e2e, sacred, recover, compact, degrade, inject, isolate, hide, repro };
(async () => {
  ...
  } else { console.log('mode: reg | rank | e2e | sacred | recover | compact | degrade | inject | isolate | hide | repro | all'); process.exit(2); }
