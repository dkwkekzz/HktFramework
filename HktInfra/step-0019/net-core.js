// HktInfra step-0019 — 발신하는 둘째 소비자 (RankingService — 버스 consume→publish 루프·이벤트 기반 읽기 모델/CQRS 프로젝션)
// step-0018(가방 저널 스냅샷 압축) 위에 *한 조각*만 더한다 — 0016 이 남긴 한계(0016 §9-4: AuditService 는 *관찰 전용*·발신 0 →
//   이벤트를 *소비해 발행*하는 서비스의 consume→publish 루프·순서가 미검증)를 닫는다. RankingService = 이벤트 기반 *읽기 모델*:
//     · ranking 플래그 OFF 면 0018 *그대로* — 0018 과 *비트 동일*(reg 0). 박스·구독 휴면.
//     · ranking ON(bus 전제): RankingService 가 `svc.item.out`(가방 결과 스트림)을 *구독해 소비*하고 — pickup→+1·give→from-1/to+1 로
//        아바타별 보유 수(rank) 프로젝션을 유지 — 변경 시 `svc.rank.out` 토픽으로 *발행*한다(consume→publish 루프). gateway 가
//        svc.rank.out 을 구독해 해당 아바타 클라에 rank_update 중계 → 클라 rank belief 가 *원장 진실*(byOwner 크기)로 수렴.
//     · 가설의 핵: **발행자(inventory) 무수정으로 발신하는 소비자가 붙는다** — ranking 은 svc.item.out 에 *구독 행 추가*만으로 얹히고
//        (inventory spec/발신 비트 동일 = decouple), rank 프로젝션이 원장의 byOwner 와 *정확히 일치*(읽기 모델 ≡ 쓰기 모델),
//        svc.rank.out 은 item 서비스가 안 먹어 *루프 없음*(발행 유계). audit 도 svc.rank.out 관찰(둘째 소비자의 둘째 소비자).
//
// 핵심 설계 — RankingService 는 *tick 무관 순수 반응형*(onTick 없음 = 신성한 tick 밖)·*권위 아닌 읽기 모델*(원장은 가방이 권위,
//   rank 는 이벤트 스트림에서 파생한 투영 — CQRS read model). consume→publish 는 전부 버스 경유(주소 무지·은닉). 월드 경로·
//   제어 평면·영속(0017)·압축(0018)·채팅(0015)은 *그대로*(복사 전진). 시나리오 inject write-seam(0016)도 그대로.
//   (이하 0017~0018 그대로 — 영속/스냅샷 압축 설계:)
//   가방 영속 = PersistStore(append-only 효과 저널·onTick 0) + write-behind 저널 + invRestart(crash→replay) + 저널 스냅샷 압축
//   (snapshot:N — N항목마다 원장 스냅샷·upToSeq 이하 폐기·스냅샷+tail replay 무손실). 복구는 제어 평면(net.log 비-기여).
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: ranking 의 consume→publish 는 *존 tick 밖*(ranking onTick 0). 존 net.log·상태가 ranking on/off 에 비트 동일·존 도달 0.
//   - 결정론 코어: rank = 이벤트 스트림(svc.item.out 순서)의 함수 → byOwner 와 정확히 일치(읽기 모델 ≡ 쓰기 모델). 발행 유계(루프 없음).
//   - 권위 단일 소유: rank 는 *권위 아닌 파생 투영*(원장 권위는 가방·CQRS read model). 발행자(inventory) 무수정 = decouple.
//   - 은닉·단일 연결: 클라는 게이트웨이만. ranking consume/publish 는 전부 *버스*(소비자 주소 무지·클라 누설 0).
//   - headless·원격 검증: node verify.js 한 줄이 ranking 을 별 프로세스로 띄워 consume→publish 루프·프로젝션 정합을 4기둥 검증.
//   (가방·채팅·버스·감사·영속/압축·존/orch/추종자·배리어·kill 생애주기는 *그대로 잇는다* — 복사 전진. 더한 박스는 RankingService 뿐.)
'use strict';
