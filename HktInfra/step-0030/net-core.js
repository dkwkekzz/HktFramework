// HktInfra step-0030 — PersistStore quorum *쓰기* ack: W 정족수 확인 후 durable 선언 (fire-and-forget 의 정합성 윈도 가시화)
// step-0028(PersistStore N-replica + quorum-*read* 복구) 위에 *한 조각*만 더한다 — STATE §2 ⒜: "Quorum 쓰기 ack(W개 복제 ack 후 durable 선언)".
//   0028 이 N복제 fan-out + 생존 union 복구로 영속 단일점을 일반 해소했지만 §9 한계가 남았다: *fan-out 은 fire-and-forget — write quorum ack 없음*.
//   → primary 도달 후 일부 복제 미도달 상태에서 crash 시 정합성 윈도가 *보이지 않는다*(서비스가 무엇이 진짜 durable 인지 모름).
//   이 step 은 *쓰기 정족수 ack* 를 더해 그 윈도를 가시화·유계화한다 — 저장소가 저널을 받으면 ack 회신, 서비스는 W개 ack 후에만 그 seq 를 durable 선언.
//   · quorumW:W → _journal 이 보내는 저널에 q:true → 각 스토어(primary+복제)가 저장 후 journal_ack{seq} 회신.
//   · InventoryService 가 seq 별 ack 집합을 추적 → durableSeq = [0..]에서 연속으로 ≥W ack 인 *커밋 워터마크*. 윈도 = (보낸 최대 seq − durableSeq).
//   · *W 정족수의 의미*: seq 가 ≥W 스토어에 durable(ack) → 그 seq 는 (N+1−W) 스토어 죽음을 견딘다(≥1 생존본 보유). durableSeq 는 그 보장된 프런티어.
//   · quorumW:0(default) 이면 0028 *그대로* — q 플래그 0·ack 회신 0·durableSeq 미사용·비트 동일(reg 0).
//
// 핵심 설계 — *write quorum = "durable" 을 낙관(primary 도달)에서 정족수-확인 워터마크로*:
//   0028 의 fan-out 은 보내고 잊는다(write-behind) — 서비스는 어떤 seq 가 진짜 N+1 사본 중 몇에 닿았는지 모른다(정합성 윈도 = 0028 §9).
//   ack 를 더하면 서비스가 seq 별 durable 사본 수를 *센다* → W 미만은 durable 선언 안 함(낙관 금지) → 윈도가 *워터마크 위*로 정확히 드러난다.
//   각 복제가 부분 저널만 받으면(per-link 손실) 그 seq 는 ack < W → durableSeq 가 그 앞에서 멈춘다 = 윈도 가시·유계. read quorum(0028)과 짝: W+R>N+1 이면 겹침 보장.
//   가방/채팅/클라/게이트웨이/버스/랭킹/0023~0028 전 코드 *그대로*(복사 전진). 더한 것: _journal q 플래그 + InventoryService ack 집계(durableSeq) + PersistStore ack 회신뿐.
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: ack 회신·집계는 *비동기 서비스 메시지/제어 평면*(존 tick 밖). 존 net.log·상태가 quorumW 0/W 에 비트 동일·존 도달 0.
//   - 결정론 코어: durableSeq 는 같은 시드·같은 손실 → 같은 ack 도착 → 같은 워터마크(결정론 집계). 복구 = §4 "복제=재현"(0028 union 유지).
//   - 권위 단일 소유: 복제 전부 *수신만*(읽기전용). ack 는 durable 확인일 뿐 쓰기 권위 이동 아님. 원장 권위는 InventoryService 단일.
//   - 은닉·단일 연결: ack 는 서버간(store→service) 내부 메시지. 클라는 gateway 만 알고 정족수 프로토콜을 모름(은닉 유지).
//   - headless·원격 검증: node verify.js wquorum 한 줄이 손실 주입 → W 미달 seq 가 durableSeq 밖(윈도) → 그 윈도만 crash 에 소실·durable 프런티어는 생존 을 4기둥 검증.
'use strict';
...
function buildTopology(opts) {
  ...
  if (inventory) add({ addr: 'inventory', kind: 'inventory', opts: { gateway: 'gateway', bus: busAddr, persist: persistAddr, persistBackup: persistBackupAddr, replicas: persistReplicaAddrs, quorumW: persistAddr ? quorumW : 0, snapshot: persistAddr ? snapshot : 0, reliable: persistAddr ? journalReliable : false, journalHb: persistAddr ? journalHeartbeat : false } });
...
const __hktNet = {
  ...
  buildTopology, makeActor, run, runMulti, routeFilters, quorumMergeJournals,
