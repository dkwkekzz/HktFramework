// HktInfra step-0025 — 헤드리스 검증 (in-flight give 손실 복구 — 클라 give-resend 로 복구 원장을 belief 로 재수렴·0024 §9 in-flight 해소[give 한정])
// 사용: node step-0025/verify.js <mode> [seed]
//   mode: reg | inflight | tail | reliable | chat-compact | recover-chat | rank | recover-rank | e2e | sacred | recover | compact | degrade | inject | isolate | hide | repro | all
//     reg          — 회귀 0: 인프로세스 모드(clientResend OFF) → step-0024 와 *비트 동일*(net.log + 상태 + inv/chat/bus/rank).
//                  새 클라 resend 코드(givePending/giveLog/resendGives)는 clientResend OFF 면 *휴면*(메시지 0)임을 직접 증명.
//     inflight     — *이 step 의 가설*: in-flight give 손실 아래 ① resend OFF = 복구 원장이 belief 보다 뒤처짐(itemDesync>0·전송 미반영) ② resend ON = 클라 재발행→원장 따라잡음·itemDesync 0·원장 == 무손실 truth
//                  ③ give 재발행 실제 발생 ④ 멱등(dupe 0·conserved·consistent·durable give 는 owner≠from 거부). pickup mint 손실은 id 재할당이라 §9.
//     tail         — *이 step 의 가설*: 저널 홉 *tail* 손실 아래 ① heartbeat ON = persist 저널 완전(maxSentSeq 통보→tail NAK→재전송)·복구 == 무손실 기준(invDigest 동일)
...
const NET16 = require('../step-0024/net-core.js');   // reg 대조용(직전 step)
...
// in-flight *give* 손실(이 step) — xfer(전송) 저널만 100% 떨군다(mint 는 전부 durable → itemId 보존):
//   → 복구 원장에 전송 효과가 빠짐(아이템이 sender 소유로 되돌려짐) → 클라 belief(전송 완료)보다 *뒤처짐* = in-flight 손실(crash 가 sentBuffer 비움의 효과).
//   재발행(resend:true) 은 제외 → 클라 give-resend 는 신뢰 배달(손실은 *최초 전송*만). pickup mint 손실은 id 재할당이라 미포함(§9).
...
const IF_OFF = (seed) => ({ ...RESTART_NOSNAP(seed), transport: XGIVELOSS(seed) });                                                  // give 손실·resend OFF(대조 — 복구 원장이 belief 보다 뒤처짐·itemDesync>0)
const IF_ON = (seed) => ({ ...RESTART_NOSNAP(seed), transport: XGIVELOSS(seed), clientResend: true, clientResync: { at: RESTART_AT + 2 } });  // give 손실·resend ON(복구 후 클라 재발행→원장 따라잡음·ds 0·원장==무손실 truth)

// ── reg: 인프로세스 0022(chatSnapshot OFF) → 0021 비트 동일(채팅 커맨드 로그 스냅샷 압축 도입 비-침습) ──
//   마지막 cfg 는 persist ON + journalReliable ON 인데 *손실 없음* — 새 신뢰 코드(sentBuffer·NAK·dedup)가 무손실에선 *휴면*(NAK 0)임을 직접 증명.
function reg(seeds) {
  console.log('== reg: 인프로세스 모드(journalReliable OFF/무손실) → step-0022 와 비트 동일(net.log + 상태 + inv/chat/bus/rank). 저널 홉 신뢰 = 비-침습 ==');
  console.log('seed   | 구성                | 0022 logHash | 0023(inproc) | log동일 | 상태동일 | inv/chat/bus/rank 동일 | 판정');
...
// ── inflight: 이 step 의 가설 — in-flight *give* 손실(원장이 클라 belief 보다 뒤처짐)을 클라 give-resend 가 재수렴시킨다(itemId 보존·멱등·dupe 0) ──
function inflight(seeds) {
  console.log('== inflight: *가설* — in-flight give 손실 아래 ① resend OFF = 복구 원장이 belief 보다 뒤처짐(itemDesync>0·전송 미반영) ② resend ON = 클라 재발행→원장 따라잡음·itemDesync 0·원장 == 무손실 truth ③ 재발행 실제 발생 ④ 멱등(dupe 0·conserved·consistent) ==');
  console.log('seed   | 무손실 inv | resend ON inv | OFF ds | ON ds | ON==truth | xfer(OFF/ON/truth) | 재발행 | own≤1 | 보존/정합 | 판정');
  for (const seed of seeds) {
    const base = run(RESTART_NOSNAP(seed));   // 무손실 truth(전송 전부 durable·전체 저널 replay) — 복구 원장의 정답
    const off = run(IF_OFF(seed));            // give 손실 + resend OFF(복구 원장이 belief 보다 뒤처짐)
    const on = run(IF_ON(seed));              // give 손실 + resend ON(복구 후 클라 재발행)
    const offDesync = itemDesync(off) > 0;                         // ① in-flight 손실 실제 발생(클라 belief 가 원장보다 앞섬)
    const offBehind = (off.inventory.transfers || 0) < (base.inventory.transfers || 0);  // 전송 효과가 복구 원장에 빠짐
    const onConverged = itemDesync(on) === 0;                      // ② resend 가 원장을 belief 로 재수렴
    const onTruth = invDigest(on) === invDigest(base);            // ② 복구 원장 == 무손실 truth(itemId 보존·전송 재적용)
    const resends = on.clients.reduce((a, c) => a + (c.giveResends || 0), 0);
    const fired = resends > 0;                                     // ③ 재발행 메커니즘 실제 작동
    const own1 = maxItemBeliefOwners(on) <= 1;                     // ④ dupe 0(belief 이중 소유 없음)
    const safe = itemConserved(on) && ledgerConsistent(on);       // ④ 멱등 재적용(보존·역인덱스 정합)
    const ok =
      check(offDesync, `seed ${seed}: resend OFF 인데 itemDesync 0(in-flight give 손실 미발생 — 손실/시나리오 조정)`) &&
      check(offBehind, `seed ${seed}: 복구 원장 전송 빠짐 없음(give 손실 미발생)`) &&
      check(onConverged, `seed ${seed}: resend ON 후 itemDesync 잔존(재수렴 실패)`) &&
      check(onTruth, `seed ${seed}: resend ON 복구 원장 != 무손실 truth(재적용 불완전)`) &&
      check(fired, `seed ${seed}: give 재발행 미발생(메커니즘 안 탐·resends ${resends})`) &&
      check(own1 && safe, `seed ${seed}: 재발행이 멱등 아님(dupe/보존/정합 위반)`);
    console.log(`${pad(seed, 6)} | ${hex(invDigest(base))} | ${hex(invDigest(on))} | ${pad(itemDesync(off), 6)} | ${pad(itemDesync(on), 5)} | ${(onTruth ? '예' : '아니오').padEnd(9)} | ${pad((off.inventory.transfers || 0) + '/' + (on.inventory.transfers || 0) + '/' + (base.inventory.transfers || 0), 18)} | ${pad(resends, 6)} | ${(own1 ? '예' : '아니')}    | ${(safe ? '예' : '아니오').padEnd(9)} | ${ok ? 'OK' : 'FAIL'}`);
  }
  console.log('  → in-flight give 손실 = 라이브 전송이 acked(클라 belief 갱신)됐으나 저널이 손실 → 복구 원장이 belief 보다 *뒤처짐*. 라이브 ack ≠ 내구성.');
  console.log('    클라가 확인한 give 를 *같은 itemId* 로 재발행 → 복구 원장(아이템이 sender 소유)이 owner==from 으로 재적용 → belief 로 수렴. 멱등(durable give 는 owner≠from 거부). pickup mint 손실은 id 재할당이라 §9.');
}
...
async function summary(seeds) {
  console.log('== summary: 저널 홉 tail 손실 감지 — heartbeat(maxSentSeq 통보)로 NAK-only 사각인 tail 갭 감지→재전송→저널 완전화·복구 무손실 + 멀티프로세스 E2E 비트 동일(휴면 경로) ==');
  for (const seed of seeds) {
  ...
  console.log('저널 홉 전송 손실은 0023(중간)+0024(tail)로 닫혔고, in-flight give 손실은 0025(클라 give-resend)로 재수렴 — write-behind 신뢰성 거의 완성. 남은 §9 = in-flight *mint* 손실(itemId 재할당·id-reconciliation 후속).');
}

// ── CLI ──
const MODES = { reg, rank, e2e, sacred, recover, 'recover-rank': recoverRank, 'recover-chat': recoverChat, compact, 'chat-compact': chatCompact, reliable, tail, inflight, degrade, inject, isolate, hide, repro };
(async () => {
  ...
  } else { console.log('mode: reg | rank | e2e | sacred | recover | recover-rank | recover-chat | compact | chat-compact | reliable | tail | inflight | degrade | inject | isolate | hide | repro | all'); process.exit(2); }
