// HktInfra step-0025 — in-flight give 손실 복구 (클라 give-resend 로 복구 원장을 belief 로 재수렴 — 0024 §9 in-flight 손실 해소[give 한정])
// step-0024(저널 홉 tail 손실 감지) 위에 *한 조각*만 더한다 — STATE §2 ⒜(write-behind 신뢰성의 진짜 나머지)의 *give-only sub-piece*: "in-flight give 손실 + 클라 resend".
//   0023~0024 는 *전송 손실*(저널 홉 중간·tail)을 닫았다. 그러나 가방이 *활성 중* 죽으면 미-ack sentBuffer(in-flight 저널)가 소실된다 — 이미 라이브로 acked 된
//   전송(give)의 저널이 영영 사라져 복구 원장이 클라 belief 보다 *뒤처진다*(라이브 ack ≠ 내구성). 이 step 은 그 격차를 *give 한정*으로 닫는다:
//     · clientResend 플래그 OFF 면 0024 *그대로* — givePending/giveLog/resendGives 휴면·비트 동일(reg 0).
//     · ON: 클라가 *확인한 give*(item_result give ok)를 giveLog 에 보관(toAvatar 는 발신 시점에만 알 수 있다 — 게이트웨이가 결과에서 은닉) →
//        가방 복구 후 트리거(clientResync)에서 *같은 itemId* 로 재발행 → 복구 원장(전송 미반영=아이템이 sender 소유)이 owner==from 으로 재적용 → belief 로 수렴.
//     · 멱등: durable 한 give 는 복구 원장에서 owner==receiver(≠from) → 가방이 거부(이중 전송 0). itemId 보존(give 는 기존 id) → belief 무오염·dupe 0.
//        한계: pickup *mint* 손실은 itemId 가 서버 할당이라 재발행 시 *새 id* 가 나옴 → belief(옛 id) 복구 불가 → §9(id-reconciliation 후속).
//
// 핵심 설계 — *라이브 ack ≠ 내구성*: 클라 belief 는 라이브 결과로 갱신되지만 그 효과의 *저널*은 in-flight 손실될 수 있다 → 복구 원장이 belief 보다 뒤처진다.
//   give 는 *기존 itemId* 를 옮기므로(재할당 없음) 클라가 같은 itemId 로 재발행하면 원장이 belief 를 따라잡는다 — **원장 측 손실을 클라 측 진실로 재수렴**(belief = 의도된 진실).
//   *손실은 전송층*(XGIVELOSS — xfer 저널만 드롭·mint 는 durable → id 보존·재발행 제외). 클라 재발행은 *제어 평면 트리거*(run 루프가 clientResync 에 주입 = 가방 복구 핸드셰이크의 클라 측).
//   월드 경로·제어 평면·채팅·버스·랭킹·영속/압축·0023~0024 신뢰 전달은 *그대로*(복사 전진). 더한 것은 Client.givePending/giveLog/resendGives/_confirmGive + run 루프 clientResync 트리거 뿐 — *가방/persist/host.js/`engine/` 무수정*(가방 give 멱등은 이미 0014 의 owner==from 체크).
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: 클라 재발행은 *존 tick 밖*(가방 결과 스트림 위 클라 행동·제어 평면 트리거). 존 net.log·상태가 clientResend on/off 에 비트 동일·존 도달 0.
//   - 결정론 코어: 복구 원장 = persist 저널(mint) + 재발행된 give 의 함수 → 무손실 truth 와 비트 동일(invDigest). 재발행은 시드 로그의 일부(결정론). = §4 "복제=재현".
//   - 권위 단일 소유: 원장 쓰기 권위는 InventoryService 하나 — 클라 재발행은 *요청*일 뿐(가방이 owner==from 으로 판정·멱등). 소유자 belief ≤ 1(dupe 0).
//   - 은닉·단일 연결: 재발행 give 는 클라→게이트웨이 *정규 와이어*(item_give)·서비스 내부 비가시. 복구 안무 누설 0.
//   - headless·원격 검증: node verify.js inflight 한 줄이 xfer 저널에 손실을 주입해 resend ON=원장 belief 재수렴(==무손실 truth) / OFF=원장 뒤처짐(itemDesync>0)을 4기둥 검증.
//   (가방·랭킹·버스·감사·영속/압축·존/orch/추종자·배리어·kill 생애주기·0023~0024 신뢰 전달은 *그대로 잇는다* — 복사 전진. 더한 로직은 Client give-resend 뿐.)
'use strict';
...
class Client {
  constructor(script) {
    ...
    // ── in-flight 손실 복구(이 step) — 클라가 *확인한 give*(전송 인텐트)를 보관했다가 복구 시 재발행. clientResend OFF 면 휴면(0024 비트 동일). ──
    this.clientResend = script.clientResend || false;  // ON 이면 확인된 give 를 giveLog 에 보관·복구 resync 에 재발행(in-flight give 손실분 = 원장이 클라 belief 보다 뒤처진 분 을 재적용). give 만(pickup mint 는 id 재할당이라 §9).
    ...
    this.giveResends = 0;         // 복구 resync 에 재발행한 give 수(이 step·계측)
  ...
  onMsg(m) {
    ...
    if (p.type === 'auth_ok') {
      ...
      if (p.ok) {
        if (p.op === 'pickup') this.items.add(p.itemId);
...
function buildTopology(opts) {
  ...
  for (let i = 0; i < clients; i++) {
    add({ addr: 'client' + i, kind: 'client', opts: { script: { account: accounts[i], seed: (seed + i * 0x9E37) >>> 0, moves, leaveTick: leave[i] != null ? leave[i] : null, resyncPeriod, inventory, itemOps, chat, chatOps, region: String(i % regions), clientResend } } });
  }
...
function run(opts) {
  ...
  for (let i = 0; i < ticks; i++) {
    ...
    if (opts.clientResync && i + 1 === opts.clientResync.at) for (const c of clis) c.resendGives();
