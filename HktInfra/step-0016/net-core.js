// HktInfra step-0016 — 이벤트 버스 서비스 층 (게임 서비스 발행/구독 의미 · gateway↔service 직접 결합 제거 · 발행자 무수정 소비자 추가)
// step-0015(채팅 서비스 분리 — 채널 팬아웃·누설 0) 위에 *한 조각*만 더한다 — SPINE 계층 4(이벤트 버스)의 *서비스 의미*:
//   0012 토픽 버스는 *전송*(broker 가 cmd/res 프레임을 토픽으로 라우팅)을 깔았으나, 게임 서비스(0014 가방·0015 채팅)는
//   여전히 gateway 가 service *주소*를 알고 직접 라우팅했다(N×N 직접 결합). 이 step 은 그 서비스 경로에 *발행/구독 의미*를
//   올린다 — ServiceBus(토픽→구독자 라우팅 테이블, onTick 없음)를 박스로 세우고:
//     · 인프로세스 모드(run): bus 플래그 OFF 면 0015 액터 *그대로* — 0015 와 *비트 동일*(reg 0, 서비스 ON/OFF 불문).
//     · bus 플래그 ON: gateway 는 item/chat 인텐트를 *토픽*(svc.item·svc.chat)으로 발행하고 — 서비스 *주소를 모른다*
//        (gateway spec 의 inventoryAddr/chatAddr = null). 서비스는 결과를 *토픽*(svc.item.out·svc.chat.out)으로 발행하고
//        — gateway 주소를 *안 쓴다*. 발행자와 소비자가 서로의 주소를 모르는 채 토픽으로만 결합(직접 결합 0).
//     · audit 플래그 ON(가설의 핵): AuditService(새 소비자 — 4개 svc 토픽 구독·*발신 0*)를 더해도 gateway/inventory/chat
//        의 spec·발신 스트림·월드/원장/deliveries 가 *비트 동일* = "새 소비자 추가가 발행자 무수정"의 수치 증명.
//     · 멀티프로세스 모드(runMulti): bus·audit = 각자 자기 프로세스. 0012 토픽 *전송* 위에 0016 서비스 *의미*가 산다.
//
// 핵심 설계 — ServiceBus 는 *tick 무관 순수 반응형*(onTick 없음 = 신성한 tick 밖). 라우팅 테이블 Map<topic, [subscriber...]>
//   이 SSOT — pub 은 그 토픽 구독자 *전부*에게 ev 사본을 팬아웃한다(구독자 0 이면 폐기 = 발행자는 소비자 존재를 모름).
//   채팅 팬아웃(channel→Set<avatar>)이 *클라 단위* pub/sub 였다면 ServiceBus 는 *서버 박스 단위* pub/sub — 0015 의 골격을
//   서버간 계약으로 승격한 것. 월드 경로(intent→존, 존→뷰)·제어 평면(orch/lease)은 *그대로 직접*(시뮬 지연 경로 — 의도적).
//   + 이번 복사 전진에 TESTBED §10-4 의 시나리오 inject write-seam(클라 intent 주입, 미제공=no-op → reg 0)을 심는다.
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: pub/sub 라우팅은 *존 tick 밖*(bus·audit onTick 0). 존 net.log·상태가 bus/audit on/off 비트 동일·존 도달 svc 0.
//   - 결정론 코어: 팬아웃 순서 = 구독 테이블 등록 순서(선언 spec — 시드·로그의 함수). 같은 입력열 → 같은 ev 스트림(E2E·repro).
//   - 권위 단일 소유: 토픽 라우팅 권위 단일 — ServiceBus 의 구독 테이블이 SSOT(원장·deliveries 의미는 0014·0015 그대로).
//   - 은닉·단일 연결: 클라는 게이트웨이만. 토픽·pub/ev·구독 테이블·audit 은 전부 *서버간* 경계(클라 누설 0).
//   - headless·원격 검증: node verify.js 한 줄이 bus·audit 을 별 프로세스로 띄워 토픽 버스로 묶어 4기둥 검증(제1 운영 제약).
//   (0014 가방·0015 채팅 박스·존/orch/추종자·배리어·kill 생애주기는 *그대로 잇는다* — 복사 전진. 더한 박스는 ServiceBus + AuditService.)
'use strict';
...
function buildTopology(opts) {
  ...
  if (inventory) add({ addr: 'inventory', kind: 'inventory', opts: { gateway: 'gateway', bus: busAddr } });
  ...
  return { specs, order, zoneAddrs, H, grid, radius, hasInventory: !!inventory, hasChat: !!chat, hasBus: !!bus, hasAudit: !!(bus && audit) };
}
...
function run(opts) {
  ...
  return { net, login, registry, gateway, orch, inventory, chat, bus, audit, zones: zoneObjs, followers, allZones, zoneAddrs: topo.zoneAddrs, clients: clis, trace, seenTrace, deltaTrace, replicaTrace, totals, H: topo.H, grid: topo.grid, radius: topo.radius, deathTick: opts.deathTick != null ? opts.deathTick : null, killZone: opts.killZone || 'zone1', mode: 'inproc' };
}
...
// ── 모듈 노출 (dual-mode) ───────────
const __hktNet = {
  mulberry32, fnv1a, Net, LoginServer, SessionRegistry, Gateway, Orchestrator, EntityZone, InventoryService, ChatService, ServiceBus, AuditService, Client,
