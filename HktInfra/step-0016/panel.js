// HktInfra step-0016 — 시각 관찰 셸 (프로세스 토폴로지 + 이벤트 버스 서비스 층 — 발행/구독 별 모양) + 헤드리스 ASCII(Node)
// 관찰하는 것: 0015 의 gateway↔inventory·gateway↔chat *직접 엣지*가 사라지고, 모든 서비스 트래픽이 *bus 박스를 지나는 별(star)
//   모양*(gateway↔bus·bus↔inventory·bus↔chat·bus→audit)이 되는 것을 *눈으로*. audit(발행자 무수정 새 소비자)은 bus 에서
//   나가는 엣지만 갖고 들어오는 직접 엣지가 없다(발신 0). 데이터 층(compute)은 환경 무관(인프로세스 run() 만 사용).
'use strict';
(function () {
  ...
  function compute(p) {
    ...
    const r = run({ seed: p.seed, ticks, clients: 6, moves: 30, radius: 4, grid: 16, zones: 2, incremental: true, recovery: true, failover: true, deathTick, leaseTimeout, inventory: true, itemOps: 10, chat: true, chatOps: 12, regions: 2, bus: true, audit: true });
    // 프로세스 경계를 넘는 메시지(IPC) 집계 — host(from) != host(to) 인 모든 로그 메시지.
    const hosts = new Set();
    for (const s of NET.buildTopology({ seed: p.seed, clients: 6, zones: 2, failover: true, inventory: true, chat: true, bus: true, audit: true }).order) hosts.add(hostOf(s));
  ...
  function printAscii(seed) {
    ...
    console.log(`\nstep-0016 관찰(이벤트 버스 서비스 층 — 발행/구독 별 모양) — seed ${seed} · 사망 tick ${r.deathTick} · lease 타임아웃 ${r.leaseTimeout}`);
    console.log(`  서비스 트래픽이 전부 bus 박스를 지난다(직접 결합 ${r.directSvc}건) — 발행자는 토픽만·소비자 무지. audit = 발행자 무수정 새 소비자.\n`);
