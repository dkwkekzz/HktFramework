// HktInfra step-0016 — broker(버스 허브). 멀티프로세스 lockstep 배리어를 *토픽 pub/sub 버스* 위에서 구동한다(Node 전용).
//   0015 대비 *이벤트 버스(bus)·감사(audit) 재구성 + 시나리오 inject seam 만* 더한다(이하 전부 0015 그대로):
...
function reconstruct(net_, topo, snaps, placement, clusterInfo, opts) {
  ...
  return {
    net: net_, zones: zoneObjs, followers, reprovZones, allZones, zoneAddrs: topo.zoneAddrs, clients: clis, inventory, chat, bus, audit,
