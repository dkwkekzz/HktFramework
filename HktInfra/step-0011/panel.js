// HktInfra step-0011 — 시각 관찰 셸 (프로세스 토폴로지 + 소켓 와이어 트래픽) + 헤드리스 ASCII(Node)
// 관찰하는 것: *전 서버가 별 프로세스가 되어 실 TCP 소켓으로만 통신*하는 그림을 *눈으로*. 0010 의 프로세스 경계 위에서 —
//   host 태그 경계를 프로세스+소켓 경계로 본다: 각 서버 박스(login·gateway·zone·orch·추종자…)를 노드로, 그 사이를 흐르는
//   메시지를 *프로세스 경계를 넘는 소켓 와이어* 로 본다(공유 메모리 0). 사망/승격 시점의 제어 평면 트래픽(lease·promote·keyframe)을
//   타임라인에 세로선으로. 데이터 층(compute)은 환경 무관(인프로세스 run() 만 사용 — 브라우저는 child_process 미로드).
'use strict';
(function () {
  ...
  function mountBrowser() {
    ...
    function drawTimeline(r) {
      ...
      ctx.fillStyle = '#bac2de'; ctx.font = '10.5px ui-monospace, monospace'; ctx.textAlign = 'left';
      ctx.fillText('프로세스 경계 소켓 와이어 트래픽 / tick  ·  빨선=존 사망(lease 끊김) · 초록선=승격(keyframe 버스트)', 4, y0 + 11);
    }
    function render() {
      ...
      note.textContent = '프로세스 경계의 와이어를 *실 TCP 소켓*(child_process.spawn·IPC 채널 0)으로 현실화했다. 각 박스는 별 OS 프로세스이고, 선은 그 사이를 '
  ...
  function printAscii(seed) {
    ...
    console.log(`\nstep-0011 관찰(실 TCP 소켓 전송) — seed ${seed} · 사망 tick ${r.deathTick} · lease 타임아웃 ${r.leaseTimeout}`);
    console.log(`  전 서버 박스를 별 프로세스로 분리 → 실 TCP 소켓(길이-프리픽스 프레임)으로만 통신(공유 메모리 0). 인프로세스와 비트 동일(E2E 동치).\n`);
    console.log('  프로세스(호스트): ' + r.hosts.join(' · '));
    console.log('  프로세스 경계 소켓 와이어: ' + r.ipcMsgs + ' 메시지 / ' + r.ipcBytes + ' payload bytes · lease ' + r.leasesSent + ' · 승격KF ' + r.promotionKeyframes);
    console.log('\n  소켓 와이어 트래픽 / tick (빨=사망 ' + r.deathTick + ' · 초=승격 ' + (r.promoTick || '-') + '):');
    ...
    console.log('\n  → host 태그 = 실 프로세스 + 실 TCP 소켓(spawn·IPC 0). broker lockstep 배리어가 전역 발신 순서 보존 → 비트 동일.\n');
  }
