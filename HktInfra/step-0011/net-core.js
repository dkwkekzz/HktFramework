// HktInfra step-0011 — 실 네트워크 소켓 전송 현실화: IPC 파이프 → 실 TCP 소켓 (와이어만 교체)
// step-0010(프로세스 경계 현실화) 위에 *한 조각*만 더한다:
//   0010 은 host 태그 경계를 *실 OS 프로세스*로 분리하고 broker↔host 를 child_process IPC 파이프로 통신시켰다 —
//   그러나 전송은 아직 *로컬 IPC 파이프*였다(0010 §8.2: 실 네트워크 소켓 아님). 이 step 은 그 *와이어*를 갈아끼운다:
//     · 인프로세스 모드(run): engine/Net 그대로 — 0010(=0009)과 *비트 동일*(reg 0). 와이어 교체는 비-침습.
//     · 멀티프로세스 모드(runMulti, channel='tcp'): broker 와 각 호스트가 *실 TCP 소켓*(net.createServer/connect,
//       127.0.0.1 임시 포트)으로 통신한다 — IPC 채널 없이 순수 소켓만. TCP 는 *바이트 스트림*이라 메시지 경계가
//       없으므로 길이-프리픽스 프레이밍(frame.js)으로 경계를 재조립한다 = "실 네트워크 전송"의 구체적 차이.
//     · channel='ipc' 면 0010 그대로(와이어 OFF 기준). 액터·lockstep 배리어·broker substrate 는 *무변경*.
//
// 핵심 설계 — *와이어만 교체, 의미는 보존*:
//   broker 의 lockstep 배리어(아래 cluster.js runMulti 루프)는 *채널 무관* — channel.rpc(host,msg) 만 부른다.
//   전역 발신 순서를 (전역 gi, 로컬 si)로 정확히 재정렬·재생하는 substrate(engine/Net)는 그대로다. 그래서
//   TCP 소켓 위 실행도 broker.net.log 가 인프로세스와 *비트 동일* → 실 전송이 의미를 완전히 보존함을 증명(sock).
...
//   - 결정론 코어: 액터는 시드 의사난수만(Math.random 0). 와이어가 갈려도 같은 입력열 → 같은 상태(실 전송 동치).
//   - 권위 단일 소유: 0009 불변(소유자=1·이중쓰기 0·failover bounded gap→회복)이 소켓 전송 너머로도 보존.
//   - 은닉·단일 연결: 클라는 게이트웨이만. 소켓 전송은 *서버간* 경계 — 클라엔 비가시(누설 0).
//   - headless·원격 검증: node verify.js 한 줄이 전 프로세스를 띄워(실 소켓 연결) 같은 4기둥 검증.
'use strict';
...
// ════════════════════════════════════════════════════════════════════════
function run(opts) {
  const { seed, ticks = 48, transport = null } = opts;
...
// ════════════════════════════════════════════════════════════════════════
//  runMulti — 멀티프로세스 모드. cluster.js 에 위임(Node 한정 — 브라우저 미로드).
//   opts.channel: 'ipc'(기본, 0010 와이어) | 'tcp'(실 소켓, 이 step). 같은 buildTopology·lockstep 배리어로 구동.
//   반환 r 은 run() 과 같은 digest 함수들이 그대로 먹는 형태(zones/clients/net.log) + r.cluster(pids/placement/channel/port).
// ════════════════════════════════════════════════════════════════════════
