// HktInfra step-0018 — 가방 저널 스냅샷 압축 (계층 6 데이터 · event sourcing 의 "intent 로그 + 주기 스냅샷" — 무한 성장 저널 압축)
// step-0017(가방 서비스 failover·영속 — 영속 저널에서 원장 재구성) 위에 *한 조각*만 더한다 — 0017 이 남긴 한계 ⓑ(저널 무한 성장·
//   스냅샷 압축 없음)를 닫는다. 0017 의 저널은 *append-only 무한 성장*이었다 — 모든 mint/xfer 가 영원히 쌓이고, 복구는 *전체 저널*을
//   처음부터 replay 했다. 이 step 은 §4 DURABLE("월드 영속 = intent 로그 + 스냅샷")·SPINE("intent 로그+스냅샷")의 정전 패턴을 가방에 깐다:
//     · snapshot 플래그 OFF(=0) 면 0017 *그대로* — 0017 과 *비트 동일*(reg 0, persist/restart ON/OFF 불문). 압축 휴면.
//     · snapshot:N ON: 가방이 저널 N항목마다 *원장 스냅샷*(현재 ledger+mintTotal+계측)을 PersistStore 로 write-behind(반응형 — op
//        처리 중 발신, onTick 0 유지). PersistStore 는 스냅샷의 upToSeq *이하 저널을 폐기*(압축) → 저널이 (스냅샷 1개 + 짧은 tail)로 *유계*.
//        라이브 원장은 압축에 *비-침습*(invDigest 불변 — 압축은 persist-측 일) · 복구 = 스냅샷 원장 베이스 + tail(seq>upToSeq) replay.
//        가설의 핵: **스냅샷+tail replay == 전체-저널 replay 비트 동일(무손실 압축)** + 저널 크기 절감. (이하 0017 그대로:)
//   0014~0016 의 게임 서비스(가방·채팅·버스·감사)는 전부 *영속 0*이었다 — 프로세스가 죽으면 원장/구독/라우팅이 RAM 과 함께
//   소멸했다(세계가 세션과 함께 죽는다). 0017 은 가방 원장에 *영속 진실*을 깔았다 — PersistStore(append-only 저널, onTick 없음)를
//   데이터 계층 박스로 세우고:
//     · persist 플래그 OFF 면 0016 액터 *그대로* — 0016 과 *비트 동일*(reg 0, 서비스/버스 ON/OFF 불문).
//     · persist 플래그 ON: InventoryService 가 수락한 *모든 변이*(mint=pickup·xfer=give)를 PersistStore 로 *write-behind 저널*
//        한다(fire-and-forget — 결과 ack 는 영속 ack 를 기다리지 않음). 저널 = event sourcing 의 인벤토리 판(itemId→owner 의 *효과 로그*).
//     · invRestart:{at} ON(가설의 핵): 그 tick 에 가방 서비스가 *죽고*(RAM 소실) *재spawn* 된다 — 인프로세스는 in-place crash()+
//        replay(저널), 멀티프로세스는 진짜 child.kill→새 호스트 spawn→init→replay(0013 재-provisioning 패턴의 *서비스 판*).
//        PersistStore 는 *죽지 않는다*(자기 프로세스·데이터 계층) → 새 가방이 저널을 replay 해 원장을 *죽기 전과 비트 동일*하게
//        재구성한다. 영속 OFF+restart 대조군은 원장이 *비고*(소실) → 영속이 보존의 원인임을 수치로 증명.
//
// 핵심 설계 — PersistStore 는 *tick 무관 순수 반응형*(onTick 없음 = 신성한 tick 밖). 저널 = [{seq,kind:'mint'|'xfer',...}] 가 SSOT.
//   복구(crash+replay)는 *제어 평면*(인프로세스=하니스 함수·멀티=broker RPC) — net.log(월드/서비스 응용 메시지 스트림)에 *안 나타난다*
//   → 인프로세스/멀티프로세스 net.log 비트 동일(E2E). 저널 *쓰기*(inventory→persist)만 응용 메시지(net.log)다. 가방의 영속 인터페이스는
//   버스가 아닌 *직접 명시 인터페이스*(서비스 자기 데이터 스토어 = DB 연결의 더미판 — §4 "버스/명시 인터페이스" 허용). 월드 경로·제어
//   평면·버스 의미(0016)·채팅(0015)은 *그대로*(복사 전진). 시나리오 inject write-seam(0016)도 그대로.
//
// 척추(SPINE.md) 준수:
//   - 신성한 tick: 저널 쓰기·스냅샷·replay 는 *존 tick 밖*(persist onTick 0). 존 net.log·상태가 persist on/off·snapshot·restart 유무에 비트 동일·존 도달 0.
//   - 결정론 코어: 저널 = 수락 순서의 함수(시드·로그). replay 는 같은 저널 → 같은 원장(mintTotal 복원으로 itemId 연속성 유지) → 복구 투명.
//   - 권위 단일 소유: 원장 쓰기 권위 단일(InventoryService) — 죽어도 PersistStore 저널이 *재현*의 진실(상태 전송 아님 = §4 "복제=재현").
//   - 은닉·단일 연결: 클라는 게이트웨이만. 저널/persist/replay 는 전부 *서버간/제어 평면* 경계(클라 누설 0).
//   - headless·원격 검증: node verify.js 한 줄이 persist 를 별 프로세스로 띄우고 가방을 진짜 kill→replay 로 복구해 4기둥 검증(제1 운영 제약).
//   (가방·채팅·버스·감사·존/orch/추종자·배리어·kill 생애주기는 *그대로 잇는다* — 복사 전진. 더한 박스는 PersistStore + 가방 crash/replay/journal.)
'use strict';
...
class Gateway {
  ...
  onMsg(m) {
    ...
    if (this.bus && m.from === this.bus) {
      ...
      if (p.type === 'ev') {
        if (p.topic === 'svc.item.out') this._relayItemResult(p.ev);
...
class InventoryService {
  constructor(opts = {}) {
    ...
    this.snapInterval = opts.snapshot || 0;  // 스냅샷 압축 주기(이 step) — 저널 N항목마다 원장 스냅샷 발신(0 = 0017 비트 동일·압축 휴면).
  ...
  _journal(entry) {
    ...
    // 스냅샷 압축(이 step) — 저널 N항목마다 *원장 스냅샷*을 persist 로(write-behind·반응형·onTick 0 유지). persist 가 upToSeq 이하
  ...
  onMsg(m) {
    let p = m.payload;
    if (p.type === 'ev' && p.topic === 'svc.item') p = p.ev;   // 버스 봉투 해체(구독 수신) — 직접 모드와 같은 item_req
...
class PersistStore {
  constructor() {
    ...
    this.writes = 0;              // append 수신 *누적* 수(압축에도 안 줄음 — 가방 수락 변이 = mint+xfer 와 대조의 진실)
  ...
  onMsg(m) {
    const p = m.payload;
    if (p.type === 'journal') { this.journal.push(p.entry); this.writes++; return; }
...
function buildTopology(opts) {
  ...
  if (persistAddr) add({ addr: 'persist', kind: 'persist', opts: {} });
  ...
  if (inventory) add({ addr: 'inventory', kind: 'inventory', opts: { gateway: 'gateway', bus: busAddr, persist: persistAddr, snapshot: persistAddr ? snapshot : 0 } });
  // [게임 서비스] 채팅 — chat ON 일 때만 토폴로지에 존재(OFF = 0014 토폴로지 비트 동일). onTick 없음 = 신성한 tick 밖.
  if (chat) add({ addr: 'chat', kind: 'chat', opts: { gateway: 'gateway', bus: busAddr } });
...
// ── 모듈 노출 (dual-mode) ───────────
const __hktNet = {
  mulberry32, fnv1a, Net, LoginServer, SessionRegistry, Gateway, Orchestrator, EntityZone, InventoryService, ChatService, ServiceBus, AuditService, PersistStore, Client,
