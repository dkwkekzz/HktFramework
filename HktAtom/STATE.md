...
## §2 NEXT — 다음 한 조각 (step-0006 = S0-⑥ 공유결합·복합체·해리)

**다음 구현 = step-0006 = S0-⑥ 공유결합** (앵커: 분자 H₂·H₂O·CH₄(가상) · [design/06-bonding.md](stages/S0-atom/design/06-bonding.md)): 카탈로그에 **결합/해리 행** 추가 + `bond` 자료(빈 배열 실사용) + **안정화 경로 필수**(접근→임시 복합체→광자/제3체 방출→안정 결합 — 2체 직접 결합 금지). ③의 **결합차수 예산 B** 로 원자가 포화(과결합 0·H₃ 덩어리 0). ④의 실행기·⑤의 전이 회계 재사용. 닫는 기준: 조성 시그니처 창발·예산 포화·안정화 없인 결합률 급감. `U_bond` 통 활성.
...
- step-0005: S0-⑤ 이온화·전자 이전 — electron 입자(힘·적분·경계·장부 통합)+R-XFER(transferElectron·오르막 게이트)+Kat/An 가상 원소+ionState. node verify.js 47 PASS. 이온 격자 NaCl 창발(마델룽 U_elec<0·교대질서). 범위: 격자 앵커 집중, 자유전자 이온화⇌재결합은 ⑳ 이월. 뷰어: +/− 이온 링.
