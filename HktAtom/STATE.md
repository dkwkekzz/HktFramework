...
## §2 NEXT — 다음 한 조각 (step-0008 = S0-⑧ 분극·응집)

**다음 구현 = step-0008 = S0-⑧ 분극·응집** (앵커: 중성 입자가 뭉친다 · [design/08-polarization-cohesion.md](stages/S0-atom/design/08-polarization-cohesion.md)): **유도 쌍극자 μ=αE**(원소별 분극률 α — 바닥 author) 도입 → 유도-유도 상관(런던 분산의 고전 근사)으로 **중성 기체 응집** 창발. `U_pol` 통 활성. 닫는 기준: 저온 중성 기체 응집·U_pol 장부 닫힘·**α=0 이면 응집 소멸**(근원 검증). 고전 근사 부족 시 C6/r⁶ 잔여항 정직 author. "없는 물리는 측정에서 안 생긴다" 원칙의 실증 — 지금까지 중성 인력이 없었음(⑥ H₂ 외). **액화·MVP 관문(⑨⑩⑪)의 전제.**
