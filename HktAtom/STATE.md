...
> **⑮ 극성(QEq) 법칙 승격 = 반응 세계의 부분 전하 (step-0035 · 게이트 7차)**: 스택에 `stage:'pre'` 도입(전하 갱신은 기반 pairForces 앞·U_pol 누적 계약). ⑤/⑮ **전하 이원 회계**: 정수층 qBase=Z−ne(R-XFER 소유·사건 단위) + 연속층 dq(QEq 재분배·매 평가) — 단원자 성분 건너뜀으로 맨 이온 ⑤ 에너지학 정확 보존. `q=qBase+x` 잠재 버그 정정(q=x·legacy qBase=0 이라 불변). 게이트 `qeqParams`(종→{χ,IE}·③ 유도). 앵커: H₂O q_O −0.207·q_H +0.104·Σq 1e-17·μ 0.168 vs O₂ 무극성 0 · Na+Cl ±1.00 정확 공존 · QEq=정확 최소화라 포락선 정확(잔차 −4.9e-5/3000틱). **발견: 극성 ON → 같은 수프에서 H₂O 1→3개** ("H₂O 우세는 ⑮ 필요" 격차 닫히기 시작). 뷰어 전하 링 2단(굵은=정수·얇은=δ±). verify 35 신설 4 PASS·**전량 198 PASS·0 FAIL**(34+35 포함·legacy 회귀 0). 세부 step-0035.md.
...
- **법칙 스택 잔여 승격** (step-0033~35 후속·수요 시·playground 기준 — 사용자 지시로 scene 호환 비목표): ⑰ 산염기(양성자 이전 행 + 용매화 — ⑮ 완료로 전제 충족) → ⑲⑳⑫(법칙 아닌 *개체 추가* — 자유전자·광자 별도 설계). ⑨① 관찰 프리셋 확장(물리 이미 있음·뷰어만). ④⑦은 specLevels 종 데이터 필요. Efield 노브 노출(⑮ 배향 실험).
