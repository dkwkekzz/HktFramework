...
- **S0-⑭ 형상 (step-0014) — 결합각·VSEPR**: 분자 형상은 author 하지 않는다 — **공통 각도 반발 하나**로 정사면체·굽음·직선이 동시 창발. **`geometry.js` 신설**(self-contained·엔진 ①–⑬ **diff 0**·⑬ computeForces 합성 패턴): 도메인=결합 방향+고립쌍(수=(외각 전자−결합)/2·**③ fillZ 유도·author 0**)·`V_ang=k_ang·Σ w_a w_b/(1−cosθ+c0)`(w=1 bond·λ_lp lone)·결합 도메인 힘=−∂V/∂r(반작용 F_center=−ΣF_nb → **P·L 정확 보존**·뇌터)·고립쌍은 질량 없는 보조 변수(매 force 준정적 최소화 → 포락선 정리로 보존·라우팅 0). scenes 3장면(s14-methane/water/linear·미리 결합·피보나치 배치·형상 이완 준비) + index.html 고립쌍 유령 화살표. `node verify.js`=**101 PASS**(①–⑬ 93 회귀 + ⑭ 8): **3단 앵커 한 상수 세트**(분자별 분기 0) CH₄ **109.5°**·H₂O **101°**(굽음)·BeH₂ **175°**(직선)·**형상 서열 창발** 굽음<정사면체<직선 · **고립쌍 압박 근원**(λ_lp=1 대조 물 109.9°>101°) · 장부 닫힘·P 1e-15·L 기계 정밀도. 뷰어(눈 확인·콘솔 0): 정사면체·굽음+청록 고립쌍 파선·직선. 발견: 준정적 고립쌍 최소화가 정직(잉여 K_tr 라우팅이 P 깸)·직선 소프트 최소는 이완 측정·CO₂ 는 이중결합(⑩ 격차) 대기라 BeH₂ 로 확인.
- **S0-⑬ z 해동 (step-0013)** — 요약(전문 §5): `frozenZ:false` 하나로 3D·**엔진 변경 0**(engine/catalog diff=0·①⑦ 자유도 회계 덕). node verify.js **93 PASS**: z 해동 등분배 창발(iso 0→~1.0·z 차갑게 출발→충돌 분배)·3D 장부·겹침 0·결합 위상. 발견: 엔진 변경 0 성립·⑦ 3D C_v 이월.
- **S0-⑫ 복사장 (step-0012) — Part II 시작** — 요약(전문 §5): ④ 광자 빈 → **photon 입자**(engine runPhotonField·radiationMode='field' additive·회귀 0). node verify.js **88 PASS**: 복사 냉각 T 2.56→1.34·공동 정상 상태·스펙트럼 단색(2준위)·Σphoton.E==E_photon통 정확·유도 방출 63 vs 20.75(3.04×·빔). 뷰어: s12-stim 축정렬 82% 빔.
...
## §2 NEXT — 다음 한 조각 (step-0015 = S0-⑮ 극성 · 부분 전하)

**⑭ 형상 닫힘 — 공통 각도 반발 하나로 정사면체·굽음·직선 동시 창발·목표각 author 0.** 두 갈래:
- **S1-① (열림)**: `output.json` 이 S1 입력으로 준비됨 — S1 분자 단계 착수 가능(design/11 경계: "S1 소비 테스트는 S1-① 몫").
- **다음 구현 = step-0015 = S0-⑮ 극성** (앵커: 물은 극성·O₂/CO₂/H₂O 3단 · [design/15-polarity.md](stages/S0-atom/design/15-polarity.md) · 전제 ⑥⑩·⑭ 권장): 전기음성도 차 → 결합 부분 전하 → 분자 쌍극자. ⑭ 형상 위에서 굽은 H₂O 는 극성·직선 CO₂ 는 무극성이 창발해야(형상×전하). **로드맵(design README): ⑮(극성)→⑯(수소결합)…㉒(실 S1 입력 ⇧).** 세부 선택 자유(⑮ 또는 S1-① 착수) — STATE §2 가 확정.
...
- step-0010: S0-⑩ 수프 관문 — 실원소 합류. engine 쌍별 D(pairD·fallback→회귀0)+실원소 H·O·He·Ne·Dpair(436:463:146)·annealSoup(E_escape 회계). node verify.js 74 PASS. 2H:1O 어닐 → H2O1:46 최다 창발·과결합 0·어닐링 이득. 발견: H–H≈O–H→H₂ 경쟁·O=O 이중결합 부재. 뷰어: CPK·라이브 어닐링.
- step-0011: S0-⑪ 승격 배관 MVP(Part I 관문). promote.js(coarse⇧·rethaw⇩·회귀0)+s11-mvp-box·runScenario(5막 한 장부). node verify.js 81 PASS. 5막 1.19e-4·왕복 보존 정확(ΔE·P 1e-15·조성 ε=0·T비 1.28)·output.json v0(S1 입력)·반데르발스 꼬리(−0.14). Part I 닫힘·S1-① 열림.
- step-0014: S0-⑭ 형상(결합각·VSEPR). geometry.js 신설(self-contained·엔진 diff 0·도메인=결합+고립쌍③유도·V_ang=k Σ/(1−cosθ+c0)·힘 −∇V·P·L 보존·고립쌍 준정적 최소화)+s14 3장면(미리 결합·형상 이완)+고립쌍 유령 화살표. node verify.js 101 PASS. 3단 앵커 한 상수 세트(CH₄ 109.5·H₂O 101·BeH₂ 175·분자별 분기 0)·형상 서열 창발·고립쌍 압박 근원(λ_lp=1 대조 109.9)·장부·P·L 보존. 발견: 준정적 고립쌍 최소화 정직·직선 소프트 최소는 이완 측정·CO₂ 이중결합 대기.
- step-0013: S0-⑬ z 해동(3D 전환) — **엔진 변경 0**(engine/catalog diff=0·①⑦ 자유도 회계). scenes frozenZ 분기+s13 3장면+measure momentumVariance+index.html 3D 투영 뷰어(자체 행렬·마우스 회전). node verify.js 93 PASS. z 동결 증거·z 해동 등분배 창발(iso 0→~1.0)·3D 장부·겹침 0·결합 위상. 발견: 엔진 변경 0 성립·⑦ 3D C_v 이월.
