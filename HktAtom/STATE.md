...
> **⑰ 산·염기 승격 = 양성자 릴레이 (step-0036 · 게이트 8차)**: `runProton` tick 후단 실행기(RPX 2.4 > rc 1.5 라 접촉 채널 밖)·solv 법칙(rank 30·게이트 protSolv·기본 0=유전 차폐 없음이 참값)·⑤⑮⑰ 3층 전하 정합(H⁺ 이동 시 전자는 공여체에 — setNe → 정수층이 H₃O⁺/OH⁻ 를 만들고 QEq 가 재분배)·1가 게이트(Q² 용매화의 O²⁻ 조장 실측 → 차단)·protSpot 겹침 회피(겹침 배치 +1247 폭탄). **일반형 발견: 불발 되돌림의 고립쌍 분지 전환 누수(+5.25/사건)** → ⑭ 씨앗을 결정론 부채꼴로(rng 폐기 — 복원되면 씨앗 동일 → 모든 되돌림 정확·수프 잔차 +11→−0.10·각도 앵커 불변). 앵커: 이전 3~12회·Σq 정수 0·±균형·H₄O²⁺ 금지·H₃O⁺ Q=+1 정확. 뷰어 🧪 프리셋+분자 전하 라벨(H3O⁺/HO⁻). verify 36 신설 3 PASS·**전량 201 PASS·0 FAIL**(legacy ⑰ 드리프트 임계 4e-3 조정 — 물리 assert 불변). 세부 step-0036.md.
...
- **법칙 스택 잔여 승격** (step-0033~36 후속·수요 시·playground 기준 — 사용자 지시로 scene 호환 비목표): ⑲⑳⑫(법칙 아닌 *개체 추가* — 자유전자·광자 별도 설계). ⑨① 관찰 프리셋 확장(물리 이미 있음·뷰어만). ④⑦은 specLevels 종 데이터 필요. Efield 노브 노출(⑮ 배향 실험). ⑰은 step-0036 완료.
