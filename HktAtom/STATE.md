...
> **핵 게이트 G-핵 개방 (step-0026)**: 사용자가 "핵분열 가능" 요구 → CONTRACT §6-② 발동. 우선순위 ㉓ 최소분 → ㉔ 붕괴 → **㉕ 분열**. 가상 핵종·실수치 정합 불요. ㉑(성능·G-성능)은 여전히 규모 벽 미도래로 보류.
> **㉒ MaterialModel 완결 (step-0022~0025)**: ①~⑳ + ㉒ 4 필드(EOS·확산·반응망·물앵커). output v0.3 실물. 경로 정정: S1-①(0021) 먼저 열었다 되돌아와 ㉒ 완결. **S1-② 응집은 핵 트랙 뒤 재개.**

- **S0-㉓·㉔ 핵종·붕괴 (step-0026·0027)**: **`nuclear.js` 신설**(엔진 diff 0·자체 완결). ㉓ 보존 다발 **c=(Z,e)→(Z,N,e)**(N 화학 미접촉)·질량 Σ 회계 m=Z·MP+N·MN−BE·C2(**질량 결손=장부 실물**·분열 방출 근원)·앵커 **동위원소 진동** ω_D/ω_H=0.7746≈√(μ_H/μ_D)(결합 스프링·author 0). ㉔ 붕괴 채널(β⁻/α/γ/n·lifetime·Q값→방출 KE+γ+중성미자 E_escape)·앵커 **지수 감쇠**(λ≈ln2/반감기·R²=0.9999·수명 재현)·**계열 붕괴**(FPa→FPb→FPc 딸 Bateman 봉우리)·붕괴열·지연 중성자(3% n·k_eff 제어 근원). `verify --only 23,24`=**7 PASS**. 발견: ⑦ ħω 고정→앵커는 결합 스프링서 측정(design/23 정정)·개체군 몬테카를로. 뷰어: 두 진자+붕괴 곡선 4종.
...
## §2 NEXT — 다음 한 조각 (step-0028 = S0-㉕ 분열·k_eff — 핵분열 실현)

**핵 게이트 개방·㉓→㉔ 완료 — 다음이 핵분열 본체(㉕).** :
- **㉕ 분열** (다음·nuclear.js): 중성자 입자(전자 형식 재사용·q=0·핵과만 단면)·단면 에너지 밴드표(fast/thermal·1/v)·행 R-N-SCAT(감속=운동학 창발)·R-N-CAP·R-FISSION(무거운 핵 F→파편 2+중성자 ν개+Q). **k_eff 측정**(세대비 N(g+1)/N(g) + 시간지수 α·교차)·**3영역**(k<1 소멸/≈1 정상/>1 지수 증가·밀도·크기 스캔 임계 곡선·누설). 파편→㉔ 붕괴 큐 자동 진입(지연 중성자·author 0). **Δm·c² 회계**(분열 전후 Σm 감소=방출 E·사건 ≤1e-9)·Σc(Z,N,e) 불변. NuclideTable 산출⇧(σ·ν·파편·Q·지연분·validRange·errorBounds). 장면 s25-moderated/density-scan/chain-burst.
- **㉖ 융합**(Gamow·별 S2 연계·후속)·**㉒-b2·η/κ·상전이** 정련·**S1-② 응집**은 핵 트랙 뒤.

**승격 계약** ([CONTRACT.md](CONTRACT.md)): output v0.3 = ⑪ MVP + ㉒ 전체. ㉕ NuclideTable 별도 발효 → 중간 해상도(몬테카를로 중성자 수송) 소비(CONTRACT §5 폭발 왕복). 위력=회계(authored 숫자 아님).
