...
- **S0-㉒-a·b·c MaterialModel ⇧ EOS+수송 D+반응망 k(T) (step-0022~0024)**: **`material.js` 신설**(엔진 diff 0·engine/scenes/measure 재사용). 물 수프 NVT 굴림→ ㉒-a P·U 표 + ㉒-b **확산 D=MSD 기울기** + ㉒-c **해리 k(T)→아레니우스 {A,Ea}** 측정(author 0). **output.json v0.1→v0.2 가법**(CONTRACT §7·스키마 태그 유지·S1 v0 소비 불변): +stateVariables·+equationOfState·+transportCoefficients.diffusion·+reactionNetwork(아레니우스)·+errorBounds(P·U·C_v·D)·observables+=EOS·D·k. `node verify.js --only 22`=**12 PASS**: 계약·회계(Σc 30→30)·EOS(P↑·**C_v>0**)·수송(**D(ρ)↓·D(T)↑**)·반응(**k(T)↑·Ea=1.62>0·R²=0.99**)·author0. **압력 정합**: waterSoup(pairForces·virial 정확)로 측정 — 분산 응집 EOS 접힘 ㉒-b2(격차). 뷰어: 히트맵 3종(P·U·D)+C_v+아레니우스 플롯.
...
## §2 NEXT — 다음 한 조각 (step-0025 = S0-㉒-d 방향/밀도 interactionModel · ㉒ 닫힘)

**㉒ 는 다-step 아크다** — a=EOS·b=확산 D·c=반응망 k(T) 완료. ㉒ 공식 닫는 기준(design/22: **물 V′ 의 방향·밀도 의존이 모델에 담김**)만 남았다:
- **㉒-d 방향/밀도 interactionModel (물 앵커·㉒ 닫힘)** (다음): pairPMF 를 각도 빈(⑯ H-결합 정합각)×밀도 빈으로 확장 측정 → 방향 보정 h(θ)·밀도 보정 g(ρ). **등방 대비 유의차가 닫는 기준** — 이게 되면 ㉒(따라서 S0 의 진짜 출력) 완결.
- **㉒-b2 분산 응집 EOS 압력** (격차·optional·㉒ 닫힘엔 불필요): 현 EOS 는 waterSoup(척력+반응)라 반데르발스 loop 없음. 부피 스케일 유한차분 압력(전 채널 virial·코어 diff 0)으로 polForces 응집 담기. η/κ 수송·상전이도 여기 묶어 후속.
- **㉒ 닫힌 뒤 = S1-② 응집 재개**: output.json 이 실물 MaterialModel 되면 S1 이 EOS·D·k·interactionModel 소비해 액적 창발(step-0021 §발견).
- **미룬 것 = S0-㉑ 성능·동적 병합**: S0 규모 O(N²) 벽 미도래 — payoff(거시) S1+ 몫([design/21-merge.md](stages/S0-atom/design/21-merge.md)·게이트 G-성능).

**승격 계약 발효 진행** ([CONTRACT.md](CONTRACT.md)): output v0.2 = ⑪ MVP + ㉒-a EOS + ㉒-b 확산 + ㉒-c 반응망(가법). 관측량 계약에 EOS·D·k 선언. 방향/밀도 interactionModel 만 남아 ㉒ 닫힘 임박.
...
- **㉒ 다-step 아크 (a·b·c 완료·d 남음)** (step-0022~0024): a=EOS·b=확산 D·c=반응망 k(T) 발효. ㉒ 공식 닫는 기준(물 V′ 방향·밀도 의존)만 남음=㉒-d. η/κ 수송·상전이는 후속/S1 — §2 순서.
...
- **어닐링 항온조=측정 도구** (step-0010): 냉각 뺀 열→E_escape 회계·장부 닫힘(④ 동형).
