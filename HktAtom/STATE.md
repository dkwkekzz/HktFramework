...
> **⑫ 복사장 합류 = 발광하는 기체·광자 개체 (step-0038 · 게이트 10차·개체 추가형 2/3)**: playground 💡 프리셋에서 가열된 수소 기체가 빛을 낸다 — 충돌 들뜸(R-COL 자유충돌)→자발 방출(R-EMI 수명 시계)→**노란 광자 개체**(질량 0·방향·전파·경계) 방출. 앵커: 방출 복사 E_escape T0.3 3.5→T3.0 26.3(×7.5·가열 응답)·open=복사 냉각(광자 탈출) vs reflect=공동(갇힘 재흡수). specLevels(첫 들뜸 dE=clamp(0.75·IE,0.4,3.0)·수소형 Rydberg 3/4·③ IE 유도·g0/g1=1/3) **base 상시**(파라미터 존재·부재=참값). **발견(설계 이탈)**: ⑳ 처럼 상시 합류 불가 — 충돌 완화가 근-영 상대 KE 에 dE 쏟으면 카타펄트(잔차 +9/사건·물형성 −0.09→+26.7) → 들뜸 원천 R-COL·field 모드는 enableRadiation(💡) 관찰 프리셋 전용(enableHBond 지위·자발 방출 R-EMI 는 KE 불변 안전). ⑳ 분리 nu_ion=0 통제. 뷰어 노란 광자(점+꼬리)·들뜬 원자 후광·+Nγ. verify 38 신설 3 PASS·**전량 207 PASS·0 FAIL**. 세부 step-0038.md.
...
**step-0039 = ⑲ 금속 합류 (다음·개체 추가형 3/3 — 비국소 전자 풀)**:
- `metal.js`(엔진 diff 0) 는 금속 결합=이온-이온 우물 Dmetal 로 이미 있다 (step-0019·고전 점전자 응집 불가의 유효 우물). playground 는 종별 Dmetal 파라미터·금속 종 게이트만 실으면 된다 (⑫⑳ 개체 패턴 참고이나 전자 풀은 비국소라 별도 설계 — 집단 플라스마·밴드는 범위 밖).
- 뷰어: 프리셋 「🔩 금속 결합」 (비포화 응집 배위 ≫ B·전도 차폐가 보임)·verify 39 신설 + 전량 회귀. **주의**: ⑫ 카타펄트 교훈 — 응집 동역학과 새 개체의 적분 결합 취약성 사전 점검 (근-영 KE·강성 스캔).

**그 뒤 백로그 (순서 권장)**: ⑨① 관찰 프리셋(물리 diff 0·뷰어만: 자유 팽창·온도 구배) → ⑫ 발광 상시화 재도전(완화 근-영 KE 가드 또는 자발-채널 전용 — step-0038 격차) → ⑫ 급랭 재결합 발광(⑳ 연계) → Efield 노브(⑮ 배향 실험) → ⑳ 사건-적분 드리프트 정련(사건 시점 힘 재동기화). 완료: ⑧⑭⑮⑯⑰(법칙 스택)·⑳⑫(행+개체)·⑤⑥⑩⑬⑱㉕㉖+중력.
...
- **법칙 스택 부분 이관** (step-0033~34): 스택 승격은 ⑧ pol·⑭ angle·⑯ hb — ⑮ QEq·⑰ 용매화·⑳⑫ 는 playground 합류(step-0035~38)·⑲ 만 미합류 (§2 백로그·개체 추가라 별도 설계). hb·angle 상시 비용: 힘 평가마다 분자 라벨/수렴 이완(원자 200 상한 안 체감 0·㉑ 캐시 대상). 고정 시드 단일 런 assert 는 법칙 추가마다 궤적 갈라짐에 취약 — KERNEL §7 통계 이행 가치. 세부 step-0033.md.
