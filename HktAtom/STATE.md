...
- **S0-⑯ 수소 결합 (step-0016)**: 물 네트워크 창발. design 2단 접근 실행 — **1차 측정(새 물리 0)**: ⑮ 전하+⑭ 형상만으로 H-결합 방향성(θ~140°)·거리는 창발하나 **E_hb/D_OH≈2.5e-3** 로 목표(0.03~0.3)의 ~1/10(점전하 부족·design 예측) → **2차 R-HB 보정 행 명시 author**(C6 지위). **`hbond.js` 신설**(self-contained·엔진 ①–⑮ **diff 0**·⑮ 힘 뒤 합성): `V_hb=−D_hb·w(d)·(û_DH·û_HA)₊ⁿ`(공여체 선형성)·힘 −∇V_hb 3체(`F_H=−F_D−F_A`→**P·L 보존**)·U_hb→U_bond·분자간 쌍만. `detect`(검출 라벨·힘 아님). scenes s16-water-cluster/mixed + H-결합=청록 점선. `node verify.js`=**116 PASS**(①–⑮ 109 회귀 + ⑯ 7): **방향 선택성** ⟨θ⟩ 155°·**Ne 선택성**(Ne H-결합 0·물 27>0)·**E_hb 위계** 0.060∈(0.03,0.3)·**거리 위계** ⟨d⟩/d0 1.49∈(1.3,2.2)·**온도 응답** 배위 3.73(저T)>2.98(고T)·**물 네트워크 배위** 3.73∈(2.5,4.5)·장부 닫힘. 뷰어(눈 확인·콘솔 0): 청록 점선 네트워크·Ne 는 밖. 발견: 점전하만 부족→R-HB 명시(정직 보정)·고립쌍 검출만·얼음 격자·밀도역전 S1.
...
- **S0-⑬ z 해동 (step-0013)** — 요약(§5): `frozenZ:false` 하나로 3D·**엔진 변경 0**(①⑦ 자유도 회계). node verify.js **93 PASS**: z 해동 등분배 창발(iso 0→~1.0)·3D 장부·겹침 0·결합 위상. ⑦ 3D C_v 이월.
- **S0-⑫ 복사장 (step-0012) — Part II 시작** — 요약(§5): ④ 광자 빈 → **photon 입자**(runPhotonField·radiationMode='field' additive·회귀 0). node verify.js **88 PASS**: 복사 냉각·공동 정상 상태·스펙트럼 단색(2준위)·Σphoton.E 정확·유도 방출 3.04×(빔).
- **S0-⑪ 승격 배관 MVP (step-0011) — Part I 관문 닫힘** — 요약(§5): `promote.js`(coarse⇧·rethaw⇩·회귀 0) = 승격·해동 왕복. node verify.js **81 PASS**: 왕복 보존 정확(ΔE·P 1e-15·조성 ε=0)·**output.json v0**(S1 입력)·반데르발스 꼬리(−0.14).
- **S0-⑩ 수프 관문 (step-0010)** — 요약(§5): 실원소 합류 — **2H:1O → H₂O 우세 창발**·쌍별 D(436:463:146·회귀0)·annealSoup. node verify.js **74 PASS**: 어닐 H2O1:46>H2:26·과결합 0. 발견: H–H≈O–H→H₂ 경쟁·O=O 이중결합 부재.
- **S0-⑨ 통계 관문 (step-0009)** — 요약(§5): 평형 창발 — **새 물리 0**. measure entropy·equilibrium·localTemp. node verify.js **69 PASS**: 엔트로피 앙상블 증가 3.74→4.35·van't Hoff 기울기 1.89(≈D). 발견: van't Hoff 는 캐논ical → 항온조 필요.
- **S0-⑧ 분극·응집 (step-0008)** — 요약(§5): `polarization.js`(기반 ②만·회귀0)·전하–유도쌍극자+분산 C6(α·IE). node verify.js **64 PASS**: 중성 응집 창발(배위 1.65→2.41)·α=0→소멸(근원=α). 발견: SCF→쌍별 근사·미시정준 잠열 공존.
- **S0-⑦ 내부 모드·열용량 계단 (step-0007)** — 요약(§5): `modes.js`(강체 회전자+양자 모드·LB 교환). node verify.js **57 PASS**: C_v 1→3/2→5/2 계단 창발(순수 양자 효과). LB 편향 ⑨ 이월.
- **S0-⑥ 공유결합 (step-0006)** — 요약(§5): 결합 상태 기계·예산 B 포화·`runBonding`. node verify.js **52 PASS**: 이량체 H₂ 우세·원자가 포화(H₃=0)·안정화 필수·해리. 이핵 H₂O 는 ⑩ 쌍별 D 로 해결.
- **S0-⑤ 이온화·전자 이전 (step-0005)** — 요약(§5): R-XFER+쿨롱 격자 = **NaCl 앵커**(author 0)·자유전자 인프라. node verify.js **47 PASS**: Σc 불변·마델룽 E<0·교대 질서. 자유전자 이온화⇌재결합 ⑳ 이월.
- **S0-④ 전이 엔진 (step-0004)** — 요약(§5): 전이 카탈로그·두 시계·`checkedApply`(E·P≤1e-9). node verify.js **40 PASS**: **볼츠만 창발**·냉각·공동·저온 억제. LB 통합(잔여 ~10% ⑨ 이월).
- **S0-③ 준위·예산 (step-0003)** — 요약(§5): 전자 구조 **순수 함수**(`levels.js`·Aufbau·간이 Slater·훈트·예산 B·가상 4종). node verify.js **32 PASS**: **주기율표 창발**(원자가 1,2,3,4,3,2,1,0). 발견: 족 내림 Li>Na>K 미창발(간이 Slater·OPEN GAP).
- **S0-①② 무대·장부 + 힘·충돌 (step-0001·0002)** — 요약(§5): ① `engine/scenes/measure/verify.js`+`index.html` 뼈대(Vec3·통 분리 장부 10통·velocity Verlet·경계 3종·z동결·사건 큐·검증 하네스) 16 PASS. ② `pairForces`(쿨롱+척력·최소 이미지)·`U_elec`·`EPS_E=5e-4`/`DT_STIFF=0.004`/`MIN_DSIGMA=0.7` 고정·비리얼 압력 24 PASS(θ(b) 러더퍼드 단조·겹침 0). 이후 전 단계가 이 뼈대·불변식을 재사용.
...
## §2 NEXT — 다음 한 조각 (step-0017 = S0-⑰ 산·염기)

**⑯ 수소 결합 닫힘 — 물 네트워크 창발(배위 3.73·방향 선택성·온도 응답)·R-HB 정직 보정.** 두 갈래:
- **S1-① (열림)**: `output.json` 이 S1 입력으로 준비됨 — S1 분자 단계 착수 가능(design/11 경계: "S1 소비 테스트는 S1-① 몫").
- **다음 구현 = step-0017 = S0-⑰ 산·염기** (앵커: 물 자동 이온화 2H₂O⇌H₃O⁺+OH⁻ · [design/17-acid-base.md](stages/S0-atom/design/17-acid-base.md) · 전제 ⑯): ⑯ H-결합 네트워크 위에서 양성자 릴레이(H⁺ 이동)로 H₃O⁺·OH⁻ 창발(산·염기 평형). **로드맵(design README): ⑰(산·염기)→⑱(연소)…㉒(실 S1 입력 ⇧).** 세부 선택 자유(⑰ 또는 S1-① 착수) — STATE §2 가 확정.
...
- **점전하만은 H-결합 부족 → R-HB 명시 author** (step-0016): 1차 측정에서 방향성·거리는 창발하나 E_hb/D_OH≈2.5e-3 로 목표(0.03~0.3)의 ~1/10 → design 예약대로 방향 가중 R-HB(`V_hb=−D_hb·w·(û_DH·û_HA)ⁿ`)를 노브로 명시(C6 지위·D_hb=1.0 로 E·배위 튜닝·숨김 0). 수용체 고립쌍 정합은 검출 게이트만(힘 미포함·준정적 단순화). 얼음 결정·밀도역전(4°C)·양성자 이동은 S1/⑰.
