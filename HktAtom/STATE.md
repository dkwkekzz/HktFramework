...
- **S0-⑬ z 해동 구현 완료 (step-0013) — 3D 전환**: ①의 "차원=장면 속성" 청구서 — **`frozenZ:false` 하나로 3D 가 켜짐**을 증명. **엔진 변경 0**(scenes·measure·index.html 만·`engine.js`·`catalog.js` **diff=0** git 확인 — ①⑦에 심어둔 자유도 회계가 frozenZ 읽어 2/3 전환·이미 존재). scenes `latticePlace`·`buildCovalent` frozenZ 분기(해동 xyz 큐빅·회귀 0)+3장면(s13-gas/collide/bond-3d) · measure `momentumVariance` · index.html 3D 투영 뷰어(자체 행렬·마우스 회전·라이브러리 0). `node verify.js`=**93 PASS**(①–⑫ 88 회귀 + ⑬ 5): z 동결 증거(⟨p_z²⟩=0 vs ⟨p_x²⟩>0)·**z 해동 등분배 창발**(z 차갑게 iso 0 출발→3D 충돌이 분배→**iso 0→~1.0∈[0.8,1.2]**·`if(3d)` 0·창발)·3D 장부 닫힘·3D 겹침 0(min d/σ>0.7)·3D 결합 위상(C 다배위≥2·과결합 0). 뷰어(눈 확인·콘솔 0): 회전 와이어프레임·depth 크기/명도·3D 결합선. 발견: 엔진 변경 0 성립·⑦ 3D C_v 이월(modes.js 2D).
- **S0-⑫ 복사장 (step-0012) — Part II 시작**: ④ 광자 빈 → **photon 입자**. engine `runPhotonField`(R-ABS 흡수+R-STIM 유도 방출·결맞음)·`radiationMode='field'` additive(회귀 0). node verify.js **88 PASS**(⑫ 7): 복사 냉각 T 2.56→1.34(광자 open 탈출)·공동 정상 상태(광자 min 9>0·reflect 재흡수)·스펙트럼 단색(2준위→E=dE 한 빈)·Σphoton.E==E_photon통 정확·유도 방출 63 vs 자발 20.75(3.04×·빔 창발). 뷰어: s12-stim 축정렬 82% 빔.
- **S0-⑪ 승격 배관 MVP (step-0011) — Part I 관문 닫힘** — 요약(전문 §5): `promote.js`(coarse⇧·rethaw⇩·회귀 0) = 승격·해동 왕복. node verify.js **81 PASS**: 5막 한 장부 1.19e-4≤EPS_E·왕복 보존 정확(ΔE·P 1e-15·조성 ε=0·T비 1.28)·**output.json v0**(S1 입력)·반데르발스 꼬리(PMF O–O −0.14·author 0). 뷰어: ⇧다운로드·⇩왕복.
- **S0-⑩ 수프 관문 (step-0010)** — 요약(전문 §5): 실원소 합류 — **2H:1O 수프 → H₂O 우세 창발**(라벨 분기 0). **쌍별 결합 우물 D**(등방 ⑥ gap 해결): engine `pairD`+`bd.D`(Dbond fallback·회귀 0). 실원소 H·O·He·Ne(B/IE ③ 유도)+`Dpair`=실 비율 436:463:146·`annealSoup`(냉각·E_escape 회계). node verify.js **74 PASS**: 어닐 최다 H2O1:46>H2:26·과결합 0·어닐링 이득(46>크래시 33). 발견: H–H≈O–H → H₂ 실 경쟁자(어닐링 필요)·O=O 이중결합 부재. 뷰어: CPK·물 분자·라이브 어닐링.
- **S0-⑨ 통계 관문 (step-0009)** — 요약(전문 §5): 평형은 창발 — **새 물리 0**(measure/verify/scenes 초기조건만). measure `entropy`·`equilibrium` K_c·`localTemp` + s09-entropy-corner·gradient. node verify.js **69 PASS**: 엔트로피 앙상블 증가 ⟨S⟩ 3.74→4.35·르샤틀리에 해리 V 0.11→2V 0.23·van't Hoff 기울기 1.89(≈D)·T_국소 정합 rel 0.005. 발견: van't Hoff 는 캐논ical → 명시적 항온조 필요(⑥ 복사가 T 자체 조절). 뷰어: S(t) 앙상블 밴드·자유 팽창.
...
## §2 NEXT — 다음 한 조각 (step-0014 = S0-⑭ 형상 · 결합각)

**⑬ z 해동 닫힘 — frozenZ:false 만으로 3D·등분배 창발·엔진 변경 0.** 두 갈래:
- **S1-① (열림)**: `output.json` 이 S1 입력으로 준비됨 — S1 분자 단계 착수 가능(design/11 경계: "S1 소비 테스트는 S1-① 몫").
- **다음 구현 = step-0014 = S0-⑭ 형상** (앵커: H₂O 굽음·CO₂ 직선·CH₄ 정사면체 · [design/14-geometry.md](stages/S0-atom/design/14-geometry.md) · 전제 ⑥⑬): 같은 원자의 전자쌍 방향들 사이 **공통 반발 하나**로 분자 형상이 창발함을 증명(VSEPR 동역학). **목표각 author 금지** — 굽음·직선·정사면체가 전부 한 규칙에서 나와야. ⑬ 3D 무대 위에서 진행. **로드맵(design README): ⑭(형상)→⑮(극성)…㉒(실 S1 입력 ⇧).** 세부 선택 자유(⑭ 또는 S1-① 착수) — STATE §2 가 확정.
...
- **⑦ 3D C_v 재검 이월** (step-0013): `modes.js` 가 2D 하드코딩 자립 모듈이라 3D C_v(병진 3/2·선형 회전 2)는 재작성 필요 — "한 step=현상 1개"로 별도 이월(⑦ 3D 판 또는 ⑭). ⑦ 2D C_v 회귀 불변. 완전 CH₄ 우세도 어닐링(⑩ H₂ 경쟁) 몫 — ⑬은 3D 무대에서 ⑥ 공유 기계 도는지(다배위·과결합 0)만.
...
- step-0013: S0-⑬ z 해동 — 3D 전환. **엔진 변경 0**(engine/catalog diff=0·①⑦ 자유도 회계 덕). scenes latticePlace·buildCovalent frozenZ 분기(3D 큐빅·회귀0)+s13-gas/collide/bond-3d + measure momentumVariance + index.html 3D 투영 뷰어(자체 행렬·마우스 회전). node verify.js 93 PASS. z 동결 증거(⟨p_z²⟩=0)·z 해동 등분배 창발(iso 0→~1.0·z 차갑게 출발→충돌 분배)·3D 장부 닫힘·3D 겹침 0·3D 결합 위상(C 다배위·과결합 0). 발견: 엔진 변경 0 성립·⑦ 3D C_v 이월(modes.js 2D). 뷰어: 회전 와이어프레임·depth 크기/명도·3D 결합선.
- step-0012: S0-⑫ 복사장 — Part II 시작. ④ 광자 빈 → photon 입자. engine.js photon{E,r,dir,t0}·c_ph·propagatePhotons(경계 photonBC 독립)·runPhotonField(R-ABS 흡수+R-STIM 유도 방출·결맞음 복제)·radiationMode='field' additive(회귀0) + scenes.js s12-open-cooling·cavity·stim + measure.js photonStats·photonSpectrum + verify ⑫ 7종. node verify.js 88 PASS. 복사 냉각 T 2.56→1.34(광자 open 탈출)·공동 정상 상태(광자 min 9>0·reflect 재흡수)·스펙트럼 단색(2준위→E=dE 한 빈)·Σphoton.E==E_photon통 정확(gap 0)·유도 방출 증폭 63 vs 자발 20.75(3.04×·빔 창발). 발견: 2준위→단색(연속 스펙트럼 다준위 몫)·field/bin 공존(E_photon bond sink 회귀 회피). 뷰어 3장면: 광자=노란 점+꼬리·s12-stim 축정렬 82% 가로 빔.
