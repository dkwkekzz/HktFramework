...
> **법칙 스택 = 규칙은 세계 속성이 켠다 (step-0033 · 게이트 5차)**: 사용자 요구("규칙은 어떤 scene/playground 든 작동해야") → 중력 패턴 일반화: engine `registerLaw({name,rank,active,force})`/`stackForces`(기반 pairForces + rank 순 법칙 기여 — 더하기만·법칙 통 초기화). **게이트 = 물리 입력(종 파라미터) 존재 · 부재 = 기여 0 이 참값**(g=0 동형 → 기존 장면 회귀 0). 이관: ⑧ pol(`alpha`)·⑯ hb(`Dhb`) — 동등성 max|ΔF| 0.0e+0 ≤ 1e-12 · 게이트 정확 0. hbond 종 게이트 개방(`hbDon`/`hbAcc` 세계 속성 — playground Z=최외각 관례와 공존·`O.Z=8` 핵 제거로 이온화 회계 충돌 해소). playground=스택 소비자: `Dhb 0.9`·`hbAcc {O}` 상시 탑재·`enableHBond` 힘 교체 삭제(관찰 노브만). **앵커: 기본 샌드박스(카탈로그 3행 ON·전환 0)에서 H-결합 10개 창발·잔차 -2.0e-3·Σc 정확** — 뷰어: water 프리셋에서 창발 H₂O 사이 청록 점선 자동. KERNEL §3.1 무대 독립 원칙 등재·DESIGN §3.2 법칙 스택. verify 33 신설 4 PASS·공유창발 9000틱 연장(8시드 hb ON 8/8 통계 정당화)·**전량 190 PASS·0 FAIL**. 세부 step-0033.md.
