...
> **법칙 스택 = 규칙은 세계 속성이 켠다 (step-0033 · 게이트 5차)**: 사용자 요구("규칙은 어떤 scene/playground 든 작동해야") → 중력 패턴 일반화: engine `registerLaw({name,rank,active,force})`/`stackForces`(기반 pairForces + rank 순 법칙 기여 — 더하기만·법칙 통 초기화). **게이트 = 물리 입력(종 파라미터) 존재 · 부재 = 기여 0 이 참값**(g=0 동형 → 기존 장면 회귀 0). 이관: ⑧ pol(`alpha`)·⑯ hb(`Dhb`) — 동등성 max|ΔF| 0.0e+0 ≤ 1e-12 · 게이트 정확 0. hbond 종 게이트 개방(`hbDon`/`hbAcc` 세계 속성 — playground Z=최외각 관례와 공존·`O.Z=8` 핵 제거로 이온화 회계 충돌 해소). playground=스택 소비자: `Dhb 0.9`·`hbAcc {O}` 상시 탑재·`enableHBond` 힘 교체 삭제(관찰 노브만). **앵커: 기본 샌드박스(카탈로그 3행 ON·전환 0)에서 H-결합 10개 창발·잔차 -2.0e-3·Σc 정확** — 뷰어: water 프리셋에서 창발 H₂O 사이 청록 점선 자동. KERNEL §3.1 무대 독립 원칙 등재·DESIGN §3.2 법칙 스택. verify 33 신설 4 PASS·공유창발 9000틱 연장(8시드 hb ON 8/8 통계 정당화). 세부 step-0033.md.
> **중력 = 엔진 법칙 (step-0032 · 게이트 4차)**: "규칙이니 장면에도 저절로 적용돼야" → 엔진 세계 속성: makeWorld `g`/`gDir`(기본 0=원자 규모 참값)·어느 computeForces 든 적용·`U_grav` 통·reflect1 ΔU 상계 반사(침강 더미 랜덤워크 −0.4→±0.07)·KERNEL §3.1 등재. author=**F=m·g·ĝ 하나** — 성층 창발 3/3 (T=1.7) [Xe 2.9·He 4.1·H 9.4]·g=0 대조 11.9·자유낙하 ≤5.3e-12·순수 장면 낙하·3D=−y. playground=소비자(setGravity 토글 회계). 규모 투명(질량=Σc·차폐 0→체크 ⑤ 자명 충족)·자기 중력=게이트. 5 PASS·전량 186.
...
## §2 NEXT — 다음 한 조각 (step-0033 = S1-② 응집·액적 재개)
...
- **법칙 스택 잔여 승격** (step-0033 후속·수요 시): ⑮ QEq(전하 갱신 단계 슬롯 + ⑤ 정수 이전과 전하 회계 통일 선행)·⑭ 각도(lones 동적 관리)·⑰ 용매화 → 그 뒤 ⑲⑳⑫(법칙 아닌 *개체 추가* — 별도 설계). 패턴은 step-0033 확립 (게이트=물리 입력 존재·동등성 ≤1e-12·전량 회귀).

**산출물 2 트랙**: `output.json` v0.3(㉒ 화학 MaterialModel·S1 입력) + `nuclide-table.json`(㉕ 핵 파라미터·서버 중간 해상도 소비). 커널 가설 실증은 S1-④ 규모 정합 관문.

## §3 OPEN GAPS — 열린 격차

- **법칙 스택 부분 이관** (step-0033): 스택 승격은 ⑧ pol·⑯ hb 만 — ⑮ QEq·⑭ 각도·⑰ 용매화·⑲⑳⑫ 미승격 (§2 백로그·⑲⑳⑫ 는 개체 추가라 별도 설계). hb 상시 비용: pairs() 가 힘 평가마다 분자 라벨 재계산(원자 200 상한 안 체감 0·㉑ 캐시 대상). 고정 시드 단일 런 assert 는 법칙 추가마다 궤적 갈라짐에 취약(공유 창발 9000틱 연장·8시드 통계로 현상 확인) — KERNEL §7 통계 이행 가치. 세부 step-0033.md.
...
- step-0033: 법칙 스택 — 규칙은 세계 속성이 켠다 (게이트 5차·새 물리 0). engine registerLaw/stackForces(기반+rank 순 기여·게이트=물리 입력 존재·부재=기여 0 참값). ⑧⑯ 이관(동등성 ΔF 0.0·게이트 정확 0)·hbond 종 게이트 개방(hbDon/hbAcc·O.Z=8 핵 제거)·playground 스택 소비자(Dhb·hbAcc 상시·enableHBond 힘 교체 삭제)·KERNEL §3.1 등재. 앵커: 기본 샌드박스(카탈로그 ON)서 H-결합 10개 창발·잔차 -2.0e-3. verify +4·공유창발 9000틱(8시드 8/8). 잔여: ⑮⑭⑰⑲⑳⑫ 미승격·hb 상시 비용·단일 시드 취약. 세부 step-0033.md.
