...
- **S0-⑫ 복사장 구현 완료 (step-0012) — Part II 시작**: ④의 광자 빈 근사를 **photon 입자**로 교체 — 방향·공간 가진 복사장. `engine.js` 에 `photon={E,r,dir,t0}`·`c_ph`(전파 속도 노브 ≈10·v_th·실 c 아님·위계만)·`randDir`(등방)·`propagatePhotons`(직진·경계 `photonBC` **원자와 독립**: open→탈출·reflect→반사·periodic→랩)·`runPhotonField`(**R-ABS 흡수**+**R-STIM 유도 방출**·같은 dir 결맞음 복제) 추가. `radiationMode='field'` **additive**(bin 경로 보존 → ④ s04 3장면·⑥⑩⑪ bond 복사 sink 회귀 0·⑩ pairD fallback 동형·design/12 "빈 제거" 이 정합으로 갱신). `scenes.js` 3장면(s12-open-cooling·s12-cavity·s12-stim) + `measure.js`(photonStats 이방성·photonSpectrum). `node verify.js`=**88 PASS**(①–⑪ 81 회귀 + ⑫ 7): **복사 냉각** T 2.56→1.34·E_escape(광자 open 탈출)·max|ΔE|/E 2.76e-4≤EPS_E(dt=DT_STIFF — 고T 표류 회피) · **공동 정상 상태** 광자 13,19,9,9,12(min 9>0·reflect 갇힘·재흡수)·rel 8.2e-5 · **스펙트럼 단색**(2준위→모든 광자 E=dE 한 빈·연속 스펙트럼·플랑크 꼬리는 다준위 몫·정직 한계) · **장부 Σphoton.E==E_photon통 정확**(gap 0·1200 tick·입자↔통 정합) · **유도 방출 증폭** 축 정렬 광자 유도 63 vs 자발 대조 20.75(**3.04×**·씨앗 방향 결맞음 복제 — 빔 창발·`if(laser)` 0·4런). 뷰어 3장면(눈 확인 완료·콘솔 0): 광자=노란 점+꼬리·s12-stim "축정렬 82%" 가로 빔·s12-cavity 들뜬 원자 후광+갇힌 광자·s12-open-cooling T 하강.
- **S0-⑪ 승격 배관 MVP (step-0011) — Part I 관문 닫힘**: `promote.js`(coarse⇧·rethaw⇩·pmf·buildOutput/validate·self-contained·회귀 0) = KERNEL §3.3·CONTRACT §3 승격·해동 왕복. 5막 시나리오(형성→응집→가열→반응→냉각) 한 장부 max|ΔE|/E 1.19e-4≤EPS_E·왕복 보존 정확(ΔE·P 1e-15·Σc 3/3·**조성 계약 ε=0**·T비 1.28 재표본)·**output.json v0**(7종·2쌍·S1 입력)·**반데르발스 꼬리**(PMF O–O −0.14·α·IE 에서·author 0). 뷰어: 5막 진행·⇧다운로드·⇩왕복 버튼.
...
## §2 NEXT — 다음 한 조각 (step-0013 = S0-⑬ z 해동 · 3D 전환)

**⑫ 복사장 닫힘 — 광자 입자로 냉각·공동·유도 방출 실증.** 두 갈래:
- **S1-① (열림)**: `output.json` 이 S1 입력으로 준비됨 — S1 분자 단계 착수 가능(design/11 경계: "S1 소비 테스트는 S1-① 몫").
- **다음 구현 = step-0013 = S0-⑬ z 해동** (앵커: 3D · [design/13-z-thaw.md](stages/S0-atom/design/13-z-thaw.md) · 전제 ①): `frozenZ` 해제만으로 3D 가 켜짐을 증명 — 차원이 코드가 아니라 장면 속성이라는 ①의 설계 결정에 대한 청구서. **엔진 변경 0**(scenes·measure·index.html 만·engine/catalog diff 0 명기 — 위반 = ① 회귀 재설계 사건). ⑭ 형상(정사면체)의 전제. **로드맵(design README): ⑬(3D)→⑭(형상)→⑮(극성)…㉒(실 S1 입력 ⇧).** 세부 선택 자유(⑬ 또는 S1-① 착수) — STATE §2 가 확정.
...
- **2준위 → 단색 스펙트럼 · field/bin 공존** (step-0012 발견): ⑫ 광자 입자는 냉각·공동·유도 방출을 재현하나 2준위라 모든 광자 E=dE → 스펙트럼 단색. 연속 스펙트럼·플랑크 꼬리(자외 파탄)는 다준위/진동-복사 결합 몫(㉒ 또는 별도). 복사압·산란·편광 범위 밖(광자 p 회계 생략). `E_photon` 통이 bond 복사 sink(⑥/⑩/⑪)로도 쓰여 빈 제거가 회귀를 건드림 → `radiationMode='field'` additive(⑩ fallback 동형·회귀 0·field 장면 E_photon==Σphoton.E)·design/12 "빈 제거"는 이 정합으로 갱신.
