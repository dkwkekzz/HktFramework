...
> **⑲ 금속 합류 = 비포화 응집·전자 바다 (step-0039 · 게이트 11차·개체 추가형 3/3 · playground 합류 완결)**: playground 🔩 프리셋에서 금속(Na) 이온이 방향성 없는 유효 우물로 조밀하게 쌓여 **비포화 응집**(내부 배위 9.38 ≥ 8 ≫ 원자가 B=1·공유 포화의 반대)하고 그 사이를 **자유전자 개체(전자 바다)**가 로밍한다(이동성 RMS ×4.7 ≫ 이온). **힘 모델 전환**: 금속 결합은 스택에 얹는 힘이 아니라 유계 regime(부드러운 쿨롱+이온-이온 유효 우물 Dmetal·고전 점전자 catapult 봉쇄·metal.js·design §9.2 author) — `enableMetal(🔩)` 이 computeForces 를 통째로 교체+화학 카탈로그 끔(화학 스택과 공존 불가·enableHBond 지위·프리셋 전용·dt 0.003). 대조: 금속 미활성 stackForces → Na 배위 1.0. 뷰어 주황 링 +1 이온 격자+청록 전자 바다. verify 39 신설 3 PASS·**전량 210 PASS·0 FAIL**. 세부 step-0039.md. **→ ⑤⑥⑧⑩⑬⑭⑮⑯⑰⑱⑲⑳⑫+중력·핵 전 현상 관찰 가능 (합류 완결).**

> **⑫ 복사장 합류 = 발광하는 기체·광자 개체 (step-0038 · 게이트 10차·개체 2/3 · 요약)**: playground 💡 프리셋 가열된 수소가 발광 — 충돌 들뜸(R-COL)→자발 방출(R-EMI)→노란 광자 개체(질량 0·방향·전파·경계). 앵커 E_escape T0.3 3.5→T3.0 26.3(×7.5)·open 냉각 vs reflect 공동. specLevels(dE=clamp(0.75·IE,0.4,3.0)·③ 유도) base 상시. **발견**: 상시 합류 불가 — 완화가 근-영 KE 에 dE 쏟는 카타펄트(잔차 +9/사건) → enableRadiation(💡) 프리셋 전용·자발 방출은 KE 불변 안전. verify +3. 세부 step-0038.md.

> **⑳ 플라스마 합류 = 자유전자 개체 (step-0037 · 게이트 9차·개체 1/3 · 요약)**: R-ION/R-REC3 행 playground 상시 합류(문턱=IE 에너지 가드→저온 잠김=참값)·specIon ⑤ 공유. 전자 개체(tick 강성·항온조·chargeOK Σq=n_e·m_e 0.5)·**전자=순수 연화 쿨롱**(eps_e 0+soft_e — catapult 1e32 근절). 앵커 ⟨x⟩ 0.57→0.81(T0.5→3)·급랭 재결합 0.83→0.25. 격차: 사건-적분 드리프트 ~1e-2/사건(허용 ≤6). 통제 2건 nu_ion=0. verify +3. 세부 step-0037.md.
...
**playground 합류 완결 (step-0029~0039)**: ⑤⑥⑧⑩⑬⑭⑮⑯⑰⑱⑲⑳⑫ + 중력·핵(㉕㉖) 전 현상이
관찰 가능하다. 남은 것은 **새 현상 합류가 아니라 정련·관찰 프리셋**뿐 (아래 백로그).

**다음 백로그 (순서 권장·전부 정련/관찰 — 새 법칙 0)**:
- **⑨① 관찰 프리셋** (물리 diff 0·뷰어만): 자유 팽창·온도 구배(엔트로피 증가·확산 눈 확인) — 가장 가벼움.
- **⑫ 발광 상시화 재도전** (step-0038 격차): 완화 근-영 KE 가드 또는 자발-채널 전용으로 카타펄트 봉쇄 → 상시 합류.
- **⑫ 급랭 재결합 발광** (⑳ 연계): R-REC3 재결합 시 광자 방출 (플라스마 식으며 빛).
- **Efield periodic 전도** (⑲⑮ 격차): periodic 상자 전도 프리셋 (정량 드리프트 속도·배향 실험).
- **⑳ 사건-적분 드리프트 정련** (step-0037): 사건 시점 힘 재동기화.

완료: ⑧⑭⑮⑯⑰(법칙 스택)·⑳⑫(행+개체)·⑲(힘 모델 전환)·⑤⑥⑩⑬⑱㉕㉖+중력.
...
- **⑲ 합류 잔여** (step-0039): 힘 모델 전환이라 화학 스택과 공존 불가(enableMetal 프리셋 전용·상시 스택 아님)·고전 유효 모델(Dmetal=Thomas-Fermi 차폐 축약·design §9.2·집단 플라스마·밴드·페르미 통계·자성 밖)·전도 정성만(전자 이동성 ≫ 이온·정량 전도는 이온이 reflect 벽으로 밀려 오염→periodic 필요)·dt 0.003 국소화(경량 실원소 이온 안정·타 프리셋 0.004 불변). 세부 step-0039.md.
- **⑫ 합류 잔여** (step-0038): 상시 미합류(설계 이탈) — 충돌 완화가 근-영 상대 KE 에 dE 쏟는 카타펄트가 결합 동역학 적분 폭발(잔차 +9/사건) → enableRadiation(💡) 관찰 프리셋 전용(근본 해소=완화 근-영 KE 가드 또는 자발-채널 전용·후속). 첫 들뜸 dE=clamp(0.75·IE,0.4,3.0)=수소형 Rydberg 프록시(알칼리 실비~0.4·IE·다준위·분광 정합 주장 없음·2준위 단색). 광자 P 미회계(|p|=E/c 생략·복사압 밖·_auditP=false·E 는 통 정확). ⑳ 급랭 재결합 발광 미구현(R-REC3+R-EMI 연계·후속). 세부 step-0038.md.
...
- **법칙 스택 부분 이관** (step-0033~34): 스택 승격은 ⑧ pol·⑭ angle·⑯ hb — ⑮ QEq·⑰ 용매화·⑳⑫ 는 playground 합류(step-0035~38)·⑲ 는 힘 모델 전환으로 합류(step-0039·스택 불가 regime). **전 현상 합류 완결**. hb·angle 상시 비용: 힘 평가마다 분자 라벨/수렴 이완(원자 200 상한 안 체감 0·㉑ 캐시 대상). 고정 시드 단일 런 assert 는 법칙 추가마다 궤적 갈라짐에 취약 — KERNEL §7 통계 이행 가치. 세부 step-0033.md.
...
- step-0039: ⑲ 금속 합류 (게이트 11차·개체 3/3·**playground 합류 완결**). enableMetal(🔩)=computeForces 를 금속 힘 모델(유계 쿨롱+이온-이온 유효 우물 Dmetal·metal.js)로 교체(화학 스택과 공존 불가·별도 regime·프리셋 전용)·전자 풀 개체·dt 0.003. 앵커 비포화 응집 배위 9.38 ≥ 8 ≫ B=1·전자 이동성 ×4.7 ≫ 이온·대조 stackForces Na 배위 1.0. 뷰어 주황 링 이온 격자+청록 전자 바다. verify +3·전량 210. 세부 step-0039.md.
- step-0038: ⑫ 복사장 합류 (게이트 10차·개체 2/3). specLevels 상시 탑재(첫 들뜸 dE=clamp(0.75·IE,0.4,3.0)·③ IE 유도·g0/g1=1/3)+enableRadiation(💡) 프리셋(R-COL 자유충돌 들뜸+field 광자). 발견: 상시 합류 불가 — 완화가 근-영 KE 에 dE 쏟는 카타펄트(잔차 +9/사건·물형성 +26.7) → 관찰 프리셋 전용(enableHBond 지위)·자발 방출은 KE 불변 안전. 앵커 E_escape T0.3 3.5→T3.0 26.3(×7.5)·open 냉각 vs reflect 공동. 뷰어 노란 광자+들뜸 후광+Nγ. verify +3·전량 207. 세부 step-0038.md.
