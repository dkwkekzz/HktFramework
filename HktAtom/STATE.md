...
> **관찰자 샌드박스 (step-0029~0031 · 사용자 게임 프로토타입 게이트 · 새 물리 0)**: `playground.js/html`(엔진 diff 0). **0029**: levels 6·7주기 가법 연장(Z=1~118 ③ 유도·Z≤54 불변)·종 118·이온 역할 유도·폴링 식 이핵 D·②⑤⑥⑧⑩ 합성·**주입 장부**(소환·가열=회계된 외부 원천 → 세계 총량−주입≈0). **0030**: 2D/3D·항온조+T/P 스파크라인·연소 ⑱ 합류·**핵분열 인월드 판**(U+n→Ba+Kr+2n·E_nuclear 저장→실측 인출·pgConv·ν=2 연쇄)·**복셀 장**(국소 T 열지도). **0031**: 반응 체감 — EA_CAP 2.5(Na+물 격렬)·분열 Q150·**융합 H+H→He+n**(중성자 고속 KE→U 분열 촉발 커플링)·적응 서브스텝+도구 VCAP·잔차 상대화. --only 29,30,31 = 12 PASS·브라우저 눈 확인.
...
- **계획 국면 (step-0000)**: 네 축 확정(CLAUDE·KERNEL·CONTRACT·DESIGN+design/①~㉖).

## §2 NEXT — 다음 한 조각 (step-0032 = S1-② 응집·액적 재개)

**핵분열(㉕)·관찰자 샌드박스(step-0029~0031) — 사용자 요구 충족. 사다리 전진으로 복귀.** :
...
- **샌드박스 고Z 경향만·부분 합성·핵 축약·체감 노브** (step-0029~0031): 간이 Slater 고Z IE/EA 과대 → χ/EA 클램프(실세계 정합 주장 없음)·EA_CAP 2.5=체감 노브(⑤ 앵커 이탈·Na+물=전자 이전 발열 추상). 동핵 D=예산 프록시(H·O 만 실비 앵커). 금속⑲/플라스마⑳/수소결합⑯/산염기⑰ 미합류. 분열=단일 단면·즉발(밴드·감속·k_eff 는 ㉕)·붕괴㉔ 미연결·융합=하드 게이트(Gamow ㉖)·실 핵종 경로 아님·중성자 τ 소멸=처분 회계. 서브스텝 잔차=에너지 비례 드리프트(상대 ~1e-3 허용). 관찰자=표현층·복셀 장=온도만·원자 200 상한(㉑ 몫). 세부 step-0029~0031.md.
...
- **응집이 액적+증기 공존에 머묾** (step-0008): 잠열 자체 가열. 상 분리는 S1. SCF→쌍별 근사.
...
- **어닐링 항온조=측정 도구** (step-0010): 열→E_escape 회계.
- **재해동 T 재표본 이동** (step-0011): 왕복 Σc·E·P 정확·미시 재샘플 T~1.3배. 실 S1 입력은 ㉒ 완결본.
- **점전하만은 H-결합 부족→R-HB 명시** (step-0016): 얼음·밀도역전은 S1. 세부 step-0016.md.
