...
## §2 NEXT — 다음 한 조각 (step-0009 = S0-⑨ 통계 관문)

**다음 구현 = step-0009 = S0-⑨ 통계 관문** (앵커: 평형은 창발한다 · [design/09-statistics-gate.md](stages/S0-atom/design/09-statistics-gate.md)): **새 물리 0 — 측정·검증만 더한다**(measure.js·verify.js). ① coarse-grained 엔트로피 S=−Σpᵢln pᵢ 의 **앙상블 증가 경향**(R런 평균·개별 요동 허용·저엔트로피 초기 배치) + 셀 2배 스캔 의존성 정직 ② K_eq 측정(V1+V1⇌V1₂): 부피 2배 → 해리 증가(르샤틀리에)·온도 스캔 van't Hoff `ln K vs 1/T` 기울기 ≈ D(⑥ author 값 일치) ③ T_국소 근사 한계·수치 불변식 전 장면 일괄. 닫는 기준: 엔트로피 앙상블 증가·K_eq 상태 수 의존 2종·van't Hoff D 일치(허용 창). **⑦·④·⑧에서 이월한 "LB ~10% 편향·T_국소 비평형 한계"를 여기서 정면 검증.**
