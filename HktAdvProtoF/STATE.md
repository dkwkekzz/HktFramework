...
### [O0] 세계관 공리
- 카드 상세는 착수 시 MODULES.md O0 행으로부터 작성.
- O2 가 남긴 입력: `DOMAIN_RECONCILIATION` 의 근거 두 줄이 O0 공리를 이미 인용한다
  (생명은 의념을 발생시킨다 · 모든 능력은 관찰 가능한 흔적을 남긴다) — O0 은 그 공리를
  값으로 세우고, 9영역 자리 중 공리를 어기는 정의를 거부해야 한다.
- O2 가 남긴 자리: `psychic.trace.{rule}` 은 "흔적 없는 능력은 세계에 설 수 없다" 를 자리로만
  적어 두었다. 그것을 강제하는 것은 O0 이다.
