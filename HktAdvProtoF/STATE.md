...
#### [S0-c] 의존·유지 자리
- 목적: 주체가 무너지지 않으려 지키는 자리(의존)와 밀고 가려는 자리(유지)를 O2 의 실재하는 자리로 못박는다.
- 입력: `Need[]`, `ValueTarget[]`, `Boundary[]`, `StateSchema`(O2)
- 출력: `SubjectViolation[]` (없는 자리 · 경계 밖 붕괴 조건 · 값 모양과 어긋난 범위 · 무너지지 않는 의존)
- 검증 장면: 사냥꾼의 허기 의존은 자기 안에서 서고, 행상의 신뢰를 원하는 것은 경계 밖이어도 선다. 남의 허기를 자기 붕괴 조건으로 삼으면 걸린다.
- 상태 원소: `Need`, `ValueTarget`, `Band`
- 시각화: diff(주체 카드 — 의존·유지 자리표)

#### [S0-d] 주체 골격
- 목적: 경계·감지·의존·유지·능력을 한 주체 프로필로 합치고, 인용한 능력과 태어난 종이 어긋나면 거부한다.
- 입력: `SubjectSpec`, `Definition[]`(O0 능력·종), `StateSchema`(O2)
- 출력: `SubjectProfile`, `SubjectReport`(선 주체 / 막힌 주체 + 사유)
- 검증 장면: 사냥꾼 한 명이 다섯 조각을 갖추고 서고, 공리를 어긴 능력을 붙이거나 종이 열지 않은 자리로 무너진다고 적으면 거부된다.
