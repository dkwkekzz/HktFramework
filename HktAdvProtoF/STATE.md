...
- S1 이 남긴 자리: `LifeStage.opens` 는 종이 여는 능력만 다룬다. 문화·역할이 능력을 더하거나
  막는다면(입문 의례·금기) 그 자리는 S2 가 연다.
- S1 이 남긴 부채: 단계별 개체를 실제로 낳는 것은 아직 S1 장면뿐이다. 개체를 시간 위에 올려
  `stageAt(생후 틱)` 으로 단계를 갱신하는 일은 S3(개별 주체 생성) 이후 D·R 계층의 몫이다.
...
**단계 1 (M1 의존하는 주체) 진행 중** — S0·S1 이 VERIFIED 로 닫혔고 레지스트리가 위상 순서
`V1 → V2 → V0 → V4 → V3 → O1 → O2 → O0 → S0 → S1` 으로 등록한다. 남은 것은 S2·S3 · D0~D4.
