...
### [O0] 세계관 공리
- 목적: 세계에 어떤 존재와 현상이 허용되는지 공리로 정의하고 위반하는 정의를 거부한다.
- 입력: `AxiomSpec[]`(원문 세 목록), 정의(능력·종)
- 출력: `AxiomSet`, `AxiomViolation[]`, `EnforcementReport`, `DerivationReport`
- 검증 장면: 붉은 장막 세계의 정의 7개(능력 3 + 종 4)가 공리를 근거로 서고, 결함 정의 12종이 각자의 공리에 걸린다.
- 상태 원소: `Axiom`, `Definition`, `AxiomViolation`
- 시각화: Lab `/o0` — 원문 세 목록 대조표 · 공리별 강제 지점 · 정의 투입 → 통과/거부 판정
- 하위 작업 5개로 분할 (WORKFLOW §3 — 상태 원소 3종 초과 · 검증 장면 2개 초과):

#### [O0-a] 공리 확정
- 목적: 원문이 세 곳에 나눠 적은 공리·불변 규칙을 대조해 하나의 공리 집합으로 확정한다.
- 입력: ModulePlan O0 예시 4 · MasterPlan §3.1 `universalInvariants` 4 · §3.2 1계층 절대 불변 규칙 4
- 출력: `AXIOM_SET`(확정 공리) + `AxiomSetReport`(해소되지 않은 원문 문장·근거 없는 공리)
- 검증 장면: 원문 12문장이 하나도 빠짐없이 확정 공리로 해소되고, 해소 하나를 지우면 그 문장이 지목된다.
- 상태 원소: `Axiom`(O1 Rule 의 확장 — 근거가 자기 자신인 규칙)
- 시각화: Lab 대조표 (원문 문장 → 해소 → 확정 공리)

#### [O0-b] 정의 검사
- 목적: 세계에 들이려는 능력·종 정의가 공리를 어기면 사유와 경로로 거부한다.
- 입력: `Definition`(능력 또는 종), `AXIOM_SET`, O2 `STATE_SCHEMA`
- 출력: `AxiomViolation[]`
- 검증 장면: 흔적 없는 능력·공짜 대능력·의념 없는 생명·유래 없는 신이 각자의 공리에 걸린다.
- 상태 원소: `Definition`, `AxiomViolation`
- 시각화: Lab 표 (정의 투입 → 걸린 공리 · 사유 · 경로)

#### [O0-c] 강제 지점 프로브
- 목적: 공리마다 지금 그것을 실제로 강제하는 지점이 있는지 실행해서 확인한다.
- 입력: `AXIOM_SET`, O1·O2 의 관문 함수
- 출력: `EnforcementReport`(공리별 프로브 통과 여부 · 아직 아무도 강제하지 않는 공리 · 갚을 모듈)
- 검증 장면: 공리 8개 중 6개가 실제 관문에서 거부를 만들어 내고, 2개는 미강제로 갚을 모듈과 함께 남는다.
- 상태 원소: `EnforcementPoint`
- 시각화: Lab 표 (공리 → 강제 지점 → 프로브 결과)

#### [O0-d] 도출 대조
- 목적: 같은 공리에서 서로 다른 여러 정의가 도출되는지 센다 (원문 O0 검증 조항 ②).
- 입력: `AXIOM_SET`, 정의 카탈로그
- 출력: `DerivationReport`(공리별 도출 정의 · 아무것도 낳지 못한 공리)
- 검증 장면: 공리 하나에서 능력 둘 또는 신 둘이 도출되고, 도출 하나를 지우면 그 공리가 불모로 찍힌다.
- 상태 원소: `Derivation`
- 시각화: Lab 표 (공리 → 도출된 정의들)

#### [O0-e] 눈 검증
- 목적: 시나리오 3종과 Lab `/o0` 로 O0 를 눈으로 확인하고 증거로 닫는다.
- 입력: O0-a~d 의 산출물
- 출력: `evidence/O0.json`, Lab `/o0`
- 검증 장면: 정상·실패·경계 3종이 통과 표로 나오고 브라우저에서 7요소가 열린다.
- 상태 원소: (새로 정의하지 않음)
- 시각화: Lab `/o0` 페이지 전체

## 남은 공용 렌더러 (WORKFLOW §6)

diff 뷰는 구현됐고, 그래프·게이지는 최소판이 V0·V4·O2 페이지에 있다 (O2 는 세계 트리를 표로 편다).
...
현재 V0~V4 · O1 · O2 가 VERIFIED — 단계 0 에 남은 것은 O0 뿐이다.
