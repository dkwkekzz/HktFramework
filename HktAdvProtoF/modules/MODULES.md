...
| ①-정보 (능력 = 정보전의 표면 — [북극성 검토](../reviews/northstar-critique.md) 격차 A) | **G5(정보 표면 — 노출/은닉/오인/봉쇄 + 계통 간 적성)** → R2·R3(노출의 현상화·선택 감지) → R4(상대의 믿음·오인) → **E3(판정 입력 확장 — `EffectMagnitude` + 상대 `BeliefGraph` 절편 + 준비·환경)**. 계약 문안은 아래 G5·E3 행에 반영됨 — 구현·검증은 해당 계층 착수 시 |
...
**원리 이질성**(미지의 위험 = 다른 공리의 노출, 전투력 환산 불가)으로 다시 정의한다 (검토 §2.6 지적).

**스코프 주의** — ①-정보·②-장르는 콘텐츠별 의무가 아니라 **퇴화를 막는 체계 하한**이다:
정보 표면은 "모든 능력에 오인 유도를 설계하라" 가 아니라 "노출·은닉이 무엇인지 선언하라"(없으면
없다고)이고, 무대는 "모든 지역이 무대여야 한다" 가 아니라 "세계가 무대를 세울 수 있어야 한다" 다.
자기 고리가 아닌 모듈 작업에는 요구하지 않는다.

**최종 검증 3문** ([북극성 검토](../reviews/northstar-critique.md) 편입) — **콘텐츠
산출물 전용** 관문이다. 모듈·계약 일반에는 적용하지 않는다 (V·D 계층 작업이 "능력 검증" 을
통과할 수는 없다):

- **능력 검증** (G5·A2 산출물) — 소유자의 정체성과 삶의 방식을 드러내면서도 공통 법칙으로
  설명되고, 상황·정보·상성·대응에 따라 결과가 달라지는가?
- **세계 요소 검증** (W1·A2 산출물) — 여러 주체에게 서로 다른 의미를 가지며, 독립된 목적들의
  충돌로 새로운 관계와 사건을 만들어내는가?
- **캐릭터 검증** (S3·C 계층·A2 산출물) — 플레이어와 무관하게 자신의 목적을 추구하며,
  능력·관계·선택으로 다른 주체와 세계의 상태를 실제로 변화시키는가?
