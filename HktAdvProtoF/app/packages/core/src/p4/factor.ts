 ...
 *
 * 무게는 원문이 적지 않았다 — 원문이 준 것은 **나열 순서**뿐이고, 그 순서는 무게의
 * 단조 규정이 아니다(단계 2 리뷰 #674 의 판정). 무게는 **P4 의 선언**이며, 나열 순서와
 * 어긋나는 자리 둘은 의도다: 가치관(0.6)이 비용·위험(0.5)보다 무겁고 — 같은 굶주림
 * 앞에서 사람이 갈리는 자리가 값싼 셈보다 앞서야 한다 — 매몰(0.4)이 관계·기억·약속(0.3)
 * 보다 무겁다 — 관성 문턱(P4-c)이 점수 밖에 있으므로 머무름의 값은 요소 안에서 서야 한다.
 * 근거는 행마다 note 로 남긴다. 결정론에 영향을 주므로 런타임에 바뀌지 않는다.
 */
