...
- **메시는 FBX 원본 그대로** — 절차 생성(v1 SDF flesh)은 폐기. 되돌리려면 근거부터.
...
- 코드 주석·문서는 **한국어**.
- 네이밍 prefix (루트 규약 준수): `FHkt`(struct)·`UHkt`(UObject)·`AHkt`(Actor) 등. 이 웹
  트랙은 UE 코드가 아니므로 프레임워크 접두 규칙은 해당 없음.
- 한 작업이 끝나면 [STATE.md](STATE.md) 를 갱신한다 — 상세는 별도 문서, STATE 에는 핵심만.
- 육안 검증이 필요한 변경은 사용자에게 `npm run dev` 확인을 요청한다(샌드박스는 headless
  Chromium 차단 → Node 검증으로 대체).
...
기본 화면에 캐릭터 한 명. 모델 버튼(남자 X Bot / 여자 Y Bot / 📁 임의 FBX)으로 갈아끼우고,
애니메이션 버튼으로 재생, 본 비율 슬라이더로 뼈 스케일 조절.
