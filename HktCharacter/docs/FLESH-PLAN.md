...
// 살 DNA v1 — 전부 JSON 직렬화 가능해야 한다 (정규식은 문자열 소스로 보관)
{
  ...
  // 감산 연산 — 턱밑·허리 옆 등 "깎기". V1 은 구(sphere) 컷만.
  cuts: [
...
- `cuts[]` — `match`+`t` 로 세그먼트 위 기준점을 잡고, 세그먼트 로컬 프레임(§5.3)에서
  `offset`(m) 만큼 이동한 중심의 구를 강도 `strength` 로 **감산**한다. `r` = 구 반지름(m).
...
- 세그먼트 로컬 프레임: `â` = 축 방향, `u` = flatten 의 u 와 동일 규약(dir 없으면 월드 +z
  투영, 퇴화 시 +x), `v = â × u`.
- 컷 중심 `c = lerp(a, b, t) + offset[0]·u + offset[1]·v + offset[2]·â` (offset 미터).
- 필드: `field[idx] -= strength × (1 − d²/R_c²)³` (d < R_c, `R_c = BLEND × r`).
  가산 필드라 순서 무관, 음수 허용(MC 는 iso 교차만 본다). bbox 순회는 캡슐과 동일 패턴.
...
먼저, `upleg` 이 `leg` 보다 먼저). `cuts` 기본은 빈 배열 — 턱밑·겨드랑이 정의가 아쉬우면
그때 추가.
...
- **프리셋**: `fleshdna.js` 에 `PRESETS = [{ name, dna }]` — `humanlike`(§5.4 기본),
  `slim`(전 그룹 0.85 + 허리 잘록 강화), `bulk`(torso/arm 1.25 + 어깨 blend↑),
  `robot`(전부 상수 profile = F1 회귀형). UI: 살 섹션에 드롭다운.
...
| 6 | F2 | cut: strength>0 에서 컷 중심 필드값이 무컷 대비 감소, 원거리 불변 | 수치 |
