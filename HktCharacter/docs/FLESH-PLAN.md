...
**방향 전환 (설계 v1.1)**: SDF 살 단독으로는 정상 형태가 나오기 어렵다는 판단에 따라
**1차 경로를 "원본 메시 워프"(Phase W, §5W)로 전환**한다 — 원본 스킨 메시(X/Y Bot)가
형태의 베이스(얼굴·손·근육 라인 등 고주파 디테일)를 담당하고, DNA 는 그 정점을 실루엣
수준에서 변형하는 **변형장**으로 쓴다. SDF→MarchingCubes 직접 생성(F2·F3)은 베이스
메시가 없는 리그(크리처·뼈만 있는 FBX)용 폴백 + DNA 프리뷰로 지위가 내려간다.
...
- **유일한 예외**: Node 검증(§10.1 #1·#4)의 "합성 2-뼈 리그"는 필드 수학의 단위검증
  픽스처로만 허용 — `tools/` 밖(뷰어·`src/`)으로 나오면 안 된다. 실루엣·스키닝·워프 등
  골격/메시 형상이 결과에 개입하는 검증(#7~#13)은 실제 FBX 의 골격 치수·지오메트리를
  덤프한 픽스처를 쓴다 (임의 치수 발명 금지).
...
| Phase | 이름 | 경로 | 산출물 | 의존 |
|---|---|---|---|---|
| F1 | 살 DNA 채널 + 두께 슬라이더 | 공통 | `src/fleshdna.js`, mcflesh 개조(부모 키 매칭), UI "살" 섹션 | — |
| **W1** | **원본 메시 워프 코어** | **1차** | `src/fleshwarp.js` — 귀속·베이스라인·배율 워프·α 슬라이더 | F1 |
| **W2** | 워프 어휘 — flatten·bump/cut 변위 | **1차** | fleshwarp 확장 | W1 |
| F4 | 프리셋 · 보간 · 변이 · 입출력 | 공통 | 프리셋 배열, lerp/mutate, JSON 입출력 UI | W1 |
| F2 | SDF 형태 어휘 — 프로파일 곡선·flatten·cut | 폴백(SDF) | 필드 수학, mcflesh 확장 | F1 |
| F3 | bake & 자동 스키닝 | 폴백(SDF) | `src/fleshbake.js` — **베이스 메시 없는 리그 전용** | F2 |
| F5 (선택) | 살아있는 살 — 근육 팽창·호흡 | 폴백(SDF) | modulation 스키마 (실시간 모드 한정) | F2 |

커밋 단위 = Phase 하나. 각 Phase 마다 STATE.md 갱신 + §10 검증 수행.
**권장 순서: F1 → W1 → W2 → F4** (인간형 캐릭터의 최단 경로). F2·F3·F5 는 크리처/
뼈-only 리그가 필요해질 때 — DNA 는 두 경로가 공유하므로 그때 그대로 재사용된다.
...
// 살 DNA v1 — 전부 JSON 직렬화 가능해야 한다 (정규식은 문자열 소스로 보관)
{
  ...
  // 세그먼트 규칙 — **부모 뼈** simpleName 첫 매칭 (위에서부터). 세그먼트(부모 원점→
  // 자식 원점)가 덮는 해부학 부위는 부모 뼈의 스팬이므로 키는 부모다 (§3.1).
  // 다자식 부모(hips·spine2·hand)는 선택 필드 child(자식 정규식)로 구분한다.
  segments: [
    { match: 'thumb|index|middle|ring|pinky', profile: [[0, 0]] },   // r=0 → 살 생략
    { match: 'head',    profile: [[0, 0.08], [0.45, 0.095], [1, 0.04]], group: 'head' },  // 두개골
    { match: 'neck',    profile: [[0, 0.045], [1, 0.042]], blend: 1.4, group: 'head' },
    { match: 'hips', child: 'spine', profile: [[0, 0.105], [1, 0.09]], group: 'torso' },
    // …(§5.4 기본 인간형 테이블 전체)
  ],
...
- `segments[].match` — `simpleName`(콜론·mixamorig 접두 제거 소문자)에 대한 정규식 **소스
  문자열**. **부모 뼈 이름으로 매칭한다** (§3.1) — 선택 필드 `child`(자식 이름 정규식)로
  다자식 부모의 세그먼트를 구분한다. 첫 매칭 승. 매칭 없으면 fallback `[[0, 0.04]]`.

### 3.1 매칭 규약 — 부모 키 (설계 v1.1 정정)

세그먼트는 부모 원점→자식 원점을 잇고, 그 구간이 덮는 **해부학 부위는 부모 뼈의 스팬**이다
(뼈 원점 = 관절: `forearm` 뼈 원점은 팔꿈치이므로, 자식 `forearm` 으로 매칭한 세그먼트는
어깨→팔꿈치 = **위팔**을 덮는다). 기존 `RADII`/설계 v1.0 은 자식 키였고 부위 의도가 한 칸씩
밀려 있었다 — v1.1 부터 **부모 키**로 통일한다. 이 규약이 맞아야 하는 실질 이유 둘:

1. **워프(Phase W)의 정점 귀속과 일치** — 스킨 가중치에서 뼈 `b` 에 묶인 정점은 해부학적으로
   `b` 의 스팬(= `b` 가 부모인 세그먼트) 위에 있다.
2. **hips 가 유효한 키가 된다** — hips 는 자식 세그먼트가 없지만(부모가 뼈 아님) 부모로는
   3개 세그먼트(→spine, →left/rightupleg)를 소유한다. 골반 정면 폭은 `child:'upleg'` 쌍이,
   몸통 연결은 `child:'spine'` 이 그린다.

`end$` 리프 본 전용 항목은 삭제 — 리프 세그먼트의 부모(head·toe 등)가 profile 로 직접
그린다 (두개골이 `head` 키로 돌아온 것이 그 예). mcflesh 의 중복 스킵(seen) 키도
`부모>자식` 쌍으로 바꾼다.
...
| `compileDna(dna)` | 정규식 컴파일 + `부모>자식` 키→세그먼트 스펙 메모이즈 캐시를 가진 평가기 반환. `resolve(parentKey, childKey)` → `{ lut: Float32Array(33), rMax, flatten, blend, group, spheres[] }` 또는 `null`(r=0). LUT 는 PCHIP 평가(§5.1)를 33 지점 사전 샘플한 것 — 핫루프는 LUT 선형 보간만 한다. `groups` 배율은 resolve 시점에 곱해 LUT 에 반영하고, groups 변경 시 캐시 무효화(`invalidate()`). |
...
3. `RADII`·`radiusFor` 삭제 → `compileDna(ch.dna).resolve(parentKey, childKey)` —
   **부모 키 매칭**(§3.1)으로 전환. 리그 2벌 중복 스킵(`seen`)의 키도 `부모>자식` 쌍으로.
...
키는 **부모 뼈** (§3.1) — 세그먼트가 덮는 해부학 부위 기준으로 읽으면 된다.

| match(부모) | child | profile `[t,r]` | flatten | blend | group | 의도 (부모 스팬) |
|---|---|---|---|---|---|---|
| `thumb\|index\|middle\|ring\|pinky` | — | `[[0,0]]` | — | — | — | 손가락 생략 |
| `head` | — | `[[0,0.08],[0.45,0.095],[1,0.04]]` | `[0,0,1]`, 0.95 | 1 | head | **두개골** (바닥→정수리) |
| `neck` | — | `[[0,0.045],[1,0.042]]` | — | 1.4 | head | 목 — 승모근 완만 fold |
| `spine2` | `neck` | `[[0,0.09],[0.5,0.105],[1,0.095]]` | `[0,0,1]`, 0.7 | 1 | torso | 흉곽 — 가장 납작 |
| `spine2` | `shoulder` | `[[0,0.05],[1,0.045]]` | — | 1.2 | arm | 쇄골 웹 |
| `spine1` | — | `[[0,0.09],[0.5,0.06],[1,0.085]]` | `[0,0,1]`, 0.72 | 1 | torso | **허리 S커브 잘록** |
| `spine` | — | `[[0,0.095],[1,0.08]]` | `[0,0,1]`, 0.78 | 1 | torso | 하복부 |
| `hips` | `spine` | `[[0,0.105],[1,0.095]]` | `[0,0,1]`, 0.8 | 1 | torso | 골반→몸통 연결 |
| `hips` | `upleg` | `[[0,0.095],[1,0.085]]` | — | 1 | torso | **골반 좌우 폭** (→고관절) |
| `shoulder` | — | `[[0,0.045],[1,0.052]]` | — | 1 | arm | 삼각근 (쇄골끝→어깨관절) |
| `forearm` | — | `[[0,0.042],[0.3,0.046],[1,0.028]]` | — | 1 | arm | 전완 볼록→손목 수렴 |
| `arm` | — | `[[0,0.048],[0.45,0.052],[1,0.04]]` | — | 1 | arm | 위팔 이두 볼록 |
| `hand` | — | `[[0,0.028],[0.5,0.036],[1,0.024]]` | `[0,0,1]`, 0.55 | 1 | hand | 손바닥 패들 |
| `upleg` | — | `[[0,0.09],[0.35,0.078],[1,0.05]]` | — | 1 | leg | 허벅지 테이퍼 |
| `leg` | — | `[[0,0.052],[0.35,0.06],[1,0.032]]` | — | 1 | leg | **종아리 볼록**→발목 수렴 |
| `foot` | — | `[[0,0.035],[1,0.03]]` | `[0,1,0]`, 0.6 | 1 | foot | 발등 납작 (상하) |
| `toe` | — | `[[0,0.025],[1,0.018]]` | `[0,1,0]`, 0.6 | 1 | foot | 발끝 |

matching 순서는 표 순서 그대로 (위에서부터 첫 매칭 — `forearm` 이 `arm` 보다 먼저,
`upleg` 이 `leg` 보다 먼저 와야 부분 문자열 오매칭이 없다). `bumps`/`cuts` 기본은 빈
배열 — 턱밑·겨드랑이 정의가 아쉬우면 그때 추가.
...
|---|---|---|
| 어깨 좁고 골반 넓은 역삼각 반전 | 본 비율 `shoulder` 0.85 + `hips>upleg` r 상향 | 길이/골격은 뼈, 폭은 살 |
...
| 가슴 (측면·3/4) | **bump** ×2 (mirror) | `{match:'spine2', child:'neck', t:0.55, offset:[0.055,0.045,0], r:0.045, strength:0.9, mirror:true}` |
...
| 큰 두개골·작은 턱 (무안면) | `head` profile — §9 기본 노선과 일치 | `[[0,0.075],[0.5,0.098],[1,0.045]]` (바닥→정수리) |
...
**1차 경로(워프, §5W)에서는 원본 메시에 직접 적용되므로 해상도 제약이 없다** — SDF
폴백 경로로 볼 때만: 튜닝은 live(RES 64, 복셀 3.6cm — 1~2cm 차이는 뭉개짐)로 거칠게,
판정은 baked(res 160, 1.4cm)로 한다.

---

## 5W. Phase W — 원본 메시 워프 (1차 경로)

**원리**: 원본 스킨 메시의 정점을 바인드 포즈에서 **DNA 가 지시하는 만큼 방사 방향으로
이동**시킨다(모프). 원본이 이미 가진 얼굴·손가락·근육 라인·UV·재질은 전부 살아 있고,
DNA 는 실루엣(반경 프로파일·flatten·bump)만 조각한다. 변형은 바인드 공간 1회 계산 —
이후는 기존 GPU 스키닝이 그대로 애니메이션하므로 프레임 비용 0, 앨리어싱 0, bake 불필요.

**DNA 의미 재정의 없이 성립한다**: 워프는 절대 반지름이 아니라 **기준 DNA 대비 비율**을
쓴다 — `g(t) = 1 + α·(R_dna(t)/R_ref(t) − 1)`, `R_ref` = `defaultDna()`(§5.4) 평가값,
`α` = 살 강도 슬라이더(0~1.5, 기본 1). DNA=기본이면 g≡1(무변형·원본 그대로), α=0 도
항등 — **문자 그대로 원본↔DNA 사이의 보간**이고 α>1 은 과장이다. 같은 DNA 가 SDF
경로(절대 반지름)와 워프 경로(비율)를 동시에 구동한다.

### 5W.1 W1 — 귀속 · 베이스라인 · 배율 워프 (`src/fleshwarp.js`)

바인드 포즈(로드 직후, `bindLocalQ/P` 상태)에서 1회 전처리 + DNA 변경 시 재적용:

1. **정점 귀속** — 각 SkinnedMesh 정점의 스킨 영향(skinIndex/skinWeight, 가중치 ≥0.02)
   뼈 `b` 마다: `b` 가 부모인 세그먼트들(§3.1) 중 정점의 축 투영 `t∈[0,1]` 클램프 거리가
   최소인 세그먼트를 고르고 `(seg, t, q⃗)` 를 기록한다 (`q⃗` = 축 수직 오프셋, 바인드 월드).
   손가락처럼 r=0 세그먼트에 귀속된 영향은 **변위 0** (원본 유지 — 얼굴·손 보호의 기본값).
   영향이 여러 개면 결과 변위를 skinWeight 로 블렌드 — 관절 연속성이 여기서 공짜로 나온다
   (legacy "이중 바인딩" 교훈의 워프판).
2. **베이스라인 측정** — 세그먼트 × t-33빈 별로 귀속 정점들의 `|q_u|`·`|q_v|` **중앙값**
   (u = flatten dir 투영, v = 직교 — §5.2/§5.3 프레임 규약 공유) → 이방성 베이스라인
   LUT `(a_base(t), b_base(t))`. 표본 <5 인 빈은 이웃 보간, 이동평균 3 스무딩.
   **원본 메시 자신이 기준**이므로 X Bot·Y Bot·임포트 메시 무엇이든 보정 없이 성립한다.
3. **워프 적용** — 정점마다 영향별로
   `v' = axis(t) + û·q_u·g_u(t) + v̂·q_v·g_v(t)`,
   `g_u = 1 + α·(R·f_dna/(R_ref·f_ref) − 1)`, `g_v = 1 + α·(R/R_ref − 1)`
   (R = 현재 DNA LUT × groups 배율) 를 구해 skinWeight 블렌드 → `position` 갱신,
   `computeVertexNormals()`. **원본 position 은 최초 1회 백업**(`userData.origPositions`)
   하고 항상 원본에서 재계산한다 — 누적 오염 방지.
4. **UI** — 살 섹션에 `살 강도 α` 슬라이더 추가. 그룹 두께 슬라이더(F1)·프리셋(F4)이
   그대로 워프를 구동한다. 재계산은 X Bot 기준 수 ms(정점 수만×영향 4) — 실시간 슬라이더
   가능, 안전하게 100ms 디바운스.
5. **표시 모드** — `ui.flesh: 'off' | 'warp' | 'live'(SDF)` 로 확장. warp 가 기본이자
   1차 경로. baked(F3)는 SDF 폴백 경로에만 존재.

### 5W.2 W2 — flatten · bump/cut 변위

- **flatten** 은 W1 의 `g_u`(f 비율 항)에 이미 포함 — 원본 몸통이 이미 타원이므로 f 는
  "원본 대비 추가 눌림"으로 작동한다.
- **bump/cut** 은 필드가 아니라 **변위장**으로 재해석: 구 중심 `c`(§5.3 프레임 동일)에서
  정점까지 거리 d < R_c 면 `Δ = ±strength_w × (1−d²/R_c²)³ × dir̂`, `dir̂` = c 에서 정점
  방향(radial). 워프 경로에서 `strength_w` 는 **미터 변위**로 읽는다 (SDF 경로의 무차원
  strength 와 단위가 다름 — DNA 에 그대로 두되 워프 컴파일 시 `strength × r` 로 환산해
  bump 반지름에 비례시킨다). mirror 규약 동일. α 가 전체 변위에 곱해진다.

### 5W.3 트윈 리그·경계 주의 (구현 함정)

- **스킨 메시의 skeleton 은 트윈 뼈를 참조할 수 있다** — `makeCh` 가 중복 뼈를
  `…__dupN` 으로 개명하므로, 워프의 이름 매칭은 `name.replace(/__dup\d+$/, '')` 후
  `simpleName` 을 적용할 것. 세그먼트 축·바인드 월드는 **그 메시의 skeleton 소속 뼈**
  기준으로 계산한다 (구동 뼈와 트윈은 바인드에서 동일 월드라 결과는 같지만, 참조를
  섞으면 안 된다).
- 워프는 `ch.meshes` 의 모든 SkinnedMesh 에 동일 적용 (X/Y Bot 의 Surface+Joints 2벌
  일관성).
- 접지 불변 유지 — 접지는 뼈 bbox 기준(CLAUDE.md)이라 워프와 무관. 다리 배율을 크게
  올리면 발바닥 메시가 수 mm 지면을 뚫을 수 있다 — 알려진 한계로 기록, foot/toe 기본
  배율 1 유지로 회피.
- 옷·소품이 붙은 임포트 메시는 옷째 변형된다(베이스라인이 옷 포함 측정) — 상대 배율이라
  자연스럽게 따라오는 것이 기대 동작.

### 5W.4 완료 기준

- DNA=기본·α=1 에서 원본과 정점 완전 일치(항등). α 슬라이더로 원본↔체형 연속 보간.
- `stylized-f`(§5.7) 적용 시 허리 잘록·골반 폭·가슴/둔부 bump 가 원본 메시 품질로 표현,
  6클립 재생에서 관절 파탄 없음 (X Bot·Y Bot 양쪽 — §1.1).
- 얼굴·손가락 정점 무변형 (head group 1 기준), UV·재질·드로콜 불변.

---

## 6. Phase F3 — bake & 자동 스키닝 (SDF 폴백 경로 한정)

> **지위 (설계 v1.1)**: 베이스 메시가 있는 캐릭터는 Phase W(§5W)가 담당한다 — 이
> Phase 는 **뼈만 있는 리그**(크리처, 애니메이션-only FBX 계열)에서 살 메시를 만들어야
> 할 때만 필요하다. F2 완료 후로 미룬다.
...
| 9 | W1 | 항등성: DNA=기본·α=1 (및 임의 DNA·α=0) 에서 원본 position 과 완전 일치 | 오차 0 |
| 10 | W1 | 배율: `groups.leg=1.3` → 종아리 t-빈 중앙값 반경이 원본 대비 ×1.3 | ±3% |
| 11 | W1 | 연속성: 관절 경계(무릎·팔꿈치) 인접 빈 간 변위 불연속 없음 (기울기 유한) | 수치 |
| 12 | W2 | bump 변위: 구 중심 근방 정점만 이동, R_c 밖 정점 불변, mirror 좌우 대칭 | 수치 |
| 13 | W | UV·index·재질 참조 불변, 실제 FBX 지오메트리 덤프 픽스처(X·Y Bot)로 실행 | 수치 |
...
| `src/fleshwarp.js` | **신규** (W1·W2, 1차 경로) — 귀속·베이스라인·워프 파이프라인 |
| `src/fleshbake.js` | **신규** (F3, SDF 폴백) — bake 파이프라인 |
| `src/main.js` | `ch.dna` 수명, 살 UI 섹션(그룹 두께 + 살 강도 α), `ui.flesh` 모드(off/warp/live), 워프 재적용·재굽기 트리거, `__hkt` 확장 |
...
| 워프 변형 누적 오염 | (일반 — 모프 재적용 버그) | 원본 position 1회 백업, 항상 원본에서 재계산 (§5W.1-3) |
| 트윈 리그 이름 불일치로 워프 무시 | `__dupN` 개명이 simpleName 매칭을 깨뜨림 | `__dup\d+$` 선제 제거 규칙 명시 (§5W.3) |
| 베이스라인 측정 노이즈(겨드랑이·가랑이) | legacy "체인별 필드 필터" 계열 | t-빈 중앙값 + 이웃 보간 + 스무딩, 얼굴·손가락은 r=0 귀속으로 무변형 (§5W.1) |
| 세그먼트 키 한 칸 밀림(해부학 불일치) | v1.0 자식 키 매칭의 실수 | 부모 키 규약으로 정정, 표도 부모 키로 재작성 (§3.1, §5.4) |
