...
- **산출물**: 근육당 `CenterPath`(cubic spline, control points). capsule/sphere wrap 만 우선(설계서 §9.6 "첫 구현").
- **검증**: smoke 에 **뼈 관통비(§19.2)** — 경로 샘플점이 뼈 캡슐 내부인 비율 < 0.1%(§20 목표). 팔꿈치 굽힘 포즈에서 이두근 경로가 상완골 관통 안 함.
- **완료조건**: 중립·최소·최대 굴곡 3개 대표 자세에서 관통 0.
- **의존**: WP-01.
