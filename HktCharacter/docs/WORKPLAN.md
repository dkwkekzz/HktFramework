...
### WP-03 · Muscle Architecture 다양화 — "소시지 탈피"
- **목적**: 전부 Fusiform 인 현재 → **MultiHead(이두)·Fan(대흉근)·Sheet(복부)** 생성 규칙. 설계서 §8·§21.4.
- **대상**: `muscles.js`(아키텍처별 벨리/브랜치 생성), `anatomy.js`(`architecture`, `profiles.width/depth/tendon`).
- **산출물**: 아키텍처 원형 3종 + `ProfileCurve`(설계서 §23 의 width/depth/tendon 배열)로 방추형 형상 제어·힘줄 구간.
- **검증**: render 정면/측면에서 대흉근 부채꼴·복부 판형·이두 두 갈래 실루엣 육안 확인(사용자 눈검증 요청). smoke 는 근육별 부피 유효성.
- **완료조건**: §23 BicepsBrachii JSON 을 그대로 로드해 재현 가능.
- **의존**: WP-01, WP-02.
...
| WP-03 | Muscle Architecture 다양화 | ◀ 다음(가시) | Phase 1 |
...
**WP-01·WP-02 완료** → 다음은 **WP-03 (Muscle Architecture 다양화)**. 이유:
1. WP-01·02 는 구조(부착·관절 통과)를 세웠지만 rest 실루엣은 거의 그대로다. WP-03 은
   전부 방추형 타원체인 근육을 **대흉근 부채꼴·복부 판형·이두 두 갈래**로 바꿔 **rest 정면
   실루엣에서 눈에 띄는 첫 변화**를 만든다("소시지 탈피", 설계서 §8·§21.4).
2. WP-01 의 `origins[]`/`insertions[]` 다패치 구조와 `architecture` 태그가 이미 깔려 있어 바로 소비 가능.
3. **WP-02b(wrap)** 는 깊은 굴곡에서 관통이 실제로 측정되는 시점에 착수(측정 선행 원칙).
