...
### 의존성 그래프 (측정으로 갱신 — WP-02b 가 WP-04 선행)
```
WP-01 ─▶ WP-02 ─┬─▶ WP-03 ─▶ WP-06
   (부착)  (관절통과) │   (형태)    (체형)
                    └─▶ WP-02b ─▶ WP-04 ─▶ WP-05 ─▶ WP-08
                        (wrap)    (길항)   (활성도)  (기능합성)
WP-07 ── 전 WP 가로지름(정량 게이트) ──
```
✅ WP-01·02·03 · ◐ WP-06① · ⚠ WP-04(WP-02b 대기) · ☐ WP-02b·05·07·08
...
| WP-04 | Joint Function & 기능 근육 | ⚠ WP-02b(wrap) 대기 | Phase 1 |
| WP-05 | 활성도(activation) 분리 | ☐ 대기 | Phase 1 |
| WP-06 | Fat/Fascia & 체형 파라미터 | ◐ ① 프리셋 완료 (2026-07-18) | Phase 1→3 |
...
**WP-06 ①(체형 프리셋) 완료** → 다음 후보:
- **WP-02b (WrapConstraint + Route Solver)** — 팔꿈치 wrap. **WP-04(삼두 길항)의 선행**임이
  측정으로 드러났다(삼두 신장은 wrap 없이는 불가). 이걸 하면 WP-04 도 풀린다. 권장.
- **WP-05 (활성도 분리)** — 포즈 고정에도 근육 팽창(등척성). 브라우저 슬라이더 데모.
- **WP-06 ② (부위별 지방·fascia)** — 균일 지방 → 복부·엉덩이 부위 가중.
