...
- 수치 자체를 바꾸려면 `src/proportions.js` 프로파일 데이터를 수정하면 된다
  ...
  핸들로도 실시간 튜닝이 가능하다.
- **Detail 층**: 규칙/볼륨 헬퍼에 `k`(세그먼트별 blend 폭), `flatten: {dir, f}`(타원 단면),
  `op: 'cut'`(깎기)을 선택적으로 지정할 수 있다 — 납작한 흉곽, 턱끝 같은 캡슐 너머의 형태.
  수정 후에는 `npm run eval` 과 함께 렌더를 눈으로도 확인할 것 (Evaluator 는 실루엣만 본다).
