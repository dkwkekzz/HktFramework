/* step-0055 패널 — diffuse advection(kQDiffuse): 질이 *등방 확산* E 흐름에 동승 — q 가 모든 E 이동을 따라가는 완전한 물리장. 직전 step-0054(명시적 질 배출) 패널을 잇되 diffuse(①)에 질 동승 블록과 노브 kQDiffuse 1개를 더했다.
 ...
 * 회귀(이중 가드): kQDiffuse=0 → advection 미진입(q 버퍼 미할당·미스왑) 바이트 동일. qInit=false(degrade off)면 미진입. 브라우저: window.HWS_PANEL_0055
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — q advection 은 그 동역학과 직교[E 미접촉]. 이 step 이 질을 *확산 따라 번지게* 한다.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — diffuse advection 아레나: verify.js qdfaArena()/seedQBlob() 와 *같은* 설정(kQDiffuse 토글).
   *   기본 패널은 회귀 상태(kQDiffuse=0)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나(2D·중앙 8×8 고질 E 블록[E 10·q 1]·빈 배경·diffuse kD 0.2·degrade 질 축 alive)에 *질 동승(kQDiffuse)* 만 켜고/끄고 두 프리셋으로 A/B 대조한다 —
   *   화면이 보여주는 것 = verify 가 단언하는 수치(OFF 면 E 는 번져도 q 가 중앙에 stranded[번진 자리 빈 질] / ON 이면 q 가 E 따라 사방으로 advect). 흑체 색(L-Q 미리보기)으로 보면 ON 일 때 번지는 E 가 *고질 색을 데리고* 퍼진다. */
  function qdfaArena(kqd) {
    return {
      ...
      kDegrade: 0.01, qInit0: 0, kStarQual: 0, kQAdvect: 0, kQTaxis: 0, kQMetab: 0, kQExport: 0, kQDiffuse: kqd,
  ...
  function seedQBlob(sim, core) {   // 중앙 8×8 고질 E 블록(E 10·q 1) + 빈 배경 — diffuse 가 사방으로 펴며 advection 이 질을 데리고 번진다. verify.js seedQBlob() 와 같다(q 축 alive).
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0055 — <span style="color:#ffd060">diffuse advection (kQDiffuse)</span>: 질이 확산 E 흐름에 동승 — q 가 모든 E 이동을 따라가는 완전한 물리장(SPINE 여섯째 축 transport)',
    subtitle: '0051 이 질을 <b>gravity</b>(하향) 흐름에 실어 침강 hot plume 을 만들었으나, <b>등방 확산</b> 흐름엔 안 실렸다 — 고질 E 가 사방으로 번질 때 <code>q</code> 는 제자리에 stranded(E 와 q 가 따로 논다). 이 step 은 질을 확산 flux 에도 싣는다: <code>diffuse</code>(①, 선형 경로 kRelief=0)가 셀 E 를 이웃과 교환할 때 받는 칸은 <b>들어온 E 의 질</b>을 질량가중으로 섞는다(new_q = (q·retained + Σ q_neighbor·inflow)/확산후E). 고질이 번지면 q 도 따라 번진다 = <b>q 가 모든 E 이동(하향[0051]+등방[이번])을 따라가는 완전한 물리장</b>. 척추: <code>q</code> 는 E 에 올라탄 intensive 속성(단일 척추) · 국소(제 4(+2z)-이웃) · advection 은 E 미접촉(diffuse 가 <i>이미 옮긴</i> E 의 질만 따라감 — q 는 비율·거래 0, 닫힌 장부). <b>0051 gravity transport 의 등방 확산 짝</b>. 검증(D=1·중앙 8×8 고질 블록[E 10·q 1]·빈 배경·diffuse kD 0.2, 20 tick): <b>kQDiffuse OFF 면 E 는 번져도 q 가 중앙에 stranded(ring q 0.00) → ON 이면 q 가 E 따라 사방으로 advect(ring q 0.82)</b>. <b>kQDiffuse=0 → q 미접촉 → 직전 step(0054) 비트 동일</b>(회귀 0·이중 가드: qInit=false 면도 미진입). <code>node step-0055/verify.js</code> 로 reg·advect·conserve·det. 우측 흑체 색(L-Q 미리보기)으로 보면 ON 일 때 번지는 E 가 고질 색을 데리고 퍼진다. (표준 스택은 step-0027 덴드라이트 그대로 — q advection 은 그와 직교[E 미접촉].)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: true, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 의 diffuse advection 을 한 클릭으로 보이는 설정(verify advect 와 동일 아레나)으로. 둘 다 중앙 고질 블록·빈 배경·diffuse·degrade — *질 동승(kQDiffuse)*만 토글. seed 로 중앙 블록을 심는다. */
    presets: [
      { label: '🌫 diffuse advection ON (고질이 E 따라 사방으로 번짐)',
        title: 'verify advect(on) 과 같은 설정: D=1·중앙 8×8 고질 E 블록(E 10·q 1)·빈 배경·diffuse(kD 0.2·kRelief=0)·degrade 0.01·*kQDiffuse=1*, 20 tick 진행. E 가 확산으로 사방 번지며 *질을 데리고 간다* → ring(블록 밖) 자리 q 0.00→0.82 로 채워진다(고질 advect). 우측 흑체 색에서 번지는 E 가 *고질 색을 데리고* 퍼지면 켜진 것 = q 가 확산 E 를 따라가는 완전한 물리장(verify 가 단언하는 수치).',
        params: qdfaArena(1), seed: seedQBlob, run: 20,
        note: 'diffuse advection ON — ring q 0.00→0.82(질이 E 따라 번짐). OFF 버튼과 번갈아 보라.' },
      { label: '🧊 diffuse advection OFF (q 중앙 stranded·대조)',
        title: 'verify advect(off) 과 같은 설정: 같은 아레나지만 kQDiffuse=0(q 미접촉). E 는 확산으로 번져도 *q 는 중앙에 stranded* → 번진 ring 자리는 E 만 있고 질은 빈 채(ring q 0.00) = 직전 step(0054) 비트 동일. ON 버튼과 번갈아 눌러 A/B 로 비교하면 질이 E 를 따라가는지가 또렷하다(고질 색이 퍼짐 vs 중앙에 갇힘).',
        params: qdfaArena(0), seed: seedQBlob, run: 20,
        note: 'diffuse advection OFF(대조) — q 중앙 stranded, ring q 0.00. ON 버튼과 번갈아 보라.' }
    ],
  ...
  else global.HWS_PANEL_0055 = panel;
