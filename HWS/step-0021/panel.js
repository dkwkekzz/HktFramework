/* step-0021 패널 — 세포 분화(differentiate: 같은 genotype 이 *위치*에 따라 다른 phenotype — 갇힌 내부=soma·표면=germ, 진짜 분업, SPINE 주요 전이 사다리 "사회 → 분화된 다세포").
 * step-0020 패널을 잇되 새 노브 행 1개(세포 분화 kDiff)를 더했다. 시뮬 로직은 engine/hws-laws.js 의 differentiate 법칙(LAW_ORDER ⑥f, pubgood 뒤·생명 앞).
 * 표준 시나리오: step-0020 그대로(별·자기제한·연소 FSM·계량·복제·생명 유전·차등 응집·막 결합·생물량 공유·공공재) + 세포 분화 on(kDiff 0.3, 균일). 브라우저: window.HWS_PANEL_0021 */
(function (global) {
  'use strict';
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0021 — <span style="color:#e0c060">세포 분화</span>: 같은 genotype 다른 역할(differentiate)',
    subtitle: 'step-0020 의 공공재(pubgood)까지 협동은 모든 협동자가 <b>같은 일</b>(공동 채집)을 했다 — *진짜 분업*은 아니었다. 분업은 <b>같은 genotype 이 위치에 따라 다른 phenotype</b> 을 갖는 것 = 세포 분화. 이 step 은 그것을 더한다: 각 생명이 제 4-근방 점유를 <b>위치 정보</b>로 읽어 — 4-근방이 다 찬 <b>갇힌 내부</b>(번식 불가)는 <b>soma(체세포)</b>로 제 잉여를 표면 kin 에게 <b>기부</b>하고, 빈칸 있는 <b>표면</b>은 <b>germ(생식)</b>으로 번식한다. 한 클론 안에서 <i>표면=번식하는 몸·내부=먹여 주는 몸</i>으로 역할이 갈린다(positional information → 분업). <code>node step-0021/verify.js</code> 로 회귀·장부·결정론·differ·sustain 검증.',
    ...
    controls: [
      ...
      { items: [
        { kind: 'check', id: 'diffc', label: '세포 분화(soma/germ 분업)', def: true, gateFor: 'kDiff', title: '같은 genotype 이 *위치*로 갈린다 — 4-근방이 다 찬 *갇힌 내부*(번식 불가)는 soma(체세포)로 제 잉여를 표면 kin 에게 기부, 빈칸 있는 *표면*은 germ(생식). 진짜 분업. 0 = step-0020(모두 같은 역할).' },
        { kind: 'slider', id: 'kDiff', label: 'kDiff', param: 'kDiff', min: 0, max: 1, step: 0.05, def: 0.3, fixed: 2, gateBy: 'diffc', gateOff: 0, title: '분화 기부 강도(0~1). 갇힌 내부 soma 가 제 잉여의 이 비율을 표면 kin germ 에게 기부(m→m). 내부의 사장 생물량을 표면 번식으로 전환(분업 이득).' }
      ]},
      { items: [
        { kind: 'check', id: 'publicc', label: '공공재 협동(시너지 b≫c)', def: true, gateFor: 'kPublic', title: '협동자가 제 잉여를 기부해 4-인접 kin *여럿*에게 나눠 주되 *시너지로 증폭*(기부 m + 그 (syn−1)배를 필드 E 에서 채집). 한 기부가 여럿을 살려(양의 합) 협동이 *강하게 침투*. 0 = step-0019(공유는 보존적).' },
        { kind: 'slider', id: 'kPublic', label: 'kPublic', param: 'kPublic', min: 0, max: 1, step: 0.05, def: 0.3, fixed: 2, gateBy: 'publicc', gateOff: 0, title: '공공재 기부 강도(0~1). 전체 스택은 균일 협동(coopFit0=1).' },
        { kind: 'slider', id: 'pubSynergy', label: '시너지 b/c', param: 'pubSynergy', min: 1, max: 4, step: 0.1, def: 2.0, fixed: 1, title: '시너지 배수(>1 이라야 양의 합). =1 이면 보존적(침투 없음), >1 이면 강한 침투.' }
      ]},
      { items: [
        { kind: 'check', id: 'sharec', label: '생물량 공유(개체↔대사)', def: true, gateFor: 'kShare', title: '같은 유전형(kin) 4-인접 생명끼리 *생물량 m* 을 공유 — 협동 유전형이 사망 직전 kin 을 떠받친다(표적 구조). 0 = step-0018.' },
        { kind: 'slider', id: 'kShare', label: 'kShare', param: 'kShare', min: 0, max: 1, step: 0.05, def: 0.5, fixed: 2, gateBy: 'sharec', gateOff: 0, title: 'kin m 구조 강도(0~1). 전체 스택은 균일 협동(coopFit0=1).' }
      ]},
      { items: [
        { kind: 'check', id: 'couplec', label: '막/flux 결합(개체↔도메인)', def: true, gateFor: 'kMembrane', title: '같은 유전형(kin) 4-인접 생명끼리 필드 E 를 공유·재분배 → 막 창발. 0 = step-0017.' },
      ...
      { items: [
        { kind: 'check', id: 'adhesionc', label: '차등 응집(개체↔액적)', def: true, gateFor: 'kAdhesion', title: '같은 유전형(kin) 생명끼리 모이고 다른 태그는 밀어낸다(Steinberg DAH) → 표면장력 액적(개체). 0 = step-0016.' },
      ...
      { items: [
        { kind: 'check', id: 'inheritc', label: '생명 유전(genotype↔대사)', def: true, gateFor: 'kInherit', title: '생명이 R-genotype 을 부트스트랩·분열 상속(±변이)하고, fit(태그)→대사세로 선택된다(step-0016). kin 을 정의해 응집·막·공유·공공재·분화의 토대.' },
      ...
      { items: [
        { kind: 'select', id: 'click', label: '클릭 동작', role: 'click', options: [['lifegene', '유전 생명 놓기'], ['tissue', '클론 조직(분화)'], ['gene', '유전 씨앗(R)'], ['star', '별 점화'], ['life', '생명 놓기'], ['rock', '둑 쌓기(R)'], ['harvest', '수확']], def: 'lifegene' },
        { kind: 'button', id: 'seedTissue', label: '클론 조직(분화 관찰)', action: 'seedTissue' },
    ...
    /* 오버레이 — 유전형 R(태그별 색) + 활성도 A + 별 + *유전 생명*(태그색 점·갇힌 내부 soma 는 작게·표면 germ 은 크게) + *개체 윤곽*(다세포 kin 액적). */
    drawHook: function (ctx, c) {
      ...
      /* 유전 생명 점 — *분화 표시*: 4-근방이 다 찬 갇힌 내부(soma)는 *작은 속 빈 점*(번식 안 함·먹여 주는 몸), 표면(germ)은 *큰 채운 점*(번식하는 몸). 같은 색=같은 genotype, 크기=phenotype(위치). */
      for (var k = 0; k < ag.length; k++) {
        ...
        var soma = occN >= 4, cstr = col3 ? col3[0] + ',' + col3[1] + ',' + col3[2] : '245,245,245';
        if (soma) {   // 갇힌 내부 soma — 작은 속 빈 고리(체세포·번식 안 함)
    ...
    stats: [
      ...
      { label: '<span style="color:#e0c060">분화</span> (soma 내부 / germ 표면)', get: function (c) { var d = window.HWS_SIM.measureDifferentiation(c.sim); return (d.soma + d.germ) ? (d.soma + ' / ' + d.germ) : '—'; } },
      { label: '<span style="color:#e0c060">역할 m</span> (soma / germ)', get: function (c) { var d = window.HWS_SIM.measureDifferentiation(c.sim); return (d.soma + d.germ) ? (d.somaM.toFixed(2) + ' / ' + d.germM.toFixed(2)) : '—'; } },
      ...
      { label: '누적 출생 / 사망 / <span style="color:#e0c060">분화 provision</span>', get: function (c) { return c.sim.births + ' / ' + c.sim.deaths + ' / ' + (c.sim.differentiated || 0).toFixed(0); } },
      { label: '장부 잔차', get: function (c) { var r = c.led.residual; return { text: r.toExponential(3) + (r < 1e-6 ? ' PASS' : ' FAIL'), cls: r < 1e-6 ? 'pass' : 'fail' }; } }
    ],

    legend:
      '<span style="color:#e86060">●</span>tag1 <span style="color:#78c860">●</span>tag2 <span style="color:#60a8e8">●</span>tag3 <span style="color:#c870e0">●</span>tag4 — 유전 생명. <b>●채운 큰 점=germ(표면·번식)</b> · <b>○속 빈 작은 고리=soma(갇힌 내부·먹여 줌)</b>: 같은 색(genotype)인데 *위치*로 갈린 두 phenotype &nbsp; <span style="color:#c89b6a">■</span>R &nbsp; <span style="color:#57e0c8">■</span>활성도 A &nbsp; <span style="color:#fff7d2">★</span>별<br>' +
      '<b>세포 분화(같은 genotype 다른 phenotype by 위치)</b>: 0020 까지 협동자는 모두 *같은 일*을 했다. 이 step 은 *위치*가 역할을 정한다 — 4-근방이 다 찬 <b>갇힌 내부</b>(번식 불가)는 <b>soma(체세포)</b>로 제 잉여를 표면 kin 에게 <b>기부</b>하고, 빈칸 있는 <b>표면</b>은 <b>germ(생식)</b>으로 번식한다. "클론 조직(분화 관찰)" 을 눌러 조밀 조직을 깔면 내부(고리)/표면(점)이 갈린다. 우측 "역할 m" 에서 germ &gt; soma 면 분업(표면이 번식 자원을 받음).<br>' +
      '<b>분업 이득</b>: <code>verify differ</code> — 위치가 phenotype 을 가른 폭 dGap +0.32(표면 germ m 배증 0.25→0.53·갇힌 내부 비율 0.94), 분업으로 갇힌 내부의 *사장 생물량*이 표면 번식으로 전환돼 개체군 ↑(2928→3006). 기부는 m→m 쌍 거래라 churn 을 안 깬다(<code>verify sustain</code>·잔차 &lt;1e-6 <span class="pass">PASS</span>). <i>전체 스택은 희소라 분화가 거의 안 켜진다(갇힌 세포 드묾) — 현상은 조밀 조직에서.</i> "세포 분화" 체크를 끄면 step-0020 으로.',

    actions: {
      seedTissue: function (api) {
        var s = api.sim, n = 0;
        for (var y = 26; y < 38; y++) for (var x = 26; x < 38; x++) { var a = api.core.spawnAgent(s, x, y); a.g = 1; n++; }
        api.toast('클론 조직 ' + n + '세포(tag1) — confluent 해지면 갇힌 내부(soma·고리)가 표면(germ·점)을 먹여 번식 분업');
      },
      seedSortMix: function (api) {
        ...
        api.toast('혼합 군집 ' + n + '개체 — 차등 응집이 같은 색끼리 액적으로 정렬, 그 안에서 갇힌 내부가 분화(soma)');
      },
    ...
    clickModes: {
      lifegene: function (api, cx, cy) { var tag = 1 + ((api.sim.tick + cx + cy) % api.sim.p.geneTypes), a = api.core.spawnAgent(api.sim, cx, cy); a.g = tag; api.toast('유전 생명 (' + cx + ',' + cy + ') tag ' + tag + ' — kin 끼리 응집해 개체로, 갇힌 내부는 soma 로 분화'); },
      tissue: function (api, cx, cy) { var s = api.sim, n = 0; for (var y = cy - 5; y < cy + 5; y++) for (var x = cx - 5; x < cx + 5; x++) { var a = api.core.spawnAgent(s, x, y); a.g = 1; n++; } api.toast('클론 조직 ' + n + '세포 (' + cx + ',' + cy + ') — 갇힌 내부(soma)가 표면(germ) 번식을 먹인다'); },
  ...
  else global.HWS_PANEL_0021 = panel;
