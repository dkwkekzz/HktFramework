/* step-0031 패널 — 중력 구배(VOXEL.md V3: E 의 하향 선호 흐름. z 확산[V2, 등방]에 *방향*을 줘 골에 고인 E = 바다 원형). 직전 step-0030(6-이웃 확산) 패널을 잇되 새 노브 행 1개(중력 kGravity)를 더했다.
 * 이 step 은 gravity 법칙 1개를 더한다 — 각 셀이 제 E 의 kGravity 비율을 아래(z−1) 이웃으로 유출(donor-제한 쌍 거래, 보존). V2 의 z 확산은 *등방*(위아래 같은 계수)이라 E 가 상자를 고루 채울 뿐 — V3 는 *아래만* 선호해 E 가 z=0 바닥부터 고인다 = 바다.
 * kGravity=0 또는 D=1 이면 gravity 산술 0 = 직전 step 비트 동일(회귀). 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0031
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 중력은 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0031 — <span style="color:#5aa0ff">중력 구배</span>: E 가 아래로 가라앉아 바닥에 고인다 = 바다 원형(VOXEL.md V3)',
    subtitle: 'V2(step-0030)의 z 확산은 <i>등방</i>(위아래 같은 계수)이라 E 가 W×H×D 상자를 <b>고루</b> 채울 뿐 — 바다가 안 고인다. 이 step(V3)은 흐름 법칙에 <b>중력(z-비등방 항)</b> <code>gravity</code> 을 더한다: 각 셀이 제 E 의 <b>kGravity 비율을 아래(z−1) 이웃으로 유출</b>한다(donor-제한 쌍 거래 — 비율이라 음수 없음·보존). 그러면 E 가 <b>가라앉아 z=0 바닥</b>(더 못 내려가는 벽)부터 고이고, 등방 확산(V2)이 옆으로 펴 <b>골을 채우는 고임 = 바다</b>. 방향 상수(아래)는 scan-order 처럼 <i>법칙의 상수</i>지 전역 조율자 아님(판정은 z−1 로컬 한 칸 — 척추 ③). <b>kGravity=0 또는 D=1 이면 gravity 산술 0 = 직전 step 비트 동일</b>(회귀 0). 검증: <b>균일 E(D=8) → 바닥 평면 분율 12.5%(kGravity=0, 균일)→100%(kGravity=0.2, z=0 바닥에 완전 침전)</b>·천장 비움·잔차 0·결정론. <code>node step-0031/verify.js</code> 로 reg·settle·conserve·det. R 차폐·지지 침착(지면)은 V4 로 전가. (표준 스택은 step-0027 덴드라이트 그대로 — 중력과 직교.)',
    ...
    controls: [
      ...
      { items: [
        ...
        { kind: 'slider', id: 'kGravity', label: 'kGravity (중력 계수)', param: 'kGravity', min: 0, max: 0.5, step: 0.02, def: 0.2, fixed: 2, gateBy: 'gravityc', gateOff: 0, title: '중력 계수(셀당/tick 하향 유출 비율). 0 또는 D=1 이면 gravity 산술 0 = 회귀(직전 step 비트 동일). 클수록 빨리 침전. *리셋 불필요* — 즉시 적용. R 차폐(아래가 고체면 옆으로)·지지 침착은 V4(지금은 순수 E 하향).' }
      ]},
      { items: [
    ...
    legend:
      ...
      '<b>중력 구배(VOXEL.md V3)</b>: V2 의 z 확산은 <i>등방</i>이라 E 가 상자를 고루 채울 뿐 — 바다가 안 고인다. 이 step 은 흐름 법칙에 <b>중력(z-비등방 항)</b>을 더한다 — 각 셀이 제 E 의 kGravity 비율을 <b>아래(z−1) 이웃으로 유출</b>(donor-제한 쌍 거래). E 가 <b>가라앉아 z=0 바닥부터 고이고</b>(더 못 내려가는 벽), 등방 확산이 옆으로 펴 골을 채우는 고임 = <b>바다</b>. "터 깊이 D" 를 8 로, "중력" 을 켜고 리셋해 보라. <code>verify settle</code>: 균일 E(D=8) → <b>바닥 평면 분율 12.5%(kGravity=0)→100%(kGravity=0.2)</b>·천장 비움·잔차 0. R 차폐·지지 침착(지면)은 V4. (3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더는 렌더러 트랙 L-V1.)<br>' +
  ...
  else global.HWS_PANEL_0031 = panel;
