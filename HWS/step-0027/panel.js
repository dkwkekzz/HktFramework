/* step-0027 패널 — 가지치기 덴드라이트(dendrite: 자라는 결정 전선이 *경계 불안정*[Mullins-Sekerka]으로 옆가지를 뻗는다. 형태 사다리 R5, R 하이트필드).
 * step-0026 패널을 잇되 새 노브 행 1개(덴드라이트 kDendrite)를 더했다. 시뮬 로직은 engine/hws-laws.js 의 dendrite 법칙(LAW_ORDER ⑤g, turing 뒤·combust 앞 — 전선 셀 곡률 증폭 E→R 침착).
 * 표준 시나리오: step-0026 그대로(별·…·튜링 불안정) + 가지치기 덴드라이트 on(kDendrite 1). 브라우저: window.HWS_PANEL_0027 */
(function (global) {
  'use strict';
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0027 — <span style="color:#9ad24a">가지치기 덴드라이트</span>: 자라는 전선이 경계 불안정으로 옆가지(dendrite)',
    subtitle: '형태 사다리 R3(방향성 결정, 0025)은 <i>곧은</i> needle(고정 축)을, R4(튜링, 0026)는 <i>균일 대칭 깨짐</i>을 빚었다. 이 step 은 <b>자라는 결정 전선</b>을 <b>경계 불안정</b>(Mullins-Sekerka — 끝이 더 빨리 자라는 양의 되먹임)으로 <b>옆가지</b>를 뻗게 한다 — 결정 성장의 마지막 큰 형태(눈송이·금속 덴드라이트). <b>둘째 필드 없이</b>(단일 척추) E·R 만: 자라는 셀이 제 E 를 R 로 당기면 그 자리 E 가 비고, <i>튀어나온 끝(tip)</i>은 바깥서 확산으로 빨리 채워지나 <i>오목한 만(notch)</i>은 둘레가 다 고체라 멈춘다(<b>기하 차폐</b>=긴 거리 억제) — 거기에 <b>곡률 증폭</b>(볼록 tip 침착 가속, 짧은 거리 활성)을 얹는다. 짧은 활성 + 긴 억제 = 평탄 전선이 <i>옆가지</i>로 갈린다(<b>창발</b> — 침착 1개만 깖, turing 과 같은 템플릿을 <i>전선</i>에). <code>node step-0027/verify.js</code> 로 회귀·장부·결정론·dendrite·sustain 검증.',
