/* HWS step-0045 헤드리스 검증 — 에너지의 질(자유에너지·둘째 법칙, SPINE 여섯째 축).
 ...
 *   회귀(가드): kQual=0 → degrade early-return = 직전 step(0044) 비트 동일(Eth 영원히 0·qualInit 미설정 → hashState Eth skip).
 *
 * 사용: node step-0045/verify.js <reg|qual|conserve|det|all> [seed]
 *  - reg      : 회귀 0 — kQual=0 → Eth 0·열화 0·2회 실행 비트 동일. *교차 버전* 회귀는
 *               `node engine/validate/verify-sim-engine.js` 골든(전 시나리오 std@~cap3@ 비트 불변 — 새 노브=0)이 권위.
...
function runMode(mode, seeds) {
  if (mode === 'reg') {
    ...
    console.log('회귀 0: kQual=0 → degrade early-return → Eth 0·열화 0·2회 실행 비트 동일. 새 법칙 degrade(LAW_ORDER evaporate 뒤·노브 kQual) — *교차 버전* 회귀는 verify-sim-engine.js 골든 해시(전 시나리오 std@~cap3@ 비트 불변·새 노브=0→degrade 미진입·qualInit=false→Eth 해시 skip)가 권위.');
