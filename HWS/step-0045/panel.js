/* step-0045 패널 — 3D 차등 응집: 차등 응집(kin 정렬)의 연직축 일반화(VOXEL.md V5+). 직전 step-0044(3D 자기제한) 패널을 잇되 adhere 법칙 *제자리 확장* + 노브 kAdhereZ 1개를 더했다.
 * 0042·0043 이 생명을 z>0 에 올렸으나(이동·번식으로 3D 거주), 차등 응집(adhere·step-0017 다세포 kin 정렬)은 여전히 2D 평면만 봤다 — occ 그리드 W·H(2D)·이동 후보 4-이웃·점수 8-이웃(Moore 평면)이라 z>0 생명은 occ[a.center] 가 범위 밖이라 무시(z>0 거주 생명이 위/아래 kin 을 못 세 *정렬을 못 함*). 0044 가 crowd 만 z-일반화했고 adhere 는 caveat 로 남았다. 이 step 은 adhere 의 occ 그리드 W·H→W·H·D·이동 후보 4→6-이웃·점수 8(Moore 평면)→26(Moore 3D) 로 일반화한다 — z>0 거주 생명도 제 z±이웃 kin 을 세 정렬한다 = 같은 유전형이 3D 액적으로 뭉침(cell sorting 의 연직 일반화·Steinberg DAH 의 3D 짝).
 * 형식: 0044 crowd 의 kCrowdZ z-확장과 같은 *제자리 확장*(새 LAW_ORDER 자리 없음) + 노브 kAdhereZ. adhere 는 위치만 바꿈(E·m 거래 0) → moveZInit 켜 agent.z 를 해싱(0042 move 와 같은 플래그)해 골든이 3D 정렬을 잠근다.
 * 회귀(이중 가드): kAdhereZ=0 → 3D 블록 미진입(2D 경로)=직전 step 비트 동일·z≥1 거주 생명은 occ 범위 밖이라 정렬 못 해 얼어붙음 / D=1 → z 이웃 없어 2D 등가. 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0045
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 3D 차등 응집은 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — 3D 차등 응집 아레나: verify.js sortArena()/seedKinBlock 과 *같은* 설정.
   *   기본 패널은 회귀 상태(D=1)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나(D=8·두 유전형 z-체커보드 블록[z∈[1,7)]·adhere 만 on·나머지 동역학 off)에 *연직 응집(kAdhereZ)* 만 켜고/끄고 두 프리셋으로 A/B 대조한다 —
   *   화면이 보여주는 것 = verify 가 단언하는 수치(kAdhereZ OFF 면 z>0 거주 생명이 2D occ 범위 밖이라 정렬 0·얼어붙음·kin 접촉 초기값 / ON 이면 z±이웃 kin 정렬로 kin 접촉↑·같은 태그가 3D 로 뭉침). (3D 뷰는 아직 z=0 투영 — voxel 렌즈 L-V1 백로그·통계 "kin 접촉"이 증거.) */
  function sortArena(kAZ) {
    return {
      ...
      kAdhesion: 1, adhesionLambda: 1.0, adhesionGain: 0.5, kAdhereZ: kAZ,
  ...
  function seedKinBlock(sim, core) {   // 균일 E=2 + 두 유전형 z-체커보드 블록(x∈[12,24)·y∈[12,24)·z∈[1,7)·≈60% 채움) — verify.js seedKinBlock 과 동일
    var W = sim.p.W, H = sim.p.H, WH = W * H, D = sim.p.D;
    for (var i = 0; i < WH * D; i++) { sim.E[i] = 2; sim.E0 += 2; }
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0045 — <span style="color:#ffd060">3D 차등 응집</span>: 다세포 kin 정렬이 연직축으로 — z>0 거주 생명도 위/아래 kin 을 세 정렬한다(VOXEL.md V5+)',
    subtitle: '0042·0043 이 생명을 z>0 에 올렸다(<b>이동</b>·<b>번식</b>으로 3D 거주). 그러나 차등 응집(<code>adhere</code>·step-0017 다세포 kin 정렬)은 여전히 2D 평면만 봤다 — occ 그리드 W·H(2D)·이동 후보 4-이웃·점수 8-이웃(Moore 평면)이라 z>0 생명은 무시(위/아래 kin 을 못 세 <b>정렬을 못 함</b> = 3D 다세포 누수). 0044 가 <code>crowd</code>만 z-일반화했고 <code>adhere</code>는 caveat 로 남았다. 이 step 은 <code>adhere</code> 의 <b>occ 그리드 W·H→W·H·D·이동 후보 4→6-이웃·점수 8(Moore 평면)→26(Moore 3D)</b> 로 일반화한다: z>0 거주 생명도 제 z±이웃 kin 을 세 정렬한다 = <b>같은 유전형이 3D 액적으로 뭉침</b>(cell sorting 의 연직 일반화·Steinberg DAH 의 3D 짝). 형식: 0044 <code>crowd</code> 의 kCrowdZ z-확장과 같은 <b>제자리 확장</b>(새 LAW_ORDER 자리 없음) + 노브 <b>kAdhereZ</b>. 검증(D=8·두 유전형 z-체커보드 블록[z∈[1,7)]·adhere 만 on): <b>kAdhereZ OFF 면 z>0 거주 생명이 2D occ 범위 밖이라 정렬 0(kin 접촉 0.379 초기값 그대로·얼어붙음) → ON 이면 z±이웃 kin 정렬로 kin 접촉 0.379→0.880 로 오른다</b>. <b>kAdhereZ=0 → 3D 블록 미진입(2D 경로)=비트 동일 / D=1 → 2D 등가</b>(회귀 0·이중 가드). adhere 는 위치만(거래 0·잔차 0)·결정론. <code>node step-0045/verify.js</code> 로 reg·sort·conserve·det. (표준 스택은 step-0027 덴드라이트 그대로 — 3D 차등 응집과 직교.)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: true, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 의 3D 차등 응집을 한 클릭으로 보이는 설정(verify sort 와 동일 아레나)으로. 둘 다 두 유전형 z-체커보드 블록·adhere 만 on — *연직 응집(kAdhereZ)*만 토글. */
    presets: [
      { label: '🧬 3D 차등 응집 ON (z>0 kin 정렬·D=8)',
        title: 'verify sort(on) 과 같은 설정: D=8·두 유전형 z-체커보드 블록(z∈[1,7))·adhere 만 on·*연직 응집 kAdhereZ=1*, 40 tick 미리 진행. z>0 거주 생명이 제 z±이웃 kin 을 세 정렬해 같은 태그가 3D 액적으로 뭉친다 = kin 접촉 0.379→0.880(다세포 정렬의 연직 일반화). 통계 "kin 접촉"이 0.88 근처면 3D 정렬이 켜진 것 = verify 가 단언하는 수치. (3D 뷰는 z=0 투영이라 블록이 평면에 겹쳐 보인다 — voxel 렌즈 L-V1 백로그·통계가 증거.)',
        params: sortArena(1), seed: seedKinBlock, run: 40,
        note: '3D 차등 응집 ON — z>0 거주 생명이 3D 로 kin 정렬(kin 접촉 0.38→0.88). OFF 버튼과 번갈아 보라(통계 "kin 접촉").' },
      { label: '🛑 3D 차등 응집 OFF (z>0 생명 얼어붙음·대조)',
        title: 'verify sort(off) 과 같은 설정: 같은 아레나·블록이지만 kAdhereZ=0. adhere 가 2D 평면만 봐 z≥1 거주 생명이 occ 범위 밖이라 정렬을 *못 한다*(정렬 0·kin 접촉 0.379 초기값 그대로·얼어붙음 — 바로 옛 2D adhere 가 z>0 생명을 못 정렬하던 caveat). ON 버튼과 번갈아 눌러 A/B 로 비교하면 연직 정렬이 또렷하다.',
        params: sortArena(0), seed: seedKinBlock, run: 40,
        note: '3D 차등 응집 OFF(대조) — z>0 생명이 정렬 못 해 얼어붙음(kin 접촉 0.38). ON 버튼과 번갈아 눌러 연직 정렬을 보라.' }
    ],
    ...
    stats: [
      ...
      { label: '<span style="color:#ffd060">생명 연직</span> (meanZ / maxZ / z>0 분율 — 이동·번식·혼잡의 z)', get: function (c) {
          var D = c.sim.p.D || 1, zOn = ((c.sim.p.kMoveZ || 0) !== 0) || ((c.sim.p.kDivZ || 0) !== 0) || ((c.sim.p.kCrowdZ || 0) !== 0) || ((c.sim.p.kAdhereZ || 0) !== 0), ag = c.sim.agents, top = D - 1;
      ...
      { label: '<span style="color:#ffd060">kin 접촉</span> (3D 분율 — 가설: z>0 정렬로 오름)', get: function (c) {
          var sim = c.sim, ag = sim.agents, W = sim.p.W, H = sim.p.H, D = sim.p.D || 1, WH = W * H; if (!ag.length) return { text: '생명 없음', cls: '' };
          var occ = {}; for (var i = 0; i < ag.length; i++) { var g = ag[i].g | 0; if (g > 0) occ[ag[i].center] = g; }
          var kinSum = 0, totSum = 0;
          for (var k = 0; k < ag.length; k++) {
            var a = ag[k], t = a.g | 0, az = a.z || 0; if (t <= 0) continue;
            for (var dz = -1; dz <= 1; dz++) { var nz = az + dz; if (nz < 0 || nz >= D) continue;
              for (var dy = -1; dy <= 1; dy++) for (var dx = -1; dx <= 1; dx++) { if (!dx && !dy && !dz) continue;
                var o = occ[nz * WH + ((a.y + dy + H) % H) * W + (a.x + dx + W) % W]; if (!o) continue; totSum++; if (o === t) kinSum++; } } }
          var kc = totSum ? kinSum / totSum : 0, azOn = (sim.p.kAdhereZ || 0) !== 0 && D > 1;
          return { text: 'kin접촉 ' + kc.toFixed(3) + (azOn ? ' (3D 응집 on — z>0 정렬)' : '') + ' · 정렬 ' + (sim.adheres || 0), cls: azOn && kc > 0.6 ? 'pass' : '' };
      } },
    ...
    legend:
      ...
      '<b>3D 차등 응집 — 다세포 kin 정렬의 연직축 일반화(VOXEL.md V5+ · SPINE 다섯째 축)</b>: 0042·0043 이 생명을 z>0 으로 올렸고(이동·번식) 0044 가 그 3D 생명에 carrying capacity(혼잡)를 줬다. 그러나 <b>차등 응집</b>(<code>adhere</code>·step-0017 = 같은 유전형 kin 끼리 모여 다세포 액적을 빚는 cell sorting)은 여전히 2D 평면만 봤다 — occ 그리드·이웃 셈이 W·H[2D]라 z>0 거주 생명은 <b>위/아래 kin 을 못 세 정렬을 못 했다</b>(3D 다세포 누수·바로 0042~0044 의 caveat). 이 step 은 <code>adhere</code> 의 <b>occ 그리드 W·H→W·H·D·이동 후보 4→6-이웃·kin 점수 8(Moore 평면)→26(Moore 3D)</b> 로 <b>제자리 확장</b>(0044 crowd 의 kCrowdZ 와 같은 형식)한다: z>0 거주 생명도 제 z±이웃 kin 을 세 정렬한다 = <b>같은 유전형이 3D 액적으로 뭉침</b>. 노브 <b>kAdhereZ</b> + agent.z(moveZInit 가법 해시). "3D 차등 응집" 켜고 "터 깊이 D" 8 로 리셋해 보라(위 "3D 차등 응집 ON" 버튼). <code>verify sort</code>(두 유전형 z-체커보드 블록[z∈[1,7)]): <b>kAdhereZ OFF 면 z>0 생명이 2D occ 범위 밖이라 정렬 0(kin 접촉 0.379 초기값·얼어붙음) → ON 이면 z±이웃 kin 정렬로 kin 접촉 0.379→0.880 로 오른다</b>. 위치만(장부 거래 0·잔차 0). <b>kAdhereZ=0 → 3D 블록 미진입=비트 동일 / D=1 → z 이웃 없어 2D 등가</b>(회귀 0·이중 가드). (3D 뷰는 아직 z=0 투영 — voxel 렌더는 렌더러 트랙 L-V1·통계 "kin 접촉"이 증거.)<br>' +
  ...
  else global.HWS_PANEL_0045 = panel;
