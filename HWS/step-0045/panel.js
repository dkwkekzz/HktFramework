/* step-0045 패널 — 에너지의 질(자유에너지·둘째 법칙, SPINE 여섯째 축). 직전 step-0044(3D 자기제한) 패널을 잇되 새 법칙 degrade + 노브 kQual 1개를 더했다.
 * 지금까지 E 는 무차별 스칼라 하나였다 — 별의 뜨거운 E 한 단위와 식은 바다의 E 한 단위가 비트 동일(질 구별 0). 그러나 생명을 굴리는 건 *양*이 아니라 *질*(자유에너지·낮은 엔트로피)이다(슈뢰딩거·프리고진). 양은 닫힌 장부서 보존돼 "다 쓸" 수 없고, 실제 소비되는 건 질이다.
 * 이 step 은 연속 질 축을 더한다: 매 tick 자유 E 의 kQual 비율이 E→Eth(열 에너지, 저질)로 *일방 강등*(둘째 법칙)·쌍 거래(총 E+Eth 불변·양 보존·질만 떨어짐). 자유 E 생산은 source/별 주입(고질). 질 q=E/(E+Eth) → 보충받는 source 셀은 질 유지·미보충 배경은 붕괴(흐름이 질을 떠받친다 = 소산구조).
 * 회귀: kQual=0 → degrade early-return = 직전 step(0044) 비트 동일(Eth 0·qualInit 미설정 → Eth 해시 skip). 렌더 L-Q(흑체 색온도)가 이 q 를 읽는다(RENDER §8 — 시뮬이 q 를 내보내야 색온도 author 0). 브라우저: window.HWS_PANEL_0045
 * (아래 표준 스택은 step-0027 그대로 — 에너지 질은 그 동역학과 직교. q 오버레이는 drawHook 끝에 가법.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — 에너지 질 아레나: verify.js qualArena() 와 *같은 정신*(확산·증발 off·균일 E + source 보충·다른 동역학 off).
   *   기본 패널은 회귀 상태(kQual=0)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나에 *질 강등(kQual)* 만 켜고/끄고 두 프리셋으로 A/B 대조한다 — 화면 q 오버레이(drawHook 끝):
   *   보충받는 source 셀은 *밝게*(고질) 유지·미보충 배경은 *붉게*(저질) 붕괴 = verify 가 단언(qSrc 0.40 > qBg 0.13·thermalFrac 0.84). */
  function qualArena(kQ) {
    return {
      W: 64, H: 64, D: 1, initE: 1, noise: 0, kEvap: 0, kD: 0, kDz: 0,
      drive: true, source: { x: 32, y: 32, r: 4, rate: 0.3 }, sink: { x: 0, y: 0, r: 0, rate: 0 },
      life: false, move: false, repro: false, pTumble: 0, kA: 0, kRelief: 0, baseCost: 0,
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0045 — <span style="color:#ffd060">에너지의 질</span>: 양은 보존되고 *질*이 소비된다 — 자유 E 가 둘째 법칙으로 열 에너지로 식는다(SPINE 여섯째 축)',
    subtitle: '지금까지 E 는 무차별 스칼라 하나였다 — 별의 뜨거운 E 한 단위와 식은 바다의 E 한 단위가 비트 동일(질 구별 0). 그러나 생명을 굴리는 건 <b>양</b>이 아니라 <b>질</b>(자유에너지·낮은 엔트로피)이다(슈뢰딩거·프리고진). 양은 닫힌 장부서 보존돼 "다 쓸" 수 없고, 실제 소비되는 건 질이다. 이 step 은 새 법칙 <code>degrade</code> + 노브 <b>kQual</b> 로 연속 질 축을 더한다: 매 tick 자유 E 의 kQual 비율이 <b>E→Eth(열 에너지, 저질)로 일방 강등</b>(둘째 법칙)·쌍 거래(총 E+Eth 불변·양 보존·질만 떨어짐). 자유 E 생산은 source/별 주입(고질). 질 <b>q=E/(E+Eth)</b>. 검증(확산 off·균일 E + source 보충·40 tick): <b>kQual OFF 면 Eth 0·q=1 전역(질 구별 없음) → ON 이면 열화(thermalFrac 0.84)하되 보충받는 source 셀 q 0.40 가 미보충 배경 q 0.13 보다 높게 유지</b> = 흐름(throughput)이 질을 떠받친다(소산구조·슈뢰딩거). <b>kQual=0 → degrade early-return = 비트 동일</b>(회귀 0). 강등은 E→Eth 쌍 거래(양 보존·잔차 1.6e-14)·결정론. 렌더 <b>L-Q(흑체 색온도)</b>가 이 q 를 읽는다(RENDER §8 — 시뮬이 q 를 내보내야 색온도 author 0). <code>node step-0045/verify.js</code> 로 reg·qual·conserve·det. *한계(후속): advection 없음·생명 동역학 미연동·Eth 재활용 없음(열사 위험). (표준 스택은 step-0027 그대로 — 에너지 질과 직교.)',
    overlays: { sourceSink: true, pools: true, life: true, centroid: false, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 의 에너지 질을 한 클릭으로 보이는 설정(verify qual 과 같은 정신)으로. 둘 다 균일 E + source(32,32) 보충·확산 off — *질 강등(kQual)*만 토글. 화면 q 오버레이(drawHook 끝): source 밝게(고질)·배경 붉게(저질). */
    presets: [
      { label: '🔥 에너지 질 강등 ON (source 가 질 떠받침·배경 붕괴)',
        title: 'verify qual(on) 과 같은 정신: 균일 E=1 + source(32,32 r4) 보충·확산 off·*질 강등 kQual=0.05*, 40 tick 미리 진행. 보충받는 source 셀은 질 q 0.40 유지(밝게)·미보충 배경은 둘째 법칙으로 q 0.13 붕괴(붉게) = 흐름이 질을 떠받친다(소산구조). 양은 보존(E→Eth 쌍 거래)·질만 떨어진다. q 오버레이가 source 환하게·배경 붉게 = verify 가 단언하는 수치(thermalFrac 0.84).',
        params: qualArena(0.05), run: 40,
        note: '에너지 질 ON — source 가 질을 떠받쳐 환하고(고질) 배경은 붉게 식는다(저질). OFF 와 번갈아 보라(q 오버레이).' },
      { label: '❄ 에너지 질 강등 OFF (q=1 전역·대조)',
        title: 'verify qual(off) 과 같은 정신: 같은 아레나지만 kQual=0. degrade early-return → Eth 0·q=1 전역(질 구별 없음·thermalFrac 0). 화면이 균일(질 오버레이 없음) — 직전 step 비트 동일. ON 버튼과 번갈아 눌러 A/B 로 비교하면 질 구배가 또렷하다.',
        params: qualArena(0), run: 40,
        note: '에너지 질 OFF(대조) — Eth 0·q=1 전역(질 구별 없음). ON 버튼과 번갈아 눌러 질 강등을 보라.' }
    ],
