...
**렌더→시뮬 의존의 단방향 게이트 (코디네이션 규칙)**: 두 트랙은 *소유*는 직교지만 *데이터*는 **생산자(시뮬)→소비자(렌더) 단방향**이다 — 렌더러는 시뮬이 *쓴 양만* 읽는다. 그래서 렌더 렌즈가 시뮬이 *아직 안 내보내는 양*(예: 에너지 질 `q`)을 필요로 하면, 그 의존을 `RENDER-STATE.md` 에 ⛔ 로 적는 것만으론 **부족**하다 — 시뮬 세션(`hws-step`)은 RENDER 문서를 *읽지 않기* 때문이다. **따라서 렌더가 막힌 의존은 반드시 `STATE.md`(시뮬 SSOT) §2/§3 에 시뮬 작업 항목으로 올린다**(렌더 해제 효과 명시). 그래야 렌더-blind 한 시뮬 세션도 그 step 의 cross-track 결과를 알고 우선순위를 잡는다. 소비자가 현 필드를 다 읽으면 새 표현은 *생산자 선행*이다(병행이 일시적으로 순차로 접힌다 — 모순 아니라 데이터 의존).
...
3. **구현** — 직전 step 을 잇고 항을 *하나만* 더한다(0011~ 은 `engine/hws-laws.js` 에 법칙 1개 + 노브 1개 + `LAW_ORDER` 한 자리; sim-core.js 복사 없음). **step 골격은 `node engine/new-step.js` 로 생성**(직전 step 복사·치환 + md/html 골격 — 손 복사 금지, 생성물은 델타만 수정). 산출물 작성 후 `verify-sim-engine.js` 로 골든 해시 불변 확인.
