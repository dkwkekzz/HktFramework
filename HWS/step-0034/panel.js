/* step-0034 패널 — 부유 R 붕괴(VOXEL.md V5+: 지지 잃은 R[공중 바위]이 아래로 무너진다 = R 의 중력의 동역학 짝). 직전 step-0033(R 차폐) 패널을 잇되 새 노브 행 1개(부유 R 붕괴 kCollapse)를 더했다.
 * 이 step 은 법칙을 *하나* 더한다 — collapse(gravity 뒤): kCollapse>0 이면 각 셀(z≥1)이 아래(z−1) 칸 R<collapseThresh(비지지) 시 제 R 의 kCollapse 비율을 아래로 떨군다(R↔R 쌍 거래). 아래가 고체(R≥문턱)면 안착 → R 이 바닥부터 *쌓인다*.
 * V4 지지 침착(R 이 *생길 때* 지지 없는 칸에 안 굳게 막음·정적)의 *동역학 짝* — 이미 떠 있는 R(풍화·다른 침착이 공중에 깐, 또는 지지가 사라진)을 *무너뜨린다*. gravity(E 의 중력)와 같은 골격이되 옮기는 양이 R, 게이트가 역방향.
 * kCollapse=0 또는 D=1 이면 붕괴 무효(collapse 가 통째/z 벽 early-return) = 직전 step 비트 동일(회귀). 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0034
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 부유 R 붕괴는 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0034 — <span style="color:#d6a05a">부유 R 붕괴</span>: 지지 잃은 공중 바위(R)가 아래로 무너진다 = R 의 중력(VOXEL.md V5+)',
    subtitle: 'V4(step-0032)의 지지 침착은 R 이 *생길 때* 지지 없는 칸에 안 굳게 막았다(정적 — 처음부터 공중 바위가 안 생김). 그러나 *이미 떠 있는* R — 풍화·다른 침착 법칙(튜링·덴드라이트)이 공중에 깐 R, 또는 아래 지지가 사라진 R — 은 V4 가 손대지 않는다. 이 step(V5+)은 새 법칙 <code>collapse</code> 하나를 더한다(gravity 뒤): <b>kCollapse</b> 를 켜면 각 셀(z≥1)이 아래(z−1) 칸 R 이 <b>collapseThresh 미만(=비지지)이면 제 R 의 kCollapse 비율을 아래로 떨군다</b>(donor-제한 <b>R↔R 쌍 거래</b>). 아래가 고체 지면(R≥문턱)이면 안착 → <b>R 이 바닥부터 쌓인다</b>. <code>gravity</code>(E 의 중력)와 *같은 골격*이되 옮기는 양이 R 이고 게이트가 <i>역방향</i>이다 — gravity 는 아래가 고체면 *막히고*(E 가 위에 고임=바다), collapse 는 아래가 비지지면 *떨어진다*(R 이 지지 위에 쌓임=지면). V4(정적 지지 침착)의 <b>동역학 짝</b> — 둘이 짝지어 R 의 중력이 완성된다. <b>kCollapse=0 또는 D=1 이면 붕괴 무효(collapse 가 통째/z 벽 early-return) = 직전 step 비트 동일</b>(회귀 0). 검증: <b>z=5 공중 R 슬랩(아래 빈칸) → 붕괴 off 면 공중 그대로(무게중심 z=5)·on 이면 바닥(z=0)부터 쌓임(무게중심 z=0.46·바닥 2층 100%)</b>·잔차 1.0e-14·결정론. <code>node step-0034/verify.js</code> 로 reg·fall·conserve·det. (표준 스택은 step-0027 덴드라이트 그대로 — 부유 R 붕괴와 직교.)',
    ...
    legend:
      ...
      '<b>부유 R 붕괴(VOXEL.md V5+)</b>: V4 지지 침착은 R 이 *생길 때* 지지 없는 칸에 안 굳게 막았다(정적). 그러나 *이미 떠 있는* R(풍화·다른 침착이 공중에 깐, 또는 지지가 사라진)은 V4 가 손대지 않는다. 이 step 은 새 법칙 <code>collapse</code> 하나를 더한다(gravity 뒤) — 각 셀(z≥1)이 아래(z−1) 칸 R<문턱(비지지)이면 제 R 의 kCollapse 비율을 아래로 떨군다(<b>R↔R 쌍 거래</b>). 아래가 고체(R≥문턱)면 안착 → <b>R 이 바닥부터 쌓인다</b>. <code>gravity</code>(E 의 중력)와 같은 골격이되 옮기는 양이 R, 게이트 역방향(고체면 *막힘*=바다 ↔ 비지지면 *떨어짐*=지면). "터 깊이 D" 8·"부유 R 붕괴" 켜고 리셋해 보라. <code>verify fall</code>: z=5 공중 R 슬랩 → <b>붕괴 off 면 공중 그대로(무게중심 z=5)·on 이면 바닥(z=0)부터 쌓임(무게중심 0.46·바닥 2층 100%)</b>·잔차 1.0e-14. V4(정적)의 동역학 짝 — R 의 중력 완성. (3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더는 렌더러 트랙 L-V1.)<br>' +
  ...
  else global.HWS_PANEL_0034 = panel;
