/* step-0054 패널 — 명시적 질 배출(kQExport): 생명이 먹은 자리 residual q 강등 — 슈뢰딩거 *엔트로피 export* 완성·생명의 *첫 q-쓰기*. 직전 step-0053(질-의존 대사) 패널을 잇되 metabolize(흡수)에 q 강등 한 줄과 노브 kQExport 1개를 더했다.
 ...
 * 이 step 은 생명이 *처음으로 q 를 쓴다*: metabolize(⑦)가 take 를 흡수할 때 먹은 비율만큼 *남은 E 의 질을 깎는다* q[idx] *= (1 − kQExport·(take/Ebefore)). 생명은 고질(엑서지 높은) 크림을 걷어먹고(0053) 자리에 *저질 찌꺼기*만 남긴다 = 슈뢰딩거의 핵심(생명은 자유에너지를 먹어 제 질서를 유지하고 *그 대가로 환경에 엔트로피를 배출*한다). degrade(둘째 법칙·균일 식음)와 달리 *생명이 일한 자리에서만* 깎임 = 능동적 저질 그림자.
 ...
 * 회귀(이중 가드): kQExport=0 → q[idx] 미접촉 바이트 동일(원식 그대로). qInit=false(degrade off)면 미진입(질 축 없는 세계엔 배출할 질 없음). 브라우저: window.HWS_PANEL_0054
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 질 배출은 그 동역학과 직교[E→m 보존]. 이 step 이 생명에게 *질을 쓰게* 한다.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — 질 배출 아레나: verify.js qmetArena()/seedQMetab() 와 *같은* 설정(kQMetab=5 고정 + kQExport 토글).
   *   기본 패널은 회귀 상태(kQExport=0)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나(2D·평탄 E=2·고질[0.9]/저질[0.1] 두 구역·정착 생명 두 무리[move off]·degrade 질 축 alive·kQMetab=5)에 *질 배출(kQExport)* 만 켜고/끄고 두 프리셋으로 A/B 대조한다 —
   *   화면이 보여주는 것 = verify 가 단언하는 수치(OFF 면 고질 자리 q 높게 유지[균일 강등만] / ON 이면 무리가 제 자리 q 를 능동적으로 깎음=저질 그림자). 흑체 색(L-Q 미리보기)으로 보면 ON 일 때 생명 자리가 *식어 어두워진다*(먹은 자리 엔트로피 배출). */
  function qexpArena(kqx) {
    return {
      ...
      kDegrade: 0.01, qInit0: 0, kStarQual: 0, kQAdvect: 0, kQTaxis: 0, kQMetab: 5, kQExport: kqx,
  ...
  function seedQMetab(sim, core) {   // 평탄 E=2 + 고질(0.9)/저질(0.1) 두 구역 + 정착 생명 두 무리(고질 위 y=26~30·저질 위 y=36~40). verify.js seedQMetab() 와 같다(q 축 alive).
    ...
    sim.qInit = true;
    for (gx = 0; gx < 3; gx++) for (gy = 0; gy < 3; gy++) core.spawnAgent(sim, 18 + gx * 2, 26 + gy * 2);   // 고질 무리(q 0.9)
    for (gx = 0; gx < 3; gx++) for (gy = 0; gy < 3; gy++) core.spawnAgent(sim, 18 + gx * 2, 36 + gy * 2);   // 저질 무리(q 0.1)
  }

  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0054 — <span style="color:#ffd060">명시적 질 배출 (kQExport)</span>: 생명이 먹은 자리 질을 깎는다 — 엔트로피 배출(슈뢰딩거 완성·생명의 첫 q-쓰기, SPINE 여섯째 축)',
    subtitle: '0049 강등·0050 별 생산·0051 수송이 질 축 <code>q</code>∈[0,1] 을 깔고, 0052 주화성(질 따라 <b>모임</b>)·0053 대사(질 따라 <b>먹음</b>)로 생명이 질로 가고 질로 산다. 그러나 0049~0053 까지 생명은 q 를 <b>읽기만</b> 했다 — 먹어도 남은 E 의 질은 그대로였다(엔트로피 배출 미명시). 이 step 은 생명이 <b>처음으로 q 를 쓴다</b>: <code>metabolize</code>(⑦)가 take 를 흡수할 때 먹은 비율만큼 <b>남은 E 의 질을 깎는다</b> q[idx] *= (1 − kQExport·(take/Ebefore)). 생명은 고질(엑서지 높은) 크림을 걷어먹고(0053) 자리에 <b>저질 찌꺼기</b>만 남긴다 = 슈뢰딩거의 핵심(생명은 <i>자유에너지</i>를 먹어 제 질서를 유지하고 <b>그 대가로 환경에 엔트로피를 배출</b>한다). <code>degrade</code>(둘째 법칙·균일 식음)와 달리 <b>생명이 일한 자리에서만</b> 깎임 = 능동적 저질 그림자. 척추: <code>q</code> 는 E 에 올라탄 intensive 속성(단일 척추) · 국소(제 disc 칸 q) · q 는 <b>비율</b>이라 강등은 E·m 미접촉(닫힌 장부 — degrade·advection 과 같은 경계). 검증(D=1·평탄 E=2·고질[q 0.9]/저질[q 0.1] 두 구역·정착 생명 두 무리·kQMetab=5, 12 tick): <b>kQExport OFF 면 균일 강등만이라 고질 자리 q 높게 유지(0.80) → ON 이면 무리가 제 자리 q 를 능동적으로 깎음(0.13)</b>·배출은 먹은 질에 비례(고질 자리 깎임 0.67 ≫ 저질 0.05). <b>kQExport=0 → q[idx] 미접촉 → 직전 step(0053) 비트 동일</b>(회귀 0·이중 가드: qInit=false 면도 미진입). <code>node step-0054/verify.js</code> 로 reg·export·conserve·det. 우측 흑체 색(L-Q 미리보기)으로 보면 ON 일 때 생명 자리가 식어 어두워진다(먹은 자리 엔트로피 배출). (표준 스택은 step-0027 덴드라이트 그대로 — 질 배출은 그와 직교[E→m 보존].)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: true, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 의 질 배출을 한 클릭으로 보이는 설정(verify export 와 동일 아레나)으로. 둘 다 평탄 E·고질/저질 두 구역·정착 생명 두 무리·degrade·kQMetab=5 — *질 배출(kQExport)*만 토글. seed 로 두 q 구역 + 두 무리를 심는다. */
    presets: [
      { label: '🔥 질 배출 ON (생명이 먹은 자리가 식어 어두워짐)',
        title: 'verify export(on) 과 같은 설정: D=1·평탄 E=2·고질(q 0.9)/저질(q 0.1) 두 구역·정착 생명 두 무리(고질 위·저질 위·move off)·degrade 0.01·kQMetab=5·*kQExport=1*, 12 tick 진행. 생명이 고질 크림을 걷어먹고 자리에 저질 찌꺼기를 남긴다 → 고질 무리 자리 q 0.80→0.13 으로 능동적으로 깎임(저질 그림자). 우측 흑체 색에서 생명 자리가 *식어 어두워지면* 켜진 것 = 슈뢰딩거 엔트로피 배출(verify 가 단언하는 수치).',
        params: qexpArena(1), seed: seedQMetab, run: 12,
        note: '질 배출 ON — 고질 자리 q 0.80→0.13(생명이 식힘). OFF 버튼과 번갈아 보라.' },
      { label: '❄ 질 배출 OFF (균일 강등만·대조)',
        title: 'verify export(off) 과 같은 설정: 같은 아레나지만 kQExport=0(q 미접촉). 생명이 먹어도 남은 E 질은 안 깎여 degrade 의 *균일* 식음만 → 고질 자리 q 높게 유지(0.80) = 직전 step(0053) 비트 동일. ON 버튼과 번갈아 눌러 A/B 로 비교하면 생명이 *제 자리만* 식히는지가 또렷하다(생명 자리만 어두워짐 vs 균일).',
        params: qexpArena(0), seed: seedQMetab, run: 12,
        note: '질 배출 OFF(대조) — 균일 강등만, 고질 자리 q 0.80 유지. ON 버튼과 번갈아 보라.' }
    ],
  ...
  else global.HWS_PANEL_0054 = panel;
