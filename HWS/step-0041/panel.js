/* step-0041 패널 — 풍화 평형: 물질 순환의 carrying capacity(VOXEL.md V5+ — SPINE 척도분리 *느린 재구성*). 직전 step-0040(에너지 고리 폐합) 패널을 잇되 *법칙 무변경 통합*이라 새 노브 없이 데모 프리셋만 이 가설로 갱신했다.
 * 이 step 은 hws-laws.js 를 *건드리지 않는다* — 풍화(R→E)는 이미 crystallize 안에 있다(step-0008). 검증된 부품(중력·지지 침착·결정화·*풍화*)을 *합성*해, 닫힌 저장소(외부 연료 주입 0)에서 *둘만으론 안 나는 제3 현상*(물질이 동결 대신 끝없이 E↔R 순환)을 창발시키는 통합 step. 통합 게이트 = 기존 노브 kWeather.
 * 완성하는 그림: 0035~0040 이 에너지를 *별→빛→비→바다→지면→새 별*로 완전 폐합했으나, 그 고리는 물질(R)을 한 방향으로만 쌓는다(영구 누적·새 동결 G2 위험). 풍화가 굳은 바위를 천천히 흐르는 E 로 되돌려 *쌓임↔풍화*의 동적 평형(carrying capacity)을 닫는다 — 0012(개체수 자기제한)의 *물질* 짝.
 * 회귀: kWeather=0 이면 풍화 분기 no-op(rel=R·0=0) → 물질이 통째로 R 로 동결(0040 까지의 동역학 그대로)=비트 동일. 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0041
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 풍화 평형은 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — 풍화 평형 closed 아레나: verify.js weqArena() 와 *같은* 설정(외부 연료 주입 0·균일 E·중력→바다·지지 침착→지면·D=8).
   *   기본 패널은 회귀 상태(D=1)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 닫힌 저장소에 *풍화(kWeather)* 만 켜고/끄고 두 프리셋으로 A/B 대조한다 — 화면이 보여주는 것 = verify 가 단언하는 수치(풍화 OFF 면 물질이 통째로 R 로
   *   동결[E 굶음]·ON 이면 R 평형 ↓·E 흐름 4배↑ 동적 순환). 풍화 ON 이면 R(호박 점)이 더 옅고 E(밝기)가 더 살아있다(물질이 흐름에 남는다). */
  function weqArena(kW) {
    return {
      ...
      kGravity: 0.2, kFSM: 0, kCollapse: 0, kCryst: 0.05, crystThresh: 2.0, kSupport: 1, supportThresh: 0.5, kWeather: kW, kOcclude: 0,
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0041 — <span style="color:#ffd060">풍화 평형</span>: 풍화가 굳은 바위를 흐름으로 되돌려 물질에 carrying capacity 를 준다 — 세계가 바위로 동결하지 않고 끝없이 순환(VOXEL.md V5+ · SPINE 척도분리 느린 재구성)',
    subtitle: '0035~0040 이 에너지를 <b>별→빛→비→바다→지면→새 별</b>로 완전 폐합했다. 그러나 그 고리는 물질(R)을 한 방향으로만 쌓는다 — 결정화(E→R)가 바다를 지면으로 굳히면 R 은 <b>영구 누적</b>. 전부 비가역이면 세계는 죽은 바위로 <b>래칫</b>되어 새 동결(SPINE G2)에 빠진다. 이 step 은 그 고리의 마지막 반쪽 = <b>느린 재구성(풍화)</b> 을 닫는다: 풍화(<code>kWeather</code>·R→E·step-0008 부터 존재)가 굳은 바위를 천천히 흐르는 E 로 되돌려 물질이 <b>쌓임↔풍화</b>의 동적 평형(carrying capacity)에 든다 — 0012(개체수 자기제한)의 <b>물질</b> 짝. 이 step 은 <b>hws-laws.js 를 안 건드린다</b>(법칙 무변경 통합): 통합 게이트 = 기존 노브 <b>kWeather</b>. 닫힌 저장소(외부 주입 0)에서: <b>풍화 OFF 면 결정화가 모든 가용 E 를 R 로 굳혀 R 이 천장(~155648)에 래칫·E 가 굶어(~8192) 세계가 바위로 동결 → ON(kWeather=0.01) 이면 R 이 더 낮은 평형(~129489·천장 ↓)에 멈추고 E 가 4.19배(~34351) 더 흐름에 남는다</b>. R 이 평형이어도 풍화 flux(~1.3e6/1000tick)는 안 멈춘다 — 정적 동결 아니라 <b>동적 순환</b>(세계 척도 churn 영속). <b>kWeather=0 이면 풍화 분기 no-op → 물질 통째 동결(0040 까지 동역학 그대로)=비트 동일</b>(회귀 0). 잔차 1.4e-13·결정론. <code>node step-0041/verify.js</code> 로 reg·equil·conserve·det. (표준 스택은 step-0027 덴드라이트 그대로 — 풍화 평형과 직교.)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: true, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 이 합성한 풍화 평형을 한 클릭으로 보이는 설정(verify equil 과 동일 closed 아레나)으로. 둘 다 균일 E·중력→바다·지지 침착→지면 on — *풍화*만 토글. */
    presets: [
      { label: '🪨 풍화 ON (쌓임↔풍화 동적 평형·D=8)',
        title: 'verify equil(on) 과 같은 설정: D=8 닫힌 저장소(외부 주입 0)·균일 E(initE=5)·중력 kGravity=0.2(→바다)·지지 침착 kSupport=1(→지면)·*풍화 kWeather=0.01*, 1000 tick 미리 진행. 굳은 R 이 천천히 E 로 되돌아와 물질이 동적 평형(R 평형 ~129489·E 흐름 ~34351 — OFF 의 4.19배). R(호박 점)이 더 옅고 E(밝기)가 더 살아있다 = 물질이 흐름에 남는다. (3D 뷰는 아직 z=0 평면만 — voxel 렌즈 L-V1 백로그. 수치가 증거.)',
        params: weqArena(0.01), run: 1000,
        note: '풍화 ON — R 평형 ↓·E 흐름 4배↑(쌓임↔풍화 carrying capacity·동결 회피). OFF 버튼과 번갈아 보라.' },
      { label: '🧊 풍화 OFF (물질이 바위로 동결·대조)',
        title: 'verify equil(off) 과 같은 설정: 같은 닫힌 저장소지만 kWeather=0. 결정화가 모든 가용 E 를 R 로 굳혀 R 이 천장(~155648)에 래칫·E 가 굶어(~8192) 세계가 바위로 동결(풍화 0·새 동결 G2). ON 버튼과 번갈아 눌러 A/B 로 비교하면 풍화의 carrying capacity 가 또렷하다.',
        params: weqArena(0), run: 1000,
        note: '풍화 OFF(대조) — R 천장 래칫·E 굶어 동결(물질이 바위로). ON 버튼과 번갈아 눌러 풍화 평형을 보라.' }
    ],
    ...
    stats: [
      ...
      { label: '<span style="color:#ffd060">물질 평형</span> (R 저장 / E 흐름 / 풍화량 — 가설: 쌓임↔풍화)', get: function (c) {
          var D = c.sim.p.D || 1, gOn = (c.sim.p.kGravity || 0) !== 0, wOn = (c.sim.p.kWeather || 0) !== 0;
          var R = c.sim.R, E = c.sim.E, N = R.length, rTot = 0, eTot = 0, k; for (k = 0; k < N; k++) { rTot += R[k]; eTot += E[k]; }
          var wth = c.sim.weathered || 0;
          if (D < 2 || !gOn) return { text: 'R ' + rTot.toFixed(0) + ' / E ' + eTot.toFixed(0) + ' (z=0/중력 off — 풍화 평형 비활성. 위 "풍화 ON" 버튼으로)', cls: '' };
          return { text: 'R ' + rTot.toFixed(0) + ' / E ' + eTot.toFixed(0) + ' / 풍화 ' + (wth >= 1e4 ? wth.toExponential(1) : wth.toFixed(0)), cls: (wOn && wth > 0 && eTot > 1e4) ? 'pass' : '' };
      } },
    ...
    legend:
      ...
      '<b>풍화 평형 — 물질 순환의 carrying capacity(VOXEL.md V5+ · SPINE 척도분리 느린 재구성)</b>: 0035~0040 이 에너지를 <b>별→빛→비→바다→지면→새 별</b>로 완전 폐합했으나, 그 고리는 물질(R)을 한 방향으로만 쌓는다(영구 누적·새 동결 G2 위험). 이 step 은 그 고리의 마지막 반쪽 = <b>느린 재구성(풍화)</b> 을 닫는다 — <b>hws-laws.js 를 안 건드리고</b>(풍화 R→E 는 이미 crystallize 안·step-0008) 검증된 부품(중력·지지 침착·결정화·<b>풍화</b>)을 <b>합성</b>해 닫힌 저장소에서 물질이 <b>쌓임↔풍화</b>의 동적 평형(carrying capacity)에 들게 한다 = 0012(개체수 자기제한)의 <b>물질</b> 짝. 통합 게이트 = 기존 노브 <b>kWeather</b>. "터 깊이 D" 8·"중력"·"지지 침착"·"결정화" 켜고 리셋해 보라(위 "풍화 ON" 버튼). <code>verify equil</code>: <b>풍화 OFF 면 결정화가 모든 가용 E 를 R 로 굳혀 R 천장(~155648)·E 굶어(~8192) 세계가 바위로 동결 → ON(kWeather=0.01) 이면 R 평형 ~129489(천장 ↓)·E 흐름 ~34351(4.19배)·풍화 flux ~1.3e6/1000tick 영속(R 평형이어도 churn 안 멈춤)</b>·잔차 1.4e-13. 부품 합성이 빚은 제3의 현상(어느 하나만으론 안 남) — 물질이 동결 대신 끝없이 순환. (3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더는 렌더러 트랙 L-V1.)<br>' +
  ...
  else global.HWS_PANEL_0041 = panel;
