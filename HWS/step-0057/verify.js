/* HWS step-0057 헤드리스 검증 — relief advection(kQRelief — 질이 *기복(무대 비탈)* E 흐름에 동승, SPINE 여섯째 축 transport 짝).
 * 0051 gravity(하향)·0055 diffuse-선형(등방)·0056 couple(막 공유)로 질이 *대부분* E 이동을 따라갔으나, diffuse 의 *기복 경로*(kRelief≠0·무대 비탈 donor-제한 upwind)는 q 를 안 데려갔다 —
 *   무대 둑 안으로 E 가 흘러도 q 는 제자리에 stranded(기복 flux 가 E 만 옮기고 질은 두고 감).
 *   이 step 은 질을 기복 flux 에도 싣는다: diffuse 기복 경로가 셀 E 를 이웃과 교환할 때, 받는 칸은 *들어온 E 의 질*을 질량가중으로 섞는다(new_q = (q·retained + Σ q_neighbor·inflow)/기복후E).
 *   고질 E 가 기복으로 번지면 q 도 따라 번진다 = q 가 *모든 E 이동*(하향[0051]+등방[0055]+막 공유[0056]+기복[이번])을 따라가는 완전한 물리장(E-이동 법칙 전반 질 수송 완성).
 *   척추: q 는 E 에 올라탄 intensive 속성(단일 척추)·국소(제 4-이웃)·advection 은 E 미접촉(기복 flux 가 *이미 옮긴* E 의 질만 따라감 — q 는 비율·거래 0, 닫힌 장부). 0055 diffuse-선형의 *기복(무대 비탈)* 짝.
 *   회귀(이중 가드): kQRelief=0 → advection 미진입(QB 미할당·미스왑·0056 비트 동일). qInit=false(degrade off)면 미진입. *교차 버전* 회귀는 verify-sim-engine.js 골든(qrlf@ 포함 전 시나리오 비트 불변)이 권위.
 *
 * 사용: node step-0057/verify.js <reg|advect|conserve|det|all> [seed]
 *  - reg      : 회귀 0 — kQRelief=0 → q 미접촉(0056 비트 동일)·2회 실행 비트 동일. *교차 버전* 회귀는 verify-sim-engine.js 골든이 권위.
 *  - advect   : 가설 — *질이 기복 E 를 따라 번진다*. kQRelief off vs on: 중앙 고질 블록이 기복으로 확산. off 면 q 가 중앙에 stranded(번진 자리 q≈0) → on 이면 q 가 E 따라 사방으로 advect(ring q↑).
...
/* relief advection 아레나 — verify-sim-engine.js qrlfArena()/seedQBlob() 와 동일 상수(골든 qrlf@ 와 일치).
 * 2D(D=1), 중앙 8×8 고질 E 블록(E 10·q 1) + 빈 배경(E 0·q 0) + diffuse 기복 경로(kRelief=1·h=E+kRelief·R, R=0 이라 h=E·kD=0.2 rate) + degrade(질 축 alive) + kQRelief 토글. 별·생명·중력 다 off(순수 기복 수송). qInit0=0. */
...
function seedQBlob(sim) {   // 중앙 8×8 고질 E 블록(E 10·q 1) + 빈 배경 — 기복이 사방으로 펴며 advection 이 질을 데리고 번진다. q 축 alive.
  var cx = (W >> 1) - 4, cy = (H >> 1) - 4, x, y;
  for (y = cy; y < cy + 8; y++) for (x = cx; x < cx + 8; x++) { var i = y * W + x; sim.E[i] = 10; sim.E0 += 10; sim.q[i] = 1; }
  sim.qInit = true;
}
var TICKS = 20;
function build(seed, kqr, ticks) { var s = ENG.createSim(seed, qrlfArena({ kQRelief: kqr })); seedQBlob(s); for (var t = 0; t < ticks; t++) ENG.step(s); return s; }

/* ring/center 질 — 중앙(블록 안)과 ring(블록 밖 Chebyshev 거리 6~9, 기복 E 가 닿은 자리)의 평균 q.
 * advection 척도: kQRelief on 이면 번진 E 가 질을 데려가 ring q↑(고질이 사방 advect). off 면 q 가 중앙에 stranded(번진 자리 q≈0). E>1e-6 칸만(빈칸 제외). */
function ringStats(sim) {
  ...
  for (y = 0; y < H2; y++) for (x = 0; x < W2; x++) {
    ...
    else if (d >= 6 && d <= 9) { sr += q[i]; nr++; }  // ring(블록 밖 기복 자리)
  }
  return { qCenter: nc ? sc / nc : 0, qRing: nr ? sr / nr : 0, nCenter: nc, nRing: nr };
}

/* ── reg: 회귀 0 — kQRelief=0 → q 미접촉(0056 비트 동일)·2회 실행 비트 동일. 교차 버전은 골든이 권위. ── */
function reg(seed) {
...
/* ── advect: 가설 — 질이 기복 E 를 따라 번진다. kQRelief off vs on(둘 다 중앙 고질 블록 기복 확산). ── */
function advectH(seed) {
  ...
  return {
    ...
    pass: sOn.qRing > 0.05 && sOff.qRing < 0.01 && sOn.qRing > sOff.qRing + 0.05
  };
}
...
function runMode(mode, seeds) {
  if (mode === 'reg') {
    ...
    console.log('회귀 0: kQRelief=0 → q 미접촉(advection 미진입·QB 미스왑·0056 비트 동일) → q 가 중앙에 stranded(번진 ring 자리 q≈' + avg(rr, 'qRing').toFixed(3) + ')·2회 실행 비트 동일. *교차 버전* 회귀는 verify-sim-engine.js 골든 해시(qrlf@ 포함 전 시나리오 비트 불변·새 노브 kQRelief=0 → q 미접촉)가 권위.');
    ...
    console.log('질이 *기복 E 를 따라 번진다*(SPINE 여섯째 축 transport — 0055 diffuse-선형의 기복[무대 비탈] 짝): D=1·중앙 8×8 고질 블록(E 10·q 1)·빈 배경·기복(kRelief 1·kD 0.2)·' + TICKS + ' tick. kQRelief OFF 면 E 는 번져도 q 가 중앙에 stranded(ring q ' + avg(rp, 'qRingOff').toFixed(3) + '≈0) → ON 이면 q 가 E 따라 사방으로 advect(ring q ' + avg(rp, 'qRingOn').toFixed(3) + ' ≫ OFF). advection 은 E 미접촉(기복 flux 가 옮긴 E 의 질만 따라감)·국소(제 4-이웃)·질량가중 혼합. 중앙 q 는 번져나가며 옅어짐(ON ' + avg(rp, 'qCenterOn').toFixed(3) + ').');
    return rp.every(function (r) { return r.pass; });
  } else if (mode === 'conserve') {
    var rc = seeds.map(conserve); table(rc, ['seed', 'residual', 'pass']);
    console.log('avg residual=' + avg(rc, 'residual').toExponential(3) + ' (advection 은 E 미접촉 — q 는 *비율*[기복 flux 가 이미 옮긴 E 의 질만 질량가중으로 따라감·거래 0·degrade·gravity/diffuse/couple advection 과 같은 경계]. 닫힌 장부 유지).');
