/* step-0033 패널 — R 차폐(VOXEL.md V5: 중력으로 흐르는 E 가 아래 R[지면]에 막혀 위에 고임 = 바다가 지면 *위에*). 직전 step-0032(지지 침착) 패널을 잇되 새 노브 행 1개(R 차폐 kOcclude)를 더했다.
 * 이 step 은 법칙을 더하지 않는다 — gravity 의 하향 유출에 *차폐 게이트*를 더한다(게이트 노브 1 + 문턱 1, in-place). kOcclude>0 이면 아래(z−1) 칸 R≥occludeThresh 시 하향 유출 차단 → E 가 못 내려가 지면 *위에* 고인다(옆 우회는 확산). V3 중력(바다)을 *완성* — E 가 지면을 통과 못 한다.
 * kOcclude=0 또는 D=1 이면 차폐 무효(gravity 가 V3 순수 하향/z 벽 early-return) = 직전 step 비트 동일(회귀). 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0033
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — R 차폐는 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0033 — <span style="color:#5ad0ff">R 차폐</span>: 흐르는 E 가 지면(R)에 막혀 그 위에 고인다 = 바다가 지면 위에(VOXEL.md V5)',
    subtitle: 'V3(step-0031)의 중력은 E 를 *순수 하향*으로만 흘렸다 — 아래에 지면(R)이 있어도 <i>통과</i>해 바닥까지 내려갔다(바다가 지면 아래로 샘). 이 step(V5)은 <code>gravity</code> 의 하향 유출에 <b>차폐 게이트</b>를 더한다(법칙 신설 아님, in-place): <b>kOcclude</b> 를 켜면 아래(z−1) 칸 R 이 <b>occludeThresh 이상(=고체 지면)이면 하향 유출을 차단</b>한다 — E 가 못 내려가 <b>지면 위에 고인다</b>. 옆으로 우회(골 채움)는 확산(①)이 담당 — <b>바다가 지면 *위에* 고인다</b>. V4 지지 침착(R 의 중력=지면)이 세운 지면을 V3 중력(E 의 중력=바다)이 통과 못 하게 막아 — V3 중력을 <i>완성</i>한다. <b>kOcclude=0 또는 D=1 이면 차폐 무효(gravity 가 V3 순수 하향/z 벽 early-return) = 직전 step 비트 동일</b>(회귀 0). 검증: <b>z=3 정적 R 지면 + 그 위 E + 중력 → 차폐 off 면 지면 통과(지면 위 E 분율 0%·최저 z=0)·on 이면 지면 위(z=4)에 고임(분율 100%)</b>·잔차 0·결정론. <code>node step-0033/verify.js</code> 로 reg·bath·conserve·det. 부유 R 붕괴·다른 침착 게이트는 V5+ 로 전가. (표준 스택은 step-0027 덴드라이트 그대로 — R 차폐와 직교.)',
    ...
    legend:
      ...
      '<b>지지 침착(VOXEL.md V4)</b>: V3 중력으로 <i>E</i> 는 바닥에 고이나(바다) <i>물질 R</i> 은 아직 공중에 떠 굳을 수 있다. 이 step 은 <code>crystallize</code> 의 E→R 침착에 <b>지지 게이트</b>를 더한다 — <b>지지 있는 칸</b>(z=0 바닥 또는 아래 R≥문턱)에서만 굳어 <b>공중 바위(부유 R)를 막고 지면이 바닥부터 쌓인다</b>. "터 깊이 D" 8·"중력"·"결정화"·"지지 침착" 을 켜고 리셋해 보라. <code>verify ground</code>: 천장 E 슬랩 + 중력 → <b>무게이트면 공중 바위 12288칸·게이트면 부유 R 0칸·R 이 바닥(z=0)부터</b>·잔차 1e-15. R 차폐·부유 R 붕괴는 V5+. (3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더는 렌더러 트랙 L-V1.)<br>' +
  ...
  else global.HWS_PANEL_0033 = panel;
