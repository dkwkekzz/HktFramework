/* HWS step-0009 sim-core — 저장체가 흐름을 휘면 *무대*가 된다: 퇴적이 자원 풍경을 내생적으로 재편
 * step-0008(확산·증발·구동·응집·생명·번식·이동·기초대사비·떠도는 자원·결정화/풍화) 을 잇는다. 더해진 것은 단 하나: 기복(relief).
 *   ── 가설 (STATE.md 지정) ──
 *   step-0008 의 저장체 R 은 *비활성 저장*이었다 — 쌓이기만 하고 세계에 아무 영향이 없다. step-0009 는 사슬의
 *   다음 고리: **저장체가 무대가 된다**. 굳은 땅은 흐름을 통과시키지 않는다 — 흐름이 저장체를 *피해* 휘고,
 *   응집 고임이 저장체 둘레에 재편된다. 고임이 자기 자리를 퇴적으로 메우면(결정화) 흐름이 옆으로 밀려 새 고임이
 *   다른 곳에 선다 — 외부 sawtooth(step-0007) 없이 *내생적으로* 움직이는 자원 풍경(G2/G3 후보).
 *   메커니즘 — 기복(하상 융기, avulsion): 퇴적 R 은 *바닥을 올린다*. 흐름 퍼텐셜 h = E + kRelief·R 을 정의하고
 *     확산을 h 의 내리막 방향 donor-제한 upwind flux 로 일반화한다:
 *       유출 수요 dem_i = kD·Σ_j max(0, h_i−h_j),  제한 f_i = min(1, E_i/dem_i)   (E 만 흐른다 — R 은 부동)
 *       변 flux(i→j) = f_i·kD·max(0, h_i−h_j)
 *     같은 변 항을 주는 쪽·받는 쪽이 동일 식으로 계산하므로 **쌍 거래 보존**(장부 불변)이고, f-제한으로 유출이
 *     E_i 를 넘지 않아 **양수성** 보장. R=0 인 곳에선 max(0,x)−max(0,−x)=x 라 *대수적으로 기존 선형 확산과 동일*
 *     하고(dem ≤ 4kD·E < E 라 f=1), 퇴적 위에서만 물리가 갈린다 — 물은 굳은 언덕에 머물지 못하고 흘러내린다.
 *   왜 전도도 차단(σ=1/(1+kR))이 아닌가(시도 1, 부정적 결과): 통과율을 낮추면 흐름이 휘는 게 아니라 *갇힌다* —
 *     source 셀 자신이 굳는 순간 주입 E 가 빠져나가지 못해 무한 결정화(별이 자기 퇴적에 매몰), 세계의 자유 E 가
 *     암석으로 붕괴해 고임이 전멸한다(kBlock×kWeather 전 스캔에서 확인). 기복은 반대로 자기 제한적이다 —
 *     쌓일수록 흐름이 *비켜가서* 그 자리의 결정화가 멎는다("고임이 자기 자리를 메우면 흐름이 옆으로 밀림"의 직역).
 *   국소성(척추 결정3·4): h 는 그 셀의 로컬 E·R 만 본다 — 전역 조율자 0. 무대 효과는 R 크기의 연속 함수
 *     (활성도 환원: authored type 분기 아님 — "암석"이라는 종류가 아니라 R 스칼라의 높이가 흐름을 휜다).
 *   응집(kA)은 변조하지 않는다(한 step=한 항) — 농도 창 응집은 여전히 E 값으로. 기복은 확산항만 일반화한다.
 *   3D 정합: hws-3d 가 지형 높이를 E+R 로 그린다(DESIGN-3D) — kRelief=1 이면 *보이는 지형이 곧 흐름 퍼텐셜*.
 * 순서: ①확산(+응집, *기복 신규*) ②증발 ③주입 ④배출 ⑤결정화·풍화 ⑥이동 ⑦생명 ⑧번식. 순서 불변.
 * 회귀: kRelief=0 이면 기복 분기가 통째로 건너뛰어져 step-0008 의 확산 루프를 같은 순서로 수행 → 비트 동일.
 *   kCryst=0 까지 끄면 step-0007, srcJump=0 이면 step-0006, … (회귀 체인 그대로).
 * 닫힌 장부: sumE + M(=Σm) + R(=ΣR) + evaporated + sunk + metabolized - injected = E0. (step-0008 그대로 —
 *   기복은 확산의 방향·세기만 바꿀 뿐 새 외부 항이 없다.)
 * 브라우저/Node 겸용. step-0009.html 의 셸이 이 파일을 그대로 로드한다(window.HWS9).
 */
(function (global) {
  'use strict';
  ...
  var DEFAULTS = {
    ...
    /* ── step-0009 신규: 무대(기복 — 퇴적이 바닥을 올려 흐름이 굳은 땅을 비켜간다) ── */
  ...
  /* 법칙 적용 순서: ①확산(+응집, 기복) ②증발 ③주입 ④배출 ⑤결정화·풍화 ⑥이동 ⑦생명 ⑧번식 — 순서 고정. */
  function step(sim) {
   ...
   * 기복(step-0009)은 확산의 방향·세기만 바꿀 뿐 쌍 거래 그대로라 장부 식이 불변이다. */
  function ledger(sim) {
    var sumE = 0, E = sim.E;
  ...
  var api = {
    ...
    detectPools: detectPools, harvest: harvest, paintStore: paintStore, localE: localE, localStore: localStore,
