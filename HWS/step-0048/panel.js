/* step-0048 패널 — 3D 막/flux 결합: couple 의 kin E-공유를 연직축으로(VOXEL.md V5+). 직전 step-0047(3D 생명 유전 상속) 패널을 잇되 couple 법칙 *제자리 확장* + 노브 kCoupleZ 1개를 더했다.
 * 0045 가 z>0 거주 생명을 3D 로 kin 정렬했고 0046·0047 이 생물량 공유·유전 상속을 z 로 풀었다. 그러나 막/flux 결합(couple·step-0018 kin 끼리 필드 E 를 공유해 막이 표면으로 창발)은 여전히 2D 평면만 봤다 — kin 쌍 우(+x)/하(+y) 4-인접인데 하 dc=((y+1)%H)·W+x 가 z 성분을 떨궈(z=0 평면) z>0 kin 은 위/아래 동료와 E 를 공유 못 함(3D 막 누수). 이 step 은 couple 의 occ 그리드 W·H→W·H·D·kin 쌍에 위(+z) 추가(하 dc 도 z 평면 키로 교정·4-인접→6-인접) 로 일반화한다 — z>0 kin 이 위/아래 동료와 필드 E 를 공유한다 = 0045 가 정렬한 3D 액적이 비로소 *연직으로 균질한 내부*(막의 연직 일반화·step-0018 의 3D 짝)를 갖는다.
 * 형식: 0046 share·0047 inherit 의 z-확장과 같은 *제자리 확장*(새 LAW_ORDER 자리 없음) + 노브 kCoupleZ. 공유는 E 쌍 거래(균등화·보존)·E 는 늘 해시 → 골든이 3D 막을 잠근다(별도 게이트 불필요).
 * 회귀(이중 가드): kCoupleZ=0 → 3D 블록 미진입(2D 경로)=직전 step 비트 동일·z>0 kin 은 하 dc 가 z=0 평면이라 위/아래 동료와 공유 못 함 / D=1 → z 이웃 없어 2D 등가. 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0048
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 3D 막/flux 결합은 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — 3D 막/flux 결합 아레나: verify.js cplArena()/seedCoupleColumns 와 *같은* 설정.
   *   기본 패널은 회귀 상태(D=1)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나(D=8·수직 kin 컬럼 3개[제 칸 E z 짝수 고·홀수 저]·couple 만 on·나머지 동역학 off)에 *연직 막 공유(kCoupleZ)* 만 켜고/끄고 두 프리셋으로 A/B 대조한다 —
   *   화면이 보여주는 것 = verify 가 단언하는 수치(kCoupleZ OFF 면 z>0 kin 이 2D 평면[하 dc=z=0]이라 위/아래 동료와 공유 0·E 연직 불균질 / ON 이면 z±1 kin 이 E 균등화·연직 균질). (3D 뷰는 아직 z=0 투영 — voxel 렌즈 L-V1 백로그·통계 "E spread"가 증거.) */
  function cplArena(kCZ) {
    return {
  ...
  function seedCoupleColumns(sim, core) {   // 수직 kin 컬럼 3개(태그 1·제 칸 E z 짝수 3.0·홀수 1.0·m=0.5 E 서 떼옴 → 짝수 2.5·홀수 0.5) — verify.js seedCoupleColumns 와 동일
    var W = sim.p.W, H = sim.p.H, WH = W * H, D = sim.p.D, cols = [[16, 16], [22, 22], [28, 28]];
    for (var i = 0; i < WH * D; i++) { sim.E[i] = 0; }
    for (var c = 0; c < cols.length; c++) for (var z = 0; z < D; z++) {
      var x = cols[c][0], y = cols[c][1], center = z * WH + y * W + x;
      sim.E[center] = (z & 1) ? 1.0 : 3.0; sim.E0 += sim.E[center];
      sim.E[center] -= 0.5;
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0048 — <span style="color:#ffd060">3D 막/flux 결합</span>: 막이 연직축으로 — z>0 kin 이 위/아래 동료와 필드 E 를 공유한다(VOXEL.md V5+)',
    subtitle: '0045 가 z>0 거주 생명을 3D 로 kin 정렬했고 0046·0047 이 생물량 공유·유전 상속을 z 로 풀었다. 그러나 막/flux 결합(<code>couple</code>·step-0018 kin 끼리 필드 E 를 공유해 막이 표면으로 창발)은 여전히 2D 평면만 봤다 — kin 쌍 우(+x)/하(+y) 4-인접인데 하 <code>dc=((y+1)%H)·W+x</code> 가 z 성분을 떨궈(z=0 평면) z>0 kin 은 위/아래 동료와 E 를 <b>공유 못 함</b>(3D 막 누수). 이 step 은 <code>couple</code> 의 <b>occ 그리드 W·H→W·H·D·kin 쌍에 위(+z) 추가(하 dc 도 z 평면 키로 교정·4-인접→6-인접)</b> 로 일반화한다: z>0 kin 이 제 위/아래 동료와 필드 E 를 공유한다 = <b>0045 가 정렬한 3D 액적이 비로소 연직으로 균질한 내부(막)를 갖는다</b>(막의 연직 일반화·step-0018 의 3D 짝). 형식: 0046 <code>share</code>·0047 <code>inherit</code> z-확장과 같은 <b>제자리 확장</b>(새 LAW_ORDER 자리 없음) + 노브 <b>kCoupleZ</b>. 검증(D=8·수직 kin 컬럼 3개[태그 1·제 칸 E z 짝수 2.5·홀수 0.5]·couple 만 on): <b>kCoupleZ OFF 면 z>0 kin 이 2D 평면(하 dc=z=0)이라 위/아래 동료와 공유 0(E spread 2.0 그대로) → ON 이면 z±1 kin 이 E 를 균등화해 공유>0·spread 0.18(컬럼 평균 1.5 로 모임)</b>. <b>kCoupleZ=0 → 3D 블록 미진입(2D 경로)=비트 동일 / D=1 → 2D 등가</b>(회귀 0·이중 가드). 공유는 E 쌍 거래(균등화·보존·잔차 0)·결정론. <code>node step-0048/verify.js</code> 로 reg·mix·conserve·det. (표준 스택은 step-0027 덴드라이트 그대로 — 3D 막/flux 결합과 직교.)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: true, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 의 3D 막/flux 결합을 한 클릭으로 보이는 설정(verify mix 와 동일 아레나)으로. 둘 다 수직 kin 컬럼 3개·couple 만 on — *연직 막 공유(kCoupleZ)*만 토글. */
    presets: [
      { label: '🧫 3D 막/flux 결합 ON (z>0 연직 막·D=8)',
        title: 'verify mix(on) 과 같은 설정: D=8·수직 kin 컬럼 3개(태그 1·제 칸 E z 짝수 고=2.5·홀수 저=0.5)·couple 만 on·*연직 막 공유 kCoupleZ=1*, 20 tick 진행. z>0 kin 이 제 위/아래(z±1) 동료와 필드 E 를 균등화해 컬럼 내부 E 가 평균(1.5)으로 모인다(E spread 2.0→0.18) = 0045 가 정렬한 3D 액적이 비로소 연직으로 균질한 내부(막)를 가짐. 통계 "E spread"가 0 근처면 연직 막이 켜진 것 = verify 가 단언하는 수치. (3D 뷰는 z=0 투영이라 컬럼이 한 점으로 겹쳐 보인다 — voxel 렌즈 L-V1 백로그·통계가 증거.)',
        params: cplArena(1), seed: seedCoupleColumns, run: 20,
        note: '3D 막/flux 결합 ON — z>0 kin 이 z±1 동료와 E 균등화(spread 2.0→0.18). OFF 버튼과 번갈아 보라(통계 "E spread").' },
      { label: '🛑 3D 막/flux 결합 OFF (z>0 kin 불균질·대조)',
        title: 'verify mix(off) 과 같은 설정: 같은 아레나·컬럼이지만 kCoupleZ=0. couple 이 2D 평면(하 dc=z=0)만 봐 z>0 kin 이 제 위/아래 동료와 E 를 공유 못 한다(공유 0·E spread 2.0 그대로·짝/홀 z 교번 불균질 — 바로 옛 2D couple 이 z>0 kin 을 못 잇던 caveat). ON 버튼과 번갈아 눌러 A/B 로 비교하면 연직 막이 또렷하다.',
        params: cplArena(0), seed: seedCoupleColumns, run: 20,
        note: '3D 막/flux 결합 OFF(대조) — z>0 kin 이 위/아래 동료와 공유 못 함(spread 2.0). ON 버튼과 번갈아 눌러 연직 막을 보라.' }
    ],
    ...
    stats: [
      ...
      { label: '<span style="color:#ffd060">E spread</span> (생명 칸 E 최대−최소 — 가설: 연직 막이 균질화)', get: function (c) {
          var sim = c.sim, ag = sim.agents, E = sim.E, D = sim.p.D || 1; if (!ag.length) return { text: '생명 없음', cls: '' };
          var mn = Infinity, mx = -Infinity; for (var k = 0; k < ag.length; k++) { var e = E[ag[k].center]; if (e < mn) mn = e; if (e > mx) mx = e; }
          var czOn = (sim.p.kCoupleZ || 0) !== 0 && D > 1, spread = mx - mn;
          return { text: 'spread ' + spread.toFixed(3) + ' (최소 ' + mn.toFixed(2) + ' / 최대 ' + mx.toFixed(2) + ')' + (czOn ? ' — 3D 막 on(균질화)' : '') + ' · 공유 ' + (sim.coupled || 0).toFixed(2), cls: czOn && spread < 0.5 ? 'pass' : '' };
      } },
    ...
    legend:
      ...
      '<b>3D 막/flux 결합 — couple kin E-공유의 연직축 일반화(VOXEL.md V5+ · SPINE 다섯째 축)</b>: 0042·0043 이 생명을 z>0 으로 올렸고(이동·번식), 0044~0047 이 혼잡·정렬·공유·유전을 z 로 풀었다. z>0 kin 이 3D 액적으로 뭉친다 — 그런데 <b>막/flux 결합</b>(<code>couple</code>·step-0018 = kin 끼리 필드 E 를 공유해 막이 *표면으로 창발*)은 여전히 2D 평면만 봤다 — kin 쌍 우/하 4-인접인데 하 <code>dc=((y+1)%H)·W+x</code> 가 z 성분을 떨궈[z=0 평면] z>0 kin 은 <b>위/아래 동료와 E 를 공유 못 했다</b>(3D 막 누수). 이 step 은 <code>couple</code> 의 <b>occ 그리드 W·H→W·H·D·kin 쌍에 위(+z) 추가(하 dc 도 z 평면 키로 교정·4-인접→6-인접)</b> 로 <b>제자리 확장</b>(0046 share·0047 inherit 와 같은 형식)한다: z>0 kin 이 제 위/아래 동료와 필드 E 를 공유한다 = <b>0045 가 정렬한 3D 액적이 비로소 연직으로 균질한 내부(막)를 갖는다</b>. 노브 <b>kCoupleZ</b>(공유는 E 쌍 거래로 골든 잠금). "3D 막/flux 결합" 켜고 "터 깊이 D" 8 로 리셋해 보라(위 "3D 막/flux 결합 ON" 버튼). <code>verify mix</code>(수직 kin 컬럼 3개[태그 1·제 칸 E z 짝수 2.5·홀수 0.5]): <b>kCoupleZ OFF 면 z>0 kin 이 2D 평면(하 dc=z=0)이라 위/아래 동료와 공유 0(E spread 2.0 그대로) → ON 이면 z±1 kin 이 E 균등화·공유>0·spread 0.18(컬럼 평균 1.5 로 모임)</b>. E 쌍 거래(균등화·나간 만큼 들어옴 — 보존)라 막은 창발일 뿐 author 0(척추 체크). <b>kCoupleZ=0 → 3D 블록 미진입=비트 동일 / D=1 → z 이웃 없어 2D 등가</b>(회귀 0·이중 가드). (3D 뷰는 아직 z=0 투영 — voxel 렌더는 렌더러 트랙 L-V1·통계 "E spread"가 증거.)<br>' +
