/* step-0048 패널 — 생명의 폐열(슈뢰딩거 음엔트로피·소산구조, SPINE 여섯째 축). 직전 step-0047(에너지의 질) 패널을 잇되 metabolize 제자리 확장 + 노브 kLifeHeat 1개를 더했다.
 * step-0047 가 E 에 질 축(자유 E ↔ 열 E[Eth])을 열었으나, 질은 아직 *측정 토대*였다 — 동역학에 안 먹였다. 슈뢰딩거 핵심: 생명은 음엔트로피(자유에너지)를 먹고 엔트로피(폐열)를 환경에 배출하는 far-from-equilibrium 소산구조.
 * 생명은 *이미* 고질 자유 E 만 먹는다(흡수 kL 이 sim.E[자유 풀]서만 — Eth 못 먹음, 구조적). 이 step 은 *배출* 짝을 더한다: 대사 손실(cost=m·mMaint+baseCost)의 kLifeHeat 분율이 세계 밖 소멸(metabolized) 대신 저질 열 Eth 로 제자리(a.center)에 머문다. 생명 = 고질 섭취 + 저질 배출 = 질 엔진.
 * 회귀: kLifeHeat=0 → heat 0 → metabolized += cost = 직전 step(0045) 비트 동일(Eth 무변경·qualInit 미설정 → Eth 해시 skip). 렌더 L-Q(흑체 색온도)가 이제 생명이 식힌 자리(저질 Eth)도 비춘다. 브라우저: window.HWS_PANEL_0048
 * (아래 표준 스택은 step-0027 그대로 — 생명 폐열은 그 동역학과 직교. q 오버레이[Eth=붉은 막]는 drawHook 끝 — 0045 가법, 폐열도 비춘다.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — 생명 폐열 아레나: verify.js lifeHeatArena() 와 *같은 정신*(균일 E + 정적 생명·확산 off·kQual=0[자발 강등 격리]).
   *   기본 패널은 회귀 상태(kLifeHeat=0)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나에 *폐열(kLifeHeat)* 만 켜고/끄고 두 프리셋으로 A/B 대조한다 — 화면 q 오버레이(drawHook 끝, 0045 가법):
   *   ON 이면 생명 자리에 저질 열 Eth 가 쌓여 *붉게*(생명이 제 발밑을 식힘 = 폐열)·OFF 면 균일(폐열 없음·대사 전부 세계 밖). */
  function lifeHeatArena(kH) {
    return {
      W: 64, H: 64, D: 1, initE: 2, noise: 0, drive: false,
      ...
      kCryst: 0, kWeather: 0, kCrowd: 0, kCrowdZ: 0, kFlux: 0,
      kIgnite: 0, kStarRise: 0, kStarFall: 0, kStarSet: 0, kFSM: 0, kGravity: 0, kCollapse: 0, kSupport: 0, kOcclude: 0,
      kTemplate: 0, kInherit: 0, kShare: 0, kPublic: 0, kDiff: 0, kGermline: 0, kAnchor: 0, kTension: 0, kMembrane: 0, kAdhesion: 0, kAniso: 0, kTuring: 0, kDendrite: 0, kPermeate: 0,
      kLifeHeat: kH
    };
  }
  function seedLifeHeat(sim) {   // 정적 생명 9 마리(격자 3×3 에 흩뿌림·각 m≤0.5 를 제 칸 E 서) — verify 정신
    var W = sim.p.W, H = sim.p.H, n = 0;
    for (var gy = 1; gy <= 3; gy++) for (var gx = 1; gx <= 3; gx++) {
      var x = (gx * W / 4) | 0, y = (gy * H / 4) | 0, c = y * W + x, m = Math.min(0.5, sim.E[c]);
      sim.E[c] -= m; sim.agents.push({ x: x, y: y, z: 0, m: m, cells: [c], center: c, bornTick: sim.tick }); n++;
    }
    return n;
  }
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0048 — <span style="color:#ffd060">생명의 폐열</span>: 생명이 고질을 먹고 저질 엔트로피를 배출한다 — 슈뢰딩거 소산구조(SPINE 여섯째 축)',
    subtitle: 'step-0047 가 E 에 질 축(자유 E ↔ 열 E[Eth])을 열었으나, 질은 아직 <b>측정 토대</b>였다 — 동역학에 안 먹였다. 슈뢰딩거 핵심: 생명은 <b>음엔트로피(자유에너지)를 먹고 엔트로피(폐열)를 환경에 배출</b>하는 far-from-equilibrium 소산구조. 생명은 <i>이미</i> 고질 자유 E 만 먹는다(흡수 <code>kL</code> 이 자유 풀 sim.E 서만 — Eth 못 먹음, 구조적). 이 step 은 새 노브 <b>kLifeHeat</b> 로 <i>배출</i> 짝을 더한다: 대사 손실(cost=m·mMaint+baseCost)의 kLifeHeat 분율이 <b>세계 밖 소멸(metabolized) 대신 저질 열 Eth 로 제자리(생명 칸)에 머문다</b>. 생명 = 고질 섭취 + 저질 배출 = 질 엔진. 검증(정적 생명 5·확산 off·kQual=0·20 tick): <b>kLifeHeat OFF 면 폐열 0(대사 전부 세계 밖) → ON 이면 저질 열 Eth 0.77 가 전량 생명 자리에</b>(확산 off 라 제자리·ethAtLife=totalEth). <b>kLifeHeat=0 → heat 0 → metabolized += cost = 비트 동일</b>(회귀 0). cost 가 Eth + metabolized 쌍 분배(둘 다 장부·잔차 1.1e-16)·결정론. 렌더 <b>L-Q(흑체 색온도)</b>가 이제 생명이 식힌 자리(저질 Eth)도 비춘다(생명 둘레 붉게). <code>node step-0048/verify.js</code> 로 reg·heat·conserve·det. *한계(후속): 폐열 제자리 고임(자기 오염)·재활용·저질 섭취. (표준 스택은 step-0027 그대로 — 생명 폐열과 직교.)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: false, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 의 생명 폐열을 한 클릭으로 보이는 설정(verify heat 과 같은 정신)으로. 둘 다 균일 E + 정적 생명 9·확산 off·kQual=0 — *폐열(kLifeHeat)*만 토글. 화면 q 오버레이(drawHook 끝, Eth=붉은 막): ON 이면 생명 둘레 붉게(폐열). */
    presets: [
      { label: '🔥 생명 폐열 ON (생명이 제 발밑을 식힘·소산구조)',
        title: 'verify heat(on) 과 같은 정신: 균일 E=2 + 정적 생명 9(3×3)·확산 off·kQual=0·*폐열 kLifeHeat=0.3*, 20 tick 미리 진행. 생명은 고질 자유 E 를 먹어 m 유지·대사 손실의 30%를 저질 열 Eth 로 제자리에 배출(슈뢰딩거 음엔트로피). q 오버레이가 생명 자리마다 붉게(폐열) = verify 가 단언(폐열 0.77 전량 생명 자리·확산 off 라 제자리). 양은 보존(cost→Eth+metabolized 쌍 분배).',
        params: lifeHeatArena(0.3), seed: seedLifeHeat, run: 20,
        note: '생명 폐열 ON — 생명 자리마다 붉은 폐열(저질 Eth). OFF 와 번갈아 보라(통계 "에너지 질"·q 오버레이).' },
      { label: '❄ 생명 폐열 OFF (폐열 없음·대조)',
        title: 'verify heat(off) 과 같은 정신: 같은 아레나지만 kLifeHeat=0. 대사 손실이 전부 세계 밖 소멸(metabolized) → Eth 0·폐열 없음(화면 균일·붉은 막 없음) = 직전 step 비트 동일. ON 버튼과 번갈아 눌러 A/B 로 비교하면 생명의 폐열이 또렷하다.',
        params: lifeHeatArena(0), seed: seedLifeHeat, run: 20,
        note: '생명 폐열 OFF(대조) — 대사 전부 세계 밖(Eth 0·폐열 없음). ON 버튼과 번갈아 눌러 폐열을 보라.' }
    ],
    ...
    controls: [
      ...
      { items: [
        { kind: 'check', id: 'qualc', label: '에너지 질 강등(자유 E→열 E·둘째 법칙)', def: false, gateFor: 'kQual', title: 'step-0047 V5+: 새 법칙 degrade. 매 tick 자유 E 의 kQual 비율이 E→Eth(열 에너지, 저질)로 일방 강등(둘째 법칙)·쌍 거래(총 E+Eth 불변·양 보존·질만 떨어짐). source/별이 자유 E 보충(고질) → 질 q=E/(E+Eth) 구배(보충 셀 고질·배경 저질). 끄면(kQual=0) Eth 0·q=1 전역(직전 step 비트 동일). 위 "에너지 질 강등 ON" 버튼이 데모. 렌더 L-Q(흑체 색온도)가 q 를 읽는다(RENDER §8).' },
        { kind: 'slider', id: 'kQual', label: 'kQual (질 강등율)', param: 'kQual', min: 0, max: 0.2, step: 0.01, def: 0, fixed: 2, gateBy: 'qualc', gateOff: 0, title: '에너지 질 강등율(매 tick 자유 E→Eth 비율). 0 이면 degrade early-return = 회귀(직전 step 비트 동일·Eth 0·qualInit 미설정 → Eth 해시 skip). 클수록 빨리 식는다(q 빨리 떨어짐). E→Eth 쌍 거래라 양 보존(ledger 가 sumEth 가법). *리셋 불필요* — 즉시 적용.' },
        { kind: 'check', id: 'lifeheatc', label: '생명 폐열(대사 손실→저질 열·슈뢰딩거)', def: false, gateFor: 'kLifeHeat', title: 'step-0048 V5+: 생명의 대사 손실(cost)의 kLifeHeat 분율이 세계 밖 소멸(metabolized) 대신 저질 열 Eth 로 제자리(생명 칸)에 머문다. 생명=고질 자유 E 섭취(흡수 kL·이미 그러함) + 저질 폐열 배출 = far-from-equilibrium 소산구조(슈뢰딩거 "생명은 음엔트로피를 먹는다"). 끄면(kLifeHeat=0) 대사 전부 세계 밖(직전 step 비트 동일). 위 "생명 폐열 ON" 버튼이 데모. q 오버레이가 생명 둘레 폐열을 붉게 비춘다.' },
        { kind: 'slider', id: 'kLifeHeat', label: 'kLifeHeat (폐열 분율)', param: 'kLifeHeat', min: 0, max: 1, step: 0.05, def: 0, fixed: 2, gateBy: 'lifeheatc', gateOff: 0, title: '생명 대사 폐열 분율(0~1). 0 이면 heat 0 → metabolized += cost = 회귀(직전 step 비트 동일·Eth 무변경·qualInit 미설정 → Eth 해시 skip). >0 이면 그 분율이 저질 열 Eth 로 제자리에(나머지는 종전 metabolized). cost 가 Eth + metabolized 쌍 분배라 양 보존. *리셋 불필요* — 즉시 적용.' }
      ]},
      { items: [
    ...
    drawHook: function (ctx, c) {
      ...
      /* 에너지 질 q 오버레이(step-0047) — kQual on 이면 셀별 열화 분율(=1−q=Eth/(E+Eth))을 *붉은 막*으로 얹는다: 저질(붉음)↔고질(투명). source 가 보충하는 자리는 맑게(고질)·미보충 배경은 붉게(저질) = 질 구배(흑체 색온도의 시뮬 토대 — 렌더 L-Q 가 이 q 를 읽는다, RENDER §8). z=0 평면(D>1 도 표층). */
      if (sim.p.kQual !== 0 && sim.qualInit && sim.Eth) {
        var E2 = sim.E, Eth2 = sim.Eth;
        for (var yq = 0; yq < H; yq++) for (var xq = 0; xq < W; xq++) {
          var iq = yq * W + xq, tot = E2[iq] + Eth2[iq]; if (tot < 1e-6) continue;
          var lowq = Eth2[iq] / tot; if (lowq < 0.05) continue;
          ctx.fillStyle = 'rgba(232,80,60,' + (0.6 * lowq).toFixed(3) + ')';
    ...
    stats: [
      ...
      { label: '<span style="color:#ffd060">에너지 질</span> (열화 분율 / 누적 Eth — 가설: source 가 질 떠받침)', get: function (c) {
          var s = c.sim; if ((s.p.kQual || 0) === 0 || !s.Eth) return { text: 'kQual=0 (질 강등 off — Eth 0·q=1 전역)', cls: '' };
          var E = s.E, Eth = s.Eth, sE = 0, sT = 0, N = E.length; for (var i = 0; i < N; i++) { sE += E[i]; sT += Eth[i]; }
          var frac = (sE + sT) > 0 ? sT / (sE + sT) : 0;
          return { text: '열화 분율 ' + (frac * 100).toFixed(0) + '% · Eth ' + sT.toFixed(1) + ' (자유 E 가 식어 저질로 — q=E/(E+Eth)·흐름이 질 떠받침)', cls: frac > 0 ? 'pass' : '' };
      } },
      { label: '<span style="color:#ffd060">생명 연직</span> (meanZ / maxZ / z>0 분율 — 이동·번식·혼잡의 z)', get: function (c) {
          var D = c.sim.p.D || 1, zOn = ((c.sim.p.kMoveZ || 0) !== 0) || ((c.sim.p.kDivZ || 0) !== 0) || ((c.sim.p.kCrowdZ || 0) !== 0), ag = c.sim.agents, top = D - 1;
      ...
      { label: '<span style="color:#ffd060">생명 m</span> (평균 — 가설: 3D 혼잡세로 깎임)', get: function (c) {
          var ag = c.sim.agents; if (!ag.length) return { text: '생명 없음', cls: '' };
          var sm = 0; for (var k = 0; k < ag.length; k++) sm += ag[k].m;
          var mm = sm / ag.length, czOn = (c.sim.p.kCrowdZ || 0) !== 0 && (c.sim.p.D || 1) > 1;
          return { text: 'meanM ' + mm.toFixed(2) + (czOn ? ' (3D 혼잡 on — 수직 적층 깎임)' : '') + ' · 혼잡세 ' + (c.sim.metabolized || 0).toFixed(2), cls: czOn && mm < 0.9 ? 'pass' : '' };
      } },
  ...
  else global.HWS_PANEL_0048 = panel;
