/* step-0046 패널 — 생명의 폐열(슈뢰딩거 음엔트로피·소산구조, SPINE 여섯째 축). 직전 step-0045(에너지의 질) 패널을 잇되 metabolize 제자리 확장 + 노브 kLifeHeat 1개를 더했다.
 * step-0045 가 E 에 질 축(자유 E ↔ 열 E[Eth])을 열었으나, 질은 아직 *측정 토대*였다 — 동역학에 안 먹였다. 슈뢰딩거 핵심: 생명은 음엔트로피(자유에너지)를 먹고 엔트로피(폐열)를 환경에 배출하는 far-from-equilibrium 소산구조.
 ...
 * 회귀: kLifeHeat=0 → heat 0 → metabolized += cost = 직전 step(0045) 비트 동일(Eth 무변경·qualInit 미설정 → Eth 해시 skip). 렌더 L-Q(흑체 색온도)가 이제 생명이 식힌 자리(저질 Eth)도 비춘다. 브라우저: window.HWS_PANEL_0046
...
(function (global) {
  ...
  var panel = {
    ...
    controls: [
      ...
      { items: [
        ...
        { kind: 'check', id: 'lifeheatc', label: '생명 폐열(대사 손실→저질 열·슈뢰딩거)', def: false, gateFor: 'kLifeHeat', title: 'step-0046 V5+: 생명의 대사 손실(cost)의 kLifeHeat 분율이 세계 밖 소멸(metabolized) 대신 저질 열 Eth 로 제자리(생명 칸)에 머문다. 생명=고질 자유 E 섭취(흡수 kL·이미 그러함) + 저질 폐열 배출 = far-from-equilibrium 소산구조(슈뢰딩거 "생명은 음엔트로피를 먹는다"). 끄면(kLifeHeat=0) 대사 전부 세계 밖(직전 step 비트 동일). 위 "생명 폐열 ON" 버튼이 데모. q 오버레이가 생명 둘레 폐열을 붉게 비춘다.' },
    ...
    drawHook: function (ctx, c) {
      ...
      /* 에너지 질 q 오버레이(step-0045) — kQual on 이면 셀별 열화 분율(=1−q=Eth/(E+Eth))을 *붉은 막*으로 얹는다: 저질(붉음)↔고질(투명). source 가 보충하는 자리는 맑게(고질)·미보충 배경은 붉게(저질) = 질 구배(흑체 색온도의 시뮬 토대 — 렌더 L-Q 가 이 q 를 읽는다, RENDER §8). z=0 평면(D>1 도 표층). */
  ...
  else global.HWS_PANEL_0046 = panel;
