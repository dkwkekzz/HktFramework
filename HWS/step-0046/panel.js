/* step-0046 패널 — 3D 생물량 공유: risk-pooling 의 연직축 일반화(VOXEL.md V5+). 직전 step-0045(3D 차등 응집) 패널을 잇되 share 법칙 *제자리 확장* + 노브 kShareZ 1개를 더했다.
 * 0045 가 z>0 거주 생명을 3D 로 kin 정렬했다(adhere z-일반화 — 같은 유전형이 3D 액적으로 뭉침). 그러나 생물량 공유(share·step-0019 굶주린 kin 을 떠받치는 risk-pooling)는 여전히 2D 평면만 봤다 — occ 그리드 W·H(2D)·kin 쌍이 우(+x)/하(+y) 4-인접이라 z>0 생명은 occ[a.center] 가 범위 밖이라 무시(위/아래로 쌓인 kin 이 서로 못 떠받침). 이 step 은 share 의 occ 그리드 W·H→W·H·D·kin 쌍에 위(+z) 추가(4-인접→6-인접) 로 일반화한다 — z>0 굶주린 kin 도 제 z±1 안전 kin 에게 구조된다 = 0045 가 정렬한 3D 액적이 *단위로* 생존(risk-pooling 의 연직 일반화·step-0019 의 3D 짝).
 * 형식: 0044 crowd·0045 adhere 의 z-확장과 같은 *제자리 확장*(새 LAW_ORDER 자리 없음) + 노브 kShareZ. 구조는 a.m 만 바꿈(이미 해시) → moveZInit 무관·m 차이로 골든이 3D risk-pooling 을 잠근다.
 * 회귀(이중 가드): kShareZ=0 → 3D 블록 미진입(2D 경로)=직전 step 비트 동일·z>0 컬럼 kin 은 occ 범위 밖이라 못 떠받쳐 얼어붙음 / D=1 → z 이웃 없어 2D 등가. 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0046
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 3D 생물량 공유는 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — 3D 생물량 공유 아레나: verify.js poolArena()/seedKinColumns 와 *같은* 설정.
   *   기본 패널은 회귀 상태(D=1)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나(D=8·수직 kin 컬럼 3개[z 짝수 안전·홀수 궁핍]·share 만 on·나머지 동역학 off)에 *연직 공유(kShareZ)* 만 켜고/끄고 두 프리셋으로 A/B 대조한다 —
   *   화면이 보여주는 것 = verify 가 단언하는 수치(kShareZ OFF 면 z>0 컬럼 kin 이 2D occ 범위 밖이라 구조 0·굶주린 kin 얼어붙음·최소 m 초기값 / ON 이면 z±1 안전 kin 이 떠받쳐 구조>0·최소 m↑). (3D 뷰는 아직 z=0 투영 — voxel 렌즈 L-V1 백로그·통계 "최소 m"이 증거.) */
  function poolArena(kSZ) {
    return {
      ...
      kShare: 0.5, coopFit0: 1.0, coopFitStep: 0.0, kShareZ: kSZ,
  ...
  function seedKinColumns(sim, core) {   // 균일 E=2 + 수직 kin 컬럼 3개(태그 1·z 짝수 안전 m=1.0·홀수 궁핍 m=0.2) — verify.js seedKinColumns 와 동일
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0046 — <span style="color:#ffd060">3D 생물량 공유</span>: risk-pooling 이 연직축으로 — z>0 굶주린 kin 도 위/아래 동료에게 구조된다(VOXEL.md V5+)',
    subtitle: '0045 가 z>0 거주 생명을 3D 로 kin 정렬했다(<code>adhere</code> z-일반화 — 같은 유전형이 3D 액적으로 뭉침). 그러나 생물량 공유(<code>share</code>·step-0019 굶주린 kin 을 떠받치는 risk-pooling)는 여전히 2D 평면만 봤다 — occ 그리드 W·H(2D)·kin 쌍이 우(+x)/하(+y) 4-인접이라 z>0 생명은 무시(위/아래로 쌓인 kin 이 서로 <b>못 떠받침</b> = 3D risk-pooling 누수). 이 step 은 <code>share</code> 의 <b>occ 그리드 W·H→W·H·D·kin 쌍에 위(+z) 추가(4-인접→6-인접)</b> 로 일반화한다: z>0 굶주린 kin 도 제 z±1 안전 kin 에게 구조된다 = <b>0045 가 정렬한 3D 액적이 *단위로* 생존</b>(risk-pooling 의 연직 일반화·step-0019 의 3D 짝). 형식: 0044 <code>crowd</code>·0045 <code>adhere</code> z-확장과 같은 <b>제자리 확장</b>(새 LAW_ORDER 자리 없음) + 노브 <b>kShareZ</b>. 검증(D=8·수직 kin 컬럼 3개[z 짝수 안전 m=1.0·홀수 궁핍 m=0.2]·share 만 on): <b>kShareZ OFF 면 컬럼이 2D 평면 투영상 한 칸이라 서로 안 닿아 구조 0(최소 m 0.20 그대로·얼어붙음) → ON 이면 z±1 안전 kin 이 떠받쳐 구조 1.20·최소 m 0.30 으로 오른다</b>. <b>kShareZ=0 → 3D 블록 미진입(2D 경로)=비트 동일 / D=1 → 2D 등가</b>(회귀 0·이중 가드). 구조는 m 쌍 거래(부유→궁핍·잔차 0)·결정론. <code>node step-0046/verify.js</code> 로 reg·pool·conserve·det. (표준 스택은 step-0027 덴드라이트 그대로 — 3D 생물량 공유와 직교.)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: true, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 의 3D 생물량 공유를 한 클릭으로 보이는 설정(verify pool 과 동일 아레나)으로. 둘 다 수직 kin 컬럼 3개·share 만 on — *연직 공유(kShareZ)*만 토글. */
    presets: [
      { label: '🤝 3D 생물량 공유 ON (z>0 risk-pooling·D=8)',
        title: 'verify pool(on) 과 같은 설정: D=8·수직 kin 컬럼 3개(태그 1·z 짝수 안전 m=1.0·홀수 궁핍 m=0.2)·share 만 on·*연직 공유 kShareZ=1*, 8 tick 미리 진행. z>0 굶주린 kin 이 제 위/아래(z±1) 안전 kin 에게 떠받쳐져 최소 m 이 0.20→0.30(사망권 위로) 오르고 구조량 1.20 = 0045 가 정렬한 3D 액적이 *단위로* 생존. 통계 "최소 m"이 0.30 근처면 연직 risk-pooling 이 켜진 것 = verify 가 단언하는 수치. (3D 뷰는 z=0 투영이라 컬럼이 한 점으로 겹쳐 보인다 — voxel 렌즈 L-V1 백로그·통계가 증거.)',
        params: poolArena(1), seed: seedKinColumns, run: 8,
        note: '3D 생물량 공유 ON — z>0 굶주린 kin 이 z±1 안전 kin 에게 구조(최소 m 0.20→0.30). OFF 버튼과 번갈아 보라(통계 "최소 m").' },
      { label: '🛑 3D 생물량 공유 OFF (z>0 굶주린 kin 얼어붙음·대조)',
        title: 'verify pool(off) 과 같은 설정: 같은 아레나·컬럼이지만 kShareZ=0. share 가 2D 평면만 봐 수직 컬럼이 한 칸으로 투영돼 서로 안 닿는다(구조 0·최소 m 0.20 그대로·굶주린 kin 얼어붙음 — 바로 옛 2D share 가 z>0 kin 을 못 떠받치던 caveat). ON 버튼과 번갈아 눌러 A/B 로 비교하면 연직 risk-pooling 이 또렷하다.',
        params: poolArena(0), seed: seedKinColumns, run: 8,
        note: '3D 생물량 공유 OFF(대조) — z>0 굶주린 kin 이 못 떠받쳐 얼어붙음(최소 m 0.20). ON 버튼과 번갈아 눌러 연직 risk-pooling 을 보라.' }
    ],
    ...
    stats: [
      ...
      { label: '<span style="color:#ffd060">최소 m</span> (굶주린 kin — 가설: z>0 구조로 오름)', get: function (c) {
          var sim = c.sim, ag = sim.agents, D = sim.p.D || 1; if (!ag.length) return { text: '생명 없음', cls: '' };
          var mn = Infinity, mx = 0; for (var k = 0; k < ag.length; k++) { var m = ag[k].m; if (m < mn) mn = m; if (m > mx) mx = m; }
          var szOn = (sim.p.kShareZ || 0) !== 0 && D > 1, danger = (sim.p.mDeath || 0) * 3;
          return { text: '최소 m ' + mn.toFixed(3) + ' / 최대 ' + mx.toFixed(2) + (szOn ? ' (3D 공유 on — z>0 구조)' : '') + ' · 구조 ' + (sim.shared || 0).toFixed(2), cls: szOn && mn >= danger - 1e-6 && danger > 0 ? 'pass' : '' };
      } },
    ...
    legend:
      ...
      '<b>3D 생물량 공유 — risk-pooling 의 연직축 일반화(VOXEL.md V5+ · SPINE 다섯째 축)</b>: 0042·0043 이 생명을 z>0 으로 올렸고(이동·번식), 0044 가 혼잡(carrying capacity)을, 0045 가 <b>차등 응집</b>(같은 유전형 kin 정렬)을 z 로 풀었다. 이제 z>0 kin 이 3D 액적으로 뭉친다 — 그런데 <b>생물량 공유</b>(<code>share</code>·step-0019 = 굶주린 kin 을 사망 직전 떠받치는 risk-pooling)는 여전히 2D 평면만 봤다 — occ 그리드·kin 쌍이 W·H[2D]·우/하 4-인접이라 z>0 거주 kin 은 <b>위/아래 동료를 못 떠받쳤다</b>(3D risk-pooling 누수). 이 step 은 <code>share</code> 의 <b>occ 그리드 W·H→W·H·D·kin 쌍에 위(+z) 추가(4-인접→6-인접)</b> 로 <b>제자리 확장</b>(0044 crowd·0045 adhere 와 같은 형식)한다: z>0 굶주린 kin 도 제 z±1 안전 kin 에게 구조된다 = <b>0045 가 정렬한 3D 액적이 *단위로* 생존</b>. 노브 <b>kShareZ</b>(구조는 a.m 만 바꿔 골든 잠금·moveZInit 무관). "3D 생물량 공유" 켜고 "터 깊이 D" 8 로 리셋해 보라(위 "3D 생물량 공유 ON" 버튼). <code>verify pool</code>(수직 kin 컬럼 3개[z 짝수 안전 m=1.0·홀수 궁핍 m=0.2]): <b>kShareZ OFF 면 컬럼이 2D occ 범위 밖이라 구조 0(최소 m 0.20 초기값·얼어붙음) → ON 이면 z±1 안전 kin 이 떠받쳐 구조 1.20·최소 m 0.30(사망권 위로) 오른다</b>. m 쌍 거래(부유→궁핍·잔차 0). <b>kShareZ=0 → 3D 블록 미진입=비트 동일 / D=1 → z 이웃 없어 2D 등가</b>(회귀 0·이중 가드). (3D 뷰는 아직 z=0 투영 — voxel 렌더는 렌더러 트랙 L-V1·통계 "최소 m"이 증거.)<br>' +
