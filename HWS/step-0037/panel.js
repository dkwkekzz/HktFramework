/* step-0037 패널 — 태양빛 비: 부력+중력 통합(VOXEL.md V5+ — 0035 상승·0036 하강이 "중력 off 격리"로 닫혔던 것을, 중력을 켜 *통합*한다). 직전 step-0036(별 하강·일생) 패널을 잇되 새 노브 행은 더하지 않는다 — 중력·별 상승·별 하강 노브가 이미 다 있다(이 step 은 *합성*).
 * 이 step 은 hws-laws.js 를 건드리지 않는다 — 이미 검증된 두 법칙(별 부력/일생 ignite·중력 gravity)을 *합성*해 둘 중 어느 하나만으론 안 나는 제3의 현상을 창발시키는 *통합 step* 이다: 별이 떴다 지며(0036) 高z 에서 뿌린 E 를 중력(V3 kGravity)이 비처럼 z=0 바닥으로 끌어내려 고이게 한다 = **바다**(별빛→비→바다).
 * 완성하는 그림: 별이 z=0(R 핵)서 나(0035 birth)→천장까지 뜨고(0035 rise)→연료 쇠해 도로 진다(0036 set)→진 별이 뿌린 E 가 중력으로 내려 바다로 고인다(0037 rain) → "구동이 세계서 나고 진다"가 *에너지 순환*으로 닫힘.
 * 회귀(이중 가드, 기존 노브 그대로): kGravity=0 이면 gravity skip(방출 E 제자리 = 0036 비트 동일) / D=1 이면 gravity 가 D<2 early-return(z 이웃 없음 = 2D 레거시 불변). 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0037
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 태양빛 비는 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — 태양빛 비 아레나: verify.js rainArena()/seedSunCore()/comZ()/bottomFrac() 와 *같은* 설정·식.
   *   기본 패널은 회귀 상태(D=1·중력 off)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나(D=8 voxel·z=0 R 핵서 단일 별 점화·부력 상승·하강 on)에 *중력*을 켜고/끄고 두 프리셋으로 A/B 대조한다 — 화면이 보여주는 것 =
   *   verify 가 단언하는 수치(E 무게중심 z·바닥 z=0 분율)가 되게. 중력 ON 이면 별빛이 z=0 바닥으로 내려 고인다(바다). */
  function rainArena(kGrav) {
    return {
      ...
      kIgnite: 1, kStarRise: 1, kStarFall: 1, starFallThresh: 0.5, starRate: 0.06, starFuel0: 500, ignThresh: 1.5, starCap: 1, starGap: 6, starR: 2, starDriftPeriod: 20,
      kGravity: kGrav, kFSM: 0, kCollapse: 0, kCryst: 0, kWeather: 0, kSupport: 0, kOcclude: 0,
  ...
  function comZE(sim) {                // E 무게중심 z + 바닥 z=0 분율(평면별 E 합) — verify.js comZ/bottomFrac 와 동일식. 중력 on 이면 별빛이 바닥에 고여 무게중심 ↓·바닥 분율 ↑
    var p = sim.p, D = p.D || 1, WH = p.W * p.H, E = sim.E, num = 0, den = 0, bot = 0;
    for (var z = 0; z < D; z++) { var t = 0; for (var k = 0; k < WH; k++) t += E[z * WH + k]; if (z === 0) bot = t; num += z * t; den += t; }
    return { comZ: den > 0 ? num / den : 0, botFrac: den > 0 ? bot / den : 0, D: D };
  }

  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0037 — <span style="color:#ffd060">태양빛 비</span>: 지는 별이 뿌린 E 를 중력이 끌어내려 바다로 고이게 한다 = 부력+중력 통합(VOXEL.md V5+)',
    subtitle: 'step-0035(별 상승)·0036(별 하강)은 별의 <i>연직 궤적</i>을 또렷이 보려 <b>중력을 꺼 격리</b>했다. 이 step 은 그 <b>중력을 켜 *통합*한다</b> — 별이 떴다 지며(0036) 高z 에서 뿌린 E 를 중력(V3 <code>kGravity</code>)이 <b>비처럼 z=0 바닥으로 끌어내려 고이게</b> 한다 = <b>바다</b>. 이 step 은 <code>hws-laws.js</code> 를 <b>건드리지 않는다</b> — 이미 검증된 두 법칙(별 부력/일생 <code>ignite</code>·중력 <code>gravity</code>)을 <b>합성</b>해 둘 중 어느 하나만으론 안 나는 제3의 현상(별빛→비→바다)을 창발시키는 <i>통합 step</i> 이다. 完成: 별이 z=0(R 핵)서 나(0035)→천장까지 뜨고(0035)→연료 쇠해 도로 진다(0036)→<b>진 별이 뿌린 E 가 중력으로 내려 바다로 고인다(0037)</b> → "구동이 세계서 나고 진다"가 <b>에너지 순환으로 닫힘</b>. <b>kGravity=0 이면 gravity skip(방출 E 제자리 = 0036 비트 동일) / D=1 이면 gravity 가 D&lt;2 early-return(z 이웃 없음 = 2D 레거시 불변)</b>(회귀 0·이중 가드·기존 노브 그대로). 검증: <b>중력 off 면 방출 E 가 별 z-궤적에 머묾(E 무게중심 z=5.83·바닥 z=0 분율 5%) → 중력 on 이면 z=0 바닥에 고임(무게중심 z=0.21 &lt; off·바닥 분율 91% = 바다)</b>·잔차 1.4e-15·결정론. <code>node step-0037/verify.js</code> 로 reg·rain·conserve·det. (표준 스택은 step-0027 덴드라이트 그대로 — 태양빛 비와 직교.)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: true, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 이 합성한 태양빛 비를 한 클릭으로 보이는 설정(verify rain 과 동일 아레나)으로. 둘 다 별 상승·하강 on(별이 떴다 진다) — *중력*만 토글. */
    presets: [
      { label: '🌧 태양빛 비 ON (별빛→바다·D=8)',
        title: 'verify rain(on) 과 같은 설정: D=8 격리 아레나·z=0 R 핵서 단일 별 점화·kStarRise=1·kStarFall=1·*kGravity=0.2*, 200 tick 미리 진행. 별이 떴다 지며(0036) 高z 에서 뿌린 E 를 중력이 z=0 바닥으로 끌어내려 고이게 한다. 통계 "E 무게중심 z·바닥 분율" 이 ≈0.21·91% 면 바다 형성 = verify 가 단언하는 수치. (3D 뷰는 아직 z=0 평면만 그려 z 분포 자체는 안 보임 — voxel 렌즈 L-V1 백로그. 수치가 증거.)',
        params: rainArena(0.2), seed: seedSunCore, run: 200,
        note: '태양빛 비 ON — "E 무게중심 z"≈0.21·"바닥 z=0 분율"≈91%(별빛이 바다로 고임). OFF 버튼과 번갈아 보라.' },
      { label: '☀ 중력 OFF (=0036·안 고임·대조)',
        title: 'verify rain(off) 과 같은 설정: 같은 아레나·핵·별 상승·하강이지만 kGravity=0(=0036 별 일생). 방출 E 가 별의 z-궤적에 머문다(E 무게중심 z=5.83·바닥 분율 5%). ON 버튼과 번갈아 눌러 A/B 로 비교하면 중력이 별빛을 바다로 만드는 것이 또렷하다.',
        params: rainArena(0), seed: seedSunCore, run: 200,
        note: '중력 OFF(대조·=0036) — "E 무게중심 z"=5.83·"바닥 분율"=5%(E 가 별 궤적에 머묾). ON 버튼과 번갈아 눌러 바다를 보라.' }
    ],
    ...
    stats: [
      ...
      { label: '<span style="color:#ffd060">E 무게중심 z·바닥 분율</span> (가설: 별빛→바다)', get: function (c) {
          var r = comZE(c.sim); if (r.D < 2) return { text: 'z=0 평면(D=1) — 중력 비활성. 위 "데모" 버튼으로 D=8', cls: '' };
          var grav = c.sim.p.kGravity !== 0, fz = (c.sim.stars && c.sim.stars.length) ? (c.sim.stars[0].z || 0) : '—';
          return { text: '별 최종 z=' + fz + ' / 천장 ' + (r.D - 1) + ' · E무게중심 z=' + r.comZ.toFixed(2) + ' · 바닥 z=0 분율 ' + (r.botFrac * 100).toFixed(0) + '%', cls: (grav && r.botFrac > 0.4 && r.comZ < 3) ? 'pass' : '' };
      } },
    ...
    legend:
      ...
      '<b>태양빛 비 — 부력+중력 통합(VOXEL.md V5+)</b>: step-0035(별 상승)·0036(별 하강)은 별의 연직 궤적을 또렷이 보려 <b>중력을 꺼 격리</b>했다. 이 step 은 그 <b>중력을 켜 *통합*한다</b> — 별이 떴다 지며(0036) 高z 에서 뿌린 E 를 중력(V3 <b>kGravity</b>)이 <b>비처럼 z=0 바닥으로 끌어내려 고이게</b> 한다 = <b>바다</b>(별빛→비→바다). 이 step 은 <code>hws-laws.js</code> 를 <b>건드리지 않는다</b> — 두 기존 법칙(별 일생 <code>ignite</code>·중력 <code>gravity</code>)의 <b>합성</b>이 빚은 제3의 현상(통합 step). "터 깊이 D" 8·"별 부력 상승"·"별 하강"·"중력" 켜고 리셋해 보라(위 "태양빛 비 ON" 버튼). <code>verify rain</code>: <b>중력 off 면 방출 E 가 별 궤적에 머묾(무게중심 z=5.83·바닥 분율 5%)·on 이면 z=0 바닥에 고임(무게중심 z=0.21·바닥 분율 91% = 바다)</b>·잔차 1.4e-15. "구동이 세계서 나고 진다"가 에너지 순환으로 닫힘. (3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더는 렌더러 트랙 L-V1.)<br>' +
  ...
  else global.HWS_PANEL_0037 = panel;
