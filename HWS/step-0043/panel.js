/* step-0043 패널 — 3D 번식: 자식의 연직 출생(VOXEL.md V5+ — 생명이 *번식으로* 3D 를 오른다). 직전 step-0042(생명 z-이동) 패널을 잇되 reproduce 법칙 *제자리 확장* + 노브 kDivZ 1개를 더했다.
 * 0042 가 생명의 *이동*을 z 로 풀었으나(개체가 올라감), 번식은 여전히 자식을 z=0 평면에 떨궜다(부모가 z>0 이라도 자식이 바닥으로). 이 step 은 reproduce 의 자식 배치를 4-이웃[2D]→6-이웃[3D]으로 일반화한다 — 부모 z-평면 + 위/아래(z±1) 후보 중 E 최고 칸에 자식을 둔다. 연직 E 구배가 있으면 자식이 *위로* 태어나 개체군이 *번식으로* 상승한다(0042 의 *이동* 상승과 짝 — 다세포가 substrate 위에 탑승하는 두 경로).
 * 형식: move 의 kMoveZ z-이동과 같은 *제자리 확장*(새 LAW_ORDER 자리 없음) + 노브 kDivZ. agent.z 는 hashState 가 moveZInit(kMoveZ 또는 kDivZ 활성)일 때만 가법 해싱.
 * 회귀(이중 가드): kDivZ=0 → pz 강제 0 → 자식이 z=0 평면에만(reproduce 가 2D 이웃만)=직전 step 비트 동일·agent.z 미설정→해시 skip / D=1 → z±1 이 z 벽 밖이라 후보 0. 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0043
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 3D 번식은 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — 3D 번식 아레나: verify.js div3Arena()/seedGradient/seedLife 와 *같은* 설정.
   *   기본 패널은 회귀 상태(D=1)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나(D=8·정적 연직 E 구배 E(z)=1+z·z=0 평면 생명 9 마리·이동 off·mDiv 작게)에 *연직 번식(kDivZ)* 만 켜고/끄고 두 프리셋으로 A/B 대조한다 —
   *   화면이 보여주는 것 = verify 가 단언하는 수치(kDivZ OFF 면 자식이 z=0 평면에 갇힘·ON 이면 자식이 천장 z=7 까지 줄지어 태어남). (3D 뷰는 아직 z=0 투영 — voxel 렌즈 L-V1 백로그·통계 "생명 연직"이 증거.) */
  function div3Arena(kDZ) {
    return {
      ...
      repro: true, mDiv: 0.005, divR: 1, popCap: 4096, kDivZ: kDZ,
  ...
  function seedDiv3(sim, core) {   // 정적 연직 E 구배 E(z)=1+z + z=0 평면 생명 9 마리(3×3) — verify.js seedGradient/seedLife 와 동일
    var W = sim.p.W, H = sim.p.H, WH = W * H;
    for (var z = 0; z < sim.p.D; z++) { var base = 1 + z; for (var i = 0; i < WH; i++) { sim.E[z * WH + i] = base; sim.E0 += base; } }
    for (var gx = 0; gx < 3; gx++) for (var gy = 0; gy < 3; gy++) core.spawnAgent(sim, 16 + gx * 6, 16 + gy * 6);
  }

  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0043 — <span style="color:#ffd060">3D 번식</span>: 번식이 연직축으로 — 자식이 위로 태어나 개체군이 번식으로 3D 를 오른다(VOXEL.md V5+ · SPINE 다섯째 축)',
    subtitle: '0042 가 생명의 <b>이동</b>을 z 로 풀었다(개체가 에너지를 향해 올라감). 그러나 번식은 여전히 자식을 z=0 평면에 떨궜다 — 부모가 z>0 으로 올라가도 자식이 바닥으로 추락. 이 step 은 <code>reproduce</code> 의 자식 배치를 <b>4-이웃[2D]→6-이웃[3D]</b>으로 일반화한다: 부모 z-평면 + 위/아래(z±1) 후보 중 E 최고 칸에 자식을 둔다(번식이 연직축으로). 연직 E 구배가 있으면 자식이 <b>더 높은 에너지 쪽(위)에 태어난다</b> → 개체군이 <b>번식으로</b> 상승한다(0042 의 <i>이동</i> 상승과 짝 — 다세포가 substrate 위에 탑승하는 두 경로). 형식: <code>reproduce</code> 법칙 <b>제자리 확장</b>(move 의 kMoveZ z-이동과 같은 형식·새 LAW_ORDER 자리 없음) + 노브 <b>kDivZ</b>. 검증(정적 연직 구배 E(z)=1+z·z=0 생명 9 마리·이동 off): <b>kDivZ OFF 면 자식이 z=0 평면에 갇힘(maxZ 0) → ON 이면 자식이 천장 z=7 까지 줄지어 태어난다(maxZ 7·meanZ 1.26·z>0 분율 0.61)</b>. <b>kDivZ=0 → pz 강제 0(자식 z=0 평면)=비트 동일 / D=1 → z±1 z 벽 밖이라 후보 0</b>(회귀 0·이중 가드). m 만 반분(쌍 거래 보존)·위치만(장부 거래 0·잔차 0)·결정론. <code>node step-0043/verify.js</code> 로 reg·climb·conserve·det. (표준 스택은 step-0027 덴드라이트 그대로 — 3D 번식과 직교.)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: true, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 의 3D 번식을 한 클릭으로 보이는 설정(verify climb 과 동일 아레나)으로. 둘 다 정적 연직 E 구배·z=0 생명 9 마리·이동 off — *연직 번식(kDivZ)*만 토글. */
    presets: [
      { label: '🌱 3D 번식 ON (자식이 위로 태어남·D=8)',
        title: 'verify climb(on) 과 같은 설정: D=8·정적 연직 E 구배 E(z)=1+z(위로 갈수록 밝음)·z=0 평면 생명 9 마리·이동 off·*연직 번식 kDivZ=1*, 30 tick 미리 진행. 부모가 위/아래 칸 E 도 비교해 더 밝은 위에 자식을 둔다 → 자식이 천장 z=7 까지 줄지어(maxZ 7·meanZ 1.26·z>0 분율 0.61·개체 ~872). 통계 "생명 연직(meanZ)"이 0→1.26 이면 개체군이 번식으로 3D 를 오른 것 = verify 가 단언하는 수치. (3D 뷰는 z=0 투영이라 생명 점이 겹쳐 보일 수 있다 — voxel 렌즈 L-V1 백로그·통계가 증거.)',
        params: div3Arena(1), seed: seedDiv3, run: 30,
        note: '3D 번식 ON — 자식이 천장까지 줄지어 태어난다(maxZ 7·meanZ 0→1.26). OFF 버튼과 번갈아 보라(통계 "생명 연직").' },
      { label: '🛑 3D 번식 OFF (자식이 z=0 평면에 갇힘·대조)',
        title: 'verify climb(off) 과 같은 설정: 같은 아레나·구배·생명이지만 kDivZ=0. 자식이 늘 z=0 평면에 태어난다(maxZ 0 — 위로 안 오름). ON 버튼과 번갈아 눌러 A/B 로 비교하면 연직 번식이 또렷하다.',
        params: div3Arena(0), seed: seedDiv3, run: 30,
        note: '3D 번식 OFF(대조) — 자식이 z=0 평면에 갇힘(maxZ 0). ON 버튼과 번갈아 눌러 연직 번식을 보라.' }
    ],
    ...
    stats: [
      ...
      { label: '<span style="color:#ffd060">생명 연직</span> (meanZ / maxZ / z>0 분율 — 가설: 번식·이동으로 오름)', get: function (c) {
          var D = c.sim.p.D || 1, zOn = ((c.sim.p.kMoveZ || 0) !== 0) || ((c.sim.p.kDivZ || 0) !== 0), ag = c.sim.agents, top = D - 1;
          ...
          if (D < 2 || !zOn) return { text: 'meanZ ' + mz.toFixed(2) + ' (z=0 평면 갇힘 — z-이동/번식 off. 위 "3D 번식 ON" 버튼으로)', cls: '' };
  ...
  else global.HWS_PANEL_0043 = panel;
