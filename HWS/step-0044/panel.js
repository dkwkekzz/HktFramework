/* step-0044 패널 — 3D 자기제한: 혼잡 carrying capacity 의 연직 일반화(VOXEL.md V5+). 직전 step-0043(3D 번식) 패널을 잇되 crowd 법칙 *제자리 확장* + 노브 kCrowdZ 1개를 더했다.
 * 0042·0043 이 생명을 z>0 에 올렸으나(이동·번식으로 3D 거주), 혼잡(crowd·step-0012 carrying capacity)은 여전히 2D 평면만 봤다 — 밀도 그리드·disc 가 W·H(2D)라 z>0 생명은 occ[a.center] 가 범위 밖이라 무시(수직 무한 적층에도 혼잡세 0). 이 step 은 crowd 의 밀도 셈을 disc[2D]→ball[3D]·occ 그리드 W·H→W·H·D 로 일반화한다 — 수직으로 쌓인 생명도 제 z±이웃을 세 혼잡세를 낸다 = 0012(2D 공멸 방지)의 연직 짝(3D 생명이 무한 적층 못 함).
 * 형식: move 의 kMoveZ·reproduce 의 kDivZ z-확장과 같은 *제자리 확장*(새 LAW_ORDER 자리 없음) + 노브 kCrowdZ. 혼잡세는 a.m 만 바꿈(이미 해시) → agent.z 미해싱이라도 m 차이로 골든이 z-효과를 잠근다.
 * 회귀(이중 가드): kCrowdZ=0 → 3D 블록 미진입(2D 경로)=직전 step 비트 동일 / D=1 → ball 의 z 항 소멸(2D 등가). 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0044
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 3D 자기제한은 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — 3D 혼잡 아레나: verify.js capArena()/seedColumns 와 *같은* 설정.
   *   기본 패널은 회귀 상태(D=1)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나(D=8·생명 수직 컬럼 3개[같은 (x,y), z=0..7]·crowd 만 on·나머지 동역학 off)에 *연직 혼잡(kCrowdZ)* 만 켜고/끄고 두 프리셋으로 A/B 대조한다 —
   *   화면이 보여주는 것 = verify 가 단언하는 수치(kCrowdZ OFF 면 컬럼이 2D 평면상 한 칸이라 혼잡세 0·ON 이면 z-이웃 세 혼잡세>0·m 깎임). (3D 뷰는 아직 z=0 투영 — voxel 렌즈 L-V1 백로그·통계 "생명 m"이 증거.) */
  function capArena(kCZ) {
    return {
      ...
      kCrowd: 0.02, crowdR: 3, kCrowdZ: kCZ,
  ...
  function seedColumns(sim, core) {   // 균일 E=2 + 생명 수직 컬럼 3개(각 z=0..D−1 적층) — verify.js seedColumns 와 동일
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0044 — <span style="color:#ffd060">3D 자기제한</span>: 혼잡 carrying capacity 가 연직축으로 — 수직으로 쌓인 생명도 혼잡세를 낸다(VOXEL.md V5+)',
    subtitle: '0042·0043 이 생명을 z>0 에 올렸다(<b>이동</b>·<b>번식</b>으로 3D 거주). 그러나 혼잡(<code>crowd</code>·step-0012 carrying capacity)은 여전히 2D 평면만 봤다 — 밀도 그리드·disc 가 W·H(2D)라 z>0 생명은 무시(수직으로 무한 적층해도 혼잡세 0 = carrying capacity 누수). 이 step 은 <code>crowd</code> 의 밀도 셈을 <b>disc[2D]→ball[3D]·occ 그리드 W·H→W·H·D</b> 로 일반화한다: 수직으로 쌓인 생명도 제 z±이웃을 세 혼잡세를 낸다 = <b>0012(2D 공멸 방지 자기제한)의 연직 짝</b> — 3D 생명이 무한 적층 못 한다. 형식: move 의 kMoveZ·reproduce 의 kDivZ z-확장과 같은 <b>제자리 확장</b>(새 LAW_ORDER 자리 없음) + 노브 <b>kCrowdZ</b>. 검증(D=8·생명 수직 컬럼 3개[각 z=0..7]·crowd 만 on): <b>kCrowdZ OFF 면 컬럼이 2D 평면상 한 칸이라 혼잡세 0(meanM 1.0 불변) → ON 이면 z-이웃 세 혼잡세 17.28 를 내 meanM 0.28 로 깎인다</b>. <b>kCrowdZ=0 → 3D 블록 미진입(2D 경로)=비트 동일 / D=1 → ball z 항 소멸(2D 등가)</b>(회귀 0·이중 가드). 혼잡세 m→metabolized 쌍 거래(소산 sink·잔차 0)·결정론. <code>node step-0044/verify.js</code> 로 reg·cap·conserve·det. (표준 스택은 step-0027 덴드라이트 그대로 — 3D 자기제한과 직교.)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: true, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 의 3D 자기제한을 한 클릭으로 보이는 설정(verify cap 과 동일 아레나)으로. 둘 다 생명 수직 컬럼 3개·crowd 만 on — *연직 혼잡(kCrowdZ)*만 토글. */
    presets: [
      { label: '🏔 3D 자기제한 ON (수직 적층에 혼잡세·D=8)',
        title: 'verify cap(on) 과 같은 설정: D=8·생명 수직 컬럼 3개(같은 (x,y), z=0..7)·crowd 만 on·*연직 혼잡 kCrowdZ=1*, 8 tick 미리 진행. 수직으로 쌓인 생명이 제 z±이웃을 세 혼잡세 17.28 를 내 m 이 1→0.28 로 깎인다 = 3D 생명이 무한 적층 못 함(carrying capacity). 통계 "생명 m"이 1→0.28 이면 연직 혼잡이 켜진 것 = verify 가 단언하는 수치. (3D 뷰는 z=0 투영이라 컬럼이 한 점으로 겹쳐 보인다 — voxel 렌즈 L-V1 백로그·통계가 증거.)',
        params: capArena(1), seed: seedColumns, run: 8,
        note: '3D 자기제한 ON — 수직 컬럼이 혼잡세로 m 깎임(1→0.28). OFF 버튼과 번갈아 보라(통계 "생명 m").' },
      { label: '🛑 3D 자기제한 OFF (컬럼이 서로 안 보임·대조)',
        title: 'verify cap(off) 과 같은 설정: 같은 아레나·컬럼이지만 kCrowdZ=0. crowd 가 2D 평면만 봐 수직 컬럼이 한 칸으로 투영돼 서로 안 보인다(혼잡세 0·m 1.0 불변). ON 버튼과 번갈아 눌러 A/B 로 비교하면 연직 혼잡이 또렷하다.',
        params: capArena(0), seed: seedColumns, run: 8,
        note: '3D 자기제한 OFF(대조) — 컬럼이 서로 안 보여 혼잡세 0(m 1.0). ON 버튼과 번갈아 눌러 연직 혼잡을 보라.' }
    ],
  ...
  else global.HWS_PANEL_0044 = panel;
