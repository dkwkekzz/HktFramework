/* step-0052 패널 — 질-구배 주화성(kQTaxis): 생명이 *질 구배*(엑서지)를 따라 오른다 — q 의 첫 동역학 되먹임(슈뢰딩거 낙차). 직전 step-0051(q advection) 패널을 잇되 move(주화성)에 질 가중 끌개를 더하고 노브 kQTaxis 1개를 더했다.
 ...
 * 척추: q 는 E 에 올라탄 intensive 상태변수(측정값 아님 — 둘째 척추 아님·SPINE 여섯째 축이 의도한 동역학화) · 국소(제 이웃 q 만) · move 는 여전히 q 를 *읽기만*(미수정·E 미접촉·위치만 — 장부 거래 0). 0049 sink·0050 source·0051 transport 위의 *첫 동역학*.
 * 회귀(이중 가드): kQTaxis=0 → att = E·(1+0) = E 바이트 동일(순수 E 주화성·q 미참조). qInit=false(degrade off)면 미진입(질 축 없는 세계엔 질 구배 없음). 브라우저: window.HWS_PANEL_0052
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 질-구배 주화성은 그 동역학과 직교[E 미접촉]. 이 step 이 생명을 *질 따라* 모은다.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — 질-구배 주화성 아레나: verify.js qtaxArena()/seedQHill() 와 *같은* 설정.
   *   기본 패널은 회귀 상태(kQTaxis=0)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나(2D·평탄 E=1·중앙 방사형 고질 q 원뿔·degrade 질 축 alive·생명 3×3·나머지 동역학 off)에 *질-구배 주화성(kQTaxis)* 만 켜고/끄고 두 프리셋으로 A/B 대조한다 —
   *   화면이 보여주는 것 = verify 가 단언하는 수치(평탄 E 라 OFF 면 생명이 시작 저질 자리에 frozen / ON 이면 생명이 q 따라 중앙 고질로 모임). 흑체 색(L-Q 미리보기)으로 보면 생명이 *밝은(고질) 중심*으로 빨려든다. */
  function qtaxArena(kqt) {
    return {
      ...
      life: true, move: true, moveR: 1, moveThresh: 0.02, pTumble: 0, kMoveZ: 0,
      ...
      kDegrade: 0.01, qInit0: 0, kStarQual: 0, kQAdvect: 0, kQTaxis: kqt,
  ...
  function seedQHill(sim, core) {   // 평탄 E=1 + 중앙(32,32) 방사형 고질 q 원뿔(반경 24) + 생명 3×3(중심서 ~17칸). verify.js seedQHill() 와 같다(q 축 alive).
    var p = sim.p, W = p.W, H = p.H, N = W * H, E = sim.E, q = sim.q, cx = 32, cy = 32, RQ = 24, qHi = 0.95, qLo = 0.05, i, x, y;
    for (i = 0; i < N; i++) { E[i] = 1; sim.E0 += 1; }
    for (y = 0; y < H; y++) for (x = 0; x < W; x++) {
      var dx = Math.min((x - cx + W) % W, (cx - x + W) % W), dy = Math.min((y - cy + H) % H, (cy - y + H) % H);
      var d = Math.sqrt(dx * dx + dy * dy), t = d < RQ ? 1 - d / RQ : 0;
      q[(y * W + x)] = qLo + (qHi - qLo) * t;
    }
    sim.qInit = true;
    for (var gx = 0; gx < 3; gx++) for (var gy = 0; gy < 3; gy++) core.spawnAgent(sim, 18 + gx * 2, 18 + gy * 2);
  }

  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0052 — <span style="color:#ffd060">질-구배 주화성 (kQTaxis)</span>: 생명이 질 구배(엑서지)를 따라 오른다 — q 의 첫 동역학 되먹임(슈뢰딩거 낙차, SPINE 여섯째 축)',
    subtitle: '0049 가 질 축 <code>q</code>∈[0,1] + 둘째 법칙 강등을, 0050 이 별 질 생산을, 0051 이 질 수송(advection)을 깔았다 — 질이 만들어지고·사라지고·흐르는 <b>완전한 물리장</b>이 됐다. 그러나 질은 아직 <b>읽기 전용</b>(생명이 질 무관 — 그저 E 많은 곳을 쫓음). 이 step 은 질을 처음으로 <b>동역학에 되먹인다</b>: <code>move</code>(주화성 run)의 이웃 비교를 <b>질 가중 끌개</b> att = E·(1 + kQTaxis·q) 로 바꾼다 → 생명이 그저 E 많은 곳이 아니라 <b>질 좋은(고 q = 엑서지 높은) E</b> 로 모인다(슈뢰딩거: 생명은 <i>자유에너지</i>[저엔트로피 질]로 산다 — 양이 아니라 질의 낙차로). 척추: <code>q</code> 는 E 에 올라탄 intensive 상태변수(측정값 아님 — <b>둘째 척추 아님</b>·SPINE 여섯째 축이 의도한 동역학화) · 국소(제 이웃 q) · move 는 여전히 q 를 <b>읽기만</b>(미수정·E 미접촉·위치만 — 장부 거래 0). <b>0049 sink·0050 source·0051 transport 위의 첫 동역학.</b> 검증(D=1·평탄 E=1·중앙 방사형 고질 q 원뿔·생명 3×3, 24 tick): <b>kQTaxis OFF 면 평탄 E 라 생명 frozen(시작 저질 자리·meanQ 0.22·중심 거리 17.8) → ON 이면 q 따라 중앙 고질로 모임(meanQ 0.69·중심 거리 1.8)</b> = 엑서지 추종. <b>kQTaxis=0 → att=E·(1+0)=E 바이트 동일 → 직전 step(0051) 비트 동일</b>(회귀 0·이중 가드: qInit=false 면도 미진입). <code>node step-0052/verify.js</code> 로 reg·taxis·conserve·det. 우측 흑체 색(L-Q 미리보기)으로 보면 생명이 <b>밝은(고질) 중심</b>으로 빨려든다. (표준 스택은 step-0027 덴드라이트 그대로 — 질-구배 주화성은 그와 직교[E 미접촉].)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: true, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 의 질-구배 주화성을 한 클릭으로 보이는 설정(verify taxis 와 동일 아레나)으로. 둘 다 평탄 E·중앙 고질 q 원뿔·degrade·생명 3×3 — *질-구배 주화성(kQTaxis)*만 토글. seed 로 q 원뿔 + 생명을 심는다. */
    presets: [
      { label: '🧬 질-구배 주화성 ON (생명이 고질 중심으로 모임)',
        title: 'verify taxis(on) 과 같은 설정: D=1·평탄 E=1·중앙(32,32) 방사형 고질 q 원뿔(반경 24)·degrade 0.01·생명 3×3(중심서 ~17칸)·*kQTaxis=5*, 24 tick 진행. 생명이 그저 E 많은 곳이 아니라 *질 좋은(고 q) E* 를 쫓아 중앙 고질로 빨려든다 → meanQ 0.22→0.69·중심 거리 17.8→1.8 = 엑서지 추종(슈뢰딩거 낙차). 우측 흑체 색에서 생명이 밝은 중심으로 모이면 켜진 것 = verify 가 단언하는 수치.',
        params: qtaxArena(5), seed: seedQHill, run: 24,
        note: '질-구배 주화성 ON — 생명이 고질 중심으로 모임(meanQ 0.69·거리 1.8). OFF 버튼과 번갈아 보라.' },
      { label: '🧊 질-구배 주화성 OFF (생명 frozen·대조)',
        title: 'verify taxis(off) 과 같은 설정: 같은 아레나지만 kQTaxis=0(순수 E 주화성). 평탄 E 라 생명이 움직일 방향 신호가 없어 시작 저질 자리에 *frozen*(meanQ 0.22·중심 거리 17.8) = 직전 step(0051) 비트 동일. ON 버튼과 번갈아 눌러 A/B 로 비교하면 생명이 질을 따라가는지가 또렷하다(중심에 모임 vs 제자리).',
        params: qtaxArena(0), seed: seedQHill, run: 24,
        note: '질-구배 주화성 OFF(대조) — 평탄 E 라 생명 frozen(meanQ 0.22). ON 버튼과 번갈아 보라.' }
    ],
    ...
    stats: [
      ...
      { label: '<span style="color:#ffd060">에너지 질</span> (엑서지 X=Σq·E / 고질 봉우리 maxQ / 질 구배 stdQ — 별 source[0050]·질 수송[0051])', get: function (c) {
          var sim = c.sim;
          if (!sim.qInit) return { text: '질 축 없음 (kDegrade=0 — q 미작동. 위 "질-구배 주화성 ON" 버튼으로)', cls: '' };
          ...
          var stdQ = n > 0 ? Math.sqrt(v / n) : 0, qOn = ((sim.p.kStarQual || 0) !== 0) || ((sim.p.kQAdvect || 0) !== 0) || ((sim.p.kQTaxis || 0) !== 0);
  ...
  else global.HWS_PANEL_0052 = panel;
