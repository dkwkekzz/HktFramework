/* step-0039 패널 — 죽은 별 잔해→새 씨앗(VOXEL.md V5+ — 0038 일몰사 위에 얹음). 직전 step-0038(별 죽음·일몰사) 패널을 잇되 새 노브 행 1개(잔해→씨앗 kAshSeed)를 더했다.
 * 이 step 은 ignite 법칙을 *제자리 확장*한다(LAW_ORDER 무변경, 0035·0036·0038 위에 얹음): 0038 은 진 별이 z=0 서 *흔적 없이* 사라졌다(잔해 없음). kAshSeed>0 이면 일몰사로 z=0(무덤)에서 꺼지는 별의 미연소 외부 연료 일부가 그 자리 저장체 R 로 가라앉아 *다음 별의 점화 씨앗*이 된다.
 * 완성하는 그림: 별이 z=0(R 핵)서 나→천장까지 뜨고→연료 쇠해 도로 지고→지평선 닿아 꺼지고(0038 死)→그 무덤에 R 잔해를 남겨(0039)→다음 별이 거기서 난다 → 出沒生死 → 잔해 → 재구성 → 다음 별(SPINE 척도분리: 빠른 비가역 死 + 느린 저장 재구성 = 세계 척도 순환·별의 외부 질량이 세계 안 R 로 내생화).
 * 회귀(이중 가드): kAshSeed=0 이면 잔해 침착 미진입(0038 비트 동일) / D=1·rise off 면 일몰사 미발생 → 잔해 침착 도달 못 함(2D 레거시 불변). 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0039
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 별 잔해→씨앗은 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — 잔해→씨앗 아레나: verify.js remnArena()/deathArena()/seedSunCore() 와 *같은* 설정.
   *   기본 패널은 회귀 상태(D=1·잔해 off)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나(D=8 voxel·z=0 R 핵서 단일 별 점화·부력 상승·하강·일몰사 on)에 *잔해 침착*을 켜고/끄고 두 프리셋으로 A/B 대조한다 — 화면이 보여주는 것 =
   *   verify 가 단언하는 수치(별 잔해 R 누적)가 되게. 잔해 ON 이면 진 별이 무덤(z=0)에 R 씨앗을 남긴다(저장체↑·중심 R 점이 밝아진다). */
  function remnArena(kSeed) {
    return {
      ...
      kIgnite: 1, kStarRise: 1, kStarFall: 1, starFallThresh: 0.5, kStarSet: 1, kAshSeed: kSeed, starRate: 0.06, starFuel0: 500, ignThresh: 1.5, starCap: 1, starGap: 6, starR: 2, starDriftPeriod: 20,
      kGravity: 0, kFSM: 0, kCollapse: 0, kCryst: 0, kWeather: 0, kSupport: 0, kOcclude: 0,
  ...
  function seedSunCore(sim, core) {   // z=0 정적 R 핵(점화 신호 — 죽으면 다음 별이 또 여기서 난다) — verify.js seedSunCore 와 동일(반경 2·값 2.0·E0 산입)
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0039 — <span style="color:#ffd060">죽은 별 잔해→새 씨앗</span>: 진 별이 무덤에 R 잔해를 남겨 다음 별의 씨앗이 된다 = 出沒生死 → 재구성(VOXEL.md V5+)',
    subtitle: 'step-0038 은 떴다 진 별을 지평선(z=0)에서 꺼지게 했으나, 진 별이 <b>흔적 없이 사라졌다</b>(잔해 없음 — 빈 자리엔 *처음 놓은* R 핵만 남아 다음 별이 났다). 이 step 은 그 끝을 닫는다 — 일몰사로 z=0(무덤)에서 꺼지는 별이 <b>미연소 외부 연료의 일부를 그 자리 저장체 R 로 남긴다</b>(잔해). 이 step 은 <code>ignite</code> 법칙을 제자리 확장한다(LAW_ORDER 무변경, 0035·0036·0038 위에 얹음): <b>kAshSeed</b> 를 켜면 진 별의 연료 × kAshSeed 가 무덤에 R 로 가라앉아 <b>다음 별의 점화 씨앗</b>이 된다. 完成: 별이 나→뜨고→지고→꺼지고(0038 死)→<b>무덤에 R 잔해를 남겨</b>(0039)→다음 별이 거기서 난다 → <b>出沒生死 → 잔해 → 재구성 → 다음 별</b>(SPINE 척도분리: 死는 빠르고 비가역[개체 척도]이나 잔해는 느리게 저장 극단으로 재구성[세계 척도 순환] — 별의 외부 질량이 세계 안 R 로 내생화). 장부: 연료 F 는 외부 질량(lhs 밖) → R 로 들이며 <b>E0 를 같은 만큼 올려</b> 닫힌 장부 유지(seedSunCore·harvest 와 같은 외부 유입 보정). <b>kAshSeed=0 이면 잔해 침착 미진입(0038 비트 동일) / D=1·rise off 면 일몰사 미발생 → 잔해 침착 도달 못 함</b>(회귀 0·이중 가드). 검증(중력 off 격리): <b>잔해 off 면 진 별이 흔적 없이 사라져 R 증가 0 → on 이면 무덤마다 미연소 연료 절반이 R 로(R 증가 119)</b>·잔차 8.6e-16·결정론. <code>node step-0039/verify.js</code> 로 reg·remnant·conserve·det. (표준 스택은 step-0027 덴드라이트 그대로 — 별 잔해→씨앗과 직교.)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: true, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 이 더한 별 잔해→씨앗을 한 클릭으로 보이는 설정(verify remnant 와 동일 아레나)으로. 둘 다 별 상승·하강·일몰사 on(별이 떴다 져 꺼진다) — *잔해 침착*만 토글. */
    presets: [
      { label: '🪦 별 잔해→씨앗 ON (재구성·D=8)',
        title: 'verify remnant(on) 과 같은 설정: D=8 격리 아레나·z=0 R 핵서 단일 별 점화·kStarRise=1·kStarFall=1·kStarSet=1·*kAshSeed=0.5*, 200 tick 미리 진행. 별이 떴다 지며 지평선(z=0) 닿아 꺼질 때, 미연소 연료 절반을 무덤에 R 로 남긴다. 통계 "별 잔해 R" 이 0보다 크면(≈119) 잔해 재구성 = verify 가 단언하는 수치(중심 R 점이 밝아진다). (3D 뷰는 아직 z=0 평면만 그려 z 궤적 자체는 안 보임 — voxel 렌즈 L-V1 백로그. 수치가 증거.)',
        params: remnArena(0.5), seed: seedSunCore, run: 200,
        note: '별 잔해 ON — "별 잔해 R"≈119(무덤에 R 씨앗을 남김=재구성). OFF 버튼과 번갈아 보라.' },
      { label: '☠ 잔해 OFF (=0038·흔적 없음·대조)',
        title: 'verify remnant(off) 과 같은 설정: 같은 아레나·핵·별 상승·하강·일몰사지만 kAshSeed=0(=0038). 진 별이 z=0 서 *흔적 없이* 사라져 R 잔해가 0 이다(저장체 안 늘어남). ON 버튼과 번갈아 눌러 A/B 로 비교하면 잔해 재구성이 또렷하다.',
        params: remnArena(0), seed: seedSunCore, run: 200,
        note: '잔해 OFF(대조·=0038) — "별 잔해 R"=0(진 별이 흔적 없이 사라짐). ON 버튼과 번갈아 눌러 잔해 재구성을 보라.' }
    ],
    ...
    stats: [
      ...
      { label: '<span style="color:#ffd060">별 잔해 R</span> (누적 씨앗 / 사망 — 가설: 出沒生死→재구성)', get: function (c) {
          var D = c.sim.p.D || 1, ashOn = (c.sim.p.kAshSeed || 0) !== 0;
          var seeded = c.sim.ashSeeded || 0, d = c.sim.starDeaths || 0, b = c.sim.starBirths || 0;
          if (D < 2) return { text: '잔해 ' + seeded.toFixed(1) + ' / 사망 ' + d + ' (z=0 평면 D=1 — 잔해 침착 비활성. 위 "데모" 버튼으로 D=8)', cls: '' };
          return { text: '잔해 R ' + seeded.toFixed(1) + ' / 사망 ' + d + ' / 출생 ' + b, cls: (ashOn && seeded > 1 && d >= 1) ? 'pass' : '' };
      } },
    ...
    legend:
      ...
      '<b>죽은 별 잔해→새 씨앗(VOXEL.md V5+)</b>: step-0038 은 떴다 진 별을 지평선(z=0)에서 꺼지게 했으나 진 별이 <b>흔적 없이 사라졌다</b>(잔해 없음·정직한 한계). 이 step 은 그 끝을 닫는다 — 일몰사로 z=0(무덤)에서 꺼지는 별이 <b>미연소 외부 연료의 일부를 그 자리 R 로 남긴다</b>(잔해). <code>ignite</code> 제자리 확장(0035·0036·0038 위에 얹음): <b>kAshSeed</b> 켜면 진 별 연료 × kAshSeed 가 무덤에 R 로 가라앉아 <b>다음 별의 점화 씨앗</b>이 된다 → 出沒生死 → 잔해 → 재구성 → 다음 별. 장부: 연료 F[외부 질량]가 R 로 들며 E0 를 같은 만큼 올려 닫힌 장부 유지. "터 깊이 D" 8·"별 부력 상승"·"별 하강"·"별 일몰사"·"별 잔해→씨앗" 켜고 리셋해 보라(위 "별 잔해→씨앗 ON" 버튼). <code>verify remnant</code>: <b>잔해 off 면 진 별이 흔적 없이 사라져 R 증가 0·on 이면 무덤마다 미연소 연료 절반이 R 로(R 증가 119)</b>·잔차 8.6e-16. SPINE 척도분리: 빠른 비가역 死 + 느린 저장 재구성 = 세계 척도 순환(별의 외부 질량이 세계 안 R 로 내생화). (3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더는 렌더러 트랙 L-V1.)<br>' +
  ...
  else global.HWS_PANEL_0039 = panel;
