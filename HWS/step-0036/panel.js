/* step-0036 패널 — 별 하강·일생(VOXEL.md V5+: 별 부력이 *활성도[연료]의 함수*가 된다 — 떠올랐다 식으며 진다 = 일출·일몰. step-0035 on/off 상승의 *시간축 닫음*). 직전 step-0035(별 부력 상승) 패널을 잇되 새 노브 행 1개(별 하강 kStarFall)를 더했다.
 * 이 step 은 ignite 법칙을 *제자리 확장*한다(LAW_ORDER 무변경, 0035 상승 위에 얹음): kStarFall>0 이면 연료가 starFallThresh 분율 아래로 쇠한 *죽어가는* 별이 부력을 잃고 매 tick z 한 칸씩 가라앉으며(z=0 까지) 그 z 에서 계속 방출 → 방출 E 가 하강 경로를 따라 내려간다.
 * 완성하는 그림: 별이 z=0(R 핵)서 나(0035 birth)→연료 충분하면 천장까지 떠오르고(0035 rise·prime)→연료가 쇠하면 도로 가라앉는다(0036 set·일몰) → 활성도 연속축이 *연직 궤적*(出·沒)으로. 高활성=떠오름·쇠퇴=가라앉음(authored "별=위" 분기 아닌 *활성도의 함수*).
 * kStarFall=0 이면 dying 늘 false(0035 상승 무변경·하강 미진입) / D=1 이면 st.z=0 고정(상승도 하강도 z 벽) = 직전 step 비트 동일(회귀). 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0036
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 별 하강은 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — 별 하강·일생 아레나: verify.js fallArena()/seedSunCore()/comZ() 와 *같은* 설정·식.
   *   기본 패널은 회귀 상태(D=1·하강 off)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나(D=8 voxel·중력 off 격리·z=0 R 핵서 단일 별 점화·부력 상승 on)를 한 클릭으로 띄워, 화면이 보여주는 것 =
   *   verify 가 단언하는 수치(별 최종 z·E 무게중심 z)가 되게 한다. 하강 ON/OFF 두 프리셋으로 A/B 대조. */
  function fallArena(kFall) {
    return {
      ...
      kIgnite: 1, kStarRise: 1, kStarFall: kFall, starFallThresh: 0.5, starRate: 0.06, starFuel0: 500, ignThresh: 1.5, starCap: 1, starGap: 6, starR: 2, starDriftPeriod: 20,
  ...
  function comZE(sim) {                // E 무게중심 z(평면별 E 합 가중) — verify.js comZ 와 동일식. 하강 on 이면 별이 졌으니 낮아짐
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0036 — <span style="color:#ffd060">별 하강·일생</span>: 부력이 활성도(연료)의 함수가 돼 별이 떴다 진다 = 일출·일몰(VOXEL.md V5+)',
    subtitle: 'step-0035 는 <i>소산 극단</i>(별)이 <b>떠오르는</b> 걸 on/off 로 더했다(부력 상승). 이 step 은 그 <b>시간축을 닫는다</b> — 부력을 <i>활성도(연료 잔량)의 함수</i>로 만들어 별이 떴다 <b>다시 진다</b>. 이 step 은 <code>ignite</code> 법칙을 제자리 확장한다(LAW_ORDER 무변경, 0035 상승 위에 얹음): <b>kStarFall</b> 을 켜면 연료가 <b>starFallThresh 분율 아래로 쇠한</b> 죽어가는 별이 부력을 잃고 <b>매 tick z 한 칸씩 가라앉으며</b>(z=0 까지) 그 z 에서 <b>계속 탄다</b> → 방출 E 가 하강 경로를 따라 내려간다(상승[0035]이 z 만 바꿔 위치-only 였던 것과 달리, 하강은 *연소 중*이라 E-필드 결과가 있다). 完成: 별이 z=0(R 핵)서 나→연료 충분하면 천장까지 뜨고→쇠하면 도로 진다 → <b>활성도 연속축(SPINE 결정2)이 *연직 궤적*(出·沒)으로</b> — 高활성=떠오름·쇠퇴=가라앉음(authored "별=위" 분기 아닌 *활성도의 함수*). <b>kStarFall=0 이면 dying 늘 false(0035 상승 무변경·하강 미진입) / D=1 이면 st.z=0 고정 = 직전 step 비트 동일</b>(회귀 0). 검증(중력 off 로 격리): <b>둘 다 천장(z=7)까지 떠오르나 → 하강 off 면 거기 머묾(최종 z=7·무게중심 z=6.36)·on 이면 연료 쇠해 바닥(최종 z=0)까지 지며 방출(무게중심 z=5.83 &lt; off)</b>·잔차 1.7e-15·결정론. <code>node step-0036/verify.js</code> 로 reg·set·conserve·det. (표준 스택은 step-0027 덴드라이트 그대로 — 별 하강과 직교.)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: true, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 이 더한 별 하강을 한 클릭으로 보이는 설정(verify set 과 동일 아레나)으로. 둘 다 부력 상승 on(별이 뜬다) — 하강만 토글. */
    presets: [
      { label: '🌇 별 하강 ON (일출·일몰·D=8)',
        title: 'verify set(on) 과 같은 설정: D=8 격리 아레나·z=0 R 핵서 단일 별 점화·kStarRise=1·kStarFall=1, 200 tick 미리 진행. 별이 천장(z=7)까지 떴다가 연료가 절반 아래로 쇠해 *다시 바닥(z=0)까지 진다*. 통계 "별 최고/최종 z" 가 7→0(졌다)이면 일몰 = verify 가 단언하는 수치. (3D 뷰는 아직 z=0 평면만 그려 z 궤적 자체는 안 보임 — voxel 렌즈 L-V1 백로그. 수치가 증거.)',
        params: fallArena(1), seed: seedSunCore, run: 200,
        note: '별 하강 ON — "별 최종 z"=0(졌다)·"E 무게중심 z"≈5.83(하강 방출로 off 6.36 보다 낮음)=일몰. OFF 버튼과 번갈아 보라.' },
      { label: '☀ 하강 OFF (안 짐·대조)',
        title: 'verify set(off) 과 같은 설정: 같은 아레나·핵·점화·부력 상승이지만 kStarFall=0. 별이 천장(z=7)까지 떠올라 *거기 머문다*(최종 z=7·E 무게중심 z=6.36). ON 버튼과 번갈아 눌러 A/B 로 비교하면 하강이 더한 것이 또렷하다.',
        params: fallArena(0), seed: seedSunCore, run: 200,
        note: '하강 OFF(대조) — "별 최종 z"=7(천장에 머묾)·"E 무게중심 z"=6.36. ON 버튼과 번갈아 눌러 일몰을 보라.' }
    ],
    ...
    stats: [
      ...
      { label: '<span style="color:#ffd060">별 z·E 무게중심</span> (가설: 떴다 진다)', get: function (c) {
          ...
          var fz = st.length ? (st[0].z || 0) : '—', fall = c.sim.p.kStarFall !== 0;
          return { text: '별 최고 z=' + maxZ + ' · 최종 z=' + fz + ' / 천장 ' + (r.D - 1) + ' · E무게중심 z=' + r.comZ.toFixed(2), cls: (fall && fz === 0 && maxZ === r.D - 1) ? 'pass' : '' };
      } },
    ...
    legend:
      ...
      '<b>별 하강·일생(VOXEL.md V5+)</b>: step-0035 는 *소산 극단*(별)이 <b>떠오르는</b> 걸 on/off 로 더했다. 이 step 은 그 <b>시간축을 닫는다</b> — 부력을 *활성도(연료)의 함수*로 만들어 별이 떴다 <b>다시 진다</b>(일출·일몰). <code>ignite</code> 제자리 확장(0035 위에 얹음): <b>kStarFall</b> 을 켜면 연료가 starFallThresh 분율 아래로 쇠한 별이 부력을 잃고 매 tick z 한 칸씩 가라앉으며(z=0 까지) <b>계속 탄다</b> → 방출 E 가 하강 경로를 따라 내려간다. 高활성=떠오름·쇠퇴=가라앉음 → <b>활성도 축이 연직 *궤적*(出·沒)</b>으로. "터 깊이 D" 8·"별 부력 상승"·"별 하강" 켜고 리셋해 보라. <code>verify set</code>(중력 off 격리): <b>둘 다 천장(z=7) 떴다 → 하강 off 면 머묾(최종 z=7·무게중심 z=6.36)·on 이면 바닥(최종 z=0)까지 지며 방출(무게중심 z=5.83)</b>·잔차 1.7e-15. step-0035 on/off 상승의 시간축 닫음. (3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더는 렌더러 트랙 L-V1.)<br>' +
  ...
  else global.HWS_PANEL_0036 = panel;
