/* step-0030 패널 — 6-이웃 확산(VOXEL.md V2: 흐름·응집 stencil 에 z 항을 더해 상위 평면이 살아난다). 직전 step-0029(격자 z-일반화) 패널을 잇되 새 노브 행 1개(z 확산 kDz)를 더했다.
 * 이 step 은 diffuse 의 선형 확산 stencil 을 z=0 평면에서 W×H×D 상자로 일반화한다(4-이웃→6-이웃). V1 은 격자만 일반화해 상위 평면이 불활성이었다 — V2 는 z 항(상하 이웃)으로 z=0 평면의 E 가 z 로 퍼져 상위 평면이 비로소 산다.
 * kDz=0 또는 D=1 이면 z 항이 0 = 직전 step 비트 동일(회귀). kDz=kD 면 등방 확산. 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0030
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — z 확산은 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0030 — <span style="color:#9ad24a">6-이웃 확산</span>: 흐름 stencil 에 z 항을 더해 상위 평면이 살아난다(VOXEL.md V2)',
    subtitle: 'V1(step-0029)은 터를 <b>W×H×D voxel</b> 로 일반화했으나 법칙이 z=0 평면만 처리해 <i>상위 평면이 불활성</i>이었다(초기 noise 만 보존). 이 step(V2)은 확산 stencil 을 <b>4-이웃에서 6-이웃</b>으로 — <code>diffuse</code> 의 선형 확산을 z=0 평면에서 <b>W×H×D 상자 전체</b>로 일반화하고 <b>z 항</b>(상하 이웃)을 더한다. 그러면 z=0 평면의 E 가 <b>z 로 퍼져 상위 평면이 비로소 산다</b>. 노브 <b>kDz</b>(z 확산 계수)가 z 결합을 켠다: <b>kDz=0 또는 D=1 이면 z 항이 산술로 0 = 직전 step 비트 동일</b>(회귀 0), <b>kDz=kD 면 등방</b>(x·y·z 동일 계수). 안정 조건(6-이웃): 4·kD + 2·kDz ≤ 1. 모든 평면이 <i>같은 수치 스킴</i>으로 확산하고, z 항은 상하 쌍 거래가 cancel 되어 <b>닫힌 장부 유지</b>. 검증: <b>z=0 한 점 주입 후 87% 가 상위 평면으로</b>(kDz=0 이면 0%)·<b>등방(z-이웃 ≡ x-이웃)</b>·잔차 3e-15·결정론. <code>node step-0030/verify.js</code> 로 reg·zdiffuse·conserve·det. 중력(바다)은 V3·지지 침착(지면)은 V4 로 전가. (표준 스택은 step-0027 덴드라이트 그대로 — z 확산과 직교.)',
    ...
    legend:
      ...
      '<b>6-이웃 확산(VOXEL.md V2)</b>: V1 은 터를 W×H×D voxel 로 일반화했으나 법칙이 z=0 평면만 처리해 상위 평면이 불활성이었다. 이 step 은 확산 stencil 을 4-이웃→6-이웃으로(z 항) — z=0 평면의 E 가 <b>z 로 퍼져 상위 평면이 산다</b>. "터 깊이 D" 를 8 로, "z 확산" 을 켜고 리셋해 보라. <code>verify zdiffuse</code>: z=0 한 점 주입 후 <b>상위 평면 분율 0%(kDz=0)→87%(kDz=0.15)</b>·등방(z-이웃≡x-이웃, 상대오차 0)·잔차 3e-15. 중력(바다)은 V3·지지 침착(지면)은 V4. (3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더는 렌더러 트랙 L-V1.)<br>' +
  ...
  else global.HWS_PANEL_0030 = panel;
