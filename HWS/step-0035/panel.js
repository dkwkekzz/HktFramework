/* step-0035 패널 — 별 부력 상승(VOXEL.md V5+: 소산 극단[별]이 떠올라 高z 하늘 배회=태양. step-0034 저장 극단[R] 침강의 짝 — 활성도 축이 연직축). 직전 step-0034(부유 R 붕괴) 패널을 잇되 새 노브 행 1개(별 부력 kStarRise)를 더했다.
 * 이 step 은 ignite 법칙을 *제자리 확장*한다(LAW_ORDER 무변경): kStarRise>0 이면 점화한 별이 z=0(지면 R 핵)에서 태어나 매 tick 천장(z=D−1)까지 떠올라(부력) 제 z 의 3D ball(discCells3)로 E 방출 → 높이서 뿜은 E 를 중력(V3)이 아래로 끌어내려 비처럼 내린다 = 태양.
 * step-0034 붕괴(저장 R 침강)와 *대칭*: 저장 극단은 가라앉고 소산 극단은 떠오른다 → 활성도 연속축이 *연직 z 축*으로 창발(저장 아래·소산 위, 측정 아닌 동역학).
 * kStarRise=0 또는 D=1 이면 부력 무효(별 z 미설정·2D disc·서행만 / D−1=0 상승 0·discCells3≡discCells) = 직전 step 비트 동일(회귀). 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0035
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 별 부력은 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  var panel = {
    ...
    controls: [
      ...
      { items: [
        ...
        { kind: 'slider', id: 'kStarRise', label: 'kStarRise (부력 상승률)', param: 'kStarRise', min: 0, max: 3, step: 1, def: 1, fixed: 0, gateBy: 'starrisec', gateOff: 0, title: '별 부력 상승률(매 tick z 상승 칸수). 0 또는 D=1 이면 부력 무효(별 z 미설정·2D / 상승 0) = 회귀(직전 step 비트 동일). 위치만 — 거래 0(장부 무관). *리셋 불필요* — 즉시 적용. 중력(V3)을 함께 켜면 高z E 가 아래로 비처럼 내린다.' }
      ]},
      { items: [
    ...
    legend:
      ...
      '<b>별 부력 상승(VOXEL.md V5+)</b>: step-0034 는 *저장 극단*(R)이 <b>가라앉는</b> 걸 더했다. 이 step 은 그 <b>대칭</b> — *소산 극단*(별=활성도 최고)이 <b>떠오른다</b>. <code>ignite</code> 제자리 확장: <b>kStarRise</b> 를 켜면 점화한 별이 z=0(지면 R 핵)에서 태어나 매 tick 천장까지 떠올라 제 z 의 <b>3D ball</b> 로 E 방출 → 하늘서 뿜은 E 를 <code>gravity</code>(V3)가 비처럼 내린다 = <b>태양</b>. 저장 아래·소산 위 → <b>활성도 축이 연직 z 축</b>으로 창발. "터 깊이 D" 8·"별 부력 상승" 켜고 리셋해 보라. <code>verify sun</code>(중력 off 격리): z=0 R 핵 점화 → <b>상승 off 면 별 z=0·E 바닥(무게중심 z=0)·on 이면 별 천장(z=7)·E 위에(무게중심 z=6.36·천장 54%)</b>·잔차 1.5e-15. step-0034 R 침강의 짝. (3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더는 렌더러 트랙 L-V1.)<br>' +
  ...
  else global.HWS_PANEL_0035 = panel;
