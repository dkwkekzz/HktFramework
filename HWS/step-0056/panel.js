/* step-0056 패널 — couple advection(kQCouple): 질이 *막 공유* E 흐름에 동승 — q 가 모든 E 이동(하향+등방+막 공유)을 따라가는 완전한 물리장. 직전 step-0055(diffuse advection) 패널을 잇되 couple(⑥c)에 질 동승 블록과 노브 kQCouple 1개를 더했다.
 * 0051 이 질을 *gravity*(하향)·0055 가 *diffuse*(등방) 흐름에 실어 q 가 *필드* E 이동을 따라갔으나, kin 막의 E 공유(couple)는 아직 q 를 안 데려간다 — 액적 안에서 E 가 균등화돼도 q 는 제자리에 stranded(막 안 E 는 평탄해지는데 q 는 비균질).
 * 이 step 은 질을 couple flux 에도 싣는다: couple 이 kin 쌍 사이 E 를 균등화(d=(E[c]−E[nb])·k 이동)할 때 받는 칸은 *흐른 E 의 질*을 질량가중으로 섞는다(받는 칸 q ← (q·E_recv + q_donor·|d|)/(E_recv+|d|)). 막 안에서 E 가 평탄해지면 그 질도 평탄해진다 = q 가 *모든 E 이동*(하향[0051]+등방[0055]+막 공유[이번])을 따라가는 완전한 물리장.
 ...
 * 회귀(이중 가드): kQCouple=0 → 혼합 미진입(q 미접촉) 바이트 동일. qInit=false(degrade off)면 미진입. 브라우저: window.HWS_PANEL_0056
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — q advection 은 그 동역학과 직교[E 미접촉]. 이 step 이 질을 *막 공유 따라 흐르게* 한다.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — couple advection 아레나: verify.js qcplArena()/seedQCouple() 와 *같은* 설정(kQCouple 토글).
   *   기본 패널은 회귀 상태(kQCouple=0)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나(2D·8×8 kin 블록[좌 4열 고 E·고질 / 우 4열 저 E·저질]·couple kMembrane 0.5·degrade 질 축 alive)에 *질 동승(kQCouple)* 만 켜고/끄고 두 프리셋으로 A/B 대조한다 —
   *   화면이 보여주는 것 = verify 가 단언하는 수치(OFF 면 couple 이 E 를 좌→우 균등화해도 q 가 좌측에 stranded[우측은 E 만] / ON 이면 q 가 E 따라 우측으로 advect → 막 안 q 균질). 흑체 색(L-Q 미리보기)으로 보면 ON 일 때 막 공유로 흐른 E 가 *고질 색을 데리고* 우측으로 번진다. */
  function qcplArena(kqc) {
    return {
      ...
      kDegrade: 0.01, qInit0: 0, kStarQual: 0, kQAdvect: 0, kQTaxis: 0, kQMetab: 0, kQExport: 0, kQDiffuse: 0, kQCouple: kqc,
  ...
  function seedQCouple(sim, core) {   // 2D 8×8 kin 블록(태그 1) — 좌 4열 고 E·고질(E 10·q 1)·우 4열 저 E·저질(E 0·q 0). spawn 먼저(E=0→m=0) 뒤 E/q 설정. verify.js seedQCouple() 와 같다(q 축 alive).
    var p = sim.p, W = p.W, cx = (W >> 1) - 4, cy = (p.H >> 1) - 4, x, y;
    for (y = cy; y < cy + 8; y++) for (x = cx; x < cx + 8; x++) {
      var a = core.spawnAgent(sim, x, y, 0); a.g = 1;
      var i = y * W + x, hi = x < cx + 4;
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0056 — <span style="color:#ffd060">couple advection (kQCouple)</span>: 질이 막 공유 E 흐름에 동승 — q 가 모든 E 이동(하향+등방+막 공유)을 따라가는 완전한 물리장(SPINE 여섯째 축 transport)',
    subtitle: '0051 이 질을 <b>gravity</b>(하향)·0055 가 <b>diffuse</b>(등방) 흐름에 실어 q 가 <b>필드</b> E 이동을 따라갔으나, kin 막의 E 공유(<code>couple</code>)는 아직 q 를 안 데려간다 — 액적(kin 도메인) 안에서 E 가 균등화돼도 <code>q</code> 는 제자리에 stranded(막 안 E 는 평탄해지는데 q 는 비균질). 이 step 은 질을 couple flux 에도 싣는다: <code>couple</code>(⑥c)이 kin 쌍 사이 E 를 균등화(d=(E[c]−E[nb])·k 이동)할 때 받는 칸은 <b>흐른 E 의 질</b>을 질량가중으로 섞는다(받는 칸 q ← (q·E_recv + q_donor·|d|)/(E_recv+|d|)). 막 안에서 E 가 평탄해지면 그 질도 평탄해진다 = <b>q 가 모든 E 이동(하향[0051]+등방[0055]+막 공유[이번])을 따라가는 완전한 물리장</b>. 척추: <code>q</code> 는 E 에 올라탄 intensive 속성(단일 척추) · 국소(제 우/하(/위) kin 이웃 한 쌍) · advection 은 E 미접촉(couple 이 <i>이미 옮긴</i> E 의 질만 따라감 — q 는 비율·거래 0, 닫힌 장부). <b>0048 수동 막 E-공유의 질 짝</b>. 검증(D=1·8×8 kin 블록[좌 고질 E 10·q 1 / 우 저질 E 0·q 0]·couple kMembrane 0.5, 20 tick): <b>kQCouple OFF 면 couple 이 E 를 좌→우 균등화해도 우측 q 가 stranded(우 q 0.00) → ON 이면 q 가 E 따라 우측으로 advect(우 q 0.82, 막 안 q 균질)</b>. <b>kQCouple=0 → q 미접촉 → 직전 step(0055) 비트 동일</b>(회귀 0·이중 가드: qInit=false 면도 미진입). <code>node step-0056/verify.js</code> 로 reg·advect·conserve·det. 우측 흑체 색(L-Q 미리보기)으로 보면 ON 일 때 막 공유로 흐른 E 가 고질 색을 데리고 우측으로 번진다. (표준 스택은 step-0027 덴드라이트 그대로 — q advection 은 그와 직교[E 미접촉].)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: true, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 의 couple advection 을 한 클릭으로 보이는 설정(verify advect 와 동일 아레나)으로. 둘 다 8×8 kin 블록(좌 고질·우 저질)·couple·degrade — *질 동승(kQCouple)*만 토글. seed 로 kin 블록을 심는다. */
    presets: [
      { label: '🫧 couple advection ON (고질이 막 공유 따라 우측으로 흐름)',
        title: 'verify advect(on) 과 같은 설정: D=1·8×8 kin 블록(좌 4열 고 E 10·q 1 / 우 4열 저 E 0·q 0·태그 1)·couple(kMembrane 0.5)·degrade 0.01·*kQCouple=1*, 20 tick 진행. couple 이 막 안 E 를 좌→우 균등화하며 *질을 데리고 간다* → 우측 저질 자리 q 0.00→0.82 로 채워진다(막 안 q 균질). 우측 흑체 색에서 막 공유로 흐른 E 가 *고질 색을 데리고* 우측으로 번지면 켜진 것 = q 가 막 공유 E 를 따라가는 완전한 물리장(verify 가 단언하는 수치).',
        params: qcplArena(1), seed: seedQCouple, run: 20,
        note: 'couple advection ON — 우측 q 0.00→0.82(질이 막 공유 E 따라 흐름). OFF 버튼과 번갈아 보라.' },
      { label: '🧊 couple advection OFF (q 좌측 stranded·대조)',
        title: 'verify advect(off) 과 같은 설정: 같은 아레나지만 kQCouple=0(q 미접촉). couple 이 E 를 좌→우 균등화해도 *q 는 좌측에 stranded* → 우측은 E 만 도착하고 질은 빈 채(우 q 0.00) = 직전 step(0055) 비트 동일. ON 버튼과 번갈아 눌러 A/B 로 비교하면 질이 막 공유 E 를 따라가는지가 또렷하다(고질 색이 우측으로 퍼짐 vs 좌측에 갇힘).',
        params: qcplArena(0), seed: seedQCouple, run: 20,
        note: 'couple advection OFF(대조) — q 좌측 stranded, 우 q 0.00. ON 버튼과 번갈아 보라.' }
    ],
    ...
    legend:
      ...
      '<b>q advection — 질이 모든 E 흐름에 동승 (에너지 질 arc · SPINE 여섯째 축 *수송* 짝 — 이번 step 으로 완성)</b>: 0049 가 질 축 <code>q</code>∈[0,1] + 둘째 법칙 강등(<b>sink</b>)을, 0050 이 별 질 생산(<b>source</b>)을 깔았다. 그러나 질은 처음엔 <b>셀 고정</b>이라 E 가 움직여도 q 는 제자리(질이 떠난 자리에 stranded)였다. 그래서 질을 E 흐름에 <b>동승</b>시켰다 — 받는 칸이 <i>흐른 E 의 질</i>을 <b>질량가중 혼합</b>으로 싣는다(donor q 불변·intensive): <b>0051 <code>gravity</code>(하향)</b> 하강 고질 E 가 제 열을 데리고 내려감=침강 hot plume → <b>0055 <code>diffuse</code>(등방)</b> 고질이 사방으로 번지며 q 동행 → <b>0056 <code>couple</code>(막 공유·이번 step)</b> kin 막 안에서 E 가 균등화되며 q 도 균질화. 이로써 q 가 <b>모든 E 이동(하향+등방+막 공유)을 따라가는 완전한 물리장</b>이 됐다. <code>q</code> 는 E 에 올라탄 intensive 속성(단일 척추·새 필드 0)·국소(제 이웃 한 쌍)·혼합은 <b>E 미접촉</b>(장부 불변 — 이미 옮긴 E 의 <i>질</i>만 따라감, q 는 비율). <b>0048 수동 막 E-공유의 질 짝.</b> "couple advection" 켜고 보라(위 "couple advection ON" 버튼). <code>verify advect</code>(D=1·8×8 kin 블록[좌 고질·우 저질]·couple): <b>OFF 면 couple 이 E 를 좌→우 균등화해도 우측 q stranded(우 q 0.00) → ON 이면 q 가 E 따라 우측으로 advect(우 q 0.82·막 안 q 균질)</b>. <b>kQCouple=0 → 혼합 미진입 → 직전 step(0055) 비트 동일</b>(회귀 0). 우측 흑체 색에서 막 공유로 흐른 E 가 고질 색을 데리고 번진다 — <b>이 질 수송이 렌더 L-Q(흑체 색온도)를 그린다</b>(RENDER §8).<br>' +
  ...
  else global.HWS_PANEL_0056 = panel;
