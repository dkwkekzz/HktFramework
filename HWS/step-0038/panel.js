/* step-0038 패널 — 별 죽음·일몰사(VOXEL.md V5+ — 0036 하강 + 0013 FSM ash 연계). 직전 step-0037(태양빛 비) 패널을 잇되 새 노브 행 1개(별 일몰사 kStarSet)를 더했다.
 * 이 step 은 ignite 법칙을 *제자리 확장*한다(LAW_ORDER 무변경, 0035·0036 위에 얹음): 0036 은 진 별이 z=0 서 *계속 탔다*(정직한 한계). kStarSet>0 이면 떠올랐다(st.rose) 다시 지는(dying) 별이 z=0(지평선)에 닿으면 *꺼진다*(FSM off=즉시 제거·on=ash). 연료 남아도 *짐* 자체가 죽음.
 * 완성하는 그림: 별이 z=0(R 핵)서 나(0035)→천장까지 뜨고(0035)→연료 쇠해 도로 지고(0036)→지평선에 닿아 *꺼지고*(0038 死)→빈 자리에 R 핵서 *다음 별*이 난다 → 활성도 연속축이 *닫힌 궤적*(出沒生死)으로 — "구동이 세계서 나고 진다"가 *세대 순환*으로.
 * 회귀(이중 가드): kStarSet=0 이면 죽음·st.rose 미진입(0037 비트 동일) / D=1·rise off 면 st.z 가 1 못 됨 → st.rose 안 켜짐 → 죽음 미발생(2D 레거시 불변). 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0038
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 별 일몰사는 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — 별 일몰사 아레나: verify.js deathArena()/fallArena()/seedSunCore() 와 *같은* 설정.
   *   기본 패널은 회귀 상태(D=1·일몰사 off)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나(D=8 voxel·z=0 R 핵서 단일 별 점화·부력 상승·하강 on)에 *일몰사*를 켜고/끄고 두 프리셋으로 A/B 대조한다 — 화면이 보여주는 것 =
   *   verify 가 단언하는 수치(별 출생/사망 누적)가 되게. 일몰사 ON 이면 진 별이 꺼지고 R 핵서 다음 별이 난다(세대 순환). */
  function deathArena(kSet) {
    return {
      ...
      kIgnite: 1, kStarRise: 1, kStarFall: 1, starFallThresh: 0.5, kStarSet: kSet, starRate: 0.06, starFuel0: 500, ignThresh: 1.5, starCap: 1, starGap: 6, starR: 2, starDriftPeriod: 20,
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0038 — <span style="color:#ffd060">별 죽음·일몰사</span>: 떴다 진 별이 지평선에 닿으면 꺼지고 다음 별이 난다 = 出沒生死 순환(VOXEL.md V5+)',
    subtitle: 'step-0036 은 연료 쇠한 별이 도로 가라앉게(하강) 했으나, 진 별이 z=0 서 <b>계속 탔다</b>(정직한 한계). 이 step 은 그 끝을 닫는다 — 떠올랐다 다시 지는 별이 <b>바닥(지평선 z=0)에 닿으면 꺼진다</b>(일몰사). 이 step 은 <code>ignite</code> 법칙을 제자리 확장한다(LAW_ORDER 무변경, 0035·0036 위에 얹음): <b>kStarSet</b> 을 켜면 떠올랐다(<code>st.rose</code>) 다시 지는(dying) 별이 z=0 닿을 때 꺼지고(FSM off=즉시 제거·on=ash), <b>빈 자리에 R 핵서 *다음 별*이 난다</b>(연료 남아도 *짐* 자체가 죽음). 完成: 별이 z=0(R 핵)서 나→천장까지 뜨고→연료 쇠해 도로 지고→지평선 닿아 <b>꺼지고</b>→다음 별이 난다 → <b>활성도 연속축(SPINE 결정2·3)이 *닫힌 궤적*(出沒生死)</b>으로 — 소산 극단(별)이 저장 바닥(z=0)에 안착 = 소산 정체성 상실 = 死(국소 z 문턱). <b>kStarSet=0 이면 죽음·st.rose 미진입(0037 비트 동일) / D=1·rise off 면 st.z 가 1 못 됨 → st.rose 안 켜짐 → 죽음 미발생</b>(회귀 0·이중 가드). 검증(중력 off 격리): <b>일몰사 off 면 진 별이 z=0 서 계속 타 200t 안 안 죽음(출생 1·사망 0) → on 이면 지며 꺼지고 다음 별 점화(출생 2·사망 1)</b>·잔차 1.1e-15·결정론. <code>node step-0038/verify.js</code> 로 reg·death·conserve·det. (표준 스택은 step-0027 덴드라이트 그대로 — 별 일몰사와 직교.)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: true, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 이 더한 별 일몰사를 한 클릭으로 보이는 설정(verify death 와 동일 아레나)으로. 둘 다 별 상승·하강 on(별이 떴다 진다) — *일몰사*만 토글. */
    presets: [
      { label: '🌇 별 일몰사 ON (出沒生死·D=8)',
        title: 'verify death(on) 과 같은 설정: D=8 격리 아레나·z=0 R 핵서 단일 별 점화·kStarRise=1·kStarFall=1·*kStarSet=1*, 200 tick 미리 진행. 별이 떴다 지며(0036) 지평선(z=0) 닿으면 꺼지고, 빈 자리에 R 핵서 *다음 별*이 난다. 통계 "별 출생/사망" 이 2/1 이면 세대 순환 = verify 가 단언하는 수치. (3D 뷰는 아직 z=0 평면만 그려 z 궤적 자체는 안 보임 — voxel 렌즈 L-V1 백로그. 수치가 증거.)',
        params: deathArena(1), seed: seedSunCore, run: 200,
        note: '별 일몰사 ON — "별 출생/사망"=2/1(지며 꺼지고 다음 별이 남=세대 순환). OFF 버튼과 번갈아 보라.' },
      { label: '☀ 일몰사 OFF (=0036·안 죽음·대조)',
        title: 'verify death(off) 과 같은 설정: 같은 아레나·핵·별 상승·하강이지만 kStarSet=0(=0036 별 일생). 진 별이 z=0 서 *계속 타* 200t 안 안 죽는다(출생 1·사망 0). ON 버튼과 번갈아 눌러 A/B 로 비교하면 일몰사가 더한 세대 순환이 또렷하다.',
        params: deathArena(0), seed: seedSunCore, run: 200,
        note: '일몰사 OFF(대조·=0036) — "별 출생/사망"=1/0(한 별이 안 죽고 계속 탐). ON 버튼과 번갈아 눌러 出沒生死를 보라.' }
    ],
    ...
    stats: [
      ...
      { label: '<span style="color:#ffd060">별 세대</span> (출생 / 사망 / 현재 — 가설: 出沒生死)', get: function (c) {
          var D = c.sim.p.D || 1, setOn = c.sim.p.kStarSet !== 0;
          var b = c.sim.starBirths || 0, d = c.sim.starDeaths || 0, n = (c.sim.stars || []).length;
          var fz = (c.sim.stars && c.sim.stars.length) ? (c.sim.stars[0].z || 0) : '—';
          if (D < 2) return { text: '출생 ' + b + ' / 사망 ' + d + ' / 현재 ' + n + ' (z=0 평면 D=1 — 일몰사 비활성. 위 "데모" 버튼으로 D=8)', cls: '' };
          return { text: '출생 ' + b + ' / 사망 ' + d + ' / 현재 ' + n + ' · 별 z=' + fz, cls: (setOn && d >= 1 && b >= 2) ? 'pass' : '' };
      } },
    ...
    legend:
      ...
      '<b>별 죽음·일몰사(VOXEL.md V5+)</b>: step-0036 은 연료 쇠한 별이 도로 지게(하강) 했으나 진 별이 z=0 서 <b>계속 탔다</b>(정직한 한계). 이 step 은 그 끝을 닫는다 — 떠올랐다 다시 지는 별이 <b>바닥(지평선 z=0)에 닿으면 꺼진다</b>(일몰사). <code>ignite</code> 제자리 확장(0035·0036 위에 얹음): <b>kStarSet</b> 켜면 떴다 진 별이 z=0 닿을 때 꺼지고(연료 남아도 *짐* 자체가 죽음), <b>빈 자리에 R 핵서 다음 별이 난다</b> → 활성도 축이 *닫힌 궤적*(born→천장→set→死→다음). "터 깊이 D" 8·"별 부력 상승"·"별 하강"·"별 일몰사" 켜고 리셋해 보라(위 "별 일몰사 ON" 버튼). <code>verify death</code>: <b>일몰사 off 면 진 별이 z=0 서 계속 타 안 죽음(출생 1·사망 0)·on 이면 지며 꺼지고 다음 별 점화(출생 2·사망 1)</b>·잔차 1.1e-15. "구동이 세계서 나고 진다"가 <b>세대 순환(出沒生死)</b>으로 — 소산 극단이 저장 바닥에 안착 = 死(SPINE 결정2·3). (3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더는 렌더러 트랙 L-V1.)<br>' +
  ...
  else global.HWS_PANEL_0038 = panel;
