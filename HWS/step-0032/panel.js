/* step-0032 패널 — 지지 침착(VOXEL.md V4: E→R 결정화를 지지 있는 칸으로 게이트. 공중 바위 차단·지면 바닥부터 쌓임 = 지면 원형). 직전 step-0031(중력 구배) 패널을 잇되 새 노브 행 1개(지지 침착 kSupport)를 더했다.
 * 이 step 은 법칙을 더하지 않는다 — crystallize 의 E→R 침착에 *지지 게이트*를 더한다(게이트 노브 1 + 문턱 1). kSupport>0 이면 결정화가 W×H×D 전 평면으로 일반화되되 *지지 있는 칸*(z=0 바닥 또는 아래 R≥문턱)에서만 굳어 공중 바위(부유 R)를 막고 지면이 바닥부터 쌓인다. V3 중력(E→바다)과 짝지어 R 의 중력(지면).
 * kSupport=0 또는 D=1 이면 게이트 무효(z=0 평면) = 직전 step 비트 동일(회귀). 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0032
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 지지 침착은 그 동역학과 직교.) */
(function (global) {
  'use strict';
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0032 — <span style="color:#c8a060">지지 침착</span>: E→R 침착을 지지 있는 칸으로 게이트 → 지면이 바닥부터 쌓인다 = 지면 원형(VOXEL.md V4)',
    subtitle: 'V3(step-0031)의 중력으로 <i>E</i> 는 바닥에 고인다(바다) — 그러나 <i>물질 R</i> 은 아직 공중에 떠 굳을 수 있다(결정화가 z 평면 무관 침착). 이 step(V4)은 <code>crystallize</code> 의 E→R 침착에 <b>지지 게이트</b>를 더한다(법칙 신설 아님): <b>kSupport</b> 를 켜면 결정화가 W×H×D 전 평면으로 일반화되되 <b>지지 있는 칸에서만</b> 굳는다 — <b>z=0 바닥(항상 지지) 또는 바로 아래 칸 R≥supportThresh</b>(로컬 z−1 한 칸). 그러면 <b>공중에 뜬 바위(부유 R)가 안 생기고 지면이 바닥부터 쌓인다</b>. V3 중력이 바닥에 모은 E 가 굳어 — <i>R 의 중력(지면)</i>을 <i>E 의 중력(바다, V3)</i>과 짝짓는다. <b>kSupport=0 또는 D=1 이면 게이트 무효(z=0 평면) = 직전 step 비트 동일</b>(회귀 0). 검증: <b>천장 E 슬랩 + 중력 → 무게이트면 공중 바위 12288칸·게이트면 부유 R 0칸·R 이 바닥(z=0)부터</b>·잔차 1e-15·결정론. <code>node step-0032/verify.js</code> 로 reg·ground·conserve·det. R 차폐(아래 고체면 옆으로)·부유 R 붕괴·다른 침착 게이트는 V5+ 로 전가. (표준 스택은 step-0027 덴드라이트 그대로 — 지지 침착과 직교.)',
  ...
  else global.HWS_PANEL_0032 = panel;
