...
(function (global) {
  ...
  var DEFAULTS = {
    ...
    /* ── step-0045: 에너지의 질(자유에너지·둘째 법칙 — SPINE 여섯째 축) ── */
  ...
  /* ②' 에너지 질 강등(step-0045 V5+ — SPINE 여섯째 축: 자유에너지·둘째 법칙) — 자유 E 의 일부가 열 에너지(Eth)로 *일방 강등*. evaporate(E→T 세계 밖 손실)의 짝: 이쪽은 E→Eth(세계 안 저질 잔류).
   *   양은 보존돼 소비되지 않고 *질*이 소비된다(슈뢰딩거·프리고진) — 자유 E(고질, 일할 수 있음)가 매 tick kQual 비율씩 열 에너지(Eth, 저질)로 식는다. 쌍 거래(E[i]-=d·Eth[i]+=d) → 총 E+Eth 불변(닫힌 장부)·비가역(Eth→E 안 됨, 시간의 화살표). 자유 E 생산은 source/별 주입(고질 유입) → 질 구배 q=E/(E+Eth) 창발(주입 근처 고질·먼/고인 곳 저질).
   *   회귀: kQual=0 → early-return = 직전 step(0044) 비트 동일(Eth 불변 0·qualInit 미설정 → hashState Eth skip → 골든 무관). 전 셀(E.length, 3D 상위 평면 포함) — D=1 이면 E.length=W·H(2D 등가). 위치 무관·장부 무관(Eth 가 순잔액 — R/weathered 와 같은 경계, ledger 가 sumEth 를 가법). */
