...
**시각 구분(자동)**: 엔진이 프리셋 행을 `▶ 이 step 보기:` 라벨이 붙은 **호박색 박스**(`.presets` CSS)로 그린다 — 회색 유틸/액션 버튼(재생·리셋·격리 시드 등)과 한눈에 갈린다. 그래서 "이 step 을 보려면 뭘 눌러야 하는가"가 즉시 보인다. 패널 작성자는 스타일을 신경 쓸 필요 없이 `presets` 만 선언하면 된다.

**검증 우선 레이아웃(자동)**: `presets` 가 있는 패널은 누적 노브·액션(35 step 치 전체 스택)을 **`고급 노브 ▾` 토글 뒤로 접는다**(기본 접힘 `.advanced`). 기본 화면엔 *재생/리셋 + 호박색 프리셋 박스 + 통계*만 남아 "확인할 건 프리셋 하나"가 분명해진다. 전체 스택은 탐험용이므로 토글로 펼친다(샌드박스 보존). `presets` 없는 옛 패널은 노브가 인라인 그대로(하위호환).
