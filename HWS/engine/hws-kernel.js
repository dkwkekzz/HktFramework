...
(function (global) {
  ...
  function ledger(sim) {
    ...
    /* step-0045: 열 에너지 Eth 가법 — degrade 가 E→Eth 쌍 거래라 sumE+sumEth 보존(kQual=0 이면 Eth=0 → lhs·잔차 직전 step 과 동일). */
  ...
  function hashState(sim) {
    ...
    feed(sim.E.buffer);
    feed(sim.R.buffer);
    /* 열 에너지 Eth(step-0045) — *가법*: 질 강등이 활성(qualInit, kQual!=0)일 때만 먹인다(kQual=0 이면 false → skip → 과거 골든 전부 불변). 저질 에너지 분포(둘째 법칙 강등 결과)가 결정론·재현에 들어간다(같은 시드 2회 Eth 비트 동일). */
