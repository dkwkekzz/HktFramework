...
(function (global) {
  ...
  function createSim(seed, params) {
    ...
    return {
      ...
      Eth: new Float64Array(N),                // step-0047: 열 에너지(저질 — 둘째 법칙으로 자유 E 가 강등된 자리). 초기 0 → kQual=0·kLifeHeat=0 이면 영원히 0(회귀). 질 q=E/(E+Eth).
      ...
      qualInit: false,     // step-0047: 에너지 질 활성 여부 — 해시 가법 가드(false 면 Eth 해시 skip → 과거 골든 불변)
      thermalized: 0,      // step-0047: 누적 열화량(통계 — Eth 가 순잔액이라 장부 무관, R/weathered 와 같은 경계)
