/* step-0029 패널 — 격자 z-일반화(VOXEL.md V1: 시뮬 공간을 W×H×D voxel 로). 직전 step-0027(덴드라이트) 패널을 잇되 새 노브 행 1개(터 깊이 D)를 더했다.
 * 이 step 은 *법칙*을 안 더한다 — 터(Ground)의 차원을 일반화한다(D=1 → 기존 2D 와 비트 동일; D>1 voxel 상자, 단 V1 은 격자만 — z 확산은 V2·중력은 V3, 상위 평면은 불활성).
 * D 는 *리셋(재시작) 시 적용*되는 구조 노브다(W·H 처럼). 3D 뷰는 아직 z=0 평면 하이트필드 — voxel 렌더(L-V1)는 렌더러 트랙(RENDER-STATE.md). 브라우저: window.HWS_PANEL_0029
...
(function (global) {
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0029 — <span style="color:#9ad24a">격자 z-일반화</span>: 시뮬 공간을 W×H×D voxel 로(VOXEL.md V1)',
    subtitle: '지금까지 세계는 납작한 2D 필드였고 높이는 렌더러가 물질 R 에서 <i>발명한</i> 착시였다(2.5D). 지면 위 바다·절벽·동굴이 <i>스스로 굴러가는</i> 세계는 납작한 판으로 표현이 불가능하다 — 그래서 공간을 <b>voxel(벽돌) 3D 상자</b>로 일반화한다. 이 step(V1)은 <b>격자만</b> 일반화한다: 깊이 <b>D</b> 노브, 인덱스 <code>(z·H+y)·W+x</code>, z 는 벽(클램프)·x·y 는 토러스, 커널에 3D 헬퍼(<code>discCells3·ballOffsets·tumbleHash3</code>) 가법. <b>법칙 산술은 무수정</b> — <b>D=1 이면 기존 2D 와 비트 동일</b>(z 이웃이 없어 z 항이 산술로 0 = 회귀 0). D&gt;1 의 z 확산은 V2·중력(바다)은 V3·지지 침착(지면)은 V4 로 전가. 검증: <b>D=4 의 z=0 평면이 D=1 전체 세계와 비트 동일</b>(maxDiff=0)·결정론·닫힌 장부. <code>node step-0029/verify.js</code> 로 회귀·voxel·conserve·det 검증. (표준 스택은 step-0027 덴드라이트 그대로 — z=0 평면에서 돈다.)',
    ...
    controls: [
      ...
      { items: [
        { kind: 'slider', id: 'D', label: '터 깊이 D (voxel)', param: 'D', min: 1, max: 8, step: 1, def: 1, fixed: 0, title: 'step-0029 V1: 시뮬 공간 깊이(z). 1 = 기존 2D(비트 동일). >1 = voxel 상자(W×H×D). *리셋 시 적용*되는 구조 노브. V1 은 격자만 일반화 — z 확산(V2)·중력(V3) 전까지 상위 평면은 불활성(초기 noise 보존). 3D 뷰는 아직 z=0 평면 하이트필드(voxel 렌더는 렌더러 트랙).' }
      ]},
      { items: [
