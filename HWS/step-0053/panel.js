/* step-0053 패널 — 질-의존 대사(kQMetab): 고질 E 가 더 영양 — 슈뢰딩거 낙차의 *에너지론* 완성. 직전 step-0052(질-구배 주화성) 패널을 잇되 metabolize(흡수)에 질 가중을 더하고 노브 kQMetab 1개를 더했다.
 ...
 * 척추: q 는 E 에 올라탄 intensive 상태변수(단일 척추) · 국소(제 disc 칸 E·q) · 흡수는 E→m 쌍 거래로 보존(닫힌 장부 — q 는 *읽기만*[미수정·A·강등·주화성과 같은 읽기 경계]). 세계 척도에선 생명이 고질 E 를 우선 소비 → 장의 평균 질이 떨어짐(고질을 먹어 질 낮춤).
 * 회귀(이중 가드): kQMetab=0 → take = E·kL 바이트 동일(균일 흡수·q 미참조). qInit=false(degrade off)면 미진입(질 축 없는 세계엔 질 대사 없음). 브라우저: window.HWS_PANEL_0053
 * (아래 표준 스택·통계는 step-0027 덴드라이트 그대로 — 질-의존 대사는 그 동역학과 직교[E→m 보존]. 이 step 이 생명을 *질로* 먹인다.) */
(function (global) {
  'use strict';
  ...
  /* ── 데모(프리셋) — 질-의존 대사 아레나: verify.js qmetArena()/seedQMetab() 와 *같은* 설정.
   *   기본 패널은 회귀 상태(kQMetab=0)로 열려 직전 step 과 화면이 같다 → "뭘 더했는지" 안 보임.
   *   이 아레나(2D·평탄 E=2·고질[0.9]/저질[0.1] 두 구역·정착 생명 두 무리[move off]·degrade 질 축 alive)에 *질-의존 대사(kQMetab)* 만 켜고/끄고 두 프리셋으로 A/B 대조한다 —
   *   화면이 보여주는 것 = verify 가 단언하는 수치(같은 E 인데 OFF 면 두 무리 m 동일 / ON 이면 고질 위 무리가 더 많이 흡수해 m↑). 흑체 색(L-Q 미리보기)으로 보면 고질(밝은) 구역 생명이 더 굵게 자란다. */
  function qmetArena(kqm) {
    return {
      ...
      kDegrade: 0.01, qInit0: 0, kStarQual: 0, kQAdvect: 0, kQTaxis: 0, kQMetab: kqm,
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0053 — <span style="color:#ffd060">질-의존 대사 (kQMetab)</span>: 고질 E 가 더 영양 — 생명이 질을 먹는다(슈뢰딩거 낙차의 에너지론, SPINE 여섯째 축)',
    subtitle: '0049 강등·0050 별 생산·0051 수송이 질 축 <code>q</code>∈[0,1] 을 깔고, 0052 가 생명을 질 따라 <b>모이게</b>(주화성) 했다. 그러나 생명은 아직 모든 E 를 <b>같은 효율로 먹는다</b>(고질이든 저질이든 흡수 동일 — 질을 쫓되 먹는 방식은 양 기반). 이 step 은 생명이 질을 <b>먹게</b> 한다: <code>metabolize</code>(⑦)가 disc 칸에서 E→m 흡수할 때 take 를 <b>질 가중</b> take = E·kL·(1 + kQMetab·q) 로 → 고질(고 q = 엑서지 높은) E 를 더 많이·빨리 빨아들인다(슈뢰딩거: 생명은 <i>자유에너지</i>[저엔트로피 질]를 먹어 제 질서를 유지한다 — 양이 아니라 질을 먹는다). <b>0052 주화성(질 따라 모임)의 에너지론 짝</b>(질 따라 먹음). 척추: <code>q</code> 는 E 에 올라탄 intensive 상태변수(단일 척추) · 국소(제 disc 칸 E·q) · 흡수는 E→m 쌍 거래로 보존(닫힌 장부 — q 는 <b>읽기만</b>[미수정]). 검증(D=1·평탄 E=2·고질[q 0.9]/저질[q 0.1] 두 구역·정착 생명 두 무리, 12 tick): <b>kQMetab OFF 면 같은 E 라 두 무리 m 동일(1.19) → ON 이면 고질 무리가 더 많이 흡수(mHigh 1.96 > mLow 1.40)</b> = 생명이 질을 먹는다. <b>kQMetab=0 → take=E·kL 바이트 동일 → 직전 step(0052) 비트 동일</b>(회귀 0·이중 가드: qInit=false 면도 미진입). <code>node step-0053/verify.js</code> 로 reg·nourish·conserve·det. 우측 흑체 색(L-Q 미리보기)으로 보면 고질(밝은) 구역 생명이 더 굵게 자란다. (표준 스택은 step-0027 덴드라이트 그대로 — 질-의존 대사는 그와 직교[E→m 보존].)',
    overlays: { sourceSink: false, pools: true, life: true, centroid: true, sparkline: true },
    poolOpts: { minE: 1.5, prom: 0.3 },

    /* 데모 프리셋 — 이 step 의 질-의존 대사를 한 클릭으로 보이는 설정(verify nourish 와 동일 아레나)으로. 둘 다 평탄 E·고질/저질 두 구역·정착 생명 두 무리·degrade — *질-의존 대사(kQMetab)*만 토글. seed 로 두 q 구역 + 두 무리를 심는다. */
    presets: [
      { label: '🍖 질-의존 대사 ON (고질 무리가 더 굵게 자람)',
        title: 'verify nourish(on) 과 같은 설정: D=1·평탄 E=2·고질(q 0.9)/저질(q 0.1) 두 구역·정착 생명 두 무리(고질 위·저질 위·move off)·degrade 0.01·*kQMetab=5*, 12 tick 진행. 같은 E 인데 고질 위 무리가 더 많이·빨리 흡수해 생물량 m 이 크게 자란다 → mHigh 1.96 > mLow 1.40(저질 무리) = 생명이 *질*을 먹는다(자유에너지로 산다·슈뢰딩거 낙차). 우측 흑체 색에서 고질(밝은) 구역 생명이 더 굵게 자라면 켜진 것 = verify 가 단언하는 수치.',
        params: qmetArena(5), seed: seedQMetab, run: 12,
        note: '질-의존 대사 ON — 고질 무리 m 1.96 > 저질 무리 1.40. OFF 버튼과 번갈아 보라.' },
      { label: '🥩 질-의존 대사 OFF (두 무리 같음·대조)',
        title: 'verify nourish(off) 과 같은 설정: 같은 아레나지만 kQMetab=0(균일 흡수). 같은 E=2 라 고질/저질 두 무리가 *똑같이* 먹어 m 동일(둘 다 1.19) = 직전 step(0052) 비트 동일. ON 버튼과 번갈아 눌러 A/B 로 비교하면 생명이 질을 먹는지가 또렷하다(고질 무리만 굵어짐 vs 둘 다 같음).',
        params: qmetArena(0), seed: seedQMetab, run: 12,
        note: '질-의존 대사 OFF(대조) — 같은 E 라 두 무리 m 동일(1.19). ON 버튼과 번갈아 보라.' }
    ],
  ...
  else global.HWS_PANEL_0053 = panel;
