...
**`∇E` → 파생 바람 flowmap** — 흐름량 E 의 공간 기울기(`∇E`)를 읽어 *바람·해류 방향장*으로 그린다. 흐르는 에너지가 어디로 향하는지(벡터필드/flowmap UV)를 표면 위 흐름선·이류 입자로 시각화 — 발광(세기)에 더해 *방향*을 보인다. 코어 불변(∇E 는 GPU 파생, 시뮬 안 건드림)·분포 author 0 — 도함수 읽기만(RENDER §5 "파생: 바람·해류").
...
| 마커 | 격차 | 설계 근거 |
|---|---|---|
| 🔴 | `∇E` 파생 바람 flowmap 미구현 — 발광은 세기만, *방향*(흐름선/이류 입자) 없음 (다음 렌즈) | RENDER §5 |
...
| ✅ | ~~물 렌즈 = R 위 반투명 막~~ → 바닥 흡광 블렌드 + 프레넬 + 시선 글린트(고체 vWet=0 불변); ~~z=R+저활성E~~ → `matH=hOf(R)` only | RENDER §2·§5 |
