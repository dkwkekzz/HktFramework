...
**`G` → 절차적 색**(해시) — 유전형 태그(`G`)를 고정 4색 팔레트(`geneCol`/`storeCol`/`geneColP`)에서 *해시 기반 절차적 색*으로 교체. 자기복제로 혈통이 무한 분화해도 author 0 으로 색이 갈린다 — 지금은 4혈통에서 캡되어 한 색으로 뭉갠다(§3 격차·RENDER §4 "G→색은 절차적이어야"). 같은 해시를 3곳(지형 progT·세계 progW·생명 progP)에 일관 적용해 2D 점 색과도 맞춘다.
...
| 마커 | 격차 | 설계 근거 |
|---|---|---|
| 🔴 | `geneCol` 고정 팔레트 4혈통 캡 → 자기복제로 굴러간 혈통이 한 색으로 뭉갬. 절차적 해시 필요 (다음 렌즈) | RENDER §4 |
| 🟡 | FSM 이산 재질(kindling/burning/ash)을 텍스처 채널에 안 실음 — A 발광이 대신 비춤 | RENDER §5 |
| ⬜ | `∇E` 파생 바람·`∇²E` 안개 미구현 | RENDER §5 |
| ✅ | ~~고체 거칠기 = `∇R` 노멀 디테일 미구현~~ → R 라플라시안으로 고체 노멀 변조(진폭=∇R·방향=셀 해시), 높이 불변 | RENDER §5 |
...
| 4 | **`G` → 절차적 색**(해시 — 혈통 무한 분화, 팔레트 캡 제거) | ⬜ NEXT |
| 5 | **`∇E` → 파생 바람** flowmap | ⬜ |
