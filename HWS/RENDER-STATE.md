...
- ✅ **z = 물질(R) only**: `matH = hOf(R)` — 에너지(흐르든 고이든)는 z 기여 0. 흐르는 E=발광·고인 E(물)=*재질*(파랑·투과·광택)로만 읽고 *안 솟는다*(RENDER §2·§5). 법선도 R 기복만. 분포 재성형 0(물은 z 서 빠지되 분포 그대로 — author 아님).
...
**물 = R 위 반투명 막** — 물(저활성 E) 셀에서 *바닥 R 색을 깊이(E)로 흡광 블렌드 + 프레넬 반사*. 지금은 물이 z 0 까지 왔으나 아직 *불투명 표면색*(water mix)일 뿐 — 바닥 R 비침·프레넬 미구현(§3 최우선 격차). 깊이=E 양으로 흡광(얕으면 청록·깊으면 짙은 남), 바닥 R 투과(고체는 불투명). 분포 author 0 — 색·투과·반사만 더한다(RENDER §5 "물" 렌즈).
...
| 마커 | 격차 | 설계 근거 |
|---|---|---|
| 🔴 | 물 렌즈 = R 위 반투명 막(바닥 R 투과·깊이 흡광·프레넬) 미구현 — 현재 z 0 + 색·광택 근사뿐, 바닥 안 비침 | RENDER §5 |
...
| 2 | **물 = R 위 반투명 막** — E→깊이·투과·색, 바닥 R 비침 | ⬜ NEXT |
| 3 | **고체 거칠기 = `∇R` 노멀 디테일** | ⬜ |
