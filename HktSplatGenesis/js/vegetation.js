...
(function (global) {
	...
	function treeSplats(cand, life, arr) {
		...
		for (let i = 0; i < 3; i++) {
			...
			pushSplat(arr, cand.x, ty, cand.z, life.trunk, 0.95, trunkR, h * 0.14, trunkR);
		}
		// 수관 — 둥근 군집(잎색1↔잎색2 램프로 명암 변주), 위로 갈수록 밝게
		const n = 11;
		for (let i = 0; i < n; i++) {
			...
			const rgb = [mix(life.leaf[0], life.leaf2[0], t), mix(life.leaf[1], life.leaf2[1], t), mix(life.leaf[2], life.leaf2[2], t)];
	...
	function rockSplats(cand, life, arr) {
		...
		for (let i = 0; i < n; i++) {
			...
			const shade = 0.8 + jit(k, 30 + i, 0.18);
	...
	function splatsFor(cands, life) {
		const arr = [];
		for (const c of cands) {
			if (c.kind === 'tree') treeSplats(c, life, arr);
			else if (c.kind === 'rock') rockSplats(c, life, arr);
		}
		return arr;
	}
	...
	function bakePanorama(world, extent, cx, cz, cfg) {
		...
		return { ply: plyFromSplats(splatsFor(inWin, life)), count: inWin.length,
	...
	function bakeTile(world, x0, z0, size, cfg) {
		...
		return plyFromSplats(splatsFor(cands, life));
	}
