...
| `node world-scatter-shot.js [pan.png] [fire.png] [seed=7]` | T4 스캐터·개체 스트리밍 — ⓪ 결정론 스폰(원점 다른 두 창 겹침 위치 diff 0, 순수 Node) + ① 카메라 +x 직진 중 슬롯 증분 교체(본 스폰 합집합>최대 활성)·활성 슬롯 상한 유지 + ② 불×나무 임의 좌표(원점 ≈119u) 스트리밍 쌍의 연소 상호작용(나무 가열 vs 불 없는 대조군 ~0) 판정 + 사진 |
| `node world-water-shot.js [lake.png] [fog.png] [seed=7]` | T5 물 + 원거리 폴리시 — ⓪ 수면 타일 재생성 diff 0(순수 Node) + ① 호수 파노라마(수면 메시>0·수면 픽셀·하늘밴드 μ가 공용 sky 톤과 근접) + ② 생명 원거리 fog on/off(on 이 sky 색에 더 가까움 = 무대와 같은 톤으로 소실, 근경 초록→원경 sky색 그라데이션) 판정 + 사진 |
| `node openworld-shot.js [out.png] [seed=7]` | T 트랙 통합 — 실제 index.html 에서 "오픈월드" 버튼을 눌러 T2(청크 지형)+T5(수면·sky/fog)+T4(스트리밍 나무)를 한 화면에 띄우고 무대(Spark)+생명(WebGPU) 합성. 판정: 지형 타일 ≥9 + 수면 메시·픽셀 + 나무(생명) + 하늘톤 공용 = 네 요소가 한 프레임에 + 사진 |
