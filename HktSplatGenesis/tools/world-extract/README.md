...
- **검증 부재 주의**: W2 스타일 프로파일/검증기는 미구현이라 아래 게놈은 기계 검증을 거치지 않았다.
  W2 완료 시 `validate(genome)` 를 통과해야 확정으로 친다.
...
- **W2**: 스타일 프로파일 + `validate(genome)` — 추출 결과가 "한 게임의 월드" 울타리 안인지 판정.
- **W4 자동화**: 이미지 파일 → vision API → 게놈 JSON (프로파일 제약 프롬프트).
