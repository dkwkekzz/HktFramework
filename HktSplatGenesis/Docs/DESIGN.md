...
- `js/scatter.js` — T4 개체 스트리밍: `world.scatter` 스폰을 카메라 거리순으로 엔진 8 슬롯에
  `setEntitySlot`(증분 교체)로 채운다. kind→프리셋 매핑만(절대 원칙 2). `?scatter=` 로 app 배선.
...
| 개체 상한 8 극복 = 슬롯 재활용(증분 교체), 상한 상향 아님 (T4) | N 을 8 슬라이스로 고정하고 `setEntitySlot` 이 슬롯 하나만 부분 업로드로 교체 — 레이아웃(N·슬라이스 수) 불변이라 바이트 일치·정렬 전제가 안 깨진다. 카메라 거리순 상위 8 스폰만 활성, 멀어지면 void(opacity 0 + emitter y=64 = 격자 밖) 로 재활용. 든 스폰은 슬롯 유지 → 재시드 없이 성장 시계 보존. MAX_ENTITIES 상향(버퍼 확장·정렬 영향)은 별도 검토 |
| 스캐터 배치는 좌표·시드 해시만 (T4) | 스폰 격자 셀 인덱스+시드 해시로 존재·종류·지터를 정해 같은 시드=같은 배치, 창 경계에서도 겹침 일치(청크 스트리밍 정합). Math.random 은 개체 *내부* 스플랫 초기화(재시드)에만 — 배치엔 금지. 종류는 새 코드 경로가 아니라 kind→프리셋 매핑(절대 원칙 2) |
| 물은 시각만 — 수면 스플랫 + 심도 색, 시뮬은 지형 바닥 그대로 (T5) | `waterY < 지형` 셀에 반투명 수평 surfel 을 waterY 평면에 얹는다(무대 층, 항등 정합). heightfield 는 지형 바닥 그대로라 생명은 물속에서도 바닥에 충돌 — 부력/헤엄은 별도 규칙(로드맵 밖). 심도(waterY−지형)로 얕은 청록→깊은 남색 |
| fog 는 무대·생명 공용 1색 — CamParams 확장, 기본 off (T5) | 무대는 three fog + clear=fog 색(스카이 톤), 생명은 RENDER FS 가 viewZ 로 fog 색에 mix. 같은 색·거리를 두 층에 주면 원거리 소실 톤이 일치(water-shot: 하늘 톤 Δ0, 생명 원거리 B−R 부호 반전). CamParams 160→176B(+fogColor vec4), fogOn 기본 0 이라 fog 미설정 프레임은 렌더 바이트 불변 — OCC 는 앞 160B 만 읽어 무영향 |
