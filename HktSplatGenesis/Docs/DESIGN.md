...
| 바이옴 = 온·습도 평면의 소프트맥스 경계 보간 (T1) | 바이옴별 relief(진폭·ridged 비중)·팔레트를 가중 혼합하면 경계에서 지형 성격과 색이 *함께* 매끄럽게 바뀐다 — 하드 경계는 이음새를 만든다. 수역은 별도 채널이 아니라 높이 기반 판정(W-Q1 에서 `macroReliefAt<waterY` 저주파 포락으로 정련 — 아래 항) |
...
| 지형/식생 명암은 생성 시점에 f_dc 로 굽는다(런타임 조명 없음) (W-Q4) | 스플랫은 SH 0차(상수색)라 런타임 조명이 없다 — 절차 지형은 무광 평면("점에 색만")으로 보인다(캡처 3DGS 는 사진에 조명이 SH 로 구워져 있어 입체적). 우리는 지형을 *생성*하므로 bake 시점에 명암을 색에 굽는 게 정당·값싸다(런타임 0). `terrain-gen.shadeAt` = reliefAt 유한차분 노멀 → diffuse(N·태양)+ambient, `plyBytes`/`tilePly` 의 f_dc 에 곱한다. 수면은 균일(1) — 평평한 수면에 바닥 요철 명암이 새지 않게. 식생(`vegetation.splatsFor`)도 스폰 자리 지면 명암을 곱해 통합. colorAt/heightAt 은 불변(순수 원본) — 명암은 *렌더용 색*에만. 한계: 물 스페큘러/프레넬은 시점 의존이라 SH0 으로 불가 → 물을 생명(WebGPU) 경로로 옮겨 FS 셰이딩하는 게 정석(후속) |
| 식생 밀도 = Bake 정적 스플랫, 8 상한은 상호작용 전용 (W-Q2) | 사진처럼 초원을 나무로 채우려면 시뮬 개체(`MAX_ENTITIES=8`)로는 불가하나, 8 은 하드웨어 벽이 아니라 스플랫 풀 N 을 8 슬라이스로 등분하는 스킴의 결과(`eid=i/sliceSize`, N 고정·정렬 O(N log²N) → 개체당 스플랫 해상도 예산). **밀도는 상한을 올려서가 아니라 Bake 로 푼다** — 정적 식생 스플랫은 시뮬 풀(8슬라이스·바이토닉)을 안 거치고 지형 PLY 로 Spark 가 그려 개수 제한 사실상 없음. 구조: **전부 Bake 로 채우고 카메라 근처 몇 개만 8 슬롯으로 승격(promote)**. 근거: Bake 하면 렌더가 상태(속도·에너지·열·성장)에서 색·모양을 유도하던 게 얼어붙어 상호작용·창발(불×나무·성장·연소)이 죽는다 → 상호작용하는 것만 시뮬로 남기고 나머지는 무대 씬(절대 원칙 1 의 "무대는 로드/생성" 예외를 원경 식생까지 확장 — 근접 생명엔 원칙 그대로). 게놈 생명 층(`genome.life`)이 Bake·시뮬 두 레이어의 스폰 규칙 단일 원본. v1 은 배양한 나무를 스냅샷해 인스턴싱(그리지 않고 배양 결과 배치) |
| 수몰 판정 = 저주파 macro 포락 < waterY, 물 스플랫은 평평한 수면(y=waterY) (W-Q1) | 예전 판정 `reliefAt(고주파) < waterY` + 물 스플랫을 지형 높이(울퉁불퉁)에 두는 조합은 고립 pit 마다 파란 얼룩(speckle)이 흩뿌려졌다. `isWater = macroReliefAt < waterY`(macro = 옥타브 2·ridged 포함·파장 동일 저주파 relief)로 바꿔 **연결된 분지**만 물로 잡고, bakers(plyBytes·tilePly·waterTilePly)가 물 셀을 평평한 수면 y=waterY 로 굽어 바닥 요철을 잠근다 → 매끄러운 호수. 색 심도는 실제 reliefAt(바닥) 기준(shallow→deep) 유지. `reliefAt` 은 `reliefCore(oct,withRidged,scaleBoost)` 로 리팩터해 macro 와 코어 공유(거동 불변). **함정 1**: macro 에서 ridged 를 빼면 능선 협곡에 고인 물(temperate 시드7 호수)이 안 잡혀 사라진다(world-water 회귀) — ridged 포함 필수. **함정 2**: macro 파장을 늘리면(scaleBoost>1) 다른 노이즈장을 봐 실 분지와 어긋난다 — 파장 동일(scaleBoost 1) 필수. 게놈 waterY 는 base 아래로 충분히 올려야 분지가 찬다(breeze 0.06→0.18). 검증: concept-shot 얼룩→호수, world-water 14250px 무회귀 |
| 승격 훅 = 시뮬로 올라간 나무의 Bake 사본 제외, 좌표계 정합이 전제 (W-Q2c) | "밀도=Bake, 상호작용=시뮬"의 마지막 조각. `ScatterStream` 이 카메라 근처 나무를 8 슬롯 시뮬로 승격하면(상태 유도=성장·연소·바람이 살아남) 그 정적 Bake 사본은 빼야 한다 — 안 그러면 같은 나무가 두 번 그려진다(Bake 블롭 + 시뮬 개체가 겹쳐 z-fighting·이중 밀도). **핵심 함정**: 제외는 승격 스폰 key(`t:u,v`)가 Bake 후보 key 와 *정확히 일치*할 때만 실효 — key 는 셀 인덱스(u,v)라 cell 이 다르면 절대 안 겹쳐 제외가 조용한 no-op 이 된다. 게다가 jitter/maxSlope 가 다르면 같은 셀이라도 샘플 위치(→biome/slope→밀도 게이트)가 갈려 한쪽엔 나무, 다른 쪽엔 없음이 되어 key 가 어긋난다. 그래서 `app.js` 가 stream 과 Bake(`vegetation.bakeTile` 기본값)에 **같은 cell 3.4·maxSlope 2.2·jitter 0.8** 를 준다 — 그러면 승격 key ⊆ Bake key 가 보장(`world-promote.js` ① 이 검증). 배선: `scatter.candidates(cfg.excludeKeys)` 가 key 제외(Set/배열/객체 정규화) · `ScatterStream.promotedKeys()` 가 현재 승격 Set · `vegetation` 이 excludeKeys 를 candidates 로 통과(cfg 스프레드) · `stage.setVegExclusion(keys)` 가 승격 집합 서명이 바뀔 때만 근접 링(0) 식생 타일을 다시 굽는다(async SplatMesh 교체 — bake 는 await 전 동기 제거 + await 후 `tiles.get(key)!==t` 팬 누수 가드) · `app.js` tick 이 매 bake 주기 `stream.promotedKeys()`→`setVegExclusion`(집합 불변이면 즉시 반환=값싼 게이트). v0 하드컷(경계 팝 허용, 후속 크로스페이드). 밀도·종은 게놈 `life`(없으면 기본 폴백)라 두 레이어가 스폰 규칙 단일 원본을 공유. 남김: 링0 전부 재Bake(타일별 diff 로 좁힐 수 있음)·크로스페이드 |
| 하늘 = 카메라 추종 구(BackSide)의 월드 y 방향 그라데이션, 스크린 배경 텍스처 아님 (W6) | 게놈 `mood`(skyTop/skyHorizon)를 하늘로 굽는 두 후보 중 `scene.background=Texture` 는 three 가 뷰포트를 덮도록 aspect **cover** 스케일을 걸어 1×N 세로 그라데이션이 세로로 크롭돼 거의 flat 이 된다(가로 넓은 뷰포트에서 세로 소실). 그래서 큰 구(r500, BackSide, depthWrite=false, renderOrder=-1, frustumCulled=false)를 카메라 위치로 매 프레임 옮겨(무한 원경) 두고, 프래그먼트가 `normalize(worldDir).y` 로 천정↔지평선을 섞는다 — **월드 y 기준이라 실제 지평선과 정합**(카메라 pitch 무관)하고 지평선(dir.y→0)에서 skyHorizon = fog 톤과 만나 무대·생명이 이어진다. mood 에 skyTop/skyHorizon 이 있을 때만 돔 생성 — 없으면(구 sky 필드/무-mood) flat clear 유지(T5 회귀 안전). 셰이더 유니폼은 **linear**(srgbToLinear)로 넣고 `#include <colorspace_fragment>` 가 출력 sRGB 인코딩 — clear 색 관례(위 T5 항)와 동일 원리. 검증: concept-shot 우패널 상단 검정비율 0%·μ=skyHorizon sRGB |
