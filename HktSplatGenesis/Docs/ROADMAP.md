...
- ✅ **T4 — 스캐터·개체 스트리밍**: ① `world.scatter(x0,z0,size)` — 결정론 스폰 테이블(스폰 격자
  셀 해시로 존재·종류, 바이옴·경사·수위 게이팅: 평야/설원 나무·산악/급경사 바위·수변 슬라임,
  같은 시드=같은 배치, Math.random 금지). ② `engine.setEntitySlot(ei, genes)` — setScene 전체
  재초기화 없이 개체 슬롯 하나만 부분 업로드(엔티티 행+슬라이스 스플랫/rest/클러스터)로 교체,
  레이아웃 불변(바이트 일치 유지). ③ `js/scatter.js` — 카메라 반경 스폰을 거리순으로 8 슬롯에
  스트리밍(멀면 void, 가까우면 빈 슬롯, 든 스폰은 유지=성장 시계 보존). `?scatter=<seed>`
  딥링크(app.js — `&tiles=` 로 지형 함께). 검증: `node test/scatter-shot.js` — 결정론+게이팅
