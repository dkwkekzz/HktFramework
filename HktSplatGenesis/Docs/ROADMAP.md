...
C1~C4(형태·채색·부속) 완료, 다음은 C5 추출기. 여기서 단계를 관리한다). 오픈월드 MMORPG 캐릭터를 이미지 몇 장에서 게놈(형태·채색·재질·부속
...
- **C5 — 이미지 → 게놈 추출기 v0**: `tools/genome-extract` + LLM vision + 스타일 프로파일 검증기. 완료 기준: 이미지 2장 → 후보정 ≤5 슬라이더로 3클립 통과.
