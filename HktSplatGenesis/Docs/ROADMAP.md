...
- ✅ **C5 — 이미지 → 게놈 추출기 v0**: `tools/genome-extract` — 컨셉 이미지를 게놈으로 *번역*
  ...
  게놈 JSON 내보내기/불러오기(후보정 그릇). 검증: `node test/genome-extract-shot.js` — 합성
  컨셉 2장(정면·측면) → 추출(키 없으면 mock) → 반려 경로 + 부속 세그 수 + 3클립(walk/idle/wave)
  사진 + 팔레트 색상 번역(초록 밴드 97%) 판정. 후보정 0 슬라이더로 3클립 통과(기준 ≤5).
  남김: 실이미지 대규모 검증·R2 Evaluator 연동은 후속, 장기 거처는 HktGameplayGenerator 이관.
