...
**에디터(editor.html) — 개체별 선택(A1e)**: 살 개체 디테일 패널의 **`입력 상태 머신 사용` 체크박스**로
개체마다 켜고 끈다. 켠 개체는 제 `AnimationController`+`CharacterInput`(제 Skeleton 인스턴스)로 독립
구동, 끈 개체는 장면 공용 클립을 따른다. 개체별 `이동 강도` 슬라이더(→moveMag)·점프/인사 버튼으로
입력을 주입 — 두 캐릭터가 한 장면에서 서로 다른 상태(예: 좌 idle / 우 run)를 동시에 재생한다.
스켈레톤 *정의*(게놈·리그)는 공용이라 세그먼트 순서가 같아 친화 인덱스 호환(애니 개체는 제 컨트롤러의
bind 포즈로 시드). 이는 A4(다개체 상태 머신)의 실질적 선행분 — 개체별 뼈대(E2) 위에 개체별 컨트롤러.
...
- ✅ **A1e — 에디터 개체별 애니 토글**(A5 선행분): 살 개체 디테일에 `입력 상태 머신 사용` 체크박스 —
  켠 개체만 제 컨트롤러로 독립 구동, 끈 개체는 공용 클립. 개체별 이동 강도 슬라이더 + 점프/인사 버튼.
  검증 `test/editor-anim-shot.js`(우 run·좌 idle 분리 사진 + 데이터). 상세는 [ROADMAP.md](ROADMAP.md) A1e.
- **A5 — 에디터 배선 심화 (E 트랙 합류)**: 상태 그래프 편집 UI + 상태별 클립 매핑 패널(FBX 클립 →
  상태 드롭다운). 완료 기준: 에디터에서 그래프·매핑을 편집해 저장/재생.
