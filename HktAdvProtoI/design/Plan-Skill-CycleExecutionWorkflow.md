...
2. **T 는 구현을 보지 않는다.** 검증 시나리오는 01-spec.md 의 검증 절만으로 쓴다
   (§13 — 검증 기준은 코드 구조가 아니다). 그래서 구현과 동시에 시작할 수 있고,
   구현이 시나리오에 맞추는 방향(기획→구현)이 유지된다.
3. **GAP 은 병렬 중에도 지어내지 않는다.** 어느 Agent 든 의미 부족을 만나면 자기
   산출물에 `GAP` 블록(CLAUDE.md 형식)을 남기고 그 부분만 미완으로 종료한다.
   통합 시 build 본체가 GAP 을 모아 plan 또는 Human 으로 반환한다.
4. **Cycle 간 병렬**: 서로 다른 Cycle 이 REUSED Semantic/Rule 을 공유하지 않을 때만
   동시 진행 허용. 공유하면 순차 — 판단 근거는 두 Cycle 의 02-world.md REUSED 목록
   교집합이다. 이 판정은 스킬 규칙 한 줄로 충분하다 (도구를 새로 만들지 않는다 — §10 정신).
