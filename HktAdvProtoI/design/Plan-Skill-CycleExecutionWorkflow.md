...
| 뒤 층 · 뒤 기획서로 (`DESIGN.md` §3 그 원본의 "남은 것") | **spec 제안** (advprotoi-spec) | 그 층·기획서를 자를 때 입력 | 지금 Cycle 이 받을 수 없는 것(다른 층의 의미 · 다른 Region 의 Play). Cycle 마감이 바로 그 원본 아래에 적고, Cycle 이 받으면 "덮인 것" 으로 옮긴다. 기획서의 원문 절 단위면 그 층의 `content/roadmap/L<N>-Handoff.md` 로 옮기고 여기에는 포인터만 (위층 문서 §5 층 규칙) |
