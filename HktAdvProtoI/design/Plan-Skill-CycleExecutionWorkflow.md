...
## 4.5 기반/확장 — 발주 관계와 engine/content 정책

기반(engine: 표현·물리·프로토콜 봉투)과 확장(content: 세계 의미·성장·능력)은
**공정 하나 + 발주 관계**로 굴린다. Cycle 이 유일한 공정이고, 기반은 Cycle 이
끌어당길 때만 자라는 수주자다.

```text
Cycle (확장 공정)         유일한 공정 — 세계 의미를 쌓는다
        │ ENGINE GAP      "이 Rule 을 실현하려면 기반에 X 가 필요하다"
        ▼                 — Cycle 의 이 발주와 Human 의 직접 지정(표현·기술 개선)만이
기반 트랙 (수주자)          기반 작업을 만든다. 발주받은 최소 범위를 별도 커밋으로 구현한다
```

Cycle 안의 코드 배치 정책 4개:

1. **새 코드는 content 팩에 쓴다.** 판정 기준은 게임의 명사 — 이 세계의 이름
   (stone·wolf·worldPressure)을 아는 코드는 content 의 것이다. 범용해 보이는
   개념도 처음엔 content 에 둔다 — 범용성은 두 번째 실제 사용처가 증명한다.
2. **engine 승격은 반복이 증명한 뒤, 기반 트랙의 별도 커밋으로.** 승격 시 게임
   명사를 벗기고("Wolf.knowledge" 가 아니라 "존재가 키-값 지식을 가진다"),
   기존 관찰 가능 행동을 유지한다 (§18).
3. **기반 결손은 ENGINE GAP 으로 흐른다.** build Agent 는 먼저 physics 솔버 조합
   등 content 쪽 표현을 시도하고, content 만으로 성립하지 않으면 ENGINE GAP 으로
   기록해 기반 트랙에 발주한다 (Human 승인 → 최소 구현 → Cycle 재개).
4. **02-world 의 State/Rule 은 전부 content 의 운명이다** — 기반은 게임 명사 없이
   동작한다 (설계 반전 ⑤). plan 은 기반의 새 능력이 필요해 보이면 `ENGINE GAP 예고`
   로 build 에 넘긴다.
