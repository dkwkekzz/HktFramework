...
| 규칙 | 어떤 장면에서 무엇이 달라지는가 — 규칙 = (조건 + 변형) × n (§7 의 형식 · 사람이 읽는 표) |
| 이름표 | 규칙의 낱말이 코드의 무엇인가 — 구현자만 읽는다 (§8) |
...
```text
규칙  <사람이 읽을 한 줄 이름>          <RULE id>          근거 <Design 절>
  ① 조건  <장면>
     변형  <달라지는 것>
  ② 조건  <장면>
     변형  <달라지는 것>
...
- **조건은 장면이다.** 플레이테스터가 손으로 만들 수 있는 세계의 상태 하나 — "누가 어디서 무엇을 한 뒤인가" 를
  현재형으로 쓴다. 변수 · 코드 이름 · 약어 · 데이터 선언("~를 밝혔다" · "~가 설정돼 있다")을 쓰지 않는다.
- **변형은 보이는 차이다.** 그 장면 뒤에 무엇이 달라져 있는가 — "상태 X = Y" 가 아니라 눈이나 도구로 확인되는 것으로 쓴다.
- **한 쌍에 조건 하나 · 변형 하나.** 경계(성립하지 않는 경우)는 별개 개념이 아니라 **반대 조건의 쌍**이다 — 규칙마다 최소 하나.
- **좋은 쌍의 잣대 하나** — "이 조건을 세계에서 만들 수 있고, 이 변형을 눈이나 도구로 확인할 수 있는가". 둘 중 하나라도 아니면 아직 쌍이 아니다.
- **사례로 쓴다.** 일반형("배속을 밝힌 원천은 그 철에 그 배로 되돌아온다")은 문장에 두지 않는다. 일반 기구는 engine 의 몫이고, 반대 사례의 쌍
  ("숲의 원천은 긴 밤에도 달라지지 않는다")이 구현자에게 일반형이 필요하다는 것을 알린다.
- **숫자는 그 숫자가 의미일 때만** (두 배 · 한 번뿐 · 하루). Design 에 없는 값이 변형에 필요하면 그 칸을 **비우고** UNRESOLVED 로 보낸다 (§5).
  지금까지는 추상 조건 뒤에 값이 숨어 결손이 보이지 않았다 — 장면 형식에서는 빈 칸으로 드러난다.

예 (Wolf):

```text
규칙  불을 아는 늑대는 횃불을 피한다          RULE-WOLF-FIRE-AVOID-001     근거 Design-NPC-Combat-Knowledge
  ① 조건  불이 위험하다는 것을 아는 늑대에게 횃불을 든 플레이어가 다가간다
     변형  늑대가 거리를 벌린다
  ② 조건  불을 모르는 늑대에게 같은 플레이어가 다가간다
     변형  늑대는 지금처럼 공격한다
  ③ 조건  불을 아는 늑대에게 횃불이 없는 플레이어가 다가간다
     변형  늑대는 지금처럼 공격한다
```
...
```text
규칙  공격 직전의 방어는 완벽한 방어다        RULE-PERFECT-GUARD-001       근거 Design-Combat §Guard
  ① 조건  공격이 닿기 직전에 방어를 올렸다
     변형  피해가 없다. 공격한 쪽이 빈틈을 보인다
  ② 조건  공격이 닿기 한참 전에 방어를 올렸다
     변형  지금까지의 방어와 같다 — 피해가 줄고 빈틈은 없다
  "직전" 이 얼마인가는 Design 이 말하지 않았다 → UNRESOLVED (§5)
```

Rule 은 최대한 Design 의 언어와 직접 대응되어야 한다. 규칙 이름 옆의 RULE id 는 구현의 함수 머리 주석과 grep 으로 만나는 자리다 —
사람은 id 를 읽지 않아도 규칙을 읽을 수 있어야 한다.

규칙 표에 두는 것은 셋이다 — **새 규칙**(ADDED) · **바뀐 규칙**(CHANGED) · **기존 규칙 위에 놓는 데이터**(그 규칙 id 를 인용 — 사람이 확인할 사실은 데이터여도 쌍이다).
대상 집합만 느는 기존 규칙(AFFECTED)은 규칙 표에 쓰지 않는다 — 이름표에 한 줄이면 된다. 회귀도 규칙 하나다
(조건: 이 Cycle 이 들어갔다 · 변형: 앞 세계의 무엇이 그대로다).

## 8. Semantic 과 Rule 의 관계 — 규칙 표 하나와 이름표 하나

독자가 둘이다 — **규칙을 읽고 판정하는 사람**과 **규칙을 코드와 테스트로 옮기는 구현자**. 한 문장에 둘을 같이 넣으면
문장이 난해해진다 (`CLIFF_FROST_VEIN 의 harvested 가 2 이면 standingConditions 에 hazard/matter`). 그래서 spec 은 둘을 나눈다.

```text
규칙 표   사람이 읽는다.  §7 의 형식. 코드 이름 · 점 경로 · 약어가 없다.
이름표    구현자가 읽는다. 규칙 문장의 낱말이 코드의 무엇인가 — 존재와 상태의 점 경로(§6 의 World Semantic) ·
          데이터 자리 · 관찰 봉투의 자리 · 재는 테스트. 사람은 읽지 않아도 된다.
```

```text
이름표 (구현자용 · 규칙 문장에는 쓰지 않는다)
  절벽의 결정면              CLIFF_FROST_VEIN (content/regions/frost-canyon.ts)
  다 캤다                    sources.<id>.phase = DEPLETED
  그 자리를 밟으면 위험이 뜬다  snapshot.standingConditions[] 에 hazard/matter + contact 코드
  두 배 속도                 ResourceSourceSpec.recoverySpeed[LONG_NIGHT] = 2
  재는 것                    c020.spec.ts — 규칙 이름이 describe · 쌍 번호가 it
```

World Semantic(§6 — 세계에 무엇이 존재해야 하는가)은 이름표의 왼쪽 열이 어떤 존재와 상태인가를 말하는 것이므로 **이름표에 든다**.
따로 절을 두지 않는다. REUSED / ADDED / CHANGED / AFFECTED 와 "투영하지 않는 것" 도 이름표에 든다.

문서 단위와 검증 단위가 같아진다 — **규칙 하나 = describe 하나 · 쌍 하나 = it 하나**. 규칙 표만 읽고 테스트를 쓸 수 있어야 하고,
쓸 수 없으면 spec 의 결손이다.
