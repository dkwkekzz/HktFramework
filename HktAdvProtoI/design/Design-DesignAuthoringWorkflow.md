...
나누지 않는다 — 남은 절이 뒤 층의 것이면 그 층의 주입이 그 절을 인용한다.
...
기반 층 주입    축 하나 — 방향 한 줄 · 기획서 · design/ 문서 지목. 위에서 아래로 하나씩, 열린 층만 받는다
              (열린 층은 plan/DESIGN.md §1). 그 층의 Cycle 들은 축을 세우면서 미지를 하나 놓는다.
...
- 채팅으로 온 주입물은 `content/roadmap/` 에 그 층의 결과물(`L<N>-*.md` · `M<N>-*.md`)로 보존한다 — 그것이 Source 다. 이 보존과 층 · 행 · 등급 판정,
  그리고 §5 의 크기 판정(크면 나눠 보존)이 `advprotoi-inject` 의 전부다 — 번역이지 탐색이 아니고, spec 을 쓰지 않고 멈출 수 있다.
...
문서 소유권: `L0-Game.md` · `L<N>-*.md` · `M<N>-*.md` · 시스템 문서는 Human 원본이다 (나누는 것은 옮기기이지 고치기가 아니다). `spec.md` 는 AI 가 쓰고
동결 뒤 아무도 고치지 않는다 (의미를 바꿔야 하면 새 Cycle). `plan/` 은 공정이 갱신하는 살아 있는 문서다.

## 12. Artifact 생성 규칙

```text
생성한다      시스템 문서 (영역별 1개) · L<N>-*.md (기반 층별 · 나뉜 것 포함) · M<N>-*.md (미지별) · cycles/C###/spec.md · shots/ ·
