...
| spec.md 의 묶음 블록 | 그 기획에서 지금 어떤 플레이 하나를 세우는가 — 어느 Cycle 들로? |
...
Cycle      C### 이름 — 한 줄 목표   (2~4개 · 순서는 의존성 + Breath 의 점진 완성. 번호는 예정 — 실제는 전 이름공간 최대+1)
           C### …
...
뒤 Cycle 은 게이트가 없다 — 첫 spec 의 답을 물려받고, 자기 spec 에서 새로 생긴 의미만 UNRESOLVED 로 묻는다.
