...
그 절을 그 층의 `content/roadmap/L<N>-Handoff.md` 로 옮긴다 — 글자 그대로 · 절마다 출처 · 원문 자리에 포인터 한 줄(spec 의 SOURCE 가 깨지지 않는다).
한 절이 두 층에 걸치면 먼저 열리는 층의 Handoff 에 두고 뒤 층은 포인터로 가리킨다 — 앞 층이 받은 뒤 남는 몫을 뒤 층의 Handoff 로 넘긴다.
Handoff 는 기획서가 아니다 — 행을 세우지 않고 Cycle 을 자르지 않는다. 그 층이 열리면 주입(advprotoi-inject)이 Handoff 를 **먼저** 받는다 — Human 의 주입물
(방향 한 줄이면 된다)과 합쳐 `L<N>-<이름>.md` 를 세우고(크기 규칙은 그때 다시) Handoff 에서 지운다 · 다 비면 파일을 지운다. 옮기지 않는 것 둘 —
...
기반 층 주입    축 하나 — 방향 한 줄 · 기획서 · design/ 문서 지목. 위에서 아래로 하나씩, 열린 층만 받는다
              (열린 층은 plan/DESIGN.md §1). 그 층의 L<N>-Handoff.md 가 있으면 먼저 받는다 — 주입물과 합쳐 기획서를 세우고 Handoff 에서 지운다.
...
- 채팅으로 온 주입물은 `content/roadmap/` 에 그 층의 결과물(`L<N>-*.md` · `M<N>-*.md`)로 보존한다 — 그것이 Source 다. 이 보존과 층 · 행 · 등급 판정,
  그리고 §5 의 크기 판정(크면 나눠 보존) · 층 판정(뒤 층의 절은 그 층의 Handoff 로 · 열린 층의 Handoff 는 먼저 받는다)이 `advprotoi-inject` 의 전부다 —
...
문서 소유권: `L0-Game.md` · `L<N>-*.md` · `M<N>-*.md` · 시스템 문서는 Human 원본이다 (나누는 것 · Handoff 로 옮기는 것은 옮기기이지 고치기가 아니다 — `L<N>-Handoff.md` 도 원본이다). `spec.md` 는 AI 가 쓰고
동결 뒤 아무도 고치지 않는다 (의미를 바꿔야 하면 새 Cycle). `plan/` 은 공정이 갱신하는 살아 있는 문서다.

## 12. Artifact 생성 규칙

```text
생성한다      시스템 문서 (영역별 1개) · L<N>-*.md (기반 층별 · 나뉜 것 포함) · L<N>-Handoff.md (뒤 층으로 옮긴 절 — 그 층이 열리면 받고 지운다) · M<N>-*.md (미지별) · cycles/C###/spec.md · shots/ ·
...
9. **기획서는 층 하나의 것이다.** 뒤 층의 절은 그 층의 `L<N>-Handoff.md` 로 옮기고, 그 층이 열리면 주입이 먼저 받는다. 뒤 층의 것을 더미로 구현하지 않는다 —
   형의 자리까지가 지금 층이다 (§5).
