...
|---|---|---|---|---|---|
| Human — 지목 · 승인 · 판정 · 주입 | Human | L0 · L1 · L2 아홉 · M1~M8 · L7 주입 · C001~C031 기획서 여섯의 판정 반영 · Foundation 지목 · Foundation Cycle 목록 승인 (Q1~Q9 제안대로 · 기반 집중) · L3-Subject-Body 지목 · Cycle 목록 승인 (C039~C042 · 질문 여섯 권장대로) · **"C039 진행"** | **판정 셋** — 협곡 · 생명 · 요구와 가능성 (컨텐츠 행 — 실주행) · **"C040 진행"** | 판정 답의 반영 · 지목 — 후보 3 · 3.5 | HundredRooms 미지 백 줄의 이름 · 4~7층은 층이 열릴 때 |
| Cycle — spec 제안 → 동결 → 실현 → 마감 | AI (advprotoi-spec → advprotoi-cycle) | C001~C038 (기획서 열 · Foundation 닫힘) · C039~C042 spec (L3-Subject-Body — 승인) · **C039 닫힘** (몸이 세계에 선다 — 성질이 물음의 답이 되고 문이 몸에게 묻는다) | **C040 실현 대기** — "C040 진행" (초안을 C039 의 「다음 Cycle 로」 로 다듬어 동결) | C041 → C042 (초안 — 자기 차례에 동결) | 3층 예심(기반 질문 서넛) → 기반 검토 |
...
순서 — 트랙 사이의 의존 (왼쪽이 먼저 · 병행은 나란히)
  ...
  (지금) L3-Subject-Body — **C039 닫힘** ──→ Human "C040 진행" ──→ C040 → C041 → C042 ──→ 예심(기반 질문 서넛) ──→ 기반 검토 ──→ 3층 닫힘
...
한 Cycle = 브랜치 `cycle/C###` = 세션 하나. **말할 것: "C040 진행"**(3층 둘째 Cycle) · "<기획서> 로 spec 써"(다음 spec 제안 — 후보 3 · 3.5) · 판정 셋의 답.
...
| 3 주체와 몸 | **진행** (2층 판정과 병행) | 기획서는 L3-Subject-Body 하나 — C039 가 닫혔다(몸의 성질이 물음의 답이 되고, 조건이 행위자를 가리키고, 문이 몸에게 묻는다 · Core). 다음은 C040 (DESIGN §3 L3-Subject-Body) |
...
| L3-Subject-Body (3층 — 기반) | C039~C042 | **진행** — C039 닫힘(기반 검토 항목 F-1~F-4 · 계약 결정 넷) · 기반 검토는 C042 마감 뒤 예심이 질문 서넛으로 |
...
| 레인 | 지금 | 기다리는 것 |
|---|---|---|
| L3-Subject-Body | **C040** 초안 — 동결 · 실현 가능 | "C040 진행" |
