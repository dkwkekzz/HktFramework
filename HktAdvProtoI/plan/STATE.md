...
한 Cycle = 브랜치 `cycle/C###` = 세션 하나. **말할 것: "<기획서> 로 spec 써"**(다음 spec 제안 — 후보 3 · 3.5) · **"3층 주입 — <방향 한 줄>"**(후보 2 — inject 가 L3-Handoff §2 를 기획서로 세운다) · 판정 넷의 답.
...
| 3 주체와 몸 | **다음** (병행) | L3-Handoff(L7 · Foundation 의 3층 몫)가 후보 2 · 5 의 원문이다 — 3층 주입(방향 한 줄)이 기획서로 세운다. 나머지 절반(몸의 값)은 주입 대기 |
| 4 · 5 · 6 | 미주입 | 각 층의 L<N>-Handoff.md 가 입력이다 — Foundation · L7 의 몫은 옮겼고, 2층 나머지 일곱 문서의 뒤 층 절은 3층 주입 때 옮긴다 (DESIGN §3) |
| 7 성장 | 미주입 (원문 확정) | L7 원문의 7층 몫 + L7-Handoff(Foundation §7.5) |
