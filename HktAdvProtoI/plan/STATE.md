...
| ENGINE — 게임 명사 없는 기구 | AI | Region 작성기 T1~T5 실측 (열세 질문 · 미지 한 줄이 사람 손 없이 선다) | **T6 대량** — 후보 여럿을 나란히 | HundredRooms 묶음의 전제 | 큰 Region 이 올 때 chunk 적재 |

```text
순서 — 트랙 사이의 의존 (왼쪽이 먼저 · 병행은 나란히)

  Foundation 묶음 — C034~C037 · C038(결손 다섯) ──→ 기반 검토 실측 ──→ 결정 확정 ──→ **닫힘** (2층 Foundation 이 섰다)
  (지금) Human 기획서 지목(후보 2 · 3) ──→ AI 묶음 + Cycle 전부의 spec (C039~) ──→ Human "C### 진행" ──→ Cycle → … ──→ 예심 ──→ 판정
                                                                                                          └─ 아니오 → GAP 주입 → 새 묶음/Cycle
  Human 판정 협곡 · 생명 · 요구와 가능성 ──→ 예: 행 닫힘 / 아니오: GAP 주입 ──→ 묶음 ──→ 판정
  ENGINE T3 · T2 확장 · T5 실주행 (닫힘 — 미지 한 줄이 사람 손 없이 선다) ──→ T6 대량 ──→ HundredRooms 묶음(Human 이름) ──→ "C### 진행" ──→ Cycle (코드 diff 0)

  묶음 후보 순서   (1 Foundation — 닫힘) → 2 편성과 무대(3층) ∥ 3 Rooms GAP 회수  →  4 HundredRooms  →  5 3층 둘째  →  6 · 7 · 8 · 9 (4 · 5 · 6 · 7층)
...
| 2 세계 자체 | **다음** | 기획 아홉 문서가 다 코드에 있고 Foundation 은 닫혔다. 판정 셋(컨텐츠 행 M5·M6·M7) · Rooms GAP 둘(후보 3) · 도구 묶음 하나가 남았다 |
...
Foundation 은 기반 층이라 잣대가 달랐다 — 실주행이 아니라 **기반으로서 제공되는가**이고 실측으로 닫혔다 (CYCLES §3.4).
...
| 방은 기억하고 때가 되면 내민다 (Foundation) | C034~C037 · C038 | **닫힘** — 기반 검토 · 결손 채움 · 결정 확정 |
...
| ENGINE B | T6 대량 후보 (world:draft --batch → world:lab) | — |
