// GameView Snapshot 봉투 — World → View 의 유일한 공개 계약의 **구조** (P2 CHANGED).
//
// 여기 있는 것은 Root 가 바뀌어도 참인 봉투뿐이다: 존재/interaction/HUD/명령/대답의
// 형태. 이 세계의 의미(생명·능력치·타격 경위·profile)는 컨텐츠 팩의
// protocol/gameview.ts 가 이 봉투를 확장해 소유한다.
//
...
// 몸 충돌체 — 모든 character 가 차지하는 캡슐 부피와 그 물리 상태.
...
export interface EntityView {
  ...
  name?: string; // Actor.Name — character 에만 실린다
  position: GameViewPosition;
  labelValue?: number | string; // 관찰 값 (예: 광맥 잔량) — 표시 형식은 View 책임
  kind?: string; // CharacterKind 등 종류 식별자 — 어떤 모습으로 그릴지는 View 책임
  progress?: number; // 0..1 — 진행 중인 행동의 진행도. 없으면 진행 개념이 없는 상태
  targetEntityId?: string; // 현재 상태의 대상 — 없을 수 있음
  attended?: boolean; // 그 몸을 지금 조종하는 이가 있는가.
  // role = other-player-character 에만 실린다. 거짓이면 그 사람은 떠났고 몸만 남은 것이다.
  body?: BodyView; // 몸 충돌체 — character 에만 실린다
  swing?: SwingView; // 행동 충돌 반경 — attack 진행 중에만 실린다
}
...
export interface HudItemView {
  ...
  progress?: number; // 0..1 — 값에 진행도가 동반되는 경우
}
...
export interface ObserverView {
  ...
  // Observer.AcknowledgedMark — 세계가 나에게서 받아들인 마지막 표식.
...
export interface GameViewSnapshot {
  ...
  observer: ObserverView; // 이 관찰 결과의 수신자
  entities: EntityView[];
  interactions: InteractionView[];
  hud: HudItemView[];
  debug: DebugAuthorityView;
