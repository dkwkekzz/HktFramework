...
export interface SceneSurfaceSection {
  ...
  emptyText?: string;
  /**
   * **나란히 서는 묶음**의 이름 — 이어지는 구획들이 같은 이름이면 한 줄에 선다.
   *
   * 무엇과 무엇이 함께 읽히는가는 결정 Layer 의 뜻이다 (`columns` 와 같은 자격의
   * 그리는 쪽 결정이며 계약에서 오지 않는다). 그리는 쪽은 이름이 같다는 것만 보고,
   * 그것이 걸어 둔 것과 지닌 것인지 알지 못한다.
   *
   * **이어진 것만 묶인다.** 사이에 다른 이름이 끼면 묶음은 거기서 끝난다 — 그래야
   * 화면에 서는 차례가 목록의 차례와 어긋나지 않는다.
   */
  group?: string;
  /**
   * 나란히 설 때 한 자리의 가장 좁은 너비(px) — 이보다 좁아지면 **다시 세로로 쌓인다**.
   *
   * 묶음의 **첫 구획**이 지닌 값만 쓴다. 좁아졌을 때 쌓이는 차례는 목록의 차례 그대로다.
   * 밝히지 않으면 그리는 쪽의 기본값이 선다.
   */
  groupMin?: number;
}
...
export interface SceneSurface {
  ...
  footer: string[];
  /**
   * 이 표면이 **얼마나 넓어도 되는가** (px) — 결정 Layer 의 청함이다.
   *
   * 나란히 놓을 것이 있는 표면은 읽을 것 하나짜리 표면보다 넓어야 한다. 무엇을 나란히
   * 놓을지는 결정 Layer 가 아는 것이므로 얼마나 넓어야 하는지도 그쪽이 안다.
   *
   * **청함이지 명령이 아니다** — 화면이 그만큼 없으면 그리는 쪽이 화면에 맞춘다.
   * 밝히지 않으면 그리는 쪽의 기본 너비가 선다.
   */
  width?: number;
}
...
export interface SceneState {
  ...
  /**
   * 시점이 몸에서 떨어져 있을 거리 — 없으면 기본값.
   * 그리는 쪽은 왜 그 값인지 모른다 (설계 반전 ⑤). 결정 Layer 가 정해 넘긴 거리일 뿐이다.
   */
  viewDistance?: number;
  /**
