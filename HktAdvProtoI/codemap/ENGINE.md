...
| `property.ts` | 성질은 물음의 답이다 — 이름 하나로 Source 들을 모아 몫 셋(상한 · 더함 · 참거짓)을 합치는 산수 — 게임 명사 0 · 저장 0 |
...
| `CheckCondition` · `CheckConditionSite` · `CheckConditionVocabulary` · `CheckConditionQueryRule` · `CONDITION_ITEM` | type/const | `{ sites: { where; condition }[]; vocabulary: { targets: Partial<Record<TargetKind, string[]>>; queries: { target; query; paths? }[] } }` · `{ mark '㊹'; id 'condition-refs' }` | ㊹ 조건의 참조 무결 — 잎마다 Target 갈래·ref · Query·path · Operator·value 형 · qualifier 형. `REF_OPTIONAL_TARGET_KINDS` 의 갈래는 ref 없이 선다 (자리로 골라지는 것) | tools |
| `Condition` · `ConditionLeaf` · `ConditionAll` · `ConditionAny` · `ConditionTarget` · `ConditionQuery` · `ConditionOperator` · `ConditionQualifier`(Time · Change) · `ConditionValue` · `ConditionVerdict` | type | `{ target: { kind; ref? }; query: { kind; path? }; operator; value?; qualifier?; chance? }` · `{ all }` / `{ any }` · `'met'\|'unmet'\|'undecidable'` | 조건의 한 형 — Target 아홉 + 자리 둘(player · faction) · Query 여섯 + 자리 넷 · Operator 아홉 · qualifier 하나까지 | content |
...
| `conditionLeaves` · `formatConditionLeaf` · `isConditionGroup` · `DECIDABLE_/DEFERRED_/SINGLETON_/REF_OPTIONAL_TARGET_KINDS` · `DECIDABLE_/DEFERRED_QUERY_KINDS` · `CONDITION_OPERATORS` · `VALUELESS_OPERATORS` · `TIME_/CHANGE_QUALIFIER_MODES` | fn/const | `(condition) => ConditionLeaf[]` · `(leaf) => string`(`kind(ref).query(path) OP value [qualifier]`) | 잎 펴기 · 기계 표기 · 어휘 상수 — 어휘 상수를 import 하는 소비처는 없다(engine 안의 check.ts 뿐) | tools |
| `PropertySource` · `PropertyShare` · `PROPERTY_SHARE_KINDS` · `PropertyAnswer` · `propertySourcesOf` · `resolveProperty` | type/const/fn | `{ origin; property; share }` · `{kind:'cap'\|'add'\|'flag'; value}` · `['cap','add','flag']` · `number\|boolean` · `(property, sources) => PropertySource[]` · `(property, sources) => PropertyAnswer\|undefined` | 이름 하나로 묻는 성질 — 답할 Source 가 없으면 **없음**(undefined · 「0」도 「거짓」도 아니다) · cap 은 가장 작은 것이 이기고 · add 는 합이고 · flag 는 하나라도 참이면 참 · 수와 참거짓이 한 이름에 섞이면 수가 이긴다 | content / — / — / content / — / content |
