...
한 Cycle = 브랜치 `cycle/C###` = 세션 하나. **말할 것: "C026 진행"** — `advprotoi-cycle` 이 명세(spec.md) → 실현 → 마감을
...
| Observe — [RoomAnswersWhenAsked](content/roadmap/play/RoomAnswersWhenAsked.md) | **C026** 자리를 지목하면 그 자리의 사실이 판에 선다 — 입력 해석 정책 + 지목 강조 + 늘 떠 있는 판(ENGINE 셋) · 대상 프레임(자리) · 세계 위 글자를 걷고 방 이름을 진입 제목으로. **세계도 봉투도 늘지 않는다** | — (선행 C008 은 닫혔다) | C027 → C028 |
...
| RoomAnswersWhenAsked | 물으면 답한다 (지목 · 대상 프레임 · 세계 위 글자 0) | C026~C028 | **C026 다음** — 순서 밖 · 병행 |
