...
한 Cycle = 브랜치 `cycle/C###` = 세션 하나. **말할 것: "C010 진행"** — `advprotoi-cycle` 이 명세(spec.md) → 실현 → 마감을
...
|---|---|---|---|
| Rule — [RuleBoundRoom](content/roadmap/play/RuleBoundRoom.md) | **C010** 세계는 하나다 — 두 관찰자가 같은 미로 State 를 본다 · 한쪽의 걸음이 다른 쪽의 압력이다 · 떠난 뒤에도 State 가 남는다. C009 가 세운 **문의 활성 조건**이 여기서 처음 "남이 내 문을 닫는다" 가 된다 (C009 TODO 부채 ①) | — | Play 닫힘 → Material |
| Material — [RoomBearsMaterial](content/roadmap/play/RoomBearsMaterial.md) | 대기 (명세는 지금도 쓸 수 있다 — 이름·성질·상수는 Play 의 위임된 결정 D1~D4 에 있다) | Rule 닫힘 (C010) | C011 → C014 |
...
| RuleBoundRoom | 방은 규칙을 품는다 (환상의 미로 = Region 하나) | C008~C010 | C008 · C009 닫힘 · **C010 다음** |
...
**Human 판정 대기 66** — 각 Cycle 의 `TODO.md` (그림은 같은 폴더 `shots/`). `npm run dev` 로 직접 본다.
...
[C009](cycles/C009-reach-by-the-rule/TODO.md) 9.

Play 둘이 닫혔으므로 **Play 전체 실주행**이 그 위에 둘 더 있다 —
...
도구    world:observe (--graph · 방 하나의 높이·표면·통행·의미 PNG 다섯 + 보고 · 읽기 전용) · world:compile · world:shot ·
        cycle:shot (마감 촬영) · 검증용 손잡이 HKT_SPAWN · HKT_SPAWN_REGION · HKT_NPCS · HKT_REGION_PATTERN
