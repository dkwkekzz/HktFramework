...
순서 T1~T6 은 roadmap README §2.1. ENGINE 레인이라 Cycle 실주행과 **병행**한다 — 말할 것: **"T4 진행"** (브랜치 `engine/T4`).
...
| ENGINE B — [Region 작성기](content/roadmap/L2-World-Tool-Scale.md) (도구 2단계) | **T4** 등급 판정기 — brief 의 "요구" 를 계약 목록과 대조해 **A · B · C** 로 가르고 빠진 것을 GAP 형식으로 낸다. 게임 명사 없음 · 분리 커밋 · Cycle 과 **병행**한다 — 말할 것: **"T4 진행"** (브랜치 `engine/T4`). T1 검사기 · T2 brief 형 · T3 뼈대 생성기 절반은 섰다 | T3 의 나머지 — phases 는 C016 · ecology 는 C022 · T6 은 Life(C025) 닫힘 | T5 → T6 → Play HundredRooms |
...
도구    world:check (검사 아홉 → JSON · 종료 코드가 판정 · npm test 에 붙었다 · 읽기 전용) ·
        world:author (brief 하나 → 방 하나의 뼈대 · --write 로 굳힌다 · 굳힌 것은 데이터다) ·
