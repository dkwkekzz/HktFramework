...
한 번이라도 지난 뒤에만 「할 수 있는 것」 이 아니라 **조건**을 말한다(지나기 전엔 조건이 서지 않는다). 화면은 그것 말고 달라지지 않는다.
...
3. 숲 가장자리의 비늘 자리(FALLEN_SCALE): 고래가 한 번도 지나지 않은 세계에서 지목하면 조건 줄이 서지 않고, 한 번 지난 뒤(HKT_PRESENCE=SKY_WHALE_ROUTE) 지목하면
   판의 조건 줄이 「지나간 것이 있어야 한다」 로 선다 — 조건이 기억을 읽었다 (Q2 제안대로일 때). 원천의 phase · 되돌아옴은 그대로다.
...
묶음 전체(C035 · C036 · C037)의 질문이다 — 답과 함께 "C035 진행" 이 오면 동결된다. 각 줄에 제안값을 붙였다 — "제안대로" 면 그것으로 간다.
