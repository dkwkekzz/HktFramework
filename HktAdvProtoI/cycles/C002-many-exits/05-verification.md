...
### 실주행 촬영에서 드러난 것 (build 통합 실측)

`HKT_SPAWN_REGION` + `HKT_NO_NPC` 로 방마다 세워 찍었다 (헤드리스 · 560×420).
**보인 것** — 방 여섯의 바닥 색이 셋으로 갈린다(civil 따뜻 · outer 초록 · wild 어두운 청록) ·
HUD 깊이 문구 셋 · 출구 표식의 종류 다섯이 색으로 구분된다(길 모래 · 고개 하늘 · 오솔길 올리브 ·
들어감 보라 · 문 붉음) · 닫힌 고대 문만 빗장 걸린 다른 표식이다 · 백왕령에서만 `채굴: E` 가 뜬다
(다른 방에 광맥이 없다 — 투영이 방으로 잘린 증거).

**보이지 않은 것 (X-②·X-⑥ 의 위험)** — 거절 문구가 화면에 뜨지 않는다.
`engine/view-kernel/hud/hud.ts` 의 프롬프트 한 줄은 `keyed.find(available) ?? keyed.find(unavailableText)`
로 **하나만** 고르는데, `기본 스킬(F)` 이 언제나 가용이라 늘 이긴다. 닫힌 문 위에 서서 `Q` 를 눌러도
화면에는 `[F] 기본 스킬` 만 남는다 — 세계는 `connector-inactive` 로 거절하고(S-012 PASS) View 의
문구표도 "잠겨 있다" 를 갖고 있으나(04-gameview), **그 사이를 잇는 화면 자리가 없다.**
관찰 계약도 결정 Layer 도 아닌 **HUD 표시의 결손**이다 → 아래 X-②·X-⑥ 은 이 자리가 생기기 전에는
Human 이 PASS 를 줄 수 없다.
