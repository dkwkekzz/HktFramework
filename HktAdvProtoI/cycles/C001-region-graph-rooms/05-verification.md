...
`실측 [ ]` 과 `관측된 State` 는 build 본체가 실행 결과로 채운다 — 여기서는 비워 둔다.
...
S-002  (SPEC-001) 두 Region 의 좌표는 서로 무관하다 — 같은 (x, z) 가 다른 자리다
...
S-004  (SPEC-002) anchor 는 각 Region 의 한 자리다
  Given   세계를 새로 만든다
...
S-006  (SPEC-003) 기본 자율 존재 둘과 광맥 하나는 백왕령에 있다 · 숲 가장자리는 비어 있다
  Given   세계를 새로 만든다 (관찰자 없음)
...
S-008  (SPEC-004) 경계는 World.Bounds 가 아니라 몸이 선 Region 의 extent 다 — 숲 가장자리에서도 같다
...
S-010  (SPEC-005) 경계 — 거리가 정확히 INTERACTION_RANGE(2.0) 이면 가용 (≤)
...
S-011  (SPEC-005) 경계 — 멀면 out-of-range (투영과 요청 모두)
...
S-012  (SPEC-005) 경계 — 멀리서 요청 (00-cycle Observable ⑥)
  Given   Actor[player-1].regionId = WHITE_KING_DOMAIN · Actor[player-1].position = (0, 0) ·
...
S-013  (SPEC-005) 경계 — 행동이 대체 불가면 action-busy
  Given   Actor[player-1].regionId = WHITE_KING_DOMAIN · Actor[player-1].position = (0, 17) ·
...
S-014  (SPEC-005) 경계 — 없는 Connector 는 unknown-connector
  Given   Actor[player-1].regionId = WHITE_KING_DOMAIN · Actor[player-1].position = (0, 17) ·
          Actor[player-1].currentAction = idle
...
S-015  (SPEC-005) 경계 — Connector 의 어느 끝도 내 Region 에 없으면 wrong-region
...
S-017  (SPEC-006) 건너는 순간 관성과 진행 중 이동은 버려진다
  Given   Actor[player-1].regionId = WHITE_KING_DOMAIN · Actor[player-1].position = (0, 17) ·
...
S-018  (SPEC-006) 왕복 — 건너고 되돌아오면 원래 anchor 자리
  Given   Actor[player-1].regionId = WHITE_KING_DOMAIN · Actor[player-1].position = (0, 17) ·
          Actor[player-1].currentAction = idle
...
S-020  (SPEC-007 경계) 숲 가장자리의 관찰 — self + region-exit 하나뿐
...
S-021  (SPEC-007) 다른 Region 의 관찰자는 서로의 entities 에 실리지 않는다
...
S-022  (SPEC-007) 목적지 Region 의 이름은 어디에도 실리지 않는다
  Given   Actor[player-1].regionId = WHITE_KING_DOMAIN · position = (0, 17)
...
S-023  (SPEC-007 ④) region.hash 는 두 관찰에서 같다
  Given   Actor[player-1].regionId = WHITE_KING_DOMAIN · position = (0, 17)
...
S-024  (SPEC-007 ④) 같은 Region 의 두 관찰자는 같은 hash 를 본다
  Given   Actor[player-1].regionId = WHITE_KING_DOMAIN · position = (0, 17)
...
S-027  (SPEC-009) 다른 Region 의 몸은 휘두름에 맞지 않는다
...
S-028  (SPEC-009) 다른 Region 의 몸은 자율 존재의 인지 범위에 들지 않는다
...
S-030  (SPEC-010 경계) 이전 버전의 스냅샷은 복구되지 않는다 — 새 세계
...
R-002  (RULE-BODY-PUSH-001 · SPEC-009 경계) 같은 Region 의 두 몸이 겹치면 여전히 민다
...
R-003  (RULE-MOVE-001) extent 안 목적지는 여전히 성공한다
