...
X-⑥  멀리서 건너기를 요청하면 거절이 온다
     ...
     그림   shots/X-06-transit-rejected-far.png — **이 그림에는 문구가 없다.** 찍을 당시 거절이 화면에 뜨지 않았다.
           C002 가 그 결손을 고쳤으므로(세계의 대답을 화면에 띄운다) 지금 실주행하면 뜬다 —
           문구가 실제로 뜬 그림은 cycles/C002-many-exits/shots/X-06-connector-inactive.png
     판정   [ ]
