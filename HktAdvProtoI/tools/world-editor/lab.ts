 ...
 *
 * 문서가 편중 요약으로 든 검사 일곱 가운데 셋(⑲ 기회 자리의 분포 · ⑳ Carrier 분포 ·
 * ㉒ 고립)은 이미 서 있지만, 그것들은 원천마다 **원인과 되돌아옴**을 요구한다 — T3 의 뼈대가
 * 아직 내지 못하는 값이다 (그 나머지는 C022 의 것이다). 넷(㉕ ㉖ ㉚ ㉝)은 검사 자체가 아직 없다.
 *
 * 그러니 지금 이 자리에 실리는 것은 **구조의 수**가 움직인 자국뿐이다. 없는 것을 통과로 적지
 * 않는 것이 이 저장소의 어법이므로(검사의 `absent`), 무엇을 못 재는지도 함께 적는다.
 */
const NOT_MEASURED =
  '<p class="none">아직 재지 못하는 편중 — 기회 자리와 Carrier 의 분포(⑲ ⑳), 고립(㉒). ' +
  '그 검사들은 원천마다 원인과 되돌아옴을 요구하는데 뼈대가 아직 그 값을 내지 못한다 (C022). ' +
  '철과 생명의 편중(㉕ ㉖ ㉚ ㉝)은 검사 자체가 아직 없다.</p>';
