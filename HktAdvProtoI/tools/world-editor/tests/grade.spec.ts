...
describe('T4 — 셋이 A · B · C 로 갈린다', () => {
  ...
  it('가스 마을은 걸린 것이 하나도 없다 — 데이터만으로 선다', () => {
    ...
    // T2 확장 CHANGED — ⑨~⑫ 물음(asking)이 ⑬ 내밂 곁에 서서 미답이 셋이 되었다
    expect(result.pending.map((g) => g.required.split('가운데 ')[1])).toEqual([
      'birth',
      'asking',
      'offering',
    ]);
...
describe('T4 — B · C 의 빠진 것이 정확히 적힌다', () => {
  ...
  it('마법도시에 빠진 것은 **축 둘**이다 — 6층(능력)과 3층(주체)', () => {
    ...
    // 축이 없으므로 **열 답**을 하나도 적지 못한다 — 그것이 C 의 모습이다 (T2 확장으로 아홉이 열이 되었다)
    expect(result.pending.length).toBe(10);
  });
