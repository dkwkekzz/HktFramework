...
//
// 어떤 글자가 어느 칸에 들어가는가는 팩의 결정 Layer 가 정한다 (Stage 7).
// 그러므로 아래 장면들은 **그 화면의 미리보기가 아니라 능력의 표본**이다.
//
// 장면 이름은 04-gameview.spec.yaml 의 fixtures 를 따른다 — 같은 것을 두 자리에서
// 다른 이름으로 부르지 않기 위해서다.
...
const SCENES: Array<{ name: string; note: string; surface: SceneSurface }> = [
  ...
  {
    name: 'VUX-IE-FX-FULL',
    note: '가득 찼는데 같은 화면에서 어떤 손은 막히고 어떤 손은 열려 있다 — 소지품 칸의 비대칭',
    surface: {
      id: 'bag',
      open: true,
      title: '가진 것',
...
const page =
  ...
  `그것은 팩의 결정 Layer 가 하는 일이며 아직 서지 않았다 (Stage 7). ` +
