@echo off
rem ============================================================
rem  HktAdvProtoI — 세계와 클라이언트를 각각 다른 창으로 실행 (Windows)
rem
rem  창 2개가 뜹니다.
...
echo 세계 창을 엽니다...
start "HktAdvProtoI 세계" cmd /k "cd /d "%~dp0.." && npm run world"

rem 세계가 포트를 잡을 때까지 잠깐 기다린다 (없어도 클라이언트가 알아서 다시 잇는다)
timeout /t 3 /nobreak >nul

echo 클라이언트 창을 엽니다...
start "HktAdvProtoI 클라이언트" cmd /k "cd /d "%~dp0.." && set HKT_WORLD_URL=http://127.0.0.1:5180 && npm run client -- --open"
