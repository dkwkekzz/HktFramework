...
export interface WorldSetup {
  ...
  presences?: string[];
  /**
   * 탄생지가 **어느 phase 로 서는가** — 검증·촬영용 초기 배치 (C022 ADDED).
   * 예: `{ ROOT_CLUTCH: 'binding' }`
   *
   ...
   *
   * 세운 State 는 언제나 **규칙이 스스로 도달할 수 있는 것**이다 — 진행은 0 이고, 조건이
   * 차 있지 않으면 다음 Tick 에 세계 자신의 규칙이 DORMANT 로 되돌린다.
   ...
   * 예: `{ ORE_EATER: 1 }`
   *
   * 왜 필요한가 — 이 Cycle 에는 그 값을 올리는 것이 세계에 하나도 없다 (올리는 것은 C023 이다).
   * "값이 0 이 아니면 결속이 서지 않는다" 를 그림으로 보려면 그 값을 세우고 시작해야 한다.
