// RULE-MINE-001 — Implements INTENT-MINING-001 · INTENT-ACTION-STATE-001 (C011 CHANGED — 대상이 원천이 된다)
// Input          Actor, Resource Source
// Preconditions  1. Mining Capability Item 보유  2. 같은 방의 InteractionRange 이내
//                3. 현재 행동이 대체 가능하다
// Transition     CurrentAction = mine(Source)           ← 즉시 획득이 아니다
// Result         Success | Failure(no-mining-tool | out-of-range | action-busy | unknown-source)
//
// RULE-MINE-COMPLETE-001 — Implements INTENT-MINING-001 · INTENT-ACTION-PROGRESS-001 (C011 CHANGED)
// Input          채굴 행동이 Duration 을 채운 Actor
// Preconditions  대상 원천을 세계가 안다
// Transition     Inventory.Items[그 원천의 materialId] += 1
// Result         Success | Failure(unknown-source)
//
// **원천은 캐도 줄지 않는다** (spec Out of Scope). 남은 양도 고갈도 이 Cycle 에 없다 —
// 채취 단위와 캔 자국은 C012 가 phase 와 함께 세운다. 그래서 deposit-depleted 가 사라졌다.
//
...
import { findResourceSource, type ResourceSource } from '../semantic/resource';
...
export type MineFailureReason = 'no-mining-tool' | 'out-of-range' | 'action-busy';

/**
 * Precondition 평가 — Observable(Mine.Availability / Mine.FailureReason)과 Rule 이 같은 판정을 공유한다.
 *
 * 방을 먼저 본다: 자리는 방마다 따로인 Local Space 좌표이므로, 방을 묻지 않으면 다른 방의
...
export function ruleMine(_state: WorldState, actor: ActorState, sourceId: string): ActionResult {
  // 원천은 State 가 아니다 — 세계 데이터에서 온다 (semantic/resource.ts).
  const source = findResourceSource(sourceId);
  if (!source) return { status: 'failure', rule: RULE_MINE, reason: 'unknown-source' };

  const failure = evaluateMinePreconditions(actor, source);
...
export function ruleMineComplete(_state: WorldState, actor: ActorState): ActionResult {
