 ...
 *
 * **이 자리가 셋에게 쓰인다** — 세계가 설 때(region-state.ts 의 createRegionStates) · 뒤척일 때
 * (RULE-SEASON-TURN-001) · 그리고 **State 가 아직 없는 원천을 물을 때**(아래 sourceStateOf ·
 * rules/mine.ts · simulation/life-binding.ts). 셋째가 갈려 있던 동안은 데이터가 바뀐 뒤 되살린
 * 세계에서 처음이 고갈인 원천이 캘 수 있는 것으로 읽혔다 — 처음 상태를 짓는 자리는 하나다.
 */
...
/**
 * 그 원천의 지금 — State 가 없으면 **처음 상태**로 친다 (C012 ADDED).
 *
 * State 가 없는 것은 "아직 아무 일도 겪지 않았다" 와 같은 뜻이다 — 그래서 답은 세계가 설 때
 * 짓는 것과 **같은 자리**(위 initialSourceState)에서 온다: 대개 available · taken 0 이고,
 * 실려 와야 생기는 원천은 고갈이다. 없는 것을 있는 것으로 읽지 않는 것이 여기서 지켜진다
 * (규칙 없는 방의 통로를 닫힌 것으로 읽지 않는 것과 같은 규율). 세계가 모르는 원천 id 는
 * 어느 데이터에도 없으므로 available · taken 0 으로 친다.
 *
...
export function sourceStateOf(
  ...
  const stored = states[regionId]?.sources?.[sourceId];
  if (stored) return stored;
  const source = findResourceSource(sourceId);
  return source
    ? initialSourceState(source)
    : { phase: 'available', taken: 0, progress: 0, siteIndex: 0 };
}
