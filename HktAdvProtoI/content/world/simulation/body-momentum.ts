...
//                REST_SPEED 아래로 잦아들면 0 이 된다. 그 몸이 선 Region 의 extent 를 벗어나면 경계에
...
// C001 AFFECTED — World.bounds 가 사라졌다. 경계는 Region 마다 그 extent 다 (02-world State: World.bounds CHANGED).
...
import { groupByRegion } from './body-push';

export function ruleBodyMomentum(state: WorldState, dt: number): void {
  for (const [regionId, group] of groupByRegion(state.actors)) {
    integrateMomentum(group, FRICTION, REST_SPEED, dt, regionExtent(regionId));
  }
}
