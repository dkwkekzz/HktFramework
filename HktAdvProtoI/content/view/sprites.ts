...
const PALETTE: Record<string, string> = {
  ...
  '6': '#ffb98a',
  // 거목 속의 잉걸 색 셋 (C030) — 벽의 한 자리에서 타는 것 하나가 이 셋을 쓴다. 사다리는
  // 앞의 것들 그대로다 (볕 → 그늘 → 땅에 닿는 밑동 = C·c·G · 3·4·5 와 같은 자리).
  // 글자가 7·8·9 인 것은 6 을 알집의 빛(C022)이 먼저 쓰고 있기 때문이다 — 뜻이 다른 색을
  // 한 글자에 겹치지 않는다.
  //
  // **계통이 색에서 먼저 갈린다** — 숲의 재료는 붉고(C·c·G), 빙정석은 채도가 선 파랑이며
  // (3·4·5), 이것만 **호박빛**이다: 서리가 앉지 않는 자리이고 김이 오른다 (Play §5.3 Trace).
  // 가장 붉은 것(C #f2684a · 색상 ≈ 11°)과 갈리도록 색상을 노란 쪽으로 스무 칸 옮겼고
  // (≈ 28~40°), 그 위에 흰 속(7)을 두어 "붉게 빛나는 것" 이 아니라 "타고 있는 것" 으로 읽히게
  // 했다 — 광석은 결정면이 빛을 되쏘고 이것은 스스로 낸다.
  //
  // 밝기의 근거 — 이것이 서는 바닥은 거목 내부 세계의 기본 지면(어두운 초록)과 그 위에 깔린
  // 온기의 자락(TRACE_EMBER_WARMTH_COLOR 0xd98b2b · 호박빛)이다. 자락과 **같은 대역**이므로
  // 색상만으로는 갈리지 않는다 — 그래서 속(7)을 자락보다 뚜렷이 밝게, 테(9)를 뚜렷이 어둡게
  // 두었다. 배어 든 색은 한 값으로 눕고 타는 것은 밝고 어두운 값을 함께 가진다.
  '7': '#ffd27a', // 잉걸의 속 (가장 뜨거운 자리) — 이 표에서 가장 밝은 따뜻한 값이다
  '8': '#f2913a', // 잉걸의 결 (그늘)
  '9': '#8a3d10', // 식은 잉걸 — 땅에 닿는 밑동이자, 다 캐 간 자리에 남는 몇 점
  '.': '',
};
...
// ── 서리가 앉지 않는 벽의 잉걸 (C030 ADDED · V24) ────────────────────
//
// 열다섯째 형태다. **새 기제를 만들지 않는다** — 앞의 열넷과 똑같이 `<sprite>:<state>` 세 벌
// 이고, 어느 그림을 세울지는 지금까지대로 kind 가 고른다 (role-presentation 의 spriteByKind).
//
// 실루엣이 앞의 열넷과 갈리는 자리 — 이것만 **벽면**이다. 땅에서 솟거나 흩어지거나 매달린
// 것이 아니라 벽 한 자리가 뚫리듯 타고 있고, 그래서 그림이 화면을 가로로 꽉 채운 면이다
// (C011 이 넷을 실루엣부터 가른 그 어법 그대로).
//
// 그 면이 말하는 것 셋 — 전부 Play §5.3 Trace 의 말이다.
//   ① 서리가 앉지 않는다 — 벽의 바깥은 서리(2)가 앉았고 잉걸 둘레만 맨 나무(u·U)다.
//   ② 잉걸이 있다        — 속이 밝고(7) 테로 갈수록 식는다(8 → 9).
//   ③ 김이 오른다        — 벽 위로 흰 김(y) 넉 점. 화면에서 이것만 벽 밖으로 나간다.
//
// 벽에 앉은 서리는 **이미 있는 색(2)** 을 쓴다 — 하늘에서 온 것의 결(비늘의 그늘)로 들어온
// 값이지만 차고 옅은 회청색 하나이고, 이 표의 글자가 다 찼으므로 같은 뜻의 값을 하나 더
// 늘리는 것보다 있는 값을 쓰는 것이 옳다 (색을 줄이는 것보다 글자를 늘리는 것이 옳다던 그
// 판단의 뒷면이다). 김(y)도 마찬가지다 — 미로의 상아빛이 여기서는 흰 김이다.
//
// 세 state 의 규율도 그대로다. available 이 가장 온전하고, depleted 는 "있던 것이 없어졌다"
// (속이 파이고 · 김이 그치고 · 그 자리에 서리가 든다), recovering 은 "없던 것이 다시 오는
// 중이다" — 파인 자리(q)는 자국으로 남고 그 둘레부터 잉걸이 되돌아온다. 색의 양도
// available > recovering > depleted 로 간다: 가장 뜨거운 속(7)은 available 에만 서고,
// recovering 은 결(8)만 몇 점 돌아오며, depleted 에는 식은 몇 점(9)뿐이다.

const SOURCE_WALL_EMBER = [
  '.....y....y.....',
  '......y..y......',
  '.......yy.......',
  '.......yy.......',
...
// 다 캐 간 자리 — 김이 그치고 속이 파였다(q). 파인 둘레에 서리(2)가 들어 "서리가 앉지 않는
// 벽" 이 아니게 된 것이 자국이고, 밑동에 식은 잉걸(8) 몇 점만 남는다
const SOURCE_WALL_EMBER_DEPLETED = [
  '................',
  '................',
  '................',
  '................',
...
// 다시 더워지는 중 — 파인 자리(depleted 의 그것 그대로)는 자국으로 남고 그 둘레부터
// 잉걸이 되돌아온다. 김도 한 줄기만 오른다 — 아직 다 온 것이 아니다 (§5.6 예보)
const SOURCE_WALL_EMBER_RECOVERING = [
  '................',
  '................',
  '.......y........',
  '.......y........',
...
// 터지는 순간의 알집 (C023 ADDED) — **한 tick 이다.** 실루엣은 그대로이고 빛(6)이 알의
// 속을 다 채워 뿌리 마디 위로 넘친다: 결속하던 빛이 마지막으로 한 번 크게 도는 그림이다.
// 사람이 이 그림을 볼 일은 드물지만, 그 한 tick 이 실려 온 화면에서 그림이 비면 안 된다
const LIFE_ROOT_CLUTCH_BORN = [
  '................',
  '....6..66..6....',
  'uu6..6666..6.uu.',
  '.uUU666666UUu...',
...
// 터진 채 남은 알집 (C023 ADDED) — **눈에 보이는 "터졌다" 가 이 그림이다.**
//
// 앞의 셋과 달리 **실루엣부터 갈린다** — 맺힌 것과 맺히는 것은 같은 것의 두 때라 색만
// 갈렸지만, 이것은 그 알이 없어지고 껍질만 남은 자리다 (원천의 available → depleted 가
// 실루엣부터 갈린 것과 같은 어법). 붉은 것(C · c · 6)이 한 점도 없고 마른 껍질색(g · k)의
// 벌어진 조각만 뿌리 마디에 매달렸다 — 빛이 꺼진 것이 멀리서도 먼저 읽힌다
const LIFE_ROOT_CLUTCH_SPENT = [
  '................',
  '................',
  'uu............uu',
  '.uUUu......uUUu.',
...
// ── 뿌리의 알 — 탄생지 둘째 (C023 ADDED · SPEC-008) ──────────────────────
//
// **알집과 눈으로 갈려야 한다.** 알집은 뿌리 마디 아래 큰 덩이 여덟이 뭉친 것이고, 이것은
// 뿌리를 따라 흩어진 **작은 붉은 점 여럿**이다 (Play §5.4 · spec Observable Result 5).
// 갈리는 축 셋 — 크기(덩이 vs 점) · 놓임(뭉침 vs 흩어짐) · 뿌리의 위치(위 vs 아래).
// 색 계통은 같다: 둘 다 이 세계의 붉은 것이므로 색으로 갈면 **다른 계통**으로 읽힌다.
//
// phase 별 외형의 어법은 알집 그대로다 — 멎음/맺힘은 색만 갈리고(같은 것의 두 때),
// 터진 자리는 실루엣부터 갈린다(있던 것이 없어졌다).

// 멎어 있는 뿌리의 알 — 뿌리 위에 마른 붉은 점 여섯이 흩어져 있다
const LIFE_ROOT_EGGS_DORMANT = [
  '................',
  '................',
  '................',
  '.....x.....x....',
...
// 맺히는 뿌리의 알 — 같은 자리의 점들에 **붉은 빛이 돈다** (알집의 그 어법 그대로)
const LIFE_ROOT_EGGS_BINDING = [
  '................',
  '................',
  '................',
  '.....C.....C....',
...
// 터지는 순간의 뿌리의 알 — 점마다 빛이 넘친다 (한 tick · 알집의 그 자리와 같은 규율)
const LIFE_ROOT_EGGS_BORN = [
  '................',
  '................',
  '................',
  '....666...666...',
...
// 터진 뒤의 뿌리 마디 — 점이 없어지고 마른 껍질 부스러기만 남았다 (실루엣부터 갈린다)
const LIFE_ROOT_EGGS_SPENT = [
  '................',
  '................',
  '................',
  '................',
...
// ── 탄생이 남기는 원천 둘 (C023 ADDED · Material §6.2 By-product) ─────────
//
// **둘은 같은 재료의 다른 형태다** (광식충 허물). 그래서 껍질의 색(m · k · x)이 둘을
// 묶고 **크기가 둘을 가른다** — 허물 조각(molt-litter)이 흩어져 깔린 것, 풀줄기에 걸린
// 조각(husk-shard)이 매달린 것인 데 반해, 이 둘은 **터진 껍데기 하나**가 그 자리에 남은
// 것이라 실루엣이 오목하게 벌어져 있다 (알이 나간 자리).
//
// 세 상태의 규율은 앞의 것들 그대로다. 다만 **되돌리는 것이 시간이 아니다** — 이 세계에서
// 그것을 다시 세우는 것은 다음 탄생이고, 그 사실은 그림이 아니라 물었을 때 판이 말한다.

// 빈 껍질 — 알집이 터진 자리에 남은 **큰** 껍데기. 벌어진 입이 위를 향한다
const SOURCE_CLUTCH_HUSK = [
  '................',
  '................',
  '..mk........km..',
  '..mkx......xkm..',
...
// 다 캐 간 자리 — 껍데기가 주저앉아 조각 몇만 남았다
const SOURCE_CLUTCH_HUSK_DEPLETED = [
  '................',
  '................',
  '................',
  '................',
...
// 되돌아오는 중 — 주저앉은 껍데기가 다시 서기 시작한다 (붉은 결 x 가 한 점 돌아왔다).
// **세계 시간이 이것을 하지 않는다** — 다음 탄생이 한다 (R4). 그림만 그 어법을 함께 쓴다
const SOURCE_CLUTCH_HUSK_RECOVERING = [
  '................',
  '................',
  '................',
  '................',
...
// 작은 껍질 — 뿌리의 알이 남긴 **작은** 껍데기. 빈 껍질과 같은 색 · 같은 오목한 실루엣이고
// 크기만 갈린다 (그래서 한 화면에 둘이 서면 "같은 것의 큰 것과 작은 것" 으로 읽힌다)
const SOURCE_EGG_HUSK = [
  '................',
  '................',
  '................',
  '................',
...
// 다 캐 간 자리 — 조각 둘만 남았다
const SOURCE_EGG_HUSK_DEPLETED = [
  '................',
  '................',
  '................',
  '................',
...
// 되돌아오는 중 — 조각이 다시 모여 껍데기의 밑이 섰다
const SOURCE_EGG_HUSK_RECOVERING = [
  '................',
  '................',
  '................',
  '................',
...
// 셋 다 낮고 작다 — 걸어 다니며 줍는 것이지 찾아가는 것이 아니다. 계통은 색이 가른다:
// 자갈은 광석의 붉은색(C·c·G), 껍질 조각은 허물의 껍질색(m·k·x), 빛 갓은 균사의 흰색(w·z)이다.
// 세 상태(있음 · 바닥남 · 되돌아오는 중)의 규율은 앞의 것들 그대로다.

// 흙 위에 흩어진 붉은 자갈 — 낮고 작은 붉은 알갱이 여섯. 노두의 결정이 부서진 것처럼 각졌다
const SOURCE_ORE_PEBBLE = [
  '................',
  '................',
  '................',
  '................',
...
// 자갈이 주워졌다 — 붉은 것이 하나도 없고 자갈이 놓였던 자리의 마른 흙 자국(d·e)만 남았다
const SOURCE_ORE_PEBBLE_DEPLETED = [
  '................',
  '................',
  '................',
  '................',
...
// 되돌아오는 중 — 뿌리가 밀어 올린 조각이 흙을 뚫고 반쯤 드러났다 (i·I 는 다시 도는 색)
const SOURCE_ORE_PEBBLE_RECOVERING = [
  '................',
  '................',
  '................',
  '................',
...
// 풀줄기에 걸린 껍질 조각 — 가는 풀줄기 셋(L·l)에 허물 조각(m·k·x)이 걸려 있다.
// 밑동의 조각(허물)이 깔려 있는 것이라면 이것은 **줄기를 타고 오른 것**이다
const SOURCE_HUSK_SHARD = [
  '................',
  '......l.........',
  '.....mkl..l.....',
  '.....mxl..l.....',
...
// 조각이 다 떼어졌다 — 줄기만 서 있다
const SOURCE_HUSK_SHARD_DEPLETED = [
  '................',
  '......l.........',
  '......l...l.....',
  '......l...l.....',
...
// 되돌아오는 중 — 줄기 하나에 새 조각 하나가 다시 걸렸다 (i·I 는 다시 도는 색)
const SOURCE_HUSK_SHARD_RECOVERING = [
  '................',
  '......l.........',
  '......l...l.....',
  '......l...l.....',
...
// 어둠에서 희게 빛나는 갓 — 균사(w·z)와 같은 흰색의 갓 셋이 짧은 자루(J) 위에 섰다.
// 낮에는 오므라들어 거기 없다 — 이 그림은 밤에만 선다 (dayPhases)
const SOURCE_GLOW_CAP = [
  '................',
  '................',
  '.......www......',
  '......wwwwwz....',
...
// 갓이 따졌다 — 자루 밑동(j·J)만 남았다
const SOURCE_GLOW_CAP_DEPLETED = [
  '................',
  '................',
  '................',
  '................',
...
// 되돌아오는 중 — 자루 끝에 작은 갓이 다시 맺혔다 (z 는 흰 것의 그늘 — 아직 빛나지 않는다)
const SOURCE_GLOW_CAP_RECOVERING = [
  '................',
  '................',
  '................',
  '................',
...
const PIXEL_MAPS: Record<string, string[]> = {
  ...
  // 열다섯째의 셋 (C030) — 표에 세 줄이 늘 뿐이다. 열넷이 열다섯이 되어도 어법은 그대로다
  'source:wall-ember:available': SOURCE_WALL_EMBER,
  'source:wall-ember:depleted': SOURCE_WALL_EMBER_DEPLETED,
  'source:wall-ember:recovering': SOURCE_WALL_EMBER_RECOVERING,
  ...
  // 탄생지의 phase 가 넷이 되고 탄생지가 둘이 되었다 (C023) — 표에 여섯 줄이 늘 뿐이고
  // 키의 어법도 고르는 규칙도 그대로다. 눈으로 갈려야 하는 것은 넷이 아니라 **셋**이다
  // (맺힌 채 멎음 · 속에서 맺힘 · 터진 것) — born 은 한 tick 이라 사람이 볼 일이 드물고,
  // 눈에 보이는 "터졌다" 는 spent 다 (spec 기본형 ⑦)
  'life:root-clutch:born': LIFE_ROOT_CLUTCH_BORN,
  'life:root-clutch:spent': LIFE_ROOT_CLUTCH_SPENT,
  'life:root-eggs:dormant': LIFE_ROOT_EGGS_DORMANT,
  ...
  'source:clutch-husk:recovering': SOURCE_CLUTCH_HUSK_RECOVERING,
  'source:egg-husk:available': SOURCE_EGG_HUSK,
  'source:egg-husk:depleted': SOURCE_EGG_HUSK_DEPLETED,
  'source:egg-husk:recovering': SOURCE_EGG_HUSK_RECOVERING,
  // 흩어진 것 셋 (RoomBearsMaterial 실주행 판정) — 표에 아홉 줄이 늘 뿐이다. 어법은 그대로다
  'source:ore-pebble:available': SOURCE_ORE_PEBBLE,
  'source:ore-pebble:depleted': SOURCE_ORE_PEBBLE_DEPLETED,
  'source:ore-pebble:recovering': SOURCE_ORE_PEBBLE_RECOVERING,
  'source:husk-shard:available': SOURCE_HUSK_SHARD,
  'source:husk-shard:depleted': SOURCE_HUSK_SHARD_DEPLETED,
  'source:husk-shard:recovering': SOURCE_HUSK_SHARD_RECOVERING,
