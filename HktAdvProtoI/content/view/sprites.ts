...
const PALETTE: Record<string, string> = {
  ...
  // (≈ 28~40°), 그 위에 흰 속(6)을 두어 "붉게 빛나는 것" 이 아니라 "타고 있는 것" 으로 읽히게
  ...
  // 색상만으로는 갈리지 않는다 — 그래서 속(6)을 자락보다 뚜렷이 밝게, 테(8)를 뚜렷이 어둡게
  // 두었다. 배어 든 색은 한 값으로 눕고 타는 것은 밝고 어두운 값을 함께 가진다.
  '6': '#ffd27a', // 잉걸의 속 (가장 뜨거운 자리) — 이 표에서 가장 밝은 따뜻한 값이다
  '7': '#f2913a', // 잉걸의 결 (그늘)
  '8': '#8a3d10', // 식은 잉걸 — 땅에 닿는 밑동이자, 다 캐 간 자리에 남는 몇 점
  '.': '',
};
...
// available > recovering > depleted 로 간다: 가장 뜨거운 속(6)은 available 에만 서고,
// recovering 은 결(7)만 몇 점 돌아오며, depleted 에는 식은 몇 점(8)뿐이다.
