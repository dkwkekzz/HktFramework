// 명령 표면 결정 Layer 단독 테스트 — World 미기동, Fixture 만으로
...
describe('쓰는 중 안내 (04 commandSurface.guide)', () => {
  ...
  it('아직 쓰는 중인 낱말은 탓하지 않는다 — 이어질 가망이 남아 있는 동안 (결함 수정)', () => {
...
describe('지목 (04 commandSurface.designation)', () => {
  it('지목하지 않으면 대상 자리가 비고 그대로 걸린다', () => {
    const invocation = invocationOf('set-attribute hp 50', entries, snapshot, OFF, codeText);
  ...
  it('세계에 있는 Id 를 앞에 두면 그것이 대상이 된다', () => {
    const invocation = invocationOf('set-attribute npc-1 hp 5', entries, snapshot, OFF, codeText);
    expect(invocation).toEqual({
      kind: 'world',
      commandId: 'set-attribute',
  ...
  it('대상이 될 수 없는 존재는 후보에 없다 — refers 가 가리키는 것만 (결함 수정)', () => {
...
describe('표면 전체 (resolvePresentation)', () => {
  ...
  it('충돌체·속성 관찰은 그대로다 — 이번 변경이 다시 만들지 않았다', () => {
    const scene = resolvePresentation(snapshot, undefined, { debugObserve: true, inspect: true });
