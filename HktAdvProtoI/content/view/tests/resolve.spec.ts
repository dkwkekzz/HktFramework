...
describe('resolvePresentation (Semantic → Render Plan)', () => {
  ...
  it('되돌아오지 않는다고 선언한 시트는 1회 재생이다 — 쓰러진 몸이 다시 일어서지 않는다', () => {
    ...
    const snapshot: GameViewSnapshot = {
      ...
      scene: 'mining-field',
      region: { id: 'mining-field', hash: '00000000', disturbance: { value: 0, threshold: 300, phase: 'dormant' as const }, memory: { turns: 0, awakenings: { times: 0 }, passages: [], births: [] } },
...
describe('결정 Layer 의 유연 대응 — 미등록 항목도 기본 결정으로 소화한다', () => {
  it('미래 Cycle 의 semantic(새 role·HUD id·사유 코드)도 기본 결정으로 해석된다', () => {
    const snapshot: GameViewSnapshot = {
      ...
      scene: 'cavern',
      region: { id: 'cavern', hash: '00000000', disturbance: { value: 0, threshold: 300, phase: 'dormant' as const }, memory: { turns: 0, awakenings: { times: 0 }, passages: [], births: [] } },
      standingConditions: [], // C006 ADDED
    // C017 — 봉투에 그 방의 자국이 실린다. 이 시나리오가 보는 것이 아니므로 빈 목록이다
    tracks: [],
    // C018 — 봉투에 그 방을 지나는 것들이 실린다. 이 시나리오가 보는 것이 아니므로 빈 목록이다
