...
| **R14** | **규칙 가독성의 최소 힌트는 표현이 진다** (§17 "규칙의 효과를 관찰할 수 있는가 · 실패 원인을 추적할 수 있는가"). 규칙을 품은 방은 자기 규칙의 **형**(무엇을 하면 무엇이 달라지는가)을 말하고, 잠긴 Connector 는 어느 갈래의 규칙이 쥐고 있는지까지 말한다 — 답(어느 패턴 · 어느 철)은 말하지 않는다. 세계는 "지금 열렸는가" 만 싣는 §10 의 규율 그대로이고, 힌트는 content/view 의 표다. RuleBoundRoom 실주행 판정이 세웠다 (그 Play 확정 8~10) | 실주행 |
