...
| 지역 하나를 쓰는 공식은 고정돼 있다 — Concept §17 일곱 질문 · Region §15 12단계 · 재료 계약 표 다섯 · 철 덧씌움 넷 · 생명 계약(탄생지 · 개체군 · 관계) · 접근 계약(Lock · 성질 · 흔적 · 답의 종류) · 검사 ①~㊷ | ① ② ②-부속 넷 |

공식이 고정됐다는 것은 **생성기가 돌릴 수 있다**는 뜻이다. 기획 여섯이 무거웠던 이유는 그것이 지역이 아니라 **문법**이었기
때문이고, 문법은 이제 닫혔다. 생명 계약([L2-World-Life.md](L2-World-Life.md) §3.5)과 접근 계약([L2-World-Access.md](L2-World-Access.md) §5.4)이 이 공식에 마지막으로 든 항이다 —
...
| 등급 | 조건 | 공정 | 예 |
|---|---|---|---|
| **A. 데이터만** | Global Rule + 공통 계약(재료 · 철 · 생명 · 접근 · 위험 태그 · 안전 조건)으로 성립한다 | Play 없음 · Cycle 없음. RegionSpec 하나(space + resourceEcology + phases + ecology + access) + graph 한 줄 + view 표 한 줄. 검사 ①~㊷ 통과 · Human 판정 | 가스로 가득 찬 마을 — hazard/climate(독성) + settlement(왜 사는가) + 재료 생태(가스 → 무엇이 남는가) + 생명(가스에서 무엇이 태어나고 무엇을 먹고 무엇을 남기는가 — 원인 없는 생물 금지 · Life F2) |
...
ENGINE 레인 B   T1~T6 이 다 섰다 — 남은 것은 값이 늘어야 서는 것들뿐이다.
               ...
               T6 의 편중 요약은 그 ecology 와 함께 온다 (지금은 구조의 수만 움직인다).
...
편중 요약      지금은 구조의 수만 움직인다. 재료 계통의 분포(⑲ ⑳ ㉒)는 뼈대가 원천마다
               원인과 되돌아옴을 내야 실린다 — C022 의 것이다
