...
T1 ── T2 ── T3 ─┬─ T5 ── T6 ── HundredRooms
                └─ T4 ─┘
T4 는 지금 — ENGINE 레인이라 Cycle 실주행과 병행한다.
```
...
content/authoring/examples/            세계에 들이지 않은 본보기 brief — 생성기의 시험이 쓴다 (등급 A 의 예)
...
```text
ENGINE 레인 B   T3 의 나머지(phases)는 C016 뒤 · ecology 는 C022 뒤.
...
```text
T4 등급 판정기  착수 가능. brief 의 "요구" 와 세계 사실을 계약 목록과 대조해 A · B · C 로 가른다.
