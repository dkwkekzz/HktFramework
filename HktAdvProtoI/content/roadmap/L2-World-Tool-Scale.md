...
T1 ── T2 ── T3 ─┬─ T5 ── T6 ── HundredRooms
                └─ T4 ─┘
T3 의 절반은 지금 — ENGINE 레인이라 Cycle 실주행과 병행한다.
```
...
```text
ENGINE 레인 B   T3 는 절반(space · graph · resourceEcology)을 지금, 나머지(phases)는 C016 뒤.
...
```text
T3 뼈대 생성기  절반(space · graph · resourceEcology) 착수 가능 — T4 도 T2 뒤이므로 함께 열려 있다.
