...
살아 있는 문서 하나 — **현재 상태만** (CLAUDE.md 원칙 10). 작업에 올린 **원본 기획 전부**가 §1 에 행으로 서고, 원본마다 어느 Play 가
무엇을 덮었고 무엇이 남아 어디로 가는지가 적힌다. §2 는 있는 Play 의 상태, §3 은 쓸 Play, §4 는 지금부터의 기획 작업 큐다.
[README.md](README.md) 가 **주입 순서**(층과 행)를, [play/README.md](play/README.md) 가 **있는 Play 의 증명 내용**을 소유한다면, 이 문서는
**원본 → Play 의 덮임과 Play 를 쓰는 일의 순서**를 소유한다. Cycle 의 상태는 여기 두지 않는다 — [../../STATE.md](../../STATE.md) §1 · §2 와
각 Play 의 Cycle Breakdown 체크박스다.
