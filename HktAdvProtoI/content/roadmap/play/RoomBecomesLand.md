...
```text
1. 경사 몇 도부터 못 가는가 — traversable 임계. 수치는 Human 의 것 (제안: 45°). 이것이 2층에서 높이가 몸에 닿는
   유일한 형태다 — 승인? (아니면 2층은 시각만이고 막힘은 3층으로)
2. 백왕령의 지형 사실 — 산맥은 북쪽 · 강은 동서 · 거목은 도시 곁. Concept §3 은 셋의 존재만 주었고 방향은 이 Play 가
   붙였다. 승인? 다른 배치?
3. 강을 건너는 자리(다리)가 하나 — 세계 사실. 승인?
4. TERRAIN_RESOLUTION — 세계 격자 해상도(제안: 1m). 결정론 상수라 Human 의 것
5. 표면 넷(평지 · 경사 · 급경사 · 젖음)의 이름 — 태그는 컨텐츠 어휘다. 이 넷으로 시작 — 승인?
6. RuleBoundRoom 보다 먼저 할 것인가 — 둘은 독립이다. 도구를 먼저 닫을지, 규칙을 먼저 볼지는 Human 의 순서
```
