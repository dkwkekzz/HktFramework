...
상태: **승인 대기** (Human 질문 열 — 아래 목록). 주입 원문은
...
**Human 질문 1** 이 받고, 답이 오면 그것이 컨텐츠 층의 새 행(M3 숲의 재료 계통)이 된다.
...
FINITE_WORLD_STATE      쓰지 않는다                 기본 공급을 유한 원천에 걸지 않는다 (S7 · Human 질문 5)
```
...
|---|---|---|---|---|---|
| `SEED_ROOT_ORE` *(가칭 — 질문 1)* | 거대 수목이 뿌리로 빨아올리는 그 광물 (Concept §4) | ① 광맥의 노두 ② 뿌리혹의 축적체 ③ 물길이 갈아 실어 온 알갱이 — **같은 것의 세 순도** | **UNRESOLVED (질문 2)** — 흙을 물들인다는 것만 세계에 이미 있다 | MIGRATORY(노두) · CONDITIONAL_RENEWABLE(뿌리혹) · EVENT_SCARCE(퇴적) | **미정 — 후속 층** |
| `SEED_MOLT` *(가칭 — 질문 1)* | 그 광물을 먹은 곤충이 벗은 것 | 허물 · 조류가 떨어뜨린 잔해 | **UNRESOLVED (질문 2)** | BASELINE_RENEWABLE | **미정 — 후속 층** |
| `SEED_FUNGUS` *(가칭 — 질문 1)* | 포식자 사체에서 자라는 특수 균류 (Concept §4) | 사체 위 균사 · 그것이 분해한 흙 | **UNRESOLVED (질문 2)** — 거목을 키운다는 것만 세계에 이미 있다 | CONDITIONAL_RENEWABLE | **미정 — 후속 층** |
...
```text
넘긴다   Material Seed 셋 · 각각의 자연 형태 · 공급 유형과 원천 위치 · 관찰 가능한 성질(질문 2 의 답)
...
```text
① Material Seed 의 고유한 세계적 성질   → Human 질문 2. 없으면 넘길 것이 형태뿐이다
② 회복 원인의 시간 규모                → Human 질문 3 (수치)
③ 채취 단위                          → Human 질문 4 (수치)
④ 살아 있는 운반체(곤충 · 조류)의 이동   → 3층. 2층은 그것이 남긴 자리만 세운다
⑤ Finite Source 를 반복 성장에 쓸 것인가 → 이 Play 는 쓰지 않는다 (질문 5). 후속 층이 뒤집을 수 있다
```
...
게임 의미(수치 · 시스템 원리의 확정 · 세계관 사실)는 지어내지 않았다. 열 개다 — 1~4 는 **빈칸**이고
5~10 은 **제안에 대한 승인**이다.

```text
1. 재료의 이름
   Concept §4 는 관계만 주고 이름을 주지 않았다 — "특정 광물" · 곤충 · 조류 · 포식자 · "특수 균류".
   (a) 정식 이름인 `BIO_ORE_FIELD 생체 광석 지대` 의 그 "생체 광석" 이 §4 의 그 광물인가?
       그렇다면 SEED_ROOT_ORE 의 이름은 "생체 광석" 이고 새로 지을 것이 없다.
   (b) 허물 계통(SEED_MOLT) 과 균류 계통(SEED_FUNGUS) 의 이름.
   답이 오면 이것이 컨텐츠 층의 새 행(M3 숲의 재료 계통)이 된다.

2. 관찰 가능한 성질 — 그 광물이 세계에서 무엇을 하는가
   열을 먹는가 · 진동에 우는가 · 빛에 무너지는가 · 살아 있는 것 안에서만 자라는가.
   이것이 후속 층에 넘길 유일한 알맹이다 (S10) — 없으면 형태만 넘어간다.
   허물과 균류에도 각각 하나씩.

3. 회복의 시간 규모 (결정론 상수)
   거목의 축적 한 바퀴 · 탈피 주기 · 물길이 불어나는 주기. 관찰자가 한 번 다녀오는 사이에
   눈에 띄어야 하는가, 여러 번 다녀와야 하는가.

4. 채취 단위 — Source 하나에서 몇 번 캐지는가 (지금 Deposit.resourceAmount 의 자리).

5. 이 Play 는 FINITE_WORLD_STATE 를 쓰지 않는다 — 승인 요청.
   S7 이 기본 성장 공급에 유한 원천을 금지했고, 이 숲의 일곱 Source 는 전부 기본 공급이다.

6. 생물은 세우지 않는다 — 승인 요청.
   곤충 · 조류 · 포식자는 **남긴 것**(허물 · 잔해 · 사체)으로만 세계에 있다. 살아 움직이는 것은 3층이고,
   그때 여기 선 Source 가 그 생물에 그대로 매달린다 (L2-World-Material §2.2).

7. 채취 행위는 기존 채광 하나 — 승인 요청.
   뜯다 · 긁다 · 건지다를 따로 만들지 않는다. Source 종류마다 다른 것은 문구와 결과이지 행위가 아니다.

8. 탐험대 폐허의 선광 더미 (`RUIN_SPOIL`) — 새 세계 사실 승인 요청.
   폐허 자체는 정식이지만, "앞서 온 탐험대가 캐다 버리고 간 더미" 는 이 Play 의 제안이다.
   경계부에서 재료 계통을 미리 읽게 하는 자리이자, 먼저 온 사람이 있었다는 흔적이다.

9. 백왕령의 Isolation Reason — 승인 요청.
   "숲의 계통은 백왕령에 들어오지 않는다 — 산맥과 강이 막기 때문이고, 그것이 백왕령이 안전한 이유와
   같은 조건이다." 새 사실을 짓지 않고 W2 를 그대로 쓴다.

10. 자리와 번호 — 승인 요청.
    이 Play 를 2층 넷째로 두고 RuleBoundRoom(C008~C010) 뒤에 세운다. Cycle 은 C011~C014.
    Region State 와 세계 과정이 먼저 서야 재료의 생애가 성립한다.
```
