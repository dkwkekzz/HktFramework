...
**흩어진 것** (실주행 판정 · 확정 12) — 방의 중심이던 원천 하나 곁에 **작은 것 여럿**이 흩어져 선다: 흙 위의 붉은
자갈(생체 광석) · 풀줄기에 걸린 껍질 조각(광식충 허물) · 밤에만 희게 빛나는 갓(거목균). 찾아가는 것이 아니라 걸어
다니며 줍는 것이고, 재료를 늘린 것이 아니라 같은 세 재료의 **다른 순도**(A.1)를 늘린 것이다. 둘레 흔적은 방 바닥보다
한 단계 짙되 반지름 4 로 작다. 빛 갓은 **낮밤을 탄다** (occurrence 의 낮밤 축 — Time §2.4 ③) — 낮에는 그 자리에
없고 사유는 철의 것과 갈린다 (not-this-hour).
...
V12′ 흩어진 것 셋의 그림 아홉(있음 · 바닥남 · 되돌아오는 중) · 이름 셋 · 사유 코드 not-this-hour 의 문구 (실주행 판정)
W-낮밤  ResourceSource 가 낮밤 출현(dayPhases)을 밝힐 수 있다 — 철 출현(occurrence)과 같은 어법의 다른 축.
        그 때가 아니면 그 자리에 없고, 되돌아옴도 멎는다 (실주행 판정 · Time §2.4 ③ 의 낮밤 쪽)
```
...
[x] C013 — 세계가 되돌린다: 회복 세계 과정 + Supply Mode 넷 + 회복 흔적 + MIGRATORY 의 자리 이동
           (뿌리 곡선의 다음 마디). 균류를 캔 것이 거목 쪽 회복을 늦추는 고리를 화면에서 읽는다
[x] C014 — 조건과 흐름, 그리고 보고: 조건부 기회(물길 주기 → 어귀 퇴적) + 부산물 기회(둥지의 균류) +
...
실주행 판정이 **순도를 셋 더했다** (종류가 아니다 · 확정 12):

| Seed | 더한 자연 형태 | 관찰 가능한 성질 | 공급 | 회복 원인 |
|---|---|---|---|---|
| `BIO_ORE` | ④ 흙 위에 흩어진 붉은 자갈 (`ore-pebble`) — 가장 옅은 순도 | 뿌리가 밀어 올린 조각이 흙 위에 낮게 깔린다 · 노두의 결정처럼 각졌다 | BASELINE_RENEWABLE | 비가 흙을 씻어 조각이 드러난다 (`pebble-wash`) |
| `ORE_EATER_MOLT` | 풀줄기에 걸린 껍질 조각 (`husk-shard`) | 광식충이 줄기를 타고 오르며 벗은 것 — 밑동의 허물이 깔린 것이라면 이것은 오른 것이다 | BASELINE_RENEWABLE | 벌레가 다시 벗는다 (`husk-shed`) |
| `GIANT_TREE_FUNGUS` | 어둠에서 희게 빛나는 갓 (`glow-cap`) — **밤에만 선다** | 낮에는 흙 속으로 오므라들어 거기 없다 · 균사와 같은 흰색 | CONDITIONAL_RENEWABLE | 다음 밤에 다시 핀다 (`night-bloom`) |
...
흩어진 것 열아홉 (실주행 판정 · 확정 12) — 위 표와 같은 어법이되 **여럿**이라 방마다 묶어 적는다. 자리는 각 방
Description 의 resource point 가 소유하고, 둘레 흔적은 방 바닥보다 한 단계 짙은 반지름 4 의 원 하나씩이다.

| 방 | 자갈 (`ORE_PEBBLE_*` · TERRAIN · Baseline · 1회 · 45 s) | 껍질 조각 (`HUSK_SHARD_*` · PLANT · Baseline · 2회 · 75 s) | 빛 갓 (`GLOW_CAP_*` · FUNGUS · By-product · 1회 · 90 s · **밤에만**) |
|---|---|---|---|
| `FOREST_EDGE` | 1 | 1 | 1 |
| `FOREST_DEEP` | 2 | 1 | 1 (+ 스밈에만 서는 껍질 `SEEP_CRUST_DEEP` — Time 의 것) |
| `EXPLORER_RUIN` | 1 | 2 | — |
| `PREDATOR_NEST` | — | 1 | 2 |
| `BIO_ORE_FIELD` | 3 | 1 | — |
| `RED_EYE_TREE` | 1 | — | 1 |
...
흩어진 것:    경계부부터 핵심부까지 방마다 여럿 — 걸어 다니며 줍는 것 (실주행 판정 · 위 표). 중간부(FOREST_DEEP)에도
             서지만 그것은 "원천은 없고 방향이 있다" 를 깨지 않는다 — 방향은 여전히 흔적이 주고, 흩어진 것은 작다
...
실주행 판정 (Human 답 · Cycle 없이 반영) — "재료가 너무 적고 채집하는 재미가 부족하다. 다양하고 더 동적이고 여러
채집물을 보여 주는 기반을 설계하고 작업."
12. **흩어진 것** — 방의 중심 원천 곁에 작은 것 여럿이 선다 (A.2 의 둘째 표 · 열아홉). 재료(Seed)는 셋 그대로이고
    순도가 셋 는다 (A.1 의 둘째 표): 자갈 · 줄기의 껍질 조각 · 밤의 빛 갓. 값(1회 45 s · 2회 75 s · 1회 90 s)은 D3 · D4
    의 어법 그대로 얕은 것은 빨리 · 여럿인 것은 적게.
13. **원천이 낮밤을 탄다** — Time §2.4 ③ 의 낮밤 축이 처음 쓰인다 (빛 갓). 그 때가 아니면 거기 없고, 사유는 철의
    "이 철이 아니다" 와 갈리는 "지금은 때가 아니다" 다. 밤이 감추는 것이 아니라 종류를 바꾼다 (RoomNeverSame Q25 의 답).
14. Q15~Q20 의 답 — 흙의 다섯 단계 · 되돌아옴의 길이 · 뿌리 선의 역할 · 원천의 분포 · 판이 재료 이름을 말하지 않는
    것은 지금 그대로 둔다.
```
...
| `ORE_PEBBLE_*` | 1 | 한 알이 한 번 — 여럿이므로 하나는 적다 (실주행 판정 · 45 s) |
| `HUSK_SHARD_*` | 2 | 줄기 하나에 조각 둘 (75 s) |
| `GLOW_CAP_*` | 1 | 갓 하나 = 한 번. 밤에만 선다 (90 s — 다음 밤이 오기 전에 돌아온다) |
