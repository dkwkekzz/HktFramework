...
```text
[x] C019 — 고개 너머 다른 갈래: 얼음 협곡 · 빙결 협곡 방 둘 + 고개 활성 + 위험 갈래 셋(눈보라 = 시야 · 절벽 = 통행 · 결정면 = 접촉 표시).
           숲과 겹치지 않는 hazard 태그
[x] C020 — 추위의 재료 생태: 빙정석 원천 넷(Carrier 넷) + Trace + 회복(옆 면으로) + 협곡의 Isolation/Outflow.
           협곡이 요구하는 것(열 결정)이 협곡에 없다는 문
[x] C021 — 추위가 고개를 넘는다: 스밈에 백왕령 북쪽 조건이 약해진다(Region 간 태그 덧씌움 · carrier WIND) +
