...
| [RoomBearsLife.md](RoomBearsLife.md) | 방이 생명을 낳는다 — 재료에는 주인이 있다. 탄생은 세계의 무언가를 먹고, 흔적을 먼저 남기고, 반복 출현은 스폰이 아니라 조건의 회복이다 | M6 붉은 알집 (M1·M3 을 깊게 한다) | C022~C024 |
...
| **생명의 성립과 탄생** (L2-World-Life) | F1~F13 전부 → **Life** (생명의 정의 · 네 탄생 방식 · 탄생의 조건과 소비 · 전후의 흔적 · 개체군 값 · 스폰이 아닌 회복 · 검사 ㉗~㉛) | 태어난 개체의 몸 · 감각 · 지식 · 행동 · 죽음 · 성장 단계 · 능력치 → 3층. 성별과 구체적 번식 · 유전과 변이 → 3층 이후. 플레이어가 탄생에 개입하는 구체적 Action → 3층 이후 (2층의 개입은 채취뿐). 요정의 원리 결속 → 7층. 최초의 생명 → **확정하지 않는다** (F12) |
