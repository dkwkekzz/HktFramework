...
방 아홉 · Connector 열둘 · 일방향 둘(추락 · 물길) · 닫힘 하나 · 중첩 하나. "그럴듯하게 복잡한" 것은 방의
수가 아니라 **연결 종류의 수**다 — 길 · 오솔길 · 문 · 추락 · 물길 · 닫힌 문 · 고개.
...
플레이 층은 제안으로 채웠다. 아래는 세계 사실 또는 수치라 AI 가 정하지 않았다. 답과 함께 승인된다.

```text
1. 거대 악마의 숲을 방 여럿로 쪼갠다 — 가장자리(FOREST_EDGE) · 안쪽(FOREST_DEEP) 두 방을 새로 짓고, WE §3 의
   POI 셋과 Landmark 를 각각 방으로 둔다 (§5.8). 두 방의 이름은 이 Play 가 붙인 것이다 — 승인? 다른 이름?
2. 방마다의 depth 배정 (§5.8) — civil 하나 · outer 하나 · wild 다섯 · deep 둘. 승인?
3. 세계 사실 셋 — ① 거목 내부의 추락이 심장 호수로 떨어진다 ② 호수의 물길이 숲 안쪽의 다른 자리로 나온다
   ③ 고대 문은 이 Play 동안 닫혀 있다. 승인?
4. 방의 크기 — 기본 40×40 (지금 WORLD_BOUNDS). 거목 내부 세계는 80×80. 수치는 Human 의 것이다.
5. 붉은 황야 · 얼음 협곡 — Connector 만 두고 방은 짓지 않는다 (건너면 거절 · 사유 "아직 없는 곳"). 아니면 빈 방을 둔다.
6. 다른 관찰자는 같은 방에 있을 때만 보인다 (W5) — 세계 규칙으로 확정?
7. 도구 1단계의 순서를 그래프 → 방 → 지형으로 바꾼다 (E4). 높이·표면·scatter 는 이 Play 뒤로 — 승인?
```
