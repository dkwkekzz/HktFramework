...
```text
방은 단순하다        이 Play 의 Region 표현 = 평평한 바닥 하나. 바닥 · 이름 · 깊이 색 · 출구 표식뿐. 지형·자연물은 없다.
                    표현이지 정의가 아니다 — Region 이 무엇인가는 넷째 줄이 말한다
...
중첩 둘. 고대 문은 C004 가 데이터로 열었다 (확정 3 · RuleBoundRoom 확정 4).
...
**한 방의 출구 수**다 — 그것도 상한을 두지 않고 표현이 받는다 (확정 9).
...
[x] C003 — 작은 문, 큰 방, 돌아올 수 없는 길: 거목 → 내부 세계(중첩 · 더 큰 방) → 추락 → 심장 호수 → 물길 → 다른 자리로
[x] C004 — 폴리싱은 데이터로: graph 와 표만 바꿔 방을 더하고 색을 바꾸고 문을 여는 것을 코드 diff 0 으로 실측.
...
3. 세계 사실 둘 — 거목 내부의 추락은 심장 호수로 · 호수의 물길은 숲 안쪽의 다른 자리로.
   ...
   이 Play 의 확정이 아니다.)
4. 이 Play 가 고른 크기 — 40×40 (지금 WORLD_BOUNDS 와 같다) · 거목 내부 세계 80×80. extent 데이터 값이지 기준이나
   기본값이 아니다 — Region 크기에 상한도 표준도 없다 (불변 조건 넷째).
...
9. **한 방의 출구 수에도 상한을 두지 않는다.** 총 수와 같은 이유다 — 세계가 그렇게 생겼으면
   그렇게 생긴 것이고, 방 하나에 출구가 서른이어도 세계는 틀리지 않았다.
   그러므로 이것은 세계가 답할 문제가 아니라 **표현이 감당할 문제**다 (아래 "출구가 많은 방").
10. **방 이름 짓기는 위임됐다** (Human "알아서") — L2-World-Region §5.2 가 그 결정을 소유한다.
```

### 출구가 많은 방 — 상한 대신 표현이 받는다 (확정 9)

상한을 두지 않기로 했으므로, 출구가 늘 때 무너지는 자리는 **고쳐야 할 것**이지 막아야 할 것이 아니다.
지금 아는 것만 적는다 — 고치는 것은 이 Play 가 아니라 그것이 실제로 걸리는 Play 의 Cycle 이다.

```text
프롬프트가 하나만 말한다   engine/view-kernel/scene/interaction-choice.ts 가 눈앞의 것 하나를 고른다.
                       출구 다섯에서 이미 넷은 말해지지 않는다 (C002 가 부채로 남겼다)
표식이 겹친다            출구마다 표식 하나. 한 자리에 여럿이 모이면 서로 가린다
한 화면에 안 들어온다      C002·C003 이월 — 40×40 도 시점을 돌려야 다 센다
```

세 가지 다 "세계에 출구가 몇 개냐"가 아니라 **"한 화면·한 순간이 몇 개를 감당하냐"** 의 문제다.
세계는 상한 없이 크고, 좁은 것은 언제나 보는 쪽이다.
