...
상태: **승인됨** (Human 승인 1회 — "Human 질문 열 전부 제안대로 승인". 확정 사항 1~11 이 문서 끝에 있다). Cycle Breakdown 의 체크박스만 앞으로 갱신된다.
계약은 [L2-World-Foundation.md](../L2-World-Foundation.md) (확정 — 이 Play 와 한 번에 승인됐다).

말 둘을 먼저 푼다. **기억**(history)은 방이 자기에게 일어난 일을 센 것이다 — 몇 번 캐였는가 · 마지막 고갈은 언제였는가 · 고래가 몇 번 지났는가.
지금 세계는 그것을 되돌아올 때 0 으로 지운다. **뒤척임**은 긴 밤이 걷히며 세계가 다시 짜이는 순간이다(Time 2.3) — 캔 자국을 처음으로 되돌리고 발자국을 묻는다.
이 Play 는 "뒤척임이 자국은 묻어도 셈은 못 묻는다" 를 세운다.
...
놓는 미지: **없음** — 미지 M4 천공고래의 길을 한 번 더 깊게 한다 (비늘이 **기회**가 된다). 확정 2 — 이름 있는 새 사실을 Human 이 주면 그때 바꾼다.
...
이 Play 가 세우는 것    RegionState.history(항목 다섯 · D4) · 수명 표(State 필드마다 지우는 손) · 판의 「기억」 줄 · Condition 형과 평가기 ·
                      흩어진 조건 자리 넷의 한 형 읽기 · 기억을 읽는 첫 조건 · Opportunity 형과 첫 기회(비늘 · Event) · 판의 「할 수 있는 것」 줄이
...
- [L2-World-Foundation.md](../L2-World-Foundation.md) — 원문 §2.8 · §3 · §4 · §5 · §8 · §10 · §14 · §15 · 확정 G1~G13 · §4 데이터 계약 · §5.2 검사 ㊸~㊼ · §8 대조 · §10 D1~D5
...
- TODO.md §2 Time 의 "Observable ⑤ 와 SPEC-006 경계 ① 중 어느 쪽인가" · §3 "World Event Opportunity" — 둘 다 이 Play 가 받아 지웠다 (확정 5 · D3)
...
(언제까지인지는 세계가 말하지 않는다), 캐고 나면 "지나간 뒤 캐였다 — 다시 지나야 한다" 로 닫히는 것을 읽으며, 뒤척임(긴 밤이 걷히며 세계가 다시 짜이는 순간)이 발자국과 자국을 묻은 뒤에도
...
② 세계를 저장하고 되살려도 기억이 남는다 (세계에 남는 것 · 스냅샷). 뒤척임은 자국 · 발자국을 묻되 기억은 묻지 않는다 (확정 4)
...
이 Play  그 사실들에 **셈**을 붙이고(기억 · history) · 그 사실들이 이미 걸고 있던 조건에 **한 형**을 주고(Condition) · 비늘이 이미 하던 일에 **이름**을 붙인다(Opportunity)
```
...
상태   세계에 남는 것 — 스냅샷에 실린다 (STATE_VERSION ↑). 되돌아옴이 taken 을 0 으로 되돌려도 takenTotal 은 남는다.
...
**무엇이 무엇을 지우는가의 표**가 함께 선다 — 지금 있는 State 필드마다 지우는 손 하나(Foundation §4.4). 이 Play 의 기억이 그 표의 첫 "지워지지 않는 것" 이다.

### 5.2 잊힘의 부재 — 뒤척임은 자국을 묻고 기억은 못 묻는다 (G7 · 확정 4)
...
추론   "묻히는 것과 남는 것이 다르다 — 자국은 뒤척임이 묻는 것, 셈은 지워지지 않는 것"
```
...
### 5.4 기다림 — 기억을 읽는 첫 조건 (G8 · TODO §2 Time 의 미결 하나)
...
둘을 **다른 과거**로 가른다: 지난 적 없음 = ⑤ · 지난 뒤 캐임 = ① . 둘 다 참이 된다 (확정 5 — TODO 의 그 줄은 지웠다).
...
조건   SINCE 가 있다 — availability 의 첫 항이 "지난 뒤로" 다. 그래서 이것은 **Event** 다 (G4). 닫는 창은 두지 않는다(확정 6) — 캐면 닫힌다
...
Required — 세계 (content/world · content/regions · protocol)
  W53  RegionState.history(기억) — 항목 다섯(D4) · 세계에 남는 것(WORLD) · STATE_VERSION ↑ · 되살려도 남는다
  W54  기억을 올리는 자리 하나 — 채취 완료 · 고갈 · 뒤척임 · 깨어남 · 경로 통과 다섯이 부른다. 뒤척임은 기억을 묻지 않는다(확정 4)
  W55  Condition 데이터 형(Foundation §4.1) — 조건 자리 넷을 이 형으로 읽는다(옮김/어댑터는 spec · 빈칸 2). 규칙 코드의 이름 0 유지
  W56  기억을 읽는 첫 조건 — 비늘 자리의 availability 에 passages EXISTS
...
```text
기회를 더한다(다른 원천 · 다른 방 · 다른 Event) · 조건을 바꾼다(SINCE 를 WITHIN 으로 · 임계를 기억으로) · 기억 항목을 더한다(Life 의 births)
...
```text
[ ] C034 — 방이 기억한다: RegionState.history — 기억(W53 · W54 · D4) + 수명 표(Foundation §4.4 — spec State 절) + 판의 「기억」 줄(W58 절반 · V26) +
           뒤척임이 자국은 묻고 기억은 안 묻는다(5.2) + 저장하고 되살려도 남는다 + 검사 ㊸ ㊼. 규칙 · 판정은 한 줄도 바뀌지 않는다
[ ] C035 — 조건은 하나의 형이다: Condition 형 + 평가기(E22 · W55) + 조건 자리 넷의 한 형 읽기(옮김/어댑터는 spec) + 기억을 읽는 첫 조건(W56 — 고래가
...
## 확정 사항 (Human 승인 — 열 전부 제안대로 · Human 이 언제든 뒤집는다)

```text
1. 2층을 한 번 더 연다 — 이 주입은 ②-부속 다섯째(새 축 없음)다. 승인으로 2층 기획은 다시 닫혔다 — 더 쌓을 기획은 없다
2. 놓는 미지는 없다 — M4 천공고래의 길을 깊게 한다(비늘이 기회가 된다). 이름 있는 새 사실을 Human 이 주면 그때 바꾼다
3. 방이 세는 것(기억) — 원천마다 캐인 횟수 누계 · 고갈된 횟수 · 마지막 고갈 시각, 방마다 뒤척임 횟수 · 깨어난(소란이 임계를 넘은) 횟수와 마지막 시각 ·
   지나간 것(고래 · 사냥꾼)의 횟수와 마지막 시각. 상한 없음. 누가 했는지는 세지 않는다
4. 뒤척임은 자국을 묻되 기억은 못 묻는다 — 자국 · 발자국은 "뒤척임이 묻는 것" 이고 셈은 "지워지지 않는 것" 이다. 세계가 뒤척여도 일어난 일은 일어난 일이다
5. TODO §2 Time 의 미결(Observable ⑤ · SPEC-006 경계 ①)은 둘 다 참이다 — 다른 과거를 말한다. 지난 적 없음 = 자리가 아무것도 내밀지 않는다 ·
   지난 뒤 캐임 = "다시 지나야 한다". 그 줄은 TODO 에서 지웠다
6. 비늘 기회는 닫는 창이 없다 — 캐면 닫히고 고래가 다시 지나면 열린다(RoomNeverSame 확정 9 그대로). "언제 다시" 는 세계가 말하지 않는다
7. 확률은 두지 않는다(L1 §3). 조건 형의 그 자리는 5층 이후가 난수 State 와 함께 채운다
8. 흩어진 조건 자리 넷을 한 형으로 **옮기는가 · 읽기만 하는가**는 C035 의 spec 이 정한다 (Access 빈칸 1 과 한 번에)
9. 원문 §2.3 의 Property 이름들은 이 세계의 어휘로 받지 않는다 — 이름공간의 형식만. 이름은 그것을 쓰는 Play 와 함께 Human 이 준다
10. 발견 상태 다섯은 3층 · Region 자체 성장의 첫 사례는 Life 뒤 · Object 류는 컨텐츠 행 — 이 경계를 승인한다
11. 자리와 번호 — 2층 기획 증명 여덟째 Play · Life · Access · Trail 과 병행 · Cycle 은 C034~C036
```
