...
상태: **승인됨** (Human 승인 1회 — "이대로". 확정 후보 13 이 확정 사항이 되었고, 질문 셋은 후보값으로 확정).
Cycle Breakdown 의 체크박스만 앞으로 갱신된다. 주입 원문과 제안은 [L2-World-Time.md](../L2-World-Time.md) 가 소유한다.
...
관찰   HUD 에 때(낮/밤 · 철) · 흙의 붉기(Material 의 soil-stain 세기에 철 배율) · 없던 track · 밤의 길이 ·
       **지금 걸린 자락이 땅에 선다** (확정 14) — 위험은 붉은 띠, 깊어진 자락은 그 깊이의 색. 철이 바뀐 순간
       "철이 바뀌었다 — 스밈" 한 마디가 지나간다 (낮밤도 같다)
...
사유 코드          not-this-season · not-this-hour(낮밤 출현 · 실주행 판정) · forest-awake · track-fresh/old · whale-shadow · turn-happened
관찰 결과 불변     WorldClock 은 전역 · 관찰자 이름은 track 에 없다 · 시각은 관찰자마다 같다 (T1)
관찰자가 잇는 것   지금 걸린 자락(철 · 소란 · 지나는 것 · 상시 · 깨진 마디)은 세계가 싣지 않는다 — 관찰자가 자기 phases
                 데이터와 봉투의 값 넷(철 · 위상 · 지나는 것 · 깨진 마디)으로 세계와 같은 차례로 얻어 땅에 그린다.
                 때가 바뀐 순간도 직전 봉투와 견주어 관찰자가 안다 (실주행 판정 · 확정 14)
```
...
V14  덧씌움 표현 — 유효 depth 로 색을 고른다 (V1 의 표 그대로, 입력만 유효 태그) · hazard 확장 area.
     **자락이 땅에 선다** (실주행 판정 · phase-presentation): 위험 자락은 붉은 띠(채움 옅게 · 테두리 굵게), 깊이 자락은
     그 깊이의 색 한 겹. 걸리는 차례는 세계의 것과 같다 (상시 → 철 → 깨어남 → 지나는 것 → 깨진 마디)
V14′ 때가 바뀐 순간의 한 마디 — 철은 그 철의 이름으로, 낮밤은 "밤이 왔다 / 날이 밝았다" (실주행 판정)
...
```text
[x] C015 — 세계에 시계가 선다: WorldClock(낮밤 · 철 · 바퀴) + 밤의 관찰 범위 + HUD 의 때 + 하늘/바닥의 철 배율.
           같은 방을 낮과 밤에 본다 — 보이는 범위와 흔적의 종류가 다르다. 고요 → 스밈 → 긴 밤 → 뒤척임이 돈다 (덧씌움은 아직)
[x] C016 — 철이 방을 바꾼다: RegionSpec.phases 덧씌움 넷 — 스밈의 depth/hazard 덧씌움 · 긴 밤에만 열리는 문 ·
           철 조건 Source 출현 · 뒤척임의 자국 묻기와 원천 이동. 같은 방에 세 철에 세 번 온다
[x] C017 — 여럿의 누적과 남의 자취: 소란(미로 압력의 일반화) + 임계 전이(숲이 깨어난다) + track.
           관찰자 하나로는 안 넘고 셋이면 넘는다 · 내가 없던 사이의 발자국
[x] C018 — 지나가는 것: PresenceRoute 둘 — 맹목의 사냥꾼(긴 밤 · 경로가 outer 로) · 천공고래(낮 · 그림자 · 비늘 원천 ·
...
실주행 판정 (Human 답 · Cycle 없이 반영) — "밤낮은 보였는데 다른 변화는 모르겠다. 채집물도 많아야 둘이고 확인해 볼
단서 자체가 없다. 기반을 확인할 다양한 맵 배치가 필요하다."
14. **자락이 땅에 선다.** 위상이 판에서만 읽히던 것을 땅에 그린다 (§5.1 · V14). T8 "시간은 관찰 가능해야 한다" 의 연장이다 —
    하늘 · 흙 · 흔적에 **자락의 띠**가 더해진다. 어느 철에 무엇이 달라지는지는 여전히 싣지 않는다: 관찰자는 띠가 서고
    사라지는 것을 여러 철에 와 보고 배운다. 때가 바뀐 순간의 한 마디도 같은 자리다 (관찰자가 직전 봉투와 견준다).
15. **철을 타는 방이 일곱**이다 — 숲 가장자리(스밈) · 숲 안쪽(스밈: 광석 지대 쪽 출구 자락이 deep 으로 · 위험 / 긴 밤:
    고대 문 언저리 hazard/phenomenon · 스밈에만 서는 껍질 하나) · 폐허(스밈: 더미 둘레 hazard/ecology) · 둥지(긴 밤: 사체
    둘레 deep · hazard/creature) · 거목(스밈: 뿌리혹 둘레 deep / 긴 밤: hazard/creature) · 생체 광석 지대(긴 밤: 노두
    자락 hazard/creature — **깨어남과 같은 자락**이라 철과 소란이 한 자락에 함께 걸리는 첫 자리) · 빙결 협곡.
    전부 phases 데이터이고 규칙은 한 줄도 늘지 않았다.
16. 낮밤 출현의 첫 사례 — 밤에만 서는 빛 갓 (Material 확정 13). Q25 "밤이 종류를 바꾸는가" 의 답이 이것이다.
17. Q26~Q38 의 답 — 시간 값 셋(긴 밤 360 · 스밈 720 · 밤 시야 20) · 소란 값 셋(10 · 300 · 0.5/s) · 깨어남의 자리 ·
    자국 · 압력과 소란의 구분 · 지나가는 것의 침묵 · 111 분 / 37 분은 지금 그대로 둔다.
```

## Human 질문 — 답 (후보값으로 확정)

```text
1. 철 이름 넷 — 고요 · 스밈 · 긴 밤 · 뒤척임. 이대로 (안에서 느끼는 말).
2. 철의 바퀴는 세계에 하나다 (T1). Region 마다 어긋나지 않는다.
3. 긴 밤은 낮이 아예 없는 하루다 — "장기간의 밤" 원문 그대로. 새벽이 곧 뒤척임이다.
```
