...
```text
[x] C022 — 허물의 주인: LifeFormation 데이터 계약 + 탄생지 State(phase 넷) + 결속 조건(균사 · 축적 · 비 · 개체군 0) +
           전조 흔적 넷 + 검사 ㉗~㉝(㉛ 은 먼저 실패를 보고한다). 아직 태어나지는 않는다
[x] C023 — 태어남은 소비다: DORMANT → BORN 전이 + 소비 넷(뿌리혹 축적 · 균사 분해 · 알집 phase · presence) +
           traces.after(빈 껍질) + 개체군 값 + 뿌리의 알(계승형 — 개체군이 있으면 알집 없이 잇는다) + 투영.
           부화 전후 · 결속과 계승이 화면에서 갈린다
[x] C024 — 스폰이 아니라 회복이다: 개체군이 MOLT_LITTER 회복을 좌우(㉛ 통과) + 균사·뿌리혹을 캐면 늦어지고 0 이 되고
           두면 되돌아온다(멸종 없음) + 사체 → 균류 변성형 + 협곡에 탄생지가 없는 사유 + ㉚ 분포 보고
[x] C025 — 숲이 스스로 돈다: 개체군 셋(광식충 · 대형 조류 · 포식수) + 관계 다섯(CALLS · EATS · LEAVES) + Region 을 넘는
