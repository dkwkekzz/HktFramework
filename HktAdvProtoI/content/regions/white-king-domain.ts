...
export const WHITE_KING_DOMAIN_SPEC: RegionSpec = {
  ...
  // ── C014 ADDED — 이 계통 **밖**의 방이라는 것을 밝힌다 (§6.4 · 확정 5) ────────────
  //
  // 원천도 흔적도 하나 없다. 그것은 결손이 아니라 **밝혀진 사실**이다 — 이유를 새로 짓지
  // 않는다: 산맥이 북을 막고 강이 남북을 가르기 때문이고(이 방의 ridge-north 와
  // river-white-king 이 그 땅이다), 그것이 백왕령이 **안전한** 이유와 같은 조건이다
  // (Concept W2 · RoomBecomesLand §5.3). 안전과 결핍이 한 원인이다.
  //
  // 그래서 여기는 얻은 것을 확인하고 다음 위험을 판단하는 자리이지 얻는 자리가 아니다
  // (A.3 "피난처"). 원천이 없으므로 이 방에는 Region State 의 sources 자리도 서지 않는다.
  resourceEcology: {
    sources: [],
    isolationReason:
      '산맥이 북을 막고 강이 남북을 가른다 — 숲의 사슬이 실려 오지 못하는 그 조건이 곧 이 자리가 안전한 이유다',
  },
};
