...
export const FROST_CANYON_SPEC: RegionSpec = {
  ...
  /**
   * 이 방이 품은 **생명 계통** — 없다. 그리고 **왜 없는지가 여기 적혀 있다**
   * (C024 ADDED · Life F6 · spec SPEC-009).
   *
   * 재료의 고립 사유(위 resourceEcology.isolationReason)를 생명에 그대로 옮긴 것이다 —
   * 없음이 침묵이 아니라 **답**이 된다: 협곡에 탄생지가 없는 것은 아직 안 만들어서가 아니라
   * 열을 먹는 결정이 있어 결속에 쓸 열이 남지 않기 때문이고(Frost 확정 2), 그것이 이 방이
   * 서리로 덮이는 이유와 **같은 조건**이다.
   *
   * 고개 너머 세 방(얼음 협곡 · 이 방 · 빙결 심층) 가운데 **하나만** 밝힌다 — 같은 글자를
   * 세 번 적지 않는다 (백왕령 하나가 재료의 고립 사유를 지는 그 어법 그대로 · 기본형 ⑦).
   * 탄생지도 개체군도 밝히지 않으므로 이 방은 여전히 그 계통 밖이고, 검사 ㉚ 은 이 글자를
   * 보고에 함께 실을 뿐 판정하지 않는다.
   */
  ecology: {
    absenceReason:
