...
- **엔진(StepPlan 6 Phase 19 step) + 콘텐츠 단계 C1~C7 + 플레이어블 P0·P0.5 완료.** (엔진: A 기질 ·
  B 코어 · C 발견 · D 시각화 · E 플래너 · F 다중 행위자 = M1~M6 / 콘텐츠: C1~C7 / **P0: 사람이
  브라우저로 접속해 실제로 플레이한다** — `/play.html` / **P0.5: 캔버스 게임 뷰에서 마우스로
  사냥·채집한다** — 존 좌표 위 이동·전투·채널링, 아래 P0/P0.5 절).
  `bash run.sh` 또는 Windows `run.bat` = 설치 → `npm test`(**170 케이스** 전건 통과) → 데모 서버.
...
### 플레이어블 P0.5 로 참이 된 명제 (`src/play/zone.js`, `demo/play.*`)

**캔버스 게임 뷰에서 마우스로 사냥·채집한다** — P0 은 버튼 패널이었고, P0.5 가 MMORPG 의 몸을
올린다. 지역(region)마다 연속 좌표의 존(field)이 있고, 그 위를 내 캐릭터·몹·타인이 실체로
움직인다. 규칙 권위는 그대로 — 전리품은 법칙 `전투`, 채집 완료는 기존 `act`(채취/수확/협상)
경로를 타서 원장·사건·done_when·파문을 벗어나지 않는다. 존은 "어디서·언제"만 정하고 "무엇이
되는가"는 법칙이 정한다.

- **서버 절반**(`src/play/zone.js` ZoneSim): 존 좌표·클릭 이동(moveTo)·몹 AI(권속·마수·평원짐승 —
  어그로·추적·반격·리시·도주·리스폰)·전투(사거리·쿨다운·처치→법칙 `전투` 전리품)·플레이어
  사망/부활·채집 채널링(GATHER_TIME)·**재료 공명 발광**(활성 말단 demand 속성을 가진 몹·노드가
  빛난다, §6.4)·`/api/play/cmd`(moveTo/attack/gather/stop). 200ms 존 스텝 + 5박자마다 세계 틱 1
  (주기·소멸)의 이중 박자를 `demo/server.js` 가 구동. 채집 노드 뷰는 무대·원천·주기 상태에서
  파생(규칙 재유도 없음, 상태 읽기만).
- **클라이언트 절반**(`demo/play.js`+`play.html`): 캔버스 게임 뷰 — 카메라 추적(내 캐릭터 중심,
  월드 경계 클램프)·폴링 250ms + 프레임 보간(lerp)·클릭 타겟팅(몹=공격/노드=채집/지면=이동)·
  WASD 이동·HP바·데미지 플로터(존 hits 소비)·채집 진행 링·공명 GLOW(shadowBlur). 지역 지도는
  이동 전용으로 유지, 관찰·정제 등 채집 밖 정밀 행동은 사이드 패널 버튼. 클라이언트에 게임 규칙
  0(src import 0 을 테스트로 강제 — 원칙 ⑥).
- **검증**: `test/zone.test.js` 6건 — 이동 수렴·채집 완주(소지품 증가+원장 보존)·사냥→전리품→
  파문(처치가 done_when 충족·파문·`audit()`)·채집 창 거부(원천 소멸=피드백)·사망/부활(무리
  한복판에서 쓰러지고 스폰 부활)·HTTP `/api/play/cmd` 종단(수락 vs 피드백). 기존 164 테스트 불변
  유지 → **총 170**.
- **눈 검증**: `demo/shot-play.mjs`(playwright-core + 시스템 chromium, 선택 실행)가 `/play.html?name=`
  를 열어 사냥 순간을 캡처 — 존 게임 뷰·권속 무리와 HP바·어그로 링·데미지 플로터·공명 노드·
  HP 피해 반영이 한 화면에 재현된다.

## NEXT — 다음 할 일 (P1 — 플레이가 깊어진다)

**P1** ([Design-Playable.md](Design-Playable.md) §4): 전투(둥지·수송대)·제작 개입(결합 중 폭주
제어)을 버튼이 아니라 **과정**으로. 방사형/별자리 뷰(`demo/graph-*.js`)를 플레이 화면에 통합해
내 믿음 그래프를 열람하고, 가설 수립·실험 UI(H1/H2), 상향 발견(retro-bind) 연출을 붙인다.
완료 판정은 여전히 "사람이 브라우저로 그 플레이를 실제로 할 수 있고, 같은 플레이가 테스트로
재현된다".
...
재사용 자산: 엔진 `src/`(A~F) + 콘텐츠 `src/content/` + 플레이 `src/play/`(game·zone) + `demo/` +
**170 케이스 회귀**. 새 콘텐츠 = 픽스처 + 시나리오 + 테스트 (엔진은 손대지 않는 게 기본).
