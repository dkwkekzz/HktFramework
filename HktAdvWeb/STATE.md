...
- **엔진(StepPlan 6 Phase 19 step) + 콘텐츠 단계 C1~C7 전부 완료.** (엔진: A 기질 · B 코어 ·
  C 발견 · D 시각화 · E 플래너 · F 다중 행위자 = M1~M6 / 콘텐츠: C1 첫 사냥터 ~ C7 결전과 그 후).
  `bash run.sh` 또는 Windows `run.bat` = 설치 → `npm test`(**158 케이스** 전건 통과) → 데모 서버.
...
- **C3** 재료의 세계: 같은 무기급 demand 를 채굴형/전투형 두 봇이 다른 경로로 충족(§5 행렬 실행) ·
  위협 제거=재료 획득 겹침(둥지→뼈) · 균류 역결합 · **균류 배양 원리(0.3.1.2)·탈것(0.1.1.4.3)** ·
  무기 결합은 에너지저장 공급 0 이라 아직 막힘.
- **C4** 앎의 문: 지식 없는 봇 문전 차단 / 지식 봇 조율 개방 · E2 백로그에서 0.1.1.3.3 이탈(공급 0→1) ·
  **월식 가설 H1 확인 → 정체 파악(0.1.1.1) 완료**.
- **C5** 타인의 세계: 거래/강탈이 같은 표본 done_when 을 충족하되 세계 상태(숭배단 적대)를 다르게
  남긴다(원칙 ①) · 수송 차단→세력 약화(0.1.1.6.2→0.1.4 이중 파문).
- **C6** 기다림의 세계: 내한 장비/적응 확보 · 독기 적응(0.4.2, R3 심부 개방) · 한파 창에만 유효
  저온 판정 · **H2 반증→가지 붕괴, 그러나 장비·적응은 완료로 잔존**(실패한 가설 ≠ 낭비) · 전 주기 겹침.
...
## NEXT — 다음 할 일 (콘텐츠 단계 C1~C7 은 완주됨)

**콘텐츠 단계 C1~C7 은 전부 구현·검증 완료** (`src/content/`, `test/content-c*.test.js`, 데모
관전 `콘텐츠 단계 C1~C7` 섹션). 남은 방향은 콘텐츠의 **넓힘과 튜닝**, 그리고 직교 트랙이다:

- **콘텐츠 확장**(Design-WorldComposition §8 규칙): 스텁 가지(0.5 거점·0.7 존재 확장·0.1.3 오염·
  0.1.6 근본 원인)에 자식을 붙이고, 새 demand 는 §5 행렬에 행을 추가(공급 무대 ≥ 2). 초기값 수치
  튜닝(재료 대역·주기 period/window)은 봇 시나리오 회귀로 지켜가며 조정.
- **단계 통합 시나리오**: 현재 각 cN 은 그 단계의 시그니처 루프를 국소 검증한다. 한 봇이 C1→C7 을
  통틀어 하나의 세계·시계·믿음으로 완주하는 "장편" 시나리오(멀티봇 `runMultiSim` 확장)가 다음.
- **직교 후속 트랙**(StepPlan §10): **월드 렌더**(Scene 서술자 계약 확정) · **LLM 플래너**(접합면은
  `decompose.js` 주석에 고정) · **멀티플레이어 권위**(F 는 단일 프로세스까지).

재사용 자산: 엔진 `src/`(A~F) + 콘텐츠 `src/content/`(기계 4 + 단계 7) + `demo/` 렌더러 +
**158 케이스 회귀**. 새 콘텐츠 = 픽스처 + 봇 시나리오 + 테스트 (엔진은 손대지 않는 게 기본).
