...
## NEXT — 다음 할 일 (P0.5 — 게임 화면: 캐릭터 조작·사냥·채집)

**P0.5** ([Design-Playable.md](Design-Playable.md) §4): P0 은 버튼 패널이었다 — 게임의 몸을
만든다. **서버 절반은 이미 구현·통합돼 있다**(`src/play/zone.js` ZoneSim: 존 좌표·클릭 이동·
몹 AI(권속·마수·짐승)·전투·사망/부활·채집 채널링·공명 발광·`/api/play/cmd`·200ms/1s 이중 박자
— 전리품=법칙 `전투`, 채집=기존 `act` 경로로 규칙 권위 불변, 기존 164 테스트 통과 유지).

남은 절반 (이것부터 시작):
1. `demo/play.js` 를 **캔버스 게임 뷰**로 재작성 — 카메라 추적, 폴링 250ms+보간, 클릭 타겟팅
   (지면=이동/몹=공격/노드=채집), WASD, HP바·데미지 플로터·채집 진행 링, 공명 GLOW.
2. `test/zone.test.js` — 이동 수렴·사냥→전리품→파문·채집 창 거부·사망/부활·HTTP cmd 종단.
3. 스크린샷 눈 검증. 완료 판정 = **사람이 브라우저에서 마우스로 사냥 1회·채집 1회를 실제
   수행** + 같은 플레이의 테스트 재현.

이후: **P1**(전투·제작 과정화, 그래프 뷰 통합, 가설 실험 UI) → P2(영속·계정) → P3(월드 렌더).
