...
**역변환 — 월드점을 캐릭터 로컬로 되돌리기** (`AActor::GetTransform().InverseTransformPosition`): 회전부 `R` 이 정규직교라 역행렬은 **전치 `R^T`** 로 공짜로 얻는다(2.2 의 `R^-1 = R^T`).
...
**합성과 비교환성 확인**: 총을 *손 소켓* 에 붙이고, 총구는 다시 *총 기준* 으로 정의하는 계층(scene graph)이라면 `M_muzzle_world = M_char * M_hand * M_gun`. 순서를 바꿔 `M_gun * M_hand` 로 곱하면 전혀 다른 자세가 나온다(`A*B != B*A`, 2.2). 실제로 yaw 90° 회전 후 (50,0,0) 이동 vs (50,0,0) 이동 후 yaw 90° 회전은 각각 월드 `(1000,550,0)` 과 `(1050,500,0)` 로 갈린다 — 부착 순서 버그의 전형이다.

> UE5 주의: UE 는 **좌수(left-handed) 좌표계**에 행벡터·행우선(`FMatrix`, `v' = v * M`) 규약이라, 위 열벡터 수학 교과서 표기와 곱 순서/yaw 부호가 다를 수 있다(3장 handedness 참조). 또 `FTransform` 은 회전을 행렬이 아닌 `FQuat` 로 저장하고 적용 순서는 **Scale → Rotate → Translate** 다. 수치 결과의 *의미* 는 같다.
