...
> 이 문서는 **목차(인덱스)와 요약**만 보관한다. 세부 이론은 [foundations/](foundations/) 하위 문서로 분할되어 있다 — 한 주제 = 한 문서.
...
> 🎨 **인터랙티브 시각화**: [foundations/vectors-dot-cross.html](foundations/vectors-dot-cross.html) — 내적·외적의 기하학적 의미를 점을 드래그하며 직접 실험(외부 라이브러리 없는 순수 Canvas 2D).
...
- **Chaos (Unreal Engine 5)**: UE 의 `FVector`(double, UE5부터 LWC — Large World Coordinates), `FQuat`, `FTransform`, `FMatrix`. **UE5 의 LWC** 가 "큰 좌표 정밀도" 문제([foundations/05](foundations/05-numerical-floating-point.md))에 대한 엔진 차원의 대응 — 좌표를 double 로 승격해 거대 월드의 떨림을 줄였다. 단 double 이라도 자동 결정론을 보장하진 않는다.
...
- **정규화 빠뜨림**: 사원수/회전 행렬을 적분하고 정규화를 안 하면 누적 오차로 스케일이 섞이거나(행렬) 회전이 왜곡된다(사원수). → 매 스텝 또는 주기적 재투영 필수. ([foundations/03](foundations/03-rotations.md))
- **`acos`/`asin`에 클램프 누락**: 내적 결과가 반올림으로 `1.0000001` 이 되면 `acos` 가 NaN. 항상 `clamp(x, -1, 1)`. ([foundations/06](foundations/06-identities-approximations.md))
- **0 길이 정규화**: `v/|v|` 에서 `|v|=0` → NaN/Inf. 길이 epsilon 체크 후 폴백(예: 영벡터 또는 기본 축) 처리. ([foundations/01](foundations/01-vectors.md))
- **부동소수점 `==` 비교**: 누적 오차로 정확히 같아지는 일은 거의 없다. epsilon 비교 사용. ([foundations/05](foundations/05-numerical-floating-point.md))
- **상쇄(cancellation)**: 큰 두 좌표의 차로 작은 변위를 구할 때 정밀도 폭락. 가능하면 로컬 좌표/상대 좌표로 계산. ([foundations/05](foundations/05-numerical-floating-point.md))
...
- **double-cover 부호**: 사원수 보간/비교 전 `q1.q2 < 0` 이면 한쪽 부호 반전. 안 하면 "먼 길로 도는" 보간이나 잘못된 거리. ([foundations/03](foundations/03-rotations.md))
- **큰 월드 떨림(far-from-origin jitter)**: 원점에서 멀어질수록 ULP 가 커져 위치/물리가 떨린다. origin rebasing, 타일 좌표, 또는 double/LWC(UE5)로 대응. ([foundations/05](foundations/05-numerical-floating-point.md))
- **비균등 스케일과 법선**: 비균등 스케일(non-uniform scale) 행렬로 법선을 변환할 때 같은 행렬을 쓰면 안 됨 → **역전치(inverse transpose)** 행렬을 써야 직교성이 유지된다. ([foundations/02](foundations/02-matrices-transforms.md))
