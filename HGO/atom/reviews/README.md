...
- **#1 | 전역 *바스* 공간 비국소** | ③ | 0001-0010 | 🟡 부분 — ~~bondE~~ ✅**0015 `bondLocalE` 해소**, **바스 `sim.escaped` 만 전역 단일** · **damp(0024)·fuse·coolOuter(0083 쓰기 `:730`) + disperse(0071·0085)·bondBreak(0075) 소비/쓰기 양방 가담**(0071-0090) | **목적지 STATE §3**(Phase E 닫힘으로 무르익음·복사 냉각/국소 복사장·render L-T 게이트 연동) → 게이트: 국소 복사장 노브(끄면 전역 바스 = 회귀 0)·`hgo-laws.js:704·730`

- **#I | degField cut=coolR>spatialCut 시 셀 이웃 누락(라텐트)** | ③ | 0081-0090 | 🟡 신규(경미·정리성) — degField 가 cut=radius 로 호출돼 셀폭≥radius 가정(`hgo-laws.js:1471·1477`)·coolR>spatialCut 무대선 셀폭<coolR → 이웃 셀 밖 쌍 누락 위험(현 scene coolR 5≤spatialCut 8 안전·0088 §한계 정직) | **목적지 STATE §3 백로그** → 게이트: degField 셀 반경 ±2 확장(끄면 현 cut=coolR 가정 = 회귀 0)·고밀도/큰 coolR 무대 열 때 선결
