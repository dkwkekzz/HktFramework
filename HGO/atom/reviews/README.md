...
- **#6 | E 완화폭 creep** | ④ | 0021-0030 | 🟡 부분(메커니즘 해소) — **step-0069 `symplectic`**(velocity-Verlet)이 2체 궤도 E 표류 553배·secular 28배 개선(고차 적분 도착)·잔여=다체 BH+fuse 적용(0070 ~3%)·적응 시간단계 | **목적지 STATE §3 적분 정밀도**(이미 추적) → 게이트: 다체 VV 적용·근접조우 정규화·적응 dt(끄면 현 거동 = 회귀 0)
...
- #5 연속력 O(n²) 5개 + 중력 장거리 → 단거리 5종 셀 배선(0055~59 collide/pauli/vdw/repulse/bond·컷오프-PE shift) + BH 트리(0060) + 장거리 gravity/coulomb 배선(0061·0062) — 모든 힘 O(n log n) (review-0051-0060)
