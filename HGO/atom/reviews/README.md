...
- **#1 | 전역 바스 공간 비국소** | ③ | 0001-0010 | 🟡 부분 — `bondE` 는 step-0015 가 국소화, **바스 `sim.escaped` 는 step-0037 까지 전역 단일** | **목적지 STATE §3**(arc Phase E 후 무르익음·render L-T 게이트 연동) → 게이트: 국소 복사장 노브(끄면 전역 바스 = 회귀 0)
- **#3 | `scatter` v1 `xMax=6` 리터럴** | ② | 0001-0010 | 🔴 열림(경미) — `hgo-laws.js:113` 노브 아님 | **목적지 STATE §3**(정리성) → 게이트: `scatterXMax` 노브화(기본 6 = 회귀 0)
- **#4 | `reheat` 운동량 일방 트랩** | ④ | 0001-0010 | 🟡 부분(#1 종속) — `bath.px·py` 미배출 | **목적지 STATE §3**(#1 에 종속) → #1 국소 복사장이 운동량 회계도 정련
