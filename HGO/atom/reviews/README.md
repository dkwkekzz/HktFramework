...
- **#1 | 전역 *바스* 공간 비국소** | ③ | 0001-0010 | 🟡 부분 — ~~bondE~~ ✅**0015 `bondLocalE` 해소**(review-0011-0020 확인), **바스 `sim.escaped` 만 전역 단일**(0037 까지) | **목적지 STATE §3**(arc Phase E 후 무르익음·render L-T 게이트 연동) → 게이트: 국소 복사장 노브(끄면 전역 바스 = 회귀 0)
- **#5 | 연속력 O(n²) 전쌍** | (성능) | 0011-0020 | 🟡 — coulomb+repulse 전쌍(0028 중력까지 5개로 늘어 시점 임박) | **목적지 STATE §3**(이미 추적 — 공간 분할/Barnes-Hut) → 신규 전파 아님·여기 기록만
