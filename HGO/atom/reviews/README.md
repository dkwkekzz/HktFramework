...
- **#1 | 전역 *바스* 공간 비국소** | ③ | 0001-0010 | 🟡 부분 — ~~bondE~~ ✅**0015 `bondLocalE` 해소**, **바스 `sim.escaped` 만 전역 단일** · **damp(0024)·fuse 쓰기 + disperse(0071)·bondBreak(0075) 소비/쓰기 가담**(0071-0080) | **목적지 STATE §3**(Phase E 닫힘으로 무르익음·복사 냉각/국소 복사장·render L-T 게이트 연동) → 게이트: 국소 복사장 노브(끄면 전역 바스 = 회귀 0)·`hgo-laws.js:1109`
...
- **#G | 핵합성 산물 → 분자 풍부한 무대(뜨거운 융합 ↔ 차가운 결합 척도 충돌)** | ①/② | 0071-0080 | 🟡 신규(load-bearing·arc) — 0074 `bondMorse`(유한 해리·열팽창)·0075 `bondBreak`(거리형 위상 해리)로 화학 *해리* 갖췄으나 cache run 격리(라이브 createSim bonds 미전달 → 자유 드리프트)·토이 covalent 모델 He 정지 → 융합 산물(C·O…)이 *라이브로* 분자를 이루는 무대 미 | **목적지 STATE §2/§1**(이미 한계 추적) → 게이트: 융합 산물 결합 라이브 scene + 결합 종류별 D·α·Morse 변곡점 자동 임계(끄면 현 토이 = 회귀 0)·Phase E→물질 전환
- **#D′ | VV 결합 1-tick 위상 지연 + 안정 id(n 무관 키)** | ①/④ | 0061-0070 | 🟡 백로그(비-load-bearing) — #D 의 *인덱스* 절반은 0073 `fuseRebond` 해소·*VV 결합 위상*(symplectic=1 + 활성 bonds → bondSpring 가 직전 tick bonds 봄·`hgo-laws.js:1186`)·안정 id 절반 잔존·현 그 조합 scene 0(무해) | **목적지 STATE §3 백로그** → 게이트: VV 결합 위상 + 안정 id(끄면 단일 트랙 = 회귀 0)·핵+화학+symplectic 무대 열 때 선결
