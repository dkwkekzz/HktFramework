...
;(function (root, factory) {
  ...
  const SCENES = {
    'step-0001': {
      id: 'step-0001',
      // observe — 뷰어 관찰 가이드(표시 전용, hash·verify 무관). step 선택만으로 에디터가 "뭘 보고 어느 watch 신호를 봐야 하나"를 자동 셋업.
      observe: {
        look: '원자(어두운 점)들이 토러스 안을 직선으로 자유 표류 — 힘 0, 부딪힘 0.',
        signals: ['meanSpeed', 'E'],
        tip: 'E(총에너지)·meanSpeed 가 매 tick 그대로면 닫힌 장부·자유 운동의 앵커 — 이후 모든 step 의 기준선.',
      },
    ...
    'step-0002': {
      id: 'step-0002',
      observe: {
        look: '어두운 점=원자가 색 있는 광자를 *제자리에* 토함 — 아직 반동·전파 0 이라 광자는 안 움직이고 빛나기만, 원자만 표류.',
        signals: ['photons', 'excited'],
        tip: '광자가 안 늘면 kEmit 슬라이더를 올려라. 색이 멈춰 보이는 건 정상 — 전파는 step-0004 부터.',
      },
    ...
    'step-0003': {
      id: 'step-0003',
      observe: {
        look: '정지 원자가 광자를 내며 *반대로* 슬슬 밀림 — 작은(가벼운) 점일수록 많이. 광자는 아직 제자리(전파 0).',
        signals: ['meanSpeed', 'totP'],
        tip: '눈엔 거의 안 보인다(반동이 작고 한 번뿐). kEmit 을 1.0 으로 올려보라. 확정 증거는 눈이 아니라 숫자 — meanSpeed 0→양수(정지에서 속력 생김) + totP≈0 유지 = "빛이 원자를 밀었다".',
      },
    ...
    'step-0004': {
      id: 'step-0004',
      observe: {
        look: '광자가 운동량 방향으로 광속 c 직진 — 복사장이 방출점을 떠나 공간으로 퍼진다(드디어 색이 날아다님).',
        signals: ['meanDisp', 'meanSpeed'],
        tip: 'meanSpeed 가 광속 c(=1)에 붙으면 직진·광속 확인. 큰 무대(400²)라 50 tick 동안 wrap 전에 측정된다.',
      },
    ...
    'step-0005': {
      id: 'step-0005',
      observe: {
        look: '날아간 광자가 원자에 닿아 *재여기* — 원자 준위가 초기(≤2) 위로 올라가고 광자는 적색이동해 살아남음(빛이 일함).',
        signals: ['scatters', 'maxLevel'],
        tip: 'scatters 가 0이면 kScatter·scatterR 을 올려 산란 빈도↑. maxLevel≥3 이면 빛이 끌어올린 증거(초기 최대 2).',
      },
    ...
    'step-0006': {
      id: 'step-0006',
      observe: {
        look: '광자가 원자에 부딪혀 *방향을 꺾으며*(등방) 재여기 — 전방 직진만이 아니라 진짜로 튄다.',
        signals: ['meanDeflect', 'scatters', 'totP'],
        tip: 'meanDeflect>0.1 이면 진짜 각도 산란(전방-전용이면 0). totP≈0 = 2D 운동량 보존 유지.',
      },
