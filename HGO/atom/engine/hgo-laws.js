  ...
  function bondAngle(sim) {
    ...
    for (const [ci, ns] of nbr) {
      ...
      if (sim.knobs.bondAngleLonePair) {
        const v = K.valenceElectrons(ai.e), lp = Math.max(0, Math.floor((v - ns.length) / 2)), sn = ns.length + lp;
        const t0sn = sn <= 2 ? Math.PI : sn === 3 ? 2 * Math.PI / 3 : sn === 4 ? Math.acos(-1 / 3) : Math.PI / 2;
        t0 = t0sn - lp * sim.knobs.bondAngleLonePair;       // 고립쌍 압축(LP개·노브 = 쌍당 압축각 rad)
      } else {
