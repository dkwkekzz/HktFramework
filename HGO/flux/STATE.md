...
### step-0005 가설 (▶ 즉시 다음 step — arc C 완성: 상관 길이)

**한 조각**: 동결 도메인 구조의 *척도 비의존* 지표 = **공간 상관 길이 ξ**(q 장 자기상관이 1/e 로 떨어지는 거리). step-0004 도메인 수는 군집 임계 의존 — ξ 는 임계 무관. 새 법칙·노브 0.

- 장면 1항: 블롭(또는 거친) 풍경 동결.
- 측정 1개: **반경별 자기상관 → ξ**(셀 단위). 1<ξ<격자크기면 척도 있는 구조(균일도 백색잡음도 아님).
- 가설(assert): ξ 유한·1셀 초과·격자 미만 — 동결 구조가 특정 척도를 가진다(척도 불변 패턴의 정량).
...
- **🟠 §4 가설 진행 중**: 확산(A)·동결(B)·θ-전이(B)·**도메인+전선 구조(C, step-0004)** 확보. 남은 건 상관 길이(C 완성)·척도 분리(D)·다채널(E). 다음은 상관 길이 ξ(step-0005).
- **🟡 coarsening 시간 척도 미측정**: step-0004 가 작은 도메인 융합(척도 선택)을 봤으나 *빠른 국소 vs 느린 전역* 시간 분리는 안 쟀다 — arc D 직접 시험.
