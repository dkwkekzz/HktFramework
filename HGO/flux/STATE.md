...
### step-0003 가설 (▶ 즉시 다음 step — arc B 후반: 임계 멱법칙)

**한 조각**: 같은 규칙·같은 격자에서 **θ 를 임계점 θc 근처로 스캔**(노브만 — 새 법칙 0). step-0002 는 *단일 relaxation* 의 동결만 봤다 — SOC 사태 분포(멱법칙)는 통계가 부족했다.

- 장면 1항: θ 를 여러 값으로(또는 약한 구동) 돌려 동결↔사태 전이의 임계점을 띄운다. `sim.fluxLast` 시계열이 토대.
- 측정 1개: **사태 크기 분포**(tick 당 활성 간선 수/총 플럭스의 히스토그램) — 멱법칙 지수를 잰다.
- 가설(assert): θc 근처에서 사태가 척도 불변(멱법칙). SPINE §4 가설의 *임계 층* 직접 시험.
...
- **🟠 §4 가설 진행 중**: 확산(A)·**동결(B 전반, step-0002)** 두 층은 측정으로 나왔다. 남은 건 **임계 멱법칙(B 후반)**·구조(C 도메인·상관 길이)·척도 분리(D)·다채널(E). 다음은 임계 θc 사태 분포(step-0003).
