...
- **닫힌 렌즈**: **렌즈-003 (L-cam)** — *내가 보고 싶은 각도로 본다.* 자동 선회(yaw=`sim.tick` 파생)를 폐기하고 **마우스 인터랙티브 카메라**로: 드래그=궤도(yaw·pitch, 짐벌락 회피 클램프)·휠=줌(distScale, 지수 배율)·우클릭|Shift드래그=팬(target). 카메라 상태는 `render.js` 의 `camState`(프레젠테이션 한 항 — 시뮬 무관·결정론 영향 0), `attachControls(canvas, onChange)` 가 마우스를 배선하고 정지 중 드래그도 `draw()` 재호출(공용 뷰어 `HGO/viewer.html` 이 1줄로 위임). 위치=sim (rx,ry,0) 그대로 — 분포 author 0. (직전 **렌즈-002 L-3d**: 평면 z=0 세계를 원근 3D 무대로 — 눈 검증 PASS.)
- **한 줄 상태**: 무대를 손으로 돌린다 — 구의 들판과 색 광자를 원하는 시점에서 본다(회전·줌·이동). 카메라는 순수 프레젠테이션이라 시뮬·골든 불변. 알리바이 성립(**atom/ diff 0** — diff 는 `render/` + 공유 셸 `HGO/viewer.html` 배선만).
- **다음**: §2 — **L-line**(누적 스펙트럼 분광 띠 정제) 또는 시뮬이 광자 방향을 내보내면 **L-recoil**(이제 3D 무대+자유 시점이라 빛 줄기를 입체로 그릴 채비 됨).
