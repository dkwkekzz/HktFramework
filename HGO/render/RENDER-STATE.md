...
- **닫힌 렌즈**: **렌즈-004 (L-line)** — *스펙트럼선을 세기로 읽는다.* 하단 분광 띠가 선의 *유무*만 보이던 것을 **전이선별 빈도(세기)**로 정제. `spectral.measureLines(photons)` 가 광자를 전이선(`from→to`)별로 집계 → `{lines(λ오름차순), maxCount}`, `drawStrip` 이 선 밝기 = 빈도/maxCount(데이터에서 *잰* 최대 빈도로 정규화). 강한 전이는 밝고 약한 전이는 흐리다 — 새 시뮬 양 0(읽기 정제), 세는 것이지 author 아님. (직전 **렌즈-003 L-cam**: 마우스 인터랙티브 카메라 — 눈 검증 ⏳.)
- **한 줄 상태**: 분광기처럼 선의 *세기*가 보인다 — 자주 일어난 전이는 밝은 띠, 드문 전이는 흐린 띠(빈도=세기). 정규화는 측정(maxCount), 손박은 임계 0. 알리바이 성립(**atom/ diff 0** — diff 는 `render/`(engine·validate)만).
- **다음**: §2 — 시뮬이 광자 방향(p=E/c)을 내보내면 **L-recoil**(3D 무대+자유 시점이라 빛 줄기를 입체로). 그전까지 가능 정제 칸만.
...
- **L-line (정제) — ✅ 렌즈-004 닫힘**: 전이선별 빈도 집계(세기=광자 수, maxCount 정규화) 완료. 후속 정제 여지: 선폭(도플러/자연폭)·로그 세기축 — *시뮬이 그 양을 내보낸 뒤*에만(현재 빈도까지가 가능 칸의 끝).
...
> **게이트 규율**: blocked 렌즈는 시뮬 선행 양이 atom 트랙에 실릴 때까지 시작하지 않는다. 가능 정제 칸(L-line 빈도)을 소진했으니, 다음 author 없는 렌즈는 시뮬 선행을 기다린다.
