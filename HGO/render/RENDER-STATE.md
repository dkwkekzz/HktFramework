...
- **닫힌 렌즈**: **렌즈-007 (L-trail)** — *빛이 공간을 가른 자취를 본다.* L-recoil(운동량 방향 글리프)의 정제: 시뮬이 굴린 **실제 변위**(광자 출생 `(rx0,ry0)`→현 위치 `(rx,ry)`, step-0004 propagate·0007 escape 가 실음)를 그대로 읽어 전파 트레일로 잇는다 — 길이=측정 실거리(손박은 창 0, L-recoil 의 STREAK_FRAC 정규화 글리프와 대비). `photonTrail(p,cam)`(출생·현재 투영 → {head,tail}, 변위 0/뒤쪽이면 null — 순수·헤드리스 검증) + `drawPhoton` 이 줄기·코어 *아래*에 가장 흐린 자취(머리 밝음→출생 투명, 색=L-λ). **변위 0(갓 방출 step-0002: rx0==rx)이면 트레일 author 0**(점만). 위치=sim (rx,ry,0) 그대로 — 시뮬 객체 비변경. (직전 **렌즈-006 L-bond**·**005 L-recoil**: 눈 검증 ⏳.)
- **한 줄 상태**: 전파 장면(step-0004·0007)에서 광자가 *출생점에서 현 위치까지 빛 자취를 끌며 난다* — 어디서 왔는지는 시뮬이 굴린 실변위를 읽을 뿐(author 0). 알리바이 성립(**atom/ diff 0** — diff 는 `render/`(engine·validate) + `HGO/viewer.html`(범례 1줄)만).
- **다음**: §2 — 가능 정제 칸 거의 소진. 남은 렌즈는 시뮬 선행 대기(온도/열=L-T, 결합 깸·도플러색, 분자별 묶음색).
