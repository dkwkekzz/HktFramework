...
- **닫힌 렌즈**: **렌즈-001 (L-λ)** — *광자가 색으로 빛난다.* atom 스냅샷(`sim.photons[].lambda`)을 읽어 가시광 스펙트럼 곡선으로 번역(`render/engine/spectral.js`). 짧은 λ=고에너지=보라 ↔ 긴 λ=저에너지=빨강 — 색을 author 하지 않고 λ 에서 읽음. 측정된 λ 범위를 가시광 창(400~700nm)에 로그 정규화(창은 데이터에서 *잼*).
- **한 줄 상태**: 트랙이 깨어났다 — 시뮬이 내보낸 빛(step-0002 방출)이 처음으로 *눈에 보이는 색*이 됐다. 원자는 어둑한 광원, 광자는 색 섬광, 측정 스펙트럼선은 띠로. 시뮬 알리바이 성립(atom/ diff 0).
...
- **L-line (정제 — 즉시 가능)**: 누적 광자를 전이선별로 빈도 집계해 *실측 분광기 띠*(세기=해당 선 광자 수)로 그린다. 현재 viewer 의 스트립은 *유무*만 보임 → 세기·빈도까지. 새 시뮬 양 불필요(읽기 정제).
...
| 🟡 | **스펙트럼 띠가 유무만(세기 없음)** | viewer 스트립이 선의 존재만 표시. L-line 이 빈도·세기로 정제. |
...
- **소유**: `render/` 폴더 전체 (`RENDER.md` · `RENDER-STATE.md` · `engine/spectral.js` · `viewer.html` · `validate/smoke.js`).
- **불가침**: `atom/` · `../SPINE.md` · `../CLAUDE.md` — 읽기만. 스킬은 `.claude/skills/hgo-render-step/`.
...
렌즈-001 (L-λ) | 광자 lambda → 가시광 스펙트럼 색 + 측정 스펙트럼 띠 | 스모크 PASS(광자 51·선 3·λ[1.333,20.571]·순서 보존) · 시뮬 알리바이 atom/ diff 0 · 눈 검증 ⏳(사용자 브라우저)
