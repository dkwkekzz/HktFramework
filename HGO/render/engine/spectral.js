...
//   *측정된* λ 범위([lo,hi])를 가시광 창(400~700nm)에 로그 정규화한다 — 창은 데이터에서
...
;(function (root, factory) {
  ...
  // 측정된 λ 범위 [lo,hi] 를 가시광 창(violet 400 ↔ red 700)에 로그 정규화.
  //   짧은 λ → 400(보라) / 긴 λ → 700(빨강) = 실제 스펙트럼 순서. 단일 선이면 중앙(녹색).
  function lambdaToNm(lambda, lo, hi) {
    if (!(hi > lo)) return 530;
    let t = (Math.log(lambda) - Math.log(lo)) / (Math.log(hi) - Math.log(lo));
