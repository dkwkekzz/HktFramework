...
| 채널 | 모양 | 읽어서 무엇으로 |
|---|---|---|
| **atoms** | `{Z,N,e,x,rx,ry,vx,vy}` | 위치(rx,ry)=화면 좌표 · 질량(Z+N)=크기 · **양성자수(Z)=색조(원소, L-element)** · 들뜸(x)=광원 밝기 · 속도(vx,vy)=온도색(후속) |
...
| **escaped** | `{E,px,py,count}` (sim 최상위 누적) | **경계 밖으로 떠난 입자 총합 → 탈출 게이지**(L-escape, step-0032~ decay/fuse) — count=개수·(px,py)=net 운동량 방향·E=에너지. 떠난 입자는 장면에 없어 *완전히 안 보였다*(실 스냅샷 감사가 잡은 미독 부채). nuc/lep 장부(추상 태그)와 달리 *실제 방출 입자*(광자처럼 방향·개수 있는 방출량) → HUD 오버레이로 집계 표시(장면 공간 글리프 아님). count=0/없음이면 0 |
...
| **L-element** | ✅ 렌즈-011 | 원자 양성자 수 `Z`(원소 정체성) → 구의 색조(측정 Z 범위 정규화·저 Z 파랑→고 Z 빨강·연속 사상) — 원소 바뀌면 색 이동 | ✅ 충족 — Z 는 atoms 채널에 늘 실림; 핵 변환(step-0031~38 decay·fuse)이 Z 를 바꿈 → 색으로 보임. 단일 원소(범위 0)는 무채색 |
| **L-ion** | ✅ 렌즈-012 | 전하 `Z−e`(이온화 상태) → 구 테두리 고리(부호 발산 — 양이온 따뜻·음이온 차가움·중성 고리 0·세기=\|Q\|/maxQ 측정) | ✅ 충족 — e·Z 는 atoms 채널에 늘 실림(계약 감사서 드러난 미독 채널). step-0001 이온·0010 이온결합·0013~14 이온화 |
| **L-isotope** | ✅ 렌즈-013 | 중성자 수 `N`(동위원소) → 안쪽 동심 코어 밝기(측정 N 범위 정규화 — 중성자↑ 밝은 코어·단일 동위원소 코어 0) | ✅ 충족 — N 은 atoms 채널에 늘 실림(계약 감사 미독 채널); 크기(Z+N)와 직교한 동심 코어로 같은 Z 다른 N 구분. step-0035 사슬(A=22·N 13~16) |
| **L-escape** | ✅ 렌즈-018 | 탈출 누적 `sim.escaped`({E,px,py,count}) → 좌상단 HUD 게이지(화살=net 운동량 방향 atan2·등방 고리=복사 탈출 \|p\|=0·개수·E 텍스트, 스칼라라 크기 정규화 없이 방향·개수·E 정확 읽기) | ✅ 충족 — escaped 는 핵 장면(step-0032~38)서 늘 실림; **실 스냅샷 재감사가 잡은 미독 부채**(문서 계약에 없던 양). nuc/lep 장부와 달리 실제 방출 입자 → 광자 스펙트럼 띠처럼 집계 오버레이. count=0 면 게이지 0 |
...
| **L-voxel** | ⛔ blocked | **cell(공간 격자) → 복셀 큐브 밭 = 3D CA 외형**: 색조=q밴드 `c0`·불투명도=국소 에너지/\|q편차\|·평형 cell 투명·들뜬 cell 빛나는 큐브(원자=큐브 덩이·고분자=큐브 사슬). 현 L-3d 음영 구(球) 글리프 대신 큐브 | **flux 트랙 선행**(atom 아님) — flux 가 *관성 파동*을 내며 `v`·국소 에너지를 cell 에 실은 뒤(flux arc F~·실 스냅샷 감사로 확인). 그 전엔 기존 L-glow(q→밝기)+L-velocity(v→자취)로 파동이 이미 보임(render 변경 0) — 복셀은 "움직이는 화면이 생긴 뒤"(§5 화면이 권위) |
