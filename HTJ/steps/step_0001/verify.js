// step_0001/verify.js — 기반 무대(셀 3D 공간)의 *수치 검증*. 순수·독립·영구.
//
...
// ── 1. 격자 크기 + 인덱싱 전단사 ──
{
  ...
  check('격자 = N³ 셀', w.cells.length === N * N * N, `${w.cells.length} == ${N ** 3}`);
...
// ── 2. 결정론: 같은 (N, seed) → 같은 지문 ──
{
  ...
  check('결정론 — 같은 시드 → 동일 지문', a.fingerprint() === b.fingerprint(),
    `0x${a.fingerprint().toString(16)} == 0x${b.fingerprint().toString(16)}`);
  // get/set 일관성: 임의 셀을 직접 set 한 뒤 get 일치
  const w = W.createWorld(16); w.set(3, 5, 9, 7);
  check('get/set 왕복 일치', w.get(3, 5, 9) === 7 && w.cells[w.index(3, 5, 9)] === 7, w.get(3, 5, 9));
}
...
// ── 3. 데모 공의 점유 셀 수 = 정확한 기대값(회귀 가드) ──
{
  ...
  const vals = new Set(); for (const v of w.cells) if (v) vals.add(v);
...
// ── 4. clear 후 빈 공간 ──
{
  ...
  w.clear();
  check('clear → 점유 0', w.count() === 0, w.count());
}

// ── 결과 ──
console.log('\n=== step_0001 수치 검증: 기반 무대(셀 3D 공간) ===');
