...
큰 목표 = *실제로 게임을 시뮬레이션할 수 있는 거대한 세계*. 한 번에 못 닿으므로 단기 **「목표 세계」(중간 목표)** 를 정해 그쪽으로 step 을 쌓고, 도달하면 사용자가 확인한다. **지금 어느 중간 목표를 향하는지·진행 순서는 항상 [STATE.md](STATE.md) 「나아갈 방향」이 권위** — 모든 step 은 그 방향을 따른다(임의 방향 금지). *중간 목표의 상세는 그때그때 STATE·design 에 두고 달성되면 사라진다 — 여기엔 남기지 않는다.*
...
| `design/` | **설계 문서** — 닫은 step 아닌 *앞으로의 계획*(중간 목표 상세 포함·달성되면 사라짐). 후속 step 이 참조해 구현한다. | 비정기 |
