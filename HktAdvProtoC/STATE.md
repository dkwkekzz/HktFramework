...
**플레이 관찰 (2026-07-30) — 샘플 데이터로 실제 플레이해 보고 "MMORPG 로 보이는가"를 17항으로 판정: 12/17.**
문법은 대부분 실물이다(캐릭터 조작·행동바 9.2종·타게팅→컨텍스트 행동·게이지·5일차 성장「정지의 손 ← 하루 세 대상」·월드 이벤트 9건·8채널 신호 660건·게이트 이동).
**보이는 모습은 아직 MMORPG 가 아니다** — ✗ 5항의 뿌리는 넷이다: ① 행동에 의한 이동이 순간이동이라 **아무도 걷지 않는다**(위치 표본 2종·이웃 이동 표본의 1%) ② 지역 이동이 행동 한 번에 **조용히 취소**되고 HUD 에 이동 표시가 없다 ③ 플레이 카메라가 지역 종횡비를 버려 화면이 세로 띠 + 좌우 공백 ④ 배속 표시 불일치(`set_speed` 미전송). 여기에 표본의 한계 둘 — 마을 상주 4~5명이라 **시야 평균 1.8명**, 행동 하나가 30~240분이라 템포가 관리 시뮬 쪽.
재현: `npm run play -- --days=5`(타임라인 + 결산 + 17항 대조, 5초) · `npm run playtest`(브라우저 조작 + 스크린샷 10장). 코어 무변경 — verify **95/95** · test **259/259**.
상세·근거·후속 순서(D-1~D-4 · S-1~S-2): [design/impl/Review-PlayObservation.md](design/impl/Review-PlayObservation.md).
...
- [x] 플레이 관찰 — 샘플 데이터로 플레이해 MMORPG 문법 17항 대조(12/17) + 결함 4건·표본 한계 2건 특정 ([Review-PlayObservation.md](design/impl/Review-PlayObservation.md))
- [ ] D-1 이동 연속화 — 행동에 의한 이동(NPC 포함)을 tick 체인으로, §35 기준선 재고정 ([근거](design/impl/Review-PlayObservation.md))
- [ ] D-2 지역 이동을 행동 취소에서 분리 + 이동 진행 HUD(남은 시간·목표) + 이동 중 행동바 비활성
- [ ] D-4 플레이 카메라를 지역 실좌표(worldSize)로 투영 + 모바일 세로 HUD 배치 · D-3 `set_speed` 전송 · V-3 플레이 렌더러 LabelLayout
- [ ] S-1 표본 세계 인구 증원(마을 상주 4~5 → 12~15) 후 같은 관측 재실행 — "붐빔" ✗→✓ 확인
