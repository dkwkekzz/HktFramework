...
§3 의 일곱 모듈이 두 국면으로 갈라졌다: 모듈 1~6(Seed→Compiler→Validator→Bootstrapper→사전 실행→Interpreter)은 **정적 파이프라인**(`WorldPackager`)이 되어 단계별 처리 보고(ok+수치)를 남기며 패키지로 구워지고, 모듈 7(플레이)은 그 패키지의 4단계 스냅샷을 **상태 해시 동일**하게 그대로 복원한다(재가공 0 — verify 가 해시로 판정). 패키지는 스튜디오 "플레이 패키지로 보관" → WorldLibrary(localStorage) → 플레이 세계 목록으로 흐른다. 플레이어 검은 화면은 진입 절차(세계 선택→주체 선택→attach 후에만 렌더)로 구조적으로 해소 — smoke 가 플레이 캔버스의 배경 외 픽셀을 직접 센다. 조작은 일반 MMORPG 문법: PC WASD/클릭 이동·대상 클릭, 모바일 가상 조이스틱·탭, 게이트로 지역 이동(§13 canCross), 자동 tick(배속·일시정지). 코어에는 연속 이동 층(`PlayerMovement` — 달리기 2/tick, 1 tick 체인 이벤트, 이동이 행동을 취소)이 §26 이벤트 루프 규약대로 얹혔고 §35 기준선은 무변경(NPC 무영향, 30일 회귀 테스트 통과). 상세: [design/impl/Phase-9.md](design/impl/Phase-9.md). 재현: verify **94/94** · test **259/259** · smoke(플레이 여정 포함).
