...
**플레이 모듈 분할 (2026-07-30) — 플레이를 목적이 분명한 11개 모듈로 나누고, 모듈마다 도달 판정을 측정으로 붙였다: 모듈 도달 3/11 · 항목 20/35.**
이제 작업 단위는 "결함 고치기"가 아니라 **한 모듈을 목적에 도달시키기**다. 도달 모듈은 M1 진입·M6 상호작용·M7 동기.
미도달: **M3 이동 2/5**(행동에 의한 이동이 순간이동 · 건너는 중 행동 한 번에 이동이 조용히 취소) · **M4 군중 1/5**(시야 1.8명 · 타인 이동 1% · 마커에 진행률·대상 없음 · 마커 배지가 실제 상태 그대로 — 플레이어=개발자 2/2) · **M11 목적구동 1/3**(목적 트리·§20 활성도 11항은 실물이고 행동을 굴리지만, 마을 지도자 마르 50% · 반향수 어미 42% 는 목적 없이 서 있다) · **M2 아바타 2/4** · **M5 화면 2/3**(village 표시 종횡비 0.48 vs 실제 1.25) · **M9 템포 1/2** · **M8 성장 1/2** · **M10 지속 0/1**.
모듈 지도·"무엇을 하고 있다"의 표현 3겹·탐침 8종·한 바퀴 절차·진행 순서 9바퀴: [design/impl/Play-Modules.md](design/impl/Play-Modules.md). 재현: `npm run play -- --days=5`(5초) · `npm run playtest`(브라우저 눈 검증). 코어 무변경 — verify **95/95** · test **259/259**.
...
- [ ] 2바퀴 **M11** — 목적 후보가 전부 탈락하는 이유 규명·해소(마르 50%·어미 42% → 80%+) → M11 2항
- [ ] 3바퀴 **M4** — 마커에 행동 표시(이름·대상·진행률) + 렌더러 캐스팅 바 + 마커 배지를 관찰 기반으로(속마음 제거) → M4 2항 + 눈 검증
- [ ] 4바퀴 **M3+M2** — `travel` 을 행동 취소에서 분리 + 이동 상태를 표시 속성으로 + 이동 중 행동바 규약 → M3 1항·M2 2항
- [ ] 5바퀴 **M4** — 표본 세계 인구 증원(마을 상주 4~5 → 12~15, `bootstrap.json` 만) → M4 1항
- [ ] 6바퀴 **M5** — 지역 실좌표(worldSize) 투영 + 라벨 배치 + 모바일 세로 HUD → M5 1항 + 눈 검증 2건
- [ ] 7바퀴 **M9** — `cycleSpeed` → `set_speed` 전송 → M9 1항
- [ ] 8바퀴 **M10** — 플레이 진행 저장·이어하기 통로(프로토콜 + WorldLibrary) → M10 1항
- [ ] 9바퀴 **M8** — 성장 빈도(M7 과 함께 관측 후 판단) → M8 1항
