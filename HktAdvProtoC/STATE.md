...
**Phase 8 완료 → 프로토타입 완료(§44 13/13) → 2차 재검증까지 종료.**
증명된 범위에는 경계가 있다: 세계 생성은 **녹화된 그 5문장**에 대해서만 성립한다(포트 뒤에 살아 있는 LLM 어댑터가 없다 — [2차 재검증 F-1](design/impl/Review-DesignValidation-2.md)). 파이프라인의 모양과 검증 체계는 실물이고, "임의의 주제에서 세계가 나온다"는 아직 아니다.
...
- 검증: `cd proto && npm run verify` (Phase 1~8 완료 조건 + §44 게이트 13항 + 재검증 갭 수정 — ✓/✗ 89항) · `npm test` (258) · `npm run build && npm run smoke`(네 화면 브라우저 왕복 + Canvas 픽셀 확인 — smoke 는 `dist/` 를 서빙하므로 build 가 선행되어야 한다) · `npm run review:2`(2차 재검증이 남긴 거리 8종) · `npm run baseline:sim`(§35 기준선 재고정)
...
그 위에서 손댈 것이 남았다면 순서는 2차 재검증이 정해 두었다 — **F-2(빌더 한 줄, 조직이 지도에 오른다) → F-8·F-9(주석·재현 명령) → F-4(배신 기억) → F-5(방어 행동) → F-1 게이트 근거 명시 → F-7 → F-3 → F-6**. 깊이의 상한을 지금 규정하는 것은 §17 제도 · §23 문서 채널 · §16 실패 반동의 대상 세 축이고, 셋 다 새 실행 개념이 필요한 별도 규모다.
