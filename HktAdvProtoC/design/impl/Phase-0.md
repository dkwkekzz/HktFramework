...
1. Vite + TS strict + Vitest 스캐폴드, ESLint(`Math.random`/`Date.now` 코어 금지 규칙).
...
8. 최소 셸 페이지: 시드 입력 → 빈 세계 초기화 → 시간 진행 버튼 → 현재 tick 표시.
...
- patch/저장 포맷 — Phase 8 렌더러와 Phase 6 자동 테스트의 입력.
