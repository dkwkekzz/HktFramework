...
>
> 갱신 이력: G-2(§5) · G-3(§6) · G-1(§7) 수정 완료 — §1 표와 §2 갭 목록에 표시를 달았다. 아래 §0 의 수치는 **검증 시점(2026-07-28)** 의 것이고, 최신 수치는 각 수정 절에 있다.
...
- 후속 작업 우선순위 제안: ~~G-2(사회 행동+신호 발신)~~ → ~~G-3(hiddenPurposes 소비)~~ → ~~G-1(확률 용도 검증)~~ → **G-4/G-5(종족·공간 필드 소비)** → G-10(게이트 항진 항 1줄 수정, 즉효). *G-1~G-3 은 §5~§7 에서 수정 완료.*
...
**잔여**: 규칙이 쓰는 확률은 지금 11지점 전부 `partial_outcome`·`mutation` 두 종에 몰려 있다 — §12 목록을 넘지 않는다는 뜻이지, 세계가 그 둘만 써야 한다는 뜻은 아니다. 또한 `no-cause` 검사는 "조건이 있는가"라는 구조 판정이라 **의미 없는 조건**(항상 참)까지 걸러내지는 못한다.
