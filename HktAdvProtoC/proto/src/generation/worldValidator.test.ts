...
describe("§34 정적 검증", () => {
  it("§34 필수 규칙 10개 + G-1·G-3·G-4·G-5·G-6 이 더한 5종을 각각 독립 검사기로 갖는다", () => {
    expect(SEMANTIC_CODES).toHaveLength(15);
    const report = validateManualWorld();
    expect(report.checks.map((check) => check.code)).toEqual([...SEMANTIC_CODES]);
    // 픽스처도 15개 — 검사기 하나에 위반 세계 하나
  ...
  it("위반 픽스처 15종을 각각 해당 코드의 error 로 검출한다", () => {
