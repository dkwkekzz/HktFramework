/* HWS step-0006 sim-core — 생명에 기초대사비(절대 생존 문턱)를 더하면 자원이 생사를 공간적으로 가른다
 * step-0005(확산·증발·구동·응집·생명·번식·이동) 을 잇는다. 더해진 것은 단 하나: 기초대사비(baseCost).
 *   step-0005 까지 유지비는 생물량 비례뿐(cost = m·mMaint)이라 "살아있음의 최소 비용"이 0 이었다 →
 *   구동된 세계엔 진짜 빈 곳이 없어(배경 E≈0.4~1.0) 정지 생명이 *어디서나* 작게 살아남았다.
 *   step-0006 은 생물량과 무관한 절대 비용 baseCost 를 더한다: cost = m·mMaint + baseCost.
 *     흡수(got)가 baseCost 에 못 미치는 *마른 곳*의 생명은 m 이 매 tick 줄어 굶어 죽는다(절대 문턱).
 *     흡수 >= baseCost 인 *진짜 고임* 위의 생명만 m 을 유지한다 → 자원이 생명을 공간적으로 가른다.
 *   기초대사도 m→소산이므로 metabolized 항에 기록(장부 불변). 가진 것보다 더 소산할 수는 없으므로
 *     cost 를 m 으로 클램프(굶어 죽을 땐 남은 m 을 다 갚고 0 → 사망) → 장부는 step-0005 식 그대로 닫힌다.
 *   순서: 이동(⑤) → 흡수·유지(기초대사 포함)·사망(⑥) → 번식(⑦). step-0005 그대로.
 * 회귀: baseCost=0 이면 cost = m·mMaint + 0 = m·mMaint, 클램프 미발동 → step-0005 와 비트 동일.
 *   이동까지 끄면 step-0004 와, 번식까지 끄면 step-0003 과, 에이전트가 없으면 step-0002 와도 비트 동일(회귀 체인).
 * 닫힌 장부: sumE + M(=Σm) + evaporated + sunk + metabolized - injected = E0 (step-0005 그대로).
 * 브라우저/Node 겸용. step-0006.html 의 인라인 코어는 이 파일과 동일해야 한다.
 */
(function (global) {
  'use strict';
  ...
  var DEFAULTS = {
    ...
    /* ── step-0006 신규: 기초대사비(절대 생존 문턱) ── */
    baseCost: 0            // 생물량과 무관한 절대 대사비. cost = m·mMaint + baseCost.
                           //   0 = off = step-0005 와 비트 동일(회귀). >0 이면 흡수<baseCost 인
                           //   마른 곳의 생명은 굶어 죽는다(자원이 생사를 공간적으로 가름).
  };
  ...
  function createSim(seed, params) {
    ...
    return {
      ...
      /* step-0006(기초대사비)은 새 상태가 없다 — cost 한 항만 더하고 사망은 step-0003 이래
       * 모두 기아사(m<mDeath)다(deaths 가 곧 아사 수). 기초대사 소산은 metabolized 에 누적된다. */
    };
  }
  ...
  function step(sim) {
    ...
    if (p.life && sim.agents.length) {
      ...
      /* ⑥ 생명(흡수·유지·사망) — step-0005 에 기초대사비 한 항을 더한다.
       * 각 에이전트: (a) 입의 E 를 kL 비율 흡수 → m  (b) 대사 cost = m·mMaint + baseCost 소산
      ...
      for (var k = 0; k < ag.length; k++) {
        ...
        a.m += got;
        /* (b) 유지+기초대사: m → metabolized. 절대 비용 baseCost 추가(step-0006).
         *     가진 것보다 더 소산할 수 없다 → cost 를 m 으로 클램프(굶어 죽을 땐 남은 m 다 갚고 0). */
       ...
       * E·M 불변 → 장부는 step-0005 식 그대로 닫힌다. step-0005 그대로. */
      if (p.repro) {
   ...
   * 상대 잔차 = |위반량| / max(1, E0 + injected). 기초대사도 m→소산이라 metabolized 에 들어간다. */
  function ledger(sim) {
    var sumE = 0, E = sim.E;
   ...
   * 개체가 source 한 점에 뭉치면 작고, 자원 분포 위에 흩어지면 크다. ③ 공간 국한/분산 측정용. */
  function spread(sim) {
    var ag = sim.agents;
   ...
   * step-0005 hashState 와 *동일 필드* 만 해시한다 — baseCost=0 이면 step-0005 와 일치(회귀). */
  ...
  /* 기본 장부만 해시 — 회귀 검사(에이전트 0 일 때 step-0005 와 비트 동일)용. */
  ...
  var api = {
    ...
    torusDist: torusDist, centroid: centroid, spread: spread, discCells: discCells,
    setSource: setSource, setSink: setSink,
    hashState: hashState, hashBase: hashBase
  };
  if (typeof module !== 'undefined' && module.exports) module.exports = api;
  else global.HWS6 = api;
