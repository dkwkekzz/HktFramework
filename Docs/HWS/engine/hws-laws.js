...
(function (global) {
  ...
  function ignite(sim) {
    ...
    for (var s = 0; s < stars.length; s++) {
      ...
      var want = starRate * nc, inj = st.fuel < want ? st.fuel : want, per = inj / nc;
      for (var bc = 0; bc < nc; bc++) E[cells[bc]] += per;
      st.fuel -= inj; sim.injected += inj; sim.burned += inj;
      if (st.fuel <= 1e-9) { sim.starDeaths++; continue; }     // 소진 — 꺼짐(비가역)
      /* 서행 — 점화 시 정한 방향으로 주기마다 한 칸(서행 봉우리가 떠돈다 → 생명이 따라온다). 위치만 바꿈(거래 0). */
      st.age++;
      if (st.age % dPer === 0) {
    ...
    if (stars.length < p.starCap) {
      ...
      if (bi >= 0) {
        ...
        stars.push({ x: bx, y: by, center: bi, fuel: p.starFuel0, cells: K.discCells(W, H, bx, by, p.starR),
  ...
  var LAW_ORDER = [diffuse, evaporate, drive, crystallize, ignite, move, crowd, metabolize, reproduce];

  var api = {
    DEFAULTS: DEFAULTS, LAW_ORDER: LAW_ORDER,
    diffuse: diffuse, evaporate: evaporate, drive: drive, crystallize: crystallize,
    ignite: ignite, move: move, crowd: crowd, metabolize: metabolize, reproduce: reproduce
  };
