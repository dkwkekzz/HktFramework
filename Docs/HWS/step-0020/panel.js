/* step-0020 패널 — 공공재 협동(pubgood: kin 끼리 양의 합 시너지(b≫c) → 협동이 *지속*을 넘어 *강하게 침투*, SPINE §다섯째 축 "사회 칸").
 * step-0019 패널을 잇되 새 노브 행 1개(공공재 kPublic·pubSynergy)를 더했다. 시뮬 로직은 engine/hws-laws.js 의 pubgood 법칙(LAW_ORDER ⑥e, share 뒤·생명 앞).
 * 표준 시나리오: step-0019 그대로(별·자기제한·연소 FSM·계량·복제·생명 유전·차등 응집·막 결합·생물량 공유) + 공공재 협동 on(kPublic 0.3·pubSynergy 2.0, 균일 협동). 브라우저: window.HWS_PANEL_0020 */
(function (global) {
  'use strict';
  ...
  var panel = {
    coreGlobal: 'HWS_SIM',
    title: 'HWS step-0020 — <span style="color:#e0c060">공공재</span>: 시너지 협동(pubgood)',
    subtitle: 'step-0019 의 생물량 공유(share)는 kin 끼리 <b>m 을 쌍 거래</b>(보존적 재분배)로 떠받쳐 협동에 차등 적합도를 줬으나 — *보존적*(b≈c+ε)이라 협동이 <b>지속</b>만 했지 배신을 빠르게 *치환*(강한 침투)하진 못했다. 강한 침투는 <b>양의 합</b>(시너지 b≫c)을 요구한다 — <b>한 기부가 여럿을 살리는 공공재</b>. 이 step 은 그 공공재를 더한다: 협동자가 제 잉여를 <b>기부</b>해 4-인접 kin *여럿*에게 나눠 주되 <b>시너지로 증폭</b>(기부 m 분 + 그 (syn−1)배를 <b>필드 E 에서 채집</b> — 공공재가 환경 자원을 푼다). <i>kin 이 뭉칠수록</i> 시너지가 커져 협동이 <b>강하게 침투</b>(positive assortment). <code>node step-0020/verify.js</code> 로 회귀·장부·결정론·invade·sustain 검증.',
    ...
    controls: [
      ...
      { items: [
        { kind: 'check', id: 'publicc', label: '공공재 협동(시너지 b≫c)', def: true, gateFor: 'kPublic', title: '협동자가 제 잉여를 기부해 4-인접 kin *여럿*에게 나눠 주되 *시너지로 증폭*(기부 m + 그 (syn−1)배를 필드 E 에서 채집). 한 기부가 여럿을 살려(양의 합) 협동이 *강하게 침투*. 0 = step-0019(공유는 보존적 — 지속만).' },
        { kind: 'slider', id: 'kPublic', label: 'kPublic', param: 'kPublic', min: 0, max: 1, step: 0.05, def: 0.3, fixed: 2, gateBy: 'publicc', gateOff: 0, title: '공공재 기부 강도(0~1). 협동자가 제 잉여의 이 비율을 kin 에게 기부(시너지 증폭). 전체 스택은 균일 협동(coopFit0=1).' },
        { kind: 'slider', id: 'pubSynergy', label: '시너지 b/c', param: 'pubSynergy', min: 1, max: 4, step: 0.1, def: 2.0, fixed: 1, title: '시너지 배수(>1 이라야 양의 합). 한 기부 c 가 집단에 b=syn·c. =1 이면 보존적(침투 없음), >1 이면 강한 침투.' }
      ]},
      { items: [
      ...
      { items: [
        { kind: 'check', id: 'inheritc', label: '생명 유전(genotype↔대사)', def: true, gateFor: 'kInherit', title: '생명이 R-genotype 을 부트스트랩·분열 상속(±변이)하고, fit(태그)→대사세로 선택된다(step-0016). kin 을 정의해 응집·막·공유·공공재의 토대.' },
    ...
    drawHook: function (ctx, c) {
      ...
      /* 개체 윤곽 — 같은 유전형 4-인접 생명 쌍을 선으로 잇는다(연결 성분 = 표면장력 액적 = flux 결합 도메인). 이 선 안에서 kin 끼리 m·E 를 공유하고 공공재를 채집한다. */
    ...
    stats: [
      ...
      { label: '누적 출생 / 사망 / m공유 / <span style="color:#e0c060">공공재</span>', get: function (c) { return c.sim.births + ' / ' + c.sim.deaths + ' / ' + (c.sim.shared || 0).toFixed(0) + ' / ' + (c.sim.pubgood || 0).toFixed(0); } },
    ...
    legend:
      ...
      '<b>공공재 협동(시너지 b≫c)</b>: step-0019 공유는 *보존적*(b≈c)이라 협동이 *지속*만 했다. 이 step 은 협동자가 제 잉여를 <b>기부</b>해 kin *여럿*에게 나눠 주되 <b>시너지로 증폭</b>(기부 m + 그 (syn−1)배를 <b>필드 E 에서 채집</b> — 한 기부가 여럿을 살린다). 우측 "공공재" 가 누적 이득. "시너지 b/c" 를 올리면 침투가 강해진다.<br>' +
      '<b>positive assortment (rb≫c)</b>: 공공재는 *kin 이 뭉칠수록* 강하게 침투한다 — <code>verify invade</code>: 뭉치면 협동 *강하게 침투*(Δ+0.30, 0019 의 지속 ≈0 을 넘어섬)·흩어지면 ~중립(Δ+0.05, 침투 무뎌짐), 상호작용 +0.25. 시너지는 *필드 E 채집*이지 질량 창조 아님 — 기부 m→m·시너지 E→m(둘 다 보존)이라 churn 을 안 깬다(<code>verify sustain</code>·잔차 &lt;1e-6 <span class="pass">PASS</span>). "공공재 협동" 체크를 끄면 step-0019(공유는 보존적)로.',
    ...
    actions: {
      seedSortMix: function (api) {
        ...
        api.toast('혼합 군집 ' + n + '개체 — 차등 응집이 같은 색끼리 액적으로 정렬하고, 그 안에서 공공재(시너지)를 채집한다');
      },
    ...
    clickModes: {
      lifegene: function (api, cx, cy) { var tag = 1 + ((api.sim.tick + cx + cy) % api.sim.p.geneTypes), a = api.core.spawnAgent(api.sim, cx, cy); a.g = tag; api.toast('유전 생명 (' + cx + ',' + cy + ') tag ' + tag + ' — kin 끼리 응집해 개체로, 그 안에서 공공재 채집(시너지)'); },
  ...
  else global.HWS_PANEL_0020 = panel;
