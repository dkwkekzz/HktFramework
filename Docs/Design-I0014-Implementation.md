...
| **인게임 검증 콘텐츠** — Phase B 베이크 자동 채움 → 런타임 read-only spawn 의 *최소* 검증 스토리. voxel 한 점 → Entity.Item.Wood (ItemId=101) 1개 drop. cap/region 메모리/lineage 미사용 — 자연 발생 경로의 vanilla path 검증. `bake_terrain.py` 가 `VoxelTypeSpawnTemplate` 기본 매핑 (Sand/Snow/Gravel/Clay → MaterialNode) 을 적용하여 재베이크 시 attribution 자동 산출. Grass/Dirt 흔한 surface 는 의도적으로 비워 둠 (재료 폭주 방지). | `Content/Stories/Natural/Material/MaterialNode_Spawn.json` + `.spec.json` · `HktCoreTags.h/.cpp` (`HktNaturalStoryTags::MaterialNodeSpawn`) · `Content/Python/bake_terrain.py` (`default_spawn_template_mapping`, `apply_spawn_template_mapping`, `--no-spawn-templates` 옵션) |
