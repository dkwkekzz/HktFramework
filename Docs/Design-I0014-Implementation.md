...
| **Phase A — Voxel attribution 인프라 (I-0014)** — baked chunk `SpawnTemplateAttribution` sparse 슬롯 (TMap<uint16 packedLocalCoord, uint16 templateId>) + baked asset `SpawnTemplateCatalog` (TMap<uint16, FGameplayTag>) + BakeVersion v4→v5. `IHktTerrainDataSource::GetChunkVoxelAttribution` + `FHktVoxelAttributionView` POD. `FHktTerrainProvider` 가 카탈로그 해석. `FHktTerrainSystem::Process` 의 voxel 평가 패스 — voxel 별로 *template StoryTag* 를 직접 EventTag 로 dispatch. `VoxelTemplateParams::` 별칭 + `HktEventBuilder::VoxelTemplateActivated` 헬퍼 + voxel 좌표 일원 시드 (I-0017 사전 적용). | `HktTerrainBakedAsset.h` (v5) · `HktTerrainBakedAsset.cpp` (`FindVoxelAttribution`) · `HktTerrainDataSource.h` (`FHktVoxelAttributionView`, `GetChunkVoxelAttribution`) · `HktTerrainProvider.h/.cpp` (카탈로그 해석 + WARN 게이트) · `HktStoryEventParams.h` (`VoxelTemplateParams::`, `HktEventBuilder::VoxelTemplateActivated`) · `HktSimulationSystems.h/.cpp` (`ScratchVoxelAttributions` + 평가 패스) |
| **Phase B — Bake-시점 자동 채움 + 호환 어댑터 제거 (I-0014)** — `FHktTerrainBakedConfig::VoxelTypeSpawnTemplate` (TMap<int32 VoxelTypeID, FGameplayTag>) 디자이너 매핑. `BakeRegion` 이 매 surface chunk 의 32×32 column 을 순회 → top-most non-air voxel 의 TypeID 를 매핑 조회 → catalog (templateId 자동 부여) + attribution 자동 산출. 런타임은 baked attribution 만 읽음 (SetVoxelTemplate opcode·런타임 placement story·overlay 모두 도입 안 함). Phase A 의 chunk-level `ChunkLoaded` fallback 어댑터 제거 — voxel attribution 이 단일 진입점. `PlacementStoryTag` deprecated 마킹 (다음 BakeVersion 증분에서 제거). | `HktTerrainBakedAsset.h` (`VoxelTypeSpawnTemplate` 필드 + `PlacementStoryTag` deprecated) · `HktTerrainBakeLibrary.cpp` (surface column scan + catalog/attribution 산출) · `HktSimulationSystems.cpp` (ChunkLoaded fallback 제거) |
...
| ~~**Placement Story 패턴 마이그레이션**~~ | placement 는 voxel 에 attribution 을 *기록* 만 한다 — 즉시 실행 금지. | **완료 (Phase B 변형)** — placement story 자체를 폐지. BakeRegion 의 voxel type → template 매핑이 attribution 을 *bake 시점* 에 산출, 런타임은 읽기 전용. `SetVoxelTemplate` opcode 미도입 (트리거 경로용으로 Phase C 에서 도입 가능). `Placement_TranquilWilds.json` 은 listen 대상 없음 — 다음 cleanup PR 에서 삭제 후보. | ✅ |
...
- ~~**A. Voxel attribution 인프라**~~ — per-surface-voxel `SpawnTemplateId` 슬롯 + template catalog + voxel 평가 패스. BakeVersion v4→v5. 기존 Oak/Birch 동작은 어댑터로 호환 유지. **✅ 완료**
- ~~**B. Placement Story 마이그레이션**~~ — *변형 완료*: 디자이너 의도 ("voxel type 별로 BakeAsset 에 물려놓고 런타임은 읽기만") 를 채택하여 **bake-시점 자동 채움** 으로 일원화. `VoxelTypeSpawnTemplate` 매핑 + `BakeRegion` surface column scan 으로 attribution 자동 산출. 런타임 `SetVoxelTemplate` opcode / placement story 신 형식 / overlay 미도입. Phase A 의 chunk-level ChunkLoaded 호환 어댑터 제거. **✅ 완료**
