...
- **Region 메모리 미드조인 동기화** — Placement Story 는 서버 권위로 실행되어 entity 는 WorldView 로 클라에 도달하지만, VM 내 `RegionMapRead/Write` 누적 상태(lineage, 영역별 spawn 한도 등) 가 WorldView replicate 범위에 포함되는지 점검 필요. 누락 시 미드조인 클라는 region 상태를 못 보고 후속 placement (만약 클라가 본인 권위로 무언가 dispatch 한다면) 결정론 불일치. 별도 의도(I-0015 예정) 로 분리 가능.
- **PlacementStoryTag bake 시점 매칭 검증** — 베이크 단계에서 `PlacementStoryTag` 에 해당하는 Placement_*.json 이 실제로 존재/유효한지 정적 검사. 미스 시 베이크 실패 또는 경고.
- **`placement-design` 스킬** (`HktGameplayGenerator/Skills/`) — feature_design 의 `map_requirements.natural_entities` → biome 분기 자동 emit. World 단위 PlacementStoryTag 자동 결정.
