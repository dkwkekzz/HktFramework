...
| `lone_peak` | 0.012 | Grassland, Forest, RockyMountain | Pangea, Plateau | asymmetric_dome stamp | r=12, h=36 | 홀로 선 대형 돔 봉우리 — 한쪽은 절벽, 다른쪽은 완만. 스카이라인 랜드마크 |
| `lake` | 0.020 | Grassland, Forest, Savanna, Swamp | Pangea, Plateau | bowl_with_water (paraboloid carve + water fill) | r=10, depth=6 | 정적 호수. 수면은 주변 지표면 높이 기준 |
...
- `bowl_with_water(center, r, depth)` — 파라볼라 carve 후 원래 지표 높이까지 Water로 채움 (Lake 전용 복합 프리미티브)
...
- `asymmetric_dome(center, r, h, tilt_x, tilt_y, voxel_type)` — 반구 프로파일 `r·sqrt(1-t²)` + 상단이 `tilt` 방향으로 `tilt_strength·r`만큼 이동해 한쪽은 절벽, 다른쪽은 완만한 돔 (LonePeak 전용)
...
13. **Step 13:** Layer 4 나머지 랜드마크 + 영향 반경 기반 N-청크 확장 + 강(river)
    - **Step 13.1 (2026-04-20 추가):** `lone_peak`, `lake` 랜드마크 + `asymmetric_dome`, `bowl_with_water` 프리미티브. MVP는 단일 청크 내 배치(N-청크 확장은 추후) — 반경 r ≤ 12로 제약해 청크 경계 절단 최소화.
...
| 랜드마크 확률 총합 | Step 13 | 육지 청크당 4~8 청크당 1개 밀도 목표로 카탈로그 전체 확률 조정. **2026-04-20 현황:** lone_peak(0.012) + lake(0.020) 추가로 Grassland 청크 총 확률 ≈ 0.06 → 약 17청크당 1개 (목표 대비 여전히 희박, 추후 상향 검토) |
| lone_peak/lake 반경 상향 시 N-청크 확장 | Step 13.1 이후 | 현재 r ≤ 12로 단일 청크 내 완결. 더 큰 봉우리/호수를 원하면 10.3절의 N-청크 확장을 먼저 구현해야 함 |
