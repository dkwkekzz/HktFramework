/* ============================================================
 * step-0007 sim-core : step-0006 코어를 그대로 잇는다
 * step-0006 까지: 점화(ignite) = 내생적 발산을 *사건화* → freeze 해소. 그러나 별의 ejecta·빛이
 *   4-이웃에만 분배되어 별이 한 자리에 고정 → 사슬(별→생명→거름→새 별)이 *공간적으로* 안 돈다.
 * step-0007 이 더하는 단 하나 — 별의 *분배 반경* 노브 (ejectRadius / igniteRadius):
 *   기존 ignite 법칙의 분배 폭을 r 디스크로 확장 (새 법칙 아님, *계수 확장* — CLAUDE.md 규약).
 *   기본값 0 일 때 step-0006 4-이웃 분배와 *비트 동일* → 회귀 0 보장.
 *   r>0 일 때 별이 ejecta·빛을 r 셀까지 흩어, 빛 닿는 곳에서 attract+eff 가 흩어진 ejecta 를
 *   *다시 응결* 시켜 *다음 별의 자리*를 옮긴다 — source→별→생명→거름→새 별 의 자가-재생산 사이클.
 *   장부는 step-0006 과 동일: rho 는 별 안에서 닫히고(burnedTick = 질량결손), light 은 injectedTick 에 기록.
 * ============================================================ */
(function (global) {
  'use strict';
