/* ============================================================
 * step-0008 sim-core : step-0007 코어를 *법칙 변경 0* 으로 잇는다 (새 법칙·계수 0)
 * step-0007 이 답한 것: ignite 의 분배 반경(ejectRadius) 을 r 디스크로 키우면 별이 옮겨다니며
 *   source 한 점만으로 자가-재생산 사슬(별→생명→거름→새 별)이 굴러간다.
 * step-0008 의 질문은 *법칙*이 아니라 *시나리오* 다 — 두 사슬의 충돌:
 *   두 source 를 거리 dist 만큼 떨어뜨려 각자 사슬을 만들면, 두 사슬은 경계(cx)에서 어떻게 만나는가?
 *   가까우면 경계가 *공유 별*로 타오르고(생명 억제), 멀면 두 사슬이 *함께* 경계에 *자식 사회*를
 *   응결시킨다(어느 한쪽만으론 안 생기는 — step-0004 ③ 의 부정 결과를 사슬이 양으로 뒤집는다).
 *   결코 밀어내지 않는다 (배제는 press=1-rho/rhoMax 뿐 — 쌓임을 막을 뿐 사슬 몸을 밀지 못함).
 * 그러므로 코어는 step-0007 과 *비트 동일* (verify reg 로 증명). 더해진 것은 tick 루프를 건드리지 않는
 *   순수 *시나리오 헬퍼* 둘 (singleChainScenario / twoChainScenario) — HTML·verify 공용, 회귀 0.
 * ============================================================ */
(function (global) {
  'use strict';
   ...
   * ★ step-0007: 분배 셀이 *반경 노브*로 확장된다. ejectRadius=0 (기본) 일 때 4-이웃 폴백 →
   *   step-0006 과 비트 동일. ejectRadius>0 일 때 ejecta 를 r 셀 디스크(중앙 제외)에 균일 분배,
   *   별이 그 반경으로 *흩뿌리고 자리를 옮긴다*. igniteRadius 가 별도로 설정되지 않으면 ejectRadius
   *   와 같은 분배 셀을 빛에도 쓴다(통일). */
  function ignite(sim) {
   ...
   * 좌표는 step-0004 ③ 의 두-사회 표준(좌 16,32 · 우 48,32 · sink 32,8)과 통일 — 같은 자리에서
   * step-0004 의 "거리로 인과 안 닿음(부정)" 을 사슬이 양으로 뒤집는지 직접 대조하기 위함. */
  function singleChainScenario(sim, opts) {
    opts = opts || {};
    sim.sources.length = 0; sim.sinks.length = 0;
    const sx = opts.x != null ? opts.x : 16, sy = opts.y != null ? opts.y : 32;
    addSource(sim, sx, sy, opts.r || 2, opts.E != null ? opts.E : 0.030, opts.rho != null ? opts.rho : 0.020);
  ...
  function twoChainScenario(sim, opts) {
    ...
    addSource(sim, lx, sy, r, E, rho);
    addSource(sim, rx, sy, r, E, rho);
    addSink(sim, opts.snkX != null ? opts.snkX : 32, opts.snkY != null ? opts.snkY : 8, 4, 0.10);
