/* ============================================================
 * step-0009 sim-core : 발산의 *내용* 분화 — 별의 종류는 물질이 운반한다 (새 법칙 0, 계수 1 확장 + 트레이서 1)
 * step-0008 이 답한 것: 두 사슬은 경계에서 공유 별(가까움) 또는 공유 자식 사회(멀음)로 만나고, 결코 밀어내지 않는다.
 *   그러나 그 별들은 전부 *같은 종류* 였다 — 발산의 내용(빛 E ↔ 열 T 비율)이 하나뿐.
 * step-0009 의 질문: 두 사슬이 *다른 종류*의 별이면 경계의 자식은 무엇이 되는가?
 * 확장 (CLAUDE.md 규약 — 새 법칙이 아니라 기존 법칙의 계수 확장):
 *   1) 계수 igniteHeat(점화 발산 중 열 비율)가 *물질의 성분*으로 일반화된다. 새 양 rhoB = "B형 물질 질량"
 *      이 rho 의 모든 이동(확산·응집·발화 흩어짐·점화 방출)에 *비례 동승*하는 순수 트레이서.
 *      점화하는 셀의 성분 φ = rhoB/rho 가 열비를 보간한다: heatEff = igniteHeat·(1-φ) + igniteHeatB·φ.
 *      φ=0(A형) = 빛별(기존 그대로), φ=1(B형) = 열별. 별의 종류는 *그 별을 이룬 물질*이 결정하고,
 *      별이 방출한 물질이 다음 별로 성분을 *유전*시킨다 — 종류가 사슬을 따라 흐른다.
 *   2) E+T 장부는 불변 — 분화는 발산의 *분배*(T↔E 비율)만 바꾸고 발산 총량(light)은 그대로.
 * 회귀: rhoB=null(기본, enableComp 안 부름)이면 추가 연산 0 → step-0008 과 비트 동일 (verify reg).
 *   또한 comp on 이어도 igniteHeatB=null(기본)이면 heatEff=igniteHeat 로 동역학 불변 —
 *   트레이서는 *측정*이고 동역학을 바꾸는 것은 오직 계수(igniteHeatB)다 (verify reg0).
 * ============================================================ */
(function (global) {
  'use strict';
