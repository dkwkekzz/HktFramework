/* ============================================================
 * step-0006 sim-core : step-0005 코어를 그대로 잇는다
 * step-0005 까지: 행동(wE) + 재귀 2층(승급) + 하향 A-편향(wBack) + 하향 에너지 채널(wDown).
 * step-0006 가 더하는 단 하나 — 내생적 발산 = 점화(ignite), "별을 본뜬다":
 *   물질(rho)이 점화 임계 thIgn 을 넘으면 그 일부를 태워 *에너지를 발산*하고(별빛),
 *   태운 물질의 대부분을 이웃으로 *방출*한다(별의 물질 방출 → 새 별의 씨앗).
 *   - threshold(E 과잉 → 폭발, 물질을 흩음)의 정확한 거울: rho 과잉 → 점화, 에너지를 냄.
 *   - 상수 source(외부 수도꼭지)는 정착하면 고정점 = 세계가 멈춘다. 점화는 발산을 *사건*으로
 *     만들어(쌓임→점화→소진→암흑→재축적 = 이완 진동자) 세계를 영구히 비평형으로 둔다.
 *   - 물질 방출(eject)이 흩은 rho 는 다른 곳에서 다시 모여 새 별이 된다 = 물질 순환 = 무한한 세계.
 * 새 법칙이 아니다. 점화는 임계 법칙([03-4] threshold)의 거울 연산자 하나(ignite)이며,
 * thIgn=Infinity(기본)이면 어떤 셀도 점화하지 않아 step-0005 와 비트 단위로 동일하다.
 *   에너지 발산분(별빛)은 미시 장부(injectedTick)에 기록 — massToE 는 이득 계수(승급 cE·하향 wDown 과 같은 패턴).
 *   물질은 닫힌 양: rho[i] 감소분 = 이웃으로 방출(eject) + 에너지가 된 질량결손(burnedTick 으로 회계).
 * ============================================================ */
(function (global) {
  'use strict';
