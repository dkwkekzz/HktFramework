...
import { labModule, runK1Scenarios } from '../../lab/index.js';
...
describe('K1 대표 장면', () => {
  it.each(k1Scenarios.map((scenario) => [scenario.id, scenario] as const))(
    '%s — 모든 단정이 통과한다',
    (_id, scenario) => {
      const run = runScenario(scenario, 'K1');
  ...
  it('원문 「24」의 여덟 구획이 모두 채워진다', () => {
    for (const run of runK1Scenarios(0n)) {
  ...
  it('다시 실행해도 결과가 같다 (GI-12)', () => {
    const first = JSON.stringify(runK1Scenarios(0n));
    for (let run = 0; run < 5; run += 1) expect(JSON.stringify(runK1Scenarios(0n))).toBe(first);
  });
});

describe('K1 계약', () => {
  it('id·name 이 디렉터리와 같고 선행이 원문 「9」와 맞는다', () => {
    expect(contract.id).toBe('K1');
    expect(contract.name).toBe('predicate-query');
    expect(contract.depends_on).toContain('K0');
  });
});
