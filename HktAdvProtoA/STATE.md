...
- [ ] **동기층 구현 (M1~M4)** — [Design-Motive.md](Design-Motive.md). 진단: 세계가 플레이어에게 쓰는 법칙 0개(생존 압력 부재)·목적 하향 노출("교두보" 문제)·정보 루프 부재(묻기 없음)·상호작용 없는 존재(하늘신전·최초의식장·침몰해안·심해). 순서: M1 허기 루프(변수·법칙·행동·중노동 전제·HUD) → M2 인지 축+묻기 행동 6곳+저널 개편(숨김 비노출·motive 사전) → M3 기존 생존 목적 4종 개인 앵커+G4.2 재배선(§7 사슬 스모크) → M4 존재 정당화 백로그(§6). 검증 M1~M5 확장이 완료 조건.
