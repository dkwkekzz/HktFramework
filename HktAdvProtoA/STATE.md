...
- [ ] **직관·욕망 신호 변환층 E1~E4 — 경험 반영 (최우선)** — 2026-07 재진단: D0~D4 는 데이터+감사로 초록이나 **변환 내용이 화면에 0바이트 도달**(반영 단절 4종 — ① conversion·material 카탈로그 클라 미로드 ② 욕망 신호 채널 부재·진입이 결핍 텍스트 우선 ③ 보상 사슬 1층 단절 ④ 조합1·분기1 밀도 미달+도감 부재). E 시리즈 검증은 데이터 아닌 **화면 도달**을 잰다. **E1 신호층 실장**(signal 채널+conversion 렌더+region 기본 진입, V8) → **E2 보상 사슬 연출**(즉시보상→예상밖결과→다음목적 순차 연출) → **E3 가능성 밀도**(성질 계열 표시·교차 조합 ≥3·분기 실험 ≥3) → **E4 도감**(경험 게이트). 로드맵 [Design-Intuition.md](design/Design-Intuition.md) §20 E 시리즈 · 재진단 [progress/intuition.md](progress/intuition.md).
- [~] **직관·욕망 신호 변환층 D0~D4** (핵심 전 단계 완료 — 데이터·감사층) — 2026-07 진단 4단절 해소 + 신호·재료·실패·상황층. **D0 위협 재발화 사이클 + 늦은 진입자 프로브 ✅**(단절1) · **D1 토스트 배선 복구 + 인지 축 4종 구멍 해소 ✅**(단절2·4) · **D2 재료 성질 계열 + 다용도 + 조합 행동 + 감사 ✅** · **D3 실패=사건 결과 분기 + 무효과 실패 0 감사 ✅** · **D4 변환 커버리지 카탈로그(§14 8항 전수) + 저널 재정위(§13) + 최종 감사 ✅**(아래 완료 항목). 잔여 중 표현·경험 항목은 **E 시리즈로 승격**(위), 세계층 잔여만 이곳에: D0 NPC 속도 차(G1.2.6 재개폐 사이클)·클럭 확장. 설계 [Design-Intuition.md](design/Design-Intuition.md) · 진단·구현 [progress/intuition.md](progress/intuition.md).
