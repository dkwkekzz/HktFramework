...
- [ ] **동기층 구현 (M1~M6)** — [Design-Motive.md](Design-Motive.md). 진단: 세계가 플레이어에게 쓰는 법칙 0개(생존 압력 부재)·목적 하향 노출("교두보" 문제)·정보 루프 부재(묻기 없음)·상호작용 없는 존재(하늘신전·최초의식장·침몰해안·심해). 과학 근거(§1): 항상성 추동(미는 결핍)·유인 현저성(당기는 욕망 — 게이지 금지, 단서로)·SDT 세 조건(자율=해결 경로 ≥2·유능=전제 안내·관계=타 주체 반응, 불변 13) — 매슬로 위계 폐기(병렬·상호증폭). 순서: M1 허기 루프 → M2 인지 축+묻기 행동 6곳+저널 개편(숨김 비노출·필요/기회 이원화·motive 사전, 단일해결 감사 연동 M8①) → M3 기존 생존 목적 4종 개인 앵커+G4.2 재배선(§9 사슬 스모크)+감염 경로 → M4 존재 정당화 백로그(§8) → M5 탐욕(교역·사건 3 채굴 기여 — 기존 TODO 기여 전환 편입) → M6 인정·앎·유산(E_플레이어.관계.F_* 신설·편들기·인스펙터 K 연동·연대기 기여 명기). 검증 M1~M8 확장이 완료 조건.
