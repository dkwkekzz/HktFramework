...
- `data/world-state.json` — 상태의 유일 원본. **vars 174 · rules 94 · actions 111 · objectives 37 · clocks 2 · subjects 24**. 다섯 층·축 문법 3종·값 4종·`basis` 필수. 인지 축 16(플레이어 노출 목적).
...
- [x] **E1~E4 경험 반영 — 변환 카탈로그의 화면 도달** ✅ 2026-07 — 재진단(반영 단절 4: 카탈로그 클라 미로드·신호 채널 부재·보상 사슬 단절·밀도 미달) 전부 해소. **E1** signals 채널(원경·잠긴문·미지) 16 전수 + conversion·material 클라 로드 + 저널 욕망대상·수단 + region 기본 진입 + V8. **E2** 착지 파문+즉시보상 토스트 → 완료 시 예상밖결과→다음 욕망 순차 연출 + 신호 강조 + C5(해결 행동 56 전수 매핑). **E3** 교차 계열 조합 2종(관측렌즈: 일식 분기 약점단서 / 성광망토: 폭풍 분기 뇌철 응결) + 감사 상향(조합≥3·교차≥1·분기 실험≥3) + 인스펙터 성질 계열. **E4** 도감 탭(경험 게이트 — 접촉·목격만 기록) + D5 앵커 감사. 검증 11종 + 스모크 3종 실측. rules 90→94·vars 172→174·actions 109→111. → [progress/intuition.md](progress/intuition.md).
