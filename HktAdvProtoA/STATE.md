...
- `data/world-state.json` — 상태의 유일 원본. **vars 81 · rules 61 · actions 22 · objectives 8 · clocks 2**. 다섯 층·축 문법 3종·값 4종·`basis` 필수.
...
- `data/detail-coverage.json` — detail 항목 분류 원장(§9-7). 전 노드 detail **335항목 중 99 분류(29.6%)** — 사건 5종 + 아르카론 보스전 권역 시드. 미분류 236은 `data/coverage-backlog.json`(생성물)에 백로그로 출력.
