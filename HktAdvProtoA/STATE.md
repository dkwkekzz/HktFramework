...
- `data/world-state.json` — 상태의 유일 원본. **vars 166 · rules 83 · actions 107 · objectives 37 · clocks 2 · subjects 24**. 다섯 층·축 문법 3종·값 4종·`basis` 필수.
...
- [~] **직관·욕망 신호 변환층 D0~D4** (진행 중 — D0 핵심 완료) — 2026-07 진단 4단절(세계 자가소진·진입 화면 분리·저널 지시 부재·인지 커버리지 구멍) 해소 트랙. **D0 세계 재장전: 위협 재발화 사이클 + 늦은 진입자 프로브 ✅ 완료**(단절1 해소 — 아래 완료 항목) · 잔여 D0(NPC 속도 차·클럭 확장)은 후속. 다음: D1 신호층(욕망 대상 가시화·진입 화면 접합·world 모드 토스트 버그·인지 축 4종 구멍) → D2 재료=가능성(성질 계열·≥3용도) → D3 실패=사건 → D4 상황 제공(저널 재정위). 설계 [Design-Intuition.md](design/Design-Intuition.md) · 진단·구현 [progress/intuition.md](progress/intuition.md).
