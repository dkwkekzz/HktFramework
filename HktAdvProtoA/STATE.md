...
- `data/world-state.json` — 상태의 유일 원본. **vars 94 · rules 63 · actions 37 · objectives 14 · clocks 2 · subjects 8**. 다섯 층·축 문법 3종·값 4종·`basis` 필수.
...
- `data/detail-coverage.json` — detail 항목 분류 원장(§9-7). 전 노드 detail **363항목 100% 분류(미분류 0)**: 번역 93·서사 85·보류 185. 보류가 곧 콘텐츠 백로그(사유 태그). `--strict-coverage` 는 신규 노드 미분류 회귀 가드.
...
- [~] G3~G9 말단 목적 전개(콘텐츠 트랙) — 말단 목적 objective·행동 미구현(§16 감사·커버리지 보류가 추적). 권역 단위로 §9 절차 적용. **닫힘(사슬 완결 14→16/56)**: ① **G2.3.1 검은 태양병 치료**(행동 2종 → X_검은태양병.강도 0, 혈청 3세력 tug의 상위 생존 목적) · ② **G2.5.2 아르카론 약점 발견**(조사 행동 2종[일식 관측·분화구 조사] → 약점단서 2 → `S_아르카론정체.발견됨`, **끊겼던 보스전 발견 고리 복구**). 잔여는 대부분 필요 요소부터 앵커해야(§16 저연결 67). 상세 [progress/world-laws.md](progress/world-laws.md).
