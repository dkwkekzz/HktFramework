...
- [ ] **트리 복수화** — 방법론 [Design-ObjectiveTree.md](Design-ObjectiveTree.md) **§18 신설**(정본=주체 트리들의 교차·주체 G0 스텁 필수·요소=서로 다른 주체 ≥2 연결·법칙·역사 동급 생성기). 남은 구현: ① validate-graph 에 검증 16(주요 주체 전부 존속 루트 목적 ≥1)·17(신규 요소 연결 목적이 타 주체 ≥2) 감사 추가 ② 무트리 주체 G0 스텁 백로그 — 아래 'NPC 목적 노드 보강' 잔여(교단·대장간·유목민·카르마·늑대 등)와 동일 트랙, 복수화 2 형식(루트+유지 조건 1+위협 1+해결 행동 1)으로.
