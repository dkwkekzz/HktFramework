...
- [x] **진입 접합 + 욕망 신호층 — 플레이어가 세계를 마주하는 방식** ✅ 2026-07 — D0~D4 가 상태·데이터·검증만 바꾸고 표현 층을 미룬 자기 비판에서 출발(첫 진입이 진단 단절2 그대로였음). 엔진 무변경. ① 진입 접합: `game/world.html` 기본 진입을 내 아바타가 선 **지역 씬으로 착지**(`landInRegion()`) → 삼축 HUD·행동 바·토스트가 즉시 보인다. ② 욕망 신호층(§4·정합 2): `world-visual.json` `signals` 4종(원경 실루엣·잠긴 문·미지 — 상태 술어 렌더, 충족 시 사라짐) + `drawSignals()`(지도·씬) + `validate-visual` V8 감사. ③ 저널 '▸ 여러 갈래(N)'. 11종 검증 초록. → [progress/intuition.md](progress/intuition.md).
