...
- [~] **직관·욕망 신호 변환층 D0~D4** (진행 중 — D0·D1·D2·D3 핵심 완료) — 2026-07 진단 4단절 해소 + 신호·재료·실패층. **D0 위협 재발화 사이클 + 늦은 진입자 프로브 ✅**(단절1) · **D1 토스트 배선 복구 + 인지 축 4종 구멍 해소 ✅**(단절2·4) · **D2 재료 성질 계열 + 다용도 + 조합 행동 + 감사 ✅** · **D3 실패=사건 결과 분기 + 무효과 실패 0 감사 ✅**(아래 완료 항목). 잔여 D0(NPC 속도 차·클럭 확장)·잔여 D1(욕망 대상 가시화 신호층·진입 화면 기본 진입)·잔여 D2(핵심 재료 배선 확대·조합 결과 다양화)·잔여 D3(분기 실험 행동 확대)는 후속. 다음: D4 상황 제공(저널 재정위·설명 후행·변환 커버리지). 설계 [Design-Intuition.md](design/Design-Intuition.md) · 진단·구현 [progress/intuition.md](progress/intuition.md).
