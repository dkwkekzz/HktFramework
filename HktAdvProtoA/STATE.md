...
- `data/world-state.json` — 상태의 유일 원본. **vars 102 · rules 63 · actions 44 · objectives 16 · clocks 2 · subjects 8**. 다섯 층·축 문법 3종·값 4종·`basis` 필수.
...
- `data/detail-coverage.json` — detail 항목 분류 원장(§9-7). 전 노드 detail **363항목 100% 분류(미분류 0)**: 번역 96·서사 85·보류 182. 보류가 곧 콘텐츠 백로그(사유 태그). `--strict-coverage` 는 신규 노드 미분류 회귀 가드.
...
- [~] G3~G9 말단 목적 전개(콘텐츠 트랙) — 말단 목적 objective·행동 미구현(§16 감사·커버리지 보류가 추적). 권역 단위로 §9 절차 적용. **닫힘(사슬 완결 14→17/56)**: ① **G2.3.1 검은 태양병 치료**(행동 2종 → X_검은태양병.강도 0, 혈청 3세력 tug의 상위 생존 목적) · ② **G2.5.2 아르카론 약점 발견**(조사 행동 2종 → 약점단서 2 → `S_아르카론정체.발견됨`, **끊겼던 보스전 발견 고리 복구**) · ③ **G1.3.1 영원한 겨울 생존**(②-층: 필요 요소 앵커 — 방한복 제작[속털 채집·봉제법 습득·제작] ∨ 지열 거점 확보 두 경로 → S_영원한겨울.방한 충분) · ④ **G3.2.1 무광의 쐐기 제작**(②-층: 설계 해독·뇌철 채굴·쐐기 단조 → `R_무광의쐐기.존재`, **끊겼던 4 보스 처리 경로[파괴·추방·의식분리·지배]의 무기 전제 복구**). 잔여는 대부분 필요 요소부터 앵커해야(§16 저연결 67). 상세 [progress/world-laws.md](progress/world-laws.md).
