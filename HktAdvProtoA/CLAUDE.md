...
| [Design-Life.md](design/Design-Life.md) | **생명·종족·개체·현상 설계 + 저작 공정 W·S·A·I·V** — 종(種) 층위·종족 목적 트리(활성 조건·반응 트리거·갈등 접점)·개체 프로파일(성장=활성 궤적)·현상 층(방출→관측→믿음→반응). **주체·종족 신설은 공정 S 가 표준 진입점**(ObjectiveTree §19 를 S-7 로 포함). 생명 불변 L1~L5 |
| [Design-Ability.md](design/Design-Ability.md) | **인간류 능력 체계 — 넨(가칭)** — 생명 원장 전용(轉用)·계통 6종(증폭·방사·변성·구성·감응·간섭)·**제약과 서약(희소성 계수로 수량화)**·개성 발현(능력=프로파일 생성물). 능력 저작은 공정 A (Life §8 체계의 일부) |
...
| [data/objective-conversion.json](data/objective-conversion.json) | **D4 변환 커버리지 카탈로그** (Intuition §14·§20 D4·§21⑤ 최종 감사) — 노출(인지) 목적 16 마다 §14 변환 8항(내부목적→욕망대상→작은방해→눈에띄는수단→간단한행동→즉시보상→예상밖결과→다음목적). 저널을 '지시' 아닌 '상황+복수 가능성'으로 |
| [data/validate-conversion.mjs](data/validate-conversion.mjs) | **D4 변환 커버리지 감사** — 노출 목적 100% 8항 완비·해결 경로 ≥2 전수(§13)·참조 무결성. D 시리즈 최종 감사. `node data/validate-conversion.mjs` |
