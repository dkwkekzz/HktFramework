...
- 실사진 평가 1호(`test/golden/references/katana-curved.referenceSpec.json`, 카타나 스톡 사진
  수동 어노테이션): 전수 탐색 IoU **0.586** — 목표 0.9 미달이지만 스코프 결과다.
  ① 곡선 검(sori)은 직선검 MVP 밖(원본 §2 — golden 20종 전부 직선검): 직선 칼날이 휜
  실루엣을 못 따라가고, 폭 ~7텍셀 실루엣에선 그 좌우 드리프트가 IoU 를 크게 깎는다
  (최적화기는 중간 폭을 0.054m 로 부풀려 보상 — 실측 ~0.027m). ② 손잡이도 축(chord)과
  어긋난 방향. 실험: `centerCurve` 중간점 x-오프셋(sori 0.01m — 메시 빌더는 이미 지원,
  §26 벡터에는 없음) 주입 시 0.577 → **0.652**. 곡률·손잡이 방향의 파라미터화가
  Phase 6 확장 후보(곡선 단날검 도메인).
