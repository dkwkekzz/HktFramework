...
여기서 확정된 결정(D-1 ~ D-12, 구현 중 확장 D-13 ~ D-17)이
[02-architecture.md](02-architecture.md) 이후 모든 문서의 근거다.
...
### D-13. 손잡이 단면 일반화 (구현 중 확장 — 2026-07-29)

원본 §8 은 손잡이를 "원형 또는 타원형 단면 Sweep" 으로 한정했다. 칼날 생성기의
"닫힌 프로파일 + crease + 전개 레이아웃" 기계가 이미 임의 단면을 지원하므로, 손잡이가
같은 기계를 재사용해 원/타원/팔각 단면·반지름 곡선·감기 기하 변위를 지원한다.
의미 UV(u=둘레, v=길이)와 Metric UV 정의는 단면과 무관하게 유지 — 검증기·Atlas·베이크
수정 없음. 스윕으로 표현 불가능한 자유 조형(손가락 홈 등)은 여전히 스코프 밖(원본 §35).

### D-14. 산화·오염 반점 확산 항 (구현 중 확장 — 2026-07-29)

원본 §18.3 의 산화/오염 마스크는 cavity·fuller 에만 게이트되어, 홈 없는 매끈한 검에서는
상태값을 아무리 올려도 표면이 변하지 않는다(구현 후 정량 확인: 상태 0→0.9 에 평균색 변화
1/255 미만). 틈새 우선 원칙은 유지하되, **상태가 높을수록 노이즈 문턱이 낮아져 열린 면까지
번지는 반점 확산 항**을 추가한다(원본 §0 의 "반점 분포" 가 AI 마스크 없이 절차 노이즈로
들어온 것). 수식: `spread = clamp01((fbm(metric) - (1 - state)) × k)` — 상태가 낮으면
문턱이 높아 틈새만, 높으면 반점이 면으로 확장. 셰이더 결정성·seed 파생 규약은 동일.

### D-15. 상태 필드는 선택자별로 저장 형태가 다르다 (구현 중 확장 — 2026-07-29)

[06-phase4](06-phase4-surface-state.md) §4.2 는 선택자 3종(edge/ridge/local_uv)을 모두 저해상
필드(256²/부품, uvLocal)에 축적하라고 했다. 그런데 `edge`·`ridge` 는 **공간 경계가 없는**
선택자(부품 전체)이고, `edgeWeight`/`ridgeWeight` 는 단면 프로파일이 명시한 정점 속성이다.
이를 uvLocal 필드에 구우면 날처럼 얇은 특징이 뭉개진다 — 칼날의 날은 uvLocal V 방향으로
프로파일 슬롯 하나 폭(1/n)이라 256² 필드에서 몇 텍셀에 불과하다.

**결정**: `edge`·`ridge` 는 **스칼라**로 누적하고 프래그먼트에서 보간된 의미값과 곱한다.
`local_uv` 만 저해상 필드에 축적한다(경계 falloff 0.05, bilinear 샘플). 06 §4.2 의 목적
— Operation 수가 늘어도 베이크 비용 불변 — 은 그대로다(스칼라도 필드도 픽셀당 O(1)).

### D-16. 긁힘은 사전 스탬프가 아니라 metric 그리드 + 프래그먼트 평가 (구현 중 수정 — 2026-07-29)

06 §4.3 은 "캡슐의 metric AABB → Atlas 픽셀 영역으로 변환(**아일랜드의 metric↔atlas 선형
관계** 이용)" 뒤 높이 필드에 사전 스탬프하라고 했다. 그 선형 관계는 성립하지 않는다:
칼날의 `uvLocal.v` 는 프로파일 인덱스 균등(i/n)인데 `uvMetric.v` 는 누적 호길이이고,
`uvMetric.u` 는 중심곡선 호길이 테이블이며 tip 감쇠로 둘레가 t 에 따라 줄어든다 →
metric→atlas 는 비선형이다. 선형 근사로 AABB 를 잡으면 찍혀야 할 픽셀을 건너뛴다.

**결정**: 캡슐을 **metric 공간 균일 그리드**에 담고 프래그먼트에서 자기 셀의 후보만 평가한다.
06 이 사전 스탬프를 고른 이유(스탬프 수 × 픽셀 수 폭발)는 그리드가 픽셀당 후보 수를 상수로
묶어 사라진다 — 측정: 1024² 5채널, 116 스탬프에서 +0.26초(총 2.8초, 예산 5초). 겹침의
max 누적(순서 무관)과 실측 크기 유지는 06 의 규정 그대로다. 부수 효과로 긁힘이 Atlas 왜곡을
따라 휘어 **실제 표면 위에서 직선**이 된다(선형 근사였다면 Atlas 위에서만 직선).
또 06 은 방향을 "longitudinal = metric U 축" 으로 못박았는데 이는 칼날 전용 전제다
(손잡이·폼멜의 uvMetric 은 [둘레, 길이]) — 방향과 seam 주기는 **부품별 축 의미**로 해석한다
(`PART_METRIC_AXES`).

### D-17. Engrave 프리셋 마스크는 결정적 생성 배열 (구현 중 확장 — 2026-07-29)

06 §4.4 는 "흑백 PNG 를 리포에 동봉 → 로드 후 Float 배열로 변환 → **배열 자체를 저장**해
디코더 편차를 격리" 였다. 프리셋 3종(룬·십자 문장·갈매기 띠)은 선분 몇 개로 그려지므로
PNG 를 거칠 이유가 없다.

**결정**: 프리셋 마스크는 커밋된 **드로잉 코드**(`masks.js` 의 캡슐 SDF stroke)가 만든다 —
이미지 디코딩이 결정 경로에 아예 들어오지 않고, 해상도 변경도 재생성으로 끝난다. 06 의
...
### D-19. 곡선 가드(휨·끝 벌어짐) — 최적화 벡터를 13차원으로 확장 (Phase 6 — 2026-07-30)

D-18 이 칼날·손잡이에 곡선을 넣는 동안 **가드만 Phase 2 의 직선 Extrude 에 남았다**
(`makeGuardOutline` 4종 전부 직선 바, `makeGuardDesign` 에 곡률 인자 없음). 실사진 평가
2호(광폭 아밍소드)에서 사진의 드룹 quillon 을 어떤 템플릿으로도 근사할 수 없어 잔차의
대부분이 가드에서 나오는 것이 확인됐다. 곡선 기계(`core/curve.js`)와 곡선을 쓰는 부품이
이미 있으므로 표현력의 한계가 아니라 **확장 누락**이다.

**결정**: ① 가드 윤곽 4종을 유지한 채 `makeGuardDesign` 에 `droop`(quillon 끝의 축 방향
변위, m — +는 칼날 쪽) 과 `endFlare`(끝 두께 배율, 1 = 없음) 를 추가한다. 변위는 정규
반폭 `s = |x|/w` 의 **2차** — `y += droop·s²`, `z *= 1 + (endFlare-1)·s²`. `s(0)=0` 이라
중앙 접합부가 고정되고 `bladeSocket`·`gripSocket` 이 불변이다. 둘 다 기본값이면 기존과
**비트 동일**(golden 불변). ② 삼각분할·UV 아일랜드 3개·uvLocal 정의는 손대지 않는다 —
변위는 같은 파라미터 공간의 후처리이므로 자동 언랩(원칙 2)을 도입하지 않는다. 노멀은
기존 `recalculateNormals` 가 변위된 기하에서 재계산하고 스무딩 그룹 경계도 그대로다(D-6).
③ **uvMetric 은 보정한다** — 앞/뒷면 u 는 x 축 변위 경로의 실제 호길이, 측면 u 는 변위된
3D 둘레로 계산한다. 보정 없이는 "1단위 = 10cm" 계약이 깨져 긁힘 metric 그리드(D-16)가
가드 끝에서 늘어난다. ④ 원본 §26 최적화 벡터를 `guardDroop` 을 뒤에 붙여 **13차원으로
확장**한다(앞 12차원 인덱스·의미 불변). `endFlare` 는 두께(z)만 바꿔 side 실루엣에
나타나지 않으므로 **최적화 대상에서 제외**하고 수동 슬라이더로만 노출한다.
⑤ quillon **비틀림**(축 회전)은 이 결정에 넣지 않는다 — 노멀·탄젠트·crease 처리가 늘어
별도 Step 이며, 그때까지 표현 불가 형상으로 STATE.md 에 명시한다.
