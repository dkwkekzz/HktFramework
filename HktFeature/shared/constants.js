...
// feature-0020 — 동면 생명체 저해상도 갱신: 관측 밖 야생 군집의 대사 순환을 이 주기마다 군집(거시) 단위로
//   한 번에 굴린다. 시간 등가(D-1) — 갱신량에 이 배수를 곱해 활성 시뮬과 같은 속도로 시간이 흐른다(관측이
//   물리 상수를 바꾸지 않는다). 결정론 시뮬 상수(런타임 튜닝 금지).
export const DORMANT_LOWRES_INTERVAL_TICKS = 32;     // 저해상도 갱신 주기(틱) — 확산 주기의 배수로 큼직하게
...
// --- 제어·욕망 (구 feature-0010(현 0018)) — 플레이어/봇이 하나의 생명체를 제어한다 ---
...
export const DESIRE = {
  ...
  EAT: 'eat',         // 식사 — 결정(밥)을 향하되 날것이면 먼저 요리(변형)한 뒤 먹는다(절차적, 구 feature-0011(현 0018))
...
// --- 욕구 우선순위·감정 (구 feature-0012(현 0018)) — 욕구는 중첩되고 우선순위가 다르다 ---
// "차이는 신호이고, 욕구는 방향이며, 감정은 중요도다." 생명체는 차이를 인지해 욕구를 품고(방향, 구 feature-0010(현 0018)),
//   그 욕구를 절차로 수행한다(구 feature-0011(현 0018)). 이 feature 는 욕구가 **하나가 아니라 여럿 중첩**될 수 있음을 세운다:
...
// 구 feature-0012(현 0018) step2 — 감정은 두 갈래로 우선순위를 키운다:
...
// --- 요리(cook) (구 feature-0011(현 0018)) — 욕구를 상황에 맞게 절차적으로 수행하는 한 단계 ---
...
// --- 제조(craft) (구 feature-0010(현 0018) step2) — 재료 결정을 조합해 새 산물을 만드는 새 욕구 ---
// 구 feature-0010(현 0018) step1 은 채집·사냥 욕구를 세웠다. 이 step 은 **새 욕구(제조)**를 절차 레지스트리(구 feature-0011(현 0018))에
...
// 구 feature-0011(현 0018) step2 — **다단계 제조**: 제조는 한 번으로 끝나지 않는다. 결정에 tier(단계)를 두어 같은 단계 둘을
//   합쳐 한 단계 올린다: 재료(tier0) → 중간물(tier1) → 완성물(tier2). 절차(shared/desires.js)에 단계를 더 얹어
//   (완성 먼저·중간 나중) 상황에 맞게 다단계로 수행한다 — 구 feature-0011(현 0018) 의 "절차 깊이 확장". tier==MAX 는 완성(더 못 만듦).
...
export const CAUSE = {
  ...
  COOK: 'cook',         // 생명체 → 심우주 / 생명체 → 국소장 (요리 = 날것을 먹을 수 있게 변형하는 일, 방출: 열+연기, 구 feature-0011(현 0018))
  CRAFT: 'craft',       // 생명체 → 심우주 / 생명체 → 국소장 / 결정 → 결정 (제조: 두 재료를 산물로 조합, 만드는 일=열+연기 방출, 구 feature-0010(현 0018) step2)
  ...
  DEMATERIALIZE: 'dematerialize', // 결정(+열) → 국소장 (관측 없는 지역의 결정이 에너지로 환원 = 구체화의 역, 구 feature-0017(현 0016 step2). 다시 관측되면 석출로 재구체화)
};
