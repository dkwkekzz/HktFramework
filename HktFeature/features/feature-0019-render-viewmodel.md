...
> 상태: 🟢 검증됨 (step 1~3) · 작업 방식: [CLAUDE.md](../CLAUDE.md)
...
- **지금은 대충, 이후 풍성하게**: step 1 은 이펙트를 데이터로만 세웠고(테스트 검증), **step 2·3 이 렌더러가 그 서술자를
  소비**해 공간 VFX(폭발 충격파·발산 muzzle·연소/용해 퍼프·파괴 스파크·열의 흐름 점)로 그린다 — 대충. 이후 파티클·라이팅
  같은 리치 렌더는 **같은 `effects` 서술자를 다르게 그리기만** 하면 된다(Scene 계약 불변, 렌더러 교체가 seam).
...
| **2** ✅ | 폭발(0013 규칙 D) 공간 VFX — `effects` 의 `explosion` 서술자를 착탄점 팽창 충격파 + 백열 플래시로 렌더 | shot:detonate — 폭발 후 공간이 붉은 열로 참 |
| **3** ✅ | 발산 muzzle·연소/용해 퍼프·파괴 스파크·열의 흐름 점 — 같은 채널 소비(렌더러가 그리는 타입만 수명 부여) | shot:blast — 발산·폭발 VFX |
