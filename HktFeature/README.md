...
```
shared/    결정론 코어 (외부 API 의존 0) — ledger·constants·protocol·field·worldgen …
server/    원장 권위(회계사) — 클램프·FIFO 중재·커밋·방송. 시뮬레이션 없음
client/    세부 시뮬레이터 — 이동 적분·예측·연출·읽기 전용 관측 뷰어
tools/     무특권 봇 — 브라우저 클라와 동일 프로토콜로 시뮬 부하 생성
test/      규칙 회귀 (npm test) — 보존 등 불변식 상시 검증
```
