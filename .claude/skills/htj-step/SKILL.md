...
> **⛔ 방향을 따른다 — 임의로 정하지 않는다.** 무슨 step 을 할지는 **`STATE.md §3 「나아갈 방향」`이 권위**다(큰 목표 = 플레이 가능한 거대 세계 · 중간 목표 = `design/playable-world.md` PW 사다리). step 은 그 방향(현 단계의 디테일 작업 → 그 뒤 PW 마일스톤)을 따라 고른다. 방향이 애매하면 *새 트랙을 지어내지 말고* 사용자와 논의한다. PW 마일스톤 *도달*은 사용자가 직접 모는 **인터랙티브 viewer** 로 확인한다(per-step 갤러리와 별개·아래 §2).
...
- **방향 정합** — 이 step 이 `STATE.md §3 「나아갈 방향」`(현 단계 디테일 작업 → 그 뒤 PW 마일스톤)에 부합하는가. 부합 안 하면 멈추고 논의(임의 방향 금지).
...
- **사용자용 라이브 viewer 갤러리(`viewer.html` STEPS·`check-viewer.js`)는 폐기** — 유지보수가 안 됐다. 새 step 은 `viewer.html` 에 등록하지 않고 갤러리 가드도 돌리지 않는다(시나리오 모듈은 *캡처용*으로만 존재). **사용자용 viewer 는 PW 마일스톤 *도달* 때만 큐레이트해 인터랙티브로 제공**한다([design/playable-world.md](../../../HTJ/design/playable-world.md)) — per-step 이 아니라 목표 세계당 한 번.
...
- `STATE.md` 갱신 — **바뀐 절만 Edit(전체 Write 금지)·각 절은 짧게 유지·닫은 step 산문을 STATE 에 쌓지 말 것**(긴 설명·수치·한계는 `step_NNNN.md`·viewer note 가 집). 절 구성과 갱신 규칙:
  ...
  - **§3 나아갈 방향**: *현 단계(디테일 작업) → 다음 마일스톤(PW)* 만(짧게). **방금 닫은 step 의 writeup 을 여기 적지 않는다** — 그건 §6·step 문서 몫. 방향 자체가 바뀌면(디테일 완료→PW 진입 등) 여기서 교체.
