...
4. STATE.md §1~6 Edit + §7 1줄 append (§7 줄 끝에 `· render:<후보 or 없음(내부)>` — §7 render 부채 1줄)
5. 닫은 step 파일은 이후 불변 (장면은 `scenes.js` 에 동결로 남아 뷰어가 영구 재현)

## 7. 활성 게이트 점검 (휴면 트랙 깨우기) + render 부채 1줄
...
**render 부채 추적(느슨한 결합 — render 가 atom 에 조용히 뒤처지지 않게)**: step 을 닫을 때 묻는다 — *이 step 이 새 **관측 가능 양**(원자·광자의 새 필드, 또는 기존 필드의 새 *변화 양상*)을 더했는가?* 더했으면 §7 INDEX 줄 끝에 `· render:<후보 렌즈명 or 격차>` 한 구절을, 안 더했으면(보존 회계·내부 장부뿐) `· render:없음(내부)` 을 붙인다. 이 한 구절이 render-step 의 "계약 감사" 입력이다(render 가 *읽기만* — atom STATE 를 고치지 않음). *주의*: "위치·질량 채널로 이미 보인다" 같은 *가정*을 적지 마라 — 보인다는 주장은 render 트랙의 **눈 검증**이 정한다(author 금지의 절차판). 예: 0031~38 은 Z(원소)를 바꿨으니 `render:L-element` 였어야 했다(이 구절이 없어 render 가 8 step 뒤처짐 — 이 절차가 그 재발을 막는다).
