...
3. **`shared/` 는 외부 의존 0**: 순수 결정론 로직만. Node/브라우저 API 를 넣는 순간 결정론·재현성이 깨진다.
...
- 리소스 작업 금지 — 맵·캐릭터·비주얼 에셋·렌더 고도화(WebGL·볼류메트릭 등)는 이 트랙 범위 밖(SPINE §0). 기존 Canvas 렌더는 읽기 전용 최소 뷰어로만 유지한다.
