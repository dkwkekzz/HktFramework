...
4. **기록** — `steps/step-NNNN.md` 압축 4절(≤4KB): **⓪ 직관(한눈에)** — 비전문가·비프로그래머도 이해할 구체 예시/비유로 "예전엔 어땠고 이 조각으로 무엇이 달라지나"를 3~6줄(가능하면 캐릭터·상황 서사 한 장면). ① 한 일(어느 파일에 무슨 메커니즘) ② 검증(테스트·시뮬 출력 수치) ③ 불변식 판정 + 정직한 한계. **⓪ 절만 직관 서사를 허용**하고 그 외에는 서사·다음 예고 금지 — *왜*는 SPINE, *다음*은 STATE 소관. (사용자에게 조각을 제안·설명할 때도 같은 직관 예시를 곁들인다.)
