...
import {
  SPAWN_POS, WORLD_SIZE, MAX_SPEED, BEACON_INTERVAL_MS,
...
class Bot {
  ...
  #think() {
    ...
    if (this.mode === 'gather') {
      ...
      if (node) {
        if (dist(this.x, this.y, node.x, node.y) <= GATHER_RANGE * 0.9) {
          this.intent(INTENT.GATHER, { nodeId: node.id });
        } else {
          this.#walkToward(node.x, node.y);
        }
      }
    } else {
      ...
      if (mob) {
        if (dist(this.x, this.y, mob.x, mob.y) <= ATTACK_RANGE * 0.9) {
          if (now - this.lastAttack >= ATTACK_COOLDOWN_MS + 50) {
            this.lastAttack = now;
            this.intent(INTENT.ATTACK, { targetId: mob.id });
          }
        } else {
          this.#walkToward(mob.x, mob.y);
        }
      }
    }

    this.send(MSG.BEACON, { x: Math.round(this.x), y: Math.round(this.y) });
  }
