...
- **닫힌 step**: [step-0010](steps/step-0010.md) — **A6-1 대사**: `server/game.js` tick 에 매 주기 upkeep `player→SINK`(1초/5) 추가 — 생명은 지속 소모하고 채집·전투로 못 채우면 잔고 0 → 아사. 소산 구조의 압력 엔진. 소산분은 A6-0 순환이 SOURCE 로 되돌린다.
- **한 줄 상태**: `npm test` 30/30 OK · 대사 지불·유지 실패=아사(잔고0)·리스폰 SOURCE 인출·총합 10⁹ 불변.

## 2. NEXT — 가설 (다음 조각의 권위는 이 절)

> 🎯 **다음 조각 = A6-2 구조 예치(성장).** A6-0 순환·A6-1 대사가 닫혔다. 다음: 성장을 `player→STRUCT:<id>` 예치로 — 자유 에너지를 잠긴 질서(구조 풀)로 재분배해 최대 용량↑, 예치분 잠금. 사망 시 `STRUCT→드랍/SINK` 환원(창조 아님·총합 불변). 이후 A6-3 스탯(구조의 함수) → A6-4 스킬 → A6-5 아이템. 아크 전체는 SPINE §1 A6.
> (리소스/렌더 방향은 범위 밖 — SPINE §0.)

**SPINE §1 arc A1~A5 는 전부 닫힘**(step-0006). **A6 아크(생명=소산 구조) 진행 중** — A6-0·A6-1 닫힘(step-0009·0010).
...
| 🔴 | A6-2~5 성장 (구조 예치·스탯·스킬·아이템) | 열림 — SPINE §1 A6 |
