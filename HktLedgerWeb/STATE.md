...
> 🎯 **다음 조각은 열림 — 방향 선택 대기.** SPINE §1 arc(A1~A5)·3D 공간·3D 원근 렌더가 전부 닫혔다. 후보:
> ① **WebGL 진짜 3D 렌더** — Canvas 소프트웨어 원근을 GPU(three.js 벤더링 등)로 올려 폴리곤·조명·오클루전. ② **볼류메트릭 파티션** — region/필드 격자를 3D 볼륨으로(수직 분할). ③ **A 잔여 완성도** — 셀 필드 클라 미러화(A1) · 정적 `M:` 완전 대체(A5) · 감사 시드 서버 비밀화(A2).
