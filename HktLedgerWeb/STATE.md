...
- **닫힌 step**: [step-0009](steps/step-0009.md) — **A6-0 태양 순환**: `server/game.js` tick 에 `SINK→SOURCE` 주기 재순환(5초, 닫힌 열역학 루프) 추가 — 소산 에너지가 SOURCE 로 되돌아 세계가 영속. 서버=태양은 생성기가 아니라 순환의 원점. A6(생명=소산 구조) 아크 개시.
- **한 줄 상태**: `npm test` 29/29 OK · 재순환 주기마다 SINK→SOURCE·SINK 유계(최고 5000→0)·SOURCE 정상상태·총합 10⁹ 불변.

## 2. NEXT — 가설 (다음 조각의 권위는 이 절)

> 🎯 **다음 조각 = A6-1 대사(metabolism).** A6-0 태양 순환(닫힌 루프)이 닫혔다. 다음: 생명이 매 틱 upkeep `player→SINK` 로 에너지를 지속 소모하고, 채우지 못하면(잔고 0) 죽는다 — "소모·갈구·유지 못하면 죽음" 의 압력 엔진. 이후 A6-2 구조 예치(성장) → A6-3 스탯 → A6-4 스킬 → A6-5 아이템. 아크 전체는 SPINE §1 A6.
> (리소스/렌더 방향은 범위 밖 — SPINE §0.)

**SPINE §1 arc A1~A5 는 전부 닫힘**(step-0006). **A6 아크(생명=소산 구조) 진행 중** — A6-0 닫힘(step-0009).
...
| 🔴 | A6-1~5 성장 (대사·구조 예치·스탯·스킬·아이템) | 열림 — SPINE §1 A6 |
